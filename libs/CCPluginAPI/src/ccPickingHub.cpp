...
ccPickingHub::ccPickingHub(ccMainAppInterface *app,
                           QObject *parent /*=nullptr*/)
    : QObject(parent), m_app(app), m_activeGLWindow(nullptr),
      m_pickingMode(ccGLWindowInterface::POINT_OR_TRIANGLE_PICKING),
      m_autoEnableOnActivatedWindow(true), m_exclusive(false) {}

// void ccPickingHub::setPickingMode(ccGLWindowInterface::PICKING_MODE mode,
// bool autoEnableOnActivatedWindow/*=true*/)
//{
//	m_pickingMode = mode;
...
void ccPickingHub::togglePickingMode(bool state) {
  // ccLog::Warning(QString("Toggle picking mode: ") + (state ? "ON" : "OFF") +
  // " --> " + (m_activeGLWindow ? QString("View ") +
  // QString::number(m_activeGLWindow->getUniqueID()) : QString("no view")));
  if (m_activeGLWindow) {
    m_activeGLWindow->setPickingMode(
        state ? m_pickingMode : ccGLWindowInterface::DEFAULT_PICKING);
  }
}

void ccPickingHub::onActiveWindowChanged(QMdiSubWindow *mdiSubWindow) {
  ccGLWindowInterface *glWindow =
      (mdiSubWindow ? ccGLWindowInterface::FromWidget(mdiSubWindow->widget())
                    : nullptr);
  // if (glWindow)
  //	ccLog::Warning("New active GL window: " +
  // QString::number(glWindow->getUniqueID())); else 	ccLog::Warning("No more
  // active GL window");
  ...
  if (glWindow) {
    ...
    connect(glWindow->signalEmitter(), &ccGLWindowSignalEmitter::itemPicked,
            this, &ccPickingHub::processPickedItem, Qt::UniqueConnection);
    connect(glWindow->signalEmitter(), &ccGLWindowSignalEmitter::aboutToClose,
...
void ccPickingHub::processPickedItem(ccHObject *entity, unsigned itemIndex,
                                     int x, int y, const CCVector3 &P3D,
  ...
  ccPickingListener::PickedItem item;
  {
    ...
    if (entity && entity->isA(CC_TYPES::SPHERE)) {
      // whether the center of sphere entities should be used when a point is
      // picked on the surface
      static QMessageBox::StandardButton s_pickSphereCenter = QMessageBox::Yes;

      if (s_pickSphereCenter != QMessageBox::YesToAll &&
          s_pickSphereCenter != QMessageBox::NoToAll) {
        s_pickSphereCenter = QMessageBox::question(
            m_activeGLWindow->asWidget(), tr("Sphere picking"),
            tr("From now on, do you want to pick sphere centers instead of a "
               "point on their surface?"),
            QMessageBox::YesToAll | QMessageBox::Yes | QMessageBox::No |
                QMessageBox::NoToAll,
            QMessageBox::YesToAll);
      }
      if (s_pickSphereCenter == QMessageBox::Yes ||
          s_pickSphereCenter == QMessageBox::YesToAll) {
...
bool ccPickingHub::addListener(
    ccPickingListener *listener, bool exclusive /*=false*/,
    bool autoStartPicking /*=true*/,
    ccGLWindowInterface::PICKING_MODE
        mode /*=ccGLWindowInterface::POINT_OR_TRIANGLE_PICKING*/) {
  if (!listener) {
    assert(false);
    return false;
  }
  ...
  if (!m_listeners.empty()) {
    if (m_exclusive) // a previous listener is exclusive
    {
      ...
      if (m_listeners.find(listener) == m_listeners.end()) {
        ccLog::Warning(
            "[ccPickingHub::addListener] Exclusive listener already "
            "registered: stop the other tool relying on point picking first");
        return false;
      }
    } else if (exclusive) // this new listener is exclusive
    {
      if (m_listeners.size() > 1 ||
        ...
        ccLog::Warning(
            "[ccPickingHub::addListener] Attempt to register an exclusive "
            "listener while other listeners are already registered");
        return false;
      }
    } else if (mode != m_pickingMode) {
      if (m_listeners.size() > 1 ||
          m_listeners.find(listener) == m_listeners.end()) {
        ccLog::Warning("[ccPickingHub::addListener] Other listeners are "

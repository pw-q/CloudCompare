...
ColorScaleElementSlider::ColorScaleElementSlider(
    double relativePos /*=0.0*/, QColor color /*=Qt::black*/,
    QWidget *parent /*=nullptr*/,
    Qt::Orientation orientation /*=Qt::Horizontal*/)
    : QWidget(parent), ccColorScaleElement(relativePos, color),
      m_selected(false), m_orientation(orientation) {
...
void ColorBarWidget::mousePressEvent(QMouseEvent *e) {
  if (e->button() == Qt::LeftButton) {
    ...
    if (contentRect.contains(e->pos(), true)) {
      ...
      if (m_orientation == Qt::Horizontal) {
        relativePos = static_cast<double>(e->pos().x() - contentRect.left()) /
                      contentRect.width();
      } else {
        relativePos = static_cast<double>(e->pos().y() - contentRect.top()) /
                      contentRect.height();
      }
...
void ColorBarWidget::paintEvent(QPaintEvent *e) {
  if (m_sliders && m_sliders->size() >= 2) {
    ...
    assert(m_sliders->elements().front()->getRelativePos() == 0.0 &&
           m_sliders->elements().back()->getRelativePos() == 1.0);
    ...
    // color gradient
    {
      ...
      if (m_orientation == Qt::Horizontal)
        gradient =
            QLinearGradient(contentRect.left(), 0, contentRect.right(), 0);
      else
        gradient =
            QLinearGradient(0, contentRect.bottom(), 0, contentRect.top());
    ...
    // draw a line for each slider position (apart from the first and the last
    // one)
    {
      ...
      for (int i = 0; i < m_sliders->size(); i++) {
        ...
        if (m_orientation == Qt::Horizontal) {
          int pos = contentRect.left() +
                    static_cast<int>(relativePos * contentRect.width());
          A.setX(pos);
          B.setX(pos);
        } else {
          int pos = contentRect.top() +
                    static_cast<int>(relativePos * contentRect.height());
          A.setY(pos);
          B.setY(pos);
        }
...
ColorScaleElementSlider *SlidersWidget::addNewSlider(double relativePos,
  ...
  ColorScaleElementSlider *slider =
      new ColorScaleElementSlider(relativePos, color, this, m_orientation);
...
void SlidersWidget::updateAllSlidersPos() {
  ...
  for (auto it = m_sliders->elements().begin();
       it != m_sliders->elements().end(); ++it) {
...
void SlidersWidget::mouseMoveEvent(QMouseEvent *e) {
  ...
  double relativePos =
      static_cast<double>(pos - DEFAULT_MARGIN) / static_cast<double>(length());
...
SliderLabelWidget::SliderLabelWidget(
    SharedColorScaleElementSliders sliders, QWidget *parent /*=nullptr*/,
    Qt::Orientation orientation /*=Qt::Horizontal*/)
    : ColorScaleEditorBaseWidget(sliders, orientation, DEFAULT_MARGIN, parent),
      m_textColor(Qt::black), m_precision(6) {
  setContentsMargins(0, 0, 0, 0);
}
    ...
    if (m_orientation == Qt::Horizontal) {
      ...
      for (int i = 0; i < m_sliders->size(); i++) {
        ...
        QString label = QString("%1 %").arg(
            val * 100.0, 0, 'f',
            std::max(m_precision - 2, 0)); // display as a percentage
      ...
      // adjust width if necessary
      {
        QString firstLabel = QString::number(
            m_sliders->elements().first()->getRelativePos(), 'f', m_precision);
        QString lastLabel = QString::number(
            m_sliders->elements().last()->getRelativePos(), 'f', m_precision);
        int labelWidth = std::max(fm.width(firstLabel), fm.width(lastLabel)) +
...
ccColorScaleEditorWidget::ccColorScaleEditorWidget(
  ...
  // color bar
  {
    ...
    connect(m_colorBarWidget, &ColorBarWidget::pointClicked, this,
            &ccColorScaleEditorWidget::onPointClicked);
  }
  ...
  // sliders widget
  {
    ...
    connect(m_slidersWidget, &SlidersWidget::sliderModified, this,
            &ccColorScaleEditorWidget::onSliderModified);
    connect(m_slidersWidget, &SlidersWidget::sliderSelected, this,
            &ccColorScaleEditorWidget::onSliderSelected);
  }
...
void ccColorScaleEditorWidget::onPointClicked(double relativePos) {
  ...
  for (int i = 0; i < m_sliders->size(); ++i) {
    if (std::abs(m_sliders->element(i)->getRelativePos() - relativePos) <
        maxDist) {

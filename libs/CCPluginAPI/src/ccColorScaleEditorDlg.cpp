...
ccColorScaleEditorDialog::ccColorScaleEditorDialog(
    ccColorScalesManager *manager, ccMainAppInterface *mainApp,
    ccColorScale::Shared currentScale /*=ccColorScale::Shared(nullptr)*/,
    QWidget *parent /*=nullptr*/)
    : QDialog(parent), m_manager(manager), m_colorScale(currentScale),
      m_scaleWidget(new ccColorScaleEditorWidget(this, Qt::Horizontal)),
      m_associatedSF(nullptr), m_modified(false), m_minAbsoluteVal(0.0),
      m_maxAbsoluteVal(1.0), m_mainApp(mainApp),
  ...
  // main combo box
  connect(m_ui->rampComboBox, qOverload<int>(&QComboBox::activated), this,
          &ccColorScaleEditorDialog::colorScaleChanged);

  // import/export buttons
  connect(m_ui->exportToolButton, &QToolButton::clicked, this,
          &ccColorScaleEditorDialog::exportCurrentScale);
  connect(m_ui->importToolButton, &QToolButton::clicked, this,
          &ccColorScaleEditorDialog::importScale);

  // upper buttons
  connect(m_ui->renameToolButton, &QToolButton::clicked, this,
          &ccColorScaleEditorDialog::renameCurrentScale);
  connect(m_ui->saveToolButton, &QToolButton::clicked, this,
          &ccColorScaleEditorDialog::saveCurrentScale);
  connect(m_ui->deleteToolButton, &QToolButton::clicked, this,
          &ccColorScaleEditorDialog::deleteCurrentScale);
  connect(m_ui->copyToolButton, &QToolButton::clicked, this,
          &ccColorScaleEditorDialog::copyCurrentScale);
  connect(m_ui->newToolButton, &QToolButton::clicked, this,
          &ccColorScaleEditorDialog::createNewScale);
  connect(m_ui->scaleModeComboBox, qOverload<int>(&QComboBox::activated), this,
          &ccColorScaleEditorDialog::relativeModeChanged);

  // scale widget
  connect(m_scaleWidget, &ccColorScaleEditorWidget::stepSelected, this,
          &ccColorScaleEditorDialog::onStepSelected);
  connect(m_scaleWidget, &ccColorScaleEditorWidget::stepModified, this,
          &ccColorScaleEditorDialog::onStepModified);

  // slider editor
  connect(m_ui->deleteSliderToolButton, &QToolButton::clicked, this,
          &ccColorScaleEditorDialog::deletecSelectedStep);
  connect(m_ui->colorToolButton, &QToolButton::clicked, this,
          &ccColorScaleEditorDialog::changeSelectedStepColor);
  connect(m_ui->valueDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccColorScaleEditorDialog::changeSelectedStepValue);

  // labels list widget
  connect(m_ui->customLabelsGroupBox, &QGroupBox::toggled, this,
          &ccColorScaleEditorDialog::toggleCustomLabelsList);
  connect(m_ui->customLabelsPlainTextEdit, &QPlainTextEdit::textChanged, this,
          &ccColorScaleEditorDialog::onCustomLabelsListChanged);

  // apply button
  connect(m_ui->applyPushButton, &QPushButton::clicked, this,
          &ccColorScaleEditorDialog::onApply);
  // close button
  connect(m_ui->closePushButton, &QPushButton::clicked, this,
          &ccColorScaleEditorDialog::onClose);
...
ccColorScaleEditorDialog::~ccColorScaleEditorDialog() { delete m_ui; }
...
void ccColorScaleEditorDialog::setActiveScale(
  ...
  // custom labels
  {
    ...
    if (customLabels.empty()) {
      ...
      QString text;
      size_t index = 0;
      for (ccColorScale::LabelSet::const_iterator it = customLabels.begin();
...
void ccColorScaleEditorDialog::onStepSelected(int index) {
  ...
  m_ui->deleteSliderToolButton->setEnabled(
      ...
      index + 1 <
          m_scaleWidget
              ->getStepCount()); // don't delete the first and last steps!
  ...
  if (index < 0) {
    ...
    bool modified = m_modified; // save 'modified' state before calling
    ...
    if (isRelativeMode()) {
      ...
      if (m_associatedSF) {
        ...
        double actualValue =
            ...
            relativePos * (m_associatedSF->getMax() - m_associatedSF->getMin());
...
void ccColorScaleEditorDialog::deletecSelectedStep() {
  ...
  if (selectedIndex >= 1 &&
      selectedIndex + 1 <
          m_scaleWidget
              ->getStepCount()) // never delete the first and last steps!
  {
...
void ccColorScaleEditorDialog::changeSelectedStepValue(double value) {
  ...
  } else // absolute scale mode
  {
      ...
      for (int i = 0; i < m_scaleWidget->getStepCount(); ++i) {
        ...
        double absolutePos =
            (i == selectedIndex ? value
                                : m_minAbsoluteVal + slider->getRelativePos() *
                                                         (m_maxAbsoluteVal -
                                                          m_minAbsoluteVal));
        newSliders->elements().push_back(
            new ColorScaleElementSlider(absolutePos, slider->getColor()));
      }
    }
    ...
    // update min and max boundaries
    {
      newSliders->sort();
      m_minAbsoluteVal =
          newSliders->elements().front()->getRelativePos(); // absolute in fact!
      m_maxAbsoluteVal =
          newSliders->elements().back()->getRelativePos(); // absolute in fact!
    }
...
QString ccColorScaleEditorDialog::exportCustomLabelsList(
    ccColorScale::LabelSet &labels) const {
  assert(m_ui->customLabelsGroupBox->isChecked());
  labels.clear();

  QString fullText = m_ui->customLabelsPlainTextEdit->toPlainText();
  QStringList lines =
      fullText.split(QRegExp("[\r\n]"), QString::SkipEmptyParts);
  if (lines.size() < 2) {
    ...
    for (QString line : lines) {
      ...
      if (firstQuoteIndex == 0) {
        ...
        if (secondQuoteIndex > firstQuoteIndex + 1) {
          text = line.mid(firstQuoteIndex + 1,
    ...
    for (size_t i = 0; i < clouds.size(); ++i) {
      ...
      for (unsigned j = 0; j < cloud->getNumberOfScalarFields(); ++j) {
        ...
        if (sf->getColorScale() == m_colorScale) {
          // trick: we unlink then re-link the color scale to update everything
          // automatically
...
void ccColorScaleEditorDialog::renameCurrentScale() {
  ...
  QString newName = QInputDialog::getText(
      this, "Scale name", "Name", QLineEdit::Normal, m_colorScale->getName());
  if (!newName.isNull()) {
  ...
  if (scale) {
    ...
    if (m_manager) {
      ...
      if (otherScale) {
        ...
        message += "\n";
        message += "Do you want to force the importation of this new scale? (a "
                   "new UUID will be generated)";

        if (QMessageBox::question(this, "UUID conflict", message,
                                  QMessageBox::Yes,
                                  QMessageBox::No) == QMessageBox::No) {
          ccLog::Warning("[ccColorScaleEditorDialog::importScale] Importation "
                         "cancelled due to a conflicting UUID (color scale may "
                         "already be in store)");
          return;
        }
        // generate a new UUID
      ...
      m_manager->addScale(scale);
      ccLog::Print(QString("[ccColorScaleEditorDialog::importScale] Color "
                           "scale '%1' successfully imported")
                       .arg(scale->getName()));
    }

...
CC_FILE_ERROR RasterGridFilter::loadFile(const QString &filename,
  ...
  try {
    ...
    if (poDataset != nullptr) {
      ...
      ccLog::Print("Driver: %s/%s", poDataset->GetDriver()->GetDescription(),
      ...
      if (poDataset->GetGeoTransform(adfGeoTransform) == CE_None) {
        ccLog::Print("Origin = (%.6f,%.6f)", adfGeoTransform[0],
                     adfGeoTransform[3]);
        ccLog::Print("Pixel Size = (%.6f,%.6f)", adfGeoTransform[1],
                     adfGeoTransform[5]);
      }
      ...
      ccLog::Print(QString("Pixel Type = ") +
      ...
      std::vector<bool> isColorBand(rasterCount, false);
      {
        ...
        for (int i = 1; i <= rasterCount; ++i) {
          ...
          case GCI_Undefined:
          case GCI_GrayIndex: {
            // Sometimes GDAL fails to recognize color bands as such, or it can
            // flag a Z band as gray values if altitudes are between 0 and
            // 255...
            int bGotMin = 0, bGotMax = 0;
            double adfMinMax[2]{poBand->GetMinimum(&bGotMin),
                                poBand->GetMaximum(&bGotMax)};
            if (0 == bGotMin || 0 == bGotMax) {
              // DGM FIXME: if the file is corrupted (e.g. ASCII ArcGrid with
              // missing rows) this method will enter in a infinite loop!
            ...
            if (adfMinMax[0] == static_cast<double>(minVal) &&
                adfMinMax[1] == static_cast<double>(maxVal) && minVal >= 0 &&
                maxVal <= 255) {
        ...
        hasUndefinedColorBands =
            (colorBands == 0 &&
             (undefinedColorBandCount == 3 || undefinedColorBandCount == 4));
      }

      bool loadAsTexturedQuad = false;
      if (colorBands >= 3) {
        loadAsTexturedQuad = false;
        if (parameters.parentWidget) // otherwise it means we are in command
                                     // line mode --> no popup
        {
          loadAsTexturedQuad =
      ...
      // check for 'big' coordinates
      {
        ...
        if (HandleGlobalShift(origin, Pshift, preserveCoordinateShift,
          ...
          ccLog::Warning("[RasterFilter::loadFile] Raster has been recentered! "
      ...
      if (loadAsTexturedQuad) {
        ...
        for (int j = 0; j < rasterY; ++j) {
          for (int i = 0; i < rasterX; ++i) {
            ...
            // adfGeoTransform(2) Ygeo = adfGeoTransform(3) + Xpixel *
            // adfGeoTransform(4) + Yline * adfGeoTransform(5)
            double x = adfGeoTransform[0] + (i + 0.5) * adfGeoTransform[1] +
                       (j + 0.5) * adfGeoTransform[2] + Pshift.x;
            double y = adfGeoTransform[3] + (i + 0.5) * adfGeoTransform[4] +
      ...
      for (int i = 1; i <= rasterCount; ++i) {
        ...
        ccLog::Print("[GDAL] Block=%dx%d, Type=%s, ColorInterp=%s", nBlockXSize,
                     nBlockYSize,
        ...
        if (colTable != nullptr) {
          ccLog::Print("[GDAL] Band has a color table with %d entries",
                       colTable->GetColorEntryCount());
        }

        if (poBand->GetOverviewCount() > 0) {
          ccLog::Print("[GDAL] Band has %d overviews",
                       poBand->GetOverviewCount());
        }

        if (colorInterp == GCI_GrayIndex && !isColorBand[i - 1]) {
          // Sometimes, GDAL flags a band as 'gray index' just because its
        ...
        if (!zRasterProcessed &&
            colorInterp ==
                GCI_Undefined // probably heights? DGM: not sufficient since
                              // GDAL is sometimes lost if the bands are coded
                              // with 64 bits values :(
          ...
          for (int j = 0; j < nYSize; ++j) {
            ...
            for (int k = 0; k < nXSize; ++k) {
              ...
              if (pointIndex <= pc->size()) {
                const_cast<CCVector3 *>(pc->getPoint(pointIndex))->z =
                    static_cast<PointCoordinateType>(z);
        ...
        } else // colors or scalars
        {
          ...
          case GCI_AlphaBand:
            if (adfMinMax[0] != adfMinMax[1]) {
              if (loadAsTexturedQuad)
                ...
                isScalar = true; // we can't load the alpha band as a cloud
                                 // color (transparency is not handled yet)
            } else {
              ccLog::Warning(
                  QString("Alpha band ignored as it has a unique value (%1)")
                      .arg(adfMinMax[0]));
            }
            break;
          default:
          ...
          if (isRGB || isPalette) {
            ...
            if (isPalette && !colTable) {
              ccLog::Warning(
                  QString("Band is declared as a '%1' but no palette is "
                          "associated!")
                      .arg(GDALGetColorInterpretationName(colorInterp)));
            } else {
              // instantiate memory for RBG colors if necessary
              if (!loadAsTexturedQuad && !pc->hasColors() &&
                  !pc->setColor(ccColor::white)) {
                ccLog::Warning(
                    QString("Failed to instantiate memory for storing color "
                            "band '%1'!")
                        .arg(GDALGetColorInterpretationName(colorInterp)));
              } else {
            ...
            QString sfName =
                QString("band #%1 (%2)")
                    ...
                    .arg(GDALGetColorInterpretationName(
                        colorInterp)); // SF names really need to be unique!
            ccScalarField *sf = new ccScalarField(sfName.toStdString());
            if (!sf->resizeSafe(pc->size(), true, CCCoreLib::NAN_VALUE)) {
              ccLog::Warning(QString("Failed to instantiate memory for storing "
                                     "'%1' as a scalar field!")
      ...
      if (quad) {
        ...
        if (!zRasterProcessed) {
          ccLog::Warning("Raster has no height (Z) information: you can "
                                       ...
                                       // line mode --> no popup
          {
            result = (s_alwaysRemoveInvalidHeights
                          ? QMessageBox::Yes
                          : QMessageBox::question(
                                nullptr, "Remove invalid points?",
                                "This raster has pixels with invalid heights. "
          ...
          if (result != QMessageBox::No) // Yes = let's remove them
          {
            ...
            ccLog::PrintDebug(
                QString("Removing %1 invalid points out of %2 points...")
                    .arg(pc->size() - validPoints.size())
                    .arg(pc->size()));
            ...
            if (newPC) {
              ...
              ccLog::Error("Not enough memory to remove the points with "
                           "invalid heights!");
            }
          }
        }

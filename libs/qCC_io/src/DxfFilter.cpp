...
DxfFilter::DxfFilter()
    : FileIOFilter(
          {"_DXF Filter",
           13.0f, // priority
           QStringList{"dxf"}, "dxf", QStringList{"DXF geometry (*.dxf)"},
           QStringList{"DXF geometry (*.dxf)"}, Import | Export | BuiltIn}) {}

bool DxfFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
                        bool &exclusive) const {
  if (type == CC_TYPES::POLY_LINE || type == CC_TYPES::MESH ||
...
public:
  ...
  DxfImporter(ccHObject *root, FileIOFilter::LoadParameters &parameters)
      : m_root(root), m_points(nullptr), m_faces(nullptr), m_poly(nullptr),
        m_polyVertices(nullptr), m_firstPoint(true), m_globalShift(0, 0, 0),
        m_preserveCoordinateShift(false), m_loadParameters(parameters) {
    assert(m_root);
  }
  ...
  CCVector3 convertPoint(double x, double y, double z) {
    ...
    if (m_firstPoint) {
      if ((!m_preserveCoordinateShift ||
           ccGlobalShiftManager::NeedShift(P + m_globalShift)) &&
          FileIOFilter::HandleGlobalShift(
              P, m_globalShift, m_preserveCoordinateShift, m_loadParameters)) {
        ccLog::Warning("[DxfImporter] All points/vertices will be recentered! "
  ...
  void addPoint(const DL_PointData &P) override {
    ...
    if (getCurrentColour(col)) {
      ...
      if (m_points->hasColors()) {
        ...
        // reserve memory (and fill the previous points with a default color if
        // necessary)
        ...
        m_points->setPointColor(m_points->size() - 1,
                                ccColor::white); // replace the last color
      }
  ...
  void addPolyline(const DL_PolylineData &poly) override {
    ...
    // m_poly->set2DMode(poly.flags & 8); //DGM: "2D" polylines in CC doesn't
    // mean the same thing ;)
  ...
  void add3dFace(const DL_3dFaceData &face) override {
        ...
        for (unsigned j = 0; j < vertCount; ++j) {
          ...
          if (P[i].x == Pj->x && P[i].y == Pj->y && P[i].z == Pj->z) {
            ...
            if (faceCol || vertices->hasColors()) {
              const ccColor::Rgb *_faceCol =
                  faceCol ? faceCol : &ccColor::whiteRGB;
              const ccColor::Rgba *_vertCol = vertices->hasColors()
                                                  ? &vertices->getPointColor(j)
                                                  : &ccColor::white;
              useCurrentVertex =
                  (_faceCol->r == _vertCol->r && _faceCol->g == _vertCol->g &&
    ...
    if (faceCol) {
      ...
      // default
        ...
        for (unsigned i = 0; i < createdVertCount; ++i)
          vertices->setPointColor(vertCount - 1 - i,
                                  ccColor::Rgba(*faceCol, ccColor::MAX));
  ...
  void addArc(const DL_ArcData &data) override {
    ...
    unsigned vertexCount =
        ...
        static_cast<unsigned>(std::max(
            1.0,
            arcLength_deg)); // we use a 1 degree resolution by default for now
    double step_deg = 1.0;
    ...
    for (unsigned i = 0; i < vertexCount; ++i) {
      ...
      CCVector3 P =
          Clocal +
          CCVector3(
              static_cast<PointCoordinateType>(data.radius * cos(angle_rad)),
              static_cast<PointCoordinateType>(data.radius * sin(angle_rad)),
              0);
...
private:
  ...
  bool getCurrentColour(ccColor::Rgb &ccColour) {
    ...
    if (colourIndex == 0) {
      ...
      // an attribute of 256 means the colours are BYLAYER, so grab it from our
      // map instead
      const int defaultIndex = -1;
      colourIndex =
          m_layerColourMap.value(attributes.getLayer().c_str(), defaultIndex);
    ...
    ccColour.r =
        static_cast<ColorCompType>(dxfColors[colourIndex][0] * ccColor::MAX);
    ccColour.g =
        static_cast<ColorCompType>(dxfColors[colourIndex][1] * ccColor::MAX);
    ccColour.b =
        static_cast<ColorCompType>(dxfColors[colourIndex][2] * ccColor::MAX);

    return true;
  }

  //! Keep track of the colour of each layer in case the colour attribute is set
  //! to BYLAYER
...
CC_FILE_ERROR DxfFilter::saveToFile(ccHObject *root, const QString &filename,
      ...
      for (ccHObject *cloud : clouds) {
        ...
        if (parent) {
          if (parent->isA(CC_TYPES::POLY_LINE)) {
            if (std::find(polylines.begin(), polylines.end(), parent) !=
                polylines.end()) {
  ...
  if (CheckForSpecialChars(filename)) {
    ccLog::Warning(
        QString("[DXF] Output filename contains special characters. It might "
    ...
    // Writing the Entities Section
    {
      ...
      for (unsigned i = 0; i < polyCount; ++i) {
        ...
        if (!poly->is2DMode())
          flags |= 8; // 3D polyline
        dxf.writePolyline(
            *dw, DL_PolylineData(static_cast<int>(vertexCount), 0, 0, flags),
            DL_Attributes(qPrintable(polyLayerNames[i]), DL_Codes::bylayer, -1,
      ...
      for (unsigned j = 0; j < meshCount; ++j) {
        ...
        for (unsigned f = 0; f < triCount; ++f) {
          ...
          dxf.write3dFace(
              *dw,
              DL_3dFaceData(A.x, A.y, A.z, B.x, B.y, B.z, C.x, C.y, C.z, C.x,
                            C.y, C.z, lineWidth),
              DL_Attributes(qPrintable(meshLayerNames[j]), DL_Codes::bylayer,
                            -1, "BYLAYER",
                            1.0)); // DGM: warning, toStdString doesn't preserve
                                   // "local" characters
        }
      }
        ...
        for (unsigned j = 0; j < pointCount; ++j) {
          ...
          dxf.writePoint(
              *dw, DL_PointData(Pg.x, Pg.y, Pg.z),
              DL_Attributes(qPrintable(pointLayerNames[i]), DL_Codes::bylayer,
                            -1, "BYLAYER",
                            1.0)); // DGM: warning, toStdString doesn't preserve
                                   // "local" characters
        }
...
CC_FILE_ERROR DxfFilter::loadFile(const QString &filename, ccHObject &container,
...
#else
    if (CheckForSpecialChars(filename)) {
      ccLog::Warning("[DXF] Input file contains special characters. It might "
                     "be rejected by the third party library...");
    }
    if (DL_Dxf().in(qPrintable(filename),
                    &importer)) // DGM: warning, toStdString doesn't preserve
                                // "local" characters
#endif
    {
      importer.applyGlobalShift(); // apply the (potential) global shift to
                                   // shared clouds

...
bool PlyFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
CC_FILE_ERROR PlyFilter::saveToFile(ccHObject *entity, const QString &filename,
...
CC_FILE_ERROR PlyFilter::saveToFile(ccHObject *entity, QString filename,
...
#else
  p_ply ply =
      ply_create(qPrintable(filename), storageType, errorCallback, 0, nullptr);
#endif
  if (!ply) {
  ...
  e_ply_type coordType =
      vertices->isShifted() || sizeof(PointCoordinateType) > 4
          ? PLY_DOUBLE
          : PLY_FLOAT; // we use double coordinates for shifted vertices (i.e.
                       // >1e6)
  ...
  if (mesh && mesh->hasMaterials()) {
    ...
    // if (!mesh->hasColors())
    {
      ...
      if (textureCount == 1) {
        if (materials->size() == 1) {
          ...
          if (mesh->hasColors()) {
            ccLog::Error(
                "PLY files can't handle multiple materials/textures!\nOnly the "
                "RGB field will be saved.");
          } else {
            if (mesh->isA(CC_TYPES::MESH)) {
              if (QMessageBox::question(
                      nullptr, "Multiple materials, one texture",
                      "This mesh has only one texture but multiple materials. "
                      "PLY files can only handle one texture.\nShall we drop "
                      "the materials (yes) or convert all materials and "
                      "texture to per-vertex RGB colors? (no)",
              ...
              // we can forget the texture
              material = ccMaterial::CShared(nullptr);
              ccLog::Warning("This sub-mesh has one texture and multiple "
                             "materials. As this is a sub-mesh, we will ignore "
                             "them...  you should convert the parent mesh "
        ...
        if (mesh->hasColors()) {
          ...
          if (mesh->isA(CC_TYPES::MESH)) {
            ...
            if (QMessageBox::question(
                    ...
                    "you want to convert them to per-vertex RGB colors?",
                    QMessageBox::Yes | QMessageBox::No,
            ...
            ccLog::Warning(
                "This sub-mesh has multiple textures/materials. PLY files "
                "can't handle them.\nAs this is a sub-mesh, we will have to "
                "ignore them... you should convert the parent mesh "
                "textures/materials to RGB colors first");
          } else {
            assert(false);
          }
    ...
    // else
    //{
    //	ccLog::Warning("[PLY] PLY files can't handle materials/textures! RGB
    // field will be saved instead"); 	ccLog::Warning("[PLY] Note: you can
      ...
      for (unsigned i = 0; i < sfCount; ++i) {
        ...
        if (sfName.isEmpty()) {
          ...
          if (originalStdPropsNames.contains(sfName)) {
            // don't change the name of a field that was already in the original
            // file
            propName = sfName;
          } else {
            // append the SF name with 'scalar' for easier detection at loading
            // time
        ...
        ScalarType maxValue = std::max(std::abs(scalarFields[i]->getMin()),
                                       std::abs(scalarFields[i]->getMax()));

        e_ply_type scalarType =
            (maxValue < ccGlobalShiftManager::MaxBoundgBoxDiagonal()
                 ? PLY_FLOAT
                 : PLY_DOUBLE);
        if (scalarType == PLY_DOUBLE) {
          ccLog::Warning(
              QString("[PLY] Scalar field '%1' has large values and will be "
                      "saved as double values instead of float values")
                  .arg(QString::fromStdString(scalarFields[i]->getName())));
        }
  ...
  if (mesh) {
    ...
    if (triNum > 0 && ply_add_element(ply, "face", triNum)) {
      ...
      if (material) {
        ...
        if (!material->getTexture().mirrored().save(
                textureFilePath)) // mirrored --> see ccMaterial
        {
          ccLog::Warning(QString("[PLY] Failed to save texture in '%1'!")
                             .arg(textureFilePath));
          material = ccMaterial::CShared(nullptr);
        } else {
          // save texture filename as a comment! Note MeshLab only supports
          // CamelCase as of writing.
          result = ply_add_comment(
              ply,
              qPrintable(QString("TextureFile %1").arg(defaultTextureName)));
          // DGM FIXME: is this the right name?
          result =
              ply_add_list_property(ply, "texcoord", PLY_UCHAR,
  ...
  ply_add_obj_info(ply, "Generated by Zoomlion!");
  ...
  for (unsigned i = 0; i < vertCount; ++i) {
    ...
    for (std::vector<ccScalarField *>::const_iterator sf = scalarFields.begin();
  ...
  if (mesh) {
    ...
    for (unsigned i = 0; i < triNum; ++i) {
      const CCCoreLib::VerticesIndexes *tsi =
          mesh->getNextTriangleVertIndexes(); // DGM: getNextTriangleVertIndexes
  ...
  if (flags & ELEM_EOL) {
    ...
    if (s_PointCount == 0) {
      ...
      if (FileIOFilter::HandleGlobalShift(
              s_Point, s_Pshift, preserveCoordinateShift, s_loadParameters)) {
        if (preserveCoordinateShift) {
          cloud->setGlobalShift(s_Pshift);
        }
        ccLog::Warning("[PLYFilter::loadFile] Cloud (vertices) has been "
...
static int rgb_cb(p_ply_argument argument) {
  ...
  case PLY_FLOAT32:
  case PLY_FLOAT64:
    s_color.rgba[flags & POS_MASK] = static_cast<ColorCompType>(
        std::min(std::max(0.0, ply_get_argument_value(argument)), 1.0) *
        ccColor::MAX);
    break;
  case PLY_INT8:
  case PLY_UINT8:
  case PLY_CHAR:
  case PLY_UCHAR:
    s_color.rgba[flags & POS_MASK] =
        static_cast<ColorCompType>(ply_get_argument_value(argument));
    break;
  default:
    s_color.rgba[flags & POS_MASK] =
        static_cast<ColorCompType>(ply_get_argument_value(argument));
...
static int face_cb(p_ply_argument argument) {
  ...
  if (value_index == 2) {
    ...
    s_hasQuads = true;
    if (s_hasMaterials) {
      // specifc case: when dealing with quads WITH materials, we must keep
      // track of the real index(es) of the corresponding triangles
...
static int texCoords_cb(p_ply_argument argument) {
  ...
  if (((value_index + 1) % 2) == 0) {
    ...
    texCoords->addElement(
        TexCoords2D(s_texCoord[value_index - 1], s_texCoord[value_index]));
    ++s_texCoordCount;
...
CC_FILE_ERROR PlyFilter::loadFile(const QString &filename, ccHObject &container,
...
CC_FILE_ERROR PlyFilter::loadFile(const QString &filename,
...
#ifdef _WIN32
  p_ply ply =
      ply_open(filename.toStdWString().c_str(), errorCallback, 0, nullptr);
#else
  ...
  try {
    ...
    while ((lastElement.elem = ply_get_next_element(ply, lastElement.elem))) {
      ...
      if (lastElement.elementInstances == 0) {
        ccLog::Warning("[PLY] Element '%s' was ignored as it has 0 instance!",
      ...
      if (lastElement.isFace) {
        ...
        for (size_t i = 0; i < lastElement.properties.size(); ++i) {
          ...
          if (prop.type == PLY_LIST) {
            // multiple elements per face (vertex indexes, texture coordinates,
            // etc.)
  ...
  stdPropsText << QString("None");
  {
    for (int i = 1; i <= static_cast<int>(stdProperties.size()); ++i) {
      ...
      // assert(pp.type != PLY_LIST || pp.value_type == PLY_FLOAT); //we don't
      // want any PLY_LIST here
      ...
      if (nxIndex == 0 &&
        ...
        bIndex = i;
               ...
               (propName.contains("INTENSITY") || propName.contains("GRAY") ||
                propName.contains("GREY") ||
                (elementName.contains("COL") && propName.endsWith("I")))) {
        sfPropIndexes.push_back(i); // DGM: we don't load the intensities as RGB
                                    // colors anymore but as a scalar field
    ...
    for (int i = 0; i < static_cast<int>(listProperties.size()); ++i) {
      ...
      if (elementName.contains("FACE") || elementName.contains("TRI")) {
        ...
        if (texCoordsIndex == 0 && propName.contains("COORD") &&
            !textureFileNames.isEmpty()) // no need to assign this value if we
  ...
  if (stdPropsCount < 2) {
    ccLog::Warning("[PLY] This ply file has less than 2 properties defined! "
                   "(not even X and Y ;)");
    ...
    if (parameters.alwaysDisplayLoadDialog ||
        stdPropsCount >
            assignedStdProperties +
                1 //+1 because of the first item in the combo box ('none')
      ...
      pod.propertiesEdit->setText(QString::number(listProperties.size() +
                                                  stdProperties.size() +
      ...
      if (!pod.restorePreviousContext(hasAPreviousContext)) {
        if (hasAPreviousContext) {
          ccLog::Warning("[PLY] Too many differences with the previous file, "
                         "we reset the dialog.");
        }
  ...
  if (xIndex > 0) {
    ...
    ply_set_read_cb(ply, pointElements[pp.elemIndex].elementName, pp.propName,
                    vertex_cb, cloud, flags);
  ...
  if (yIndex > 0) {
    ...
    if (numberOfPoints > 0) {
      if (static_cast<long>(numberOfPoints) !=
        ...
        ccLog::Warning(
            "[PLY] Bad/uncompatible assignation of point properties!");
        delete cloud;
        ply_close(ply);
        return CC_FERR_BAD_ENTITY_TYPE;
      }
  ...
  if (numberOfNormals > 0) {
    if (numberOfPoints != numberOfNormals) {
      ccLog::Warning(
          "[PLY] The number of normals doesn't match the number of points!");
      delete cloud;
      ply_close(ply);
      return CC_FERR_BAD_ENTITY_TYPE;
    }
  ...
  if (gIndex > 0) {
    ...
    numberOfColors = std::max(
        numberOfColors, (unsigned)pointElements[pp.elemIndex].elementInstances);
  }
  ...
  if (iIndex > 0) {
    if (numberOfColors > 0) {
      ccLog::Error(
          "Can't import colors AND intensity (intensities will be ignored)!");
  ...
  /* SCALAR FIELDS (SF) */
  {
    for (size_t i = 0; i < sfPropIndexes.size(); ++i) {
      ...
      if (numberOfPoints != numberOfScalars) {
        ccLog::Error(
            QString("Scalar field #%1: the number of scalars doesn't match the "
                    "number of points (they will be ignored)!")
                .arg(i + 1));
        ...
        if (sfIdx < 0) {
          ccLog::Error(QString("Scalar field #%1: not enough memory to load "
                               "scalar field (they will be ignored)!")
                           .arg(i + 1));
          ccLog::Warning(QString("[PLY] Scalar field #%1 ignored!").arg(i + 1));
        }
      }
  ...
  if (facesIndex > 0) {
    ...
    if (!mesh->reserve(numberOfFacets)) {
      ...
      ply_set_read_cb(ply, meshElements[pp.elemIndex].elementName, pp.propName,
                      face_cb, mesh, 0);
    }
  }
  ...
  if (texCoordsIndex > 0) {
    ...
    if (!texCoords->reserveSafe(numberOfCoordinates * 3)) {
      ccLog::Error("Not enough memory to load texture coordinates (they will "
                   "be ignored)!");
  ...
  if (texNumberIndex > 0) {
    ...
    if (!texIndexes->reserveSafe(numberOfCoordinates)) {
      ccLog::Error(
          "Not enough memory to load texture indexes (they will be ignored)!");
  ...
  if (texIndexes) {
    if (!texCoords) {
      ccLog::Error("No texture coordinates were loaded (texture indexes will "
                   "be ignored)");
      texIndexes->release();
      texIndexes = nullptr;
      ...
      if (!s_hasQuads) {
        ...
        // for quads, we must resize the texture indexes table (as we have more
        // triangles than input 'faces')
  ...
  if (mesh) {
    ...
    if (maxVertIndex >= numberOfPoints) {
      if (maxVertIndex == numberOfPoints && minVertIndex > 0) {
        ccLog::Warning(
            "[PLY] Vertex indexes seem to be shifted (+1)! We will try to "
            "'unshift' the indexes (otherwise the file might be corrupted...)");
        for (unsigned i = 0; i < s_triCount; ++i) {
    ...
    // cloud->setLocked(true); //DGM: no need to lock it as it is only used by
    // one mesh!
      ...
      if (!textureFileNames.isEmpty()) {
        ...
        for (int ti = 0; ti < textureFileNames.size(); ++ti) {
          ...
          if (material->loadAndSetTexture(textureFilePath)) {
            ...
            ccLog::Print(QString("[PLY][Texture] Successfully loaded texture "
                                 "'%1' (%2x%3 pixels)")
                             .arg(textureFileName)
                             .arg(texture.width())
                             .arg(texture.height()));
            ...
            ccLog::Warning(QString("[PLY][Texture] Failed to load texture '%1'")
                               .arg(textureFilePath));
          }
        }
        ...
        ccLog::Warning(
            "[PLY][Texture] Texture coordinates loaded without an associated "
            "image! (we look for the 'TextureFile' keyword in comments)");
      }

      if (materials) {
        // reserve memory for per triangle texture coordinates and optionally
        // for material (texture) indexes
        if (mesh->reservePerTriangleTexCoordIndexes() &&
          ...
          for (unsigned i = 0; i < mesh->size(); ++i) {
            ...
            if (!s_hasQuads) {
              ...
              mesh->addTriangleTexCoordIndexes(lastTexCoordIndex,
                                               lastTexCoordIndex + 1,
                                               lastTexCoordIndex + 2);
              lastTexCoordIndex += 3;
            } else {
              ...
              if (texIndexes && i != lastTexIndexIndex) {
                texIndexes->setValue(i,
                                     texIndexes->getValue(lastTexIndexIndex));
              }
          ...
          ccLog::Warning("[PLY][Texture] Failed to reserve per-triangle "
    ...
    if (cloud->hasNormals()) {
      ...
      // the user do it himself later Moreover it's not always good idea if the
      // user doesn't want normals (especially in ccViewer!)
      // mesh->computeNormals();
      ccLog::Warning("[PLY] Mesh has no normal! You can manually compute them "

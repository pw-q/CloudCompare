...
ccShiftAndScaleCloudDlg::ccShiftAndScaleCloudDlg(const CCVector3d &Pg,
    ...
    : QDialog(parent), m_ui(nullptr), m_applyAll(false), m_cancel(false),
      m_activeInfoIndex(-1), m_originalPoint(Pg), m_originalDiagonal(Dg),
      m_localPoint(0, 0, 0), m_localDiagonal(-1.0), m_reversedMode(false) {
  init();
...
ccShiftAndScaleCloudDlg::ccShiftAndScaleCloudDlg(const CCVector3d &Pl,
    ...
    : QDialog(parent), m_ui(nullptr), m_applyAll(false), m_cancel(false),
      m_activeInfoIndex(-1), m_originalPoint(Pg), m_originalDiagonal(Dg),
      m_localPoint(Pl), m_localDiagonal(Dl), m_reversedMode(true) {
  init();
...
void ccShiftAndScaleCloudDlg::init() {
  ...
  connect(m_ui->moreInfoToolButton, &QToolButton::clicked, this,
          &ccShiftAndScaleCloudDlg::displayMoreInfo);
  connect(m_ui->keepGlobalPosCheckBox, &QCheckBox::toggled, this,
          &ccShiftAndScaleCloudDlg::onGlobalPosCheckBoxToggled);
  connect(m_ui->loadComboBox, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &ccShiftAndScaleCloudDlg::onLoadIndexChanged);
  connect(m_ui->buttonBox,
          qOverload<QAbstractButton *>(&QDialogButtonBox::clicked), this,
          &ccShiftAndScaleCloudDlg::onClick);
  connect(m_ui->shiftX, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccShiftAndScaleCloudDlg::updateGlobalAndLocalSystems);
  connect(m_ui->shiftY, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccShiftAndScaleCloudDlg::updateGlobalAndLocalSystems);
  connect(m_ui->shiftZ, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccShiftAndScaleCloudDlg::updateGlobalAndLocalSystems);
  connect(m_ui->scaleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccShiftAndScaleCloudDlg::updateGlobalAndLocalSystems);
}
...
void ccShiftAndScaleCloudDlg::updateGlobalSystem() {
  ...
  m_ui->xOriginLabel->setStyleSheet(
      AlmostEq(P.x, m_originalPoint.x) ? QString() : QString("color: purple;"));
  m_ui->yOriginLabel->setText(QString("y = %1").arg(P.y, 0, 'f'));
  m_ui->yOriginLabel->setStyleSheet(
      AlmostEq(P.y, m_originalPoint.y) ? QString() : QString("color: purple;"));
  m_ui->zOriginLabel->setText(QString("z = %1").arg(P.z, 0, 'f'));
  m_ui->zOriginLabel->setStyleSheet(
      AlmostEq(P.z, m_originalPoint.z) ? QString() : QString("color: purple;"));
...
void ccShiftAndScaleCloudDlg::updateLocalSystem() {
  ...
  m_ui->xDestLabel->setStyleSheet(ccGlobalShiftManager::NeedShift(localPoint.x)
                                      ? QString("color: red;")
                                      : QString());
  m_ui->yDestLabel->setText(QString("y = %1").arg(localPoint.y, 0, 'f', prec));
  m_ui->yDestLabel->setStyleSheet(ccGlobalShiftManager::NeedShift(localPoint.y)
                                      ? QString("color: red;")
                                      : QString());
  m_ui->zDestLabel->setText(QString("z = %1").arg(localPoint.z, 0, 'f', prec));
  m_ui->zDestLabel->setStyleSheet(
      ccGlobalShiftManager::NeedShift(localPoint.z) || (getShift().z != 0)
          ? QString("color: red;")
          : QString());
  ...
  m_ui->diagDestLabel->setStyleSheet(
      ccGlobalShiftManager::NeedRescale(localDiagonal) ? QString("color: red;")
                                                       : QString());
}
...
bool ccShiftAndScaleCloudDlg::getInfo(
    size_t index, ccGlobalShiftManager::ShiftInfo &info) const {
...
int ccShiftAndScaleCloudDlg::addShiftInfo(
    const ccGlobalShiftManager::ShiftInfo &info) {
  try {
...
int ccShiftAndScaleCloudDlg::addShiftInfo(
    const std::vector<ccGlobalShiftManager::ShiftInfo> &infos) {

...
bool BinFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
static int ReadEntityHeader(QFile &in, unsigned &numberOfPoints,
                            HeaderFlags &header) {
  assert(in.isOpen());
...
CC_FILE_ERROR BinFilter::saveToFile(ccHObject *root, const QString &filename,
  ...
  // concurrent call
  QFuture<CC_FILE_ERROR> future =
      QtConcurrent::run([&]() { return BinFilter::SaveFileV2(out, root); });
...
CC_FILE_ERROR BinFilter::SaveFileV2(QFile &out, ccHObject *object) {
  ...
  while (!toCheck.empty()) {
    ...
    // we check objects that have links to other entities (meshes, polylines,
    // etc.)
    std::unordered_set<const ccHObject *> dependencies;
    if (currentObject->isA(CC_TYPES::MESH) ||
        currentObject->isKindOf(CC_TYPES::PRIMITIVE)) {
      ...
      CCCoreLib::GenericIndexedCloudPersist *cloud =
          static_cast<ccPolyline *>(currentObject)->getAssociatedCloud();
      ccPointCloud *pc = dynamic_cast<ccPointCloud *>(cloud);
      if (pc)
        dependencies.insert(pc);
      else
        ccLog::Warning(QString("[BIN] Poyline '%1' is associated to an "
    ...
    for (std::unordered_set<const ccHObject *>::const_iterator it =
      ...
      if (!object->find((*it)->getUniqueID())) {
        ccLog::Warning(QString("[BIN] Dependency broken: entity '%1' must also "
                               "be in selection in order to save '%2'")
                           .arg((*it)->getName(), currentObject->getName()));
  ...
  char firstBytes[5] = "CCB2";
  {
    ...
    flags |= static_cast<char>(
        ccSerializableObject::DF_SCALAR_VAL_32_BITS); // internal representation
                                                      // of scalar fields is now
                                                      // always floats
    assert(flags <= 8);
  ...
  {
    ccLog::Print(QString("[BIN] Output file version: %1.%2 (automatically "
...
CC_FILE_ERROR BinFilter::loadFile(const QString &filename, ccHObject &container,
  ...
  if (v1) {
    ...
    int flags = 0;
    {
      ...
      if (!ok || flags > 8) {
        ccLog::Error(
            QString("Invalid file header (4th byte is '%1'?!)").arg(c));
...
inline bool Match(ccHObject *object, unsigned uniqueID,
...
ccHObject *FindRobust(ccHObject *root, ccHObject *source,
                      ...
                      unsigned oldUniqueID, CC_CLASS_ENUM expectedType) {
...
static bool ContinueAfterError(bool &forceLoadAfterError,
  ...
  if (!forceLoadAfterError) {
    // If forceLoadAfterError, it means we haven't asked the question yet, so
    // let's do it
    if (QMessageBox::Yes ==
        QMessageBox::critical(
            nullptr, QObject::tr("Reading error"),
            couldBeAMemoryIssue ? "The file couldn't be completely loaded, but "
                                  "some entities were loaded.\nDo you want to "
                                  "take the risk to load them? (CC could crash)"
                                : "The file seems corrupted, but some entities "
                                  "were loaded.\nDo you want to take the risk "
                                  "to load them? (CC could crash)",
...
CC_FILE_ERROR BinFilter::LoadFileV2(QFile &in, ccHObject &container, int flags,
  ...
  QString scalarFormat =
      ((flags & ccSerializableObject::DF_SCALAR_VAL_32_BITS) ? "float"
                                                             : "double");
  ccLog::Print(QString("[BIN] Version %1.%2 (coords: %3 / scalar: %4)")
                   .arg(binVersion / 10)
                   .arg(binVersion % 10)
                   .arg(coordsFormat)
                   .arg(scalarFormat));
  ...
  if (ccObject::GetCurrentDBVersion() < binVersion) {
    ccLog::Error("This version of CloudCompare is too old and can't load this "
                 "file, sorry");
    return CC_FERR_CONSOLE_ERROR;
  }

  // we read the first entity type
  CC_CLASS_ENUM classID =
  ...
  if (classID == CC_TYPES::CUSTOM_H_OBJECT) {
    ...
    root->fromFileNoChildren(
        in, static_cast<short>(binVersion), flags,
        oldToNewIDMap);    // this will load it, should be pretty quick
  ...
  if (parallel) {
    ...
    while (!future.isFinished()) {
      ...
      // pDlg.setValue(static_cast<int>(in.pos())); //DGM: in fact, the file
      // reading part is just half of the work!
      QApplication::processEvents();
    }
  ...
  if (!success) {
    ...
    ccLog::Error(QString("Failed to read file (file position: %1 / %2")
                     .arg(in.pos())
                     .arg(in.size()));
  ...
  while (!toCheck.empty()) {
    ...
    if (currentObject->isKindOf(CC_TYPES::MESH)) {
      ...
      if (currentObject->isA(CC_TYPES::MESH_GROUP)) {
        // TODO
        ccLog::Warning(QString("[BIN] Mesh groups are deprecated! Entity %1 "
                               "should be ignored...")
                           .arg(currentObject->getName()));
      } else if (currentObject->isA(CC_TYPES::SUB_MESH)) {
        ...
        if (meshID > 0) {
          ccHObject *mesh =
              FindRobust(root, subMesh, oldToNewIDMap, meshID, CC_TYPES::MESH);
          if (mesh) {
            subMesh->setAssociatedMesh(
                ccHObjectCaster::ToMesh(mesh),
                false); //'false' because previous mesh
                        // is not null (= real mesh ID)!!!
          } else {
            // we have a problem here ;)
            // normally, the associated mesh should be the sub-mesh's parent!
            if (subMesh->getParent() &&
                subMesh->getParent()->isA(CC_TYPES::MESH)) {
              subMesh->setAssociatedMesh(
                  ccHObjectCaster::ToMesh(subMesh->getParent()),
                  false); //'false' because previous mesh is not null (= real
                          // mesh ID)!!!
            } else {
              subMesh->setAssociatedMesh(
                  nullptr, false); //'false' because previous mesh is not null
                                   //(= real mesh ID)!!!
              // DGM: can't delete it, too dangerous (bad pointers ;)
              // delete subMesh;
              ccLog::Warning(QString("[BIN] Couldn't find associated mesh "
                                     "(ID=%1) for sub-mesh '%2' in the file!")
                                 .arg(meshID)
                                 .arg(subMesh->getName()));
      ...
      } else if (currentObject->isA(CC_TYPES::MESH) ||
                 currentObject->isKindOf(
                     CC_TYPES::PRIMITIVE)) // CC_TYPES::MESH or
                                           // CC_TYPES::PRIMITIVE!
      {
        ccMesh *mesh = ccHObjectCaster::ToMesh(currentObject);
        assert(mesh);
        ...
        if (cloudID > 0) {
          ccHObject *cloud = FindRobust(root, mesh, oldToNewIDMap, cloudID,
          ...
          if (cloud) {
            ...
            ccLog::Warning(QString("[BIN] Couldn't find vertices (ID=%1) for "
                                   "mesh '%2' in the file!")
                               .arg(cloudID)
                               .arg(mesh->getName()));
          ...
          ccLog::Warning(
              QString("[BIN] Mesh '%1' has no vertices! It will be ignored.")
                  .arg(mesh->getName()));
          if (mesh->getParent()) {
            mesh->getParent()->removeChild(mesh);
          } else {
        ...
        if (mesh) {
          ...
          if (matSetID > 0) {
            materials = FindRobust(root, mesh, oldToNewIDMap, matSetID,
                                   CC_TYPES::MATERIAL_SET);
            if (materials) {
              mesh->setMaterialSet(static_cast<ccMaterialSet *>(materials),
                                   false);
            } else {
              // we have a (less severe) problem here ;)
              mesh->setMaterialSet(nullptr, false);
              mesh->showMaterials(false);
              ccLog::Warning(QString("[BIN] Couldn't find shared materials set "
                                     "(ID=%1) for mesh '%2' in the file!")
                                 .arg(matSetID)
                                 .arg(mesh->getName()));
              result = CC_FERR_BROKEN_DEPENDENCY_ERROR;
            }
          ...
          if (triNormsTableID > 0) {
            triNormsTable =
                FindRobust(root, mesh, oldToNewIDMap, triNormsTableID,
                           CC_TYPES::NORMAL_INDEXES_ARRAY);
            if (triNormsTable) {
              mesh->setTriNormsTable(
                  static_cast<NormsIndexesTableType *>(triNormsTable), false);
            } else {
              // we have a (less severe) problem here ;)
              mesh->setTriNormsTable(nullptr, false);
              mesh->showTriNorms(false);
              ccLog::Warning(QString("[BIN] Couldn't find shared normals "
                                     "(ID=%1) for mesh '%2' in the file!")
                                 .arg(triNormsTableID)
                                 .arg(mesh->getName()));
              result = CC_FERR_BROKEN_DEPENDENCY_ERROR;
          ...
          if (texCoordArrayID > 0) {
            texCoordsTable =
                FindRobust(root, mesh, oldToNewIDMap, texCoordArrayID,
                           CC_TYPES::TEX_COORDS_ARRAY);
            if (texCoordsTable) {
              mesh->setTexCoordinatesTable(
                  static_cast<TextureCoordsContainer *>(texCoordsTable), false);
            } else {
              // we have a (less severe) problem here ;)
              mesh->setTexCoordinatesTable(nullptr, false);
              ccLog::Warning(
                  QString("[BIN] Couldn't find shared texture coordinates "
                          "(ID=%1) for mesh '%2' in the file!")
                      .arg(texCoordArrayID)
                      .arg(mesh->getName()));
              result = CC_FERR_BROKEN_DEPENDENCY_ERROR;
          ...
          if (checkErrors) {
            ...
            for (unsigned i = 0; i < faceCount; ++i) {
              ...
              if (tri->i1 >= vertCount || tri->i2 >= vertCount ||
                  tri->i3 >= vertCount) {
                ccLog::Warning(QString("[BIN] File is corrupted: missing "
                                       "vertices for mesh '%1'!")
                                   .arg(mesh->getName()));
                ...
                orphans->addChild(pc);
                if (texCoordsTable) {
                  texCoordsTable->setName(mesh->getName() + QString(".") +
                                          texCoordsTable->getName());
                  orphans->addChild(texCoordsTable);
                }
                if (triNormsTable) {
                  triNormsTable->setName(mesh->getName() + QString(".") +
                                         triNormsTable->getName());
                  orphans->addChild(triNormsTable);
                }
                if (materials) {
                  materials->setName(mesh->getName() + QString(".") +
                                     materials->getName());
      ...
      ccHObject *cloudEntity =
          FindRobust(root, poly, oldToNewIDMap, cloudID, CC_TYPES::POINT_CLOUD);
      if (cloudEntity) {
        ...
        for (unsigned i = 0; i < poly->size(); ++i) {
          if (poly->getPointGlobalIndex(i) >= pointCount) {
            ccLog::Warning(
                QString("[BIN] Polyline '%1' (ID=%2) seems corrupted!")
                    .arg(poly->getName())
      ...
      if (bufferID > 0) {
        ccHObject *buffer = FindRobust(root, sensor, oldToNewIDMap, bufferID,
                                       CC_TYPES::TRANS_BUFFER);
        if (buffer) {
          ...
          currentObject = nullptr;
          ccLog::Warning(QString("[BIN] Couldn't find trans. buffer (ID=%1) "
                                 "for sensor '%2' in the file!")
                             .arg(bufferID)
                             .arg(sensor->getName()));

          // positions are optional, so we can simply set them to nullptr and go
          // ahead, we do not need to return. return CC_FERR_MALFORMED_FILE;
        }
      }
    } else if (currentObject->isA(CC_TYPES::LABEL_2D)) {
      ...
      for (unsigned i = 0; !invalidLabel && i < label->size(); ++i) {
        ...
        if (pp._cloud) {
          ...
          ccHObject *cloud = FindRobust(root, label, oldToNewIDMap, cloudID,
                                        CC_TYPES::POINT_CLOUD);
          if (cloud) {
            ...
            // we have a problem here ;)
            ccLog::Warning(
                QString("[BIN] Couldn't find cloud (ID=%1) associated to label "
                        "ID=%2 in the file!")
                    .arg(cloudID)
                    .arg(label->getUniqueID())); // we can't call getName as it
                                                 // relies on the cloud/mesh
                                                 // pointers!
            if (label->getParent()) {
          ...
          ccHObject *mesh =
              FindRobust(root, label, oldToNewIDMap, meshID, CC_TYPES::MESH);
          if (mesh) {
            ccGenericMesh *genMesh = ccHObjectCaster::ToGenericMesh(mesh);
            assert(genMesh->size() > pp.index);
            correctedPickedPoints.push_back(cc2DLabel::PickedPoint(
                genMesh, pp.index, pp.uv, pp.entityCenterPoint));
          } else {
            // we have a problem here ;)
            ccLog::Warning(
                QString("[BIN] Couldn't find mesh (ID=%1) associated to label "
                        "ID=%2 in the file!")
                    .arg(meshID)
                    .arg(label->getUniqueID())); // we can't call getName as it
                                                 // relies on the cloud/mesh
      ...
      // origin points
      {
        ...
        if (cloudID > 0) {
          ccHObject *cloud = FindRobust(root, facet, oldToNewIDMap, cloudID,
                                        CC_TYPES::POINT_CLOUD);
          if (cloud && cloud->isA(CC_TYPES::POINT_CLOUD)) {
            ...
            currentObject = nullptr;
            ccLog::Warning(QString("[BIN] Couldn't find origin points (ID=%1) "
                                   "for facet '%2' in the file!")
                               .arg(cloudID)
                               .arg(facet->getName()));
          }
        }
      ...
      // contour points
      {
        ...
        if (cloudID > 0) {
          ...
          if (cloud) {
            ...
            currentObject = nullptr;
            ccLog::Warning(QString("[BIN] Couldn't find contour points (ID=%1) "
                                   "for facet '%2' in the file!")
                               .arg(cloudID)
                               .arg(facet->getName()));
          }
      ...
      // contour polyline
      {
        ...
        if (polyID > 0) {
          ccHObject *poly = FindRobust(root, facet, oldToNewIDMap, polyID,
                                       CC_TYPES::POLY_LINE);
          if (poly) {
            ...
            facet->setContourVertices(nullptr);
            currentObject = nullptr;
            ccLog::Warning(QString("[BIN] Couldn't find contour polyline "
      ...
      // polygon mesh
      {
        ...
        if (polyID > 0) {
          ccHObject *poly =
              FindRobust(root, facet, oldToNewIDMap, polyID, CC_TYPES::MESH);
          if (poly) {
            ...
            currentObject = nullptr;
            ccLog::Warning(QString("[BIN] Couldn't find polygon mesh (ID=%1) "
                                   "for facet '%2' in the file!")
                               .arg(polyID)
                               .arg(facet->getName()));
          }
      ...
      if (sensorID > 0) {
        ccHObject *sensor = FindRobust(root, image, oldToNewIDMap, sensorID,
                                       CC_TYPES::CAMERA_SENSOR);
        if (sensor) {
          ...
          currentObject = nullptr;
          ccLog::Warning(QString("[BIN] Couldn't find camera sensor (ID=%1) "
    ...
    if (currentObject) {
      ...
      if (shifted) {
        // it may be interesting to re-use the Global Shift when loading other
        // files
        ccGlobalShiftManager::StoreShift(shifted->getGlobalShift(),
                                         shifted->getGlobalScale());

        // TODO: we should also check that other entities with global shift not
        // too far away have not already been loaded. In which case we should
...
CC_FILE_ERROR BinFilter::LoadFileV1(QFile &in, ccHObject &container,
  ...
  if (nbScansTotal > 99) {
    if (QMessageBox::question(
            nullptr, QString("Oops"),
            QString("Hum, do you really expect to load %1 point clouds?")
                .arg(nbScansTotal),
  ...
  for (unsigned k = 0; k < nbScansTotal; k++) {
    ...
    if (header.name) {
      for (int i = 0; i < 256; ++i) {
        if (in.read(cloudName + i, 1) < 0) {
          // Console::print("[BinFilter::loadModelFromBinaryFile] Error reading
          // the cloud name!\n");
          return CC_FERR_READING;
        }
        if (cloudName[i] == 0) {
    ...
    if (header.scalarField) {
      if (loadedCloud->enableScalarField()) {
        ...
        ccLog::Warning(QString("Failed to allocate scalar field on cloud '%1'")
    ...
    for (unsigned i = 0; i < nbOfPoints; ++i) {
      if (lineRead == fileChunkPos + fileChunkSize) {
        ...
        fileChunkSize =
            std::min(nbOfPoints - lineRead, CC_MAX_NUMBER_OF_POINTS_PER_CLOUD);
        ...
        if (header.normals) {
          if (loadedCloud->reserveTheNormsTable()) {
            ...
            ccLog::Warning(QString("Failed to allocate normals on cloud '%1'")
                               .arg(loadedCloud->getName()));
            delete loadedCloud;
            return CC_FERR_NOT_ENOUGH_MEMORY;
          }

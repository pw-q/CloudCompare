...
bool DepthMapFileFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
CC_FILE_ERROR DepthMapFileFilter::saveToFile(ccHObject *entity,
  ...
  for (size_t i = 0; i < sensorCount && result == CC_FERR_NO_ERROR; ++i) {
    ...
    QString sensorFilename =
        (sensorCount > 1
             ? QString("%1_%2.%3").arg(baseName).arg(i).arg(extension)
             : filename);
...
CC_FILE_ERROR DepthMapFileFilter::saveToFile(const QString &filename,
  ...
  if (db.zBuff.empty()) {
    ccLog::Warning(QString("[DepthMap] sensor '%1' has no associated depth map "
                           "(you must compute it first)")
                       .arg(sensor->getName()));
    return CC_FERR_NO_SAVE; // this is not a severe error (the process can go
                            // on)
  }
  ...
  if (!cloud) {
    ccLog::Warning(
        QString("[DepthMap] sensor '%1' is not associated to a point cloud!")
            .arg(sensor->getName()));
  ...
  if (CheckForSpecialChars(filename)) {
    ccLog::Warning(
        QString("[DepthMap] Output filename contains special characters. It "
  ...
  if (!fp) {
    ccLog::Error(
        QString("[DepthMap] Can't open file '%1' for writing!").arg(filename));
  ...
  if (cloud && cloud->isA(CC_TYPES::POINT_CLOUD)) {
    ...
    if (nbPoints == 0) {
      ccLog::Warning(
          QString("[DepthMap] sensor '%1' is associated to an empty cloud?!")
              .arg(sensor->getName()));
      // this is not a severe error (the process can go on)
    } else {
      ...
      if (pc->hasNormals()) {
        ...
        try {
          ...
          ccLog::Warning(QString("[DepthMap] not enough memory to load normals "
                                 "on sensor '%1'!")
                             .arg(sensor->getName()));
        }
      }
      ...
      if (pc->hasColors()) {
        try {
          ...
          ccLog::Warning(
              QString(
                  "[DepthMap] not enough memory to load colors on sensor '%1'!")
                  .arg(sensor->getName()));
        }
      }
    }

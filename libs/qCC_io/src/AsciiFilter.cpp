...
void AsciiFilter::SetNoLabelCreated(bool state) { s_doNotCreateLabels = state; }
...
bool AsciiFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
                          bool &exclusive) const {
  if (type == CC_TYPES::POINT_CLOUD          // only one cloud per file
      || type == CC_TYPES::HIERARCHY_OBJECT) // but we can also save a group
                                             // (each cloud inside will be saved
                                             // as a separated file)
  {
    multiple = true;
    exclusive = true;
        ...
        for (unsigned i = 0; i < count; ++i) {
          ...
          if (child->isKindOf(CC_TYPES::POINT_CLOUD)) {
            ...
            subFilename += QString(baseName).replace(
                "cloudname", child->getName(), Qt::CaseInsensitive);
            counter++;
            assert(counter <= cloudCount);
            subFilename +=
                QString("_%1").arg(cloudCount - counter, 6, 10, QChar('0'));
            if (!extension.isEmpty())
              subFilename += QString(".") + extension;

            CC_FILE_ERROR result =
                saveToFile(entity->getChild(i), subFilename, parameters);
            if (result != CC_FERR_NO_ERROR) {
              return result;
            } else {
              ccLog::Print(QString("[ASCII] Cloud '%1' has been saved in: %2")
                               .arg(child->getName(), subFilename));
            }
          } else {
            ccLog::Warning(
                QString("[ASCII] Entity '%1' can't be saved this way!")
                    .arg(child->getName()));
          }
        }
  ...
  for (unsigned i = 0; i < numberOfPoints; ++i) {
    ...
    if (writeColors) {
      ...
      if (saveFloatColors) {
        ...
        colorLine.append(
            QString::number(static_cast<double>(col.r) / ccColor::MAX));
        colorLine.append(separator);
        colorLine.append(
            QString::number(static_cast<double>(col.g) / ccColor::MAX));
        colorLine.append(separator);
        colorLine.append(
            QString::number(static_cast<double>(col.b) / ccColor::MAX));
        if (saveAlphaChannel) {
          colorLine.append(separator);
          colorLine.append(
              QString::number(static_cast<double>(col.a) / ccColor::MAX));
        }
      } else {
        colorLine.append(separator);
...
CC_FILE_ERROR AsciiFilter::loadStream(QTextStream &stream,
                                      QString filenameOrTitle, qint64 dataSize,
                                      ccHObject &container,
                                      LoadParameters &parameters) {
  if (dataSize == 0) {
  ...
  return loadCloudFromFormatedAsciiStream(
      ...
      skipLineCount, quaternionScale, parameters, showLabelsIn2D);
}
...
prepareCloud(const AsciiOpenDlg::Sequence &openSequence,
             unsigned numberOfPoints, int &maxIndex, unsigned step = 1) {
  ...
  for (int i = 0; i < seqSize; ++i) {
    ...
    case ASCII_OPEN_DLG_Scalar: {
      ...
      if (sfIdx >= 0) {
        ...
        ccLog::Warning("Failed to add scalar field #%i to cloud #%i! (skipped)",
                       sfIndex);
        sf->release();
        sf = nullptr;
      }
...
CC_FILE_ERROR AsciiFilter::loadCloudFromFormatedAsciiStream(
    QTextStream &stream, QString filenameOrTitle, ccHObject &container,
    const AsciiOpenDlg::Sequence &openSequence, char separator,
    bool commaAsDecimal, unsigned approximateNumberOfLines, qint64 fileSize,
    unsigned maxCloudSize, unsigned skipLines, double quaternionScale,
    LoadParameters &parameters, bool showLabelsIn2D /*=false*/) {
  ...
  while (true) {
    ...
    if (pointsRead == nextLimit) {
      ccLog::PrintDebug("[ASCII] Point %i -> end of chunk (%i points)",
                        pointsRead, cloudChunkSize);

      // we re-evaluate the average line size
      {
        double averageLineSize =
            static_cast<double>(charactersRead) / (pointsRead + skipLines);
        double newNbOfLinesApproximation =
            std::max(1.0, static_cast<double>(fileSize) / averageLineSize -
        ...
        if (newNbOfLinesApproximation <= pointsRead) {
          newNbOfLinesApproximation = std::max(
              static_cast<double>(cloudChunkPos + cloudChunkSize) + 1.0,
              static_cast<double>(pointsRead) * 1.02);
        }
        approximateNumberOfLines =
            static_cast<unsigned>(ceil(newNbOfLinesApproximation));
        ccLog::PrintDebug("[ASCII] New approximate nb of lines: %i",
                          approximateNumberOfLines);
      }
      ...
      if (cloudChunkSize < maxCloudSize ||
        ...
        cloudChunkSize =
            std::min(maxCloudSize, approximateNumberOfLines - cloudChunkPos);
      ...
      } else // otherwise we have to create new clouds
      {
        ...
        if (!cloudDesc.cloud->resize(cloudChunkSize))
          ccLog::Warning("Memory reallocation failed ... some memory may have "
                         "been wasted ...");
        ...
        cloudChunkSize =
            std::min(maxCloudSize, approximateNumberOfLines - cloudChunkPos);
        cloudDesc = prepareCloud(openSequence, cloudChunkSize, maxPartIndex,
                                 ++chunkRank);
        if (!cloudDesc.cloud) {
          ccLog::Error("Not enough memory! Process stopped ...");
          break;
    ...
    QStringList parts =
        currentLine.simplified().split(separator, QString::SkipEmptyParts);
    ...
    if (nParts > maxPartIndex) // fake loop for easy break
    {
      ...
      if (lineIsCorrupted) {
        ccLog::Warning("[AsciiFilter::Load] Line %i is corrupted (non "
      ...
      if (pointsRead == 0) {
        if (HandleGlobalShift(P, Pshift, preserveCoordinateShift, parameters)) {
          ...
          ccLog::Warning("[ASCIIFilter::loadFile] Cloud has been recentered! "
      ...
      if (cloudDesc.hasRGBColors) {
        if (cloudDesc.iRgbaIndex >= 0) {
          ...
          if (cloudDesc.redIndex >= 0) {
            float multiplier = cloudDesc.hasFloatRGBColors[0]
                                   ? static_cast<float>(ccColor::MAX)
                                   : 1.0f;
            col.r = static_cast<ColorCompType>(
                locale.toFloat(parts[cloudDesc.redIndex]) * multiplier);
          }
          if (cloudDesc.greenIndex >= 0) {
            float multiplier = cloudDesc.hasFloatRGBColors[1]
                                   ? static_cast<float>(ccColor::MAX)
                                   : 1.0f;
            col.g = static_cast<ColorCompType>(
                locale.toFloat(parts[cloudDesc.greenIndex]) * multiplier);
          }
          if (cloudDesc.blueIndex >= 0) {
            float multiplier = cloudDesc.hasFloatRGBColors[2]
                                   ? static_cast<float>(ccColor::MAX)
                                   : 1.0f;
            col.b = static_cast<ColorCompType>(
                locale.toFloat(parts[cloudDesc.blueIndex]) * multiplier);
          }
          if (cloudDesc.alphaIndex >= 0) {
            float multiplier = cloudDesc.hasFloatRGBColors[3]
                                   ? static_cast<float>(ccColor::MAX)
                                   : 1.0f;
            col.a = static_cast<ColorCompType>(
                locale.toFloat(parts[cloudDesc.alphaIndex]) * multiplier);
          }
        }
        cloudDesc.cloud->addColor(col);
      } else if (cloudDesc.greyIndex >= 0) {
        col.r = col.g = col.b =
            static_cast<ColorCompType>(parts[cloudDesc.greyIndex].toInt());
        col.a = ccColor::MAX;
        cloudDesc.cloud->addColor(col);
      }
      ...
      if (cloudDesc.hasQuaternion) {
        ...
        ccCoordinateSystem *cs = new ccCoordinateSystem(
            ccCoordinateSystem::DEFAULT_DISPLAY_SCALE, CCCoreLib::PC_ONE, &mat,
      ...
      ccLog::Warning("[AsciiFilter::Load] Line %i is corrupted (found %i "
  ...
  if (cloudDesc.cloud) {
    ...
    if (!cloudDesc.scalarFields.empty()) {
      for (size_t j = 0; j < cloudDesc.scalarFields.size(); ++j) {
        if (cloudDesc.scalarFields[j]->resizeSafe(cloudDesc.cloud->size(), true,
                                                  CCCoreLib::NAN_VALUE)) {
      ...
      if (labels.size() > 1) {
        ...
        for (size_t i = 0; i < labels.size(); ++i) {
          ...
          static_cast<cc2DLabel *>(labels[i])->setPosition(
              0.5 + 0.4 * cos(i * angle_rad), 0.5 + 0.4 * sin(i * angle_rad));
        }
      }
    }

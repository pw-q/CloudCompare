  ...
  try {
    // standard combos
    {
      ...
      for (size_t i = 0; i < m_standardCombos.size(); ++i) {
        if (m_standardCombos[i] && m_standardCombos[i]->currentIndex() >
...
bool PlyOpenDlg::restoreContext(PlyLoadingContext *context,
                                int &unassignedProps, int &mismatchProps) {
    ...
    for (size_t i = 0; i < m_singleCombos.size(); ++i) {
      if (m_singleCombos[i]) {
        ...
        if (!context->singleCombosProperties[i].isNull()) {
          // try to find it in the new property list!
          int idx =
              m_singleCombos[i]->findText(context->singleCombosProperties[i]);
          if (idx >= 0) {
            ++assignedEntries;
  ...
  for (int i = 1; i < n + p + q; ++i) {
    if (assignedIndexCount[i] > 1) {
      if (displayErrors)
        QMessageBox::warning(
            nullptr, "Error",
            QString("Can't assign same property to multiple fields! (%1)")

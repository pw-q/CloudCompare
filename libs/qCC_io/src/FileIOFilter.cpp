...
FileIOFilter::FileIOFilter(const FilterInfo &info) : m_filterInfo(info) {
#ifdef QT_DEBUG
...
void FileIOFilter::checkFilterInfo() const {
  ...
  if (m_filterInfo.features & Import) {
    if (m_filterInfo.importFileFilterStrings.isEmpty()) {
      ccLog::Warning(
          QStringLiteral(
              "I/O filter marked as import, but no filter strings set: %1")
              .arg(m_filterInfo.id));
    }
    ...
    if (m_filterInfo.importExtensions.isEmpty()) {
      ccLog::Warning(
          QStringLiteral(
              "I/O filter marked as import, but no extensions set: %1")
              .arg(m_filterInfo.id));
    }
  } else {
    if (!m_filterInfo.importFileFilterStrings.isEmpty()) {
      ccLog::Warning(
          QStringLiteral(
              "I/O filter not marked as import, but filter strings are set: %1")
              .arg(m_filterInfo.id));
    }
    ...
    if (!m_filterInfo.importExtensions.isEmpty()) {
      ccLog::Warning(
          QStringLiteral(
              "I/O filter not marked as import, but extensions are set: %1")
              .arg(m_filterInfo.id));
    }
  }
  ...
  if (m_filterInfo.features & Export) {
    if (m_filterInfo.exportFileFilterStrings.isEmpty()) {
      ccLog::Warning(
          QStringLiteral(
              "I/O filter marked as export, but no filter strings set: %1")
              .arg(m_filterInfo.id));
    }
  } else {
    if (!m_filterInfo.exportFileFilterStrings.isEmpty()) {
      ccLog::Warning(
          QStringLiteral(
              "I/O filter not marked as export, but filter strings are set: %1")
              .arg(m_filterInfo.id));
    }
  }
#endif
...
void FileIOFilter::Register(Shared filter) {
  ...
  if (std::any_of(s_ioFilters.cbegin(), s_ioFilters.cend(), compareFilters)) {
    ccLog::Warning(
        QStringLiteral(
            "[FileIOFilter] I/O filter already registered with id '%1'")
            .arg(id));
...
FileIOFilter::Shared
FileIOFilter::FindBestFilterForExtension(const QString &ext) {
...
ccHObject *FileIOFilter::LoadFromFile(const QString &filename,
                                      ...
                                      Shared filter, CC_FILE_ERROR &result) {
  if (!filter) {
    ccLog::Error(
        QString("[Load] Internal error (invalid input filter)").arg(filename));
  ...
  try {
    ...
    ccLog::Warning(
        QString("[I/O] CC has caught an exception while loading file '%1'")
            .arg(filename));
    ...
    ccLog::Warning(QString("[I/O] CC has caught an unhandled exception while "
                           "loading file '%1'")
                       .arg(filename));
    if (container) {
      container->removeAllChildren();
  ...
  if (childCount != 0) {
    ...
    for (unsigned i = 0; i < childCount; ++i) {
      ...
      if (newName.startsWith("unnamed")) {
        // we automatically replace occurrences of 'unnamed' in entities names
        // by the base filename (no path, no extension)
...
ccHObject *FileIOFilter::LoadFromFile(const QString &inputFilename,
  ...
  if (!fileFilter.isEmpty()) {
    ...
    if (!filter) {
      ccLog::Error(
          QString(
              "[Load] Internal error: no I/O filter corresponds to filter '%1'")
              .arg(fileFilter));
      result = CC_FERR_CONSOLE_ERROR;
      return nullptr;
    }
  } else // we need to guess the I/O filter based on the file format
  {
    ...
    if (!filter) {
      ccLog::Error(
          QString(
              "[Load] Can't guess file format: unhandled file extension '%1'")
              .arg(extension));
      result = CC_FERR_CONSOLE_ERROR;
      return nullptr;
    }
...
CC_FILE_ERROR FileIOFilter::SaveToFile(ccHObject *entities,
  ...
  try {
    ...
    ccLog::Warning(
        QString(
            "[I/O] CC has caught an unhandled exception while saving file '%1'")
            .arg(filename));
    result = CC_FERR_CONSOLE_ERROR;
  }
...
CC_FILE_ERROR FileIOFilter::SaveToFile(ccHObject *entities,
  ...
  if (!filter) {
    ccLog::Error(
        QString("[Load] Internal error: no filter corresponds to filter '%1'")
            .arg(fileFilter));
...
void FileIOFilter::DisplayErrorMessage(CC_FILE_ERROR err, const QString &action,
                                       const QString &filename) {
  QString errorStr;
  ...
  case CC_FERR_FILE_WAS_WRITTEN_BY_UNKNOWN_PLUGIN:
    errorStr = QObject::tr("the file was written by a plugin but none of the "
                           "loaded plugins can deserialize it");
    break;
  case CC_FERR_THIRD_PARTY_LIB_FAILURE:
    errorStr =
        QObject::tr("the third-party library in charge of saving/loading the "
                    "file has failed to perform the operation");
    break;
  case CC_FERR_THIRD_PARTY_LIB_EXCEPTION:
    errorStr = QObject::tr("the third-party library in charge of "
                           "saving/loading the file has thrown an exception");
    break;
...
bool FileIOFilter::HandleGlobalShift(
    const CCVector3d &P, CCVector3d &Pshift, bool &preserveCoordinateShift,
    LoadParameters &loadParameters,
    bool useInputCoordinatesShiftIfPossible /*=false*/) {
  bool shiftAlreadyEnabled =
      ((nullptr != loadParameters._coordinatesShiftEnabled) &&
  ...
  if (shiftAlreadyEnabled) {
    ...
    if (nullptr != loadParameters._coordinatesShiftForced &&
        *loadParameters._coordinatesShiftForced) {
  ...
  if (sizeof(PointCoordinateType) < 8 &&
      ccGlobalShiftManager::Handle(
          P, 0.0, loadParameters.shiftHandlingMode,
          shiftAlreadyEnabled || useInputCoordinatesShiftIfPossible, Pshift,
          &preserveCoordinateShift, nullptr, &applyAll)) {
    // we save coordinates shift information
    if (applyAll ||
        loadParameters.shiftHandlingMode ==
            ccGlobalShiftManager::
                NO_DIALOG_AUTO_SHIFT) // in command line mode, with the 'AUTO
                                      // GLOBAL SHIFT' option, we want to
                                      // retrieve the applied coordinate shift
    {
      ...
      if (applyAll && nullptr != loadParameters._coordinatesShiftForced) {
        ...
        if (needShift) {
          // if the user has clicked on 'apply all' while the inupt shift is not
          // sufficient, this means that this shift needs to be forced to all
          // the entities! (otherwise, if the shift is sufficient, then it will
          // probably be ok for the next entities)

...
QString ImageFileFilter::GetSaveFilename(const QString &dialogTitle,
  ...
  if (formats.empty()) {
    ccLog::Error(
        ...
        "'imageformats' directory is alongside the application executable)");
    return QString();
  }
  ...
  QString outputFilename = QFileDialog::getSaveFileName(
      ...
      imageSavePath +
          QString("/%1.%2").arg(baseName, pngFilter.isEmpty()
                                              ? QString(formats[0].data())
                                              : QString("png")),
      filters, pngFilter.isEmpty() ? nullptr : &pngFilter);
...
QString ImageFileFilter::GetLoadFilename(const QString &dialogTitle,
  ...
  return QFileDialog::getOpenFileName(parentWidget, dialogTitle, imageLoadPath,
                                      imageFilter);
}

bool ImageFileFilter::canSave(CC_CLASS_ENUM type, bool &multiple,

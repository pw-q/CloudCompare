...
static int32_t SizeofPolyLine16Bits(ESRI_SHAPE_TYPE polylineType, bool hasSF,
                                    int32_t numPoints, int32_t numParts = 1) {
  switch (polylineType) {
  case ESRI_SHAPE_TYPE::POLYLINE:
  case ESRI_SHAPE_TYPE::POLYLINE_M:
  case ESRI_SHAPE_TYPE::POLYLINE_Z:
...
static int32_t SizeofMultipatch16Bits(unsigned numPoints,
                                      int32_t numParts = 1) {
  size_t recordSize8bits = 0;
  recordSize8bits += 4;                                      // ShapeType
...
static bool AreVerticesCounterClockwise(
    const CCCoreLib::GenericIndexedCloudPersist *vertices, int32_t numPoints,
    unsigned char dim1, unsigned char dim2) {
  ...
  for (int32_t i = 0; i + 1 < numPoints; ++i) {
    ...
    sum += static_cast<double>(P2.u[dim1] - P1.u[dim1]) *
           (P2.u[dim2] + P1.u[dim2]);
  }
...
static CCVector2d
MinMaxOfEnabledScalarField(const CCCoreLib::GenericIndexedCloudPersist *cloud) {
...
static ccHObject::GlobalBoundingBox
BBoxOfHObjectContainer(const ccHObject::Container &objects) {
...
static CCVector2d MRangeOfContainer(ccHObject::Container &objects) {
  ...
  auto updateRange =
      [&range](const CCCoreLib::GenericIndexedCloudPersist *cloud) {
  ...
  for (ccHObject *obj : objects) {
    ...
    case CC_TYPES::POLY_LINE: {
      ...
      const CCCoreLib::GenericIndexedCloudPersist *vertices =
...
CC_FILE_ERROR ShapeFileHeader::readFrom(QDataStream &sin) {
  ...
  if (fileCode != ESRI_SHAPE_FILE_CODE) {
    ccLog::Warning("[SHP] wrong file code (%d), is this a shape file?",
                   fileCode);
  ...
  if (!IsValidESRIShapeCode(shapeTypeInt)) {
    ccLog::Warning("[SHP] invalid shape type code in header (%d)",
                   shapeTypeInt);
    return CC_FERR_MALFORMED_FILE;
  }
...
bool ShpFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
                        bool &exclusive) const {
  if (type == CC_TYPES::POLY_LINE || type == CC_TYPES::POINT_CLOUD ||
      type == CC_TYPES::HIERARCHY_OBJECT || type == CC_TYPES::MESH) {
    multiple = true;
    exclusive = true;
    return true;
  }
    ...
    if (baseEntity->getChildrenNumber()) {
      ...
      for (unsigned i = 1; i < baseEntity->getChildrenNumber(); ++i) {
        if (baseEntity->getChild(i) &&
            baseEntity->getChild(i)->getClassID() != child->getClassID()) {
      ...
      for (unsigned i = 1; i < baseEntity->getChildrenNumber(); ++i) {
        ...
        if (otherShapeType != shapeType) {
          if (child)
            ccLog::Warning(
                QString("[SHP] Entity %1 has not the same type (%2) as the "
                        "others in the selection (%3)! Can't mix types...")
                    .arg(child->getName())
...
static CC_FILE_ERROR ReadParts(QDataStream &shpStream, int32_t numParts,
...
static CC_FILE_ERROR ReadPoints(QDataStream &shpStream, int32_t numPoints,
...
static CC_FILE_ERROR ReadMeasures(QDataStream &shpStream, int32_t numPoints,
  ...
  int32_t expectedSize =
      readSize +
      (2 + numPoints) *
          sizeof(double); // 2 bounding values + 'numPoints' measurements
  if (recordSize16bits * 2 >=
      expectedSize) // recordSize is expressed as a number of 16-bit words
  {
    // M boundaries
    double mMin;
    double mMax;
    ...
    if (readSize < recordSize16bits * 2) {
      // that would be strange, but the specifications don't say if everything
      // should be ignored, or if boundaries can be present without the values...
 ...
 **/
static ccPointCloud *BuildVertices(const std::vector<CCVector3> &points,
...
//! Creates the ccMesh
static ccMesh *CreateMesh(const std::vector<CCVector3> &points,
  ...
  if (!scalarValues.empty()) {
    ...
    if (firstIndex < scalarValues.size() && lastIndex < scalarValues.size()) {
      areAllValuesOfPartsNaNs = std::all_of(
          scalarValues.begin() + firstIndex, scalarValues.begin() + lastIndex,
...
**/
static CC_FILE_ERROR BuildPatches(ccHObject &container,
  ...
  for (int32_t i = 0; i < numParts; ++i) {
    if (!IsValidEsriPartType(partTypes[i])) {
      ccLog::Warning("[SHP] Multipatch part %d has an invalid part type (%d)",
                     i, partTypes[i]);
      continue;
    }
    ...
    default: {
      ccLog::Print(
          QString("[SHP] Cannot handle Patch of type: %1").arg(ToString(type)));
...
static CC_FILE_ERROR LoadMultiPatch(QDataStream &shpStream,
                                    ccHObject &container,
                                    int32_t recordSize16bits,
                                    int64_t recordStart, CCVector3d Pshift) {
  // skip record bbox
  ...
  std::vector<ScalarType> scalarValues;
  error = ReadMeasures(shpStream, numPoints, scalarValues, recordSize16bits,
                       recordStart);
  if (error != CC_FERR_NO_ERROR) {
    return error;
  }
 ...
 */
static void Save3DCloud(QDataStream &stream, const ccGenericPointCloud *cloud,
                        const CCVector3d &bbMinGlobal,
                        const CCVector3d &bbMaxGlobal) {
 ...
 * triangle strip
 */
static CC_FILE_ERROR FindTriangleOrganisation(ccMesh *mesh,
                                              ESRI_PART_TYPE &type) {
  const CCCoreLib::VerticesIndexes *firstTriangle =
      mesh->getNextTriangleVertIndexes();
  if (!IsTriangleFan(firstTriangle) && !IsTriangleStrip(firstTriangle)) {
    return CC_FERR_BAD_ENTITY_TYPE;
  }

  const CCCoreLib::VerticesIndexes *secondTriangle =
      mesh->getNextTriangleVertIndexes();
...
static CC_FILE_ERROR SaveMesh(ccMesh *mesh, QDataStream &stream,
                              int32_t recordNumber, int32_t &recordSize16bits) {
  ESRI_PART_TYPE triangleType;
  if (FindTriangleOrganisation(mesh, triangleType) == CC_FERR_BAD_ENTITY_TYPE) {
    ccLog::Warning("[SHP] A mesh has to be organized as a triangle fan or a "
                   "triangle trip (see MultiPatch geometry specifications)");
    return CC_FERR_BAD_ENTITY_TYPE;
  }
...
static CC_FILE_ERROR LoadPolyline(QDataStream &shpStream, ccHObject &container,
                                  int32_t index, ESRI_SHAPE_TYPE shapeType,
                                  int32_t recordSize16bits, int64_t recordStart,
                                  const CCVector3d &Pshift,
  ...
  for (int32_t i = 0; i < numParts; ++i) {
    ...
    bool isClosed = false;
    if (vertCount > 2 && CCCoreLib::LessThanEpsilon(
    ...
    if (!scalarValues.empty()) {
      bool allNaNs = std::all_of(scalarValues.begin() + firstIndex,
                                 scalarValues.begin() + lastIndex,
                                 [](ScalarType v) { return std::isnan(v); });
      if (!allNaNs) {
        ccScalarField *sf = new ccScalarField("Measures");
        if (!sf->reserveSafe(vertCount)) {
          ccLog::Warning(QString("[SHP] Polyline #%1.%2: not enough memory to "
                                 "load scalar values!")
                             .arg(index)
                             .arg(i + 1));
          sf->release();
...
static CC_FILE_ERROR SavePolyline(ccPolyline *poly, QDataStream &out,
                                  int32_t &recordSize16bits,
  ...
  case ESRI_SHAPE_TYPE::POLYGON_Z:
    if (realVertexCount < 3) {
      ccLog::Warning(QObject::tr("[SHP] Polyline %1 does not have enough "
  ...
  recordSize16bits =
      SizeofPolyLine16Bits(outputShapeType, hasSF, numPoints, numParts);
  ...
  for (int32_t i = 0; i < numPoints; ++i) {
    ...
    const CCVector3 *P = vertices->getPoint(
        ii % iRealVertexCount); // warning: handle loop if polyline is closed
  ...
  if (hasSF && HasMeasurements(
                   outputShapeType)) // M values (for each part - just one here)
  {
    ...
    for (int32_t i = 0; i < numPoints; ++i) {
      ...
      out << (ccScalarField::ValidValue(scalar) ? ESRI_NO_DATA
...
static CC_FILE_ERROR LoadCloud(QDataStream &shpStream, ccHObject &container,
                               int32_t index, ESRI_SHAPE_TYPE shapeType,
                               int32_t recordSize16bits, int64_t recordStart,
                               const CCVector3d &Pshift,
                               bool preserveCoordinateShift) {
        ...
        expectedSize) // recordSize is expressed as a number of 16-bit words
    {
      ...
      if (sf) {
        for (int32_t i = 0; i < numPoints; ++i) {
          double m;
          shpStream >> m;
          ScalarType s = IsESRINoData(m) ? CCCoreLib::NAN_VALUE
      ...
      if (readSize < recordSize16bits * 2) {
        // that would be strange, but the specifications don't say if everything
        // should be ignored, or if boundaries can be present without the
        // values...
        shpStream.skipRawData(recordSize16bits * 2 - readSize);
      }
    } else if (readSize > recordSize16bits * 2) {
...
static CC_FILE_ERROR SaveAsCloud(ccGenericPointCloud *cloud, QDataStream &out,
                                 int32_t recordNumber,
                                 int32_t &recordSize16bits) {
  if (!cloud) {
    assert(false);
...
LoadSinglePoint(QDataStream &shpStream, ccPointCloud *&singlePoints,
                ESRI_SHAPE_TYPE shapeType, int32_t recordSize16bits,
                const CCVector3d &Pshift, bool preserveCoordinateShift) {
  if (!singlePoints) {
  ...
  if (HasMeasurements(shapeType) &&
      (readBytes + 4 + 8 <=
       recordSize16bits * 2)) // +4 = shape type / +8 = measure (double)
  {
    double m;
    shpStream >> m;
    if (!IsESRINoData(m)) {
  ...
  if (singlePoints->size() == singlePoints->capacity() &&
      !singlePoints->reserve(singlePoints->size() +
                             256)) // 256 each time because it appears some SHP
                                   // files have many isolated points...
  {
    delete singlePoints;
...
CC_FILE_ERROR ShpFilter::saveToFile(ccHObject *entity, const QString &filename,
...
CC_FILE_ERROR ShpFilter::saveToFile(
    ccHObject *entity, const std::vector<GenericDBFField *> &fields,
    const QString &filename, const SaveParameters &parameters) {
  if (!entity)
    return CC_FERR_BAD_ENTITY_TYPE;
  ...
  for (ccHObject *child : toSave) {
    ...
    if (child->isA(CC_TYPES::POLY_LINE)) {
      if (static_cast<ccPolyline *>(child)->size() < 2) {
        ccLog::Warning(
            QString("Polyline '%1' is too small! It won't be saved...")
                .arg(child->getName()));
        continue;
      }
    }
    ...
    case ESRI_SHAPE_TYPE::POLYGON_M:
      ...
      error = SavePolyline(static_cast<ccPolyline *>(child), shpStream,
                           recordSize16bits, shapeIndex, outputShapeType,
                           s_poly2DVertDim);
      break;
    case ESRI_SHAPE_TYPE::MULTI_POINT_Z:
      assert(child->isKindOf(CC_TYPES::POINT_CLOUD));
      error = SaveAsCloud(ccHObjectCaster::ToGenericPointCloud(child),
                          shpStream, shapeIndex, recordSize16bits);
      break;
    case ESRI_SHAPE_TYPE::MULTI_PATCH:
      error = SaveMesh(ccHObjectCaster::ToMesh(child), shpStream, shapeIndex,
                       recordSize16bits);
      break;
    default:
      assert(false);
    ...
    idxStream << static_cast<int32_t>(
        recordStart /
        2); // recordStart must be converted to a number of 16-bit words
    idxStream << recordSize16bits; // recordSize should already be expressed as
                                   // a number of 16-bit words

    ccLog::PrintDebug("[SHP] Saved shape #%d (%d bytes)", shapeIndex,
                      recordSize16bits * 2);
    shapeIndex++;
  }

  // update file lengths
  UpdateFileLength(shpStream,
                   static_cast<int32_t>(shpStream.device()->pos() / 2));
  UpdateFileLength(idxStream,
                   static_cast<int32_t>(idxStream.device()->pos() / 2));
  ...
  if (dbfHandle) {
    while (true) // trick: we use 'while' to be able to break anytime
    {
      // always write an 'index' table
      {
        ...
        if (fieldIdx >= 0) {
          for (size_t i = 0; i < toSave.size(); ++i)
            DBFWriteIntegerAttribute(dbfHandle, static_cast<int>(i), fieldIdx,
                                     static_cast<int>(i) + 1);
        } else {
          ccLog::Warning(
              QString("[SHP] Failed to save field 'index' (default)"));
      ...
      if (save3DPolyHeightInDBF) {
        ...
        if (fieldIdx >= 0) {
          for (size_t i = 0; i < toSave.size(); ++i) {
            ...
            DBFWriteDoubleAttribute(dbfHandle, static_cast<int>(i), fieldIdx,
                                    height);
          }
        } else {
          ccLog::Warning(QString(
              "[SHP] Failed to save field 'height' (3D polylines height)"));
          result = CC_FERR_WRITING;
          break;
        }
      }
      ...
      for (GenericDBFField *field : fields) {
        ...
        {
          int xFieldIdx =
              DBFAddField(dbfHandle, qPrintable(field->name() + QString("_x")),
                          field->type(), field->width(), field->decimal());
          int yFieldIdx =
              DBFAddField(dbfHandle, qPrintable(field->name() + QString("_y")),
                          field->type(), field->width(), field->decimal());
          int zFieldIdx =
              DBFAddField(dbfHandle, qPrintable(field->name() + QString("_z")),
                          field->type(), field->width(), field->decimal());
          ...
          if (xFieldIdx < 0) {
            ccLog::Warning(
                QString("[SHP] Failed to save field '%1'").arg(field->name()));
            result = CC_FERR_WRITING;
            break;
          }
        } else // 1D case
        {
          int fieldIdx =
              DBFAddField(dbfHandle, qPrintable(field->name()), field->type(),
...
CC_FILE_ERROR ShpFilter::loadFile(const QString &filename, ccHObject &container,
  ...
  if (HandleGlobalShift(Pmin, Pshift, preserveCoordinateShift, parameters)) {
    ccLog::Warning(
        "[SHP] Entities will be recentered! Translation: (%.2f ; %.2f ; %.2f)",
  ...
  while (hdr.fileLength - shpStream.device()->pos() > 0) {
    ...
    if (!IsValidESRIShapeCode(shapeTypeInt)) {
      ccLog::Warning("[SHP] Shape %d has an invalid shape code (%d)",
    ...
    if (recordNumber < 64)
      ccLog::Print(QString("[SHP] Record #%1 - type: %2 (%3 bytes)")
                       .arg(recordNumber)
                       .arg(ToString(shapeType))
                       .arg(recordSize16bits *
                            2)); // recordSize is measured in 16-bit words
    else if (recordNumber == 64)
      ccLog::Print("[SHP] Records won't be displayed in the Console anymore to "
                   "avoid flooding it...");
    ...
    case ESRI_SHAPE_TYPE::POLYGON_M: {
      ...
      error = LoadPolyline(shpStream, container, recordNumber, shapeType,
                           recordSize16bits, recordStart, Pshift,
    ...
    case ESRI_SHAPE_TYPE::POINT:
      error =
          LoadSinglePoint(shpStream, singlePoints, shapeType, recordSize16bits,
      ...
      break;
    case ESRI_SHAPE_TYPE::MULTI_PATCH:
      error = LoadMultiPatch(shpStream, container, recordSize16bits,
                             recordStart, Pshift);
    case ESRI_SHAPE_TYPE::NULL_SHAPE:
      // ignored
      break;
    default:
      // unhandled entity
      shpStream.skipRawData(
          recordSize16bits * 2 -
          sizeof(shapeTypeInt)); // recordSize is measured in 16-bit words
    ...
    ccLog::PrintDebug(QString("[SHP] File position = %1 / record start = %2 / "
                              "record size = %3 (x2) / position shift = %4")
                          .arg(filePos)
                          .arg(recordStart)
                          .arg(recordSize16bits)
                          .arg(filePos - (recordStart + recordSize16bits * 2)));

    assert(shpStream.device()->pos() ==
           recordStart +
               recordSize16bits * 2); // recordSize is measured in 16-bit words
  ...
  // try to load the DBF to see if there's a 'height' field or something similar
  // for polylines
      ...
      if (fieldCount == 0) {
        ...
        for (int i = 0; i < fieldCount; ++i) {
          ...
          DBFFieldType fieldType =
              DBFGetFieldInfo(dbfHandle, i, fieldName, nullptr, nullptr);
          ...
          if (parameters.parentWidget) {
            ...
            if (lsfDlg.exec()) {
              ...
              for (int i = 0; i < candidateFields.size(); ++i) {
                if (lsfDlg.listWidget->isItemSelected(
                        lsfDlg.listWidget->item(i))) {
              ...
              if (index >= 0) {
                ...
                DBFFieldType fieldType = DBFGetFieldInfo(
                    dbfHandle, index, nullptr, nullptr, nullptr);
                ...
                if (hasPolylines) {
                  ...
                  for (QMap<ccPolyline *, int32_t>::iterator it =
                    ...
                    if (fieldType == FTDouble)
                      z = DBFReadDoubleAttribute(dbfHandle, it.value() - 1,
                                                 index);
                    else // if (fieldType == FTInteger)
                      z = static_cast<double>(DBFReadIntegerAttribute(
                          dbfHandle, it.value() - 1, index));
                    z *= scale;
                  ...
                  for (unsigned i = 0; i < singlePoints->size(); ++i) {
                    ...
                    double z = 0.0;
                    if (fieldType == FTDouble)
                      z = DBFReadDoubleAttribute(dbfHandle, static_cast<int>(i),
                                                 index);
                    else // if (fieldType == FTInteger)
                      z = static_cast<double>(DBFReadIntegerAttribute(
                          dbfHandle, static_cast<int>(i), index));
                    z *= scale;
            ...
            ccLog::Warning(
                "[SHP] Silent mode: no field will be used as Z coordinate!");
          }
        } else {
          ccLog::Warning(
              "[SHP] No numerical field in the associated DBF file!");
        }
      }
      DBFClose(dbfHandle);
    } else {
      ccLog::Warning(QString("[SHP] Failed to load associated DBF file ('%1')")
                         .arg(dbfFilename));
    }
  }

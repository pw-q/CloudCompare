...
// default and last input shift/scale entries (don't use it directly, use
// GetLast() instead)
static std::vector<ccGlobalShiftManager::ShiftInfo> s_lastInfoBuffer;
const std::vector<ccGlobalShiftManager::ShiftInfo> &
ccGlobalShiftManager::GetLast() {
  // the first time this method is called, load the default values from the
  // 'bookmark' files
  static bool s_firstTime = true;
  if (s_firstTime) {
    LoadInfoFromFile(QCoreApplication::applicationDirPath() + QString("/") +
...
static bool SameShift(const ccGlobalShiftManager::ShiftInfo &shiftInfo,
                      const CCVector3d &shift, double scale) {
  return ((shiftInfo.shift - shift).norm() <= CCCoreLib::ZERO_TOLERANCE_D &&
          std::abs(shiftInfo.scale - scale) <= CCCoreLib::ZERO_TOLERANCE_D);
}

void ccGlobalShiftManager::StoreShift(const CCVector3d &shift, double scale,
                                      bool preserve /*=true*/) {
...
static bool ShiftAndScaleAreSimilar(double scale1, const CCVector3d &shift1,
                                    double scale2, const CCVector3d &shift2) {
  return ((shift1 - shift2).norm() <= CCCoreLib::ZERO_TOLERANCE_D) &&
         (std::abs(scale1 - scale2) <= CCCoreLib::ZERO_TOLERANCE_D);
}
...
bool ccGlobalShiftManager::Handle(const CCVector3d &P, double diagonal,
  ...
  if (useInputCoordinatesShiftIfPossible) {
    if (nullptr != _coordinatesScale) {
      ...
      *_coordinatesScale =
          std::max(*_coordinatesScale, CCCoreLib::ZERO_TOLERANCE_D);
    ...
    if (mode == NO_DIALOG) {
      // without a dialog, we don't have the choice, we will use the input
      // shift...
    ...
    if (mode == NO_DIALOG) {
      // it would be a bit strange to call this method with NO_DIALOG and no
      // input shift but that's theoretically possible ;)
      return false;
    }
  ...
  if (needShift || needRescale) {
    preserveCoordinateShift = true; // if we fall here, this means that even the
                                    // provided input doesn't work
    if (mode != NO_DIALOG_AUTO_SHIFT) {
      // at this point, we will display the dialog since the input info doesn't
      // work
      mode = ALWAYS_DISPLAY_DIALOG;
    }
  }

  const std::vector<ShiftInfo> &lastInfoBuffer =
      ccGlobalShiftManager::GetLast();
  ...
  if (mode == NO_DIALOG_AUTO_SHIFT) {
    // since we can't display a dialog, we'll just use the best shift found so
    // far
    if (needShift) {
  ...
  // should we still display the dialog?
  else if (needShift || needRescale || mode == ALWAYS_DISPLAY_DIALOG) {
    ...
    int index = sasDlg.addShiftInfo(
        ShiftInfo("Automatic", suggestedShift, suggestedScale));
    ...
    if (useInputCoordinatesShiftIfPossible) {
      ...
      for (size_t i = 0; i < lastInfoBuffer.size(); ++i) {
        const ShiftInfo &shiftInfo = lastInfoBuffer[i];
        if (ShiftAndScaleAreSimilar(shiftInfo.scale, shiftInfo.shift,
                                    inputScale, inputCoordinatesShift)) {
      ...
      if (!alreadyRegistered) {
        ...
        int addedEntryIndex =
            sasDlg.addShiftInfo(ShiftInfo("Input", coordinatesShift, scale));
        if (canUseInputCoordinatesShift) {
...
double ccGlobalShiftManager::BestScale(double d) {
  return d < MAX_DIAGONAL_LENGTH
             ? 1.0
             : pow(10.0,
                   -static_cast<double>(ceil(log(d / MAX_DIAGONAL_LENGTH))));
}

bool ccGlobalShiftManager::LoadInfoFromFile(QString filename,
                                            std::vector<ShiftInfo> &infos) {
  QFile file(filename);
  ...
  while (true) {
    ...
    if (tokens.size() != 5) {
      // invalid file
      ccLog::Warning(QString("[ccGlobalShiftManager::LoadInfoFromFile] File "
    ...
    if (errors) {
      // invalid file
      ccLog::Warning(
          QString("[ccGlobalShiftManager::LoadInfoFromFile] File '%1' is "
                  "malformed (wrong item type encountered on line %2)")
              .arg(filename)
              .arg(lineNumber));
      return false;
    }
    ...
    try {
      ...
      // not enough memory
      ccLog::Warning(QString("[ccGlobalShiftManager::LoadInfoFromFile] Not "

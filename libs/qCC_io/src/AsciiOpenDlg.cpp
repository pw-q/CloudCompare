...
struct AsciiOpenContext {
  //! Default initializer
  AsciiOpenContext()
      : separator(' '), extractSFNameFrom1stLine(false),
        maxPointCountPerCloud(0), skipLines(0), applyAll(false),
        commaDecimal(false) {}
...
AsciiOpenDlg::AsciiOpenDlg(QWidget *parent)
    : QDialog(parent), m_ui(new Ui_AsciiOpenDialog), m_separator(' '),
      m_averageLineSize(-1.0), m_stream(nullptr), m_columnsCount(0) {
  m_ui->setupUi(this);
  ...
  connect(m_ui->spinBoxSkipLines, qOverload<int>(&QSpinBox::valueChanged), this,
          &AsciiOpenDlg::onSkippedLinesChanged);
  connect(m_ui->resetColumnsToolButton, &QToolButton::clicked, this,
          &AsciiOpenDlg::resetColumnRoles);
...
bool AsciiOpenDlg::setInput(const QString &filename,
  ...
  return false; // the current configuration shouldn't be applied automatically
                // ('apply all' is false, or the configuration is different from
                // the previous one)
}
...
static bool CouldBeQuatW(const QString &colHeader) {
  return colHeader.startsWith(AsciiHeaderColumns::QuatX().toUpper()) ||
         colHeader == "QW" || colHeader == "Q0";
}
static bool CouldBeQuatX(const QString &colHeader) {
  return colHeader.startsWith(AsciiHeaderColumns::QuatY().toUpper()) ||
         colHeader == "QX";
}
static bool CouldBeQuatY(const QString &colHeader) {
  return colHeader.startsWith(AsciiHeaderColumns::QuatZ().toUpper()) ||
         colHeader == "QY";
}
static bool CouldBeQuatZ(const QString &colHeader) {
  return colHeader.startsWith(AsciiHeaderColumns::QuatW().toUpper()) ||
         colHeader == "QZ";
}
...
void AsciiOpenDlg::updateTable() {
  ...
  std::vector<bool>
      valueIsBelowOne; // identifies columns with values between -1 and 1 only
  std::vector<bool>
      valueIsInteger; // identifies columns with integer values only
  std::vector<bool> valueIsBelow255; // identifies columns with integer values
  ...
  while (lineCount < LINES_READ_FOR_STATS) {
    ...
    if (!currentLine.startsWith("//") /* || !currentLine.startsWith("#")*/) {
      QStringList parts =
          currentLine.simplified().split(m_separator, QString::SkipEmptyParts);

      if (lineCount < DISPLAYED_LINES) {
        unsigned partsCount =
            std::min(MAX_COLUMNS, static_cast<unsigned>(parts.size()));
        ...
        for (unsigned i = 0; i < partsCount; ++i) {
          ...
          if (!isANumber) {
            ...
            if (columnCountHasIncreased ||
              ...
              // we can forget about their content otherwise it will prevent us
              // from detecting the right pattern
            ...
            valueIsInteger[i] =
                valueIsInteger[i] && !parts[i].contains(decimalPoint);
            valueIsBelow255[i] = valueIsBelow255[i] && valueIsInteger[i] &&
  ...
  // setup table and widgets
  {
    ...
    static const QIcon LabelIcon(
        QString::fromUtf8(":/CC/images/dbLabelSymbol.png"));
    static const QIcon QuatIcon(
        QString::fromUtf8(":/CC/images/typeQuaternion.png"));
    ...
    for (unsigned i = 0; i < columnsCount; i++) {
      ...
      if (!columnHeaderWidget) {
        ...
        connect(columnHeaderWidget,
                ...
                &AsciiOpenDlg::columnsTypeHasChanged);
      }
      ...
      if (!valueIsNumber[i]) {
        ...
        //
        // Description:
        // once we open the file, we will get a %m_columnType with 5 values of
        // "TEXT" then if we choose to skip the 3 first lines, we get a
        // %valueIsNumber with 5 "true" but the %m_columnType is still with 5
        // values of "TEXT" which leads to the failure!
  ...
  // auto-detect columns 'roles'
  {
    ...
    QStringList headerParts =
        m_headerLine.simplified().split(m_separator, QString::SkipEmptyParts);
    bool validHeader = (headerParts.size() >= static_cast<int>(columnsCount));
    m_ui->extractSFNamesFrom1stLineCheckBox->setEnabled(
        validHeader); // can we consider the first ignored line as a header?
    if (!validHeader) {
      ...
      // std::fill(m_columnType.begin(), m_columnType.end(), UNKNOWN); //if the
      // header has changed, we force update of all columns!
    }
        ...
        if (m_columnType[i] == UNKNOWN || m_columnType[i] == IGNORED) {
          ...
          if ((assignedXYZFlags & X_BIT) == 0 && CouldBeX(colHeader)) {
            ...
            m_columnType[i] = VALID;
                     ...
                     CouldBeLabel(colHeader)) // no need to promote labels if
        ...
        if (m_columnType[i] == UNKNOWN && columnsCount > 1) {
          ...
          if (assignedXYZFlags < XYZ_BITS) {
            ...
            if (columnsCount > 3 && i == 0 &&
                (EnabledBits(assignedXYZFlags) == 0) && valueIsInteger[i] &&
                i + 1 < columnsCount && !valueIsInteger[i + 1]) {
            ...
            if (valueIsBelow255[i] && assignedRGBFlags < XYZ_BITS &&
                (i + 2 - EnabledBits(assignedRGBFlags)) <
                    columnsCount // make sure that we can put all values there!
                && (EnabledBits(assignedRGBFlags) > 0 ||
                    (valueIsBelow255[i + 1] &&
                     valueIsBelow255[i + 2])) // make sure that next values are
                                              // also ok!
            ) {
            ...
            else if (valueIsBelowOne[i] && assignedNormFlags < XYZ_BITS &&
                     (i + 2 - EnabledBits(assignedNormFlags) <
                      columnsCount) // make sure that we can put all values
                                    // there!
                     && (EnabledBits(assignedNormFlags) > 0 ||
                         (valueIsBelowOne[i + 1] &&
                          valueIsBelowOne[i + 2])) // make sure that next values
                                                   // are also ok!
  ...
  checkSelectedColumnsValidity(); // will eventually enable of disable the "OK"
                                  // button
...
void AsciiOpenDlg::checkSelectedColumnsValidity() {
  ...
  bool m_selectedInvalidColumns = false;
  {
    ...
    for (unsigned i = 0; i < m_columnsCount; i++) {
      ...
      } else if (columnHeaderWidget->currentIndex() >= ASCII_OPEN_DLG_QuatW &&
                 columnHeaderWidget->currentIndex() <= ASCII_OPEN_DLG_QuatZ) {
        ++quaternionFields;
      }
    }
...
bool AsciiOpenDlg::CheckOpenSequence(const AsciiOpenDlg::Sequence &sequence,
  ...
  unsigned quaternionTotal =
      counters[ASCII_OPEN_DLG_QuatW] + counters[ASCII_OPEN_DLG_QuatX] +
      counters[ASCII_OPEN_DLG_QuatY] + counters[ASCII_OPEN_DLG_QuatZ];
...
void AsciiOpenDlg::columnsTypeHasChanged(int index) {
  ...
  for (unsigned i = 0; i < m_columnsCount; i++) {
    ...
    if (changedCombo == combo) {
      if (index == int(ASCII_OPEN_DLG_X) || index == int(ASCII_OPEN_DLG_NX) ||
          index == int(ASCII_OPEN_DLG_R) || index == int(ASCII_OPEN_DLG_Rf) ||
          index == int(ASCII_OPEN_DLG_QuatW)) {
        // Auto select the next columns type
        if (i + 2 < m_columnsCount) {
          QComboBox *nextCombo =
              static_cast<QComboBox *>(m_ui->tableWidget->cellWidget(0, i + 1));
          QComboBox *nextNextCombo =
              static_cast<QComboBox *>(m_ui->tableWidget->cellWidget(0, i + 2));
          ...
          if (nextCombo->currentIndex() == int(ASCII_OPEN_DLG_None) &&
            ...
            if (index == int(ASCII_OPEN_DLG_X)) {
              ...
              if (i + 3 <
                  m_columnsCount) // we need one more value for quaternions
              {
                QComboBox *nexNextNextCombo = static_cast<QComboBox *>(
                    m_ui->tableWidget->cellWidget(0, i + 3));
                if (nexNextNextCombo->currentIndex() ==
    ...
    } else if (index < ASCII_OPEN_DLG_Scalar) // check that the other combo as
                                              // the same index (apart from SF)
    {
...
unsigned AsciiOpenDlg::getMaxCloudSize() const {
  return static_cast<unsigned>(
      ...
      1.0e6); // static_cast is equivalent to floor if value >= 0
}

...
ccStereoModeDlg::ccStereoModeDlg(QWidget *parent)
    : QDialog(parent, Qt::Tool), m_ui(new Ui::StereoModeDialog) {
  m_ui->setupUi(this);

  glassTypeChanged(m_ui->glassTypeComboBox->currentIndex());

  connect(m_ui->glassTypeComboBox,
          qOverload<int>(&QComboBox::currentIndexChanged), this,
          &ccStereoModeDlg::glassTypeChanged);
}
...
void ccStereoModeDlg::glassTypeChanged(int index) {
  ...
  case COMBO_INDEX_OCULUS:
    ...
    m_ui->warningTextEdit->setVisible(true);
    m_ui->warningTextEdit->setText("To use the Oculus Rift make sure that:\
			<ul>\
...
ccGLWindowInterface::StereoParams ccStereoModeDlg::getParameters() const {
  ...
  params.stereoStrength =
      m_ui->stereoStrengthHorizontalSlider->sliderPosition();

  return params;
}

void ccStereoModeDlg::setParameters(
    const ccGLWindowInterface::StereoParams &params) {
  // glass type
  ...
  m_ui->stereoStrengthHorizontalSlider->setSliderPosition(
      params.stereoStrength);
}

bool ccStereoModeDlg::updateFOV() const {
  return m_ui->glassTypeComboBox->currentIndex() != COMBO_INDEX_OCULUS &&
         m_ui->autoFocalCheckBox->isChecked();
}

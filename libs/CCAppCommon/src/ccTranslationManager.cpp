...
void ccTranslationManager::loadTranslation(QString language) {
  ...
  for (const auto &fileInfo : mTranslatorFileInfo) {
    ...
    bool loaded = translator->load(locale, fileInfo.prefix, QStringLiteral("_"),
                                   fileInfo.path);
...
void ccTranslationManager::populateMenu(QMenu *menu,
  ...
  const LanguageList languageList = availableLanguages(
      QStringLiteral("CloudCompare"), pathToTranslationFiles);
...
ccTranslationManager::LanguageList ccTranslationManager::availableLanguages(
    const QString &appName, const QString &pathToTranslationFiles) const {
  ...
  for (const auto &fileName : fileNames) {
    ...
    QString menuItem =
        QStringLiteral("%1%2").arg(language[0].toUpper(), language.mid(1));

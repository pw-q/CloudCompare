...
ccCameraParamEditDlg::ccCameraParamEditDlg(QWidget *parent,
    ...
    : ccOverlayDialog(parent,
                      pickingHub ? Qt::FramelessWindowHint | Qt::Tool
                                 : Qt::Tool) //  / otherwise = ccViewer
      ,
      m_pickingHub(pickingHub), m_ui(new Ui::CameraParamDlg) {
  m_ui->setupUi(this);

  connect(m_ui->phiSlider, &QAbstractSlider::valueChanged, this,
          &ccCameraParamEditDlg::iPhiValueChanged);
  connect(m_ui->thetaSlider, &QAbstractSlider::valueChanged, this,
          &ccCameraParamEditDlg::iThetaValueChanged);
  connect(m_ui->psiSlider, &QAbstractSlider::valueChanged, this,
          &ccCameraParamEditDlg::iPsiValueChanged);

  connect(m_ui->phiSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccCameraParamEditDlg::dPhiValueChanged);
  connect(m_ui->thetaSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccCameraParamEditDlg::dThetaValueChanged);
  connect(m_ui->psiSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccCameraParamEditDlg::dPsiValueChanged);

  // rotation center
  connect(m_ui->rcxDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccCameraParamEditDlg::pivotChanged);
  connect(m_ui->rcyDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccCameraParamEditDlg::pivotChanged);
  connect(m_ui->rczDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccCameraParamEditDlg::pivotChanged);

  // camera center
  connect(m_ui->exDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccCameraParamEditDlg::cameraCenterChanged);
  connect(m_ui->eyDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccCameraParamEditDlg::cameraCenterChanged);
  connect(m_ui->ezDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccCameraParamEditDlg::cameraCenterChanged);

  connect(m_ui->fovDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccCameraParamEditDlg::fovChanged);
  connect(m_ui->nearClippingDepthDoubleSpinBox,
          &QDoubleSpinBox::editingFinished, [&] {
            nearClippingDepthChanged(
                m_ui->nearClippingDepthDoubleSpinBox->value());
          });
  connect(m_ui->farClippingDepthDoubleSpinBox, &QDoubleSpinBox::editingFinished,
          [&] {
            farClippingDepthChanged(
                m_ui->farClippingDepthDoubleSpinBox->value());
          });
  connect(m_ui->nearClippingDepthDoubleSpinBox,
          QOverload<double>::of(&QDoubleSpinBox::valueChanged), [&](double d) {
            if (d != 0)
              nearClippingDepthChanged(d);
          });
  connect(m_ui->farClippingDepthDoubleSpinBox,
          QOverload<double>::of(&QDoubleSpinBox::valueChanged), [&](double d) {
            if (d != 0)
              farClippingDepthChanged(d);
          });
  connect(m_ui->nearClippingCheckBox, &QCheckBox::toggled, this,
          &ccCameraParamEditDlg::nearClippingCheckBoxToggled);
  connect(m_ui->farClippingCheckBox, &QCheckBox::toggled, this,
          &ccCameraParamEditDlg::farClippingCheckBoxToggled);

  connect(m_ui->viewUpToolButton, &QAbstractButton::clicked, this,
          &ccCameraParamEditDlg::setTopView);
  connect(m_ui->viewDownToolButton, &QAbstractButton::clicked, this,
          &ccCameraParamEditDlg::setBottomView);
  connect(m_ui->viewFrontToolButton, &QAbstractButton::clicked, this,
          &ccCameraParamEditDlg::setFrontView);
  connect(m_ui->viewBackToolButton, &QAbstractButton::clicked, this,
          &ccCameraParamEditDlg::setBackView);
  connect(m_ui->viewLeftToolButton, &QAbstractButton::clicked, this,
          &ccCameraParamEditDlg::setLeftView);
  connect(m_ui->viewRightToolButton, &QAbstractButton::clicked, this,
          &ccCameraParamEditDlg::setRightView);
  connect(m_ui->viewIso1ToolButton, &QAbstractButton::clicked, this,
          &ccCameraParamEditDlg::setIso1View);
  connect(m_ui->viewIso2ToolButton, &QAbstractButton::clicked, this,
          &ccCameraParamEditDlg::setIso2View);

  connect(m_ui->pushMatrixToolButton, &QAbstractButton::clicked, this,
          &ccCameraParamEditDlg::pushCurrentMatrix);
  connect(m_ui->revertMatrixToolButton, &QAbstractButton::clicked, this,
          &ccCameraParamEditDlg::revertToPushedMatrix);

  connect(m_ui->pivotPickingToolButton, &QAbstractButton::toggled, this,
          &ccCameraParamEditDlg::pickPointAsPivot);
}
...
void ccCameraParamEditDlg::pivotChanged() {
  ...
  m_associatedWin->signalEmitter()->blockSignals(true);
  m_associatedWin->setPivotPoint(CCVector3d(m_ui->rcxDoubleSpinBox->value(),
...
void ccCameraParamEditDlg::nearClippingDepthChanged(double depth) {
  ...
  if (m_associatedWin->setNearClippingPlaneDepth(depth)) {
    ...
    updateNearClippingDepth(
        m_associatedWin->getViewportParameters().nearClippingDepth);
  }
}
...
void ccCameraParamEditDlg::nearClippingCheckBoxToggled(bool state) {
  if (state) {
    if (m_ui->nearClippingCheckBox->isChecked()) {
      if (m_associatedWin &&
          m_ui->nearClippingDepthDoubleSpinBox->value() <= 0) {
        // auto set the near clipping depth the first time
        m_ui->nearClippingDepthDoubleSpinBox->setValue(
            m_associatedWin->getViewportParameters().zNear);
      } else {
...
void ccCameraParamEditDlg::farClippingDepthChanged(double depth) {
  ...
  if (m_associatedWin->setFarClippingPlaneDepth(depth)) {
    ...
    updateFarClippingDepth(
        m_associatedWin->getViewportParameters().farClippingDepth);
  }
}
...
void ccCameraParamEditDlg::farClippingCheckBoxToggled(bool state) {
  if (state) {
    if (m_ui->farClippingCheckBox->isChecked()) {
      if (m_associatedWin &&
          m_ui->farClippingDepthDoubleSpinBox->value() >= 1.0e6) {
        // auto set the far clipping depth the first time
        m_ui->farClippingDepthDoubleSpinBox->setValue(
            m_associatedWin->getViewportParameters().zFar);
      } else {
        // force the window update
  ...
  if (m_pickingHub) {
    if (state) {
      if (!m_pickingHub->addListener(this, true)) {
        ccLog::Error(
            "Can't start the picking process (another tool is using it)");
        state = false;
      }
    } else {
    ...
    if (state) {
      ...
      connect(m_associatedWin->signalEmitter(),
              &ccGLWindowSignalEmitter::itemPicked, this,
              &ccCameraParamEditDlg::processPickedItem);
    } else {
      m_associatedWin->setPickingMode(ccGLWindowInterface::DEFAULT_PICKING);
      disconnect(m_associatedWin->signalEmitter(),
                 &ccGLWindowSignalEmitter::itemPicked, this,
                 &ccCameraParamEditDlg::processPickedItem);
    }
  }
...
void ccCameraParamEditDlg::processPickedItem(ccHObject *entity, unsigned, int,
                                             int, const CCVector3 &P,
...
void ccCameraParamEditDlg::setTopView() { setView(CC_TOP_VIEW); }

void ccCameraParamEditDlg::setBottomView() { setView(CC_BOTTOM_VIEW); }

void ccCameraParamEditDlg::setFrontView() { setView(CC_FRONT_VIEW); }

void ccCameraParamEditDlg::setBackView() { setView(CC_BACK_VIEW); }

void ccCameraParamEditDlg::setLeftView() { setView(CC_LEFT_VIEW); }

void ccCameraParamEditDlg::setRightView() { setView(CC_RIGHT_VIEW); }

void ccCameraParamEditDlg::setIso1View() { setView(CC_ISO_VIEW_1); }

void ccCameraParamEditDlg::setIso2View() { setView(CC_ISO_VIEW_2); }

bool ccCameraParamEditDlg::start() {
  ccOverlayDialog::start();

  m_processing = false; // no such concept for this dialog! (+ we want to allow
                        // dynamic change of associated window)
...
void ccCameraParamEditDlg::linkWith(QMdiSubWindow *qWin) {
  ...
  ccGLWindowInterface *associatedWin =
      (qWin ? ccGLWindowInterface::FromWidget(qWin->widget()) : nullptr);
...
bool ccCameraParamEditDlg::linkWith(ccGLWindowInterface *win) {
  ...
  if (m_associatedWin) {
    ...
    connect(m_associatedWin->signalEmitter(),
            &ccGLWindowSignalEmitter::baseViewMatChanged, this,
            &ccCameraParamEditDlg::initWithMatrix);
    connect(m_associatedWin->signalEmitter(),
            &ccGLWindowSignalEmitter::cameraPosChanged, this,
            &ccCameraParamEditDlg::updateCameraCenter);
    connect(m_associatedWin->signalEmitter(),
            &ccGLWindowSignalEmitter::pivotPointChanged, this,
            &ccCameraParamEditDlg::updatePivotPoint);
    connect(m_associatedWin->signalEmitter(),
            &ccGLWindowSignalEmitter::perspectiveStateChanged, this,
            &ccCameraParamEditDlg::updateViewMode);
    connect(m_associatedWin->signalEmitter(),
            &ccGLWindowSignalEmitter::aboutToClose, this, &QWidget::hide);
    connect(m_associatedWin->signalEmitter(),
            &ccGLWindowSignalEmitter::fovChanged, this,
            &ccCameraParamEditDlg::updateWinFov);
    connect(m_associatedWin->signalEmitter(),
            &ccGLWindowSignalEmitter::nearClippingDepthChanged, this,
            &ccCameraParamEditDlg::updateNearClippingDepth);
    connect(m_associatedWin->signalEmitter(),
            &ccGLWindowSignalEmitter::farClippingDepthChanged, this,
            &ccCameraParamEditDlg::updateFarClippingDepth);
...
void ccCameraParamEditDlg::updateViewMode() {
  if (m_associatedWin) {
    ...
    if (!perspective)
      ...
      m_ui->currentModeLabel->setText(
          QString(objectBased ? "object" : "viewer") +
          QString("-based perspective"));
...
void ccCameraParamEditDlg::updateNearClippingDepth(double depth) {
  ...
  m_ui->nearClippingDepthDoubleSpinBox->setValue(std::isnan(depth) ? 0.0
                                                                   : depth);
...
void ccCameraParamEditDlg::updateFarClippingDepth(double depth) {
  ...
  m_ui->farClippingDepthDoubleSpinBox->setValue(std::isnan(depth) ? 1.0e6
                                                                  : depth);

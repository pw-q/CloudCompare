...
static QString GetPluginIID(QPluginLoader *loader) {
  ...
  if (metaObject.isEmpty()) {
    ...
    ccLog::Warning(
        QStringLiteral("\t%1 does not supply meta data in the "
...
static bool IsMetaDataValid(QPluginLoader *loader) {
  ...
  if (metaObject.isEmpty() || metaObject["MetaData"].toObject().isEmpty()) {
    ccLog::Error(
        QStringLiteral("%1 does not supply meta data in the Q_PLUGIN_METADATA "
    ...
    if (!validTypes.contains(pluginType)) {
      ccLog::Error(
          QStringLiteral("%1 does not supply a valid plugin type in its "
                         "info.json.\n\nFound: %2\n\nIt must be one of: %3")
...
ccPluginManager &ccPluginManager::Get() { return *s_pluginManager; }

void ccPluginManager::setPaths(const QStringList &paths) {
  m_pluginPaths = paths;
}

QStringList ccPluginManager::pluginPaths() const { return m_pluginPaths; }
...
void ccPluginManager::getDisabledPluginIIDs(
  ...
  for (const QString &path : paths) {
    ...
    for (const QString &fileName : fileNames) {
      ...
      if ((plugin == nullptr) || (ccPlugin == nullptr)) {
        if (plugin == nullptr) {
          ccLog::Warning(tr("\t%1 does not seem to be a valid plugin\t(%2)")
                             .arg(fileName, loader->errorString()));
        } else {
          ccLog::Warning(tr("\t%1 does not seem to be a valid plugin or it is "
      ...
      // If we have already loaded a plugin with this IID, unload it and replace
      // the interface in the plugin list
      if (pluginIIDToLoaderMap.contains(pluginIID)) {
        QSharedPointer<QPluginLoader> previousLoader =
            pluginIIDToLoaderMap[pluginIID];
        assert(previousLoader);

        ccPluginInterface *pluginInterface =
            qobject_cast<ccPluginInterface *>(previousLoader->instance());
      ...
      ccLog::Print(
          tr("\tPlugin found: %1 (%2)").arg(ccPlugin->getName(), fileName));
    }
  }

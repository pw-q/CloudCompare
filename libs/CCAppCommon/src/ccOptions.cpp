...
void ccOptions::ReleaseInstance() { s_options.release(); }

void ccOptions::Set(const ccOptions &params) { InstanceNonConst() = params; }

ccOptions::ccOptions() { reset(); }
...
void ccOptions::fromPersistentSettings() {
  ...
  settings.beginGroup(ccPS::Options());
  {
    normalsDisplayedByDefault =
        settings.value("normalsDisplayedByDefault", false).toBool();

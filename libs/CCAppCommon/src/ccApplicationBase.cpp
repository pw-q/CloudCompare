...
#error Software does not support versions of Qt prior to 5.5
#endif
...
void ccApplicationBase::InitOpenGL() {
  ...
  QApplication instance is mandatory on some platforms (for example, OS X) when
  an OpenGL core profile context is requested. This is to ensure that resource
  sharing between contexts stays functional as all internal contexts are created
          using the correct version and profile.
  **/
  {
    QSurfaceFormat format = QSurfaceFormat::defaultFormat();
    format.setStencilBufferSize(0);
#ifndef CC_LINUX // seems to cause some big issues on Linux if Quad-buffering is
                 // not supported we would need to find a way to check whether
                 // it's supported or not in advance...
    format.setStereo(true); // we request stereo support by default, but this
...
#ifdef Q_OS_MAC
    format.setVersion(2,
                      1); // must be 2.1 - see ccGLWindowInterface::functions()
  ...
  // The 'AA_ShareOpenGLContexts' attribute must be defined BEFORE the creation
  // of the Q(Gui)Application DGM: this is mandatory to enable exclusive full
  // screen for ccGLWindow (at least on Windows)
  QCoreApplication::setAttribute(Qt::AA_ShareOpenGLContexts);
}

ccApplicationBase::ccApplicationBase(int &argc, char **argv, bool isCommandLine,
                                     const QString &version)
    : QApplication(argc, argv), m_versionStr(version),
  ...
  if (utf8Codec) {
    ...
    ccLog::Warning(
        "Failed to set the UTF-8 codec as default (codec not found)");
  }
  ...
  ccTranslationManager::Get().registerTranslatorFile(
      QStringLiteral("CloudCompare"), m_translationPath);
...
void ccApplicationBase::setupPaths() {
  ...
  // Shaders & plugins are relative to the bin directory where the executable is
  // found
  QDir theDir = appDir;
  ...
  if (theDir.dirName() == "bin") {
    ...
    m_translationPath =
        (theDir.absolutePath() + "/share/cloudcompare/translations");
  } else {
...
#warning Need to specify the shader path for this OS.
  ...
  // If the environment variables are specified, overwrite the shader and
  // translation paths.
  ...
  for (const QString &appDataPath : appDataPaths) {
    ...
    if (!m_pluginPaths.contains(
            path)) // avoid duplicate entries (can happen, at least on Windows)
    {
      m_pluginPaths << path;
    }
  }
...
bool ccApplicationBase::setAppStyle(QString styleKey) {
  ...
  if (styleKey == "QDarkStyleSheet::Dark") {
    ...
    if (!style) {
      ccLog::Warning("Invalid style key or style couldn't be created: " +
                     styleKey);
      return false;
    }

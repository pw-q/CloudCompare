...
ccDisplaySettingsDlg::ccDisplaySettingsDlg(QWidget *parent)
    : QDialog(parent, Qt::Tool), m_ui(new Ui::DisplaySettingsDlg),
      m_defaultAppStyleIndex(-1) {
  m_ui->setupUi(this);

  connect(m_ui->ambientColorButton, &QAbstractButton::clicked, this,
          &ccDisplaySettingsDlg::changeLightAmbientColor);
  connect(m_ui->diffuseColorButton, &QAbstractButton::clicked, this,
          &ccDisplaySettingsDlg::changeLightDiffuseColor);
  connect(m_ui->specularColorButton, &QAbstractButton::clicked, this,
          &ccDisplaySettingsDlg::changeLightSpecularColor);
  connect(m_ui->meshBackColorButton, &QAbstractButton::clicked, this,
          &ccDisplaySettingsDlg::changeMeshBackDiffuseColor);
  connect(m_ui->meshSpecularColorButton, &QAbstractButton::clicked, this,
          &ccDisplaySettingsDlg::changeMeshSpecularColor);
  connect(m_ui->meshFrontColorButton, &QAbstractButton::clicked, this,
          &ccDisplaySettingsDlg::changeMeshFrontDiffuseColor);
  connect(m_ui->bbColorButton, &QAbstractButton::clicked, this,
          &ccDisplaySettingsDlg::changeBBColor);
  connect(m_ui->bkgColorButton, &QAbstractButton::clicked, this,
          &ccDisplaySettingsDlg::changeBackgroundColor);
  connect(m_ui->labelBkgColorButton, &QAbstractButton::clicked, this,
          &ccDisplaySettingsDlg::changeLabelBackgroundColor);
  connect(m_ui->labelMarkerColorButton, &QAbstractButton::clicked, this,
          &ccDisplaySettingsDlg::changeLabelMarkerColor);
  connect(m_ui->pointsColorButton, &QAbstractButton::clicked, this,
          &ccDisplaySettingsDlg::changePointsColor);
  connect(m_ui->textColorButton, &QAbstractButton::clicked, this,
          &ccDisplaySettingsDlg::changeTextColor);

  connect(m_ui->doubleSidedCheckBox, &QCheckBox::toggled, this,
          [&](bool state) { m_parameters.lightDoubleSided = state; });
  connect(m_ui->enableGradientCheckBox, &QCheckBox::toggled, this,
          [&](bool state) { m_parameters.drawBackgroundGradient = state; });
  connect(m_ui->showCrossCheckBox, &QCheckBox::toggled, this,
          [&](bool state) { m_parameters.displayCross = state; });
  connect(m_ui->colorScaleShowHistogramCheckBox, &QCheckBox::toggled, this,
          [&](bool state) { m_parameters.colorScaleShowHistogram = state; });
  connect(m_ui->useColorScaleShaderCheckBox, &QCheckBox::toggled, this,
          [&](bool state) { m_parameters.colorScaleUseShader = state; });
  connect(m_ui->decimateMeshBox, &QCheckBox::toggled, this,
          [&](bool state) { m_parameters.decimateMeshOnMove = state; });
  connect(m_ui->decimateCloudBox, &QCheckBox::toggled, this,
          [&](bool state) { m_parameters.decimateCloudOnMove = state; });
  connect(m_ui->drawRoundedPointsCheckBox, &QCheckBox::toggled, this,
          [&](bool state) { m_parameters.drawRoundedPoints = state; });
  connect(m_ui->singleClickPickingCheckBox, &QCheckBox::toggled, this,
          [&](bool state) { m_parameters.singleClickPicking = state; });
  connect(m_ui->autoDisplayNormalsCheckBox, &QCheckBox::toggled, this,
          [&](bool state) { m_options.normalsDisplayedByDefault = state; });
  connect(m_ui->useNativeDialogsCheckBox, &QCheckBox::toggled, this,
          [&](bool state) { m_options.useNativeDialogs = state; });
  connect(m_ui->confirmQuitCheckBox, &QCheckBox::toggled, this,
          [&](bool state) { m_options.confirmQuit = state; });

  connect(m_ui->useVBOCheckBox, &QAbstractButton::clicked, this,
          &ccDisplaySettingsDlg::changeVBOUsage);

  connect(m_ui->colorRampWidthSpinBox, qOverload<int>(&QSpinBox::valueChanged),
          this, &ccDisplaySettingsDlg::changeColorScaleRampWidth);

  connect(m_ui->defaultFontSizeSpinBox, qOverload<int>(&QSpinBox::valueChanged),
          this, &ccDisplaySettingsDlg::changeDefaultFontSize);
  connect(m_ui->labelFontSizeSpinBox, qOverload<int>(&QSpinBox::valueChanged),
          this, &ccDisplaySettingsDlg::changeLabelFontSize);
  connect(m_ui->numberPrecisionSpinBox, qOverload<int>(&QSpinBox::valueChanged),
          this, &ccDisplaySettingsDlg::changeNumberPrecision);
  connect(m_ui->labelOpacitySpinBox, qOverload<int>(&QSpinBox::valueChanged),
          this, &ccDisplaySettingsDlg::changeLabelOpacity);
  connect(m_ui->labelMarkerSizeSpinBox, qOverload<int>(&QSpinBox::valueChanged),
          this, &ccDisplaySettingsDlg::changeLabelMarkerSize);

  connect(m_ui->zoomSpeedDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccDisplaySettingsDlg::changeZoomSpeed);
  connect(m_ui->maxCloudSizeDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccDisplaySettingsDlg::changeMaxCloudSize);
  connect(m_ui->maxMeshSizeDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccDisplaySettingsDlg::changeMaxMeshSize);

  connect(m_ui->autoComputeOctreeComboBox,
          qOverload<int>(&QComboBox::currentIndexChanged), this,
          &ccDisplaySettingsDlg::changeAutoComputeOctreeOption);
  connect(m_ui->pickingCursorComboBox,
          qOverload<int>(&QComboBox::currentIndexChanged), this,
          &ccDisplaySettingsDlg::changePickingCursor);
  connect(m_ui->logVerbosityComboBox,
          qOverload<int>(&QComboBox::currentIndexChanged), this,
          &ccDisplaySettingsDlg::changeLogVerbosity);

  connect(m_ui->okButton, &QAbstractButton::clicked, this,
          &ccDisplaySettingsDlg::doAccept);
  connect(m_ui->applyButton, &QAbstractButton::clicked, this,
          &ccDisplaySettingsDlg::apply);
  connect(m_ui->resetButton, &QAbstractButton::clicked, this,
          &ccDisplaySettingsDlg::reset);
  connect(m_ui->cancelButton, &QAbstractButton::clicked, this,
          &ccDisplaySettingsDlg::doReject);
  ...
  // fill the application style combo-box
  {
    ...
    // and since it's the first time the user opens the settings for this
    // session, then, the style was not changed.
    QSettings settings;
...
void ccDisplaySettingsDlg::refresh() {
  // Parameters
  {
    ...
    ccQtHelpers::SetButtonColor(m_ui->specularColorButton,
    ...
    ccQtHelpers::SetButtonColor(m_ui->labelBkgColorButton,
                                m_labelBackgroundCol);
    ...
    m_ui->enableGradientCheckBox->setChecked(
        m_parameters.drawBackgroundGradient);
    m_ui->decimateMeshBox->setChecked(m_parameters.decimateMeshOnMove);
    m_ui->maxMeshSizeDoubleSpinBox->setValue(m_parameters.minLoDMeshSize /
                                             1000000.0);
    ...
    m_ui->maxCloudSizeDoubleSpinBox->setValue(m_parameters.minLoDCloudSize /
                                              1000000.0);
    m_ui->useVBOCheckBox->setChecked(m_parameters.useVBOs);
    m_ui->showCrossCheckBox->setChecked(m_parameters.displayCross);
    m_ui->singleClickPickingCheckBox->setChecked(
        m_parameters.singleClickPicking);

    m_ui->colorScaleShowHistogramCheckBox->setChecked(
        m_parameters.colorScaleShowHistogram);
    m_ui->useColorScaleShaderCheckBox->setChecked(
        m_parameters.colorScaleUseShader);
    m_ui->useColorScaleShaderCheckBox->setEnabled(
        m_parameters.colorScaleShaderSupported);
    ...
    m_ui->autoComputeOctreeComboBox->setCurrentIndex(
        m_parameters.autoComputeOctree);
    ...
    m_ui->logVerbosityComboBox->setCurrentIndex(
        std::min(static_cast<int>(m_parameters.logVerbosityLevel),
                 static_cast<int>(ccLog::LOG_WARNING)));
  }

  // Options
  {
    m_ui->autoDisplayNormalsCheckBox->setChecked(
        m_options.normalsDisplayedByDefault);
...
void ccDisplaySettingsDlg::changeMeshFrontDiffuseColor() {
  QColor newCol = QColorDialog::getColor(m_meshFrontDiff, this, QString(),
...
void ccDisplaySettingsDlg::changeMeshBackDiffuseColor() {
  QColor newCol = QColorDialog::getColor(m_meshBackDiff, this, QString(),
                                         QColorDialog::ShowAlphaChannel);
  if (!newCol.isValid())
    return;
...
void ccDisplaySettingsDlg::changeMeshSpecularColor() {
  QColor newCol = QColorDialog::getColor(m_meshSpecularColor, this, QString(),
                                         QColorDialog::ShowAlphaChannel);
  if (!newCol.isValid())
    return;
...
void ccDisplaySettingsDlg::changePointsColor() {
  QColor newCol = QColorDialog::getColor(m_pointsDefaultCol, this, QString(),
                                         QColorDialog::ShowAlphaChannel);
  if (!newCol.isValid())
    return;
...
void ccDisplaySettingsDlg::changeBBColor() {
  QColor newCol = QColorDialog::getColor(m_bbDefaultCol, this, QString(),
                                         QColorDialog::ShowAlphaChannel);
  if (!newCol.isValid())
    return;
...
void ccDisplaySettingsDlg::changeTextColor() {
  QColor newCol = QColorDialog::getColor(m_textDefaultCol, this, QString(),
                                         QColorDialog::ShowAlphaChannel);
  if (!newCol.isValid())
    return;
...
void ccDisplaySettingsDlg::changeLabelBackgroundColor() {
  QColor newCol = QColorDialog::getColor(m_labelBackgroundCol, this, QString(),
                                         QColorDialog::ShowAlphaChannel);
  if (!newCol.isValid())
    return;
...
void ccDisplaySettingsDlg::changeLabelMarkerColor() {
  QColor newCol = QColorDialog::getColor(m_labelMarkerCol, this, QString(),
                                         QColorDialog::ShowAlphaChannel);
  if (!newCol.isValid())
    return;
...
void ccDisplaySettingsDlg::changeLogVerbosity(int index) {
  if (index >= 0 && index < ccLog::LOG_ERROR) {
    m_parameters.logVerbosityLevel =
        static_cast<ccLog::MessageLevelFlags>(index);
  } else {
    // unexepected value
    assert(false);
  }
...
void ccDisplaySettingsDlg::changeAutoComputeOctreeOption(int index) {
  ...
  m_parameters.autoComputeOctree =
      static_cast<ccGui::ParamStruct::ComputeOctreeForPicking>(index);
}
...
void ccDisplaySettingsDlg::apply() {
  ...
  if (ccLog::VerbosityLevel() != m_parameters.logVerbosityLevel) {
    ...
    ccLog::Print(QString("New log verbosity level: %1")
                     .arg(m_parameters.logVerbosityLevel));
  }

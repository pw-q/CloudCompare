...
public:
  bool nativeEventFilter(const QByteArray &eventType, void *msg,
...
bool Mouse3DInput::connect(QWidget *mainWidget, QString appName) {
  ...
  if (SiGetDeviceInfo(m_siHandle, &info) == SPW_NO_ERROR) {
    ...
    ccLog::Print(tr("[3D Mouse] Device: %1 (%2 buttons) - firmware v%3.%4")
                     .arg(name.name)
                     .arg(info.numButtons)
...
bool Mouse3DInput::onSiEvent(void *siGetEventData) {
  ...
  case SI_MOTION_EVENT: {
    ...
    if (eventData.mData[SI_TX] != 0 || eventData.mData[SI_TY] != 0 ||
      ...
      double ds = eventData.period * c_3dmouseAngularVelocity; // period is in
                                                               // ms
      // translation data
...
void Mouse3DInput::Apply(const std::vector<float> &motionData,
  ...
  if (!bubbleViewMode) {
    ...
    // ccLog::Print(QString("Mouse translation:
    ...
    if (CCCoreLib::GreaterThanEpsilon(std::abs(X)) ||
      ...
      if (viewParams.perspectiveView) {
        double distanceToWidthRatio =
            win->getViewportParameters().computeDistanceToWidthRatio();
  ...
  if (CCCoreLib::GreaterThanEpsilon(std::abs(vec[3])) ||
    ...
    // ccLog::Print(QString("Mouse rotation:
    ...
    if (!bubbleViewMode) {
      ...
      win->rotateBaseViewMat(viewParams.objectCenteredView ? rotMat
                                                           : rotMat.inverse());
    } else {
      // Ry = horizontal
      // Rx = vertical

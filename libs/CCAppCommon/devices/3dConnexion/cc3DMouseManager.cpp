...
cc3DMouseManager::cc3DMouseManager(ccMainAppInterface *appInterface,
                                   QObject *parent)
...
void cc3DMouseManager::enableDevice(bool state, bool silent) {
  ...
  if (state) {
    ...
    if (m3dMouseInput->connect(m_appInterface->getMainWindow(),
      ...
      if (!silent) {
        ccLog::Error(
            "[3D Mouse] No device found"); // warning message has already been
                                           // issued by Mouse3DInput::connect
      }
      state = false;
    }
  } else {

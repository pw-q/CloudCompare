...
ccGamepadManager::ccGamepadManager(ccMainAppInterface *appInterface,
                                   QObject *parent)
    : QObject(parent), m_appInterface(appInterface), m_gamepadInput(nullptr),
      m_menu(nullptr), m_actionEnable(nullptr) {
  setupMenu();
  setupGamepadInput(); // DGM:at this point we can only init the gamepadInput
                       // structure (Qt will send a signal when the gamepad is
                       // connected - at least from its point of view)
#ifdef CC_GAMEPAD_SUPPORT
#if QT_VERSION >= QT_VERSION_CHECK(5, 9, 0)
#if QT_VERSION < QT_VERSION_CHECK(5, 10, 0)
  enableDevice(true,
               true); // DGM: due to the workaround (in main.cpp) if it's
                      // already connected, we'll have to 'enable' it ourselves
#endif
#endif
#endif
}
...
void ccGamepadManager::enableDevice(bool state, bool silent,
  ...
  if (state) {
    for (int step = 0; step < 1; ++step) // fake loop for easy break
    {
      ...
      if (deviceID < 0) {
        ...
        if (gamepads.empty()) {
          ...
          ccLog::Print(
              QString("[Gamepad] Found %1 gamepad(s)").arg(gamepads.size()));
        }
        ...
        if (!silent && gamepads.size() > 1) {
          ...
          ccPickOneElementDlg poeDlg(
              ...
              m_appInterface ? m_appInterface->getMainWindow() : nullptr);
          for (int id : gamepads) {
      ...
      if (m_gamepadInput->isConnected()) {
        ccLog::Print(
            QString("[Gamepad] Device %1 is now enabled").arg(deviceID));
        m_gamepadInput->start();
        break;
      } else {
        showMessage(
            QString("[Gamepad] Device %1 is not connected").arg(deviceID),
            silent);
        state = false;
        break;
      }
    ...
    m_gamepadInput->stop();
    ccLog::Warning(QString("[Gamepad] Device %1 is now disabled")
...
void ccGamepadManager::setupGamepadInput() {
  ...
  if (manager) {
    connect(manager, &QGamepadManager::gamepadConnected, this,
            [&](int deviceId) {
              ccLog::Print(
                  QString("gamepad device %1 has been detected").arg(deviceId));
    ...
    connect(
        manager, &QGamepadManager::gamepadDisconnected, this,
        [&](int deviceId) {
          ccLog::Print(
              QString("gamepad device %1 has been disconnected").arg(deviceId));
          // auto-disable the device (if this device is the one currently
          // enabled)
          if (m_actionEnable && m_actionEnable->isChecked() && m_gamepadInput &&

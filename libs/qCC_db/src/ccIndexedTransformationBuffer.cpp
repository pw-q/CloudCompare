...
ccIndexedTransformationBuffer::ccIndexedTransformationBuffer(
    const QString &name)
    : ccHObject(name), m_bBoxValidSize(0), m_showAsPolyline(false),
      m_showTrihedrons(true), m_trihedronsScale(1.0f) {
  lockVisibility(false);
}

ccIndexedTransformationBuffer::ccIndexedTransformationBuffer(
    const ccIndexedTransformationBuffer &buffer)
    : ccHObject(buffer), m_bBox(buffer.m_bBox),
  ...
  try {
    ...
    ccLog::Warning("[ccIndexedTransformationBuffer] Failed to copy original "
...
bool ccIndexedTransformationBuffer::findNearest(
    double index, const ccIndexedTransformation *&trans1,
    const ccIndexedTransformation *&trans2, size_t *trans1IndexInBuffer,
...
bool ccIndexedTransformationBuffer::getInterpolatedTransformation(
    double index, ccIndexedTransformation &trans,
...
short ccIndexedTransformationBuffer::minimumFileVersion_MeOnly() const {
  short minVersion =
      std::max(static_cast<short>(34), ccHObject::minimumFileVersion_MeOnly());
  if (!empty()) {
    minVersion = std::max(
        minVersion,
        front().minimumFileVersion()); // we assume they are all the same
  }
  return minVersion;
}
...
void ccIndexedTransformationBuffer::drawMeOnly(CC_DRAW_CONTEXT &context) {
  ...
  // show path
  {
    ...
    glFunc->glBegin(count > 1 && m_showAsPolyline
                        ? GL_LINE_STRIP
                        : GL_POINTS); // show path as a polyline or points?
    for (ccIndexedTransformationBuffer::const_iterator it = begin();
         it != end(); ++it)

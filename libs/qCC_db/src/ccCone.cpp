...
ccCone::ccCone(PointCoordinateType bottomRadius, PointCoordinateType topRadius,
               PointCoordinateType height, PointCoordinateType xOff /*=0*/,
    ...
    : ccGenericPrimitive(name, transMat, uniqueID),
      m_bottomRadius(std::abs(bottomRadius)), m_topRadius(std::abs(topRadius)),
      m_xOff(xOff), m_yOff(yOff), m_height(std::abs(height)) {
  setDrawingPrecision(std::max<unsigned>(
      precision, MIN_DRAWING_PRECISION)); // automatically calls buildUp &
                                          // applyTransformationToVertices
}

ccCone::ccCone(QString name /*="Cylinder"*/)
    : ccGenericPrimitive(name), m_bottomRadius(0), m_topRadius(0), m_xOff(0),
      m_yOff(0), m_height(0) {}

ccGenericPrimitive *ccCone::clone() const {
  return finishCloneJob(new ccCone(m_bottomRadius, m_topRadius, m_height,
                                   m_xOff, m_yOff, &m_transformation, getName(),
                                   m_drawPrecision));
}
...
bool ccCone::buildUp() {
  ...
  // invalid dimensions?
  if (CCCoreLib::LessThanEpsilon(m_height) ||
      CCCoreLib::LessThanEpsilon(m_bottomRadius + m_topRadius)) {
    return false;
  }
  ...
  // then, angular sweep for top and/or bottom surfaces
  {
    ...
    // side normals
    {
      for (unsigned i = 0; i < steps; ++i) {
        ...
        CCVector3 v(
            bottomCenter.x - topCenter.x + u.y * (m_bottomRadius - m_topRadius),
    ...
    if (!singlePointBottom && !singlePointTop) {
      ...
      for (unsigned i = 0; i < steps; ++i) {
        ...
        addTriangleNormalIndexes(
            2 + i, 2 + iNext,
            2 + iNext); // TODO: middle normal should be halfbetween?!
      }

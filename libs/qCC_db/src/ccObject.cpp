...
/** Versions:
        ...
        V2.3 - 02/07/2013 - attribute 'm_selectionBehavior' added to ccHObject
class v2.4 - 02/22/2013 - per-cloud point size + whether name is displayed in 3D
...
offset and names as std::string
**/
...
ccObject::ccObject(const QString &name,
    ...
    : m_name(name.isEmpty() ? "unnamed" : name), m_flags(CC_ENABLED),
      m_uniqueID(uniqueID == ccUniqueIDGenerator::InvalidUniqueID
                     ? GetNextUniqueID()
                     : uniqueID) {}

ccObject::ccObject(const ccObject &object)
    : m_name(object.m_name), m_flags(object.m_flags),
      m_uniqueID(GetNextUniqueID()), m_metaData(object.m_metaData) {}
...
bool ccObject::fromFile(QFile &in, short dataVersion, int flags,
  ...
  if (oldToNewIDMap.contains(uniqueID)) {
    ccLog::Warning(QString("Malformed file: uniqueID #%1 is used several "

...
ccHObject::ccHObject(
    ...
    : ccObject(name, uniqueID), ccDrawableObject(), m_parent(nullptr),
      m_selectionBehavior(SELECTION_AA_BBOX), m_isDeleting(false) {
  setVisible(false);
  lockVisibility(true);
...
ccHObject::ccHObject(const ccHObject &object)
    : ccObject(object), ccDrawableObject(object), m_parent(nullptr),
      m_selectionBehavior(object.m_selectionBehavior),
      m_glTransHistory(object.m_glTransHistory), m_isDeleting(false) {}
  ...
  for (std::map<ccHObject *, int>::const_iterator it = m_dependencies.begin();
    ...
    if ((it->second & DP_DELETE_OTHER) == DP_DELETE_OTHER) {
      it->first->removeDependencyFlag(
          this, DP_NOTIFY_OTHER_ON_DELETE); // in order to avoid any loop!
      // delete object
...
ccHObject *ccHObject::New(CC_CLASS_ENUM objectType,
  ...
  case CC_TYPES::GBL_SENSOR:
    ...
    // CCCoreLib::GroundBasedLidarSensor::setRotationOrder)
  ...
  case CC_TYPES::POINT_KDTREE:
    ...
    ccLog::ErrorDebug("[ccHObject::New] This object (type %i) can't be "
...
ccHObject *ccHObject::New(const QString &pluginId, const QString &classId,
                          const char *name) {
...
void ccHObject::addDependency(ccHObject *otherObject, int flags,
                              bool additive /*=true*/) {
...
void ccHObject::removeDependencyWith(ccHObject *otherObject) {
  m_dependencies.erase(const_cast<ccHObject *>(
      otherObject)); // DGM: not sure why erase won't accept a const pointer?!
                     // We try to modify the map here, not the pointer object!
...
bool ccHObject::addChild(ccHObject *child,
  ...
  child->addDependency(
      this,
      DP_NOTIFY_OTHER_ON_DELETE); // DGM: potentially redundant with calls to
                                  // 'addDependency' but we can't miss that ;)
...
unsigned ccHObject::filterChildren(
    Container &filteredChildren, bool recursive /*=false*/,
    CC_CLASS_ENUM filter /*=CC_TYPES::OBJECT*/, bool strict /*=false*/,
  ...
  for (auto child : m_children) {
    if ((!strict && child->isKindOf(filter)) ||
      ...
      if (!inDisplay || child->getDisplay() == inDisplay) {
        // warning: we have to handle unicity as a sibling may be in the same
...
void ccHObject::transferChildren(ccHObject &newParent,
  ...
  for (auto child : m_children) {
    ...
    // we must explicitly remove any dependency with the child as we don't call
    // 'detachChild'
...
ccHObject::getOwnGlobalBB(bool withGLFeatures /*=false*/) {
...
ccHObject::GlobalBoundingBox
ccHObject::getGlobalBB_recursive(bool withGLFeatures /*=false*/,
                                 bool onlyEnabledChildren /*=true*/) {
...
ccBBox ccHObject::getDisplayBB_recursive(
    bool relative, const ccGenericGLDisplay *display /*=nullptr*/) {
  ccBBox box;
...
void ccHObject::drawNameIn3D(CC_DRAW_CONTEXT &context) {
  if (context.display && m_nameIn3DPosIsValid) {
    ...
    context.display->displayText(getName(), static_cast<int>(m_nameIn3DPos.x),
                                 static_cast<int>(m_nameIn3DPos.y),
                                 ccGenericGLDisplay::ALIGN_HMIDDLE |
...
void ccHObject::draw(CC_DRAW_CONTEXT &context) {
  ...
  bool drawInThisContext =
      ((m_visible || m_selected) && m_currentDisplay == context.display);
  ...
  if (m_currentDisplay == context.display && m_showNameIn3D &&
      !MACRO_EntityPicking(context)) {
    if (MACRO_Draw3D(context)) {
      // we have to compute the 2D position during the 3D pass!
      ccBBox bBox =
          getBB_recursive(true); // DGM: take the OpenGL features into account
                                 // (as some entities are purely 'GL'!)
      if (bBox.isValid()) {
...
void ccHObject::removeChild(int pos) {
  ...
  removeDependencyWith(child);
  // child->removeDependencyWith(this); //DGM: no, don't do this otherwise this
  // entity won't be warned that the child has been removed!
...
bool ccHObject::fromFile(QFile &in, short dataVersion, int flags,
  ...
  for (uint32_t i = 0; i < serializableCount; ++i) {
    ...
    if (dataVersion >= 35 && dataVersion <= 47 &&
      ...
      // bug fix: for a long time the CC_CAMERA_BIT and CC_QUADRIC_BIT were
      // wrongly defined with two bits instead of one! The additional and wrongly
      // defined bit was the CC_CUSTOM_BIT :(
    ...
    if ((classID & CC_TYPES::CUSTOM_H_OBJECT) == CC_TYPES::CUSTOM_H_OBJECT) {
      ...
      QString classId =
          child->getMetaData(ccCustomHObject::DefaultMetaDataClassName())
              .toString();
      QString pluginId =
          child->getMetaData(ccCustomHObject::DefaultMetaDataPluginName())
              .toString();
      ...
      if (newChild) // found a plugin that can deserialize it
      {
        ...
        ccLog::Warning(QString("[ccHObject::fromFile] Couldn't find a plugin "
                               "able to deserialize custom object '%1' "
                               "(class_ID = %2 / plugin_ID = %3)")
                           .arg(childName)
                           .arg(classID)
                           .arg(pluginId));
        return false; // FIXME: for now simply return false. We may want to skip
                      // it but I'm not sure if there is a simple way of doing
                      // that
      }
    }
    ...
    if (child) {
      if (child->fromFile(in, dataVersion, flags, oldToNewIDMap)) {
        ...
        addChild(child); // but it might still be partly 'valid', we'll let the
                         // user decide if (s)he takes the risk to load it
        return false;
      }
    } else {
      return CorruptError();
...
bool ccHObject::toFile_MeOnly(QFile &out, short dataVersion) const {
  ...
  /*** ccHObject takes in charge the ccDrawableObject properties (which is not a
  ...
  //'colorIsOverridden' state (dataVersion>=20)
  {
    ...
    if (m_colorIsOverridden) {
      ...
      if (out.write(reinterpret_cast<const char *>(m_tempColor.rgba),
                    sizeof(ColorCompType) * 3) <
          0) // TODO: save the alpha channel?
      {
        return WriteError();
      }
    }
...
bool ccHObject::fromFile_MeOnly(QFile &in, short dataVersion, int flags,
  ...
  //'lockedVisibility' state (dataVersion>=20)
  if (in.read(reinterpret_cast<char *>(&m_lockedVisibility), sizeof(bool)) <
      0) {
    return ReadError();
  }
  //'colorsDisplayed' state (dataVersion>=20)
  ...
  //'normalsDisplayed' state (dataVersion>=20)
  if (in.read(reinterpret_cast<char *>(&m_normalsDisplayed), sizeof(bool)) <
      0) {
    return ReadError();
  }
  //'sfDisplayed' state (dataVersion>=20)
  ...
  if (m_colorIsOverridden) {
    //'tempColor' (dataVersion>=20)
    if (in.read(reinterpret_cast<char *>(m_tempColor.rgba),

...
cc2DLabel::cc2DLabel(QString name /*=QString()*/)
    : ccHObject(name.isEmpty() ? "label" : name),
      m_showFullBody(true), m_screenPos{0.05f, 0.5f}, m_lastScreenPos{0, 0},
      m_dispPointsLegend(false), m_dispIn2D(true), m_relMarkerScale(1.0f) {
  clear(false);
...
cc2DLabel::cc2DLabel(const cc2DLabel &label, bool copyPoints /*=true*/)
    : ccHObject(label), m_showFullBody(label.m_showFullBody),
      m_screenPos(label.m_screenPos), m_lastScreenPos(label.m_lastScreenPos),
      m_labelROI(label.m_labelROI),
      m_dispPointsLegend(label.m_dispPointsLegend),
      m_dispIn2D(label.m_dispIn2D), m_relMarkerScale(label.m_relMarkerScale) {
  if (copyPoints) {
...
bool cc2DLabel::move2D(int x, int y, int dx, int dy, int screenWidth,
                       int screenHeight) {
...
void cc2DLabel::updateName() {
  ...
  case 2: {
    if (m_pickedPoints[0].entity() == m_pickedPoints[1].entity()) {
      ...
      setName(QString("Vector ") + m_pickedPoints[0].prefix(POINT_INDEX_0) +
              QString("@") + ENTITY_INDEX_0 + QString(" - ") +
              m_pickedPoints[1].prefix(POINT_INDEX_1) + QString("@") +
              ENTITY_INDEX_1);
    }
  } break;
  ...
  case 3: {
    if (m_pickedPoints[0].entity() == m_pickedPoints[2].entity() &&
      ...
      setName(QString("Triplet ") + m_pickedPoints[0].prefix(POINT_INDEX_0) +
              QString("@") + ENTITY_INDEX_0 + QString(" - ") +
              m_pickedPoints[1].prefix(POINT_INDEX_1) + QString("@") +
              ENTITY_INDEX_1 + QString(" - ") +
              m_pickedPoints[2].prefix(POINT_INDEX_2) + QString("@") +
              ENTITY_INDEX_2);
    }
  } break;
  }
...
bool cc2DLabel::addPickedPoint(ccGenericMesh *mesh, unsigned triangleIndex,
                               const CCVector2d &uv,
                               bool entityCenter /*=false*/) {
...
bool cc2DLabel::toFile_MeOnly(QFile &out, short dataVersion) const {
  ...
  for (std::vector<PickedPoint>::const_iterator it = m_pickedPoints.begin();
    ...
    if (out.write((const char *)&index, 4) < 0)
      return WriteError();
    // cloud ID (will be retrieved later --> make sure that the cloud is saved
    // alongside!)
    uint32_t cloudID =
        static_cast<uint32_t>(it->_cloud ? it->_cloud->getUniqueID() : 0);
    if (out.write((const char *)&cloudID, 4) < 0)
      return WriteError();

    // mesh ID (dataVersion >= 49 - will be retrieved later --> make sure that
    // the mesh is saved alongside!)
    uint32_t meshID =
        static_cast<uint32_t>(it->_mesh ? it->_mesh->getUniqueID() : 0);
...
void AddPointCoordinates(QStringList &body, QString pointShortName,
                         const CCVector3 &P,
                         const ccShiftedObject &shiftedObject, int precision) {
...
void AddPointCoordinates(QStringList &body, const cc2DLabel::PickedPoint &pp,
                         int precision, QString pointName = QString()) {
  ...
  if (pp._cloud) {
    ...
    if (pp.entityCenterPoint)
      pointShortName =
          CENTER_STRING + QString("@%1").arg(pp._cloud->getUniqueID());
    else
    ...
    if (pp.entityCenterPoint)
      pointShortName =
          CENTER_STRING + QString("@%1").arg(pp._mesh->getUniqueID());
    else
  ...
  AddPointCoordinates(body, pointShortName, pp.getPointPosition(),
                      *shiftedObject, precision);
}
    ...
    if (pp._cloud) {
      ...
      if (info.hasSF) {
        ...
        if (pp._cloud->isA(CC_TYPES::POINT_CLOUD)) {
          sf = static_cast<ccPointCloud *>(pp._cloud)
      ...
      if (info.hasSF) {
        CCCoreLib::VerticesIndexes *vi =
            pp._mesh->getTriangleVertIndexes(pp.index);
        assert(vi);
        ...
        if (vertices->isA(CC_TYPES::POINT_CLOUD)) {
          sf = static_cast<ccPointCloud *>(vertices)
                   ->getCurrentDisplayedScalarField();
        }
        ...
        if (ccScalarField::ValidValue(s1) && ccScalarField::ValidValue(s2) &&
            ccScalarField::ValidValue(s3)) {
          info.sfValue =
              static_cast<ScalarType>(s1 * w.u[0] + s2 * w.u[1] + s3 * w.u[2]);
        }
...
void cc2DLabel::getLabelInfo3(LabelInfo3 &info) const {
  ...
  // angle
  info.angles.u[0] =
      CCCoreLib::RadiansToDegrees(P1P2.angle_rad(P1P3)); // angleAtP1
  info.angles.u[1] =
      CCCoreLib::RadiansToDegrees(P2P3.angle_rad(-P1P2)); // angleAtP2
  info.angles.u[2] = CCCoreLib::RadiansToDegrees(
...
QStringList cc2DLabel::getLabelContent(int precision) const {
  ...
  case 1: // point
  {
    ...
    // color
    if (info.hasRGB) {
      QString colorStr = QString("Color: (%1;%2;%3;%4)")
                             .arg(info.color.r)
                             .arg(info.color.g)
                             .arg(info.color.b)
                             .arg(info.color.a);
  ...
  case 2: // vector
  {
    ...
    QString vecStr = MathSymbolDelta +
...
void cc2DLabel::drawMeOnly3D(CC_DRAW_CONTEXT &context) {
  ...
  // we can't use the context 'ccGLCameraParameters' (viewport, modelView
  // matrix, etc. ) because it doesn't take the temporary 'GL transformation'
  // into account! context.display->getGLCameraParameters(camera);
  ...
  case 2: {
    // This is now done in 2D so that it's always displayed on top of the
    // entities
    ////segment width
    ...
    // display point marker as spheres
    {
      ...
      for (size_t i = 0; i < count; i++) {
        ...
        if (viewportParams.perspectiveView && viewportParams.zFar > 0) {
          // in perspective view, the actual scale depends on the distance to
          // the camera!
          double d = (camera.modelViewMat * P).norm();
          double unitD =
              viewportParams.zFar /
              2; // we consider that the 'standard' scale is at half the depth
          scale = static_cast<float>(
              scale *
              sqrt(d / unitD)); // sqrt = empirical (probably because the marker
                                // size is already partly compensated by
                                // ccGLWindowInterface::computeActualPixelSize())
        }
...
struct Tab {
  ...
  Tab(int _maxBlockPerRow = 2)
      : maxBlockPerRow(_maxBlockPerRow), blockCount(0), rowCount(0),
        colCount(0) {}
  ...
  // in 2D so that they always appear above the entities
  {
    ...
    if (visibleCount) {
      ...
      if (count > 1) {
        ...
        for (unsigned j = 0; j < count; ++j) {
          glFunc->glVertex2d(m_pickedPoints[j].pos2D.x - halfW,
                             m_pickedPoints[j].pos2D.y - halfH);
        }
        glFunc->glEnd();
        glFunc->glPopAttrib(); // GL_LINE_BIT
      }
        ...
        for (size_t j = 0; j < count; j++) {
          ...
          context.display->displayText(
              ...
              static_cast<int>(m_pickedPoints[j].pos2D.y) +
                  context.labelMarkerTextShift_pix,
              ccGenericGLDisplay::ALIGN_DEFAULT, context.labelOpacity / 100.0f,
              &ccColor::white, &font);
        }
      }
...
#define DRAW_CONTENT_AS_TAB
  ...
  if (!entityPickingMode) {
    ...
    bodyFont =
        context.display
            ->getLabelDisplayFont(); // takes rendering zoom into account!
    ...
    // int buttonSize    = static_cast<int>(c_buttonSize * context.renderZoom);
    {
      ...
      if (m_showFullBody) {
        ...
        try {
          if (count == 1) {
            ...
            if (info.hasNormal) {
              ...
              tab.colContent[c] << "Nx";
              tab.colContent[c + 1]
                  << QString::number(info.normal.x, 'f', precision);
              tab.colContent[c] << "Ny";
              tab.colContent[c + 1]
                  << QString::number(info.normal.y, 'f', precision);
              tab.colContent[c] << "Nz";
              tab.colContent[c + 1]
                  << QString::number(info.normal.z, 'f', precision);
            }
            ...
            // 1st block: dX, dY, dZ
            {
              ...
              tab.colContent[c + 1]
                  << QString::number(info.diff.x, 'f', precision);
              tab.colContent[c] << MathSymbolDelta + QString("Y");
              tab.colContent[c + 1]
                  << QString::number(info.diff.y, 'f', precision);
              tab.colContent[c] << MathSymbolDelta + QString("Z");
              tab.colContent[c + 1]
                  << QString::number(info.diff.z, 'f', precision);
            }
            ...
            // next block: edges length
            {
              ...
              tab.colContent[c] << "AB";
              tab.colContent[c + 1]
                  << QString::number(info.edges.u[0], 'f', precision);
              tab.colContent[c] << "BC";
              tab.colContent[c + 1]
                  << QString::number(info.edges.u[1], 'f', precision);
              tab.colContent[c] << "CA";
              tab.colContent[c + 1]
                  << QString::number(info.edges.u[2], 'f', precision);
            }
            // next block: angles
            {
              int c = tab.add2x3Block();
              ...
              tab.colContent[c + 1]
                  << QString::number(info.angles.u[0], 'f', precision);
              tab.colContent[c] << "angle.B";
              tab.colContent[c + 1]
                  << QString::number(info.angles.u[1], 'f', precision);
              tab.colContent[c] << "angle.C";
              tab.colContent[c + 1]
                  << QString::number(info.angles.u[2], 'f', precision);
            }
            // next block: normal
            {
              int c = tab.add2x3Block();
        ...
        int tabWidth =
            totalWidth + tab.colCount * (2 * tabMarginX); // add inner margins
  ...
  glFunc->glMatrixMode(GL_MODELVIEW);
  glFunc->glPushMatrix();
  glFunc->glTranslatef(static_cast<GLfloat>(xStart - halfW),
                       static_cast<GLfloat>(yStart - halfH), 0);

  // display the arrow from the 2D rectangle to the 3D point
  if (!entityPickingMode) {
    // compute arrow base position relatively to the label rectangle (for 0 to
    // 8)
    ...
    if (arrowBaseConfig != 4) // 4 = label above point!
    {
      ...
      case 0: // top-left corner
        glFunc->glVertex2i(m_labelROI.left(),
                           -m_labelROI.top() - 2 * arrowBaseSize);
        glFunc->glVertex2i(m_labelROI.left(), -m_labelROI.top());
        glFunc->glVertex2i(m_labelROI.left() + 2 * arrowBaseSize,
                           -m_labelROI.top());
        break;
      case 1: // top-middle edge
        glFunc->glVertex2i(
            std::max(m_labelROI.left(), iArrowDestX - arrowBaseSize),
            -m_labelROI.top());
        glFunc->glVertex2i(
            std::min(m_labelROI.right(), iArrowDestX + arrowBaseSize),
            -m_labelROI.top());
        break;
      case 2: // top-right corner
        glFunc->glVertex2i(m_labelROI.right(),
                           -m_labelROI.top() - 2 * arrowBaseSize);
        glFunc->glVertex2i(m_labelROI.right(), -m_labelROI.top());
        glFunc->glVertex2i(m_labelROI.right() - 2 * arrowBaseSize,
                           -m_labelROI.top());
        break;
      case 3: // middle-left edge
        glFunc->glVertex2i(
            m_labelROI.left(),
            std::min(-m_labelROI.top(), iArrowDestY + arrowBaseSize));
        glFunc->glVertex2i(
            m_labelROI.left(),
            std::max(-m_labelROI.bottom(), iArrowDestY - arrowBaseSize));
        break;
      ...
      case 6: // bottom-left corner
        glFunc->glVertex2i(m_labelROI.left(),
                           -m_labelROI.bottom() + 2 * arrowBaseSize);
  ...
  if (!entityPickingMode) {
    ...
    // label title
    context.display->displayText(title, xStart + xStartRel, yStart + yStartRel,
                                 ccGenericGLDisplay::ALIGN_DEFAULT, 0,
                                 &defaultTextColor, &titleFont);
    yStartRel -= margin;
    ...
    if (m_showFullBody) {
      ...
      for (int c = 0; c < tab.colCount; ++c) {
        ...
        bool labelCol = ((c & 1) == 0);
        const ccColor::Rgba *textColor =
            labelCol ? &ccColor::white : &defaultTextColor;
        ...
        for (int r = 0; r < actualRowCount; ++r) {
          ...
          if (labelCol) {
            ...
            glFunc->glBegin(GL_QUADS);
            glFunc->glVertex2i(m_labelROI.left() + xCol,
                               -m_labelROI.top() + yRow);
            glFunc->glVertex2i(m_labelROI.left() + xCol,
                               -m_labelROI.top() + yRow - height);
            glFunc->glVertex2i(m_labelROI.left() + xCol + width,
                               -m_labelROI.top() + yRow - height);
            glFunc->glVertex2i(m_labelROI.left() + xCol + width,
                               -m_labelROI.top() + yRow);
            glFunc->glEnd();
          ...
          context.display->displayText(str, xStart + xCol + tabMarginX + xShift,
      ...
      if (!body.empty()) {
        ...
        for (int i = 0; i < body.size(); ++i) {
          yStartRel -= rowHeight;
          context.display->displayText(body[i], xStart + xStartRel,
                                       yStart + yStartRel,
                                       ccGenericGLDisplay::ALIGN_DEFAULT, 0,
...
bool cc2DLabel::pointPicking(const CCVector2d &clickPos,
  ...
  nearestSquareDist = -1.0;
  {
    ...
    for (unsigned i = 0; i < size(); ++i) {
      ...
      ccGLMatrix trans;
      bool noGLTrans =
          pp.entity() ? !pp.entity()->getAbsoluteGLTransformation(trans) : true;

...
ccOctree::ccOctree(ccGenericPointCloud *aCloud)
    : CCCoreLib::DgmOctree(aCloud), m_theAssociatedCloudAsGPC(aCloud),
      m_displayedLevel(1), m_displayMode(WIRE), m_glListID(0),
      m_glListIsDeprecated(true), m_frustumIntersector(nullptr) {}
...
void ccOctree::draw(CC_DRAW_CONTEXT &context,
                    ccColor::Rgb *pickingColor /*=nullptr*/) {
  ...
  if (m_displayMode == WIRE) {
    ...
    if (m_glListIsDeprecated) {
      ...
      if (m_displayMode == MEAN_POINTS) {
        void *additionalParameters[] = {
            reinterpret_cast<void *>(&glParams),
        ...
        executeFunctionForAllCellsAtLevel(m_displayedLevel, &DrawCellAsAPoint,
                                          additionalParameters);
        glFunc->glEnd();
        ...
        executeFunctionForAllCellsAtLevel(
            m_displayedLevel, &DrawCellAsAPrimitive, additionalParameters);
      } else {
        assert(false);
      }
...
bool ccOctree::DrawCellAsABox(
  ...
  QOpenGLFunctions_2_1 *glFunc =
      static_cast<QOpenGLFunctions_2_1 *>(additionalParameters[1]);
  assert(glFunc != nullptr);
...
bool ccOctree::DrawCellAsAPoint(
  ...
  if (glParams->showSF) {
    ScalarType dist =
        CCCoreLib::ScalarFieldTools::computeMeanScalarValue(cell.points);
  ...
  const CCVector3 *gravityCenter =
      CCCoreLib::Neighbourhood(cell.points).getGravityCenter();
...
ccColor::Rgb ccOctree::ComputeAverageColor(CCCoreLib::ReferenceCloud *subset,
  ...
  assert(subset->getAssociatedCloud() ==
         static_cast<CCCoreLib::GenericIndexedCloud *>(sourceCloud));

  Tuple3Tpl<double> sum(0, 0, 0);

  unsigned n = subset->size();
  for (unsigned i = 0; i < n; ++i) {
    const ccColor::Rgb &_theColor =
...
CCVector3 ccOctree::ComputeAverageNorm(CCCoreLib::ReferenceCloud *subset,
                                       ccGenericPointCloud *sourceCloud) {
  CCVector3 N(0, 0, 0);
...
bool ccOctree::intersectWithFrustum(ccCameraSensor *sensor,
  ...
  sensor->computeGlobalPlaneCoefficients(globalPlaneCoefficients, globalCorners,
                                         globalEdges, globalCenter);
  ...
  m_frustumIntersector->computeFrustumIntersectionWithOctree(
      pointsToTest, inCameraFrustum, globalPlaneCoefficients, globalCorners,
      globalEdges, globalCenter);
...
bool ccOctree::pointPicking(const CCVector2d &clickPos,
  ...
  CCVector3 rayOrigin;
  {
    ...
    rayAxis.normalize(); // normalize afterwards as the local transformation may
                         // have a scale != 1
  }
  ...
  if (camera.perspective) {
    maxFOV_rad = 0.002 * pickWidth_pix; // empirical conversion from pixels to
  ...
  if (!CCCoreLib::AABB<PointCoordinateType>(m_dimMin - margin,
                                            m_dimMax + margin)
           .intersects(ray)) {
    // no intersection
    return true; // DGM: false would mean that an error occurred! (output.point
                 // == 0 means that nothing has been found)
  }
  ...
  if (m_theAssociatedCloudAsGPC->sfShown() &&
    ...
    if (sf && sf->mayHaveHiddenValues() && sf->getColorScale()) {
      // we must take this SF display parameters into account as some points may
      // be hidden!
      activeSF = sf;
    }
  }
  ...
  for (cellsContainer::const_iterator it = m_thePointsAndTheirCellCodes.begin();
    ...
    if (truncatedCode != currentCellTruncatedCode) {
      // look for the biggest 'parent' cell that englobes this cell and the
      // previous one (if any)
      while (level > 1) {
      ...
      while (level < maxLevel) {
        ...
        if (camera.perspective) {
          ...
          double dy = std::max<double>(0, sqrt(radialSqDist) -
                                              CCCoreLib::SQRT_3 * halfCellSize);
          double fov_rad = atan2(dy, dx);

          skipThisCell = (fov_rad > maxFOV_rad);
        } else {
          skipThisCell = !CCCoreLib::AABB<PointCoordinateType>(
    ...
    if (!skipThisCell) {
      ...
      if ((!visTable ||
           visTable->at(it->theIndex) == CCCoreLib::POINT_VISIBLE) &&
        ...
        if (insideFrustum) {
          if (std::abs(Q2D.x - clickPos.x) <= pickWidth_pix &&
            ...
            double squareDist =
                CCVector3d(X.x - Q.x, X.y - Q.y, X.z - Q.z).norm2d();
...
PointCoordinateType
ccOctree::GuessBestRadiusAutoComputeOctree(ccGenericPointCloud *cloud,
  ...
  if (!cloud->getOctree()) {
    ...
    if (!cloud->computeOctree(&pDlg)) {
      ccLog::Error(
          tr("Could not compute octree for cloud '%1'").arg(cloud->getName()));
      return 0;
    }
  }
...
PointCoordinateType ccOctree::GuessBestRadius(
    ccGenericPointCloud *cloud, const BestRadiusParams &params,
  ...
  // density
  {
    ...
    for (size_t attempt = 0; attempt < 10; ++attempt) {
      ...
      for (size_t i = 0; i < sampleCount; ++i) {
        ...
        int n = octree->getPointsInSphericalNeighbourhood(*P, radius, Yk,
                                                          octreeLevel);
        assert(n >= 1);
      ...
      double stdDevPop =
          sqrt(std::abs(static_cast<double>(totalSquareCount) / sampleCount -
                        meanPop * meanPop));
      double aboveMinPopRatio =
          static_cast<double>(aboveMinPopCount) / sampleCount;

      ccLog::Print(
          QString("[GuessBestRadius] Radius = %1 -> samples population in [%2 "
                  "; %3] (mean %4 / std. dev. %5 / %6% above minimum)")
              .arg(radius)
              .arg(minPop)
              .arg(maxPop)
              .arg(meanPop)
      ...
      if (std::abs(meanPop - aimedPop) < params.aimedPopulationRange) {
        ...
        if (aboveMinPopRatio < params.minAboveMinRatio) {
          // ccLog::Warning("[GuessBestRadius] The cloud density is very
          // inhomogeneous! You may have to increase the radius to get valid
          // normals everywhere... but the result will be smoother");
      ...
      //(warning: we consider below that the number of points is proportional to
      //the SURFACE of the neighborhood)
      assert(meanPop >= 1.0);
      if (attempt == 0) {
        ...
        double slope = (radius * radius - lastRadius * lastRadius) /
                       (meanPop - lastMeanPop);
        double newSquareRadius = static_cast<double>(lastRadius) * lastRadius +

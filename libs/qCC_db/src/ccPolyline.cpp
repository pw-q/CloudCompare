...
ccPolyline::ccPolyline(
  ...
  if (cloud) {
    ...
    // as it will try to set the Global Shift & Scale info on the associated
    // cloud!
...
bool ccPolyline::initWith(ccPointCloud *&vertices, const ccPolyline &poly) {
  ...
  if (!vertices) {
    ...
    if (clone) {
      if (cloud)
        clone->setName(cloud->getName()); // as 'partialClone' adds the
      ...
      // not enough memory?
      ccLog::Warning(
          "[ccPolyline::initWith] Not enough memory to duplicate vertices!");
      success = false;
    }
...
void ccPolyline::showArrow(bool state, unsigned vertIndex,
...
ccBBox ccPolyline::getOwnBB(bool withGLFeatures /*=false*/) {
  ...
  emptyBox.setValidity(
      ...
      size() != 0); // a 2D polyline is considered as a purely 'GL' fature
  return emptyBox;
}
...
void ccPolyline::drawMeOnly(CC_DRAW_CONTEXT &context) {
  ...
  bool visFiltering = (_verticesVisibility && _verticesVisibility->size() >=
                                                  getAssociatedCloud()->size());
  ...
  if (visFiltering) {
    ...
    for (unsigned i = 0; i < maxIndex; ++i) {
      ...
      if (_verticesVisibility->at(pointIndex) !=
          CCCoreLib::POINT_VISIBLE) // segment is hidden
        continue;

      unsigned nextIndex = ((i + 1) % vertCount);
      unsigned nextPointIndex = getPointGlobalIndex(nextIndex);
      if (_verticesVisibility->at(nextPointIndex) !=
          CCCoreLib::POINT_VISIBLE) // segment is hidden
        continue;
  ...
  if (m_showArrow && m_arrowIndex < vertCount &&
    ...
    if (!visFiltering ||
        (_verticesVisibility->at(i0) == CCCoreLib::POINT_VISIBLE &&
         _verticesVisibility->at(i1) == CCCoreLib::POINT_VISIBLE)) {
      ...
      if (m_mode2D) {
        ...
        static const PointCoordinateType s_defaultArrowAngle =
            CCCoreLib::DegreesToRadians(static_cast<PointCoordinateType>(15.0));
        ...
        ccGLMatrix rotMat =
            ccGLMatrix::FromToRotation(u, CCVector3(0, 0, CCCoreLib::PC_ONE));
  ...
  if (m_showVertices) {
    ...
    for (unsigned i = 0; i < vertCount; ++i) {
      if (!visFiltering ||
          _verticesVisibility->at(i) == CCCoreLib::POINT_VISIBLE) {
        ccGL::Vertex3v(glFunc, getPoint(i)->u);
      }
    }
    glFunc->glEnd();
...
bool ccPolyline::toFile_MeOnly(QFile &out, short dataVersion) const {
  ...
  // polylines) so instead we save it's unique ID (dataVersion>=28) WARNING: the
  ...
  if (!vertices) {
    ccLog::Warning("[ccPolyline::toFile_MeOnly] Polyline vertices is not a "
...
bool ccPolyline::fromFile_MeOnly(QFile &in, short dataVersion, int flags,
  ...
  // shared by multiple polylines) we only store its unique ID (dataVersion>=28)
  // --> we hope we will find it at loading time (i.e. this is the responsibility
  // of the caller to make sure that all dependencies are saved together)
  {
...
bool ccPolyline::split(PointCoordinateType maxEdgeLength,
  ...
  while (startIndex <= lastIndex) {
    ...
    if (partSize > 1) // otherwise we skip that point
    {
      ...
      ccPointCloud *subset =
          vertices ? vertices->partialClone(&ref) : ccPointCloud::From(&ref);
...
const CCVector3d &ccPolyline::getGlobalShift() const {
  const ccPointCloud *pc =
      dynamic_cast<const ccPointCloud *>(m_theAssociatedCloud);
  if (pc && pc->getParent() == this) {
...
ccPointCloud *ccPolyline::samplePoints(bool densityBased,
                                       double samplingParameter, bool withRGB) {
  ...
  for (unsigned i = 0; i < pointCount;) {
    ...
    if (relativePos >= lAB) {
      ...
      if (i + 1 == pointCount) {
        assert(
            relativePos <
            lAB * 1.01); // it should only be a rounding issue in the worst case
        relativePos = lAB;
...
ccPolyline *ccPolyline::smoothChaikin(PointCoordinateType ratio,
  ...
  const CCCoreLib::GenericIndexedCloudPersist *currentIterationVertices =
...
bool ccPolyline::IsCloudVerticesOfPolyline(ccGenericPointCloud *cloud,
  ...
  // check whether the input point cloud acts as the vertices of a polyline
  {
    ...
    if (parent && parent->isKindOf(CC_TYPES::POLY_LINE) &&
...
bool ccPolyline::createNewPolylinesFromSelection(
  ...
  if (!verticesCloud) {
    ...
    ccLog::Warning("[ccPolyline::createNewPolylinesFromSelection] Unsupported "
                   "vertex cloud");
    return false;
  }
  ...
  if (verticesVisibility.size() < verticesCloud->size()) {
    // no visibility table instantiated
    ccLog::Warning("[ccPolyline::createNewPolylinesFromSelection] No "
    ...
    for (unsigned i = 0; i < maxIndex; ++i) {
      ...
      if (verticesVisibility.at(pointIndex) == CCCoreLib::POINT_VISIBLE &&
          verticesVisibility.at(nextPointIndex) ==
              CCCoreLib::POINT_VISIBLE) // segment should be kept
      {
        kept = true;
      ...
      if (!kept || i + 1 == maxIndex) {
        ...
        if (chunkPoly) {
          ...
          chunkPoly->setName(
              getName() +
              QString(".segmented (part %1)").arg(output.size() + 1));
  ...
  if (!success) {
    ccLog::Warning(
        "[ccPolyline::createNewPolylinesFromSelection] Not enough memory");

...
ccGenericPointCloud::ccGenericPointCloud(QString name, unsigned uniqueID)
    : ccShiftedObject(name, uniqueID), m_pointSize(0) {
  setVisible(true);
  lockVisibility(false);
}

ccGenericPointCloud::ccGenericPointCloud(const ccGenericPointCloud &cloud)
    : ccShiftedObject(cloud), m_pointsVisibility(cloud.m_pointsVisibility),
...
bool ccGenericPointCloud::resetVisibilityArray() {
  ...
  std::fill(m_pointsVisibility.begin(), m_pointsVisibility.end(),
            CCCoreLib::POINT_VISIBLE); // by default, all points are visible
...
void ccGenericPointCloud::invertVisibilityArray() {
  ...
  for (unsigned char &vis : m_pointsVisibility) {
    vis = (vis == CCCoreLib::POINT_HIDDEN ? CCCoreLib::POINT_VISIBLE
...
bool ccGenericPointCloud::pointPicking(const CCVector2d &clickPos,
  ...
  if (pickWidth == pickHeight) {
    ...
    if (!octree && autoComputeOctree) {
      ccProgressDialog pDlg(false,
                            getDisplay() ? getDisplay()->asWidget() : nullptr);
    ...
    if (octree) {
      ...
      if (octree->pointPicking(clickPos, camera, point, pickWidth)) {
        ...
        ccLog::Warning("[Point picking] Failed to use the octree. We'll fall "
  ...
  nearestSquareDist = -1.0;
  {
    ...
    const ccGenericPointCloud::VisibilityTableType *visTable =
        isVisibilityTableInstantiated() ? &getTheVisibilityArray() : nullptr;
    ...
    if (sfShown() && isA(CC_TYPES::POINT_CLOUD) &&
      ...
      if (sf && sf->mayHaveHiddenValues() && sf->getColorScale()) {
        // we must take this SF display parameters into account as some points
        // may be hidden!
        activeSF = sf;
      }
    }
          ...
          continue;
            ...
            if (std::abs(Q2D.x - clickPos.x) <= pickWidth &&
              ...
              const double squareDist =
                  CCVector3d(X.x - P->x, X.y - P->y, X.z - P->z).norm2d();
...
CCCoreLib::ReferenceCloud *ccGenericPointCloud::getTheVisiblePoints(
    const VisibilityTableType *visTable /*=nullptr*/, bool silent /*=false*/,
  ...
  if (!visTable || visTable->size() != count) {
    assert(false);
    ccLog::Warning(
        "[ccGenericPointCloud::getTheVisiblePoints] Invalid visibility table!");
    return nullptr;
  }
  ...
  if (pointCount) {
    if (rc->reserve(pointCount)) {
      ...
      ccLog::Warning(
          "[ccGenericPointCloud::getTheVisiblePoints] Not enough memory!");
      delete rc;
      rc = nullptr;
    }
  } else if (!silent) {
    ccLog::Warning(
        "[ccGenericPointCloud::getTheVisiblePoints] No point in selection");
  }

...
ccDrawableObject::ccDrawableObject() : m_currentDisplay(nullptr) {
  setVisible(true);
  setSelected(false);
...
ccDrawableObject::ccDrawableObject(const ccDrawableObject &object)
    : m_visible(object.m_visible), m_selected(object.m_selected),
      ...
      m_sfDisplayed(object.m_sfDisplayed), m_tempColor(object.m_tempColor),
      m_colorIsOverridden(object.m_colorIsOverridden),
      m_glTrans(object.m_glTrans), m_glTransEnabled(object.m_glTransEnabled),
...
void ccDrawableObject::toggleClipPlanes(CC_DRAW_CONTEXT &context, bool enable) {
  ...
  if (planeCount > maxPlaneCount) {
    if (enable) {
      ccLog::Warning("[ccDrawableObject::enableClipPlanes] Clipping planes "
                     "count exceeds the maximum supported number");
    }
...
ccDrawableObject::DisplayState::DisplayState(const ccDrawableObject &dobj)
    : visible(dobj.m_visible), colorsDisplayed(dobj.m_colorsDisplayed),
      normalsDisplayed(dobj.m_normalsDisplayed),
      sfDisplayed(dobj.m_sfDisplayed),
      colorIsOverridden(dobj.m_colorIsOverridden),
      showNameIn3D(dobj.m_showNameIn3D), display(dobj.m_currentDisplay) {}

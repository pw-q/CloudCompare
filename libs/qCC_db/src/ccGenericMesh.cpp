...
ccGenericMesh::ccGenericMesh(
    ...
    : GenericIndexedMesh(), ccShiftedObject(name, uniqueID),
      m_triNormsShown(false), m_materialsShown(false), m_showWired(false),
      m_stippling(false) {
  setVisible(true);
...
void ccGenericMesh::EnableGLStippleMask(const QOpenGLContext *context,
                                        bool state) {
  ...
  if (MACRO_Draw2D(context)) {
    if (MACRO_Foreground(context) && !context.sfColorScaleToDisplay) {
      if (sfShown()) {
        ...
        // we just need to 'display' the current SF scale if the vertices cloud
        // is hidden (otherwise, it will be taken in charge by the cloud itself)
        if (!cloud->sfColorScaleShown() ||
            (cloud->sfShown() && cloud->isEnabled() && cloud->isVisible()))
          return;

        // we must also check that the parent is not a mesh itself with the same
        // vertices! (in which case it will also take that in charge)
...
void ccGenericMesh::drawMeOnly(CC_DRAW_CONTEXT &context) {
  ...
  if (MACRO_Draw3D(context)) {
    ...
    // fix 'showNorms'
    glParams.showNorms =
        showTriNormals || (vertices->hasNormals() && m_normalsDisplayed);
    ...
    if (!visFiltering && !(applyMaterials || showTextures) &&
      ...
      for (size_t k = 0; k < chunkCount; ++k, chunkStart += ccChunk::SIZE) {
        ...
        for (size_t n = 0; n < chunkSize; n += decimStep) {
          const CCCoreLib::VerticesIndexes *ti =
              getTriangleVertIndexes(static_cast<unsigned>(chunkStart + n));
        ...
        if (glParams.showNorms) {
          ...
          if (showTriNormals) {
            for (unsigned n = 0; n < chunkSize; n += decimStep) {
              ...
              CCVector3 Nb;
              CCVector3 Nc;
              getTriangleNormals(static_cast<unsigned>(chunkStart + n), Na, Nb,
                                 Nc);
              *_normals++ = Na;
              *_normals++ = Nb;
              *_normals++ = Nc;
      ...
      for (unsigned n = 0; n < triNum; ++n) {
        ...
        if (applyMaterials || showTextures) {
          ...
          if (lasMtlIndex != newMatlIndex) {
            ...
            if (newMatlIndex >= 0)
              (*materials)[newMatlIndex]->applyGL(context.qGLContext,
                                                  glParams.showNorms, false);
            else
...
ccPointCloud *ccGenericMesh::samplePoints(
    bool densityBased, double samplingParameter, bool withNormals, bool withRGB,
    bool withTexture, CCCoreLib::GenericProgressCallback *pDlg /*=nullptr*/) {
  ...
  if (densityBased) {
    sampledCloud = CCCoreLib::MeshSamplingTools::samplePointsOnMesh(
  ...
  if (sampledCloud) {
    if (sampledCloud->size() == 0) {
      ccLog::Warning("[ccGenericMesh::samplePoints] No point was generated "
  ...
  if (withFeatures && triIndices && triIndices->size() >= cloud->size()) {
    ...
    if (withNormals && hasNormals()) {
      if (cloud->reserveTheNormsTable()) {
        ...
        ccLog::Warning("[ccGenericMesh::samplePoints] Failed to interpolate "
    ...
    if (withTexture && hasMaterials()) {
      if (cloud->reserveTheRGBTable()) {
        ...
        ccLog::Warning("[ccGenericMesh::samplePoints] Failed to export texture "
...
bool ccGenericMesh::trianglePicking(unsigned triIndex,
                                    ...
                                    const ccGLMatrix &trans, bool noGLTrans,
  ...
  // if none of the vertices fall inside the frustum, the triangle is (probably)
  // not visible...
  // FIXME: if there's one huge triangle or the user zoom in a lot, if's not
  // true!
...
bool ccGenericMesh::trianglePicking(
    const CCVector2d &clickPos, const ccGLCameraParameters &camera,
    int &nearestTriIndex, double &nearestSquareDist, CCVector3d &nearestPoint,
...
//#define TEST_PICKING
#ifdef TEST_PICKING
  QImage testImage(camera.viewport[2], camera.viewport[3],
  ...
  painter.drawEllipse(
      QPointF(clickPosd.x - brushRadius_pix, clickPosd.y - brushRadius_pix),
      2 * brushRadius_pix, 2 * brushRadius_pix);
  QPen pen(Qt::black);
  pen.setWidth(3);
  painter.setPen(pen);
#endif
...
#pragma omp parallel for num_threads(omp_get_max_threads())
  ...
  for (int i = 0; i < static_cast<int>(size()); ++i) {
    CCVector3d P;
    CCVector3d BC;
    if (!trianglePicking(i, clickPos, trans, noGLTrans, *vertices, camera, P,
...
bool ccGenericMesh::trianglePicking(
    unsigned triIndex, const CCVector2d &clickPos,
    const ccGLCameraParameters &camera, CCVector3d &point,
    CCVector3d *barycentricCoords /*=nullptr*/) const {
  if (triIndex >= size()) {
    assert(false);
    return false;
...
bool ccGenericMesh::computePointPosition(
    unsigned triIndex, const CCVector2d &uv, CCVector3 &P,
...
bool ccGenericMesh::IsCloudVerticesOfMesh(ccGenericPointCloud *cloud,
  ...
  // check whether the input point cloud acts as the vertices of a mesh
  {
    ...
    if (parent && parent->isKindOf(CC_TYPES::MESH) &&
        static_cast<ccGenericMesh *>(parent)->getAssociatedCloud() == cloud) {
      if (mesh) {
  ...
  for (unsigned i = 0; i < cloud->getChildrenNumber(); ++i) {
    ...
    if (child && child->isKindOf(CC_TYPES::MESH) &&

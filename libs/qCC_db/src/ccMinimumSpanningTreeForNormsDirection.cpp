...
static bool ResolveNormalsWithMST(ccPointCloud *cloud,
                                  ...
                                  const Graph &graph,
#else
                                  ccOctree::Shared &octree, unsigned char level,
                                  unsigned kNN,
#endif
    ...
    while (visitedCount < vertexCount) {
      ...
      // set it as "visited"
      {
        ...
        for (Graph::IndexSet::const_iterator it = neighbors.begin();
             it != neighbors.end(); ++it) {
          priorityQueue.push(Edge(firstUnvisitedIndex, *it,
        ...
        unsigned neighborCount =
            octree->findNearestNeighborsStartingFromCell(nNSS, false);
        ...
        for (unsigned j = 0; j < neighborCount; ++j) {
          ...
          if (firstUnvisitedIndex != neighborIndex && !visited[neighborIndex]) {
            const CCVector3 &N2 = cloud->getPointNormal(neighborIndex);
            // dot product
            float weight =
                std::max(0.0f, 1.0f - static_cast<float>(std::abs(N1.dot(N2))));
            ...
            // mutual dot product
            // const CCVector3* P2 =
            // cloud->getPoint(static_cast<unsigned>(neighborIndex)); CCVector3
            // uAB = *P2 - *P1; uAB.normalize(); float weight =
            // (std::abs(CCVector3::vdot(uAB.u, N1) +
            // std::abs(CCVector3::vdot(uAB.u, N2)))) / 2;
      ...
      while (!priorityQueue.empty() && (visitedCount < vertexCount)) {
        ...
        const CCVector3 &N1 =
            cloud->getPointNormal(static_cast<unsigned>(element.v1()));
        const CCVector3 &N2 =
            cloud->getPointNormal(static_cast<unsigned>(element.v2()));
          ...
          for (unsigned j = 0; j < neighborCount; ++j) {
            ...
            if (v != neighborIndex && !visited[neighborIndex]) {
              ...
              float weight = std::max(
                  0.0f, 1.0f - static_cast<float>(std::abs(N1.dot(N2))));
...
#ifdef WITH_GRAPH
static bool
ComputeMSTGraphAtLevel(const CCCoreLib::DgmOctree::octreeCell &cell,
  ...
  for (unsigned i = 0; i < n; ++i) {
    ...
    // look for neighbors in a sphere
    unsigned neighborCount =
        cell.parentOctree->findNearestNeighborsStartingFromCell(nNSS, false);
    neighborCount = std::min(neighborCount, kNN + 1);
    ...
    for (unsigned j = 0; j < neighborCount; ++j) {
      ...
      if (index != neighborIndex) {
        ...
        // mutual dot product
        // const CCVector3* P2 =
        // cloud->getPoint(static_cast<unsigned>(neighborIndex)); CCVector3 uAB =
        // *P2 - *P1; uAB.normalize(); float weight =
        // (std::abs(CCVector3::vdot(uAB.u, N1) + std::abs(CCVector3::vdot(uAB.u,
        // N2)))) / 2;
...
bool ccMinimumSpanningTreeForNormsDirection::OrientNormals(
    ccPointCloud *cloud, unsigned kNN /*=6*/,
  ...
  if (!cloud->getOctree()) {
    if (!cloud->computeOctree(progressDlg)) {
      ccLog::Warning(
          QString(
              "[orientNormalsWithMST] Could not compute octree on cloud '%1'")
              .arg(cloud->getName()));
      return false;
    }
  }
  ...
  try {
    ...
    if (octree->executeFunctionForAllCellsAtLevel(
      ...
      ccLog::Warning(QString("Failed to compute Spanning Tree on cloud '%1'")
                         .arg(cloud->getName()));
      result = false;
    } else {
      if (!ResolveNormalsWithMST(cloud, graph, progressDlg)) {
        // something went wrong
        ccLog::Warning(
            QString("Failed to compute Minimum Spanning Tree on cloud '%1'")
                .arg(cloud->getName()));
        result = false;
      }
    }
    ...
    if (!ResolveNormalsWithMST(cloud, octree, level, kNN, progressDlg)) {
      // something went wrong
      ccLog::Warning(QString("Failed to resolve normals orientation with "

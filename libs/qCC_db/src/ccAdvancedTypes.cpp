...
bool NormsIndexesTableType::fromFile_MeOnly(QFile &in, short dataVersion,
                                            int flags,
  ...
  if (dataVersion < 41) {
    ...
    if (!ccSerializationHelper::GenericArrayFromFile<unsigned short, 1,
                                                     unsigned short>(
            *oldNormals, in, dataVersion, "old compressed normals")) {
      oldNormals->release();
      return false;
    }
    ...
    for (size_t i = 0; i < oldNormals->size(); ++i) {
      ...
      // and recompress
      CompressedNormType index =
          static_cast<CompressedNormType>(ccNormalCompressor::Compress(N.u));
      at(i) = index;
    }
    ...
    return ccSerializationHelper::GenericArrayFromFile<CompressedNormType, 1,
                                                       CompressedNormType>(

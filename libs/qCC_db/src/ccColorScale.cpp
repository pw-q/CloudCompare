...
ccColorScale::ccColorScale(const QString &name,
    ...
    : m_name(name), m_uuid(uuid), m_updated(false), m_relative(true),
      m_locked(false), m_absoluteMinValue(0.0), m_absoluteRange(1.0) {
  if (m_uuid.isNull())
    generateNewUuid();
}
...
ccColorScale::Shared
ccColorScale::copy(const QString &uuid /*=QString()*/) const {
...
void ccColorScale::insert(const ccColorScaleElement &step,
  ...
  if (m_locked) {
    ccLog::Warning(
        QString("[ccColorScale::insert] Scale '%1' is locked!").arg(m_name));
    return;
  }
...
void ccColorScale::clear() {
  if (m_locked) {
    ccLog::Warning(
        QString("[ccColorScale::clear] Scale '%1' is locked!").arg(m_name));
    return;
  }
...
void ccColorScale::remove(int index, bool autoUpdate /*=true*/) {
  if (m_locked) {
    ccLog::Warning(
        QString("[ccColorScale::remove] Scale '%1' is locked!").arg(m_name));
    return;
  }
...
void ccColorScale::update() {
  ...
  if (m_steps.size() >= static_cast<int>(MIN_STEPS)) {
    ...
    if (m_steps.front().getRelativePos() != 0.0 ||
      ...
      ccLog::Warning(QString("[ccColorScale] Scale '%1' is invalid! "
      ...
      for (unsigned i = 0; i < MAX_STEPS; ++i) {
        ...
        m_rgbaScale[i] = ccColor::Rgb(
            static_cast<ColorCompType>(interpCol.x * ccColor::MAX),
    ...
    ccLog::Warning(
        QString("[ccColorScale] Scale '%1' is invalid! (not enough elements)")
            .arg(getName()));
  }
        ...
        stream.writeStartElement(s_xmlColorScaleProperties);
        {
          ...
          if (!isRelative()) {
            stream.writeTextElement(
                "minValue", QString::number(m_absoluteMinValue, 'g', 12));
            stream.writeTextElement("range",
...
ccColorScale::Shared ccColorScale::LoadFromXML(const QString &filename) {
  ...
  while (true) // fake loop for easy break
  {
    ...
    if (!stream.readNextStartElement() || stream.name() != s_xmlCloudCompare) {
      break;
    }
    ...
    if (!ok || version > s_xmlColorScaleVer) {
      if (ok)
        ccLog::Warning(
            QString("[ccColorScale::LoadFromXML] Unhandled version: %1")
                .arg(version));
      break;
    }
    ...
    while (!stream.atEnd() && missingItems > 0) {
      ...
      ccLog::Print(
          QString("[XML] Item '%1': '%2'").arg(itemName.toString(), itemValue));
      ...
      if (itemName == "name") {
        ...
        if (itemValue == "1") {
          scale->setAbsolute(0, 1); // the true values will be updated
                                    // afterwards
    ...
    if (!stream.readNextStartElement() ||
      ...
      ccLog::Warning(
          QString("[ccColorScale::LoadFromXML] Unexpected element: %1")
        ...
        if (stream.name() == "step") {
          ...
          if (attributeCount < 4) {
            ccLog::Warning(QString(
                "[ccColorScale::LoadFromXML] Missing data attributes!"));
            dataError = true;
            break;
          }
          stream.skipCurrentElement();
          ...
          for (int i = 0; i < attributes.size(); ++i) {
            ...
            if (name == "VAL") {
              ...
              if (!ok) {
                ccLog::Warning(
                    QString("[ccColorScale::LoadFromXML] Invalid value:") +
                    valueStr);
  ...
  if (error) {
    scale.clear();
    ccLog::Error(
        QString("An error occurred while reading file '%1'").arg(filename));
  }

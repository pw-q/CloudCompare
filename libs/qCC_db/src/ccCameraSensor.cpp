...
ccCameraSensor::IntrinsicParameters::IntrinsicParameters()
    : vertFocal_pix(1.0f), skew(0), vFOV_rad(0), zNear_mm(0.001f),
      zFar_mm(1000.0f), arrayWidth(0), arrayHeight(0) {
  pixelSize_mm[0] = 1.0f;
  pixelSize_mm[1] = 1.0f;
...
void ccCameraSensor::IntrinsicParameters::GetKinectDefaults(
  ...
  constexpr float focal_mm =
      static_cast<float>(5.45e-3); // focal length (real distance in meter)
  constexpr float pixelSize_mm =
      static_cast<float>(9.3e-6); // pixel size (real distance in meter)
  ...
  params.zNear_mm =
      static_cast<float>(0.5); // distance to the closest recordable depth
  params.zFar_mm =
      static_cast<float>(5.0); // distance to the furthest recordable depth
...
ccCameraSensor::FrustumInformation::FrustumInformation()
    : isComputed(false), drawFrustum(false), drawSidePlanes(false),
      frustumCorners(nullptr), frustumHull(nullptr) {}
...
bool ccCameraSensor::FrustumInformation::initFrustumHull() {
  ...
  if (!frustumCorners || frustumCorners->size() < 8) {
    ccLog::Warning("[ccCameraSensor::FrustumInformation::initFrustumHull] "
  ...
  if (!frustumHull->reserve(6 * 2)) {
    ccLog::Warning("[ccCameraSensor::FrustumInformation::initFrustumHull] Not "
                   "enough memory!");
    delete frustumHull;
    frustumHull = nullptr;
    return false;
...
ccCameraSensor::ccCameraSensor()
    : ccSensor("Camera Sensor"), m_projectionMatrixIsValid(false) {
...
ccCameraSensor::ccCameraSensor(const ccCameraSensor &sensor)
    : ccSensor(sensor), m_projectionMatrix(sensor.m_projectionMatrix),
  ...
  if (m_distortionParams) {
    ...
    case EXTENDED_RADIAL_DISTORTION: {
      ...
      *clone = *static_cast<const ExtendedRadialDistortionParameters *>(
          sensor.m_distortionParams.data());
      clonedDistParams = LensDistortionParameters::Shared(clone);
    } break;
...
ccBBox ccCameraSensor::getOwnFitBB(ccGLMatrix &trans) {
  ...
  return ccBBox(-upperLeftPoint,
                CCVector3(upperLeftPoint.x, upperLeftPoint.y, 0), true);
}
...
bool ccCameraSensor::applyViewport(ccGenericGLDisplay *win /*=nullptr*/) const {
  ...
  // aspect ratio
  double sensorAR = static_cast<double>(m_intrinsicParams.arrayWidth) /
                    m_intrinsicParams.arrayHeight;
  // the sensor width (fully) fits the 3D view width
  QSize screenSize = win->getScreenSize();
  double screenAR =
      static_cast<double>(screenSize.width()) / screenSize.height();
  double fOV_rad = 2 * atan(tan(m_intrinsicParams.vFOV_rad / 2) * screenAR *
                            (screenAR >= sensorAR ? 1.0 : sensorAR));
  double fov_deg = CCCoreLib::RadiansToDegrees(fOV_rad);
  ccLog::Print(
      QString("[ccCameraSensor::applyViewport] Horizontal FOV = %1 deg")
          .arg(fov_deg));
...
bool ccCameraSensor::applyImageViewport(
    ccImage *image, ccGenericGLDisplay *win /*=nullptr*/) const {
  if (m_intrinsicParams.arrayHeight <= 0) {
    ccLog::Warning("[ccCameraSensor::applyViewport] Sensor height is 0!");
    return false;
  }
  ...
  if (!win) {
    ...
    if (!win) {
      ccLog::Warning(
          "[ccCameraSensor::applyImageViewport] No associated display");
      return false;
    }
  }

  if (image->getDisplay() != win) {
    ccLog::Warning("[ccCameraSensor::applyImageViewport] Image display does "
                   "not match with the provided or default display");
    return applyViewport(win);
  }

  // image aspect ratio
  double imageAR = static_cast<double>(m_intrinsicParams.arrayWidth) /
                   m_intrinsicParams.arrayHeight;
  ...
  double screenAR =
      static_cast<double>(screenSize.width()) / screenSize.height();
  double fOV_rad = 2 * atan((m_intrinsicParams.arrayHeight /
                             (2 * m_intrinsicParams.vertFocal_pix)) *
                            screenAR * (screenAR >= imageAR ? 1.0 : imageAR));
  double fov_deg = CCCoreLib::RadiansToDegrees(fOV_rad);
  ccLog::Print(
      QString("[ccCameraSensor::applyImageViewport] Horizontal FOV = %1 deg")
          .arg(fov_deg));
...
bool ccCameraSensor::getProjectionMatrix(ccGLMatrix &matrix) {
  ...
  return m_projectionMatrixIsValid; // even if we have computed the projection
                                    // matrix, it may still have failed!
}
...
bool ccCameraSensor::toFile_MeOnly(QFile &out, short dataVersion) const {
  ...
  DistortionModel distModel =
      m_distortionParams ? m_distortionParams->getModel() : NO_DISTORTION_MODEL;
  ...
  if (m_distortionParams) {
    ...
    case SIMPLE_RADIAL_DISTORTION: {
      RadialDistortionParameters *params =
          static_cast<RadialDistortionParameters *>(m_distortionParams.data());
      outStream << params->k1;
      outStream << params->k2;
    } break;
    ...
    case BROWN_DISTORTION: {
      BrownDistortionParameters *params =
          static_cast<BrownDistortionParameters *>(m_distortionParams.data());
...
bool ccCameraSensor::fromFile_MeOnly(QFile &in, short dataVersion, int flags,
  ...
  // FrustumInformation
  if (dataVersion < 38) {
    bool dummyBool; // formerly: m_frustumInfos.isComputed (no need to save/load
                    // it!)
    inStream >> dummyBool;
  }
  m_frustumInfos.isComputed = false;
...
bool ccCameraSensor::fromLocalCoordToImageCoord(
    const CCVector3 &localCoord, CCVector2 &imageCoord,
  ...
  // We test if the point is in front or behind the sensor ? If it is behind (or
  // in the center of the sensor i.e. z=0.0), we can't project!
  ...
  if (!getProjectionMatrix(mat))
    return false;
  CCVector3 projCoord =
      mat * imageCoordSystem; // at this stage, coordinates are homogeneous
  ...
  // We test if the point is in front or behind the sensor ? If it is behind (or
  // in the center of the sensor i.e. depth = 0), we can't project!
  double depth =
      -static_cast<double>(localCoord.z); // warning: the camera looks backward!
    ...
    if (m_distortionParams->getModel() == SIMPLE_RADIAL_DISTORTION) {
      ...
      double rp =
          ...
          norm2 *
              (params->k1 +
               norm2 *
                   params->k2); // scaling factor to undo the radial distortion
      factor *= rp;
      ...
      double rp =
          ...
          norm2 * (params->k1 +
                   norm2 * (params->k2 +
                            norm2 * params->k3)); // scaling factor to undo the
                                                  // radial distortion
      factor *= rp;
    }
  }
...
bool ccCameraSensor::fromImageCoordToLocalCoord(
    const CCVector2 &imageCoord, CCVector3 &localCoord,
    PointCoordinateType depth, bool withLensCorrection /*=true*/) const {
...
bool ccCameraSensor::fromGlobalCoordToImageCoord(
    const CCVector3 &globalCoord, CCVector2 &imageCoord,
    bool withLensError /*=true*/) const {
...
bool ccCameraSensor::fromImageCoordToGlobalCoord(
    const CCVector2 &imageCoord, CCVector3 &globalCoord, PointCoordinateType z0,
  ...
  CCVector3 localCoord;
  if (!fromImageCoordToLocalCoord(imageCoord, localCoord, CCCoreLib::PC_ONE,
                                  withLensCorrection))
    return false;

  // update altitude: we must compute the intersection between the plane Z = Z0
  // (world) and the camera (input pixel) viewing direction
...
// TODO
// bool ccCameraSensor::fromIdealImCoordToRealImCoord(const CCVector2& ideal,
// CCVector2& real) const
//{
//	return true;
//}
...
bool ccCameraSensor::computeUncertainty(const CCVector2 &pixel,
  ...
  case BROWN_DISTORTION: {
    const BrownDistortionParameters *distParams =
        static_cast<BrownDistortionParameters *>(m_distortionParams.data());
    // TODO ==> check if the input pixel coordinate must be the real or ideal
    // projection
...
bool ccCameraSensor::computeUncertainty(
    ...
    std::vector<Vector3Tpl<ScalarType>> &accuracy /*, bool lensCorrection*/) {
  if (!points || points->size() == 0) {
    ccLog::Warning("[ccCameraSensor::computeUncertainty] Internal error: "
                   "invalid input cloud");
    return false;
  }
  ...
  if (!m_distortionParams ||
    ...
    ccLog::Warning("[ccCameraSensor::computeUncertainty] Sensor has no "
                   "associated uncertainty model! (Brown, etc.)");
    return false;
  }
...
QImage ccCameraSensor::undistort(const QImage &image) const {
  ...
  case EXTENDED_RADIAL_DISTORTION: {
    ...
    float k2 = params->k2;
    if (k1 == 0 && k2 == 0) {
      ccLog::Warning("[ccCameraSensor::undistort] Invalid radial distortion "
                     "coefficients!");
      return QImage();
    }
    float k3 = 0;
    ...
    float xScale =
        image.width() / static_cast<float>(m_intrinsicParams.arrayWidth);
    float yScale =
        image.height() / static_cast<float>(m_intrinsicParams.arrayHeight);
    ...
    // image undistortion
    {
      for (int i = 0; i < width; ++i) {
        ...
        for (int j = 0; j < height; ++j) {
          ...
          float rp =
              1.0f +
              p2 * (k1 + p2 * (k2 + p2 * k3)); // r(p) = 1.0 + k1 * ||p||^2 + k2
                                               // * ||p||^4 + k3 * ||p||^6
          float eqx = rp * x + cx;
          float eqy = rp * y + cy;
  ...
  ccLog::Warning("[ccCameraSensor::undistort] Can't undistort the image with "
                 "the current distortion model!");
...
bool ccCameraSensor::computeFrustumCorners() {
  ...
  // ORDER!!
  m_frustumInfos.frustumCorners->addPoint(
      CCVector3(xIn, yIn, -CCCoreLib::PC_ONE) * zNear);
  m_frustumInfos.frustumCorners->addPoint(
      CCVector3(xIn, yIn, -CCCoreLib::PC_ONE) * zFar);
  m_frustumInfos.frustumCorners->addPoint(
      CCVector3(xIn, -yIn, -CCCoreLib::PC_ONE) * zNear);
  m_frustumInfos.frustumCorners->addPoint(
      CCVector3(xIn, -yIn, -CCCoreLib::PC_ONE) * zFar);
...
bool ccCameraSensor::computeGlobalPlaneCoefficients(
    float planeCoefficients[6][4], CCVector3 frustumCorners[8],
    CCVector3 edges[6], CCVector3 &center) {
  if (!m_frustumInfos.isComputed) {
  ...
  // It means that if you have your plane equation in the form (ax + by + cz + d
  = 0), then --> k = sqrt(a*a + b*b + c*c) and your new coefficients are -->
...
void ccCameraSensor::drawMeOnly(CC_DRAW_CONTEXT &context) {
  ...
  ccGL::Vertex3(glFunc, -upperLeftPoint.x, upperLeftPoint.y, -upperLeftPoint.z);
  ccGL::Vertex3(glFunc, -upperLeftPoint.x, -upperLeftPoint.y,
                -upperLeftPoint.z);
  ccGL::Vertex3(glFunc, upperLeftPoint.x, -upperLeftPoint.y, -upperLeftPoint.z);
  glFunc->glEnd();
  ...
  if (m_frustumInfos.drawFrustum || m_frustumInfos.drawSidePlanes) {
    ...
    if (m_frustumInfos.frustumCorners &&
      ...
      if (m_frustumInfos.drawSidePlanes && m_frustumInfos.initFrustumHull()) {
        ...
        m_frustumInfos.frustumHull->setTempColor(
            entityPickingMode ? pickingColor : m_color);
        ...
        frustumContext.drawingFlags &=
            (~CC_ENTITY_PICKING); // we must remove the 'entity picking flag' so
                                  // that the sphere doesn't override the picking
                                  // color!
...
float ccCameraSensor::ConvertFocalPixToMM(float focal_pix,
  ...
  if (ccdPixelSize_mm < FLT_EPSILON) {
    ccLog::Warning(
        "[ccCameraSensor::convertFocalPixToMM] Invalid CCD pixel size! (<= 0)");
    return -1.0f;
  }
...
float ccCameraSensor::ConvertFocalMMToPix(float focal_mm,
  ...
  if (ccdPixelSize_mm < FLT_EPSILON) {
    ccLog::Warning(
        "[ccCameraSensor::convertFocalMMToPix] Invalid CCD pixel size! (<= 0)");
    return -1.0f;
  }
...
bool ccCameraSensor::computeOrthoRectificationParams(
    const ccImage *image, CCCoreLib::GenericIndexedCloud *keypoints3D,
    std::vector<KeyPoint> &keypointsImage, double a_out[3], double b_out[3],
  ...
  // conjugate gradient iterations
  {
    ...
    for (unsigned i = 0; i < 1500; ++i) {
      ...
      {
        ccLog::PrintDebug(
            QString("[computeOrthoRectificationParams] Convergence reached in "
                    "%1 iteration(s) (error: %2)")
                .arg(i + 1)
                .arg(lastError));
        break;
      }
...
ccImage *ccCameraSensor::orthoRectifyAsImageDirect(
    const ccImage *image, PointCoordinateType Z0, double &pixelSize,
    bool undistortImages /*=true*/, double *minCorner /*=nullptr*/,
    double *maxCorner /*=nullptr*/, double *realCorners /*=nullptr*/) const {
...
ccImage *ccCameraSensor::orthoRectifyAsImage(
    const ccImage *image, CCCoreLib::GenericIndexedCloud *keypoints3D,
    std::vector<KeyPoint> &keypointsImage, double &pixelSize,
    double *minCorner /*=nullptr*/, double *maxCorner /*=nullptr*/,
    double *realCorners /*=nullptr*/) const {
  double a[3]{0.0, 0.0, 0.0};
  double b[3]{0.0, 0.0, 0.0};
  double c[3]{0.0, 0.0, 0.0};

  if (!computeOrthoRectificationParams(image, keypoints3D, keypointsImage, a, b,
                                       c)) {
    return nullptr;
  }
  ...
  for (unsigned i = 0; i < w; ++i) {
    ...
    for (unsigned j = 0; j < h; ++j) {
      ...
      double q =
          (c2 * xip - a2) * (c1 * yip - b1) - (c2 * yip - b2) * (c1 * xip - a1);
...
bool ccCameraSensor::OrthoRectifyAsImages(
    std::vector<ccImage *> images, double a[], double b[], double c[],
    unsigned maxSize, QDir *outputDir /*=nullptr*/,
  ...
  if (outputDir) {
    ...
    if (f.open(QIODevice::WriteOnly | QIODevice::Text)) {
      ...
      stream << "Global2DBBox" << ' ' << 0 << ' ' << 0 << ' ' << globalWidth - 1
             << ' ' << globalHeight - 1 << endl;
    }
  }
    ...
    if (outputDir) {
                 ...
                 QIODevice::Text)) // always append
      {
        ...
        stream << "Local3DBBox" << ' ' << minC[0] << ' ' << minC[1] << ' '
               << maxC[0] << ' ' << maxC[1] << ' ';
        stream << "Local2DBBox" << ' ' << xShiftGlobal << ' ' << yShiftGlobal
               << ' ' << xShiftGlobal + static_cast<double>(w - 1) << ' '
               << yShiftGlobal + static_cast<double>(h - 1) << endl;
        f.close();
      }
    }
...
ccPointCloud *ccCameraSensor::orthoRectifyAsCloud(
  ...
  // ortho rectification
  {
    for (unsigned pi = 0; pi < width; ++pi) {
      ...
      for (unsigned pj = 0; pj < height; ++pj) {
        ...
        CCVector3 P(
            static_cast<PointCoordinateType>((a0 + a1 * xi + a2 * yi) / qi),
        ...
        if (r + g + b > 0) {
          ...
          int a = qAlpha(rgb);
          ccColor::Rgba color(
              static_cast<ColorCompType>(r), static_cast<ColorCompType>(g),
              static_cast<ColorCompType>(b), static_cast<ColorCompType>(a));
          proj->addColor(color);
          ++realCount;
        }
      }
  ...
  if (realCount == 0) {
    ccLog::Warning(
        QString(
            "[orthoRectifyAsCloud] Image '%1' was black, nothing to project!")
            .arg(image->getName()));
    delete proj;
    proj = nullptr;
  } else {
...
bool ccOctreeFrustumIntersector::build(CCCoreLib::DgmOctree *octree) {
  ...
  const CCCoreLib::DgmOctree::cellsContainer &thePointsAndTheirCellCodes =
      octree->pointsAndTheirCellCodes();
  CCCoreLib::DgmOctree::cellsContainer::const_iterator it =
  ...
  try {
    for (it = thePointsAndTheirCellCodes.begin();
      ...
      for (unsigned char level = 1;
           level <= CCCoreLib::DgmOctree::MAX_OCTREE_LEVEL; level++) {
...
//
// unsigned char boxIntersectPlane(const CCVector3& minCorner, const CCVector3&
// maxCorner, const float planeCoefficient[4])
//{
//	CCVector3 n(planeCoefficient[0], planeCoefficient[1],
//planeCoefficient[2]); 	float d = planeCoefficient[3];
//
...
//}
//
// unsigned char boxIntersectFrustum(const CCVector3& minCorner, const
// CCVector3& maxCorner, const float planesCoefficients[6][4])
//{
...
//
//		int result = boxIntersectPlane(minCorner, maxCorner,
//onePlaneCoefficients);
//
//		//pay attention to the signification of OUTSIDE and INSIDE there :
//INSIDE means that the box is in the positive half space delimited by the
//plane, OUTSIDE means that the box is in the negative half space !! 		if (result
//== CELL_OUTSIDE_FRUSTUM) 			return CELL_OUTSIDE_FRUSTUM; 		if (result ==
//CELL_INTERSECT_FRUSTUM) 			intersecting = true;
//	}
//
...
ccOctreeFrustumIntersector::separatingAxisTest(
    const CCVector3 &bbMin, const CCVector3 &bbMax,
    const float planesCoefficients[6][4], const CCVector3 frustumCorners[8],
    const CCVector3 frustumEdges[6], const CCVector3 &frustumCenter) {
  ...
  CCVector3 boxCorners[8];
  {
    for (unsigned i = 0; i < 8; i++)
      boxCorners[i] =
          CCVector3((i & 4) ? bbMin.x : bbMax.x, (i & 2) ? bbMin.y : bbMax.y,
                    (i & 1) ? bbMin.z : bbMax.z);
  }
...
void ccOctreeFrustumIntersector::computeFrustumIntersectionByLevel(
    unsigned char level, CCCoreLib::DgmOctree::CellCode parentTruncatedCode,
    OctreeCellVisibility parentResult, const float planesCoefficients[6][4],
    const CCVector3 ptsFrustum[8], const CCVector3 edges[6],
  ...
  for (unsigned i = 0; i < 8; i++) {
    ...
    // if the current cell has not been built (contains no 3D points), we skip
    // it
    std::unordered_set<CCCoreLib::DgmOctree::CellCode>::const_iterator got =
        m_cellsBuilt[level].find(truncatedCode);
    if (got != m_cellsBuilt[level].end()) {
      // get extrema of the current cell
      CCVector3 bbMin;
      CCVector3 bbMax;
      m_associatedOctree->computeCellLimits(truncatedCode, level, bbMin, bbMax,
                                            true);
      ...
      OctreeCellVisibility result =
          (parentResult == CELL_INSIDE_FRUSTUM
               ? CELL_INSIDE_FRUSTUM
               : separatingAxisTest(bbMin, bbMax, planesCoefficients,
                                    ptsFrustum, edges, center));

      // if the cell is not outside the frustum, there is a kind of intersection
      // (inside or intersecting)
      if (result != CELL_OUTSIDE_FRUSTUM) {
        ...
        // we do the same for the children (if we have not already reached the
        // end of the tree)
        if (level < CCCoreLib::DgmOctree::MAX_OCTREE_LEVEL)
          computeFrustumIntersectionByLevel(level + 1, truncatedCode, result,
                                            planesCoefficients, ptsFrustum,
                                            edges, center);
      }
    }
  }
...
void ccOctreeFrustumIntersector::computeFrustumIntersectionWithOctree(
    ...
    const float planesCoefficients[6][4], const CCVector3 ptsFrustum[8],
    const CCVector3 edges[6], const CCVector3 &center) {
  // clear old result
  {
    for (int i = 0; i <= CCCoreLib::DgmOctree::MAX_OCTREE_LEVEL; i++) {
  ...
  // get points
  unsigned char level =
      static_cast<unsigned char>(CCCoreLib::DgmOctree::MAX_OCTREE_LEVEL);
  ...
  for (it = m_cellsInFrustum[level].begin();
    ...
    if (m_associatedOctree->getPointsInCell(*it, level, &pointsInCell, true)) {
      // all points are inside the frustum since the cell itself is completely
      // inside
      for (size_t i = 0; i < pointsInCell.size(); i++)
        inCameraFrustum.push_back(
            pointsInCell.getPointGlobalIndex(static_cast<unsigned>(i)));
    }
  }

...
ccPointCloud *
ccHObjectCaster::ToPointCloud(ccHObject *obj,
                              bool *lockedVertices /*= nullptr*/) {
  if (lockedVertices) {
    ...
    if (obj->isA(CC_TYPES::POINT_CLOUD)) {
      ...
      if (vertices) {
        if (!obj->isA(CC_TYPES::MESH) &&
            lockedVertices) // no need to 'lock' the vertices if the user works
                            // on the parent mesh
        {
...
ccGenericPointCloud *
ccHObjectCaster::ToGenericPointCloud(ccHObject *obj,
                                     bool *lockedVertices /*=nullptr*/) {
  if (lockedVertices) {
    *lockedVertices = false;
  }
...
ccGenericMesh *ccHObjectCaster::ToGenericMesh(ccHObject *obj) {
  return (obj && obj->isKindOf(CC_TYPES::MESH)
...
ccMesh *ccHObjectCaster::ToMesh(ccHObject *obj) {
  return (
      obj && (obj->isA(CC_TYPES::MESH) || obj->isKindOf(CC_TYPES::PRIMITIVE))
          ? static_cast<ccMesh *>(obj)
          : nullptr);
}

ccSubMesh *ccHObjectCaster::ToSubMesh(ccHObject *obj) {
  return (obj && obj->isA(CC_TYPES::SUB_MESH) ? static_cast<ccSubMesh *>(obj)
                                              : nullptr);
}

ccPolyline *ccHObjectCaster::ToPolyline(ccHObject *obj) {
  return (obj && obj->isA(CC_TYPES::POLY_LINE) ? static_cast<ccPolyline *>(obj)
                                               : nullptr);
}

ccFacet *ccHObjectCaster::ToFacet(ccHObject *obj) {
  return obj && obj->isA(CC_TYPES::FACET) ? static_cast<ccFacet *>(obj)
                                          : nullptr;
}
...
ccGenericPrimitive *ccHObjectCaster::ToPrimitive(ccHObject *obj) {
  return obj && obj->isKindOf(CC_TYPES::PRIMITIVE)
             ? static_cast<ccGenericPrimitive *>(obj)
             : nullptr;
}

ccSphere *ccHObjectCaster::ToSphere(ccHObject *obj) {
  return obj && obj->isA(CC_TYPES::SPHERE) ? static_cast<ccSphere *>(obj)
                                           : nullptr;
}

ccCylinder *ccHObjectCaster::ToCylinder(ccHObject *obj) {
  return obj && obj->isA(CC_TYPES::CYLINDER) ? static_cast<ccCylinder *>(obj)
                                             : nullptr;
}

ccCone *ccHObjectCaster::ToCone(ccHObject *obj) {
  return obj && obj->isKindOf(CC_TYPES::CONE) ? static_cast<ccCone *>(obj)
                                              : nullptr;
}

ccPlane *ccHObjectCaster::ToPlane(ccHObject *obj) {
  return obj && obj->isA(CC_TYPES::PLANE) ? static_cast<ccPlane *>(obj)
                                          : nullptr;
}
...
ccExtru *ccHObjectCaster::ToExtru(ccHObject *obj) {
  return obj && obj->isA(CC_TYPES::EXTRU) ? static_cast<ccExtru *>(obj)
                                          : nullptr;
}

ccTorus *ccHObjectCaster::ToTorus(ccHObject *obj) {
  return obj && obj->isA(CC_TYPES::TORUS) ? static_cast<ccTorus *>(obj)
                                          : nullptr;
}

ccOctreeProxy *ccHObjectCaster::ToOctreeProxy(ccHObject *obj) {
  return obj && obj->isA(CC_TYPES::POINT_OCTREE)
...
bool ccHObjectCaster::CloneChildren(
  ...
  if (newPointOrTriangleIndex) {
    if (sourceEntity == destEntity) {
      ccLog::Warning("[ccHObjectCaster::CloneChildren] Providing a "
    ...
    if (!sourceAndDestAreCloud && !sourceAndDestAreMeshes) {
      ccLog::Warning(
          "[ccHObjectCaster::CloneChildren] A point/triangle correspondance "
          "map can only work between 2 entities of the same type");
      return false;
    }
    ...
    if (newPointOrTriangleIndex->size() != numberOfPointOrTriangle) {
      ccLog::Warning(
          ...
          "point/triangle correspondance map and the source entity size");
      return false;
    }
  }
  ...
  for (unsigned i = 0; i < currentSourceEntity->getChildrenNumber(); ++i) {
      ...
      if (newPointOrTriangleIndex) {
        for (unsigned i = 0; i < label->size(); ++i) {
          ...
          if (pp.entity() == sourceEntity &&
            ...
            // this label relies on a point or triangle that has no
            // correspondance in the destination entity
            keepThisLabel = false;
            break;
          }
        }
      ...
      if (keepThisLabel) {
        ...
        for (unsigned i = 0; i < label->size(); ++i) {
          ...
          if (pp.entity() == sourceEntity) {
            ...
            if (newPointOrTriangleIndex) {
              pp.index = static_cast<unsigned>(newPointOrTriangleIndex->at(
                  pp.index)); // we've checked above that it's >= 0
            }
          }
          clonedLabel->addPickedPoint(pp);
        }
        clonedLabel->setName(label->getName()); // the label name is overridden
    ...
    case CC_TYPES::HIERARCHY_OBJECT: {
      ...
      if (CloneChildren(sourceEntity, destEntity, newPointOrTriangleIndex,
                        child, newGroup)) {

...
ccTorus::ccTorus(QString name /*=QString("Torus")*/)
    : ccGenericPrimitive(name), m_insideRadius(0), m_outsideRadius(0),
      m_rectSection(false), m_rectSectionHeight(0), m_angle_rad(0) {}

ccGenericPrimitive *ccTorus::clone() const {
  return finishCloneJob(new ccTorus(
      m_insideRadius, m_outsideRadius, m_angle_rad, m_rectSection,
      m_rectSectionHeight, &m_transformation, getName(), m_drawPrecision));
}
...
bool ccTorus::buildUp() {
  ...
  unsigned sweepSteps =
      ...
      (closed
           ? steps
           : static_cast<unsigned>(ceil((m_angle_rad * steps) / (2.0 * M_PI))));
  ...
  if (m_rectSection) {
    ...
    for (unsigned i = 0; i < sectSteps; ++i) {
      ...
      sectPoints[i].x =
          static_cast<PointCoordinateType>(cos(sect_angle_rad) * sectionRadius);
      sectPoints[i].z =
          static_cast<PointCoordinateType>(sin(sect_angle_rad) * sectionRadius);
    }
  }
  ...
  for (unsigned t = 0; t < (closed ? sweepSteps : sweepSteps + 1); ++t) {
    ...
    for (unsigned i = 0; i < sectSteps; ++i) {
      CCVector3 P(sweepU.x * (sweepRadius + sectPoints[i].x),
                  sweepU.y * (sweepRadius + sectPoints[i].x), sectPoints[i].z);
      verts->addPoint(P);
    }
  ...
  if (!closed && !m_rectSection) {
    ...
    verts->addPoint(P);
    CCVector3 P2(
        static_cast<PointCoordinateType>(cos(m_angle_rad)) * sweepRadius,
        static_cast<PointCoordinateType>(sin(m_angle_rad)) * sweepRadius, 0);
    verts->addPoint(P2);
  }
  ...
  // mesh faces
  {
    ...
    for (unsigned t = 0; t < sweepSteps; ++t) {
      ...
      for (unsigned i = 0; i < sectSteps; ++i) {
        ...
        addTriangle(sweepStart + i, (sweepStart + i + sectSteps) % vertCount,
                    (sweepStart + iNext + sectSteps) % vertCount);
        if (m_rectSection)
          addTriangleNormalIndexes(
              sweepStart + i, (sweepStart + i + sectSteps) % faceNormCount,
        ...
        addTriangle(sweepStart + i,
                    ...
                    sweepStart + iNext);
        if (m_rectSection)
          addTriangleNormalIndexes(sweepStart + i,
                                   (sweepStart + i + sectSteps) % faceNormCount,
                                   sweepStart + iNext);
        else
          addTriangleNormalIndexes(
              sweepStart + i, (sweepStart + iNext + sectSteps) % faceNormCount,
              sweepStart + iNext);
      }
    }
    ...
    if (!closed) {
      ...
      if (m_rectSection) {
        ...
        for (unsigned i = 0; i < sectSteps; ++i) {
          unsigned iNext = (i + 1) % sectSteps;
          addTriangle(lastSectionCenterShift + 1, lastSectionShift + iNext,
                      lastSectionShift + i);
          addTriangleNormalIndexes(faceNormCount + 1, faceNormCount + 1,
                                   faceNormCount + 1);
        }
...
bool ccTorus::fromFile_MeOnly(QFile &in, short dataVersion, int flags,
  ...
  ccSerializationHelper::CoordsFromDataStream(inStream, flags, &m_insideRadius,
                                              1);
  ccSerializationHelper::CoordsFromDataStream(inStream, flags, &m_outsideRadius,
                                              1);

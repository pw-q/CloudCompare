...
ccFacet::ccFacet(PointCoordinateType maxEdgeLength /*=0*/,
    ...
    : ccHObject(name), m_polygonMesh(nullptr), m_contourPolyline(nullptr),
      m_contourVertices(nullptr), m_originPoints(nullptr), m_center(0, 0, 0),
      m_rms(0.0), m_surface(0.0), m_maxEdgeLength(maxEdgeLength) {
  m_planeEquation[0] = 0;
  m_planeEquation[1] = 0;
  m_planeEquation[2] = 1;
  m_planeEquation[3] = 0;
...
ccFacet *ccFacet::clone() const {
  ...
  if (m_contourPolyline) {
    ...
    if (!facet->m_contourPolyline || !facet->m_contourVertices) {
      // not enough memory?!
      ccLog::Warning(QString("[ccFacet::clone][%1] Failed to clone contour!")
                         .arg(getName()));
      delete facet;
      return nullptr;
    }

    // the copy constructor of ccPolyline creates a new cloud (the copy of this
    // facet's 'contour points') but set it by default as a child of the polyline
    // (while we want the opposite in a facet)
  ...
  if (m_polygonMesh) {
    ...
    if (!facet->m_polygonMesh) {
      // not enough memory?!
      ccLog::Warning(QString("[ccFacet::clone][%1] Failed to clone polygon!")
                         .arg(getName()));
      delete facet;
      return nullptr;
    }
  ...
  if (m_originPoints) {
    ...
    if (!facet->m_originPoints) {
      ccLog::Warning(
          QString("[ccFacet::clone][%1] Failed to clone origin points!")
              .arg(getName()));
      // delete facet;
      // return 0;
    } else {
...
ccFacet *
ccFacet::Create(CCCoreLib::GenericIndexedCloudPersist *cloud,
  ...
  if (!cloud || cloud->size() < 3) {
    ccLog::Error(
        "[ccFacet::Create] Need at least 3 points to create a valid facet!");
    return nullptr;
  }
...
bool ccFacet::createInternalRepresentation(
    CCCoreLib::GenericIndexedCloudPersist *points,
    const PointCoordinateType *planeEquation /*=nullptr*/) {
  assert(points);
  if (!points)
    return false;
  ...
  const ccGenericPointCloud *pointsAsGPC =
      dynamic_cast<const ccGenericPointCloud *>(points);
  ...
  if (!planeEquation) {
    ...
    if (!planeEquation) {
      ccLog::Warning("[ccFacet::createInternalRepresentation] Failed to "
                     "compute the LS plane passing through the input points!");
      return false;
    }
  }
  ...
  if (!Yk.projectPointsOn2DPlane<
          CCCoreLib::PointProjectionTools::IndexedCCVector2>(
  ...
  m_rms = CCCoreLib::DistanceComputationTools::computeCloud2PlaneDistanceRMS(
      points, m_planeEquation);
  ...
  // the polygon
  {
    ...
    if (!CCCoreLib::PointProjectionTools::extractConcaveHull2D(
            points2D, hullPoints, m_maxEdgeLength * m_maxEdgeLength)) {
      ccLog::Error("[ccFacet::createInternalRepresentation] Failed to compute "
    ...
    m_contourVertices = new ccPointCloud();
    {
      ...
      for (std::list<CCCoreLib::PointProjectionTools::IndexedCCVector2
                         *>::const_iterator it = hullPoints.begin();
    ...
    // we create the corresponding (3D) polyline
    {
      ...
      if (m_contourPolyline->reserve(hullPtsCount)) {
        ...
        ccLog::Warning("[ccFacet::createInternalRepresentation] Not enough "
    ...
    try {
      ...
      ccLog::Warning("[ccFacet::createInternalRepresentation] Not enough "
                     "memory to create the contour mesh!");
    }
    ...
    if (!hullPointsVector.empty() && CCCoreLib::Delaunay2dMesh::Available()) {
      ...
      if (dm.buildMesh(hullPointsVector,
                       CCCoreLib::Delaunay2dMesh::USE_ALL_POINTS, errorStr)) {
          ...
          if (m_polygonMesh->reservePerTriangleNormalIndexes()) {
            ...
            for (unsigned i = 0; i < triCount; ++i)
              m_polygonMesh->addTriangleNormalIndexes(
                  0, 0, 0); // all triangles will have the same normal!
            ...
            ccLog::Warning("[ccFacet::createInternalRepresentation] Not enough "
                           "memory to create the polygon mesh's normals!");
          }

          // update facet surface
          m_surface =
              CCCoreLib::MeshSamplingTools::computeMeshArea(m_polygonMesh);
        } else {
          delete m_polygonMesh;
          m_polygonMesh = nullptr;
          ccLog::Warning("[ccFacet::createInternalRepresentation] Not enough "
                         "memory to create the polygon mesh!");
        }
      } else {
        ccLog::Warning(
            QStringLiteral(
                "[ccFacet::createInternalRepresentation] Failed to create the "
                "polygon mesh (third party lib. said '%1'")
...
void ccFacet::drawMeOnly(CC_DRAW_CONTEXT &context) {
  ...
  if (normalVectorIsShown() && m_contourPolyline) {
    ...
    if (m_surface > 0) // the surface might be 0 if Delaunay 2.5D triangulation
                       // is not supported
    {
...
bool ccFacet::toFile_MeOnly(QFile &out, short dataVersion) const {
  ...
  // we can't save the origin points here (as it will be automatically saved as
  ...
  // cloud must be saved in the same BIN file! (responsibility of the caller)
  {
    uint32_t originPointsUniqueID =
        (m_originPoints ? static_cast<uint32_t>(m_originPoints->getUniqueID())
                        : 0);
    if (out.write((const char *)&originPointsUniqueID, 4) < 0)
      return WriteError();
  }

  // we can't save the contour points here (as it will be automatically saved as
  // a child) so instead we save it's unique ID (dataVersion>=32) WARNING: the
  // cloud must be saved in the same BIN file! (responsibility of the caller)
  {
    uint32_t contourPointsUniqueID =
        (m_contourVertices
             ? static_cast<uint32_t>(m_contourVertices->getUniqueID())
             : 0);
    if (out.write((const char *)&contourPointsUniqueID, 4) < 0)
      return WriteError();
  }

  // we can't save the contour polyline here (as it will be automatically saved
  // as a child) so instead we save it's unique ID (dataVersion>=32) WARNING: the
  // polyline must be saved in the same BIN file! (responsibility of the caller)
  {
    uint32_t contourPolyUniqueID =
        (m_contourPolyline
             ? static_cast<uint32_t>(m_contourPolyline->getUniqueID())
             : 0);
    if (out.write((const char *)&contourPolyUniqueID, 4) < 0)
      return WriteError();
  }

  // we can't save the polygon mesh here (as it will be automatically saved as a
  // child) so instead we save it's unique ID (dataVersion>=32) WARNING: the mesh
  // must be saved in the same BIN file! (responsibility of the caller)
  {
    uint32_t polygonMeshUniqueID =
        (m_polygonMesh ? static_cast<uint32_t>(m_polygonMesh->getUniqueID())
                       : 0);

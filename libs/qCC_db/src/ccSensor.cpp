...
ccSensor::ccSensor(const QString &name)
    : ccHObject(name), m_posBuffer(nullptr), m_activeIndex(0),
      m_color(ccColor::green), m_scale(CCCoreLib::PC_ONE) {
  m_rigidTransformation.toIdentity();
}

ccSensor::ccSensor(const ccSensor &sensor)
    : ccHObject(sensor), m_posBuffer(nullptr),
      m_rigidTransformation(sensor.m_rigidTransformation),
      m_activeIndex(sensor.m_activeIndex), m_color(sensor.m_color),
...
bool ccSensor::getAbsoluteTransformation(ccIndexedTransformation &trans,
  ...
  trans.toIdentity();
  if (m_posBuffer &&
      !m_posBuffer->getInterpolatedTransformation(index, trans)) {
    return false;
  }
...
bool ccSensor::getActiveAbsoluteTransformation(
  ...
  if (!getAbsoluteTransformation(trans, m_activeIndex)) {
    ccLog::Warning("[ccSensor::getActiveAbsoluteTransformation] Failed to get "
                   "a valid transformation for active index!");
    return false;
  }
...
bool ccSensor::toFile_MeOnly(QFile &out, short dataVersion) const {
  ...
  // WARNING: the buffer must be saved in the same BIN file! (responsibility of
  // the caller)
  uint32_t bufferUniqueID =
      (m_posBuffer ? static_cast<uint32_t>(m_posBuffer->getUniqueID()) : 0);

...
struct SFPair {
  SFPair(const CCCoreLib::ScalarField *sfIn = nullptr,
         CCCoreLib::ScalarField *sfOut = nullptr)
      : in(sfIn), out(sfOut) {}
...
bool cellSFInterpolator(const CCCoreLib::DgmOctree::octreeCell &cell,
  ...
  const CCCoreLib::DgmOctree *srcOctree =
      reinterpret_cast<CCCoreLib::DgmOctree *>(additionalParameters[1]);
      ...
      if (params->algo == ccPointCloudInterpolator::Parameters::MEDIAN) {
        ...
        for (unsigned j = 0; j < sfCount; ++j) {
          ...
          for (unsigned k = 0; k < neighborCount; ++k) {
            CCCoreLib::DgmOctree::PointDescriptor &P =
...
bool ccPointCloudInterpolator::InterpolateScalarFieldsFrom(
    ccPointCloud *destCloud, ccPointCloud *srcCloud,
    const std::vector<int> &inSFIndexes, const Parameters &params,
  ...
  if (std::abs(dist.x) > dimSum.x / 2 || std::abs(dist.y) > dimSum.y / 2 ||
    ...
    ccLog::Warning("[InterpolateScalarFieldsFrom] Clouds are too far from each "
                   "other! Can't proceed.");
    return false;
  }
  ...
  for (size_t i = 0; i < inSFIndexes.size(); ++i) {
    ...
    if (inSFIndex < 0 ||
        inSFIndex >= static_cast<int>(srcCloud->getNumberOfScalarFields())) {
      // invalid index
      ccLog::Warning(QString("[InterpolateScalarFieldsFrom] Source cloud has "
                             "no scalar field with index #%1")
                         .arg(inSFIndex));
      assert(false);
      return false;
    }
  ...
  if (params.method == Parameters::NEAREST_NEIGHBOR) {
    ...
    if (soCode != CCCoreLib::DistanceComputationTools::SYNCHRONIZED) {
      ...
      ccLog::Warning(
          "[InterpolateScalarFieldsFrom] Failed to build the octrees");
      return false;
    }
    ...
    if (octreeLevel == 0) {
      if (params.method ==
          ccPointCloudInterpolator::Parameters::K_NEAREST_NEIGHBORS) {
        octreeLevel =
            srcOctree->findBestLevelForAGivenPopulationPerCell(params.knn);
      } else {
        octreeLevel =
            srcOctree->findBestLevelForAGivenNeighbourhoodSizeExtraction(
                params.radius);
      }
    }
    ...
    try {
      ...
      if (destOctree->executeFunctionForAllCellsAtLevel(
              octreeLevel, cellSFInterpolator, additionalParameters, true,
              progressCb, "Scalar field interpolation", 0) == 0) {
        // something went wrong
        ccLog::Warning("[InterpolateScalarFieldsFrom] Failed to perform the "
                       "interpolation");
        return false;
      }
    } catch (const std::bad_alloc &) {
  ...
  if (overwrite) {
    ccLog::Warning("[InterpolateScalarFieldsFrom] Some scalar fields with the "

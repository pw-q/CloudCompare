...
bool ccGriddedTools::DetectParameters(
    const ccPointCloud *cloud, const ccPointCloud::Grid::Shared grid,
    GridParameters &parameters, bool verbose /*=false*/,
  ...
  // pi) we'll compute all parameters for both cases, and choose the best one at
  // the end!
  ...
  try {
    // determine the PITCH angular step
    {
      ...
      // for each ROW we determine the min and max valid grid point (i.e. index
      // >= 0)
      ...
      for (unsigned j = 0; j < grid->h; ++j) {
        ...
        if (maxIndex > minIndex) {
          ...
          for (unsigned k = minIndex; k <= maxIndex; ++k) {
            ...
            if (index >= 0) {
              ...
              PointCoordinateType p =
                  atan2(P.z, sqrt(P.x * P.x +
                                  P.y * P.y)); // see ccGBLSensor::projectPoint
              PointCoordinateType pShifted =
                  (p < 0 ? p + static_cast<PointCoordinateType>(2.0 * M_PI)
                         : p);
              if (k != minIndex) {
          ...
          ScalarType angleShifted_rad = static_cast<ScalarType>(
              (maxPhiCurrentLineShifted - minPhiCurrentLineShifted) / span);
      ...
      if (!angles.empty()) {
        ...
        parameters.deltaPhiRad =
            static_cast<PointCoordinateType>(angles[maxSpanIndex].first);
        if (verbose) {
          ccLog::Print(
              QStringLiteral("[Scan grid] Detected pitch step: %1 degrees "
                             "(span [%2 - %3])")
        ...
        ccLog::Warning("[Scan grid] Not enough valid points to compute the "
    ...
    // now determine the YAW angular step
    {
      ...
      // for each COLUMN we determine the min and max valid grid point (i.e.
      // index >= 0)
      for (unsigned i = 0; i < grid->w; ++i) {
        ...
        if (maxIndex > minIndex) {
          ...
          for (unsigned k = minIndex; k <= maxIndex; ++k) {
            ...
            if (index >= 0) {
              ...
              PointCoordinateType t =
                  atan2(P.y, P.x); // see ccGBLSensor::projectPoint
              PointCoordinateType tShifted =
                  (t < 0 ? t + static_cast<PointCoordinateType>(2.0 * M_PI)
                         : t);
              if (k != minIndex) {
                ...
                minThetaCurrentColShifted = maxThetaCurrentColShifted =
                    tShifted;
              }
            }
          }
          ...
          ScalarType angleShifted_rad = static_cast<ScalarType>(
              (maxThetaCurrentColShifted - minThetaCurrentColShifted) / span);
          anglesShifted.emplace_back(angleShifted_rad, span);
        }
      }
      ...
      if (!angles.empty()) {
        ...
        if (verbose) {
          ccLog::Print(
              QStringLiteral(
                  "[Scan grid] Detected yaw step: %1 degrees (span [%2 - %3])")
    ...
    ccLog::Warning(
        "[Scan grid] Not enough memory to compute the scan angular steps!");
    return false;
  }
...
ccGBLSensor *
ccGriddedTools::ComputeBestSensor(ccPointCloud *cloud,

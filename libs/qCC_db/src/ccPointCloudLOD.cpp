...
public:
  ...
  ccPointCloudLODThread(ccPointCloud &cloud, ccPointCloudLOD &lod,
      ...
      : QThread(), m_cloud(cloud), m_lod(lod), m_octree(nullptr),
        m_maxCountPerCell(maxCountPerCell), m_maxLevel(0), m_earlyStop(0) {}

  //! Destructor
  ~ccPointCloudLODThread() override {
    if (isRunning()) {
      ccLog::Warning("[ccPointCloudLODThread] Destructor called when the "
                     "thread is still running: will have to terminate it...");
      terminate();
    }
  }
  ...
  void stop() {
    ...
    m_earlyStop = 1;
    if (!wait(5000)) {
      ccLog::Warning("[ccPointCloudLODThread] Failed to stop the thread "
                     "properly, will have to terminate it...");
      terminate();
    }
...
protected:
  ...
  uint8_t fillNode_flat(ccPointCloudLOD::Node &node) const {
    ...
    const unsigned char bitDec =
        CCCoreLib::DgmOctree::GET_BIT_SHIFT(node.level);
    const CCCoreLib::DgmOctree::CellCode currentTruncatedCellCode =
    ...
    // first count the number of points and compute their center
    {
      ...
      for (uint32_t codeIndex = node.firstCodeIndex;
           ...
           (cellCodes[codeIndex].theCode >> bitDec) == currentTruncatedCellCode;
           ++codeIndex) {
        ++node.pointCount;
      ...
      if (node.pointCount > 1) {
        ...
        for (uint32_t i = 0; i < node.pointCount; ++i) {
          const CCVector3 *P =
              m_cloud.getPoint(cellCodes[node.firstCodeIndex + i].theIndex);
  ...
  void run() override {
    ...
    if (m_earlyStop != 0) {
      ccLog::Error("[LoD] Thread not properly terminated previously... can't "
                   "run it again");
      return;
    }
    ...
    ccLog::Print(QString("[LoD] Preparing LoD acceleration structure for cloud "
                         "'%1' [%2 points]...")
                     .arg(m_cloud.getName())
                     .arg(pointCount));
    ...
    if (!m_octree) {
      ...
      if (m_octree->build(nullptr) <= 0) {
        if (0 == m_earlyStop) {
          // not enough memory
          ccLog::Warning(QString("[LoD] Failed to compute octree on cloud '%1' "
                                 "(not enough memory)")
                             .arg(m_cloud.getName()));
          m_lod.setState(ccPointCloudLOD::BROKEN);
        } else {
          // we have cancelled the octree computation process (see the stop()
          // method)
        }
        m_earlyStop = 1;
      }
      ...
      if (!m_cloud.getOctree()) // make sure that it hasn't been built in the
                                // meantime!
      {
    ...
    if (!m_lod.initInternal(m_octree)) {
      // not enough memory
      ccLog::Warning(QString("[LoD] Failed to compute LOD structure on cloud "
                             "'%1' (not enough memory)")
                         .arg(m_cloud.getName()));
      m_earlyStop = 1;
    }
    ...
    m_maxLevel =
        static_cast<uint8_t>(std::max<size_t>(1, m_lod.m_levels.size())) - 1;
    ...
    for (uint8_t currentLevel = 0; currentLevel < m_maxLevel; ++currentLevel) {
      ...
      ccLog::Print(QString("[LoD] Level %1: %2 cells")
                       .arg(currentLevel)
                       .arg(level.data.size()));
    ...
    if (true) {
      ...
      for (uint8_t currentLevel = 0; currentLevel < biggestLevel;
        ...
        ccLog::Print(QString("[LoD][pass 2] Level %1: %2 cells (+%3)")
                         .arg(currentLevel + 1)
                         .arg(cellCountAfter)
    ...
    ccLog::Print(
        QString("[LoD] Acceleration structure ready for cloud '%1' (max level: "
                "%2 / mem. = %3 Mb / duration: %4 s.)")
            .arg(m_cloud.getName())
            .arg(m_maxLevel)
...
ccPointCloudLOD::ccPointCloudLOD()
    : m_indexMap(0), m_lastIndexMap(0), m_octree(nullptr), m_thread(nullptr),
...
public:
  PointCloudLODVisibilityFlagger(ccPointCloudLOD &lod, const Frustum &frustum,
                                 unsigned char maxLevel)
      : m_lod(lod), m_frustum(frustum), m_maxLevel(maxLevel),
  ...
  uint32_t flag(ccPointCloudLOD::Node &node) {
    ...
    if (m_hasClipPlanes && node.intersection != Frustum::OUTSIDE) {
      for (const ccClipPlane &clipPlane : m_clipPlanes) {
        ...
        double dist = clipPlane.equation.x * node.center.x +
                      ...
                      clipPlane.equation
                          .w /* / CCVector3d::vnorm(clipPlane.equation.u) */;
    ...
    case Frustum::INSIDE:
      ...
      // no need to propagate the visibility to the children as the default
      // value should already be 'INSIDE'
      break;
    ...
    case Frustum::INTERSECT:
      // we have to test the children
      {
        if (node.level < m_maxLevel && node.childCount) {
          ...
          if (visibleCount == 0) {
            // as no point is visible we can flag this node as being
            // outside/invisible
            node.intersection = Frustum::OUTSIDE;
          }
        } else {
...
uint32_t
ccPointCloudLOD::flagVisibility(const Frustum &frustum,
    ...
    for (int i = 0; i < 8; ++i) {
      if (node.childIndexes[i] >= 0) {
        ...
        if (displayAll) {
          ...
          double ratio = static_cast<double>(childNodeRemainingCount) /
...
LODIndexSet &
ccPointCloudLOD::getIndexMap(unsigned char level, unsigned &maxCount,
  ...
  if (totalRemainingCount != 0 && thisPassDisplayCount < maxCount) {
    ...
    for (size_t i = 0; i < l.data.size(); ++i) {
      ...
      if (displayAll) {
        ...
        double ratio =
            static_cast<double>(nodeRemainingCount) / totalRemainingCount;

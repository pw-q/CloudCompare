...
ccPlane::ccPlane(PointCoordinateType xWidth, PointCoordinateType yWidth,
                 const ccGLMatrix *transMat /*=nullptr*/,
                 QString name /*=QString("Plane")*/)
    : ccGenericPrimitive(name, transMat), m_xWidth(xWidth), m_yWidth(yWidth) {
...
void ccPlane::drawMeOnly(CC_DRAW_CONTEXT &context) {
  ...
  if (MACRO_Draw3D(context) && normalVectorIsShown()) {
    PointCoordinateType scale = static_cast<PointCoordinateType>(
        sqrt(static_cast<double>(m_xWidth) * m_yWidth) /
        2); // DGM: highly empirical ;)
...
ccPlane *ccPlane::Fit(CCCoreLib::GenericIndexedCloudPersist *cloud,
                      double *rms /*=nullptr*/) {
  // number of points
  unsigned count = cloud->size();
  if (count < 3) {
    ccLog::Warning(
        "[ccPlane::Fit] Not enough points in input cloud to fit a plane!");
    return nullptr;
  }
  ...
  if (rms) {
    *rms = CCCoreLib::DistanceComputationTools::computeCloud2PlaneDistanceRMS(
        cloud, theLSPlane);
...
ccBBox ccPlane::getOwnFitBB(ccGLMatrix &trans) {
  ...
  return ccBBox(CCVector3(-m_xWidth / 2, -m_yWidth / 2, 0),
                CCVector3(m_xWidth / 2, m_yWidth / 2, 0), true);
}

ccMaterial::Shared ccPlane::setAsTexture(QImage image,
                                         QString imageFilename /*=QString()*/) {
  return SetQuadTexture(this, image, imageFilename);
}

ccMaterial::Shared
ccPlane::SetQuadTexture(ccMesh *quadMesh, QImage image,
                        QString imageFilename /*=QString()*/) {

...
CompressedNormType
ccNormalVectors::GetNormIndex(const PointCoordinateType N[]) {
...
bool ccNormalVectors::UpdateNormalOrientations(
    ccGenericPointCloud *theCloud, NormsIndexesTableType &theNormsCodes,
  ...
  case MINUS_Z: {
    ...
    prefOrientation.u[preferredOrientation >> 1] =
        ((preferredOrientation & 1) == 0
             ? CCCoreLib::PC_ONE
             : -CCCoreLib::PC_ONE); // odd number --> inverse direction
  } break;

  case PLUS_BARYCENTER:
  case MINUS_BARYCENTER: {
    originPoint =
        CCCoreLib::GeometricalAnalysisTools::ComputeGravityCenter(theCloud);
    ccLog::Print(QString("[UpdateNormalOrientations] Barycenter: (%1;%2;%3)")
                     .arg(originPoint.x)
                     .arg(originPoint.y)
                     .arg(originPoint.z));
    useOriginPoint = true;
  ...
  case PREVIOUS: {
    if (!theCloud->hasNormals()) {
      ccLog::Warning("[UpdateNormalOrientations] Can't orient the new normals "
                     "with the previous ones... as the cloud has no normals!");
      return false;
    }
  } break;
  ...
  case PLUS_SENSOR_ORIGIN: {
    ...
    if (!sensorFound) {
      ccLog::Warning(
          "[UpdateNormalOrientations] Could not find a valid sensor child");
      return false;
    }
  } break;
...
bool ccNormalVectors::ComputeCloudNormals(
    ccGenericPointCloud *theCloud, NormsIndexesTableType &theNormsCodes,
    CCCoreLib::LOCAL_MODEL_TYPES localModel, PointCoordinateType localRadius,
  ...
  case CCCoreLib::LS: {
    ...
    processedCells = theOctree->executeFunctionForAllCellsAtLevel(
        level, &(ComputeNormsAtLevelWithLS), additionalParameters, true,
        progressCb, "Normals Computation[LS]");
  } break;
  case CCCoreLib::TRI: {
    unsigned char level = theOctree->findBestLevelForAGivenPopulationPerCell(
        NUMBER_OF_POINTS_FOR_NORM_WITH_TRI);
    processedCells = theOctree->executeFunctionForAllCellsStartingAtLevel(
        level, &(ComputeNormsAtLevelWithTri), additionalParameters,
        NUMBER_OF_POINTS_FOR_NORM_WITH_TRI / 2,
        NUMBER_OF_POINTS_FOR_NORM_WITH_TRI * 3, true, progressCb,
  ...
  case CCCoreLib::QUADRIC: {
    unsigned char level =
        theOctree->findBestLevelForAGivenNeighbourhoodSizeExtraction(
            localRadius);
    processedCells = theOctree->executeFunctionForAllCellsAtLevel(
        level, &(ComputeNormsAtLevelWithQuadric), additionalParameters, true,
        progressCb, "Normals Computation[QUADRIC]");
  } break;
...
bool ccNormalVectors::ComputeNormalWithQuadric(
    CCCoreLib::GenericIndexedCloudPersist *points, const CCVector3 &P,
    CCVector3 &N) {
...
bool ccNormalVectors::ComputeNormalWithLS(
    CCCoreLib::GenericIndexedCloudPersist *pointAndNeighbors, CCVector3 &N) {
  N = CCVector3(0, 0, 0);
...
bool ccNormalVectors::ComputeNormalWithTri(
  ...
  CCCoreLib::GenericIndexedMesh *theMesh = Z.triangulateOnPlane(
      CCCoreLib::Neighbourhood::DO_NOT_DUPLICATE_VERTICES,
      CCCoreLib::Neighbourhood::IGNORE_MAX_EDGE_LENGTH, errorStr);
  if (!theMesh) {
    return false;
  }
  ...
  for (unsigned j = 0; j < triCount; ++j) {
    // we can't use getNextTriangleVertIndexes (which is faster on mesh groups
    // but not multi-thread compatible) but anyway we'll never get mesh groups
    // here!
...
bool ccNormalVectors::ComputeNormsAtLevelWithQuadric(
  ...
  for (unsigned i = 0; i < pointCount; ++i) {
    ...
    if (k >= NUMBER_OF_POINTS_FOR_NORM_WITH_QUADRIC) {
      CCCoreLib::DgmOctreeReferenceCloud neighbours(&nNSS.pointsInNeighbourhood,
                                                    k);
...
QString
ccNormalVectors::ConvertDipAndDipDirToString(PointCoordinateType dip_deg,
...
void ccNormalVectors::ConvertNormalToStrikeAndDip(
    const CCVector3 &N, PointCoordinateType &strike_deg,
  ...
  if (N.norm2() > std::numeric_limits<PointCoordinateType>::epsilon()) {
    strike_deg =
        180.0 - CCCoreLib::RadiansToDegrees(
                    atan2(N.y, N.x)); // atan2 output is between -180 and 180!
                                      // So strike is always positive here
...
void ccNormalVectors::ConvertNormalToDipAndDipDir(const CCVector3f &N,
                                                  float &dip_deg,
                                                  float &dipDir_deg) {
  ...
  if (N.norm2() > std::numeric_limits<float>::epsilon()) {
    ...
    // point downwards.
    float Nsign =
        N.z < 0 ? -1.0f : 1.0f; // DGM: copysign is not available on VS2012
...
void ccNormalVectors::ConvertNormalToDipAndDipDir(const CCVector3d &N,
                                                  double &dip_deg,
                                                  double &dipDir_deg) {
  // http://en.wikipedia.org/wiki/Structural_geology#Geometries
  ...
  if (N.norm2d() > std::numeric_limits<double>::epsilon()) {
    ...
    // point downwards.
    double Nsign =
        N.z < 0 ? -1.0 : 1.0; // DGM: copysign is not available on VS2012
...
CCVector3f ccNormalVectors::ConvertDipAndDipDirToNormal(float dip_deg,
                                                        float dipDir_deg,
                                                        bool upward /*=true*/) {
  // specific case
  ...
  CCVector3f N(Nxy * sin(dipDir_rad), Nxy * cos(dipDir_rad), Nz);

#ifdef _DEBUG
  // internal consistency test
  float dip2, dipDir2;
  ConvertNormalToDipAndDipDir(N, dip2, dipDir2);
  assert(std::abs(dip2 - dip_deg) < 1.0e-3f &&
         (dip2 == 0 || std::abs(dipDir2 - dipDir_deg) < 1.0e-3f));
#endif
...
CCVector3d ccNormalVectors::ConvertDipAndDipDirToNormal(double dip_deg,
                                                        double dipDir_deg,
                                                        bool upward /*=true*/) {
  // specific case
  if (std::isnan(dip_deg) || std::isnan(dipDir_deg)) {
  ...
  CCVector3d N(Nxy * sin(dipDir_rad), Nxy * cos(dipDir_rad), Nz);

#ifdef _DEBUG
  // internal consistency test
  double dip2, dipDir2;
  ConvertNormalToDipAndDipDir(N, dip2, dipDir2);
  assert(std::abs(dip2 - dip_deg) < 1.0e-3 &&
         (dip2 == 0 || std::abs(dipDir2 - dipDir_deg) < 1.0e-3));
#endif
...
void ccNormalVectors::ConvertNormalToHSV(const CCVector3 &N, float &H, float &S,
                                         float &V) {

...
void ccPointCloud::SetShaderPath(const QString &path) { s_shaderPath = path; }

void ccPointCloud::ReleaseShaders() { s_programDrawNormals.clear(); }
...
static bool InitProgramDrawNormals(QOpenGLContext *context) {
  if (s_programDrawNormals.isNull()) {
    ...
    if (!s_programDrawNormals->addShaderFromSourceFile(QOpenGLShader::Vertex,
                                                       vertexShaderFile)) {
    ...
    if (!s_programDrawNormals->addShaderFromSourceFile(QOpenGLShader::Geometry,
                                                       geometryShaderFile)) {
      error = s_programDrawNormals->log();
      ccLog::Error(error);
      return false;
    ...
    if (!s_programDrawNormals->addShaderFromSourceFile(QOpenGLShader::Fragment,
                                                       fragmentShaderFile)) {
      error = s_programDrawNormals->log();
      ccLog::Error(error);
      return false;
    ...
    s_drawNormalsShaderParameters.vertexLocation =
        s_programDrawNormals->attributeLocation("vertexIn");
    s_drawNormalsShaderParameters.normalLocation =
        s_programDrawNormals->attributeLocation("normal");
    s_drawNormalsShaderParameters.normalLengthLocation =
        s_programDrawNormals->uniformLocation("normalLength");
    s_drawNormalsShaderParameters.matrixLocation =
        s_programDrawNormals->uniformLocation("modelViewProjectionMatrix");
    s_drawNormalsShaderParameters.colorLocation =
        s_programDrawNormals->uniformLocation("color");
  }
...
ccPointCloud::ccPointCloud(
    ...
    unsigned uniqueID /*=ccUniqueIDGenerator::InvalidUniqueID*/) throw()
    : BaseClass(name, uniqueID), m_rgbaColors(nullptr), m_normals(nullptr),
      m_sfColorScaleDisplayed(false), m_currentDisplayedScalarField(nullptr),
      m_currentDisplayedScalarFieldIndex(-1), m_visibilityCheckEnabled(false),
      m_lod(nullptr), m_fwfData(nullptr), m_normalsDrawnAsLines(false) {
  setName(
      name); // sadly we cannot use the ccGenericPointCloud constructor argument
  showSF(false);
}

ccPointCloud *
ccPointCloud::From(CCCoreLib::GenericCloud *cloud,
  ...
  if (n == 0) {
    ...
    if (!pc->reserveThePointsTable(n)) {
      ccLog::Error(
          "[ccPointCloud::From] Not enough memory to duplicate cloud!");
      delete pc;
      pc = nullptr;
    } else {
      // import points
...
ccPointCloud *
ccPointCloud::From(const CCCoreLib::GenericIndexedCloud *cloud,
                   const ccGenericPointCloud *sourceCloud /*=nullptr*/) {
  ccPointCloud *pc = new ccPointCloud("Cloud");
        ...
        *_gridIndex = newIndexMap[*_gridIndex];
        if (*_gridIndex >= 0) {
          if (scanGrid->validCount) {
            scanGrid->minValidIndex = std::min(
                scanGrid->minValidIndex, static_cast<unsigned>(*_gridIndex));
            scanGrid->maxValidIndex = std::max(
                scanGrid->maxValidIndex, static_cast<unsigned>(*_gridIndex));
          } else {
...
ccPointCloud *
ccPointCloud::partialClone(const CCCoreLib::ReferenceCloud *selection,
  ...
  if (selectionSize != 0) {
    if (!result->reserveThePointsTable(selectionSize)) {
      ccLog::Error(
          "[ccPointCloud::partialClone] Not enough memory to duplicate cloud!");
      delete result;
      return nullptr;
    }
    ...
    // import points
    {
      for (unsigned i = 0; i < selectionSize; i++) {
        result->addPoint(
            *getPointPersistentPtr(selection->getPointGlobalIndex(i)));
      }
    }
    ...
    if (hasColors()) {
      if (result->reserveTheRGBTable()) {
        ...
        ccLog::Warning("[ccPointCloud::partialClone] Not enough memory to copy "
                       "RGB colors!");
        if (warnings)
    ...
    if (hasNormals()) {
      if (result->reserveTheNormsTable()) {
        for (unsigned i = 0; i < selectionSize; i++) {
          result->addNormIndex(
              getPointNormalIndex(selection->getPointGlobalIndex(i)));
        }
        result->showNormals(normalsShown());
      } else {
        ccLog::Warning(
            "[ccPointCloud::partialClone] Not enough memory to copy normals!");
        if (warnings)
    ...
    if (hasFWF()) {
      if (result->reserveTheFWFTable()) {
        try {
          ...
          ccLog::Warning("[ccPointCloud::partialClone] Not enough memory to "
        ...
        if (sf) {
          ...
          if (sfIdx >= 0) // success
          {
            ccScalarField *currentScalarField =
                static_cast<ccScalarField *>(result->getScalarField(sfIdx));
            ...
            if (currentScalarField->resizeSafe(selectionSize)) {
              ...
              for (unsigned i = 0; i < selectionSize; i++) {
                currentScalarField->setValue(
                    i, sf->getValue(selection->getPointGlobalIndex(i)));
              }
              ...
              ccLog::Warning(QString("[ccPointCloud::partialClone] Not enough "
                                     "memory to copy scalar field '%1'!")
    ...
    if (gridCount() != 0) {
      ...
      try {
        ...
        // not enough memory
        ccLog::Warning(QString("[ccPointCloud::partialClone] Not enough memory "
                               "to copy the grid structure(s)"));
      }
    }
...
void ccPointCloud::unallocatePoints() {
  clearLOD(); // we have to clear the LOD structure before clearing the colors /
              // SFs, so we can't leave it to notifyGeometryUpdate()
...
ccGenericPointCloud *
ccPointCloud::clone(ccGenericPointCloud *destCloud /*=nullptr*/,
                    bool ignoreChildren /*=false*/) {
  if (destCloud && !destCloud->isA(CC_TYPES::POINT_CLOUD)) {
    ccLog::Error("[ccPointCloud::clone] Invalid destination cloud provided! "
                 "Not a ccPointCloud...");
    return nullptr;
  }
...
const ccPointCloud &ccPointCloud::append(ccPointCloud *addedCloud,
  ...
  if (size() == pointCountBefore) // in some cases points have already been
    ...
    } else // otherwise
    {
      ...
      if (!hasColors()) {
        ...
        if (reserveTheRGBTable()) {
          ...
          ccLog::Warning("[ccPointCloud::fusion] Not enough memory: failed to "
                         "allocate colors!");
          showColors(false);
        }
      }
    ...
    } else // otherwise
    {
      ...
      if (!hasFWF()) {
        ...
        try {
          ...
          mergedContainer->insert(mergedContainer->end(), fwfData()->begin(),
                                  fwfData()->end());
          ...
          ccLog::Warning("[ccPointCloud::fusion] Not enough memory: failed to "
                         "merge waveform containers!");
        }
      }

      if (success) {
        // assert(hasFWF()); //DGM: new waveforms are not pushed yet, so there
        // might not be any in the cloud at the moment!
        ...
        try {
          ...
          if (!m_fwfDescriptors.empty()) {
            ...
            // we'll have to find free descriptor IDs (not used in the
            // destination cloud) before merging
            ...
            for (auto it = addedCloud->m_fwfDescriptors.begin();
              ...
              if (freeDescriptorIDs.empty()) {
                ccLog::Warning(
                    "[ccPointCloud::fusion] Not enough free FWF descriptor IDs "
                    "on destination cloud: some waveforms won't be imported!");
                break;
              }
              uint8_t newKey = freeDescriptorIDs.front();
              freeDescriptorIDs.pop();
              descriptorIDMap.insert(it.key(),
                                     newKey); // remember the ID transposition
              m_fwfDescriptors.insert(
                  newKey,
                  it.value()); // insert the descriptor at its new position (ID)
            }
          } else {
            for (auto it = addedCloud->m_fwfDescriptors.begin();
                 it != addedCloud->m_fwfDescriptors.end(); ++it) {
              descriptorIDMap.insert(
                  it.key(), it.key()); // same descriptor ID, no conversion
              m_fwfDescriptors.insert(it.key(),
        ...
        if (success && m_fwfWaveforms.size() == pointCountBefore) {
          for (unsigned i = 0; i < addedPoints; i++) {
            ...
            if (descriptorIDMap.contains(
                    w.descriptorID())) // the waveform can be imported :)
            {
            ...
            } else // the waveform is associated to a descriptor that couldn't
                   // be imported :(
            {
              m_fwfWaveforms.emplace_back(0);
              ++lostWaveformCount;
        ...
        if (lostWaveformCount) {
          ccLog::Warning(QString("[ccPointCloud::fusion] %1 waveform(s) were "
                                 "lost in the fusion process")
    ...
    for (unsigned k = 0; k < newSFCount; ++k) {
      ...
      if (sf) {
        ...
        if (sfIdx >= 0) // yes
        {
          ...
          assert(sameSF &&
                 sameSF->capacity() >= pointCountBefore + addedPoints);
        ...
        } else // otherwise we create a new SF
        {
          ...
          // we fill the beginning with NaN (as there is no equivalent in the
          // current cloud)
          if (newSF->resizeSafe(pointCountBefore + addedPoints, true,
            ...
            newSF->release();
            newSF = nullptr;
            ccLog::Warning("[ccPointCloud::fusion] Not enough memory: failed "
                           "to allocate a copy of scalar field '%s'",
                           sf->getName().c_str());
          }
        }
      }
    ...
    for (unsigned j = 0; j < sfCount; ++j) {
      if (!sfUpdated[j]) {
        ...
        if (sf->currentSize() == pointCountBefore) {
          // we fill the end with NaN (as there is no equivalent in the added
          // cloud)
    ...
    if (getNumberOfScalarFields() == 0) {
      ...
      if (sfCount == 0) {
        ...
        if (dispSF) {
          ...
          int sfIdx = getScalarFieldIndexByName(dispSF->getName()); // same
                                                                    // name!
    ...
    for (size_t i = 0; i < addedCloud->gridCount(); ++i) {
          ...
          otherGrid->validCount != 0) // no need to copy empty grids!
      {
        try {
          ...
          m_grids.resize(0);
          ccLog::Warning(
              QString("[ccPointCloud::fusion] Not enough memory: failed to "
                      "copy the grid structure(s) from '%1'")
  ...
  {
    ccLog::Warning(QString("[ccPointCloud::fusion] Grid structure(s) will be "
    ...
    if (addedCloud->isShifted()) {
      if (!isShifted()) {
        ...
        copyGlobalShiftAndScale(*addedCloud);
                 ...
                 (getGlobalShift() - addedCloud->getGlobalShift()).norm2d() >
                     1.0e-6) {
        // the clouds have different shift & scale information!
        ccLog::Warning(
            QString("[ccPointCloud::fusion] Global shift/scale information "
                    "conflict: shift/scale of cloud '%1' will be ignored!")
                .arg(addedCloud->getName()));
      }
    }
  }
  ...
  if (!ignoreChildren) {
    ...
    for (unsigned c = 0; c < childrenCount; ++c) {
      ...
      if (child->isA(CC_TYPES::MESH)) // mesh --> FIXME: what for the other
                                      // types of MESH?
      {
        ...
        ccMesh *cloneMesh = mesh->cloneMesh(
            mesh->getAssociatedCloud() == addedCloud ? this : nullptr);
        if (cloneMesh) {
          ...
          ccLog::Warning(QString("[ccPointCloud::fusion] Not enough memory: "
...
bool ccPointCloud::reserveTheRGBTable() {
  if (m_points.capacity() == 0) {
    ccLog::Warning("[ccPointCloud] Calling reserveTheRGBTable with an zero "
                   "capacity cloud");
  }
...
bool ccPointCloud::resizeTheRGBTable(bool fillWithWhite /*=false*/) {
  if (m_points.empty()) {
    ccLog::Warning(
        "[ccPointCloud] Calling resizeTheRGBTable with an empty cloud");
  }
  ...
  static const ccColor::Rgba s_white(ccColor::MAX, ccColor::MAX, ccColor::MAX,
                                     ccColor::MAX);
...
bool ccPointCloud::reserveTheNormsTable() {
  if (m_points.capacity() == 0) {
    ccLog::Warning("[ccPointCloud] Calling reserveTheNormsTable with an zero "
                   "capacity cloud");
  }
...
bool ccPointCloud::resizeTheNormsTable() {
  if (m_points.empty()) {
    ccLog::Warning(
        "[ccPointCloud] Calling resizeTheNormsTable with an empty cloud");
  }
...
bool ccPointCloud::compressFWFData() {
  ...
  try {
    ...
    if (newIndex >= initialCount) {
      // nothing to do
      ccLog::Print(QString("[ccPointCloud::compressFWFData] Cloud '%1': no "
                           "need to compress FWF data")
                       .arg(getName()));
      return true;
    }
    ...
    ccLog::Print(
        QString("[ccPointCloud::compressFWFData] Cloud '%1': FWF data "
                "compressed --> %2 / %3 (%4%)")
            .arg(getName())
            .arg(newIndex)
            .arg(initialCount)
            .arg(100.0 - (newIndex * 100.0) / initialCount, 0, 'f', 1));
  } catch (const std::bad_alloc &) {
...
bool ccPointCloud::reserveTheFWFTable() {
  if (m_points.capacity() == 0) {
    ccLog::Warning(
        "[ccPointCloud] Calling reserveTheFWFTable with a zero capacity cloud");
  }
      ...
      if (m_fwfDescriptors.contains(w.descriptorID())) {
        WaveformDescriptor &d =
            const_cast<ccPointCloud *>(this)->m_fwfDescriptors
                [w.descriptorID()]; // DGM: we really want the reference to the
                                    // element, not a copy as QMap returns in the
                                    // const case :(
...
bool ccPointCloud::resizeTheFWFTable() {
  if (m_points.capacity() == 0) {
    ccLog::Warning(
        "[ccPointCloud] Calling resizeTheFWFTable with an empty cloud");
  }
...
bool ccPointCloud::reserve(unsigned newNumberOfPoints) {
  ...
  // if we are changing the cloud contents, let's stop the LOD construction
  // process
  clearLOD();
  ...
  // ccLog::Warning(QString("[ccPointCloud::reserve] Cloud is %1 and its
...
const CompressedNormType &
ccPointCloud::getPointNormalIndex(unsigned pointIndex) const {
...
bool ccPointCloud::convertNormalToRGB() {
  ...
  const std::vector<ccColor::Rgb> &normalHSV =
...
bool ccPointCloud::convertRGBToGreyScale() {
  ...
  for (unsigned i = 0; i < count; ++i) {
    ...
    rgba.r = rgba.g = rgba.b =
        static_cast<unsigned char>(std::max(std::min(luminance, 255.0), 0.0));
  }
...
additionalParameters is positive.
...
**/
static bool
ComputeCellGaussianFilter(const CCCoreLib::DgmOctree::octreeCell &cell,
  ...
  ccPointCloud::RgbFilterOptions filterParams =
      *(static_cast<ccPointCloud::RgbFilterOptions *>(additionalParameters[2]));
  ...
  // we already know the points lying in the first cell (this is the one we are
  // treating :)
  try {
  ...
  CCCoreLib::DgmOctree::NeighboursSet::iterator it =
  ...
  for (unsigned i = 0; i < n; ++i) // for each point in cell
  {
    ...
    if (bilateralFilter) {
      ...
      // check that the query SF value is valid, otherwise no need to compute
      // anything
    ...
    if (median) {
      ...
      for (unsigned j = 0; j < k; ++j, ++it) {
        ...
        if (filterParams.blendGrayscale) {
          double grayscaleMin = (col.r / 3.0) + (col.g / 3.0) + (col.b / 3.0) -
                                filterParams.blendGrayscaleThreshold;
          double grayscaleMax =
              grayscaleMin + 2.0 * filterParams.blendGrayscaleThreshold;
      ...
      if (wSum != 0.0) {
        ccColor::Rgb avgCol(static_cast<ColorCompType>(std::max(
        ...
        if (filterParams.blendGrayscale) {
          // if the neighbor set contains more grayscale point than given
          // percent, so use only use grayscale points
          if ((static_cast<double>(nrOfGrayscale) >
               filterParams.blendGrayscalePercent * nrOfUsedNeighbours) &&
              wGrayscaleSum != 0) {
            avgCol.r = static_cast<ColorCompType>(std::max(
                std::min(255.0, rgbGrayscaleSum.r / wGrayscaleSum), 0.0));
            avgCol.g = static_cast<ColorCompType>(std::max(
                std::min(255.0, rgbGrayscaleSum.g / wGrayscaleSum), 0.0));
            avgCol.b = static_cast<ColorCompType>(std::max(
                std::min(255.0, rgbGrayscaleSum.b / wGrayscaleSum), 0.0));
          } else // else, we have more RGB colors than grayscale ones. We use
                 // only the RGB values.
          {
            double wRGBSum = wSum - wGrayscaleSum;
            if (wRGBSum != 0.0) {
              avgCol.r = static_cast<ColorCompType>(std::max(
                  std::min(255.0, (rgbSum.r - rgbGrayscaleSum.r) / wRGBSum),
                  0.0));
              avgCol.g = static_cast<ColorCompType>(std::max(
                  std::min(255.0, (rgbSum.g - rgbGrayscaleSum.g) / wRGBSum),
                  0.0));
              avgCol.b = static_cast<ColorCompType>(std::max(
                  std::min(255.0, (rgbSum.b - rgbGrayscaleSum.b) / wRGBSum),
                  0.0));
            }
          }
        }
...
bool ccPointCloud::applyFilterToRGB(
    PointCoordinateType sigma, PointCoordinateType sigmaSF,
  ...
  if ((sigmaSF > 0) && (nullptr == getCurrentOutScalarField())) {
    ccLog::Warning("[ccPointCloud::applyFilterToRGB] A non-zero scalar field "
  ...
  if (!theOctree) {
    if (!computeOctree(progressCb)) {
      ccLog::Warning(
          "[ccPointCloud::applyFilterToRGB] Failed to compute the octree");
      delete theOctree;
      return false;
    } else {
  ...
  // best octree level
  unsigned char level =
      theOctree->findBestLevelForAGivenNeighbourhoodSizeExtraction(3 * sigma);
...
bool ccPointCloud::setRGBColorByBanding(unsigned char dim, double freq) {
  ...
  for (unsigned i = 0; i < count; i++) {
    ...
    ccColor::Rgba C(static_cast<ColorCompType>(((sin(z + 0.0f) + 1.0f) / 2.0f) *
                                               ccColor::MAX),
                    static_cast<ColorCompType>(
                        ((sin(z + 2.0944f) + 1.0f) / 2.0f) * ccColor::MAX),
                    static_cast<ColorCompType>(
                        ((sin(z + 4.1888f) + 1.0f) / 2.0f) * ccColor::MAX),
                    ccColor::MAX);
...
bool ccPointCloud::setColor(const ccColor::Rgba &col) {
  ...
  assert(m_rgbaColors);
  m_rgbaColors->resize(
      size()); // reserve might have set a capacity larger than the cloud size!
...
void ccPointCloud::scale(PointCoordinateType fx, PointCoordinateType fy,
                         PointCoordinateType fz, CCVector3 center) {
  ...
  if (hasNormals()) {
    ...
    if (fx < 0 || fy < 0 || fz < 0) {
      PointCoordinateType signX =
          (fx < 0 ? -CCCoreLib::PC_ONE : CCCoreLib::PC_ONE);
      PointCoordinateType signY =
          (fy < 0 ? -CCCoreLib::PC_ONE : CCCoreLib::PC_ONE);
      PointCoordinateType signZ =
          (fz < 0 ? -CCCoreLib::PC_ONE : CCCoreLib::PC_ONE);
...
inline float
GetSymmetricalNormalizedValue(ScalarType sfVal,
...
static GLenum GL_COORD_TYPE =
    sizeof(PointCoordinateType) == 4 ? GL_FLOAT : GL_DOUBLE;

void ccPointCloud::glChunkVertexPointer(const CC_DRAW_CONTEXT &context,
                                        size_t chunkIndex, unsigned decimStep,
                                        bool useVBOs) {
  QOpenGLFunctions_2_1 *glFunc = context.glFunctions<QOpenGLFunctions_2_1>();
  assert(glFunc != nullptr);

  if (useVBOs && m_vboManager.state == vboSet::INITIALIZED &&
    ...
    if (m_vboManager.vbos[chunkIndex]->bind()) {
      glFunc->glVertexPointer(3, GL_COORD_TYPE,
                              decimStep * 3 * sizeof(PointCoordinateType),
                              nullptr);
      m_vboManager.vbos[chunkIndex]->release();
    } else {
      ccLog::Warning(
          "[VBO] Failed to bind VBO?! We'll deactivate them then...");
    ...
    glFunc->glVertexPointer(3, GL_COORD_TYPE,
                            decimStep * 3 * sizeof(PointCoordinateType),
                            ccChunk::Start(m_points, chunkIndex));
  }
}
...
static PointCoordinateType
    s_pointBuffer[MAX_POINT_COUNT_PER_LOD_RENDER_PASS * 3];
static PointCoordinateType
    s_normalBuffer[MAX_POINT_COUNT_PER_LOD_RENDER_PASS * 3];
...
void ccPointCloud::glChunkNormalPointer(const CC_DRAW_CONTEXT &context,
                                        size_t chunkIndex, unsigned decimStep,
                                        bool useVBOs) {
  assert(m_normals);
  ...
  if (useVBOs && m_vboManager.state == vboSet::INITIALIZED &&
    ...
    if (m_vboManager.vbos[chunkIndex]->bind()) {
      const GLbyte *start =
          nullptr; // fake pointer used to prevent warnings on Linux
      int normalDataShift = m_vboManager.vbos[chunkIndex]->normalShift;
      glFunc->glNormalPointer(
          GL_COORD_TYPE, decimStep * 3 * sizeof(PointCoordinateType),
          static_cast<const GLvoid *>(start + normalDataShift));
      m_vboManager.vbos[chunkIndex]->release();
    } else {
      ccLog::Warning(
          "[VBO] Failed to bind VBO?! We'll deactivate them then...");
    ...
    const CompressedNormType *_normalsIndexes =
        ccChunk::Start(*m_normals, chunkIndex);
    ...
    for (size_t j = 0; j < chunkSize;
         j += decimStep, _normalsIndexes += decimStep) {
...
void ccPointCloud::glChunkColorPointer(const CC_DRAW_CONTEXT &context,
                                       size_t chunkIndex, unsigned decimStep,
                                       bool useVBOs) {
  assert(m_rgbaColors);
  ...
  if (useVBOs && m_vboManager.state == vboSet::INITIALIZED &&
    ...
    if (m_vboManager.vbos[chunkIndex]->bind()) {
      ...
      glFunc->glColorPointer(
          4, GL_UNSIGNED_BYTE, decimStep * 4 * sizeof(ColorCompType),
          static_cast<const GLvoid *>(start + colorDataShift));
      m_vboManager.vbos[chunkIndex]->release();
    } else {
      ccLog::Warning(
          "[VBO] Failed to bind VBO?! We'll deactivate them then...");
    ...
    assert(m_rgbaColors);
    // standard OpenGL copy
    glFunc->glColorPointer(4, GL_UNSIGNED_BYTE,
                           decimStep * 4 * sizeof(ColorCompType),
                           ccChunk::Start(*m_rgbaColors, chunkIndex));
  } else {
    assert(false);
  }
}

void ccPointCloud::glChunkSFPointer(const CC_DRAW_CONTEXT &context,
                                    size_t chunkIndex, unsigned decimStep,
                                    bool useVBOs) {
  assert(m_currentDisplayedScalarField);
  assert(sizeof(ColorCompType) == 1);
  ...
  if (useVBOs && m_vboManager.state == vboSet::INITIALIZED &&
    ...
    // we must convert the scalar values to RGB colors in a dedicated static
    // array
    size_t chunkStart = ccChunk::StartPos(chunkIndex);
    ColorCompType *_sfColors = s_rgbBuffer4ub;
    size_t chunkSize =
        ccChunk::Size(chunkIndex, m_currentDisplayedScalarField->size());
    for (size_t j = 0; j < chunkSize; j += decimStep) {
      // SF value
      ScalarType sfValue =
          m_currentDisplayedScalarField->getValue(chunkStart + j);
      // convert the scalar value to a RGB color
      const ccColor::Rgb *col =
          m_currentDisplayedScalarField->getColor(sfValue);
      assert(col);
      *_sfColors++ = col->r;
      *_sfColors++ = col->g;
      *_sfColors++ = col->b;
...
void glLODChunkVertexPointer(ccPointCloud *cloud, QOpenGLFunctions *glFunc,
                             const LODIndexSet &indexMap, unsigned startIndex,
                             unsigned stopIndex) {
...
template <class QOpenGLFunctions>
void glLODChunkSFPointer(ccScalarField *sf, QOpenGLFunctions *glFunc,
                         const LODIndexSet &indexMap, unsigned startIndex,
                         unsigned stopIndex) {
  assert(startIndex < indexMap.size() && stopIndex <= indexMap.size());
  assert(sf && glFunc);
  ...
  // we must re-order and convert SF values to RGB colors in a dedicated static
  // array
  ColorCompType *_sfColors = s_rgbBuffer4ub;
  for (unsigned j = startIndex; j < stopIndex; j++) {
    unsigned pointIndex = indexMap[j];
...
struct DisplayDesc : LODLevelDesc {
  ...
  DisplayDesc(unsigned startIndex, unsigned count)
      : LODLevelDesc(startIndex, count), endIndex(startIndex + count),
        decimStep(1), indexMap(nullptr) {}
...
void ccPointCloud::drawMeOnly(CC_DRAW_CONTEXT &context) {
  ...
  if (MACRO_Draw3D(context)) {
    ...
    if (entityPickingMode) {
      ...
      if (glParams.showSF &&
          !m_currentDisplayedScalarField->mayHaveHiddenValues()) {
        glParams.showSF = false; //--> we keep it only if SF 'NaN' values are
                                 //potentially hidden
      }
    }
      ...
      if (context.decimateCloudOnMove &&
        ...
        if (!m_lod || !m_lod->isBroken()) {
          if (!m_lod || m_lod->isNull()) {
            ...
            // DGM: can't spawn a progress dialog here as the process will be
            // async ccProgressDialog pDlg(false, context.display ?
            // context.display->asWidget() : 0);
            initLOD(/*&pDlg*/);
          } else {
            assert(m_lod);
            ...
            if (underConstruction || maxLevel == 0) {
              ...
              if (context.currentLODLevel == 0) {
                ...
                glFunc->glGetDoublev(GL_PROJECTION_MATRIX,
                                     camera.projectionMat.data());
                glFunc->glGetDoublev(GL_MODELVIEW_MATRIX,
                                     camera.modelViewMat.data());
                // camera frustum
                Frustum frustum(camera.modelViewMat, camera.projectionMat);

                // first time: we flag the cells visibility and count the number
                // of visible points
                m_lod->flagVisibility(
                    frustum, m_clipPlanes.empty() ? nullptr : &m_clipPlanes);
              }
              ...
              toDisplay.indexMap =
                  &m_lod->getIndexMap(context.currentLODLevel, toDisplay.count,
              ...
              context.moreLODPointsAvailable =
                  (remainingPointsAtThisLevel != 0);
              context.higherLODLevelsAvailable =
                  (!m_lod->allDisplayed() &&
                   context.currentLODLevel + 1 <= maxLevel);
            }
          }
        }
        ...
        if (!toDisplay.indexMap && !skipLoD) {
          ...
          if (context.minLODPointCount &&
            ...
            toDisplay.decimStep =
                static_cast<int>(ceil(static_cast<float>(toDisplay.count) /
                                      context.minLODPointCount));
            toDisplay.decimStep = std::min<unsigned>(
                toDisplay.decimStep,
                maxStride / (3 * sizeof(PointCoordinateType)));
          }
        }
      }
    }

    // ccLog::Print(QString("Rendering %1 points starting from index %2 (LoD =
    // %3 / PN =
    // %4)").arg(toDisplay.count).arg(toDisplay.startIndex).arg(toDisplay.indexMap
    // ? "yes" : "no").arg(pushName ? "yes" : "no"));

    glFunc->glPushAttrib(GL_LIGHTING_BIT | GL_COLOR_BUFFER_BIT |
                         GL_TRANSFORM_BIT | GL_POINT_BIT);
    ...
    if (glParams.showNorms) {
      ...
      glFunc->glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT,
                           CC_DEFAULT_CLOUD_AMBIENT_COLOR.rgba);
      glFunc->glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR,
                           CC_DEFAULT_CLOUD_SPECULAR_COLOR.rgba);
      glFunc->glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE,
                           CC_DEFAULT_CLOUD_DIFFUSE_COLOR.rgba);
      glFunc->glMaterialfv(GL_FRONT_AND_BACK, GL_EMISSION,
                           CC_DEFAULT_CLOUD_EMISSION_COLOR.rgba);
      glFunc->glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS,
    ...
    if (context.drawRoundedPoints) {
      ...
      // DGM: alpha/blending doesn't work well because it creates a halo around
      // points with a potentially wrong color (due to the display order)
    ...
    {
      // if some points are hidden (= visibility table instantiated), we can't
      // use display arrays :(
      if (isVisibilityTableInstantiated()) {
        ...
        if (!entityPickingMode) {
          for (unsigned j = toDisplay.startIndex; j < toDisplay.endIndex;
               j += toDisplay.decimStep) {
            // we must test each point visibility
            unsigned pointIndex =
                toDisplay.indexMap ? toDisplay.indexMap->at(j) : j;
            if (m_pointsVisibility.empty() ||
                m_pointsVisibility[pointIndex] == CCCoreLib::POINT_VISIBLE) {
              if (glParams.showSF) {
                assert(pointIndex <
                       m_currentDisplayedScalarField->currentSize());
                const ccColor::Rgb *col =
                    m_currentDisplayedScalarField->getValueColor(pointIndex);
                // we force display of points hidden because of their scalar
                // field value to be sure that the user doesn't miss them (during
                // manual segmentation for instance)
                ccGL::Color(
                    glFunc,
                    col ? *col : ccColor::lightGreyRGB); // Make sure all points
                                                         // are visible. No alpha
                                                         // used on purpose
              } else if (glParams.showColors) {
                ccGL::Color(glFunc, m_rgbaColors->getValue(pointIndex));
              }
              if (glParams.showNorms) {
                ccGL::Normal3v(glFunc,
                               compressedNormals
                                   ->getNormal(m_normals->getValue(pointIndex))
                                   .u);
              }
      ...
      } else if (glParams.showSF) // no visibility table enabled + scalar field
      {
        ...
        if (!hiddenPoints && context.useVBOs &&
            !toDisplay.indexMap) // VBOs are not compatible with LoD
        {
        ...
        const ccScalarField::Range &sfDisplayRange =
            m_currentDisplayedScalarField->displayRange();
        const ccScalarField::Range &sfSaturationRange =
            m_currentDisplayedScalarField->saturationRange();
        ...
        if (colorRampShader) {
          ...
          GLint maxComponents =
              (maxBytes >> 2) - 4; // leave space for the other uniforms!
          unsigned steps = m_currentDisplayedScalarField->getColorRampSteps();
          assert(steps != 0);

          if (steps > ccColorRampShader::MaxColorRampSize() ||
              maxComponents < static_cast<GLint>(steps)) {
            ...
            if (!m_currentDisplayedScalarField->symmetricalScale()) {
              sfMinSatRel = GetNormalizedValue(
                  sfSaturationRange.start(),
                  sfDisplayRange); // doesn't need to be between 0 and 1!
              sfMaxSatRel = GetNormalizedValue(
                  sfSaturationRange.stop(),
                  sfDisplayRange); // doesn't need to be between 0 and 1!
            } else {
              // we can only handle 'maximum' saturation
              sfMinSatRel = GetSymmetricalNormalizedValue(
                  -sfSaturationRange.stop(), sfSaturationRange);
              sfMaxSatRel = GetSymmetricalNormalizedValue(
                  sfSaturationRange.stop(), sfSaturationRange);
            ...
            if (!colorRampShader->setup(glFunc, sfMinSatRel, sfMaxSatRel, steps,
                                        colorScale)) {
              ...
              // we must get rid of lights material (other than ambient) for the
              // red and green fields
              glFunc->glPushAttrib(GL_LIGHTING_BIT);

              // we use the ambient light to pass the scalar value (and 'grayed'
              // marker) without any modification from the GPU pipeline, even if
              // normals are enabled!
        ...
        if (!hiddenPoints) {
          ...
          if (toDisplay.indexMap) // LoD display
          {
            ...
            while (s < toDisplay.endIndex) {
              unsigned count = std::min(MAX_POINT_COUNT_PER_LOD_RENDER_PASS,
                                        toDisplay.endIndex - s);
              unsigned e = s + count;

              // points
              glLODChunkVertexPointer<QOpenGLFunctions_2_1>(
                  this, glFunc, *toDisplay.indexMap, s, e);
              // normals
              if (glParams.showNorms) {
                glLODChunkNormalPointer<QOpenGLFunctions_2_1>(
                    m_normals, glFunc, *toDisplay.indexMap, s, e);
              }
              // SF colors
              if (colorRampShader) {
                float *_sfColors = s_rgbBuffer3f;
                bool symScale =
                    m_currentDisplayedScalarField->symmetricalScale();
                for (unsigned j = s; j < e; j++, _sfColors += 3) {
                  unsigned pointIndex = toDisplay.indexMap->at(j);
                  ScalarType sfVal =
                      m_currentDisplayedScalarField->getValue(pointIndex);
                  // normalized sf value
                  _sfColors[0] =
                      symScale ? GetSymmetricalNormalizedValue(
                                     sfVal, sfSaturationRange)
                               : GetNormalizedValue(sfVal, sfDisplayRange);
                  // flag: whether point is grayed out or not (NaN values are
                  // also rejected!)
                ...
                glLODChunkSFPointer<QOpenGLFunctions_2_1>(
                    m_currentDisplayedScalarField, glFunc, *toDisplay.indexMap,
                    s, e);
              }
            ...
            for (size_t k = 0; k < chunkCount; ++k) {
              ...
              if (colorRampShader) {
                ...
                bool symScale =
                    m_currentDisplayedScalarField->symmetricalScale();
                for (size_t j = 0; j < chunkSize;
                     j += toDisplay.decimStep, _sfColors += 3) {
                  // SF value
                  ScalarType sfValue =
                      m_currentDisplayedScalarField->getValue(chunkStart + j);
                  // normalized sf value
                  _sfColors[0] =
                      symScale ? GetSymmetricalNormalizedValue(
                                     sfValue, sfSaturationRange)
                               : GetNormalizedValue(sfValue, sfDisplayRange);
                  // flag: whether point is grayed out or not (NaN values are
                  // also rejected!)
                  _sfColors[1] =
                      sfDisplayRange.isInRange(sfValue) ? 1.0f : 0.0f;
                  // reference value (to get the true lighting value)
                  _sfColors[2] = 1.0f;
                }
                glFunc->glColorPointer(3, GL_FLOAT, 0, s_rgbBuffer3f);
              ...
              if (toDisplay.decimStep > 1) {
                chunkSize = static_cast<unsigned>(
                    static_cast<double>(chunkSize) /
                    toDisplay.decimStep); // static_cast is equivalent to floor
                                          // if value >= 0
              }
              glFunc->glDrawArrays(GL_POINTS, 0,
            ...
            if (colorRampShader) {
              if (!m_currentDisplayedScalarField->symmetricalScale()) {
                for (unsigned j = toDisplay.startIndex; j < toDisplay.endIndex;
                  ...
                  {
                    glFunc->glColor3f(GetNormalizedValue(sf, sfDisplayRange),
                                      1.0f, 1.0f);
                    ccGL::Normal3v(
                        glFunc, compressedNormals
                                    ->getNormal(m_normals->getValue(pointIndex))
                                    .u);
                    ccGL::Vertex3v(glFunc, m_points[pointIndex].u);
                  }
                ...
                for (unsigned j = toDisplay.startIndex; j < toDisplay.endIndex;
                  ...
                  {
                    glFunc->glColor3f(
                        GetSymmetricalNormalizedValue(sf, sfSaturationRange),
                        1.0f, 1.0f);
                    ccGL::Normal3v(
                        glFunc, compressedNormals
                                    ->getNormal(m_normals->getValue(pointIndex))
            ...
            if (colorRampShader) {
              ...
              for (unsigned j = toDisplay.startIndex; j < toDisplay.endIndex;
                ...
                const ccColor::Rgb *col =
                    m_currentDisplayedScalarField->getValueColor(pointIndex);
                if (col) {
                  // for entity picking, don't change the color, we just need to
                  // know whether the point is visible
                  ccGL::Vertex3v(glFunc, m_points[pointIndex].u);
                }
              }
            } else {
      ...
      {
        bool useVBOs = context.useVBOs && !toDisplay.indexMap
                           ? updateVBOs(context, glParams)
                           : false; // VBOs are not compatible with LoD
        ...
        if (toDisplay.indexMap) // LoD display
        {
          ...
          while (s < toDisplay.endIndex) {
            ...
            // colors
            if (glParams.showColors)
              glLODChunkColorPointer<QOpenGLFunctions_2_1>(
                  m_rgbaColors, glFunc, *toDisplay.indexMap, s, e);
          ...
          for (size_t k = 0; k < chunkCount; ++k) {
            ...
            if (toDisplay.decimStep > 1) {
              chunkSize = static_cast<unsigned>(
                  static_cast<double>(chunkSize) /
                  toDisplay.decimStep); // static_cast is equivalent to floor if
                                        // value >= 0
            }
    ...
    glFunc->glPopAttrib(); // GL_LIGHTING_BIT | GL_COLOR_BUFFER_BIT |
                           // GL_TRANSFORM_BIT | GL_POINT_BIT --> will switch the
                           // light off
...
ccPointCloud *ccPointCloud::filterPointsByScalarValue(ScalarType minVal,
  ...
  QSharedPointer<CCCoreLib::ReferenceCloud> c(
      CCCoreLib::ManualSegmentationTools::segment(this, minVal, maxVal,
                                                  outside));
...
void ccPointCloud::hidePointsByScalarValue(ScalarType minVal,
  ...
  if (!resetVisibilityArray()) {
    ccLog::Error(
        QString("[Cloud %1] Visibility table could not be instantiated!")
            .arg(getName()));
    return;
  }
  ...
  if (!sf) {
    ccLog::Error(
        QString("[Cloud %1] Internal error: no activated output scalar field!")
            .arg(getName()));
    return;
  }
...
ccGenericPointCloud *ccPointCloud::createNewCloudFromVisibilitySelection(
    ...
    bool silent /*=false*/, CCCoreLib::ReferenceCloud *selection /*=nullptr*/) {
  if (!visTable) {
    if (!isVisibilityTableInstantiated()) {
      ccLog::Error(QString("[Cloud %1] Visibility table not instantiated!")
                       .arg(getName()));
      return nullptr;
    }
    ...
    if (visTable->size() != size()) {
      ccLog::Error(
          QString("[Cloud %1] Invalid input visibility table").arg(getName()));
      return nullptr;
    }
  }
  ...
  if (!result) {
    ccLog::Warning("[ccPointCloud::createNewCloudFromVisibilitySelection] "
                   "Failed to generate a subset cloud");
    return nullptr;
  }
  ...
  try {
    if (newIndexes) {
      if (newIndexes->empty()) {
        ...
        ccLog::Error(
            "[removeVisiblePoints] Input 'new indexes' has a wrong size");
        return false;
      }
...
bool ccPointCloud::convertCurrentScalarFieldToColors(
  ...
  if (!hasDisplayedScalarField()) {
    ccLog::Warning("[ccPointCloud::setColorWithCurrentScalarField] No active "
    ...
    for (unsigned i = 0; i < count; i++) {
      ...
      if (col) {
        ...
        _color.r = static_cast<ColorCompType>(
            _color.r * (static_cast<float>(col->r) / ccColor::MAX));
        _color.g = static_cast<ColorCompType>(
            _color.g * (static_cast<float>(col->g) / ccColor::MAX));
        _color.b = static_cast<ColorCompType>(
            _color.b * (static_cast<float>(col->b) / ccColor::MAX));
      }
    }
  }
...
QSharedPointer<CCCoreLib::ReferenceCloud> ccPointCloud::computeCPSet(
  ...
  // create temporary SF for the nearest neighbors determination
  // (computeCloud2CloudDistances) so that we can properly remove it afterwards!
  ...
  result = CCCoreLib::DistanceComputationTools::computeCloud2CloudDistances(
  ...
  if (result < 0) {
    ccLog::Warning(
        "[ccPointCloud::ComputeCPSet] Closest-point set computation failed!");
    CPSet.clear();
  }
...
bool ccPointCloud::interpolateColorsFrom(
  ...
  if (!otherCloud || otherCloud->size() == 0) {
    ccLog::Warning(
        "[ccPointCloud::interpolateColorsFrom] Invalid/empty input cloud!");
    return false;
  }
  ...
  if (std::abs(dist.x) > dimSum.x / 2 || std::abs(dist.y) > dimSum.y / 2 ||
      std::abs(dist.z) > dimSum.z / 2) {
    ccLog::Warning("[ccPointCloud::interpolateColorsFrom] Clouds are too far "
                   "from each other! Can't proceed.");
    return false;
  }
...
static void ProjectOnCylinder(const CCVector3 &AP, const CCVector3 &xDir,
                              const CCVector3 &yDir, PointCoordinateType radius,
...
static void ProjectOnCone(const CCVector3 &AP, PointCoordinateType alpha_rad,
                          const CCVector3 &axisDir, const CCVector3 &xDir,
                          const CCVector3 &yDir, PointCoordinateType &s,
                          PointCoordinateType &delta,
                          PointCoordinateType &phi_rad) {
  PointCoordinateType x = AP.dot(xDir);
  PointCoordinateType y = AP.dot(yDir);
  ...
  PointCoordinateType gamma_rad =
      beta_rad - alpha_rad; // if gamma_rad > 0, the point is outside the cone
...
ccPointCloud *ccPointCloud::unroll(
    UnrollMode mode, UnrollBaseParams *params,
    bool exportDeviationSF /*=false*/, double startAngle_deg /*=0.0*/,
    double stopAngle_deg /*=360.0*/, bool arbitraryOutputCS /*=false*/,
    CCCoreLib::GenericProgressCallback *progressCb /*=nullptr*/) const {
  if (!params || startAngle_deg >= stopAngle_deg) {
  ...
  CCVector3 xDir, yDir;
  {
    ...
    // now project the X and Y main directions on the plane perpendicular to
    // axisDir
  ...
  ccLog::Print(QString("[Unroll] Axis direction: (%1;%2;%3)")
                   .arg(axisDir.x)
                   .arg(axisDir.y)
                   .arg(axisDir.z));
  ccLog::Print(QString("[Unroll] 1st orthogonal direction: (%1;%2;%3)")
                   .arg(xDir.x)
                   .arg(xDir.y)
                   .arg(xDir.z));
  ccLog::Print(QString("[Unroll] 2nd orthogonal direction: (%1;%2;%3)")
                   .arg(yDir.x)
                   .arg(yDir.y)
                   .arg(yDir.z));
  ...
  if (progressCb) {
    if (progressCb->textCanBeEdited()) {
      ...
      progressCb->setInfo(
          qPrintable(QString("Number of points = %1").arg(numberOfPoints)));
    }
    progressCb->update(0);
    progressCb->start();
  }
  ...
  for (unsigned i = 0; i < numberOfPoints; i++) {
    ...
    case CONE_CONICAL: {
      ...
      ProjectOnCone(AP, alpha_rad, axisDir, xDir, yDir, posAlongAxis, delta,
                    longitude_rad);
      // unrolling
    ...
    if (withNormals) {
      ...
      case CYLINDER: {
        ...
        ProjectOnCylinder(AP2, xDir, yDir, params->radius, delta2,
                          longitude2_rad);

        N2.x = static_cast<PointCoordinateType>(
      ...
      case CONE_CONICAL: {
        ...
        PointCoordinateType delta2 = 0;
        PointCoordinateType longitude2_rad = 0;
        ProjectOnCone(AP2, alpha_rad, axisDir, xDir, yDir, posAlongAxis2,
                      delta2, longitude2_rad);
        // unrolling
        PointCoordinateType theta_rad = longitude_rad * coneParams->spanRatio;
        ...
        N2.y =
            -(posAlongAxis2 * cos(theta2_rad) - posAlongAxis * cos(theta_rad));
        N2.z = delta2 - delta;
      } break;
      ...
      case CONE_CYLINDRICAL_ADAPTIVE_RADIUS: {
        ...
        ProjectOnCone(AP2, alpha_rad, axisDir, xDir, yDir, posAlongAxis2,
                      delta2, longitude2_rad);
        N2.x = static_cast<PointCoordinateType>(
            (longitude2_rad * posAlongAxis2 - longitude_rad * posAlongAxis) *
            sin_alpha);
        N2.y = -(delta2 - delta);
        N2.z = posAlongAxis - posAlongAxis2;
      } break;
      ...
      if (!arbitraryOutputCS) {
        // projects the output point to a coordinate system linked to the input
        // cylinder/cone CS
    ...
    if (!arbitraryOutputCS) {
      ...
      switch (mode) {
      case CYLINDER:
        Pout = cylParams->center /*+ (Pout.x * xDir)*/ + (Pout.y * yDir) +
               (Pout.z * axisDir);
        break;
      ...
      case CONE_CYLINDRICAL_FIXED_RADIUS:
      case CONE_CYLINDRICAL_ADAPTIVE_RADIUS:
        Pout = coneParams->apex + (Pout.x * xDir) + (Pout.y * yDir) +
               (Pout.z * axisDir);
        break;
    ...
    for (; dLongitude_rad < stopAngle_rad; dLongitude_rad += 2 * M_PI) {
      ...
      case CONE_CONICAL: {
        ...
        if (!arbitraryOutputCS) {
          Pout2 = coneParams->apex + (Pout.x * xDir) + (Pout.y * yDir) +
                  (Pout.z * axisDir);
        } else {
          Pout2 = Pout;
        }
      ...
      case CONE_CYLINDRICAL_FIXED_RADIUS:
        Pout +=
            static_cast<PointCoordinateType>(dLongitude_rad * params->radius) *
            xDir2;
        Pout2 = Pout;
        break;
      case CONE_CYLINDRICAL_ADAPTIVE_RADIUS:
        Pout += static_cast<PointCoordinateType>(dLongitude_rad * posAlongAxis *
                                                 sin_alpha) *
                xDir2;
        Pout2 = Pout;
        break;
  ...
  if (clone) {
    ...
    if (exportDeviationSF) {
      ...
      if (sfIdx < 0) {
        ...
        if (sfIdx < 0) {
          ccLog::Warning("[unrollOnCylinder] Not enough memory to init the "
...
int ccPointCloud::addScalarField(ccScalarField *sf) {
  ...
  if (getScalarFieldIndexByName(sf->getName()) >= 0) {
    ccLog::Warning(
        QString("[ccPointCloud::addScalarField] Name '%1' already exists!")
            .arg(QString::fromStdString(sf->getName())));
    return -1;
  }
  ...
  // points array (dataVersion>=20)
  {
            ...
            8) // file is 'float' and current type is 'double'
    {
      result = ccSerializationHelper::GenericArrayFromTypedFile<
          CCVector3, 3, PointCoordinateType, float>(m_points, in, dataVersion,
                                                    "3D points");
    ...
    {
      result = ccSerializationHelper::GenericArrayFromTypedFile<
          CCVector3, 3, PointCoordinateType, double>(m_points, in, dataVersion,
                                                     "3D points");
    } else {
      result = ccSerializationHelper::GenericArrayFromFile<CCVector3, 3,
                                                           PointCoordinateType>(
    ...
    // test: look for NaN values
    {
      ...
      if (nanPointsCount) {
        ccLog::Warning(QString("[BIN] Cloud '%1' contains %2 NaN point(s)!")
                           .arg(getName())
                           .arg(nanPointsCount));
      }
    }
    ...
    if (hasColorsArray) {
      ...
      if (classID == CC_TYPES::RGB_COLOR_ARRAY) {
        ...
        for (size_t i = 0; i < count; ++i) {
          m_rgbaColors->addElement(
              ccColor::Rgba(oldRGBColors->getValue(i), ccColor::MAX));
        }
...
bool ccPointCloud::Grid::fromFile(QFile &in, short dataVersion, int flags,
                                  LoadedIDMap &oldToNewIDMap) {
  // width (dataVersion>=41)
  uint32_t _w = 0;
...
short ccPointCloud::minimumFileVersion_MeOnly() const {
  short minVersion = std::max(static_cast<short>(27),
                              ccGenericPointCloud::minimumFileVersion_MeOnly());
  minVersion = std::max(minVersion,
  ...
  if (hasScalarFields())
    minVersion =
        std::max(minVersion,
                 static_cast<ccScalarField *>(getScalarField(0))
                     ->minimumFileVersion()); // we assume they are all the same

  if (gridCount() != 0) {
    minVersion = std::max(minVersion, static_cast<short>(41));
    minVersion = std::max(
        minVersion,
        grid(0)->minimumFileVersion()); // we assume they are all the same
  }
...
CCCoreLib::ReferenceCloud *ccPointCloud::crop(const ccBBox &box,
                                              bool inside /*=true*/) {
  if (!box.isValid()) {
...
CCCoreLib::ReferenceCloud *ccPointCloud::crop2D(const ccPolyline *poly,
  ...
  for (unsigned i = 0; i < count; ++i) {
    ...
    bool pointIsInside =
        CCCoreLib::ManualSegmentationTools::isPointInsidePoly(P2D, poly);
    if (inside == pointIsInside) {
      ref->addPointIndex(i);
    }
  }
...
static bool CatchGLErrors(GLenum err, const char *context) {
  // catch GL errors
  {
    ...
    case GL_INVALID_FRAMEBUFFER_OPERATION:
      ccLog::Warning("[%s] OpenGL error: invalid framebuffer operation",
                     context);
      break;
    }
  }
...
// DGM: normals are so slow to display that it's a waste of memory and time to
// load them in VBOs!
#define DONT_LOAD_NORMALS_IN_VBOS

bool ccPointCloud::updateVBOs(const CC_DRAW_CONTEXT &context,
  ...
  if (m_vboManager.state == vboSet::FAILED) {
    // ccLog::Warning(QString("[ccPointCloud::updateVBOs] VBOs are in a 'failed'
    // state... we won't try to update them! (cloud '%1')").arg(getName()));
    return false;
  }

  if (!m_currentDisplay) {
    ccLog::Warning(QString("[ccPointCloud::updateVBOs] Need an associated GL "
                           "context! (cloud '%1')")
                       .arg(getName()));
    assert(false);
    return false;
  ...
  if (m_vboManager.state == vboSet::INITIALIZED) {
    ...
    if (glParams.showColors &&
        (!m_vboManager.hasColors || m_vboManager.colorIsSF)) {
  ...
  if (m_vboManager.vbos.size() != chunksCount) {
    ...
    try {
      ...
      ccLog::Warning(
          QString("[ccPointCloud::updateVBOs] Not enough memory! (cloud '%1')")
              .arg(getName()));
      m_vboManager.state = vboSet::FAILED;
      return false;
    }
  ...
  {
    // DGM: the context should be already active as this method should only be
    // called from 'drawMeOnly'
    assert(!glParams.showSF || m_currentDisplayedScalarField);
    assert(!glParams.showColors || m_rgbaColors);
#ifndef DONT_LOAD_NORMALS_IN_VBOS
    assert(!glParams.showNorms ||
           (m_normals && m_normals->chunksCount() >= chunksCount));
#endif

    m_vboManager.hasColors = glParams.showSF || glParams.showColors;
    m_vboManager.colorIsSF = glParams.showSF;
    m_vboManager.sourceSF =
        glParams.showSF ? m_currentDisplayedScalarField : nullptr;
#ifndef DONT_LOAD_NORMALS_IN_VBOS
    ...
    for (size_t chunkIndex = 0; chunkIndex < chunksCount; ++chunkIndex) {
      ...
      int vboSizeBytes = m_vboManager.vbos[chunkIndex]->init(
          chunkSize, m_vboManager.hasColors, m_vboManager.hasNormals,
          &reallocated);

      QOpenGLFunctions_2_1 *glFunc =
          context.glFunctions<QOpenGLFunctions_2_1>();
      if (glFunc) {
        CatchGLErrors(glFunc->glGetError(), "ccPointCloud::vbo.init");
      }

      if (vboSizeBytes > 0) {
        // ccLog::Print(QString("[VBO] VBO #%1 initialized
        // (ID=%2)").arg(chunkIndex).arg(m_vboManager.vbos[chunkIndex]->bufferId()));
        ...
        if (chunkUpdateFlags & vboSet::UPDATE_POINTS) {
          m_vboManager.vbos[chunkIndex]->write(
              0, ccChunk::Start(m_points, chunkIndex),
              sizeof(PointCoordinateType) * chunkSize * 3);
        }
        // load colors
          ...
          if (glParams.showSF) {
            ...
            if (m_vboManager.sourceSF) {
              ...
              for (int j = 0; j < chunkSize; j++) {
                // SF value
                ScalarType sfValue =
                    m_vboManager.sourceSF->getValue(chunkStart + j);
                // we need to convert scalar value to color into a temporary
                // structure
                const ccColor::Rgb *col =
                    m_vboManager.sourceSF->getColor(sfValue);
                if (!col) {
            ...
            m_vboManager.vbos[chunkIndex]->write(
                m_vboManager.vbos[chunkIndex]->rgbShift, s_rgbBuffer4ub,
                sizeof(ColorCompType) * chunkSize * 4);
            // upadte 'modification' flag for current displayed SF
            m_vboManager.sourceSF->setModificationFlag(false);
          } else if (glParams.showColors) {
            m_vboManager.vbos[chunkIndex]->write(
                m_vboManager.vbos[chunkIndex]->rgbShift,
                ccChunk::Start(*m_rgbaColors, chunkIndex),
                sizeof(ColorCompType) * chunkSize * 4);
          }
        }
        ...
        if (glParams.showNorms && (chunkUpdateFlags & UPDATE_NORMALS)) {
          ...
          m_vboManager.vbos[chunkIndex]->write(
              m_vboManager.vbos[chunkIndex]->normalShift, s_normalBuffer,
              sizeof(PointCoordinateType) * chunkSize * 3);
        }
#endif
        m_vboManager.vbos[chunkIndex]->release();
      ...
      if (vboSizeBytes < 0) // VBO initialization failed
      {
        ...
        if (chunkIndex == 0) {
          ccLog::Warning(
              QString("[ccPointCloud::updateVBOs] Failed to initialize VBOs "
                      "(not enough memory?) (cloud '%1')")
                  .arg(getName()));
          m_vboManager.state = vboSet::FAILED;
  ...
  //			.arg(chunkIndex)
  //			.arg(m_vboManager.vbos[chunkIndex] &&
  //m_vboManager.vbos[chunkIndex]->isCreated() ? "created" : "not created")
  //			.arg(m_vboManager.vbos[chunkIndex] ?
  //m_vboManager.vbos[chunkIndex]->bufferId() : -1));
  //}

#ifdef _DEBUG
  if (m_vboManager.totalMemSizeBytes != totalSizeBytesBefore)
    ccLog::Print(QString("[VBO] VBO(s) (re)initialized for cloud '%1' (%2 Mb = "
                         "%3% of points could be loaded)")
                     .arg(getName())
                     .arg(static_cast<double>(m_vboManager.totalMemSizeBytes) /
                              (1 << 20),
                          0, 'f', 2)
                     .arg(static_cast<double>(pointsInVBOs) / size() * 100.0, 0,
                          'f', 2));
#endif
...
int ccPointCloud::VBO::init(int count, bool withColors, bool withNormals,
  ...
  if (!isCreated()) {
    ...
    setUsagePattern(
        QGLBuffer::DynamicDraw); //"StaticDraw: The data will be set once and
                                 //used many times for drawing operations."
                                 //"DynamicDraw: The data will be modified
                                 //repeatedly and used many times for drawing
                                 //operations.
  }
...
size_t ccPointCloud::vboSize() const { return m_vboManager.totalMemSizeBytes; }
...
bool ccPointCloud::computeNormalsWithGrids(
    double minTriangleAngle_deg /*=1.0*/, ccProgressDialog *pDlg /*=nullptr*/) {
  unsigned pointCount = size();
  if (pointCount < 3) {
    ccLog::Warning(
        QString("[computeNormalsWithGrids] Cloud '%1' has not enough points")
            .arg(getName()));
    return false;
  }
  ...
  PointCoordinateType minAngleCos = static_cast<PointCoordinateType>(
      cos(CCCoreLib::DegreesToRadians(minTriangleAngle_deg)));
  // double minTriangleAngle_rad =
  // CCCoreLib::DegreesToRadians(minTriangleAngle_deg);
  ...
  for (size_t gi = 0; gi < gridCount(); ++gi) {
    ...
    if (!scanGrid || scanGrid->h == 0 || scanGrid->w == 0 ||
      ...
      // invalid grid
      ccLog::Warning(
          QString("[computeNormalsWithGrids] Grid structure #%i is invalid")
              .arg(gi + 1));
      continue;
    }
    ...
    if (pDlg) {
      pDlg->setLabelText(
          QObject::tr("Grid: %1 x %2").arg(scanGrid->w).arg(scanGrid->h));
      pDlg->setValue(0);
    ...
    CCVector3 sensorOrigin =
        (scanGrid->sensorPosition.getTranslationAsVec3D() /* + m_globalShift*/)
            .toPC();
    ...
    for (int j = 0; j < static_cast<int>(scanGrid->h) - 1; ++j) {
      for (int i = 0; i < static_cast<int>(scanGrid->w) - 1; ++i) {
        ...
        Tuple3i tris[4]{{v0, v2, v1}, {v0, v3, v1}, {v0, v2, v3}, {v1, v2, v3}};
...
bool ccPointCloud::orientNormalsWithGrids(ccProgressDialog *pDlg /*=nullptr*/) {
  ...
  if (pointCount == 0) {
    ccLog::Warning(
        QString("[orientNormalsWithGrids] Cloud '%1' is empty").arg(getName()));
    return false;
  }

  if (!hasNormals()) {
    ccLog::Warning(QString("[orientNormalsWithGrids] Cloud '%1' has no normals")
                       .arg(getName()));
    return false;
  }
  if (gridCount() == 0) {
    ccLog::Warning(
        QString(
            "[orientNormalsWithGrids] Cloud '%1' has no associated grid scan")
            .arg(getName()));
    return false;
  }
  ...
  for (size_t gi = 0; gi < gridCount(); ++gi) {
    ...
    if (!scanGrid || scanGrid->h == 0 || scanGrid->w == 0 ||
      ...
      // invalid grid
      ccLog::Warning(
          QString("[orientNormalsWithGrids] Grid structure #%i is invalid")
              .arg(gi + 1));
      continue;
    }
        ...
        if (*_indexGrid >= 0) {
          ...
          if (pDlg) {
            ...
            if (pDlg->wasCanceled()) {
              unallocateNorms();
              ccLog::Warning(
                  "[orientNormalsWithGrids] Process cancelled by user");
              return false;
            } else {
...
bool ccPointCloud::computeNormalsWithOctree(
    ...
    PointCoordinateType defaultRadius, ccProgressDialog *pDlg /*=nullptr*/) {
  // compute the normals the 'old' way ;)
  if (!getOctree() && !computeOctree(pDlg)) {
    ccLog::Warning(
        QString("[computeNormals] Could not compute octree for cloud '%1'")
            .arg(getName()));
    return false;
  }
  ...
  if (!ccNormalVectors::ComputeCloudNormals(
          ...
          getOctree().data())) {
    ccLog::Warning(
        QString("[computeNormals] Failed to compute normals on cloud '%1'")
            .arg(getName()));
    return false;
  }

  ccLog::Print("[ComputeCloudNormals] Timing: %3.2f s.",
               eTimer.elapsed() / 1000.0);

  if (!hasNormals()) {
    if (!resizeTheNormsTable()) {
      ccLog::Error(QString("Not enough memory to compute normals on cloud '%1'")
                       .arg(getName()));
...
bool ccPointCloud::normalsAreDrawn() const { return m_normalsDrawnAsLines; }
...
void ccPointCloud::drawNormalsAsLines(CC_DRAW_CONTEXT &context) {
  if (!InitProgramDrawNormals(context.qGLContext)) {
    ccLog::Warning(
        "[ccPointCloud::drawNormalsAsLines] impossible to init shader program");
    return;
  }
  ...
  QMatrix4x4 projectionModelView =
      projection * modelView; // QMatrix4x4 expects row major data

  s_programDrawNormals->bind();
  // set uniforms
  s_programDrawNormals->setUniformValue(
      s_drawNormalsShaderParameters.matrixLocation, projectionModelView);
  s_programDrawNormals->setUniformValue(
      s_drawNormalsShaderParameters.normalLengthLocation,
      GLfloat(m_normalLineParameters.length));
  s_programDrawNormals->setUniformValue(
      s_drawNormalsShaderParameters.colorLocation,
      m_normalLineParameters.color.r, m_normalLineParameters.color.g,
      m_normalLineParameters.color.b, m_normalLineParameters.color.a);

  // set the vertex locations array
  s_programDrawNormals->setAttributeArray(
      s_drawNormalsShaderParameters.vertexLocation,
      static_cast<GLfloat *>(m_points.front().u), 3);
  // set the normals array
  s_programDrawNormals->setAttributeArray(
      s_drawNormalsShaderParameters.normalLocation,
      static_cast<GLfloat *>(m_decompressedNormals.front().u), 3);
  // enable the vertex locations array
  s_programDrawNormals->enableAttributeArray(
      s_drawNormalsShaderParameters.vertexLocation);
  ...
  s_programDrawNormals->disableAttributeArray(
      s_drawNormalsShaderParameters.vertexLocation);
  s_programDrawNormals->disableAttributeArray(
      s_drawNormalsShaderParameters.normalLocation);
...
unsigned char ccPointCloud::testVisibility(const CCVector3 &P) const {
  if (m_visibilityCheckEnabled) {
    // if we have associated sensors, we can use them to check the visibility of
    // other points
...
bool ccPointCloud::computeFWFAmplitude(
    double &minVal, double &maxVal, ccProgressDialog *pDlg /*=nullptr*/) const {
  minVal = maxVal = 0;
...
bool ccPointCloud::enhanceRGBWithIntensitySF(
    int sfIdx, bool useCustomIntensityRange /*=false*/, double minI /*=0.0*/,
    double maxI /*=1.0*/) {
  ...
  if (intRange < 1.0e-6) {
    ccLog::Warning("[ccPointCloud::enhanceRGBWithIntensitySF] Intensity range "
                   "is too small");
    return false;
  }
  ...
  for (unsigned i = 0; i < size(); ++i) {
    ...
    col.r = static_cast<ColorCompType>(
        std::max<ScalarType>(std::min<ScalarType>(scale * col.r, 255), 0));
    col.g = static_cast<ColorCompType>(
        std::max<ScalarType>(std::min<ScalarType>(scale * col.g, 255), 0));
    col.b = static_cast<ColorCompType>(
        std::max<ScalarType>(std::min<ScalarType>(scale * col.b, 255), 0));
  }
...
ccMesh *
ccPointCloud::triangulateGrid(const Grid &grid,
                              double minTriangleAngle_deg /*=0.0*/) const {
  // the code below has been kindly provided by Romain Janvier
  CCVector3 sensorOrigin =
      (grid.sensorPosition.getTranslationAsVec3D() /* + m_globalShift*/).toPC();
...
bool ccPointCloud::setCoordFromSF(bool importDims[3],
  ...
  for (unsigned i = 0; i < pointCount; ++i) {
    ...
    // handle NaN values
    PointCoordinateType coord = CCCoreLib::ScalarField::ValidValue(s)
...
ccPointCloud *
ccPointCloud::removeDuplicatePoints(double minDistanceBetweenPoints,
                                    ccProgressDialog *pDlg /*=nullptr*/) {
  ...
  CCCoreLib::GeometricalAnalysisTools::ErrorCode result =
      CCCoreLib::GeometricalAnalysisTools::FlagDuplicatePoints(
          this, minDistanceBetweenPoints, pDlg, octree.data());
  ...
  if (duplicateCount == 0) {
    ...
    ccLog::Print(
        QObject::tr("Cloud '%1' has no duplicate points").arg(getName()));
    deleteScalarField(sfIdx);
    return this;
  }

  ccLog::Warning(QObject::tr("Cloud '%1' has %2 duplicate point(s)")
                     .arg(getName())
                     .arg(duplicateCount));
  ...
  if (!filteredCloud) {
    ccLog::Warning(
        QObject::tr("Not enough memory to create the filtered cloud"));
    return nullptr;
...
bool ccPointCloud::shiftPointsAlongNormals(PointCoordinateType shift) {
  if (!hasNormals()) {
    ccLog::Warning(
        "[ccGenericPointCloud::shiftPointsAlongNormals] Cloud has no normals");
    return false;
  }
...
bool ccPointCloud::Grid::init(unsigned rowCount, unsigned colCount,
                              bool withRGB /*=false*/) {
...
void ccPointCloud::Grid::setColor(unsigned row, unsigned col,
                                  const ccColor::Rgb &rgb) {
  assert(row < h);
  assert(col < w);
  assert(!colors.empty());

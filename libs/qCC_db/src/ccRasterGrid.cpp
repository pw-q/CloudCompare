...
void ccRasterCell::getPointIndexes(
    std::vector<unsigned> &indexes,
    const std::vector<void *> &pointRefList) const {
  indexes.clear();
  // Assemble a list of all point indexes in this cell
  // Start with first point in cell, and browse through every point using the
  // linked ref list, and collect point indexes
...
ccRasterGrid::ccRasterGrid()
    : width(0), height(0), gridStep(1.0), minCorner(0, 0, 0), minHeight(0),
      maxHeight(0), meanHeight(0), nonEmptyCellCount(0), validCellCount(0),
      hasColors(false), valid(false) {}

ccRasterGrid::~ccRasterGrid() { clear(); }

bool ccRasterGrid::ComputeGridSize(unsigned char Z, const ccBBox &box,
                                   double gridStep, unsigned &gridWidth,
                                   unsigned &gridHeight) {
  ...
  // DGM: we use the 'PixelIsArea' convention but minCorner is the lower left
  // cell CENTER
...
ccRasterGrid::InterpolationType
...
bool ccRasterGrid::fillWith(
    ccGenericPointCloud *cloud, unsigned char Z, ProjectionType projectionType,
      ...
      try {
        ...
        for (unsigned i = 0; i < sfCount; ++i) {
          scalarFields[i].resize(
              gridTotalSize, std::numeric_limits<SF::value_type>::quiet_NaN());
        }
      } catch (const std::bad_alloc &) {
        // not enough memory
        scalarFields.resize(0);
        ccLog::Warning(
            "[Rasterize] Failed to allocate memory for scalar fields!");
      }
    } else {
      projectSFs = false;
    }
  ...
  if (projectionType == PROJ_INVERSE_VAR_VALUE ||
      sfProjectionType == PROJ_INVERSE_VAR_VALUE) {
    if (zStdDevSfIndex >= 0) {
      assert(pc);
  ...
  std::vector<ScalarType>
      sfValues; // common vector used to sort SF values in each cell
  ...
  for (unsigned j = 0; j < height; ++j) {
    ...
    for (unsigned i = 0; i < width; ++i) {
      ...
      double cellModelStdDevHeight =
          std::numeric_limits<double>::quiet_NaN(); // for inv. var. projection
                                                    // mode only

      if (aCell.nbPoints) {
        // Assemble a list of all points in this cell.
        // Start with first point in cell, and browse through the other points
        // using the linked ref list, and collect point indexes
        void **pRef = aCell.pointRefHead;
        for (unsigned n = 0; n < aCell.nbPoints; ++n) {
          unsigned pointIndex =
              static_cast<unsigned>(pRef - pointRefList.data());
        ...
        auto cellPointIndexedHeightEnd =
            std::next(cellPointIndexedHeight.begin(), aCell.nbPoints);
        // sorting indexed points in cell based on height in ascending order
        ParallelSort(cellPointIndexedHeight.begin(), cellPointIndexedHeightEnd,
                     [](const IndexAndValue &a, const IndexAndValue &b) {
                       return a.val < b.val;
                     });
        ...
        if (projectionType != PROJ_INVERSE_VAR_VALUE) {
          ...
          cellStdDevHeight =
              sqrt(std::max(0.0, cellSquareSum / aCell.nbPoints -
                                     cellAvgHeight * cellAvgHeight));
        } else // inverse variance projection mode
        {
          ...
          for (unsigned n = 0; n < aCell.nbPoints; ++n) {
            ...
            ScalarType stdDev =
                zStdDevSF->getValue(cellPointIndexedHeight[n].index);
            if (ccScalarField::ValidValue(stdDev) &&
                CCCoreLib::GreaterThanEpsilon(stdDev)) {
              double invVar = 1.0 / (static_cast<double>(stdDev) * stdDev);
              weightedSum += invVar * cellPointIndexedHeight[n].val;
              weightedSquareSum += invVar * cellPointIndexedHeight[n].val *
                                   cellPointIndexedHeight[n].val;
          ...
          if (CCCoreLib::GreaterThanEpsilon(sumInverseVariance)) {
            ...
            cellStdDevHeight =
                std::sqrt(std::abs(weightedSquareSum / sumInverseVariance -
                                   cellAvgHeight * cellAvgHeight));
            cellModelStdDevHeight = std::sqrt(1.0 / sumInverseVariance);
          } else {
            // we can't compute these values if the weight is null (= no valid
            // SF value)
            cellAvgHeight = cellStdDevHeight = cellModelStdDevHeight =
        ...
        case PROJ_INVERSE_VAR_VALUE:
          ...
          if (std::isfinite(aCell.h)) {
            // we choose the point which is the closest to the cell center (in
            // 2D)
        ...
        case PROJ_MEDIAN_VALUE: {
          ...
          if (aCell.nbPoints % 2) // odd value
          {
            ...
            aCell.h = (cellPointIndexedHeight[indexMid - 1].val +
                       cellPointIndexedHeight[indexMid].val) /
                      2;
          }
        ...
        case PROJ_MAXIMUM_VALUE:
          ...
          aCell.nearestPointIndex =
              cellPointIndexedHeight[aCell.nbPoints - 1].index;
          break;
        default:
          assert(false);
          break;
        ...
        if (hasColors) {
          ...
          if (projectionType == PROJ_AVERAGE_VALUE) {
            ...
            const ccColor::Rgb &col =
                cloud->getPointColor(aCell.nearestPointIndex);
            aCell.color = CCVector3d(col.r, col.g, col.b);
          }
        }
        ...
        if (projectSFs) {
          ...
          for (size_t k = 0; k < scalarFields.size(); ++k) {
            ...
            case PROJ_MEDIAN_VALUE: {
              ...
              if (sfValues.size() > 1) {
                ...
                if (sfValues.size() % 2) // odd number
                {
                  ...
                  scalarFields[k][pos] = static_cast<ScalarType>(
                      (static_cast<double>(sfValues[midIndex - 1]) +
                       sfValues[midIndex]) /
                      2);
                }
            ...
            case PROJ_AVERAGE_VALUE: {
              // for average, we do a simple average of unsorted SF-values in
              // cell
              ...
              scalarFields[k][pos] =
                  validPointCount != 0
                      ? scalarFieldWeightedSum / validPointCount
                      : std::numeric_limits<double>::quiet_NaN();
            } break;

            case PROJ_INVERSE_VAR_VALUE:
              // inverse variance projection mode: weighted average with weights
              // of 1/var
              if (projectionType == PROJ_INVERSE_VAR_VALUE &&
                  k == zStdDevSfIndex) {
                // Special case for the 'std deviation' scalar field, output
                // layer should just be filled with the updated model standard
                // deviation.
                ...
                for (unsigned n = 0; n < aCell.nbPoints; n++) {
                  ...
                  if (ccScalarField::ValidValue(stdDev) &&
                    ...
                    if (ccScalarField::ValidValue(value)) {
                      ...
                      scalarFieldWeightedSum +=
                          static_cast<double>(weight) * value;
                ...
                scalarFields[k][pos] =
                    CCCoreLib::GreaterThanEpsilon(scalarFieldWeightSum)
                        ? scalarFieldWeightedSum / scalarFieldWeightSum
                        : std::numeric_limits<double>::quiet_NaN();
              }
              break;
  ...
  case InterpolationType::DELAUNAY: {
    DelaunayInterpolationParams *const params =
        reinterpret_cast<DelaunayInterpolationParams *const>(
            interpolationParams);
    if (params) {
      interpolateEmptyCells(params->maxEdgeLength * params->maxEdgeLength);
    } else {
      ccLog::Error("[Rasterize] Internal error: Delauny interpolation "
  ...
  case InterpolationType::KRIGING: {
    KrigingParams *krigingParams =
        reinterpret_cast<KrigingParams *>(interpolationParams);
    if (krigingParams) {
      fillGridCellsWithKriging(Z, krigingParams->kNN, krigingParams->params,
                               !krigingParams->autoGuess, progressDialog);
    } else {
      ccLog::Error("[Rasterize] Internal error: Krigin interpolation "
                   "parameters are not set");
    }
  } break;
  }
...
static void InterpolateOnBorder(const std::vector<uint8_t> &pointsOnBorder,
                                const CCVector2i P[3], int i, int j, int coord,
                                int dim, ccRasterCell &cell,
                                ccRasterGrid &grid) {
  ...
  if (P[minIndex][dim] <= coord && coord <= P[maxIndex][dim]) {
    ...
    if (d > 0) {
      ...
      for (auto &gridSF : grid.scalarFields) {
        ...
        gridSF[i + j * grid.width] =
            (1.0 - relativePos) * sfValA + relativePos * sfValB;
      }
...
bool ccRasterGrid::interpolateEmptyCells(double maxSquareEdgeLength) {
  ...
  for (unsigned j = 0; j < height; ++j) {
    ...
    for (unsigned i = 0; i < width; ++i) {
      if (row[i].nbPoints) {
        ...
        the2DPoints[index++] = CCVector2(static_cast<PointCoordinateType>(i),
  ...
  if (!delaunayMesh.buildMesh(
          the2DPoints, CCCoreLib::Delaunay2dMesh::USE_ALL_POINTS, errorStr)) {
    ccLog::Warning(
        QStringLiteral("[Rasterize] Empty cells interpolation failed. Could "
                       "not compute the 2.5D mesh ('%1')")
            .arg(QString::fromStdString(errorStr)));
    return false;
  }

  if (maxSquareEdgeLength > 0.0) {
    // we have to scale the maxSquareEdgeLength parameters as we will consider
    // now 'cells' and not real units
  ...
  for (unsigned k = 0; k < triNum; ++k) {
    ...
    // now scan the cells
    {
      ...
      int det = (P[1].y - P[2].y) * (P[0].x - P[2].x) -
      ...
      for (int j = yMin; j <= yMax; ++j) {
        ...
        for (int i = xMin; i <= xMax; ++i) {
          ...
          if (!row[i].nbPoints && !std::isfinite(row[i].h)) {
            ...
            // Point Inclusion in Polygon Test (inspired from W. Randolph
            // Franklin - WRF)
            bool inside = false;
            if (det != 0) {
              for (int ti = 0; ti < 3; ++ti) {
                ...
                if ((P2.y <= j && j < P1.y) || (P1.y <= j && j < P2.y)) {
                  int t =
                      (i - P2.x) * (P1.y - P2.y) - (P1.x - P2.x) * (j - P2.y);
                  if (P1.y < P2.y)
                    t = -t;
                  if (t < 0)
                    inside = !inside;
            ...
            // can we interpolate?
            if (inside) {
              double l1 = ((P[1].y - P[2].y) * (i - P[2].x) -
                           (P[1].x - P[2].x) * (j - P[2].y)) /
                          static_cast<double>(det);
              double l2 = ((P[2].y - P[0].y) * (i - P[2].x) -
                           (P[2].x - P[0].x) * (j - P[2].y)) /
                          static_cast<double>(det);
              double l3 = 1.0 - l1 - l2;

              row[i].h = l1 * valA + l2 * valB + l3 * valC;
              // assert(std::isfinite(row[i].h)); //it can happen with the inv.
              // var. projection mode
#ifdef _DEBUG
              ++interpolatedCells;
#endif
            ...
            } else // second test for the borders (only the top and right
                   // borders have this issue in fact)
            {
              /*if (i == 0 && onLeftBorder.size() > 1)
              {
                      InterpolateOnBorder(onLeftBorder, P, i, j, j, 1, row[i],
              *this);
              }
              else */
              if (static_cast<unsigned>(i + 1) == width &&
                  onRightBorder.size() > 1) {
                InterpolateOnBorder(onRightBorder, P, i, j, j, 1, row[i],
                                    *this);
              }

              /*if (j == 0 && onBottomBorder.size() > 1)
              {
                      InterpolateOnBorder(onBottomBorder, P, i, j, i, 0, row[i],
              *this);
              }
              else*/
              if (static_cast<unsigned>(j + 1) == height &&
                  onTopBorder.size() > 1) {
...
bool ccRasterGrid::fillGridCellsWithKriging(
    unsigned char Z, int knn, Kriging::KrigeParams &krigeParams,
    bool useInputParams, ccProgressDialog *progressDialog /*=nullptr*/) {
  if (Z > 2) {
  ...
  CCCoreLib::NormalizedProgress nProgress(
      progressDialog, static_cast<unsigned>(nonEmptyCellCount * stepCount));
  ...
  for (size_t sfIndex = 0; sfIndex < scalarFields.size(); ++sfIndex) {
    ...
    for (unsigned j = 0; j < height; ++j) {
      ...
      for (unsigned i = 0; i < width; ++i) {
        sf[i + j * width] =
            kriging.ordinaryKrigeSingleCell(sfKrigeParams, i, j, context);
  ...
  if (hasColors) {
    for (unsigned char c = 0; c < 3; ++c) {
      ...
      // compute default parameters
      Kriging::KrigeParams colorKrigeParams =
          kriging.computeDefaultParameters();
      // use the same model as for the altitudes
      if (krigeParams.model != Kriging::Invalid) {
      ...
      for (unsigned j = 0; j < height; ++j) {
        ...
        for (unsigned i = 0; i < width; ++i) {
          double col =
              kriging.ordinaryKrigeSingleCell(colorKrigeParams, i, j, context);
          ccRasterCell &cell = row[i];
...
void ccRasterGrid::fillEmptyCells(EmptyCellFillOption fillEmptyCellsStrategy,
  ...
  case INTERPOLATE_DELAUNAY: // we still fill the empty cells, as some cells may
                             // not be interpolated in the end!
...
ccPointCloud *ccRasterGrid::convertToCloud(
    ...
    const std::vector<ExportableFields> &exportedStatistics, bool projectSFs,
    bool projectColors, bool resampleInputCloudXY, bool resampleInputCloudZ,
    ccGenericPointCloud *inputCloud, unsigned char Z, const ccBBox &box,
    double percentileValue, bool exportToOriginalCS,
  ...
  ccPointCloud *inputCloudAsPC =
      (inputCloud && inputCloud->isA(CC_TYPES::POINT_CLOUD))
          ? static_cast<ccPointCloud *>(inputCloud)
          : nullptr;

  // if we 'resample' the input cloud, we actually resample it (one point in
  // each cell) and we may have to change some things afterwards (height, scalar
  // fields, etc.)
  if (resampleInputCloudXY) {
    if (!inputCloud) {
      ccLog::Warning("[Rasterize] Internal error: no input clouds specified "
                     "(for resampling)");
      assert(false);
      return nullptr;
    }
    ...
    cloudGrid = inputCloudAsPC ? inputCloudAsPC->partialClone(&refCloud)
    ...
    if (!resampleInputCloudZ) {
      ...
      for (unsigned j = 0; j < height; ++j) {
        for (unsigned i = 0; i < width; ++i) {
          ...
          if (cell.nbPoints) // non empty cell
          {
            const_cast<CCVector3 *>(cloudGrid->getPoint(pointIndex))->u[Z] =
    ...
    if (!projectSFs) {
      ...
      // original cloud, we may have to create additional points and/or scalar
      // fields return cloudGrid;
    }
  } else {
  ...
  if (exportSFStats && inputCloudAsPC && sfStatCount != 0) {
    maxNumberOfExportedSfStatisticsFields =
        inputCloudAsPC->getNumberOfScalarFields() * exportedStatistics.size();

    if (std::find(exportedStatistics.begin(), exportedStatistics.end(),
                  PER_CELL_VALUE) != exportedStatistics.end()) {
      // we ignore 'per cell value' which is not a statistic
      --sfStatCount;
    }
    numberOfExportedSfStatisticsFields =
        inputCloudAsPC->getNumberOfScalarFields() * sfStatCount;
  }

  std::vector<CCCoreLib::ScalarField *> exportedSFs;
  size_t totalNumberOfExportedFields = numberOfExportedHeightStatisticsFields +
                                       numberOfExportedSfStatisticsFields;
  ...
  for (size_t k = 0; k < numberOfExportedHeightStatisticsFields +
                             maxNumberOfExportedSfStatisticsFields;
       ++k) {
    size_t statIndex = 0;
    QString sfName;
    if (k < numberOfExportedHeightStatisticsFields) {
      ...
      if (exportedStatistics[k] == PER_CELL_VALUE)
        ...
        sfName +=
            QChar(' ') + GetDefaultFieldName(exportedStatistics[statIndex]);
    } else {
      ...
      sfName = QString::fromStdString(inputCloudAsPC->getScalarFieldName(
                   static_cast<int>(sfIndex))) +
               " " + GetDefaultFieldName(exportedStatistics[statIndex]);
    }
    ...
    if (sfIndex < 0) {
      ...
      if (sfIndex < 0) {
        ccLog::Warning("[Rasterize] Couldn't allocate scalar field(s)! Try to "
                       "free some memory ...");
        delete cloudGrid;
        return nullptr;
      }
    } else {
      ccLog::Warning(QString("[Rasterize] Scalar field '%1' already exists. It "
                             "will be overwritten.")
                         .arg(sfName));
    }
  ...
  if (resampleInputCloudXY) {
    // if the cloud already has colors and we filled some empty cells, we must
    // also add (black) colors
    projectColors =
        (cloudGrid->hasColors() && (validCellCount > nonEmptyCellCount));
  } else {
  ...
  if (progressDialog) {
    ...
    progressDialog->setInfo(
        QObject::tr("Exporting %1 fields").arg(totalNumberOfExportedFields));
    progressDialog->start();
  ...
  // export points
  {
    ...
    if (progressDialog) {
      nProgress.reset(new CCCoreLib::NormalizedProgress(
          progressDialog, static_cast<unsigned>(height * width)));
    }
    ...
    for (unsigned j = 0; j < height; ++j) {
      ...
      double Px =
          box.minCorner().u[X]; // minCorner is the lower left cell CENTER

      for (unsigned i = 0; i < width; ++i, ++aCell) {
        if (std::isfinite(aCell->h)) // valid cell (could have been filled or
                                     // interpolated)
        {
            ...
            if (projectColors) {
              ccColor::Rgb col(
                  static_cast<ColorCompType>(std::min(
                      static_cast<double>(ccColor::MAX), aCell->color.x)),
                  static_cast<ColorCompType>(std::min(
                      static_cast<double>(ccColor::MAX), aCell->color.y)),
                  static_cast<ColorCompType>(std::min(
                      static_cast<double>(ccColor::MAX), aCell->color.z)));
          ...
          assert(
              ...
              nonEmptyCellIndex <
                  inputCloud->size()); // we can't be here if we have a fully
                                       // resampled cloud! (resampleInputCloudXY
                                       // implies that inputCloud is defined)
          ...
          for (size_t k = 0; k < numberOfExportedHeightStatisticsFields +
            ...
            if (k < numberOfExportedHeightStatisticsFields &&
                exportedStatistics[k] == PER_CELL_VALUE) {
              sVal = aCell->h;
            } else {
              ...
              size_t statIndex = 0;
              if (k <
                  numberOfExportedHeightStatisticsFields) // height statistics
              {
                statIndex = k;
                ...
                for (unsigned n = 0; n < aCell->nbPoints; ++n) {
                  const CCVector3 *P =
                      inputCloud->getPoint(cellPointIndexes[n]);
                  cellPointVal.push_back(P->u[Z]);
                }
              } else // SF statistics
              {
                assert(exportedStatistics.size() != 0);
                size_t indexOfSFStatsField =
                    k - numberOfExportedHeightStatisticsFields;
                size_t sfIndex =
                    indexOfSFStatsField / exportedStatistics.size();

                statIndex =
                    indexOfSFStatsField - sfIndex * exportedStatistics.size();
                ...
                CCCoreLib::ScalarField *inputScalarField =
                    inputCloudAsPC->getScalarField(static_cast<int>(sfIndex));

                // Set up vector of valid SF values for current cell
                for (unsigned n = 0; n < aCell->nbPoints; ++n) {
                  ScalarType sfValue =
                      inputScalarField->getValue(cellPointIndexes[n]);
              ...
              if (exportedStatisticsNeedSorting) {
                ...
                ParallelSort(cellPointVal.begin(), cellPointVal.end(),
                             [](ScalarType a, ScalarType b) { return a < b; });
              }
              ...
              if (cellPointVal.size()) {
                ...
                case PER_CELL_AVG_VALUE:
                  sVal = static_cast<ScalarType>(
                      std::accumulate(cellPointVal.begin(), cellPointVal.end(),
                                      0.0) /
                      cellPointVal.size());
                  break;
                case PER_CELL_MEDIAN_VALUE: {
                  ...
                  if (cellPointVal.size() % 2) // odd number
                  {
                    ...
                    sVal = static_cast<ScalarType>(
                        (cellPointVal[midIndex - 1] + cellPointVal[midIndex]) /
                        2);
                  }
                } break;
                case PER_CELL_PERCENTILE_VALUE: {
                  size_t index = static_cast<size_t>(
                      percentileValue * cellPointVal.size() / 100.0);
                  assert(index < cellPointVal.size());
                  sVal = static_cast<ScalarType>(
                      cellPointVal[std::min(index, cellPointVal.size() - 1)]);
                } break;
                case PER_CELL_UNIQUE_COUNT_VALUE: {
                  size_t count = 0;
                ...
                case PER_CELL_VALUE_STD_DEV: {
                  double cellSum = std::accumulate(cellPointVal.begin(),
                                                   cellPointVal.end(), 0.0);
                  double cellSquareSum = 0.0;
                  for (size_t n = 0; n < cellPointVal.size(); n++) {
                    cellSquareSum += cellPointVal[n] * cellPointVal[n];
                  }
                  double cellAvg = cellSum / cellPointVal.size();
                  sVal = static_cast<ScalarType>(std::sqrt(
                      std::max(0.0, cellSquareSum / cellPointVal.size() -
                                        cellAvg * cellAvg)));
                } break;
                default:
                  assert(false);
  ...
  if (!resampleInputCloudXY) {
    ...
    if (projectSFs && inputCloud && inputCloudAsPC) {
      QScopedPointer<CCCoreLib::NormalizedProgress> nProgress;
      if (progressDialog) {
        progressDialog->setInfo(QObject::tr("Projecting %1 scalar fields")
                                    .arg(scalarFields.size()));
        progressDialog->setValue(0);
        QCoreApplication::processEvents();
        nProgress.reset(new CCCoreLib::NormalizedProgress(
            progressDialog, static_cast<unsigned>(scalarFields.size())));
      }
      ...
      for (size_t k = 0; k < scalarFields.size(); ++k) {
        ...
        ccScalarField *formerSf = static_cast<ccScalarField *>(
            inputCloudAsPC->getScalarField(static_cast<int>(k)));
        assert(formerSf);

        // we try to create an equivalent SF on the output grid
        int sfIdx = cloudGrid->addScalarField(formerSf->getName());
        if (sfIdx < 0) // if we aren't lucky, the input cloud already had a SF
                       // with the same name
        {
        ...
        if (sfIdx < 0) {
          ccLog::Warning(
              "[Rasterize] Failed to allocate a new scalar field for storing "
              "SF '%s' values! Try to free some memory ...",
              formerSf->getName().c_str());
        } else {
          ccScalarField *sf =
              static_cast<ccScalarField *>(cloudGrid->getScalarField(sfIdx));
          assert(sf);
          // set sf values
          unsigned n = 0;
          ...
          for (unsigned j = 0; j < height; ++j) {
            ...
            for (unsigned i = 0; i < width; ++i, ++_sfGrid) {
              if (std::isfinite(row[i].h)) // valid cell (could have been filled
                                           // or interpolated)
              {
  ...
  } else // the cloud has already been resampled
  {
    ...
    for (int k = 0; k < static_cast<int>(cloudGrid->getNumberOfScalarFields());
         ++k) {
...
void ccRasterGrid::getInterRatioWithPolyline(ccPolyline *line) {
  if (line->size() < 3) {
    return;
  }

  ccGenericPointCloud *vertices =
      dynamic_cast<ccGenericPointCloud *>(line->getAssociatedCloud());

  // input cloud as a ccPointCloud
  ccPointCloud *pc = nullptr;
  if (vertices->isA(CC_TYPES::POINT_CLOUD)) {
    pc = static_cast<ccPointCloud *>(vertices);
  }

  bool bIn = false;
  for (int i = 0; i < pc->size(); i++) {
    if (pc->getPoint(i)->x > minCorner.x &&
        pc->getPoint(i)->x < minCorner.x + width * gridStep &&
        pc->getPoint(i)->y > minCorner.y &&
        pc->getPoint(i)->y < minCorner.y + height * gridStep) {
      bIn = true;
      break;
    }
  }
  ...
  std::vector<std::vector<InterRange>> edgeRangeVec(width);
  for (int i = 0; i < pc->size(); i++) {
    CCVector3d nextP;
    if (i != pc->size() - 1) {
      nextP = *pc->getPoint(i + 1);
    } else {
      nextP = *pc->getPoint(0);
    }

    double minX = pc->getPoint(i)->x;
    double maxX = nextP.x;
    if (minX > maxX) {
      minX = nextP.x;
      maxX = pc->getPoint(i)->x;
    }

    int w1 = std::floor((minX - minCorner.x) / gridStep);
    int w2 = std::floor((maxX - minCorner.x) / gridStep);
    assert(w1 < width && w2 < width);

    double k = (nextP.y - pc->getPoint(i)->y) / (nextP.x - pc->getPoint(i)->x);
    double b = pc->getPoint(i)->y - k * pc->getPoint(i)->x;

    for (int j = w1; j <= w2; j++) {
      double leftx = minCorner.x + gridStep * j;
      if (j == w1 && leftx < minX)
        leftx = minX;
      double rightx = minCorner.x + gridStep * (j + 1);
      if (j == w2 && rightx > maxX) {
        rightx = maxX;
      }
      int h1 = std::floor((k * leftx + b - minCorner.y) / gridStep);
      int h2 = std::floor((k * rightx + b - minCorner.y) / gridStep);

      InterRange tempRange;
      tempRange.idx = i;
      tempRange.row = j;
      if (h1 < h2) {
        tempRange.startH = h1;
      ...
      edgeRangeVec[j].push_back(tempRange);
    }
  }

  std::vector<std::vector<std::vector<int>>> edgeGroups(edgeRangeVec.size());
  for (int i = 0; i < edgeRangeVec.size(); i++) {
    auto &singleEdgeRange = edgeRangeVec[i];
    std::vector<std::vector<int>> edgeGroup;
    groupEdge(line, singleEdgeRange, edgeGroup);
    edgeGroups[i] = edgeGroup;
  }

  for (int i = 0; i < rows.size(); i++) {
    auto &curRow = rows[i];
    assert(curRow.size() == edgeGroups.size());
    for (int j = 0; j < curRow.size(); j++) {
      auto &singleEdgeRange = edgeRangeVec[j];
      auto &edgeGroup = edgeGroups[j];
      curRow[j].interAreaRatio =
          computeEdgeGridArea(j, i, line, singleEdgeRange, edgeGroup) /
          (gridStep * gridStep);
    }
  }
}

void ccRasterGrid::groupEdge(ccPolyline *line, std::vector<InterRange> &edges,
  ...
  std::vector<EdgeInfo> infoVec(edges.size());
  for (int i = 0; i < edges.size(); i++) {
    int nextIdx = edges[i].idx + 1;
    if (nextIdx >= line->size()) {
      nextIdx = 0;
    }

    EdgeInfo &info = infoVec[i];
    if (pc->getPoint(edges[i].idx)->x < pc->getPoint(nextIdx)->x) {
      info.xMin = pc->getPoint(edges[i].idx)->x;
      info.xMax = pc->getPoint(nextIdx)->x;
    } else {
      info.xMax = pc->getPoint(edges[i].idx)->x;
      info.xMin = pc->getPoint(nextIdx)->x;
    }
    if (pc->getPoint(edges[i].idx)->y < pc->getPoint(nextIdx)->y) {
      info.yMin = pc->getPoint(edges[i].idx)->y;
      info.yMax = pc->getPoint(nextIdx)->y;
    } else {
      info.yMax = pc->getPoint(edges[i].idx)->y;
      info.yMin = pc->getPoint(nextIdx)->y;
    }
    info.dir = *pc->getPoint(nextIdx) - *pc->getPoint(edges[i].idx);
    info.dir.y = 0;
    info.dir.z = 0;
  }

  std::vector<bool> markVec(edges.size(), false);
  for (int i = 0; i < edges.size(); i++) {
    if (markVec[i])
      continue;
    std::vector<int> edgeGroup;
    edgeGroup.push_back(i);
    markVec[i] = true;
    for (int j = 0; j < edges.size(); j++) {
      if (j == i || markVec[j])
        continue;
      if (infoVec[j].dir.dot(infoVec[i].dir) > 0 &&
          (infoVec[j].xMax <= infoVec[i].xMin ||
           infoVec[j].xMin >= infoVec[i].xMax)) {
        edgeGroup.push_back(j);
        markVec[j] = true;
      }
    }
  ...
  std::sort(edgeGroups.begin(), edgeGroups.end(),
            [=](std::vector<int> e1, std::vector<int> e2) {
              if (infoVec[e1[0]].yMin == infoVec[e2[0]].yMin) {
                return infoVec[e1[0]].yMax < infoVec[e2[0]].yMax;
              }
              return infoVec[e1[0]].yMin < infoVec[e2[0]].yMin;
            });
}

double ccRasterGrid::computeEdgeGridArea(
    int w, int h, ccPolyline *line, const std::vector<InterRange> &edges,
    const std::vector<std::vector<int>> &edgeGroups) {
  if (edges.empty() || edgeGroups.empty()) {
    return 0;
  }
  ...
  double leftx = minCorner.x + w * gridStep;
  double rightx = leftx + gridStep;
  double buttony = minCorner.y + h * gridStep;
  double topy = buttony + gridStep;
  double area = 0;
  for (int i = 0; i < edgeGroups.size(); i++) {
    auto &singleGroup = edgeGroups[i];
    ...
    std::vector<EdgeRegion> regionVec;

    std::vector<int> idxVec;
    for (int j = 0; j < singleGroup.size(); j++) {
      EdgeRegion region;
      InterRange curRange = edges[singleGroup[j]];
      if (h < curRange.startH || h > curRange.endH)
        continue;
      CCVector3d curP = *pc->getPoint(curRange.idx);
      CCVector3d nextP = *pc->getPoint(0);
      if (curRange.idx != pc->size() - 1) {
        nextP = *pc->getPoint(curRange.idx + 1);
      }
      if (nextP.x == curP.x)
        continue;
      double k = (nextP.y - curP.y) / (nextP.x - curP.x);
      double b = curP.y - k * curP.x;

      region.leftx = leftx;
      region.rightx = rightx;

      double lefty = k * leftx + b;
      double righty = k * rightx + b;

      double buttonx = (buttony - b) / k;
      double topx = (topy - b) / k;
      if (lefty < buttony || lefty > topy) {
        if (buttonx > leftx && buttonx < rightx) {
          if (topx > leftx && topx < rightx && topx < buttonx) {
            region.leftx = topx;
          } else {
            region.leftx = buttonx;
          }
        } else if (topx > leftx && topx < rightx) {
          region.leftx = topx;
        }
      }
      if (righty < buttony || righty > topy) {
        if (buttonx > leftx && buttonx < rightx) {
          if (topx > leftx && topx < rightx && topx > buttonx) {
            region.rightx = topx;
          } else {
            region.rightx = buttonx;
          }
        } else if (topx > leftx && topx < rightx) {
          region.rightx = topx;
        }
      ...
      if (curP.x >= leftx && curP.x <= rightx && curP.y >= buttony &&
          curP.y <= topy) {
        if (nextP.x < curP.x) {
          region.rightx = curP.x;
        } else {
          region.leftx = curP.x;
        }
      }
      if (nextP.x >= leftx && nextP.x <= rightx && nextP.y >= buttony &&
          nextP.y <= topy) {
        if (curP.x < nextP.x) {
          region.rightx = nextP.x;
        } else {
          region.leftx = nextP.x;
        }
      }

      region.lefty = k * region.leftx + b;
      region.righty = k * region.rightx + b;
      region.idx = curRange.idx;
      regionVec.push_back(region);
    }

    double curArea = 0;
    if (regionVec.empty()) {
      if (h < edges[singleGroup[0]].startH) {
        curArea = gridStep * gridStep;
      } else {
        curArea = 0;
      }
    } else {
      std::sort(
          regionVec.begin(), regionVec.end(),
          [](EdgeRegion r1, EdgeRegion r2) { return r1.leftx < r2.leftx; });

      curArea += (regionVec[0].leftx - leftx) * (regionVec[0].lefty - buttony);
      for (int k = 0; k < regionVec.size(); k++) {
        curArea +=
            (regionVec[k].rightx - regionVec[k].leftx) *
            ((regionVec[k].lefty - buttony) + (regionVec[k].righty - buttony)) /
            2;
        if (k != regionVec.size() - 1) {
          curArea += (regionVec[k + 1].leftx - regionVec[k].rightx) *
                     ((regionVec[k + 1].lefty - buttony) +
                      (regionVec[k].righty - buttony)) /
                     2;
        }
      }
      curArea += (rightx - regionVec.back().rightx) *
                 (regionVec.back().righty - buttony);
    }
    area += ((i % 2) * (1) + ((i + 1) % 2) * (-1)) * curArea;
  }

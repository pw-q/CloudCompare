...
ccViewportParameters::ccViewportParameters()
    : defaultPointSize(1), defaultLineWidth(1), perspectiveView(false),
      objectCenteredView(true), zNearCoef(0.005),
      nearClippingDepth(std::numeric_limits<double>::quiet_NaN()),
      farClippingDepth(std::numeric_limits<double>::quiet_NaN()), zNear(0),
      zFar(0), fov_deg(50.0f), cameraAspectRatio(1.0f), focalDistance(1.0),
      pivotPoint(0, 0, 0), cameraCenter(0, 0, focalDistance) {
  viewMat.toIdentity();
}

ccViewportParameters::ccViewportParameters(const ccViewportParameters &params)
    : viewMat(params.viewMat), defaultPointSize(params.defaultPointSize),
      ...
      zNearCoef(params.zNearCoef), nearClippingDepth(params.nearClippingDepth),
      farClippingDepth(params.farClippingDepth), zNear(params.zNear),
      zFar(params.zFar), fov_deg(params.fov_deg),
      cameraAspectRatio(params.cameraAspectRatio),
      focalDistance(params.focalDistance), pivotPoint(params.pivotPoint),
...
bool ccViewportParameters::fromFile(QFile &in, short dataVersion, int flags,
  ...
  if (dataVersion < 51) {
    // we read these values for backward compatibility only: we don't handle
  ...
  if (dataVersion >=
      36) // we now save the camera center and pivot point in double precision
  {
    ...
    ccSerializationHelper::CoordsFromDataStream(inStream, flags, _pivotPoint.u,
                                                3);
    pivotPoint = _pivotPoint;
    if (dataVersion >=
        25) // after version 25 the camera center is saved as a separate point!
    {
  ...
  if (dataVersion < 25) // screenPan has been replaced by cameraCenter(x,y) in
                        // object centered mode!
  {
    float screenPan[2];
  ...
  // for older version, deduce the focal distance from the old parameters
  // (pixelSize and zoom)
  if (dataVersion < 51 && zoom != 1.0f) {
    if (perspectiveView) {
      ...
      focalDistance =
          pixelSize * 2048 /
          computeDistanceToWidthRatio(); // 2048 = average screen size? Sadly we
                                         // don't have this information
    }
    setFocalDistance(focalDistance / zoom);
    ccLog::Warning("[ccViewportParameters] Approximate focal distance (sorry, "
...
short ccViewportParameters::minimumFileVersion() const {
  ...
  short minVersion =
      (std::isnan(nearClippingDepth) && std::isnan(farClippingDepth) ? 51 : 53);
...
ccGLMatrixd ccViewportParameters::computeViewMatrix() const {
  ...
  viewMatd.setTranslation(
      /*viewMatd.getTranslationAsVec3D()*/ -rotationCenter); // viewMatd.getTranslationAsVec3D()
                                                             // = (0, 0, 0)
...
ccGLMatrixd
ccViewportParameters::computeScaleMatrix(const QRect &glViewport) const {
  ccGLMatrixd scaleMatd;
  scaleMatd.toIdentity();

  // for proper aspect ratio handling
  if (glViewport.height() != 0) {
    double ar = static_cast<double>(glViewport.width() /
                                    (glViewport.height() * cameraAspectRatio));
    if (ar < 1.0) {
      // glScalef(ar, ar, 1.0);
      scaleMatd.data()[0] = ar;
      scaleMatd.data()[5] = ar;
...
void ccViewportParameters::log() const {
  ...
  ccLog::Print(
      QString("Perspective view: %1").arg(perspectiveView ? "yes" : "no"));
  ccLog::Print(QString("Object-centered view: %1")
                   .arg(objectCenteredView ? "yes" : "no"));
  ...
  ccLog::Print(QString("pivot point:(%1 ; %2; %3)")
                   .arg(pivotPoint.x)
                   .arg(pivotPoint.y)
                   .arg(pivotPoint.z));
  ccLog::Print(QString("camera center:(%1 ; %2; %3)")
                   .arg(cameraCenter.x)
                   .arg(cameraCenter.y)
                   .arg(cameraCenter.z));
}

...
ccQuadric::ccQuadric(
    CCVector2 minCorner, CCVector2 maxCorner, const PointCoordinateType eq[6],
    const CCCoreLib::SquareMatrix *toLocalOrientation /*=nullptr*/,
    const ccGLMatrix *transMat /*=nullptr*/,
    QString name /*=QString("Quadric")*/,
    unsigned precision /*=DEFAULT_DRAWING_PRECISION*/)
    : ccGenericPrimitive(name, transMat), m_minCorner(minCorner),
      m_maxCorner(maxCorner), m_minZ(0), m_maxZ(0) {
  ...
  setDrawingPrecision(std::max<unsigned>(
      precision,
      MIN_DRAWING_PRECISION)); // automatically calls updateRepresentation
}

ccQuadric::ccQuadric(QString name /*=QString("Plane")*/)
    : ccGenericPrimitive(name), m_minCorner(0, 0), m_maxCorner(0, 0), m_minZ(0),
      m_maxZ(0) {}
...
bool ccQuadric::buildUp() {
  ...
  for (unsigned x = 0; x < m_drawPrecision; ++x) {
    ...
    for (unsigned y = 0; y < m_drawPrecision; ++y) {
      ...
      P.z = m_eq[0] + m_eq[1] * P.x + m_eq[2] * P.y + m_eq[3] * P.x * P.x +
            m_eq[4] * P.x * P.y + m_eq[5] * P.y * P.y;
...
ccGenericPrimitive *ccQuadric::clone() const {
  return finishCloneJob(new ccQuadric(
      m_minCorner, m_maxCorner, m_eq,
      m_toLocalOrientation.size() != 0 ? &m_toLocalOrientation : nullptr,
      &m_transformation, getName(), m_drawPrecision));
}

ccQuadric *ccQuadric::Fit(CCCoreLib::GenericIndexedCloudPersist *cloud,
                          double *rms /*=nullptr*/) {
  // number of points
  unsigned count = cloud->size();
  if (count < CCCoreLib::CC_LOCAL_MODEL_MIN_SIZE[CCCoreLib::QUADRIC]) {
    ccLog::Warning(
        QString("[ccQuadric::fitTo] Not enough points in input cloud to fit a "
                "quadric! (%1 at the very least are required)")
            .arg(CCCoreLib::CC_LOCAL_MODEL_MIN_SIZE[CCCoreLib::QUADRIC]));
    return nullptr;
  }
  ...
  CCCoreLib::SquareMatrix globalOrientation =
      toLocalOrientation.transposed(); // transposed is equivalent to inverse
                                       // for a 3x3 rotation matrix
  ...
  for (unsigned i = 0; i < cloud->size(); ++i) {
    ...
    if (rms) {
      ...
      PointCoordinateType z = eq[0] + eq[1] * Plocal.x + eq[2] * Plocal.y +
  ...
  ccQuadric *quadric =
      new ccQuadric(minXY, maxXY, eq, &toLocalOrientation, &glMat);
...
PointCoordinateType ccQuadric::projectOnQuadric(const CCVector3 &P,
  ...
  QLocal.z = m_eq[0] + m_eq[1] * QLocal.x + m_eq[2] * QLocal.y +
             m_eq[3] * QLocal.x * QLocal.x + m_eq[4] * QLocal.x * QLocal.y +
...
ccBBox ccQuadric::getOwnFitBB(ccGLMatrix &trans) {
  ...
  return ccBBox(CCVector3(m_minCorner.x, m_minCorner.y, m_minZ),
                CCVector3(m_maxCorner.x, m_maxCorner.y, m_maxZ), true);
}

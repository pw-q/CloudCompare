...
ccGenericPrimitive::ccGenericPrimitive(
    QString name /*=QString()*/, const ccGLMatrix *transMat /*=nullptr*/,
    unsigned uniqueID /*=ccUniqueIDGenerator::InvalidUniqueID*/)
    : ccMesh(new ccPointCloud("vertices"), uniqueID), m_drawPrecision(0) {
  setName(name);
  showNormals(true);
  ...
  // we don't want the user to transform the vertices for instance (as they are
  // only temporary)
  vert->setLocked(true);
...
const ccGenericPrimitive &
ccGenericPrimitive::operator+=(const ccGenericPrimitive &prim) {
  ...
  if (verts->reserve(newVertCount) &&
    ...
    if (primHasFaceNorms) {
      ...
      if (!normsTable ||
          !normsTable->reserveSafe(triFacesNormCount + primTriNormCount)) {
        ccLog::Error("[ccGenericPrimitive::operator +] Not enough memory!");
        return *this;
      }
...
bool ccGenericPrimitive::fromFile_MeOnly(QFile &in, short dataVersion,
  ...
  if (getChildrenNumber() && getChild(0)->isKindOf(CC_TYPES::POINT_CLOUD) &&
...
short ccGenericPrimitive::minimumFileVersion_MeOnly() const {
  short minVersion =
      std::max(static_cast<short>(21), m_transformation.minimumFileVersion());
...
bool ccGenericPrimitive::init(unsigned vertCount, bool vertNormals,
                              unsigned faceCounts, unsigned faceNormCounts) {
  ccPointCloud *verts = vertices();
  assert(verts);
  if (!verts)
    return false;
...
ccGenericPrimitive::finishCloneJob(ccGenericPrimitive *primitive) const {
  if (primitive) {
    ...
    // if the calling primitive provide a null pointer, it means that the cloned

...
ccSubMesh::ccSubMesh(ccMesh *parentMesh)
    : ccGenericMesh("Sub-mesh"), m_associatedMesh(nullptr),
      m_globalIterator(0) {
  setAssociatedMesh(
...
CCCoreLib::GenericTriangle *ccSubMesh::_getNextTriangle() // temporary object
{
  return m_associatedMesh && m_globalIterator < size()
             ? m_associatedMesh->_getTriangle(m_triIndexes[m_globalIterator++])
             : nullptr;
}
...
bool ccSubMesh::interpolateNormals(unsigned triIndex, const CCVector3 &P,
                                   CCVector3 &N) {
  if (m_associatedMesh && triIndex < size())
    return m_associatedMesh->interpolateNormals(getTriGlobalIndex(triIndex), P,
                                                N);
...
bool ccSubMesh::interpolateNormalsBC(unsigned triIndex, const CCVector3d &w,
                                     CCVector3 &N) {
  if (m_associatedMesh && triIndex < size())
    return m_associatedMesh->interpolateNormalsBC(getTriGlobalIndex(triIndex),
                                                  w, N);
...
bool ccSubMesh::interpolateColors(unsigned triIndex, const CCVector3 &P,
                                  ccColor::Rgb &color) {
  if (m_associatedMesh && triIndex < size())
    return m_associatedMesh->interpolateColors(getTriGlobalIndex(triIndex), P,
                                               color);
...
bool ccSubMesh::interpolateColorsBC(unsigned triIndex, const CCVector3d &w,
                                    ccColor::Rgb &color) {
  if (m_associatedMesh && triIndex < size())
    return m_associatedMesh->interpolateColorsBC(getTriGlobalIndex(triIndex), w,
                                                 color);
...
bool ccSubMesh::getColorFromMaterial(unsigned triIndex, const CCVector3 &P,
                                     ccColor::Rgba &color,
                                     bool interpolateColorIfNoTexture) {
  if (m_associatedMesh && triIndex < size())
    return m_associatedMesh->getColorFromMaterial(
        getTriGlobalIndex(triIndex), P, color, interpolateColorIfNoTexture);
...
bool ccSubMesh::getVertexColorFromMaterial(unsigned triIndex,
  ...
  if (m_associatedMesh && triIndex < size())
    return m_associatedMesh->getVertexColorFromMaterial(
        getTriGlobalIndex(triIndex), vertIndex, color, returnColorIfNoTexture);
...
ccSubMesh::_getTriangle(unsigned triIndex) // temporary object
{
  if (m_associatedMesh && triIndex < size())
...
void ccSubMesh::getTriangleVertices(unsigned triIndex, CCVector3 &A,
...
ccSubMesh::getTriangleVertIndexes(unsigned triIndex) {
  if (m_associatedMesh && triIndex < size())
...
ccSubMesh *ccSubMesh::createNewSubMeshFromSelection(
  ...
  if (verticesVisibility.size() < vertices->size()) {
    ccLog::Warning(QString("[Sub-mesh %1] Internal error: vertex visibility "
  ...
  return createNewSubMeshFromSelection(removeSelectedTriangles,
...
ccSubMesh *ccSubMesh::createNewSubMeshFromSelection(
  ...
  if (selectedTriangleIndexes.size() < m_associatedMesh->size()) {
    ccLog::Error(QString("[Sub-mesh %1] Internal error: input triangle indexes "
  ...
  if (!newSubMesh->reserve(size())) {
    ccLog::Error(
        "[ccSubMesh::createNewSubMeshFromSelection] Not enough memory!");
    return nullptr;
  }
  ...
  // create sub-mesh
  {
    ...
    for (size_t i = 0; i < triCount; ++i) {
      ...
      // triangle is selected?
      if (selectedTriangleIndexes[globalIndex] >= 0) {
        newSubMesh->addTriangleIndex(
            static_cast<unsigned>(selectedTriangleIndexes[globalIndex]));
      } else if (removeSelectedTriangles) // triangle is not selected? It stays
...
void ccSubMesh::getTriangleTexCoordinates(unsigned triIndex, TexCoords2D *&tx1,
                                          TexCoords2D *&tx2,
...
void ccSubMesh::getTriangleNormalIndexes(unsigned triIndex, int &i1, int &i2,
                                         int &i3) const {
  if (m_associatedMesh && triIndex < size()) {
    m_associatedMesh->getTriangleNormalIndexes(getTriGlobalIndex(triIndex), i1,
                                               i2, i3);
  } else {
    i1 = i2 = i3 = -1;
  }
...
bool ccSubMesh::getTriangleNormals(unsigned triIndex, CCVector3 &Na,
...
bool ccSubMesh::toFile_MeOnly(QFile &out, short dataVersion) const {
  ...
  uint32_t meshUniqueID =
      (m_associatedMesh ? static_cast<uint32_t>(m_associatedMesh->getUniqueID())
                        : 0);
...
bool ccSubMesh::fromFile_MeOnly(QFile &in, short dataVersion, int flags,
  ...
  // at loading time (i.e. this is the responsibility of the caller to make sure
  // that all dependencies are saved together)

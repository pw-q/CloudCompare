...
static CCVector3 ComputeRobustAverageNorm(CCCoreLib::ReferenceCloud *subset,
  ...
  // now we can compute the mean normal, using the first normal as reference for
  // the sign
  ...
  for (unsigned i = 0; i < n; ++i) {
    ...
    // compute the scalar product between the ith point normal and the robust
    // one
...
float ccFastMarchingForNormsDirection::computePropagationConfidence(
  ...
  float psOri =
      std::abs(static_cast<float>(AB.dot(originCell->N))); // ideal: 90 degrees
  float psDest =
      std::abs(static_cast<float>(AB.dot(destCell->N))); // ideal: 90 degrees
...
//#define USE_BEST_NEIGHBOR_ONLY
  ...
  bestConf = inverseNormal
                 ? confNeg
                 : confPos; // DGM: absolute confidence seems to work better...
  ...
  if (minTCell->T < Cell::T_INF()) {
    ...
    if (s_cellIndex == 0) {
      ...
      for (size_t i = 0; i < m_activeCells.size(); ++i)
        static_cast<DirectionCell *>(m_theGrid[m_activeCells[i]])->scalar =
            static_cast<float>(0);
      s_cellIndex++;
    }
...
float ccFastMarchingForNormsDirection::computeTCoefApprox(
  ...
  float orientationConfidence =
      computePropagationConfidence(oCell, dCell); // between 0 and 1 (ideal: 1)
...
unsigned ccFastMarchingForNormsDirection::updateResolvedTable(
    ccGenericPointCloud *cloud, std::vector<unsigned char> &resolved,
    NormsIndexesTableType *theNorms) {
...
int ccFastMarchingForNormsDirection::OrientNormals(
    ccPointCloud *cloud, unsigned char octreeLevel,
    ccProgressDialog *progressCb) {
  if (!cloud || !cloud->normals()) {
    const QString name((cloud == nullptr) ? QStringLiteral("[unnamed]")
                                          : cloud->getName());

    ccLog::Warning(QString("[orientNormalsWithFM] Cloud '%1' is invalid (or "
  ...
  if (!cloud->getOctree()) {
    if (!cloud->computeOctree(progressCb)) {
      ccLog::Warning(
          QString(
              "[orientNormalsWithFM] Could not compute octree on cloud '%1'")
              .arg(cloud->getName()));
      return 0;
    }
  }
  ...
  if (sfIdx >= 0) {
    ...
    ccLog::Warning("[orientNormalsWithFM] Couldn't create temporary scalar "
                   "field! Not enough memory?");
    return -3;
  }

  if (!cloud->enableScalarField()) {
    ccLog::Warning("[orientNormalsWithFM] Couldn't enable temporary scalar "
                   "field! Not enough memory?");
  ...
  if (result < 0) {
    ccLog::Error(
        "[orientNormalsWithFM] Something went wrong during initialization...");
    cloud->deleteScalarField(sfIdx);
    cloud->setCurrentScalarField(oldSfIdx);
    return -6;
  }
  ...
  while (success) {
    ...
    if (propagationResult >= 0) {
      ...
      ccLog::Error(
          "An error occurred during front propagation! Process cancelled...");
      success = false;
    }
  }

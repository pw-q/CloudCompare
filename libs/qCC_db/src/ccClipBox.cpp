...
void DrawUnitArrow(bool entityPickingMode, const CCVector3 &start,
                   const CCVector3 &direction, PointCoordinateType scale,
                   const ccColor::Rgb &col, CC_DRAW_CONTEXT &context) {
  ...
  if (!c_arrowShaft)
    c_arrowShaft.reset(
        new ccCylinder(0.15f, 0.6f, nullptr, "ArrowShaft", 12,
                       0)); // we don't want to increase the unique ID counter
                            // for this 'invisible' entities
  if (!c_arrowHead)
    c_arrowHead.reset(new ccCone(0.3f, 0, 0.4f, 0, 0, nullptr, "ArrowHead", 24,
                                 0)); // we don't want to increase the unique ID
                                      // counter for this 'invisible' entities
...
static void DrawUnitTorus(bool entityPickingMode, const CCVector3 &center,
                          const CCVector3 &direction, PointCoordinateType scale,
                          const ccColor::Rgb &col, CC_DRAW_CONTEXT &context) {
  // get the set of OpenGL functions (version 2.1)
  QOpenGLFunctions_2_1 *glFunc = context.glFunctions<QOpenGLFunctions_2_1>();
  ...
  if (!c_torus)
    c_torus.reset(new ccTorus(0.2f, 0.4f, 2.0 * M_PI, false, 0, nullptr,
                              "Torus", 12,
                              0)); // we don't want to increase the unique ID
                                   // counter for this 'invisible' entities
...
static void DrawUnitCross(bool entityPickingMode, const CCVector3 &center,
                          PointCoordinateType scale, const ccColor::Rgb &col,
  ...
  scale /= 2;
  DrawUnitArrow(entityPickingMode, center, CCVector3(-1, 0, 0), scale, col,
                context);
  DrawUnitArrow(entityPickingMode, center, CCVector3(1, 0, 0), scale, col,
                context);
  DrawUnitArrow(entityPickingMode, center, CCVector3(0, -1, 0), scale, col,
                context);
  DrawUnitArrow(entityPickingMode, center, CCVector3(0, 1, 0), scale, col,
                context);
  DrawUnitArrow(entityPickingMode, center, CCVector3(0, 0, -1), scale, col,
                context);
  DrawUnitArrow(entityPickingMode, center, CCVector3(0, 0, 1), scale, col,
                context);
}
...
ccClipBox::ccClipBox(
    QString name /*= QString("clipping box")*/,
    unsigned uniqueID /*=ccUniqueIDGenerator::InvalidUniqueID*/)
    : ccHObject(name, uniqueID), m_entityContainer("entities"), m_showBox(true),
...
void ccClipBox::update() {
  ...
  for (unsigned d = 0; d < 3; ++d) {
    ...
    // positive side
    {
      ...
      // compute the 'constant' coefficient knowing that P belongs to the plane
      // if (P - (C - half_dim * N)).N = 0
    ...
    // negative side
    {
      ...
      // compute the 'constant' coefficient knowing that P belongs to the plane
      // if (P - (C + half_dim * N)).N = 0 negPlane.equation.w =
...
void ccClipBox::setActiveComponent(Components id) { m_activeComponent = id; }

static CCVector3d PointToVector(int x, int y, int screenWidth,
                                int screenHeight) {
...
void ccClipBox::setClickedPoint(int x, int y, int screenWidth, int screenHeight,
...
#pragma omp parallel for num_threads(omp_get_max_threads())
    ...
    for (int i = 0; i < count; ++i) {
      if (!shrink || visTable->at(i) == CCCoreLib::POINT_VISIBLE) {
        ...
        transMat.apply(P);
        visTable->at(i) = (m_box.contains(P) ? CCCoreLib::POINT_VISIBLE
...
void ccClipBox::drawMeOnly(CC_DRAW_CONTEXT &context) {
  ...
  // draw the interactors
  {
    ...
    componentContext.drawingFlags &=
        (~CC_ENTITY_PICKING); // we must remove the 'entity picking flag' so
                              // that the arrows don't push their own!
    ...
    DrawUnitArrow(entityPickingMode, CCVector3(minC.x, center.y, center.z),
                  CCVector3(-1.0, 0.0, 0.0), scale,
                  (entityPickingMode ? context.entityPicking.registerEntity(
                                           m_parts[X_MINUS_ARROW])
                                     : ccColor::red),
                  componentContext);
    DrawUnitArrow(entityPickingMode, CCVector3(maxC.x, center.y, center.z),
                  CCVector3(1.0, 0.0, 0.0), scale,
                  (entityPickingMode ? context.entityPicking.registerEntity(
                                           m_parts[X_PLUS_ARROW])
                                     : ccColor::red),
                  componentContext);
    DrawUnitArrow(entityPickingMode, CCVector3(center.x, minC.y, center.z),
                  CCVector3(0.0, -1.0, 0.0), scale,
                  (entityPickingMode ? context.entityPicking.registerEntity(
                                           m_parts[Y_MINUS_ARROW])
                                     : ccColor::green),
                  componentContext);
    DrawUnitArrow(entityPickingMode, CCVector3(center.x, maxC.y, center.z),
                  CCVector3(0.0, 1.0, 0.0), scale,
                  (entityPickingMode ? context.entityPicking.registerEntity(
                                           m_parts[Y_PLUS_ARROW])
                                     : ccColor::green),
                  componentContext);
    DrawUnitArrow(entityPickingMode, CCVector3(center.x, center.y, minC.z),
                  CCVector3(0.0, 0.0, -1.0), scale,
                  (entityPickingMode ? context.entityPicking.registerEntity(
                                           m_parts[Z_MINUS_ARROW])
                                     : ccColor::blue),
                  componentContext);
    DrawUnitArrow(entityPickingMode, CCVector3(center.x, center.y, maxC.z),
                  CCVector3(0.0, 0.0, 1.0), scale,
                  (entityPickingMode ? context.entityPicking.registerEntity(
                                           m_parts[Z_PLUS_ARROW])
                                     : ccColor::blue),
                  componentContext);
    DrawUnitCross(entityPickingMode,
                  minC - CCVector3(scale, scale, scale) / 2.0, scale,
                  (entityPickingMode
                       ? context.entityPicking.registerEntity(m_parts[CROSS])
                       : ccColor::yellow),
                  componentContext);
    DrawUnitTorus(entityPickingMode, CCVector3(minC.x, center.y, center.z),
                  CCVector3(-1.0, 0.0, 0.0), scale,
                  (entityPickingMode ? context.entityPicking.registerEntity(
                                           m_parts[X_MINUS_TORUS])
                                     : c_lightRed),
                  componentContext);
    DrawUnitTorus(entityPickingMode, CCVector3(center.x, minC.y, center.z),
                  CCVector3(0.0, -1.0, 0.0), scale,
                  (entityPickingMode ? context.entityPicking.registerEntity(
                                           m_parts[Y_MINUS_TORUS])
                                     : c_lightGreen),
                  componentContext);
    DrawUnitTorus(entityPickingMode, CCVector3(center.x, center.y, minC.z),
                  CCVector3(0.0, 0.0, -1.0), scale,
                  (entityPickingMode ? context.entityPicking.registerEntity(
                                           m_parts[Z_MINUS_TORUS])
                                     : c_lightBlue),
                  componentContext);
    DrawUnitTorus(entityPickingMode, CCVector3(maxC.x, center.y, center.z),
                  CCVector3(1.0, 0.0, 0.0), scale,
                  (entityPickingMode ? context.entityPicking.registerEntity(
                                           m_parts[X_PLUS_TORUS])
                                     : c_lightRed),
                  componentContext);
    DrawUnitTorus(entityPickingMode, CCVector3(center.x, maxC.y, center.z),
                  CCVector3(0.0, 1.0, 0.0), scale,
                  (entityPickingMode ? context.entityPicking.registerEntity(
                                           m_parts[Y_PLUS_TORUS])
                                     : c_lightGreen),
                  componentContext);
    DrawUnitTorus(entityPickingMode, CCVector3(center.x, center.y, maxC.z),
                  CCVector3(0.0, 0.0, 1.0), scale,
                  (entityPickingMode ? context.entityPicking.registerEntity(
                                           m_parts[Z_PLUS_TORUS])
                                     : c_lightBlue),
                  componentContext);

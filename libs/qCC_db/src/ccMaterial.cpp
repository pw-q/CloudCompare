...
ccMaterial::ccMaterial(const QString &name)
    : m_name(name), m_uniqueID(QUuid::createUuid().toString()),
      m_diffuseFront(ccColor::bright), m_diffuseBack(ccColor::bright),
      m_ambient(ccColor::night), m_specular(ccColor::night),
...
ccMaterial::ccMaterial(const ccMaterial &mtl)
    : m_name(mtl.m_name), m_textureFilename(mtl.m_textureFilename),
      m_uniqueID(mtl.m_uniqueID), m_diffuseFront(mtl.m_diffuseFront),
      m_diffuseBack(mtl.m_diffuseBack), m_ambient(mtl.m_ambient),
      m_specular(mtl.m_specular), m_emission(mtl.m_emission),
...
ccMaterial::~ccMaterial() { releaseTexture(); }
...
void ccMaterial::applyGL(const QOpenGLContext *context, bool lightEnabled,
  ...
  if (lightEnabled) {
    ...
    glFunc->glMaterialf(GL_FRONT, GL_SHININESS,
                        std::max(0.0f, std::min(m_shininessFront, 128.0f)));
    glFunc->glMaterialf(GL_BACK, GL_SHININESS,
                        std::max(0.0f, std::min(m_shininessBack, 128.0f)));
  } else {
...
bool ccMaterial::loadAndSetTexture(const QString &absoluteFilename) {
  if (absoluteFilename.isEmpty()) {
    ccLog::Warning(
        QString("[ccMaterial::loadAndSetTexture] filename can't be empty!"));
    return false;
  }
  ccLog::PrintDebug(
      QString("[ccMaterial::loadAndSetTexture] absolute filename = %1")
          .arg(absoluteFilename));

  if (s_materialDB.hasTexture(absoluteFilename)) {
    // if the image is already in memory, we simply update the texture filename
    // for this material
    ...
    if (image.isNull()) {
      ccLog::Warning(
          QString("[ccMaterial::loadAndSetTexture] Failed to load image '%1'")
              .arg(absoluteFilename));
      return false;
    } else {
...
void ccMaterial::setTexture(QImage image,
  ...
  ccLog::PrintDebug(
      QString(
          "[ccMaterial::setTexture] absoluteFilename = '%1' / size = %2 x %3")
          .arg(absoluteFilename)
          .arg(image.width())
          .arg(image.height()));
  ...
  if (absoluteFilename.isEmpty()) {
    ...
    if (s_materialDB.hasTexture(absoluteFilename)) {
      ...
      if (s_materialDB.getTexture(absoluteFilename).size() != image.size()) {
        ...
        ccLog::Warning(
            QString("[ccMaterial] A texture with the same name (%1) but with a "
                    "different size has already been loaded!")
                .arg(absoluteFilename));
      }
      m_textureFilename = absoluteFilename;
      s_materialDB.increaseTextureCounter(absoluteFilename);
  ...
  s_materialDB.addTexture(m_textureFilename,
                          mirrorImage ? image.mirrored() : image);
}
...
bool ccMaterial::hasTexture() const {
  return m_textureFilename.isEmpty()
             ? false
             : s_materialDB.hasTexture(m_textureFilename) &&
                   !s_materialDB.getTexture(m_textureFilename).isNull();
}
  ...
  for (int i = 0; i < maxLightCount; ++i) {
    if (glFunc->glIsEnabled(GL_LIGHT0 + i)) {
      ...
      diffuse[0] = diffuse[1] = diffuse[2] =
          (diffuse[0] + diffuse[1] + diffuse[2]) / 3; //'mean' (gray) value
      ambiant[0] = ambiant[1] = ambiant[2] =
          (ambiant[0] + ambiant[1] + ambiant[2]) / 3; //'mean' (gray) value
      specular[0] = specular[1] = specular[2] =
          (specular[0] + specular[1] + specular[2]) / 3; //'mean' (gray) value
...
void ccMaterial::setTextureMinMagFilters(
  ...
  if (minificationFilter != m_texMinificationFilter ||
    ...
    if (!m_textureFilename.isEmpty() &&
        s_materialDB.openGLTextures.contains(m_textureFilename)) {
      // remove the existing texture (if any) so that it's initialized again
      // next time

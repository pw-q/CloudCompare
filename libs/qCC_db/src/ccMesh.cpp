...
ccMesh::ccMesh(ccGenericPointCloud *vertices,
    ...
    : ccGenericMesh("Mesh", uniqueID), m_associatedCloud(nullptr),
      m_triNormals(nullptr), m_texCoords(nullptr), m_materials(nullptr),
      m_triVertIndexes(nullptr), m_globalIterator(0), m_triMtlIndexes(nullptr),
      m_texCoordIndexes(nullptr), m_triNormalIndexes(nullptr) {
...
ccMesh::ccMesh(CCCoreLib::GenericIndexedMesh *giMesh,
               ccGenericPointCloud *giVertices)
    : ccGenericMesh("Mesh"), m_associatedCloud(nullptr), m_triNormals(nullptr),
      m_texCoords(nullptr), m_materials(nullptr), m_triVertIndexes(nullptr),
      m_globalIterator(0), m_triMtlIndexes(nullptr), m_texCoordIndexes(nullptr),
...
void ccMesh::setAssociatedCloud(ccGenericPointCloud *cloud) {
  ...
  if (m_associatedCloud)
    m_associatedCloud->addDependency(this, DP_NOTIFY_OTHER_ON_DELETE |
...
bool ccMesh::computePerVertexNormals() {
  if (!m_associatedCloud ||
      !m_associatedCloud->isA(CC_TYPES::POINT_CLOUD)) // TODO
  {
    ccLog::Warning("[ccMesh::computePerVertexNormals] Vertex set is not a "
                   "standard cloud?!");
    return false;
  }
  ...
  if (/*!normalsWereAllocated && */ !cloud
          ->resizeTheNormsTable()) // we call it whatever the case (just to be
                                   // sure)
  {
...
bool ccMesh::computePerTriangleNormals() {
  ...
  if (!normIndexes ||
      normIndexes->size() <
          triCount) // warning: the previous table size may not be big enough!
  {
  ...
  for (unsigned i = 0; i < triCount; ++i) {
    setTriangleNormalIndexes(i, static_cast<int>(i), static_cast<int>(i),
                             static_cast<int>(i));
  }
...
bool ccMesh::processScalarField(MESH_SCALAR_FIELD_PROCESS process) {
  ...
  unsigned nTri = size();
  {
    ...
    for (unsigned i = 0; i < nTri; ++i) {
      const CCCoreLib::VerticesIndexes *tsi =
          getNextTriangleVertIndexes(); // DGM: getNextTriangleVertIndexes is
...
void ccMesh::setTriNormsTable(NormsIndexesTableType *triNormsTable,
  ...
  if (m_triNormals) {
    ...
    removePerTriangleNormalIndexes(); // auto-remove per-triangle indexes (we
...
void ccMesh::setMaterialSet(ccMaterialSet *materialSet,
  ...
  if (m_materials) {
    ...
    removePerTriangleMtlIndexes(); // auto-remove per-triangle indexes (we don't
                                   // need them anymore)
  }
...
void ccMesh::transformTriNormals(const ccGLMatrix &trans) {
  ...
  if (m_triNormals &&
      (!getParent() || !getParent()->isKindOf(CC_TYPES::MESH))) {
...
bool ccMesh::laplacianSmooth(unsigned nbIteration, PointCoordinateType factor,
  ...
  for (unsigned iter = 0; iter < nbIteration; iter++) {
    ...
    for (unsigned i = 0; i < vertCount; i++) {
      if (edgesCount[i]) {
        // this is a "persistent" pointer and we know what type of cloud is
        // behind ;)
...
ccMesh *ccMesh::cloneMesh(ccGenericPointCloud *vertices /*=nullptr*/,
  ...
  if (!newVertices) {
    ...
    if (realVertCount == vertNum) {
      ...
      if (rc.reserve(realVertCount)) {
        ...
        newVertices =
            static_cast<ccPointCloud *>(m_associatedCloud)->partialClone(&rc);
  ...
  if (m_triNormals && m_triNormalIndexes) {
    ...
    if (cloneMesh->reservePerTriangleNormalIndexes()) {
      ...
      if (!clonedNormsTable) {
        clonedNormsTable =
            m_triNormals->clone(); // TODO: keep only what's necessary!
        if (clonedNormsTable)
          cloneMesh->addChild(clonedNormsTable);
        else {
          ccLog::Warning("[ccMesh::clone] Not enough memory: failed to clone "
                         "per-triangle normals!");
          cloneMesh
              ->removePerTriangleNormalIndexes(); // don't need this anymore!
        }
      }

      // if we have both the main array and per-triangle normals indexes, we can
      // finish the job
      cloneMesh->setTriNormsTable(clonedNormsTable);
      assert(cloneMesh->m_triNormalIndexes);
      m_triNormalIndexes->copy(
          *cloneMesh->m_triNormalIndexes); // should be ok as array is already
                                           // reserved!
    } else {
      ccLog::Warning("[ccMesh::clone] Not enough memory: failed to clone "
  ...
  if (m_materials && m_triMtlIndexes) {
    ...
    if (cloneMesh->reservePerTriangleMtlIndexes()) {
      ...
      if (!clonedMaterials) {
        clonedMaterials =
            getMaterialSet()->clone(); // TODO: keep only what's necessary!
        if (clonedMaterials) {
      ...
      // if we have both the main array and per-triangle materials indexes, we
      // can finish the job
      if (clonedMaterials) {
        cloneMesh->setMaterialSet(clonedMaterials);
        assert(cloneMesh->m_triMtlIndexes);
        m_triMtlIndexes->copy(
            *cloneMesh->m_triMtlIndexes); // should be ok as array is already
                                          // reserved!
      }
    } else {
      ccLog::Warning("[ccMesh::clone] Not enough memory: failed to clone "
  ...
  if (m_texCoords && m_texCoordIndexes) {
    ...
    if (cloneMesh->reservePerTriangleTexCoordIndexes()) {
      ...
      if (!cloneTexCoords) {
        cloneTexCoords =
            m_texCoords->clone(); // TODO: keep only what's necessary!
        if (!cloneTexCoords) {
          ccLog::Warning("[ccMesh::clone] Not enough memory: failed to clone "
                         "texture coordinates!");
          cloneMesh
              ->removePerTriangleTexCoordIndexes(); // don't need this anymore!
        }
      }

      // if we have both the main array and per-triangle texture info, we can
      // finish the job
      if (cloneTexCoords) {
        cloneMesh->setTexCoordinatesTable(cloneTexCoords);
        assert(cloneMesh->m_texCoordIndexes);
        m_texCoordIndexes->copy(
            *cloneMesh->m_texCoordIndexes); // should be ok as array is already
                                            // reserved!
      }
    } else {
      ccLog::Warning("[ccMesh::clone] Not enough memory: failed to clone "
...
ccMesh *ccMesh::TriangulateTwoPolylines(ccPolyline *p1, ccPolyline *p2,
  ...
  if (projectionDir) {
    ...
    if (!N.getLSPlane()) {
      ccLog::Warning("[ccMesh::TriangulateTwoPolylines] Failed to fit a plane "
  ...
  if (!delaunayMesh->buildMesh(points2D, segments2D, errorStr)) {
    ccLog::Warning(QStringLiteral("Third party library error: %1")
  ...
  // remove the points outside of the 'concave' hull
  {
    ...
    try {
      ...
      if (CCCoreLib::PointProjectionTools::extractConvexHull2D(indexedPoints2D,
                                                               hullPoints)) {
        std::list<CCCoreLib::PointProjectionTools::IndexedCCVector2 *>::iterator
            A = hullPoints.begin();
        for (; A != hullPoints.end(); ++A) {
          // current hull segment
          std::list<
              CCCoreLib::PointProjectionTools::IndexedCCVector2 *>::iterator B =
              A;
          ++B;
          if (B == hullPoints.end()) {
            B = hullPoints.begin();
          ...
          if (Apoly == Bpoly) {
            ...
            if (abs(static_cast<int>(Bindex) - static_cast<int>(Aindex)) > 1) {
              ...
              delaunayMesh->removeOuterTriangles(points2D, contour,
                                                 /*remove inside = */ false);
            }
          }
        }
      } else {
        ccLog::Warning("[ccMesh::TriangulateTwoPolylines] Failed to compute "
                       "the convex hull (can't clean the mesh borders)");
      }
    } catch (const std::bad_alloc &) {
      ccLog::Warning("[ccMesh::TriangulateTwoPolylines] Not enough memory to "
...
ccMesh *ccMesh::Triangulate(ccGenericPointCloud *cloud,
  ...
  // compute raw mesh
  std::string errorStr;
  CCCoreLib::GenericIndexedMesh *dummyMesh =
      CCCoreLib::PointProjectionTools::computeTriangulation(
          cloud, type, maxEdgeLength, dim, errorStr);
  if (!dummyMesh) {
    ccLog::Warning(QStringLiteral("[ccMesh::Triangulate] Failed to construct "
  ...
  if (!mesh) {
    ccLog::Warning("[ccMesh::Triangulate] An error occurred while computing "
                   "mesh! (not enough memory?)");
    return nullptr;
  }
...
bool ccMesh::merge(const ccMesh *mesh, bool createSubMesh) {
    ...
    if (hasTriangleNormals || otherMeshHasTriangleNormals) {
      ...
      if (otherMeshHasTriangleNormals) {
        ...
        if (m_triNormals != mesh->m_triNormals) {
          ...
          if (!m_triNormals->reserveSafe(triNormalsCountBefore +
        ...
        // copy the values
        {
          for (unsigned i = 0; i < mesh->m_triNormalIndexes->size(); ++i) {
            ...
            Tuple3i newIndexes(
                indexes.u[0] < 0
                    ? -1
                    : indexes.u[0] + static_cast<int>(triIndexShift),
                indexes.u[1] < 0
                    ? -1
                    : indexes.u[1] + static_cast<int>(triIndexShift),
                indexes.u[2] < 0
                    ? -1
                    : indexes.u[2] + static_cast<int>(triIndexShift));
      ...
      if (otherMeshHasMaterials) {
        ...
        if (m_materials != mesh->m_materials) {
          ...
          for (size_t m = 0; m != otherMatSetSize; ++m) {
            materialIndexMap[m] =
                m_materials->addMaterial(mesh->m_materials->at(m));
          }
        }
        ...
        // copy the values
        {
          for (unsigned i = 0; i < mesh->m_texCoordIndexes->size(); ++i) {
            ...
            Tuple3i newIndexes(
                indexes.u[0] < 0
                    ? -1
                    : indexes.u[0] + static_cast<int>(texCoordIndexShift),
                indexes.u[1] < 0
                    ? -1
                    : indexes.u[1] + static_cast<int>(texCoordIndexShift),
                indexes.u[2] < 0
                    ? -1
                    : indexes.u[2] + static_cast<int>(texCoordIndexShift));
  ...
  if (createSubMesh) {
    ...
    if (subMesh->reserve(triNumAfter - triNumBefore)) {
      ...
      ccLog::Warning(QString("[Merge] Not enough memory to create the sub-mesh "
                             "corresponding to mesh '%1'!")
                         .arg(mesh->getName()));
      delete subMesh;
      subMesh = nullptr;
    }
...
CCCoreLib::GenericTriangle *
ccMesh::_getTriangle(unsigned triangleIndex) // temporary
{
  assert(triangleIndex < m_triVertIndexes->size());

  const CCCoreLib::VerticesIndexes &tri =
...
void ccMesh::getTriangleVertices(unsigned triangleIndex, CCVector3 &A,
                                 CCVector3 &B, CCVector3 &C) const {
  assert(triangleIndex < m_triVertIndexes->size());
...
CCCoreLib::VerticesIndexes *
ccMesh::getTriangleVertIndexes(unsigned triangleIndex) {
  return &m_triVertIndexes->at(triangleIndex);
}

const CCCoreLib::VerticesIndexes *
ccMesh::getTriangleVertIndexes(unsigned triangleIndex) const {
  return &m_triVertIndexes->at(triangleIndex);
}
...
void ccMesh::drawMeOnly(CC_DRAW_CONTEXT &context) {
  ...
  if (MACRO_Draw3D(context)) {
    ...
    // L.O.D.
    bool lodEnabled =
        (triNum > context.minLODTriangleCount && context.decimateMeshOnMove &&
         MACRO_LODActivated(context));
    unsigned decimStep =
        (lodEnabled
             ? static_cast<unsigned>(ceil(static_cast<double>(triNum * 3) /
    ...
    // fix 'showNorms'
    glParams.showNorms = showTriNormals || (m_associatedCloud->hasNormals() &&
                                            m_normalsDisplayed);
    ...
    if (glParams.showSF) {
      ...
      sfMayHaveHiddenValues =
          currentDisplayedScalarField
              ? currentDisplayedScalarField->mayHaveHiddenValues()
              : false;

      if (!currentDisplayedScalarField ||
          (entityPickingMode &&
           !sfMayHaveHiddenValues)) // in picking mode, no need to take SF into
                                    // account if we don't hide any points!
      {
    ...
    if (glParams.showColors) {
      if (isColorOverridden()) {
        ...
        assert(m_associatedCloud->isA(CC_TYPES::POINT_CLOUD));
        rgbaColorsTable =
            static_cast<ccPointCloud *>(m_associatedCloud)->rgbaColors();
      }
    ...
    if (glParams.showNorms) {
      assert(m_associatedCloud->isA(CC_TYPES::POINT_CLOUD));
      normalsIndexesTable =
          static_cast<ccPointCloud *>(m_associatedCloud)->normals();
    ...
    if (!visFiltering && !(applyMaterials || showTextures) &&
      ...
      for (size_t k = 0; k < chunkCount; ++k) {
        ...
        const CCCoreLib::VerticesIndexes *_vertIndexesChunkOrigin =
            ccChunk::Start(*m_triVertIndexes, k);

        // vertices
        {
          const CCCoreLib::VerticesIndexes *_vertIndexes =
              _vertIndexesChunkOrigin;
          CCVector3 *_vertices = GetVertexBuffer();
          for (size_t n = 0; n < chunkSize;
               n += decimStep, _vertIndexes += decimStep) {
        ...
        if (glParams.showSF) {
          const CCCoreLib::VerticesIndexes *_vertIndexes =
              _vertIndexesChunkOrigin;
          ccColor::Rgb *_rgbColors =
              reinterpret_cast<ccColor::Rgb *>(GetColorsBuffer());
          assert(colorScale);
          ...
          for (size_t n = 0; n < chunkSize;
            ...
            *_rgbColors++ =
                *currentDisplayedScalarField->getValueColor(_vertIndexes->i1);
            *_rgbColors++ =
                *currentDisplayedScalarField->getValueColor(_vertIndexes->i2);
            *_rgbColors++ =
                *currentDisplayedScalarField->getValueColor(_vertIndexes->i3);
          }
        }
        // colors
        else if (glParams.showColors) {
          const CCCoreLib::VerticesIndexes *_vertIndexes =
              _vertIndexesChunkOrigin;
          ccColor::Rgba *_rgbaColors =
              reinterpret_cast<ccColor::Rgba *>(GetColorsBuffer());
          for (size_t n = 0; n < chunkSize;
               n += decimStep, _vertIndexes += decimStep) {
        ...
        if (glParams.showNorms) {
          ...
          if (showTriNormals) {
            ...
            const Tuple3i *_triNormalIndexes =
                ccChunk::Start(*m_triNormalIndexes, k);
            for (size_t n = 0; n < chunkSize;
                 n += decimStep, _triNormalIndexes += decimStep) {
              assert(_triNormalIndexes->u[0] <
                     static_cast<int>(m_triNormals->size()));
              assert(_triNormalIndexes->u[1] <
                     static_cast<int>(m_triNormals->size()));
              assert(_triNormalIndexes->u[2] <
                     static_cast<int>(m_triNormals->size()));

              *_normals++ =
                  (_triNormalIndexes->u[0] >= 0
                       ? compressedNormals->getNormal(
                             m_triNormals->at(_triNormalIndexes->u[0]))
                       : s_blankNorm);
              *_normals++ =
                  (_triNormalIndexes->u[1] >= 0
                       ? compressedNormals->getNormal(
                             m_triNormals->at(_triNormalIndexes->u[1]))
                       : s_blankNorm);
              *_normals++ =
                  (_triNormalIndexes->u[2] >= 0
                       ? compressedNormals->getNormal(
                             m_triNormals->at(_triNormalIndexes->u[2]))
                       : s_blankNorm);
            }
          } else {
            const CCCoreLib::VerticesIndexes *_vertIndexes =
            ...
            for (size_t n = 0; n < chunkSize;
              ...
              *_normals++ = compressedNormals->getNormal(
                  normalsIndexesTable->at(_vertIndexes->i1));
              *_normals++ = compressedNormals->getNormal(
                  normalsIndexesTable->at(_vertIndexes->i2));
              *_normals++ = compressedNormals->getNormal(
                  normalsIndexesTable->at(_vertIndexes->i3));
            }
          }
        }

        if (!showWired) {
          glFunc->glDrawArrays(lodEnabled ? GL_POINTS : GL_TRIANGLES, 0,
                               (static_cast<int>(chunkSize) / decimStep) * 3);
        } else {
          glFunc->glDrawElements(GL_LINES,
                                 (static_cast<int>(chunkSize) / decimStep) * 6,
                                 GL_UNSIGNED_INT, GetWireVertexIndexes());
        }
      }
      ...
      GLenum triangleDisplayType =
          lodEnabled ? GL_POINTS : showWired ? GL_LINE_LOOP : GL_TRIANGLES;
      ...
      for (size_t n = 0; n < triNum; ++n) {
        ...
        if (glParams.showSF) {
          ...
          if (entityPickingMode) {
            // in picking mode, we don't want to apply the colors, just filter
            // the invisible triangles
            rgb1 = nullptr;
            rgb2 = nullptr;
            rgb3 = nullptr;
          }
        ...
        if (glParams.showNorms) {
          if (showTriNormals) {
            ...
            N1 = (idx.u[0] >= 0 ? ccNormalVectors::GetNormal(
                                      m_triNormals->getValue(idx.u[0]))
                                      .u
                                : nullptr);
            N2 = (idx.u[0] == idx.u[1]
                      ? N1
                      : idx.u[1] >= 0 ? ccNormalVectors::GetNormal(
                                            m_triNormals->getValue(idx.u[1]))
                                            .u
                                      : nullptr);
            N3 = (idx.u[0] == idx.u[2]
                      ? N1
                      : idx.u[2] >= 0 ? ccNormalVectors::GetNormal(
                                            m_triNormals->getValue(idx.u[2]))
                                            .u
                                      : nullptr);
          } else {
            N1 = compressedNormals
                     ->getNormal(normalsIndexesTable->getValue(tsi.i1))
                     .u;
            N2 = compressedNormals
                     ->getNormal(normalsIndexesTable->getValue(tsi.i2))
                     .u;
            N3 = compressedNormals
                     ->getNormal(normalsIndexesTable->getValue(tsi.i3))
                     .u;
          }
        }
        ...
        if (applyMaterials || showTextures) {
          ...
          if (lasMtlIndex != newMatlIndex) {
            ...
            if (newMatlIndex >= 0)
              (*m_materials)[newMatlIndex]->applyGL(context.qGLContext,
                                                    glParams.showNorms, false);
            else
              context.defaultMat->applyGL(context.qGLContext,
                                          glParams.showNorms, false);
          ...
          if (showTextures) {
            ...
            Tx1 = (txInd.u[0] >= 0 ? &m_texCoords->getValue(txInd.u[0])
                                   : nullptr);
            Tx2 = (txInd.u[1] >= 0 ? &m_texCoords->getValue(txInd.u[1])
                                   : nullptr);
            Tx3 = (txInd.u[2] >= 0 ? &m_texCoords->getValue(txInd.u[2])
                                   : nullptr);
          }
        }
...
ccMesh *ccMesh::createNewMeshFromSelection(
  ...
  ccMesh *newMesh = nullptr;
  {
    ...
    if (!newVertices) {
      ccLog::Warning("[ccMesh::createNewMeshFromSelection] Failed to create "
      ...
      ccLog::Warning(
          "[ccMesh::createNewMeshFromSelection] No visible point in selection");
      delete newVertices;
      return nullptr;
    }
    assert(newVertices);
    ...
    assert(rc.size() !=
           m_associatedCloud
               ->size()); // in this case createNewCloudFromVisibilitySelection
                          // would have return 'm_associatedCloud' itself

    CCCoreLib::GenericIndexedMesh *selection =
        CCCoreLib::ManualSegmentationTools::segmentMesh(
            this, &rc, true, nullptr, newVertices, 0, &triangleIndexMap);
    if (!selection) {
      ccLog::Warning("[ccMesh::createNewMeshFromSelection] Process failed: not "
                     "enough memory?");
      return nullptr;
    }
    ...
    if (!newMesh) {
      delete newVertices;
      newVertices = nullptr;
      ccLog::Warning("[ccMesh::createNewMeshFromSelection] An error occurred: "
                     "not enough memory?");
      return nullptr;
    }
    ...
    if (addFeatures) {
      ...
      if (m_triNormals && m_triNormalIndexes) {
        ...
        try {
          ...
          ccLog::Warning("[ccMesh::createNewMeshFromSelection] Failed to "
      ...
      if (m_texCoords && m_texCoordIndexes) {
        ...
        try {
          ...
          ccLog::Warning(
              "[ccMesh::createNewMeshFromSelection] Failed to create new "
              "texture indexes subset! (not enough memory)");
      ...
      if (m_materials && m_triMtlIndexes) {
        ...
        // create new 'minimal' subset
        newMaterials =
            new ccMaterialSet(m_materials->getName() + QString(".subset"));
        newMaterials->link();
        try {
          ...
          if (newTriTexIndexes) // we can release texture coordinates as well
                                // (as they depend on materials!)
          {
            newMesh->removePerTriangleTexCoordIndexes();
            newTriTexIndexes->release();
            newTriTexIndexes = nullptr;
      ...
      for (size_t i = 0; i < triCount; ++i) {
        if (triangleIndexMap[i] >= 0) // triangle should be copied over
        {
          ...
          if (newTriNormals) {
            ...
            // for each triangle of this mesh, try to determine if its normals
            // are already in use (otherwise add them to the new container and
            //increase its index)
            for (unsigned j = 0; j < 3; ++j) {
              if (triNormIndexes.u[j] >= 0 &&
                  newNormIndexes[triNormIndexes.u[j]] < 0) {
                if (newTriNormals->size() == newTriNormals->capacity() &&
                    !newTriNormals->reserveSafe(newTriNormals->size() +
                                                4096)) // auto expand
                {
                  ccLog::Warning(
                      "[ccMesh::createNewMeshFromSelection] Failed to create "
                      "new normals subset! (not enough memory)");
                  newMesh->removePerTriangleNormalIndexes();
                  newTriNormals->release();
                  newTriNormals = nullptr;
                  break;
                ...
                newNormIndexes[triNormIndexes.u[j]] = static_cast<int>(
                    newTriNormals->size()); // new element index = new size - 1
                                            // = old size!
                newTriNormals->emplace_back(
                    m_triNormals->getValue(triNormIndexes.u[j]));
              }
            }
            ...
            if (newTriNormals) // structure still exists?
            {
              newMesh->addTriangleNormalIndexes(
                  triNormIndexes.u[0] < 0 ? -1
                                          : newNormIndexes[triNormIndexes.u[0]],
                  triNormIndexes.u[1] < 0 ? -1
                                          : newNormIndexes[triNormIndexes.u[1]],
                  triNormIndexes.u[2] < 0
                      ? -1
                      : newNormIndexes[triNormIndexes.u[2]]);
            }
          }
          ...
          if (newTriTexIndexes) {
            ...
            // for each triangle of this mesh, try to determine if its textures
            // coordinates are already in use (otherwise add them to the new
            //container and increase its index)
            for (unsigned j = 0; j < 3; ++j) {
              if (triTexIndexes.u[j] >= 0 &&
                  newTexIndexes[triTexIndexes.u[j]] < 0) {
                if (newTriTexIndexes->size() == newTriTexIndexes->capacity() &&
                    !newTriTexIndexes->reserveSafe(newTriTexIndexes->size() +
                                                   4096)) // auto expand
                {
                  ccLog::Warning("Failed to create new texture coordinates "
                ...
                // import old texture coordinate to new subset (create new
                // index)
                newTexIndexes[triTexIndexes.u[j]] = static_cast<int>(
                    newTriTexIndexes->size()); // new element index = new size -
                                               // 1 = old size!
                newTriTexIndexes->emplace_back(
                    m_texCoords->getValue(triTexIndexes.u[j]));
              }
            }
            ...
            if (newTriTexIndexes) // structure still exists?
            {
              newMesh->addTriangleTexCoordIndexes(
                  triTexIndexes.u[0] < 0 ? -1
                                         : newTexIndexes[triTexIndexes.u[0]],
                  triTexIndexes.u[1] < 0 ? -1
                                         : newTexIndexes[triTexIndexes.u[1]],
                  triTexIndexes.u[2] < 0 ? -1
                                         : newTexIndexes[triTexIndexes.u[2]]);
            }
          }
          ...
          if (newMaterials) {
            ...
            // for each triangle of this mesh, try to determine if its material
            // is already in use (otherwise add it to the new container and
            //increase its index)
            if (triMatIndex >= 0 && newMatIndexes[triMatIndex] < 0) {
              ...
              newMatIndexes[triMatIndex] = static_cast<int>(
                  newMaterials
                      ->size()); // new element index = new size - 1 = old size!
              try {
                newMaterials->emplace_back(m_materials->at(triMatIndex));
              } catch (const std::bad_alloc &) {
                ccLog::Warning(
                    "[ccMesh::createNewMeshFromSelection] Failed to create new "
                    "materials subset! (not enough memory)");
                newMesh->removePerTriangleMtlIndexes();
                newMaterials->release();
                newMaterials = nullptr;
              }
            ...
            if (newMaterials) // structure still exists?
            {
              newMesh->addTriangleMtlIndex(
                  triMatIndex < 0 ? -1 : newMatIndexes[triMatIndex]);
            }
          }
        }
      }
      ...
      if (newTriNormals) {
        newTriNormals->resize(
            newTriNormals->size()); // smaller so it should always be ok!
  ...
  if (filterChildren(subMeshes, false, CC_TYPES::SUB_MESH) != 0) {
    ...
    try {
      ...
      if (removeSelectedTriangles) {
        newRemainingTriangleIndexes.resize(triCount,
                                           static_cast<unsigned>(triCount));

        unsigned newInvisibleIndex = 0;
        for (size_t i = 0; i < triCount; ++i) {
          if (triangleIndexMap[i] < 0) // triangle is not used in the new mesh,
                                       // it will be kept in this one
          {
  ...
  if (removeSelectedTriangles) {
    if (newIndexesOfRemainingTriangles) {
      if (newIndexesOfRemainingTriangles->empty()) {
        try {
          ...
          ccLog::Warning(
              "[ccMesh::createNewMeshFromSelection] Not enough memory");
          return nullptr;
        }
      } else if (newIndexesOfRemainingTriangles->size() != triCount) {
        ccLog::Warning(
            "[ccMesh::createNewMeshFromSelection] Input 'new indexes of "
            "reamining triangles' vector has a wrong size");
        return nullptr;
      }
    }
    ...
    // we need to change the visibility status of some vertices that belong to
    // partially 'invisible' triangles
    auto &visArray = m_associatedCloud->getTheVisibilityArray();
    assert(visArray.size() == m_associatedCloud->size());

    size_t lastTri = 0;
    for (size_t i = 0; i < triCount; ++i) {
      if (triangleIndexMap[i] < 0) // triangle is not used in the new mesh, it
                                   // will be kept in this one
      {
    ...
    if (m_associatedCloud->removeVisiblePoints(nullptr, &newIndexes)) {
      ...
      ccLog::Warning("[ccMesh::createNewMeshFromSelection] Failed to remove "
                     "unused vertices");
    }
    ...
    ccLog::Warning(
        "[ccMesh::createNewMeshFromSelection] A 'new indexes of reamining "
        "triangles' vector was provided while no triangle shall be removed");
  }
...
void ccMesh::setTriangleNormalIndexes(unsigned triangleIndex, int i1, int i2,
                                      int i3) {
...
void ccMesh::getTriangleNormalIndexes(unsigned triangleIndex, int &i1, int &i2,
                                      int &i3) const {
...
bool ccMesh::getTriangleNormals(unsigned triangleIndex, CCVector3 &Na,
                                CCVector3 &Nb, CCVector3 &Nc) const {
  if (m_triNormals && m_triNormalIndexes &&
...
void ccMesh::setTexCoordinatesTable(TextureCoordsContainer *texCoordsTable,
  ...
  if (m_texCoords) {
    ...
    removePerTriangleTexCoordIndexes(); // auto-remove per-triangle indexes (we
                                        // don't need them anymore)
  }
}

void ccMesh::getTriangleTexCoordinates(unsigned triIndex, TexCoords2D *&tx1,
                                       TexCoords2D *&tx2,
...
void ccMesh::setTriangleTexCoordIndexes(unsigned triangleIndex, int i1, int i2,
                                        int i3) {
  assert(m_texCoordIndexes && m_texCoordIndexes->size() > triangleIndex);
  m_texCoordIndexes->setValue(triangleIndex, Tuple3i(i1, i2, i3));
}

void ccMesh::getTriangleTexCoordinatesIndexes(unsigned triangleIndex, int &i1,
                                              int &i2, int &i3) const {
  assert(m_texCoordIndexes && m_texCoordIndexes->size() > triangleIndex);
...
bool ccMesh::toFile_MeOnly(QFile &out, short dataVersion) const {
  ...
  // per-triangle normals array (dataVersion>=20)
  {
    ...
    // meshes) so instead we save it's unique ID (dataVersion>=20) WARNING: the
    // normals array must be saved in the same BIN file! (responsibility of the
    // caller)
  ...
  {
    // we can't save the texture coordinates array here (as it may be shared by
    // multiple meshes) so instead we save it's unique ID (dataVersion>=20)
    // WARNING: the texture coordinates array must be saved in the same BIN
    // file! (responsibility of the caller)
  ...
  // materials
  {
    ...
    // meshes) so instead we save it's unique ID (dataVersion>=20) WARNING: the
    // material set must be saved in the same BIN file! (responsibility of the
    // caller)
    uint32_t matSetID =
        (m_materials ? static_cast<uint32_t>(m_materials->getUniqueID()) : 0);
  ...
  if (!m_triVertIndexes)
    return ccLog::Warning(
        "Internal error: mesh has no triangles array! (not enough memory?)");
  if (!ccSerializationHelper::GenericArrayToFile<CCCoreLib::VerticesIndexes, 3,
                                                 unsigned>(*m_triVertIndexes,
                                                           out))
    return false;
...
bool ccMesh::fromFile_MeOnly(QFile &in, short dataVersion, int flags,
  ...
  // we hope we will find it at loading time (i.e. this is the responsibility of
  ...
  *(uint32_t *)(&m_associatedCloud) = vertUniqueID;
  ...
  // texture coordinates array (dataVersion>=20)
  {
    // as the associated texture coordinates array can't be saved directly (as
    // it may be shared by multiple meshes) we only store its unique ID
    // (dataVersion>=20) --> we hope we will find it at loading time (i.e. this
  ...
  // materials
  {
    // as the associated materials can't be saved directly (as it may be shared
    // by multiple meshes) we only store its unique ID (dataVersion>=20) --> we
  ...
  if (!ccSerializationHelper::GenericArrayFromFile<CCCoreLib::VerticesIndexes,
                                                   3, unsigned>(
...
void ccMesh::computeInterpolationWeights(unsigned triIndex, const CCVector3 &P,
...
void ccMesh::computeInterpolationWeights(
    const CCCoreLib::VerticesIndexes &vertIndexes, const CCVector3 &P,
    CCVector3d &weights) const {
...
bool ccMesh::interpolateNormals(unsigned triIndex, const CCVector3 &P,
                                CCVector3 &N) {
  ...
  return interpolateNormals(
      tri, w, N, hasTriNormals() ? &m_triNormalIndexes->at(triIndex) : nullptr);
}

bool ccMesh::interpolateNormalsBC(unsigned triIndex, const CCVector3d &w,
                                  CCVector3 &N) {
  assert(triIndex < size());
...
bool ccMesh::interpolateNormals(const CCCoreLib::VerticesIndexes &vertIndexes,
  ...
  CCVector3d Nd(0, 0, 0);
  {
    if (!triNormIndexes || triNormIndexes->u[0] >= 0) {
      const CCVector3 &N1 =
          triNormIndexes ? ccNormalVectors::GetNormal(
                               m_triNormals->getValue(triNormIndexes->u[0]))
    ...
    if (!triNormIndexes || triNormIndexes->u[1] >= 0) {
      const CCVector3 &N2 =
          triNormIndexes ? ccNormalVectors::GetNormal(
                               m_triNormals->getValue(triNormIndexes->u[1]))
    ...
    if (!triNormIndexes || triNormIndexes->u[2] >= 0) {
      const CCVector3 &N3 =
          triNormIndexes ? ccNormalVectors::GetNormal(
                               m_triNormals->getValue(triNormIndexes->u[2]))
...
bool ccMesh::interpolateColors(unsigned triIndex, const CCVector3 &P,
                               ccColor::Rgb &color) {
  assert(triIndex < size());
...
bool ccMesh::interpolateColorsBC(unsigned triIndex, const CCVector3d &w,
                                 ccColor::Rgb &color) {
  assert(triIndex < size());
...
bool ccMesh::interpolateColors(const CCCoreLib::VerticesIndexes &vertIndexes,
                               const CCVector3d &w, ccColor::Rgb &color) {
  ...
  color.r = static_cast<ColorCompType>(
      C1.r * w.u[0] + C2.r * w.u[1] +
      C3.r * w.u[2]); // static_cast is equivalent to floor if value >= 0
  color.g = static_cast<ColorCompType>(
      C1.g * w.u[0] + C2.g * w.u[1] +
      C3.g * w.u[2]); // static_cast is equivalent to floor if value >= 0
  color.b = static_cast<ColorCompType>(
      C1.b * w.u[0] + C2.b * w.u[1] +
      C3.b * w.u[2]); // static_cast is equivalent to floor if value >= 0
...
bool ccMesh::interpolateColors(const CCCoreLib::VerticesIndexes &vertIndexes,
                               const CCVector3d &w, ccColor::Rgba &color) {
  const ccColor::Rgba &C1 = m_associatedCloud->getPointColor(vertIndexes.i1);
  const ccColor::Rgba &C2 = m_associatedCloud->getPointColor(vertIndexes.i2);
  const ccColor::Rgba &C3 = m_associatedCloud->getPointColor(vertIndexes.i3);
  ...
  color.b = static_cast<ColorCompType>(
      C1.b * w.u[0] + C2.b * w.u[1] +
      C3.b * w.u[2]); // static_cast is equivalent to floor if value >= 0
  color.a = static_cast<ColorCompType>(
      C1.a * w.u[0] + C2.a * w.u[1] +
      C3.a * w.u[2]); // static_cast is equivalent to floor if value >= 0
...
bool ccMesh::getVertexColorFromMaterial(unsigned triIndex,
  ...
  assert(vertIndex < 3);
  if (vertIndex > 2) {
    ccLog::Warning("[ccMesh::getVertexColorFromMaterial] Internal error: "
  ...
  if (matIndex >= 0) {
    ...
    if (material->hasTexture()) {
      ...
      const Tuple3i &txInd = m_texCoordIndexes->getValue(triIndex);
      const TexCoords2D *T =
          (txInd.u[vertIndex] >= 0 ? &m_texCoords->getValue(txInd.u[vertIndex])
                                   : nullptr);
      if (T) {
        ...
        int xPix =
            std::min(static_cast<int>(tx * texture.width()),
                     texture.width() -
                         1); // static_cast is equivalent to floor if value >= 0
        int yPix =
            std::min(static_cast<int>(ty * texture.height()),
                     texture.height() -
                         1); // static_cast is equivalent to floor if value >= 0
...
bool ccMesh::getColorFromMaterial(unsigned triIndex, const CCVector3 &P,
                                  ccColor::Rgba &color,
  ...
  // get color from texture image
  {
    const QImage texture = material->getTexture();
    int xPix =
        std::min(static_cast<int>(x * texture.width()),
                 texture.width() -
                     1); // static_cast is equivalent to floor if value >= 0
    int yPix =
        std::min(static_cast<int>(y * texture.height()),
                 texture.height() -
                     1); // static_cast is equivalent to floor if value >= 0
...
bool ccMesh::pushSubdivide(/*PointCoordinateType maxArea, */ unsigned indexA,
                           unsigned indexB, unsigned indexC) {
  ...
  if (!getAssociatedCloud() ||
      !getAssociatedCloud()->isA(CC_TYPES::POINT_CLOUD)) {
    ccLog::Warning(
        "[ccMesh::pushSubdivide] Vertices set must be a true point cloud!");
    return false;
  }
    ...
    unsigned indexG1 = 0;
    {
      ...
      if (it == s_alreadyCreatedVertices.constEnd()) {
        ...
        if (vertices->hasColors()) {
          ...
          ccColor::Rgba color;
          interpolateColors(CCCoreLib::VerticesIndexes(indexA, indexB, indexC),
                            w1, color);
...
ccMesh *ccMesh::subdivide(PointCoordinateType maxArea) const {
  ...
  try {
    ...
    for (unsigned i = 0; i < newTriCount; ++i) {
      CCCoreLib::VerticesIndexes &tri = resultMesh->m_triVertIndexes->getValue(
          i); // warning: array might change at each call to reallocate!
      ...
      {
        QMap<qint64, unsigned>::const_iterator it =
            s_alreadyCreatedVertices.constFind(GenerateKey(indexA, indexB));
        if (it != s_alreadyCreatedVertices.constEnd())
      ...
      {
        QMap<qint64, unsigned>::const_iterator it =
            s_alreadyCreatedVertices.constFind(GenerateKey(indexB, indexC));
        if (it != s_alreadyCreatedVertices.constEnd())
      ...
      {
        QMap<qint64, unsigned>::const_iterator it =
            s_alreadyCreatedVertices.constFind(GenerateKey(indexC, indexA));
        if (it != s_alreadyCreatedVertices.constEnd())
          indexG3 = (int)it.value();
      }

      // at least one edge is 'wrong'
      unsigned brokenEdges =
          (indexG1 < 0 ? 0 : 1) + (indexG2 < 0 ? 0 : 1) + (indexG3 < 0 ? 0 : 1);
      ...
      if (brokenEdges == 1) {
        ...
        // and add the other half (we can use pushSubdivide as the area should
        // already be ok!)
        ...
        if (indexG1 < 0) // broken edges: BC and CA
        {
          ...
          if (!resultMesh->pushSubdivide(/*maxArea, */ indexA, indexG2,
                                         indexG3) ||
        ...
        } else if (indexG2 < 0) // broken edges: AB and CA
        {
          ...
          tri.i3 = indexG3;
          // split the remaining 'trapezoid' in 2
          if (!resultMesh->pushSubdivide(/*maxArea, */ indexB, indexG3,
                                         indexG1) ||
        ...
        } else /*if (indexG3 < 0)*/ // broken edges: AB and BC
        {
          ...
          tri.i3 = indexG1;
          // split the remaining 'trapezoid' in 2
          if (!resultMesh->pushSubdivide(/*maxArea, */ indexC, indexG1,
                                         indexG2) ||
                 ...
                 3) // works just as a standard subdivision in fact!
      {
        ...
        tri.i2 = indexG1;
        tri.i3 = indexG3;
        // and add the other 3 quarters (we can use pushSubdivide as the area
...
bool ccMesh::convertMaterialsToVertexColors() {
  if (!hasMaterials()) {
    ccLog::Warning(
        "[ccMesh::convertMaterialsToVertexColors] Mesh has no material!");
    return false;
  }

  if (!m_associatedCloud->isA(CC_TYPES::POINT_CLOUD)) {
    ccLog::Warning("[ccMesh::convertMaterialsToVertexColors] Need a true point "
                   "cloud as vertices!");
    return false;
  }
  ...
  if (!cloud->resizeTheRGBTable(true)) {
    ccLog::Warning("[ccMesh::convertMaterialsToVertexColors] Failed to resize "
  ...
  for (unsigned i = 0; i < faceCount; ++i) {
    ...
    for (unsigned char j = 0; j < 3; ++j) {
      ...
      if (getVertexColorFromMaterial(i, j, color, true)) {
        // FIXME: could we be smarter? (we process each point several times! And
        // we assume the color is always the same...)
...
static bool
TagDuplicatedVertices(const CCCoreLib::DgmOctree::octreeCell &cell,
  ...
  static const PointCoordinateType c_defaultSearchRadius =
      static_cast<PointCoordinateType>(sqrt(CCCoreLib::ZERO_TOLERANCE_F));
  ...
  try {
    ...
    // tag the duplicated vertices
    {
      ...
      unsigned result = octree->executeFunctionForAllCellsAtLevel(
          10, TagDuplicatedVertices, additionalParameters, false, pDlg.data(),
          "Tag duplicated vertices");

      if (result == 0) {
        ccLog::Warning("[MergeDuplicatedVertices] Duplicated vertices removal "
                       "algorithm failed?!");
        return false;
      }
    }
    ...
    // copy root points in a new cloud
    {
      for (unsigned i = 0; i < vertCount; ++i) {
        ...
        if (eqIndex >= static_cast<int>(vertCount)) // root point
          ...
          equivalentIndexes[i] =
              equivalentIndexes[eqIndex]; // and update the other indexes
      }
    }
    ...
    // update face indexes
    {
      ...
      for (unsigned i = 0; i < faceCount; ++i) {
        ...
        // very small triangles (or flat ones) may be implicitly removed by
        // vertex fusion!
      ...
      if (newFaceCount == 0) {
        ccLog::Warning(
            "[MergeDuplicatedVertices] After vertex fusion, all triangles "
            "would collapse! We'll keep the non-fused version...");
        delete newVertices;
        newVertices = nullptr;
      } else {
        resize(newFaceCount);
    ...
    ccLog::Print("[MergeDuplicatedVertices] Remaining vertices after "
                 "auto-removal of duplicate ones: %i",
                 vertCount);
    ccLog::Print("[MergeDuplicatedVertices] Remaining faces after auto-removal "
                 "of duplicate ones: %i",
                 size());
  } catch (const std::bad_alloc &) {
    ccLog::Warning("[MergeDuplicatedVertices] Not enough memory: could not "

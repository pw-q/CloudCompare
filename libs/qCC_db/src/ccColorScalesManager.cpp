...
void ccColorScalesManager::fromPersistentSettings() {
  ...
  ccLog::Print(QString("[ccColorScalesManager] Found %1 custom scale(s) in "
                       "persistent settings")
                   .arg(scales.size()));
    ...
    try {
      // steps
      {
        ...
        for (int i = 0; i < size; ++i) {
          ...
          double relativePos =
              settings.value(c_csm_stepRelativePos, 0.0).toDouble();
          QRgb rgb =
              static_cast<QRgb>(settings.value(c_csm_stepColor, 0).toInt());
      ...
      // custom labels
      {
        ...
        for (int i = 0; i < size; ++i) {
          ...
          QString text =
              settings.value(c_csm_customLabelText, QString()).toString();
          scale->customLabels().insert({label, text});
        }
        settings.endArray();
      }
    } catch (const std::bad_alloc &) {
      ccLog::Warning(QString("[ccColorScalesManager] Failed to load scale '%1' "
                             "(not enough memory)")
                         .arg(scale->getName()));
      scale.clear();
    }
...
void ccColorScalesManager::removeScale(QString UUID) {
  ...
  if (it != m_scales.constEnd()) {
    if ((*it)->isLocked()) {
      ccLog::Warning(QString("[ccColorScalesManager::addScale] Can't remove a "
                             "locked scale (%1)!")
                         .arg(UUID));
    } else {
...
ccColorScale::Shared ccColorScalesManager::Create(DEFAULT_SCALES scaleType) {
  ...
  if (name.isNull()) {
    ccLog::Error(
        QStringLiteral("Unhandled pre-defined scale (%1)").arg(scaleType));
  ...
  case VERTEX_QUALITY:
    ...
    scale->insert(ccColorScaleElement(1.0, Qt::red), false);
    assert(CCCoreLib::MeshSamplingTools::VERTEX_NORMAL <
               CCCoreLib::MeshSamplingTools::VERTEX_BORDER &&
           CCCoreLib::MeshSamplingTools::VERTEX_BORDER <
               CCCoreLib::MeshSamplingTools::VERTEX_NON_MANIFOLD);
    scale->setAbsolute(CCCoreLib::MeshSamplingTools::VERTEX_NORMAL,
                       CCCoreLib::MeshSamplingTools::VERTEX_NON_MANIFOLD);
    scale->customLabels().insert(0);
    scale->customLabels().insert(0.5);
    scale->customLabels().insert(1.0);
  ...
  case CIVIDIS: {
    for (unsigned i = 0; i < 256; ++i) {
      ...
      scale->insert(
          ccColorScaleElement(step.pos, qRgb(step.red, step.green, step.blue)),
          false);
    }
    break;
  }

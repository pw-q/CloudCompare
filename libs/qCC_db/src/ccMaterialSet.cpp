...
int ccMaterialSet::findMaterialByName(const QString &mtlName) {
  ccLog::PrintDebug(QString("[ccMaterialSet::findMaterialByName] Query: ") +
                    mtlName);
...
int ccMaterialSet::findMaterialByUniqueID(const QString &uniqueID) {
  ccLog::PrintDebug(QString("[ccMaterialSet::findMaterialByUniqueID] Query: ") +
                    uniqueID);

  int i = 0;
  for (ccMaterialSet::const_iterator it = begin(); it != end(); ++it, ++i) {
    ccMaterial::CShared mtl = *it;
    ccLog::PrintDebug(QString("\tmaterial #%1 ID: %2")
                          .arg(i)
...
// MTL PARSER INSPIRED BY KIXOR.NET "objloader"
// (http://www.kixor.net/dev/objloader/)
bool ccMaterialSet::ParseMTL(const QString &path, const QString &filename,
                             ccMaterialSet &materials, QStringList &errors) {
  // open mtl file
  ...
  while (!currentLine.isNull()) {
    ...
    if (tokens.empty() || tokens.front().startsWith('/', Qt::CaseInsensitive) ||
    ...
    if (tokens.front() == "newmtl") {
      ...
      QString materialName =
          currentLine.mid(7)
              .trimmed(); // we must take the whole line! (see OBJ filter)
    ...
    } else if (currentMaterial) // we already have a "current" material
    {
      ...
      if (tokens.front() == "Ka") {
        if (tokens.size() > 3) {
          ccColor::Rgbaf ambient(tokens[1].toFloat(), tokens[2].toFloat(),
      ...
      // diff
        ...
        if (tokens.size() > 3) {
          ccColor::Rgbaf diffuse(tokens[1].toFloat(), tokens[2].toFloat(),
                                 tokens[3].toFloat(), 1.0f);
      ...
      // specular
        ...
        if (tokens.size() > 3) {
          ccColor::Rgbaf specular(tokens[1].toFloat(), tokens[2].toFloat(),
                                  tokens[3].toFloat(), 1.0f);
      ...
      // emission
        ...
        if (tokens.size() > 3) {
          ccColor::Rgbaf emission(tokens[1].toFloat(), tokens[2].toFloat(),
                                  tokens[3].toFloat(), 1.0f);
      ...
      // transparency (inverse of)
        ...
        if (tokens.size() > 1) {
          ...
          if (d == 0) {
            ccLog::Warning(
                QString(
                    "Material %1 'alpha' (=d) value is 0 (= fully transparent)")
      ...
      // transparency
        ...
        if (tokens.size() > 1) {
          ...
          if (tr == 1.0f) {
            ccLog::Warning(QString("Material %1 'transparency' (=Tr) value is "
                                   "1 (= fully transparent)")
                               .arg(currentMaterial->getName()));
          }
      ...
      // texture map
               ...
               tokens.front() == "map_Ks") {
        // DGM: in case there's hidden or space characters at the beginning of
        // the line...
        int shift = currentLine.indexOf("map_K", 0);
        QString textureFilename = (shift + 7 < currentLine.size()
...
bool ccMaterialSet::saveAsMTL(const QString &path, const QString &baseFilename,
  ...
  stream << "# Generated by Zoomlion" << endl;
  ...
  for (ccMaterialSet::const_iterator it = begin(); it != end();
    ...
    // openGL "a = 1.0" is for "full opacity" / MTL "Tr = 1.0" is for "full
    // transparency"
  ...
  try {
    for (const auto &mtl : source) {
      if (addMaterial(mtl) <= 0) {
        ccLog::WarningDebug(
            QString("[ccMaterialSet::append] Material %1 couldn't be added to "
                    "material set and will be ignored")
                .arg(mtl->getName()));
      }
    }
  ...
  // Load each material
  {
    for (uint32_t i = 0; i < count; ++i) {
      ...
      if (!mtl->fromFile(in, dataVersion, flags, oldToNewIDMap))
        return false;
      addMaterial(mtl, true); // if we load a file, we can't allow that
                              // materials are not in the same order as before!
    }
  }

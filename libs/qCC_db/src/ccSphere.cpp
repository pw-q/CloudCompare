...
ccSphere::ccSphere(PointCoordinateType radius,
    ...
    : ccGenericPrimitive(name, transMat, uniqueID), m_radius(radius) {
...
ccGenericPrimitive *ccSphere::clone() const {
  return finishCloneJob(
      new ccSphere(m_radius, &m_transformation, getName(), m_drawPrecision));
}
...
bool ccSphere::buildUp() {
  ...
  // then, angular sweep
  PointCoordinateType angle_rad_step = static_cast<PointCoordinateType>(M_PI) /
    ...
    for (unsigned j = 1; j < steps; ++j) {
      ...
      for (unsigned i = 0; i < steps; ++i) {
        PointCoordinateType phi =
            static_cast<PointCoordinateType>(2 * i) * angle_rad_step;
...
void ccSphere::drawNameIn3D(CC_DRAW_CONTEXT &context) {
  ...
  int bkgBorder =
      QFontMetrics(context.display->getTextDisplayFont()).height() / 4 + 4;
  QFont font = context.display

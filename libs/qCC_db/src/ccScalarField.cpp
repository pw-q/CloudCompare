...
ccScalarField::ccScalarField(const std::string &name /*=std::string()*/)
    : ScalarField(name), m_showNaNValuesInGrey(true), m_symmetricalScale(false),
      m_logScale(false), m_alwaysShowZero(false), m_colorScale(nullptr),
      m_colorRampSteps(0), m_modified(true) {
...
ccScalarField::ccScalarField(const ccScalarField &sf)
    : ScalarField(sf), m_displayRange(sf.m_displayRange),
      ...
      m_symmetricalScale(sf.m_symmetricalScale), m_logScale(sf.m_logScale),
      m_alwaysShowZero(sf.m_alwaysShowZero), m_colorScale(sf.m_colorScale),
      m_colorRampSteps(sf.m_colorRampSteps), m_histogram(sf.m_histogram),
...
ScalarType ccScalarField::normalize(ScalarType d) const {
    ...
    } else // symmetric scale
    {
      ...
      if (d >= 0) {
        if (d >= m_saturationRange.stop())
          return static_cast<ScalarType>(1);
        return (static_cast<ScalarType>(1) +
                (d - m_saturationRange.start()) / m_saturationRange.range()) /
               2;
      } else {
        if (d <= -m_saturationRange.stop())
          return 0;
  ...
  {
    ScalarType dLog = log10(std::max(static_cast<ScalarType>(std::abs(d)),
                                     CCCoreLib::ZERO_TOLERANCE_SCALAR));
...
void ccScalarField::setLogScale(bool state) {
  if (m_logScale != state) {
    ...
    if (m_logScale && getMin() < 0) {
      ccLog::Warning("[ccScalarField] Scalar field contains negative values! "
  ...
  // update histogram
  {
    if (m_displayRange.maxRange() == 0 || currentSize() == 0) {
      ...
      unsigned numberOfClasses =
          static_cast<unsigned>(ceil(sqrt(static_cast<double>(count))));
      ...
      try {
        ...
        ccLog::Warning("[ccScalarField::computeMinAndMax] Failed to update "
      ...
      if (!m_histogram.empty()) {
        ...
        // compute histogram
        {
          ...
          for (unsigned i = 0; i < count; ++i) {
            ...
            if (ValidValue(val)) {
              unsigned bin =
                  static_cast<unsigned>((val - m_displayRange.min()) * step);
...
void ccScalarField::updateSaturationBounds() {
  ...
  {
    ScalarType minAbsVal = (getMax() < 0 ? std::min(-getMax(), -getMin())
                                         : std::max<ScalarType>(getMin(), 0));
    ...
    // if (m_logScale)
    {
      ScalarType minSatLog =
          log10(std::max(minAbsVal, CCCoreLib::ZERO_TOLERANCE_SCALAR));
      ScalarType maxSatLog =
          log10(std::max(maxAbsVal, CCCoreLib::ZERO_TOLERANCE_SCALAR));
  ...
  } else // absolute scale
  {
    ...
    // if (m_logScale)
    {
      ScalarType minAbsVal = static_cast<ScalarType>(
          maxVal < 0 ? std::min(-maxVal, -minVal) : std::max(minVal, 0.0));
      ScalarType maxAbsVal =
          static_cast<ScalarType>(std::max(std::abs(minVal), std::abs(maxVal)));
      ScalarType minSatLog =
          log10(std::max(minAbsVal, CCCoreLib::ZERO_TOLERANCE_SCALAR));
      ScalarType maxSatLog =
          log10(std::max(maxAbsVal, CCCoreLib::ZERO_TOLERANCE_SCALAR));
...
bool ccScalarField::toFile(QFile &out, short dataVersion) const {
  ...
  if (dataVersion < 55) {
    ...
    if (m_name.length() > 255) {
      ccLog::Warning(QString("Scalar field '%1...' name is too long, it will "
                             "be truncated to the first 255 characters")
                         .arg(QString::fromStdString(m_name).left(128)));
    }
    char nameBuffer[256];
    ...
    if (m_name.length() > MaxSFNameLength) {
      ccLog::Warning(QString("Scalar field '%1...' name is too long, it will "
                             "be truncated to the first %2 characters")
                         .arg(QString::fromStdString(m_name).left(128))
                         .arg(MaxSFNameLength));
    }
    uint16_t nameLength =
        static_cast<uint16_t>(std::min(m_name.length(), MaxSFNameLength));
  ...
  if (dataVersion >= 42) {
    ...
    ccLog::Warning("Saving a scalar field with a non-zero offset with a "
                   "version prior to 4.2 will lead to information loss!");
    assert(false);
  }
...
bool ccScalarField::fromFile(QFile &in, short dataVersion, int flags,
  ...
  double baseOffset = 0.0;
  {
    ...
    {
      result = ccSerializationHelper::GenericArrayFromFile<float, 1, float>(
          *this, in, dataVersion, sfDescription);
    } else // file is 'double'
    {
      // we load it as float, but apply an automatic offset (based on the first
      // element) to not lose information/accuracy
      result = ccSerializationHelper::GenericArrayFromTypedFile<float, 1, float,
                                                                double>(
  ...
  if (dataVersion < 27) {
    ...
    if (!autoBoundaries) {
      ccLog::Warning(
          "[ccScalarField] Former 'released' boundaries are deprecated!");
      ccLog::Warning(
          "[ccScalarField] You'll have to create the corresponding 'absolute' "
          "color scale (see the Color Scale Manager) and replace the file.");
    }
  }
  ...
  // color scale
  {
    ...
    if (!colorScalesManager) {
      ccLog::Warning(
          "[ccScalarField::fromFile] Failed to access color scales manager?!");
      assert(false);
    }
    ...
    if (dataVersion < 27) {
      ...
      default:
        ccLog::Warning(
            "[ccScalarField::fromFile] Color scale is no more supported!");
        break;
      }
      m_colorScale =
...
short ccScalarField::minimumFileVersion() const {
  ...
  if (m_name.length() > 255) {
    // we need version 55 to save a scalar field name with more than 255
    // characters

...
ccKdTree::ccKdTree(ccGenericPointCloud *aCloud)
    : CCCoreLib::TrueKdTree(aCloud), ccHObject("Kd-tree"),
...
public:
  DrawMeOnlyVisitor(const ccBBox &box, const ccColor::Rgb &color)
      : m_drawCellBBox(box), m_color(color) {}
...
void ccKdTree::drawMeOnly(CC_DRAW_CONTEXT &context) {
  ...
  DrawMeOnlyVisitor(m_associatedGenericCloud->getOwnBB(), color)
      .visit(context, m_root);
}

bool ccKdTree::convertCellIndexToSF() {
  if (!m_associatedGenericCloud ||
      !m_associatedGenericCloud->isA(CC_TYPES::POINT_CLOUD))
    return false;
...
public:
  ...
  GetCellBBoxVisitor() {
    ...
    m_UpdatedBox.maxCorner() =
        CCVector3(CCCoreLib::PC_NAN, CCCoreLib::PC_NAN, CCCoreLib::PC_NAN);
  ...
  void visit(ccKdTree::BaseNode *node) {
    ...
    if (node && node->parent) {
      ...
      // we choose the right 'side' of the box that corresponds to the parent's
      // split plane
      CCVector3 &boxCorner =
...
public:
  GetNeighborLeavesVisitor(ccKdTree::BaseNode *cell,
                           ccKdTree::LeafSet &neighbors, const ccBBox &cellBox,
                           const ccBBox &treeBox)
      : m_targetCell(cell), m_targetCellBox(cellBox), m_currentCellBox(treeBox),
        m_neighbors(&neighbors), m_userDataFilterEnabled(false),
...
bool ccKdTree::getNeighborLeaves(ccKdTree::BaseNode *cell,
                                 ...
                                 const int *userDataFilter /*=nullptr*/) {
  if (!m_root)
    return false;

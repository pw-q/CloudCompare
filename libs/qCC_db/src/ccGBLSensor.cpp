...
ccGBLSensor::ccGBLSensor(ROTATION_ORDER rotOrder /*=YAW_THEN_PITCH*/)
    : ccSensor("TLS/GBL"), m_phiMin(0), m_phiMax(0), m_deltaPhi(0),
      m_pitchAnglesAreShifted(false), m_thetaMin(0), m_thetaMax(0),
      m_deltaTheta(0), m_yawAnglesAreShifted(false), m_rotationOrder(rotOrder),
      m_sensorRange(0), m_uncertainty(static_cast<PointCoordinateType>(0.005)) {
...
ccGBLSensor::ccGBLSensor(const ccGBLSensor &sensor,
                         bool copyDepthBuffer /*=false*/)
    : ccSensor(sensor), m_phiMin(sensor.m_phiMin), m_phiMax(sensor.m_phiMax),
      m_deltaPhi(sensor.m_deltaPhi),
      m_pitchAnglesAreShifted(sensor.m_pitchAnglesAreShifted),
      m_thetaMin(sensor.m_thetaMin), m_thetaMax(sensor.m_thetaMax),
      m_deltaTheta(sensor.m_deltaTheta),
      m_yawAnglesAreShifted(sensor.m_yawAnglesAreShifted),
      m_rotationOrder(sensor.m_rotationOrder),
      m_sensorRange(sensor.m_sensorRange), m_uncertainty(sensor.m_uncertainty) {
  if (copyDepthBuffer) {
...
void ccGBLSensor::projectPoint(const CCVector3 &sourcePoint,
                               CCVector2 &destPoint, PointCoordinateType &depth,
  ...
  case YAW_THEN_PITCH: {
    ...
    // pitch = angle around the lateral axis, between -pi (-Z) to pi (+Z) by
    // default
...
bool ccGBLSensor::convertToDepthMapCoords(PointCoordinateType yaw,
  ...
  i = static_cast<unsigned>(
      (yaw - m_thetaMin) /
      m_depthBuffer
          .deltaTheta); // static_cast is equivalent to floor if value >= 0
  if (i == m_depthBuffer.width) {
    --i;
  }
  // yaw angles are in the wrong way! (because they are expressed relatively to
  // the sensor)
  ...
  j = static_cast<unsigned>(
      (pitch - m_phiMin) /
      m_depthBuffer
          .deltaPhi); // static_cast is equivalent to floor if value >= 0
...
ccGBLSensor::NormalGrid *
ccGBLSensor::projectNormals(CCCoreLib::GenericCloud *cloud,
      ...
      if (CCCoreLib::GreaterThanEpsilon(distToSensor)) {
        ...
        if (CCCoreLib::GreaterThanSquareEpsilon(squareS2D)) {
          ...
          PointCoordinateType coef =
              sqrt((CCCoreLib::PC_ONE - S.z * S.z) / squareS2D);
          S.x *= coef;
          S.y *= coef;
        } else {
          S.x = 0;
...
ccGBLSensor::ColorGrid *
ccGBLSensor::projectColors(CCCoreLib::GenericCloud *cloud,
...
struct Interval {
  ...
  template <class T>
  static Interval FindBiggest(const std::vector<T> &values, T intValue,
                              bool allowLoop = true) {
...
bool ccGBLSensor::computeDepthBuffer(
    CCCoreLib::GenericCloud *theCloud, int &errorCode,
  ...
  // init new Z-buffer
  {
    ...
    // yaw as X
    int width =
        static_cast<int>(ceil((m_thetaMax - m_thetaMin) / m_deltaTheta));
...
void ccGBLSensor::drawMeOnly(CC_DRAW_CONTEXT &context) {
  ...
  //	sphereContext.drawingFlags &= (~CC_ENTITY_PICKING); //we must remove the
  //'entity picking flag' so that the sphere doesn't override the picking color!
  //	sphereContext.display = 0;

  //	sphere.setTempColor(entityPickingMode ? pickingColor :
  //ccColor::magenta); 	sphere.draw(sphereContext);
  //}

  const PointCoordinateType halfHeadSize =
  ...
  // sensor legs
  {
    CCVector3 headConnect = /*headCenter*/ -CCVector3(
        0, 0, static_cast<PointCoordinateType>(halfHeadSize) * m_scale);
    if (entityPickingMode)
...
ccBBox ccGBLSensor::getOwnFitBB(ccGLMatrix &trans) {
  ...
  return ccBBox(CCVector3(-m_scale, -m_scale, -m_scale),
                CCVector3(m_scale, m_scale, m_scale), true);
}
...
bool ccGBLSensor::applyViewport(ccGenericGLDisplay *win /*=nullptr*/) const {
  ...
  case ccGBLSensor::YAW_THEN_PITCH: {
    ...
    roty.initFromParameters(
        ...
        CCVector3d(
            0, 0,
            0)); // theta = 0 corresponds to the upward vertical direction!
...
bool ccGBLSensor::fromFile_MeOnly(QFile &in, short dataVersion, int flags,
  ...
  if (dataVersion < 38) {
    ...
    ccSerializationHelper::ScalarsFromDataStream(inStream, flags, &sensorRange,
                                                 1);
    ccSerializationHelper::ScalarsFromDataStream(inStream, flags, &uncertainty,
                                                 1);
    m_sensorRange = static_cast<PointCoordinateType>(sensorRange);
    m_uncertainty = static_cast<PointCoordinateType>(uncertainty);
  } else {
    ccSerializationHelper::CoordsFromDataStream(inStream, flags, &m_sensorRange,
                                                1);
    ccSerializationHelper::CoordsFromDataStream(inStream, flags, &m_uncertainty,
                                                1);
  }

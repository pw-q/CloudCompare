...
void ccRenderingTools::ShowDepthBuffer(ccGBLSensor *sensor,
  ...
  PointCoordinateType maxDist = 0.0f;
  {
    ...
    if (count) {
      ...
      maxDist = std::min(maxDist,
                         static_cast<PointCoordinateType>(avg + 1.0 * stdDev));
    }
  }
                     ...
                     QImage::Format_RGB32);
  {
    ...
    assert(colorScale);
    ScalarType coef =
        CCCoreLib::LessThanEpsilon(maxDist - minDist)
            ? 0
    ...
    for (unsigned y = 0; y < depthBuffer.height; ++y) {
      for (unsigned x = 0; x < depthBuffer.width; ++x, ++_zBuff) {
        const ccColor::Rgb &col =
            (*_zBuff >= minDist
                 ? colorScale->getColorByIndex(static_cast<unsigned>(
                       (std::min(maxDist, *_zBuff) - minDist) * coef))
                 : ccColor::black);
...
struct VLabel {
  ...
  VLabel(int y, int y1, int y2, const ccColorScale::Label &l)
      : yPos(y), yMin(y1), yMax(y2), label(l) {
    assert(y2 >= y1);
  }
};
...
void ccRenderingTools::DrawColorRamp(const CC_DRAW_CONTEXT &context) {
  ...
  ccGLWindowInterface *display =
      static_cast<ccGLWindowInterface *>(context.display);
...
static void DrawRectangleAsQuad(OpenGLFunc &glFunc, int x1, int y1, int x2,
                                int y2) {
...
void ccRenderingTools::DrawColorRamp(const CC_DRAW_CONTEXT &context,
                                     ...
                                     ccGLWindowInterface *win, int glW, int glH,
  ...
  // magic fix (for infinite values!)
  {
    for (ccColorScale::LabelSet::iterator it = keyValues.begin();
      ...
      if (!std::isfinite(it->value)) {
        ...
        it = keyValues.begin(); // restart the process (easier than trying to be
                                // intelligent here ;)
      }
    }
  }
  ...
  if (!customLabels && !sf->areNaNValuesShownInGrey()) {
    ...
    if (!logScale) {
      for (ccColorScale::LabelSet::iterator it = keyValues.begin();
           it != keyValues.end();) {
        if (!sf->displayRange().isInRange(static_cast<ScalarType>(it->value)) &&
            (!alwaysShowZero ||
             it->value !=
                 0)) // we keep zero if the user has explicitely asked for it!
        {
  ...
  QString sfTitle =
      (sfName.empty() ? "Unnamed" : QString::fromStdString(sfName));
  if (logScale) {
  ...
  const int strHeight =
      fm.boundingRect("1")
          .height(); // we assume that all chararcter will have the same height
  ...
  const float spaceBelowTitleRatio =
      0.75f; // we add a margin below the title, proportional to the font size
  ...
  // draw the color ramp
  {
    ...
    DrawRectangleAsQuad<QOpenGLFunctions_2_1>(
        *glFunc, x - borderWidth, y - borderWidth, x + scaleWidth + borderWidth,
        y + scaleMaxHeight + 1 + borderWidth);

    glFunc->glPushAttrib(GL_LINE_BIT);
    glFunc->glLineWidth(renderZoom);

    if (keyValues.size() > 1) {
      int xHisto = x + scaleWidth +
                   std::min(std::max(scaleWidth / 8, 3),
                            static_cast<int>(15 * renderZoom));
      ...
      for (int j = 0; j <= scaleMaxHeight; ++j) {
        ...
        if (!col) {
          ...
          if (customLabels) {
            ...
            col = sf->getColorScale()->getColorByValue(value,
        ...
        if (showHistogram) {
          double bind = (value - sf->displayRange().min()) *
                        (histogram.size() - 1) / sf->displayRange().maxRange();
          int bin = static_cast<int>(floor(bind));

          double hVal = 0.0;
          if (bin >= 0 &&
              bin < static_cast<int>(
                        histogram.size())) // in symmetrical case we can get
                                           // values outside of the real SF range
          {
            hVal = histogram[bin];
          ...
          int xSpan = std::max(
              static_cast<int>(hVal / histogram.maxValue * (scaleWidth / 2)),
              1);
      ...
      DrawRectangleAsQuad(*glFunc, x, y, x + scaleWidth,
                          y + scaleMaxHeight + 1);
    }
  ...
  // Some versions of Qt seem to need glColorf instead of glColorub! (see
  // https://bugreports.qt-project.org/browse/QTBUG-6217)
  glFunc->glColor3f(textColor.r / 255.0f, textColor.g / 255.0f,
                    textColor.b / 255.0f);
  ...
  if (showSFTitle) {
    ...
    win->displayText(sfTitle, xText, glH - 1 - yStart + titleMargin, align,
                     0.0f, nullptr, &font);
  }
  ...
  // display the labels
  {
      ...
      while (drawnLabelsAfter > drawnLabelsBefore) {
        ...
        for (; it2 != drawnLabels.end(); ++it2) {
          if (it1->yMax + 2 * minGap < it2->yMin) {
            ...
            int yScale = static_cast<int>((val - sortedKeyValues[0].value) *
                                          scaleMaxHeight / maxRange);

            // insert it at the right place (so as to keep a sorted list!)
            drawnLabels.insert(it2, VLabel(yScale, yScale - strHeight / 2,
    ...
    for (VLabelSet::iterator it = drawnLabels.begin(); it != drawnLabels.end();
         ++it) {
      ...
      if (it->label.text.isEmpty()) {
        ...
        numberStr = QString::number(value, numberFormat,
                                    displayParams.displayedNumPrecision);
      } else {
        numberStr = it->label.text;
      }

      // display label
      win->displayText(numberStr, xText, yText + it->yPos, align, 0.0f, nullptr,
                       &font);

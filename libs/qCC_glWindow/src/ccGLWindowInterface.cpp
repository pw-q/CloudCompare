...
bool ccGLWindowInterface::StereoSupported() { return s_stereoSupported; }

ccGLWindowInterface::HotZone::HotZone(ccGLWindowInterface *win)
    : textHeight(0), yTextBottomLineShift(0),
      color(133, 193, 39) // default color ("greenish")
      ,
      bbv_label("bubble-view mode"), fs_label("fullscreen mode"),
      psi_label("default point size"), lsi_label("default line width"),
      margin(16), iconSize(16), topCorner(0, 0) {
  if (win) {
    font = win->getFont();
  ...
  psi_totalWidth = /*margin() + */ psi_labelRect.width() + margin + iconSize +
                   margin + iconSize /* + margin*/;
  lsi_totalWidth = /*margin() + */ lsi_labelRect.width() + margin + iconSize +
                   margin + iconSize /* + margin*/;
  bbv_totalWidth =
      /*margin() + */ bbv_labelRect.width() + margin + iconSize /* + margin*/;
  fs_totalWidth =
      /*margin() + */ fs_labelRect.width() + margin + iconSize /* + margin*/;
  ...
  textHeight =
      (3 * textHeight) / 4; // --> factor: to recenter the baseline a little
  yTextBottomLineShift = (iconSize / 2) + (textHeight / 2);
}

QRect ccGLWindowInterface::HotZone::rect(bool clickableItemsVisible,
...
ccGLWindowInterface::ccGLWindowInterface(QObject *parent /*=nullptr*/,
                                         bool silentInitialization /*=false*/)
    : m_uniqueID(++s_GlWindowNumber) // GL window unique ID
      ,
      m_initialized(false), m_trihedronGLList(GL_INVALID_LIST_ID),
      m_pivotGLList(GL_INVALID_LIST_ID), m_lastMousePos(-1, -1),
      m_validModelviewMatrix(false), m_validProjectionMatrix(false),
      m_LODEnabled(true), m_LODAutoDisable(false), m_shouldBeRefreshed(false),
      m_mouseMoved(false), m_mouseButtonPressed(false), m_unclosable(false),
      m_interactionFlags(MODE_TRANSFORM_CAMERA), m_pickingMode(NO_PICKING),
      m_pickingModeLocked(false), m_lastClickTime_ticks(0),
      m_sunLightEnabled(true), m_customLightEnabled(false),
      m_clickableItemsVisible(false), m_activeShader(nullptr),
      m_shadersEnabled(false), m_activeFbo(nullptr), m_fbo(nullptr),
      m_fbo2(nullptr), m_pickingFbo(nullptr), m_alwaysUseFBO(false),
      m_updateFBO(true), m_colorRampShader(nullptr),
      m_customRenderingShader(nullptr), m_activeGLFilter(nullptr),
      m_glFiltersEnabled(false), m_winDBRoot(nullptr),
      m_globalDBRoot(nullptr) // external DB
      ,
      m_font{}, m_pivotVisibility(PIVOT_SHOW_ON_MOVE),
      m_pivotSymbolShown(false), m_allowRectangularEntityPicking(true),
      m_rectPickingPoly(nullptr), m_overriddenDisplayParametersEnabled(false),
      m_showScale(true), m_showTrihedron(true),
      m_silentInitialization(silentInitialization),
      m_bubbleViewModeEnabled(false), m_bubbleViewFov_deg(90.0f),
      m_LODPendingRefresh(false), m_touchInProgress(false),
      m_touchBaseDist(0.0), m_scheduledFullRedrawTime(0),
      m_stereoModeEnabled(false), m_formerParent(nullptr),
      m_exclusiveFullscreen(false), m_showDebugTraces(false),
      m_pickRadius(DefaultPickRadius), m_glExtFuncSupported(false),
      m_autoRefresh(false), m_hotZone(nullptr), m_showCursorCoordinates(false),
      m_autoPickPivotAtCenter(true), m_ignoreMouseReleaseEvent(false),
      m_rotationAxisLocked(false), m_lockedRotationAxis(0, 0, 1),
      m_texturePoolLastIndex(0), m_clippingPlanesEnabled(true),
  ...
  // auto-load previous perspective settings
  {
    ...
    // DGM: we force object-centered view by default now, as the viewer-based
    // perspective is too dependent on what is displayed (so restoring this
    // parameter at next startup is rarely a good idea)
    ...
    int pivotVisibility =
        settings.value(c_ps_pivotVisibility, PIVOT_SHOW_ON_MOVE).toInt();
    int glassType = settings
                        .value(c_ps_stereoGlassType,
                               ccGLWindowInterface::StereoParams::RED_BLUE)
                        .toInt();

    settings.endGroup();

    // update stereo parameters
    m_stereoParams.glassType =
        static_cast<ccGLWindowInterface::StereoParams::GlassType>(glassType);
    ...
    if (!m_silentInitialization) {
      if (!perspectiveView)
        ...
        ccLog::Print(QString("[ccGLWindow] Perspective is on by default (%1)")
                         .arg(objectCenteredView ? "object-centered"
                                                 : "viewer-centered"));
    }
    ...
    if (m_customLightEnabled)
      displayNewMessage("Warning: custom light is ON",
                        ccGLWindowInterface::LOWER_LEFT_MESSAGE, false, 2,
                        CUSTOM_LIGHT_STATE_MESSAGE);
    if (!m_sunLightEnabled)
      displayNewMessage("Warning: sun light is OFF",
                        ccGLWindowInterface::LOWER_LEFT_MESSAGE, false, 2,
...
static void HandleLoggedMessage(const QOpenGLDebugMessage &message,
                                int uniqueID) {
  // Decode severity
  QString sevStr;
...
bool ccGLWindowInterface::initialize() {
  ...
  if (!m_initialized) {
    if (!glFunc->initializeOpenGLFunctions()) // DGM: seems to be necessary at
                                              // least with Qt 5.4
    {
      assert(false);
      return false;
    ...
    if (!m_silentInitialization) {
      ccLog::Print("[3D View %i] Graphics card manufacturer: %s", m_uniqueID,
                   vendorName);
      ccLog::Print("[3D View %i] Renderer: %s", m_uniqueID,
                   glFunc->glGetString(GL_RENDERER));
      ccLog::Print("[3D View %i] GL version: %s", m_uniqueID,
                   glFunc->glGetString(GL_VERSION));
      ccLog::Print("[3D View %i] GLSL Version: %s", m_uniqueID,
                   glFunc->glGetString(GL_SHADING_LANGUAGE_VERSION));
    }

    ccGui::ParamStruct params = getDisplayParameters();

    // VBO support
    if (glContext->hasExtension(
            QByteArrayLiteral("GL_ARB_vertex_buffer_object"))) {
      QStringList glVersion =
          QString((const char *)glFunc->glGetString(GL_VERSION)).split('.');
      int majorVersion = 0;
      int minorVersion = 0;
      if (glVersion.size() >= 2) {
      ...
      if (params.useVBOs &&
          (!vendorName ||
           (vendorNameStr.startsWith("ATI") &&
            (majorVersion < 4 ||
             (majorVersion == 4 &&
              minorVersion <
                  6))))) // only if OpenGL version is earlier than 4.6
      {
        if (!m_silentInitialization)
          ccLog::Warning("[3D View %i] VBO support has been disabled as it may "
                         "not work on %s cards!\nYou can manually activate it "
                         "in the display settings (at your own risk!)",
                         m_uniqueID, vendorName);
    ...
    m_shadersEnabled =
        glContext->hasExtension(
            QByteArrayLiteral("GL_ARB_shading_language_100")) &&
    ...
    if (!m_shadersEnabled) {
      ...
      if (!m_silentInitialization)
        ccLog::Warning("[3D View %i] Shaders and GL filters unavailable",
                       m_uniqueID);
    } else {
      if (!m_silentInitialization)
        ccLog::Print("[3D View %i] Shaders available", m_uniqueID);

      m_glFiltersEnabled = glContext->hasExtension(
          QByteArrayLiteral("GL_EXT_framebuffer_object"));
      if (m_glFiltersEnabled) {
        ...
        ccLog::Warning(
            "[3D View %i] GL filters unavailable (FBO not supported)",
            m_uniqueID);
      }
      ...
      if (!m_colorRampShader) {
        ...
        if (maxBytes < minRequiredBytes) {
          if (!m_silentInitialization)
            ccLog::Warning("[3D View %i] Not enough memory on shader side to "
                           "use color ramp shader! (max=%i/%i bytes)",
                           m_uniqueID, maxBytes, minRequiredBytes);
        } else {
          ccColorRampShader *colorRampShader = new ccColorRampShader();
          QString error;
          const QString shaderPath =
              QStringLiteral("%1/ColorRamp/color_ramp.frag")
                  .arg(GetShaderPath());

          if (!colorRampShader->loadProgram(QString(), shaderPath, error)) {
            if (!m_silentInitialization)
              ccLog::Warning(
                  QString("[3D View %1] Failed to load color ramp shader: '%2'")
                      .arg(m_uniqueID)
                      .arg(error));
            ...
            if (!m_silentInitialization)
              ccLog::Print("[3D View %i] Color ramp shader loaded successfully",
                           m_uniqueID);
            m_colorRampShader = colorRampShader;
            params.colorScaleShaderSupported = true;

            // if global parameter is not yet defined
            if (!getDisplayParameters().isInPersistentSettings(
                    "colorScaleUseShader")) {
              bool shouldUseShader = true;
              if (!vendorName || vendorNameStr.startsWith("ATI") ||
                  vendorNameStr.startsWith("VMWARE")) {
                if (!m_silentInitialization) {
                  ccLog::Warning(
                      "[3D View %i] Color ramp shader will remain disabled as "
                      "it may not work on %s cards!\nYou can manually activate "
                      "it in the display settings (at your own risk!)",
                      m_uniqueID, vendorName);
                }
      ...
      // stereo mode
      {
        ...
        if (!m_silentInitialization) {
          ccLog::Print(
              QString("[3D View %1] Stereo mode: %2")
                  .arg(m_uniqueID)
                  .arg(isStereoEnabled ? "supported" : "not supported"));
        }
      }
    }
    ...
    if (glContext->hasExtension(QByteArrayLiteral("GL_KHR_debug"))) {
      if (!m_silentInitialization)
        ccLog::Print("[3D View %i] GL KHR (debug) extension available",
                     m_uniqueID);
...
void ccGLWindowInterface::onResizeGL(int w, int h) {
  ...
  displayNewMessage(
      QString("New size = %1 * %2 (px)").arg(glWidth()).arg(glHeight()),
      ccGLWindow::LOWER_LEFT_MESSAGE, false, 2, SCREEN_SIZE_MESSAGE);
...
void ccGLWindowInterface::redraw(bool only2D /*=false*/,
                                 bool resetLOD /*=true*/) {
...
void ccGLWindowInterface::setGLViewport(const QRect &rect) {
  ...
  m_glViewport = QRect(rect.left() * retinaScale, rect.top() * retinaScale,
  ...
  if (getOpenGLContext() && getOpenGLContext()->isValid()) {
    ...
    functions()->glViewport(m_glViewport.x(), m_glViewport.y(),
                            m_glViewport.width(), m_glViewport.height());
  }
}
...
const ccGui::ParamStruct &ccGLWindowInterface::getDisplayParameters() const {
  return m_overriddenDisplayParametersEnabled ? m_overriddenDisplayParameters
                                              : ccGui::Parameters();
}

void ccGLWindowInterface::setDisplayParameters(const ccGui::ParamStruct &params,
                                               bool thisWindowOnly) {
  if (thisWindowOnly) {
...
bool ccGLWindowInterface::setLODEnabled(bool state,
  ...
  if (state && (!m_fbo || (m_stereoModeEnabled &&
                           !m_stereoParams.isAnaglyph() && !m_fbo2))) {
...
void ccGLWindowInterface::drawClickableItems(int xStart0, int &yStart) {
  ...
  m_hotZone->topCorner =
      QPoint(xStart0, yStart) + QPoint(m_hotZone->margin, m_hotZone->margin);
  ...
  {
    QRect areaRect = m_hotZone->rect(
        m_clickableItemsVisible, m_bubbleViewModeEnabled, fullScreenEnabled);
    areaRect.translate(m_hotZone->topCorner);

    // draw rectangle
    glFunc->glColor4ub(ccColor::darkGrey.r, ccColor::darkGrey.g,
                       ccColor::darkGrey.b, 210);
  ...
  if (fullScreenEnabled) {
    ...
    renderText(xStart, yStart + m_hotZone->yTextBottomLineShift,
               m_hotZone->fs_label,
               static_cast<uint16_t>(RenderTextReservedIDs::FullScreenLabel),
               m_hotZone->font);
    ...
    //"full-screen" icon
    {
      ccGLUtils::DisplayTexture2DPosition(
          c_exitIcon, -halfW + xStart, halfH - (yStart + m_hotZone->iconSize),
          m_hotZone->iconSize, m_hotZone->iconSize);
      m_clickableItems.emplace_back(
          ClickableItem::LEAVE_FULLSCREEN_MODE,
          QRect(xStart, yStart, m_hotZone->iconSize, m_hotZone->iconSize));
  ...
  if (m_bubbleViewModeEnabled) {
    ...
    renderText(xStart, yStart + m_hotZone->yTextBottomLineShift,
               m_hotZone->bbv_label,
               static_cast<uint16_t>(RenderTextReservedIDs::BubbleViewLabel),
               m_hotZone->font);
  ...
  if (m_clickableItemsVisible) {
    static const QImage c_minusPix =
        QImage(":/CC/images/ccMinus.png").mirrored();
    ...
    // default point size
    {
      ...
      renderText(xStart, yStart + m_hotZone->yTextBottomLineShift,
                 m_hotZone->psi_label,
                 static_cast<uint16_t>(RenderTextReservedIDs::PointSizeLabel),
                 m_hotZone->font);
      ...
      {
        ccGLUtils::DisplayTexture2DPosition(
            c_minusPix, -halfW + xStart, halfH - (yStart + m_hotZone->iconSize),
            m_hotZone->iconSize, m_hotZone->iconSize);
        m_clickableItems.emplace_back(
      ...
      // separator
      {
        ...
        glFunc->glVertex2i(-halfW + xStart + m_hotZone->margin / 2,
                           halfH - (yStart + m_hotZone->iconSize / 2));
        glFunc->glEnd();
      ...
      {
        ccGLUtils::DisplayTexture2DPosition(
            c_plusPix, -halfW + xStart, halfH - (yStart + m_hotZone->iconSize),
            m_hotZone->iconSize, m_hotZone->iconSize);
        m_clickableItems.emplace_back(
    ...
    // default line size
    {
      ...
      renderText(xStart, yStart + m_hotZone->yTextBottomLineShift,
                 m_hotZone->lsi_label,
                 static_cast<uint16_t>(RenderTextReservedIDs::LineSizeLabel),
                 m_hotZone->font);
...
void ccGLWindowInterface::addToOwnDB(ccHObject *obj,
  ...
  if (m_winDBRoot) {
    m_winDBRoot->addChild(obj, noDependency ? ccHObject::DP_NONE
...
void ccGLWindowInterface::zoomGlobal() { updateConstellationCenterAndZoom(); }

void ccGLWindowInterface::updateConstellationCenterAndZoom(
    const ccBBox *boundingBox /*=nullptr*/) {
  if (m_bubbleViewModeEnabled) {
    ccLog::Warning(
        "[updateConstellationCenterAndZoom] Not when bubble-view is enabled!");
    return;
  }
  ...
  double focalDistance =
      targetWidth / m_viewportParams.computeDistanceToWidthRatio();
...
void ccGLWindowInterface::setShader(ccShader *_shader) {
  if (!m_shadersEnabled) {
    ccLog::Warning(
        "[ccGLWindowInterface::setShader] Shader ignored (not supported)");
    return;
  }
...
void ccGLWindowInterface::setGlFilter(ccGlFilter *filter) {
  if (!m_glFiltersEnabled) {
    ccLog::Warning(
        "[ccGLWindowInterface::setGlFilter] GL filter ignored (not supported)");
    return;
  }
...
void ccGLWindowInterface::setFocalDistance(double focalDistance) {
  if (focalDistance != m_viewportParams.getFocalDistance()) {
    ...
    if (m_viewportParams.objectCenteredView) {
      Q_EMIT m_signalEmitter->cameraPosChanged(
          m_viewportParams.getCameraCenter());
    }
...
void ccGLWindowInterface::setCameraPos(const CCVector3d &P) {
  if ((m_viewportParams.getCameraCenter() - P).norm2d() != 0.0) {
    ...
    // ccLog::Print(QString("[ccGLWindow] Focal distance =
    // %1").arg(m_viewportParams.getFocalDistance()));
...
void ccGLWindowInterface::moveCamera(CCVector3d &v) {
  // current X, Y and Z viewing directions correspond to the 'model view' matrix
  // rows
  if (!m_viewportParams.objectCenteredView) {
...
void ccGLWindowInterface::setPivotPoint(const CCVector3d &P,
  ...
  if (autoUpdateCameraPos && m_viewportParams.objectCenteredView) {
    ...
    CCVector3d newCameraPos =
        m_viewportParams.getCameraCenter() + pivotShiftInCameraCS - pivotShift;

    if (!m_viewportParams.perspectiveView) {
      // in orthographic mode, make sure the level of 'zoom' is maintained by
  ...
  if (verbose) {
    ...
    displayNewMessage(QString(), ccGLWindowInterface::LOWER_LEFT_MESSAGE,
...
void ccGLWindowInterface::computeColorRampAreaLimits(int &yStart,
                                                     int &yStop) const {
  ...
  if (trihedronIsDisplayed()) {
    int totalTrihedronHeight =
        2 * static_cast<int>(computeTrihedronLength() + 2 * defaultMargin);
...
ccGLMatrixd ccGLWindowInterface::computeProjectionMatrix(
    bool withGLfeatures, ProjectionMetrics *metrics /*=nullptr*/,
    double *eyeOffset /*=nullptr*/) const {
  static const CCVector3 MinusOne(
      -CCCoreLib::PC_ONE / 2, -CCCoreLib::PC_ONE / 2, -CCCoreLib::PC_ONE / 2);
  static const CCVector3 PlusOne(CCCoreLib::PC_ONE / 2, CCCoreLib::PC_ONE / 2,
                                 CCCoreLib::PC_ONE / 2);
  ...
  double zMax = zMin;
  {
    ...
    std::array<CCVector3, 8> bbCorners{bbMin,
  ...
  double zMaxDisplayed = zMax;
  {
    ...
    if (m_pivotSymbolShown && m_pivotVisibility != PIVOT_HIDE &&
        withGLfeatures && m_viewportParams.objectCenteredView) {
      // take the pivot symbol into account
      double pivotActualRadius_pix = CC_DISPLAYED_PIVOT_RADIUS_PERCENT *
                                     std::min(glWidth(), glHeight()) / 2.0;
      double pivotSymbolScale = pivotActualRadius_pix * pixelSize;
      CCVector3d rotationCenterInCameraCS =
          viewMatd * m_viewportParams.getRotationCenter();
      zMinDisplayed = std::min(zMinDisplayed,
                               -rotationCenterInCameraCS.z - pivotSymbolScale);
      zMaxDisplayed = std::max(zMaxDisplayed,
                               -rotationCenterInCameraCS.z + pivotSymbolScale);
    }

    if (withGLfeatures && m_customLightEnabled) {
      // take the custom light into account
      CCVector3d customLightPosInCameraCS =
          viewMatd * CCVector3d::fromArray(m_customLightPos);
  ...
  if (m_viewportParams.perspectiveView) {
    double minZFar = static_cast<double>(CCCoreLib::ZERO_TOLERANCE_F) /
                     m_viewportParams.zNearCoef;
    if (zFar < minZFar) {
    ...
    double distanceToHalfWidthRatio =
        (m_bubbleViewModeEnabled
             ? std::tan(CCCoreLib::DegreesToRadians(m_bubbleViewFov_deg / 2.0))
             : m_viewportParams.computeDistanceToHalfWidthRatio());
    ...
    if (eyeOffset) {
      // see 'NVIDIA 3D VISION PRO AND STEREOSCOPIC 3D' White paper (Oct 2010,
      // p. 12)
      ...
      // m_stereoParams.screenDistance_mm;	//DGM: we don't want to depend
      // on the cloud size anymore as it can produce very strange visual effects
      // on very large clouds we now keep something related to the focal distance
      // (multiplied by the 'zNearCoef' that can be tweaked by the user if
      // necessary)
      double scale = convergence * m_viewportParams.zNearCoef *
                     m_stereoParams.stereoStrength /
                     m_stereoParams.screenDistance_mm;
      ...
      *eyeOffset *= eyeSeperation;
      ...
      // if (*eyeOffset < 0.0)
      //	const_cast<ccGLWindow*>(this)->displayNewMessage(QString("eye
      //sep. = %1 - convergence = %3").arg(*eyeOffset).arg(convergence),
      //ccGLWindowInterface::LOWER_LEFT_MESSAGE, false, 2,
      //ccGLWindowInterface::PERSPECTIVE_STATE_MESSAGE);
    }

    projMatrix =
        ccGL::Frustum(-xMax - frustumAsymmetry, xMax - frustumAsymmetry, -yMax,
                      yMax, zNear, zFar);
  } else {
    zFar += epsilon;
    zNear -= epsilon;
    // ccLog::Print(QString("cameraCenterToPivotDist = %0 / zNear = %1 / zFar =
    // %2").arg(cameraCenterToPivotDist).arg(zNear).arg(zFar));

    double xMax = std::abs(m_viewportParams.getFocalDistance()) *
                  m_viewportParams.computeDistanceToHalfWidthRatio();
    double yMax = xMax * ar;
  ...
  if (metrics) {
    // ccLog::PrintDebug(QString("[%1 ; %2] R =
    // %3").arg(zNear).arg(zFar).arg(zFar / zNear));
    metrics->zNear = zNear;
    metrics->zFar = zFar;
...
void ccGLWindowInterface::updateProjectionMatrix() {
  ...
  m_projMatd = computeProjectionMatrix(true, &metrics,
...
void ccGLWindowInterface::getContext(CC_DRAW_CONTEXT &CONTEXT) {
  ...
  CONTEXT.labelMarkerSize =
      static_cast<float>(guiParams.labelMarkerSize * computeActualPixelSize());
...
void ccGLWindowInterface::setPickingMode(
    PICKING_MODE mode /*=DEFAULT_PICKING*/,
  ...
  if (m_pickingModeLocked) {
    if ((mode != m_pickingMode) && (mode != DEFAULT_PICKING))
      ccLog::Warning("[ccGLWindowInterface::setPickingMode] Picking mode is "
                     "locked! Can't change it...");
    return;
  }
  ...
  // ccLog::Warning(QString("[%1] Picking mode set to: ").arg(m_uniqueID) +
  // ToString(m_pickingMode));
}

CCVector3d ccGLWindowInterface::convertMousePositionToOrientation(int x,
                                                                  int y) {
  double xc = width() / 2.0;
  ...
  if (m_viewportParams.objectCenteredView) {
    ...
    // we set the virtual rotation pivot closer to the actual one (but we always
    // stay in the central part of the screen!)
...
void ccGLWindowInterface::updateActiveItemsList(
    ...
    if (label) {
      if (!label->isSelected() || !extendToSelectedLabels) {
        ...
        for (auto &label : labels) {
          if (label->isA(CC_TYPES::LABEL_2D) &&
              label->isVisible()) // Warning: cc2DViewportLabel is also a kind
                                  // of 'CC_TYPES::LABEL_2D'!
          {
...
bool ccGLWindowInterface::processClickableItems(int x, int y) {
  ...
  for (std::vector<ClickableItem>::const_iterator it = m_clickableItems.begin();
       it != m_clickableItems.end(); ++it) {
...
void ccGLWindowInterface::onWheelEvent(float wheelDelta_deg) {
  if (m_bubbleViewModeEnabled) {
    ...
    double delta = 0.0;
    if (m_viewportParams.perspectiveView) {
      delta = static_cast<double>(wheelDelta_deg * computeDefaultIncrement()) /
              8.0 * getDisplayParameters().zoomSpeed;
      double speedRatio =
          10.0 * m_viewportParams.zNear / m_visibleObjectsBBox.getMaxBoxDim();
      ...
      delta = (std::abs(cameraCenterToPivotDist) /
               (wheelDelta_deg < 0.0 ? -20.0 : 20.0)) *
              getDisplayParameters().zoomSpeed;
    }
...
void ccGLWindowInterface::startPicking(PickingParameters &params) {
  ...
  if (params.mode == POINT_OR_TRIANGLE_PICKING ||
      ...
      params.mode ==
          LABEL_PICKING // = spawn a label on the clicked point or triangle
  ) {
...
void ccGLWindowInterface::startOpenGLPicking(const PickingParameters &params) {
  ...
  // 3D objects picking
  {
    ...
    // display 3D objects
    // DGM: all of them, even if we don't pick the window own DB for instance,
    // as they can hide the other objects!
    if (m_globalDBRoot)
  ...
  // 2D objects picking
  if (params.mode == ENTITY_PICKING || params.mode == ENTITY_RECT_PICKING ||
    ...
    // DGM: all of them, even if we don't pick the own DB for instance, as they
    // can hide the other objects!
    if (m_globalDBRoot)
      m_globalDBRoot->draw(CONTEXT);
    if (m_winDBRoot)
      m_winDBRoot->draw(CONTEXT);
  ...
  try {
    // crop the picking rectangle so that's it strictly inside the displayed
    // window
    ...
    glFunc->glReadPixels(xTop, yTop, xWidth, yWidth, GL_RGBA, GL_UNSIGNED_BYTE,
                         pickedPixels.data());
    ...
    for (int j = 0; j < yWidth; ++j) {
      for (int i = 0; i < xWidth; ++i, ++_pickedPixels) {
        // testImage.setPixelColor(i, yWidth - 1 - j, QColor(_pickedPixels->r,
        // _pickedPixels->g, _pickedPixels->b));
        if (_pickedPixels->r != 0 || _pickedPixels->g != 0 ||
            _pickedPixels->b != 0) {
          if (params.mode == ENTITY_RECT_PICKING) {
            // avoid reprocessing the pixel corresponding to the same cloud over
            // and over
            if (_pickedPixels->r != previousPixelColor.r ||
                _pickedPixels->g != previousPixelColor.g ||
                _pickedPixels->b != previousPixelColor.b) {
              previousPixelColor = *_pickedPixels;
              ccHObject *object =
                  CONTEXT.entityPicking.objectFromColor(*_pickedPixels);
              if (object) {
            ...
            if (minSquareDistToCenter < 0 ||
                squareDistToCenter < minSquareDistToCenter) {
  ...
  processPickingResult(params, pickedEntity, -1, nullptr, nullptr,
                       &selectedIDs);
}

void ccGLWindowInterface::processPickingResult(
    const PickingParameters &params, ccHObject *pickedEntity,
    int pickedItemIndex, const CCVector3 *nearestPoint /*=nullptr*/,
  ...
  // 3D point or triangle or label picking
    ...
    Q_EMIT m_signalEmitter->itemPicked(
        pickedEntity, static_cast<unsigned>(pickedItemIndex), params.centerX,
        params.centerY, *nearestPoint, *nearestPointBC);
  }
  // fast picking (labels, interactors, etc.)
  else if (params.mode == FAST_PICKING) {
    Q_EMIT m_signalEmitter->itemPickedFast(pickedEntity, pickedItemIndex,
    ...
    if (m_globalDBRoot && pickedEntity && pickedItemIndex >= 0) {
      ...
      // ccLog::Print(QString("[Picking] entity ID %1 - item #%2 (time: %3
      ...
      if (pickedEntity->isKindOf(CC_TYPES::POINT_CLOUD)) {
        ...
        label = new cc2DLabel();
        label->addPickedPoint(ccHObjectCaster::ToGenericMesh(pickedEntity),
                              pickedItemIndex,
...
void ccGLWindowInterface::startCPUBasedPointPicking(
  ...
  static ccGui::ParamStruct::ComputeOctreeForPicking
      autoComputeOctreeThisSession = ccGui::ParamStruct::ASK_USER;
  ...
  try {
    ...
    while (!toProcess.empty()) {
      ...
      if (ent->isDisplayedIn(this)) {
        if (ent->isKindOf(CC_TYPES::POINT_CLOUD)) {
          ...
          if (firstCloudWithoutOctree && !cloud->getOctree() &&
              cloud->size() >
                  MIN_POINTS_FOR_OCTREE_COMPUTATION) // no need to use the
                                                     // octree for a few points!
          {
            // can we compute an octree for picking?
            ccGui::ParamStruct::ComputeOctreeForPicking behavior =
            ...
            case ccGui::ParamStruct::ASK_USER: {
              QMessageBox question(
                  QMessageBox::Question, "Picking acceleration",
                  "Automatically compute octree(s) to accelerate the picking "
                  "process?\n(this behavior can be changed later in the "
                  "Display Settings)",
                  QMessageBox::NoButton, asWidget());
              ...
              if (clickedButton == yes) {
                ...
                globalParams.autoComputeOctree =
                    autoComputeOctree ? ccGui::ParamStruct::ALWAYS
                                      : ccGui::ParamStruct::NEVER;
          ...
          if (cloud->pointPicking(
                  clickedPos, camera, nearestPointIndex, nearestSquareDist,
                  params.pickWidth, params.pickHeight,
                  autoComputeOctree &&
                      cloud->size() > MIN_POINTS_FOR_OCTREE_COMPUTATION)) {
            if (nearestElementIndex < 0 ||
              ...
              nearestEntity = cloud;
                   ...
                   !ent->isA(
                       CC_TYPES::MESH_GROUP) // we don't need to process mesh
                                             // groups as their children will be
                                             // processed later
                   &&
                   !ent->isA(CC_TYPES::COORDINATESYSTEM) // we ignore coordinate
                                                         // system entities
        ) {
          ...
          if (mesh->trianglePicking(clickedPos, camera, nearestTriIndex,
                                    nearestSquareDist, P, &barycentricCoords)) {
            if (nearestElementIndex < 0 ||
                (nearestTriIndex >= 0 &&
                 nearestSquareDist < nearestElementSquareDist)) {
              nearestElementSquareDist = nearestSquareDist;
              ...
              nearestPointBC = barycentricCoords;
                       ...
                       PICKING_MODE::POINT_OR_TRIANGLE_OR_LABEL_PICKING &&
                   ent->isA(CC_TYPES::LABEL_2D)) {
          cc2DLabel *label = static_cast<cc2DLabel *>(ent);

          int nearestPointIndex = -1;
          double nearestSquareDist = 0.0;

          if (label->pointPicking(clickedPos, camera, nearestPointIndex,
                                  nearestSquareDist)) {
            if (nearestElementIndex < 0 ||
                (nearestPointIndex >= 0 &&
                 nearestSquareDist < nearestElementSquareDist)) {
              ...
              nearestElementIndex = nearestPointIndex;
              nearestPoint =
                  label->getPickedPoint(nearestPointIndex).getPointPosition();
              nearestEntity = label;
            }
          }
        }
      ...
      for (unsigned i = 0; i < ent->getChildrenNumber(); ++i) {
        ...
        if (ignoreSubmeshes && ent->getChild(i)->isKindOf(CC_TYPES::SUB_MESH) &&
  ...
  processPickingResult(params, nearestEntity, nearestElementIndex,
                       &nearestPoint, &nearestPointBC);
}

void ccGLWindowInterface::displayNewMessage(
    const QString &message, MessagePosition pos, bool append /*=false*/,
    int displayMaxDelay_sec /*=2*/, MessageType type /*=CUSTOM_MESSAGE*/) {
  if (message.isEmpty()) {
    if (!append) {
      ...
      ccLog::Warning("[ccGLWindowInterface::displayNewMessage] Appending an "
  ...
  if (!append) {
    ...
    if (pos == SCREEN_CENTER_MESSAGE) {
      ccLog::Warning("[ccGLWindowInterface::displayNewMessage] Append is not "
  ...
  // ccLog::Print(QString("[displayNewMessage] New message valid until %1
  ...
  if (m_viewportParams.defaultPointSize != newSize) {
    ...
    if (!silent) {
      displayNewMessage(QString("New default point size: %1").arg(newSize),
                        ccGLWindowInterface::LOWER_LEFT_MESSAGE, false, 2,
                        SCREEN_SIZE_MESSAGE); // DGM HACK: we cheat and use the
                                              // same 'slot' as the window size
    }
  } else {
    if (!silent) {
      if (size < MIN_POINT_SIZE_F)
        ccLog::Print(QString("Default point size is already at minimum : %1")
                         .arg(MIN_POINT_SIZE_F));
      else
        ccLog::Print(QString("Default point size is already at maximum : %1")
                         .arg(MAX_POINT_SIZE_F));
    }
  }
}
...
void ccGLWindowInterface::setLineWidth(float width, bool silent /*=false*/) {
  if (!silent) {
    if (width < MIN_LINE_WIDTH_F)
      ccLog::Warning(QString("Line width is too small: %1/%2")
                         .arg(width)
                         .arg(MIN_LINE_WIDTH_F));
    else if (width > MAX_LINE_WIDTH_F)
      ccLog::Warning(QString("Line width is too big: %1/%2")
                         .arg(width)
                         .arg(MAX_LINE_WIDTH_F));
  }
  float newWidth =
...
void ccGLWindowInterface::glEnableSunLight() {
  ...
  glFunc->glLightfv(GL_LIGHT0, GL_DIFFUSE,
                    getDisplayParameters().lightDiffuseColor.rgba);
  glFunc->glLightfv(GL_LIGHT0, GL_AMBIENT,
                    getDisplayParameters().lightAmbientColor.rgba);
  glFunc->glLightfv(GL_LIGHT0, GL_SPECULAR,
                    getDisplayParameters().lightSpecularColor.rgba);
  glFunc->glLightfv(GL_LIGHT0, GL_POSITION, m_sunLightPos);
  glFunc->glLightModelf(GL_LIGHT_MODEL_TWO_SIDE,
                        getDisplayParameters().lightDoubleSided ? GL_TRUE
                                                                : GL_FALSE);
...
void ccGLWindowInterface::toggleSunLight() { setSunLight(!m_sunLightEnabled); }

void ccGLWindowInterface::glEnableCustomLight() {
  ccQOpenGLFunctions *glFunc = functions();
  assert(glFunc);

  glFunc->glLightfv(GL_LIGHT1, GL_DIFFUSE,
                    getDisplayParameters().lightDiffuseColor.rgba);
  glFunc->glLightfv(GL_LIGHT1, GL_AMBIENT,
                    getDisplayParameters().lightAmbientColor.rgba);
  glFunc->glLightfv(GL_LIGHT1, GL_SPECULAR,
                    getDisplayParameters().lightSpecularColor.rgba);
...
void ccGLWindowInterface::drawCustomLight() {
  ...
  GLfloat d = static_cast<GLfloat>(CC_DISPLAYED_CUSTOM_LIGHT_LENGTH *
  ...
  glFunc->glVertex3f(m_customLightPos[0] - d, m_customLightPos[1],
                     m_customLightPos[2]);
  glFunc->glVertex3f(m_customLightPos[0] + d, m_customLightPos[1],
                     m_customLightPos[2]);
  glFunc->glVertex3f(m_customLightPos[0], m_customLightPos[1] - d,
                     m_customLightPos[2]);
  glFunc->glVertex3f(m_customLightPos[0], m_customLightPos[1] + d,
                     m_customLightPos[2]);
  glFunc->glVertex3f(m_customLightPos[0], m_customLightPos[1],
                     m_customLightPos[2] - d);
  glFunc->glVertex3f(m_customLightPos[0], m_customLightPos[1],
                     m_customLightPos[2] + d);
  glFunc->glEnd();
...
void ccGLWindowInterface::showPivotSymbol(bool state) {
  ...
  if (state && !m_pivotSymbolShown && m_viewportParams.objectCenteredView &&
...
double ccGLWindowInterface::computeActualPixelSize() const {
  double pixelSize = m_viewportParams.computePixelSize(
      glWidth()); // we now use the width as the driving dimension for scaling
...
void ccGLWindowInterface::setBubbleViewMode(bool state) {
  ...
  if (state) {
    ...
    setPerspectiveState(m_preBubbleViewParameters.perspectiveView,
                        m_preBubbleViewParameters.objectCenteredView);
...
void ccGLWindowInterface::setPerspectiveState(bool state,
                                              bool objectCenteredView) {
  // previous state
  ...
  if (m_viewportParams.perspectiveView) {
    ...
    m_viewportParams.objectCenteredView =
        true; // object-centered mode is forced for otho. view
  ...
  CCVector3d cameraCenterToPivot =
      m_viewportParams.getCameraCenter() - m_viewportParams.getPivotPoint();
  ...
  if (viewWasObjectCentered && !m_viewportParams.objectCenteredView) {
    m_viewportParams.viewMat.transposed().apply(
        cameraCenterToPivot); // inverse rotation
  ...
  if (m_bubbleViewModeEnabled) {
    ...
    // and camera state
    {
      ...
      displayNewMessage(
          ...
          ccGLWindowInterface::LOWER_LEFT_MESSAGE, // DGM HACK: we cheat and use
                                                   // the same 'slot' as the
                                                   // window size
...
float ccGLWindowInterface::getFov() const {
  return m_bubbleViewModeEnabled ? m_bubbleViewFov_deg
                                 : m_viewportParams.fov_deg;
}
...
bool ccGLWindowInterface::setNearClippingPlaneDepth(double depth) {
  ...
  if (std::isnan(depth) || depth <= CCCoreLib::ZERO_TOLERANCE_F) {
    if (!std::isnan(m_viewportParams.nearClippingDepth)) {
      m_viewportParams.nearClippingDepth =
    ...
    if (depth < 0.0) {
      ccLog::Warning("[ccGLWindowInterface::setNearClippingPlaneDepth] Invalid "
                     "depth value!");
      return false;
    }
    ...
    if (depth > m_viewportParams.farClippingDepth) {
      ccLog::Warning(
          QString(
              "[ccGLWindowInterface::setNearClippingPlaneDepth] near clipping "
              "depth (%1) can't be larger than far clipping depth (%2)!")
              .arg(depth)
              .arg(m_viewportParams.farClippingDepth));
      return false;
    }
  ...
  displayNewMessage(
      message,
      ccGLWindowInterface::LOWER_LEFT_MESSAGE, // DGM HACK: we cheat and use the
                                               // same 'slot' as the window size
      false, 2, SCREEN_SIZE_MESSAGE);

  Q_EMIT m_signalEmitter->nearClippingDepthChanged(
      m_viewportParams.nearClippingDepth);
...
bool ccGLWindowInterface::setFarClippingPlaneDepth(double depth) {
  ...
  if (std::isnan(depth) || depth >= 1.0e6) {
    if (!std::isnan(m_viewportParams.farClippingDepth)) {
      m_viewportParams.farClippingDepth =
          std::numeric_limits<double>::quiet_NaN();
    ...
    if (depth < 0.0) {
      ccLog::Warning("[ccGLWindowInterface::setFarClippingPlaneDepth] Invalid "
                     "depth value!");
      return false;
    }
    ...
    if (depth < m_viewportParams.nearClippingDepth) {
      ccLog::Warning(
          QString(
              "[ccGLWindowInterface::setFarClippingPlaneDepth] far clipping "
              "depth (%1) can't be smaller than near clipping depth (%2)!")
              .arg(depth)
              .arg(m_viewportParams.nearClippingDepth));
      return false;
    }
  ...
  Q_EMIT m_signalEmitter->farClippingDepthChanged(
      m_viewportParams.farClippingDepth);

  return true;
}

void ccGLWindowInterface::setViewportParameters(
  ...
  if (m_stereoModeEnabled && !params.perspectiveView) {
    ccLog::Warning("Applied viewport projeciton is not perspective: stereo "
...
void ccGLWindowInterface::setupProjectiveViewport(
    const ccGLMatrixd &cameraMatrix, float fov_deg /*=0.0f*/,
    bool viewerBasedPerspective /*=true*/, bool bubbleViewMode /*=false*/) {
...
void ccGLWindowInterface::setCustomView(const CCVector3d &forward,
                                        const CCVector3d &up,
...
void ccGLWindowInterface::setView(CC_VIEW_ORIENTATION orientation,
                                  bool forceRedraw /*=true*/) {
  bool wasViewerBased = !m_viewportParams.objectCenteredView;
  if (wasViewerBased)
    setPerspectiveState(m_viewportParams.perspectiveView, true);
...
bool ccGLWindowInterface::renderToFile(QString filename,
  ...
  QImage outputImage =
      renderToImage(zoomFactor, dontScaleFeatures, renderOverlayItems);
  ...
  if (getDisplayParameters().drawRoundedPoints) {
    // convert the image to plain RGB to avoid issues with points transparency
    // when saving to PNG
  ...
  if (success) {
    ccLog::Print(QString("[Snapshot] File '%1' saved! (%2 x %3 pixels)")
                     .arg(filename)
...
QString ccGLWindowInterface::GetShaderPath() { return s_shaderPath; }
...
bool ccGLWindowInterface::initGLFilter(int w, int h, bool silent /*=false*/) {
  ...
  QString error;
  if (!_filter->init(static_cast<unsigned>(w), static_cast<unsigned>(h),
                     s_shaderPath, error)) {
    if (!silent) {
      ccLog::Warning(QString("[GL Filter] Initialization failed: ") +
                     error.trimmed());
    }
    return false;
  }
...
void ccGLWindowInterface::display3DLabel(
    const QString &str, const CCVector3 &pos3D,
    const ccColor::Rgba *color /*=nullptr*/, const QFont &font /*=QFont()*/) {
  glColor4ubv_safe<ccQOpenGLFunctions>(
      functions(), color ? *color : getDisplayParameters().textDefaultCol);
  renderText(pos3D.x, pos3D.y, pos3D.z, str, font);
}

void ccGLWindowInterface::displayText(
    QString text, int x, int y, unsigned char align /*=ALIGN_HLEFT|ALIGN_VTOP*/,
    float bkgAlpha /*=0*/, const ccColor::Rgba *color /*=nullptr*/,
  ...
  // actual text color
  const ccColor::Rgba &rgba =
      (color ? *color : getDisplayParameters().textDefaultCol);
  ...
  // compoensate for the fact that the '0' of the text quad is lower than one
  // would expect
  y2 -= textDescent;

  glColor4ubv_safe<ccQOpenGLFunctions>(glFunc, rgba);
  renderText(x2 + 1, glHeight() - 1 - y2, text, 0,
             textFont); // x2 + 1 --> empirical
}

CCVector3 ccGLWindowInterface::backprojectPointOnTriangle(
    const CCVector2i &P2D, const CCVector3 &A3D, const CCVector3 &B3D,
    const CCVector3 &C3D) {
  // viewing parameters
  ccGLCameraParameters camera;
  getGLCameraParameters(camera);
  ...
  GLdouble detT =
      (B2D.y - C2D.y) * (A2D.x - C2D.x) + (C2D.x - B2D.x) * (A2D.y - C2D.y);
  GLdouble l1 =
      ((B2D.y - C2D.y) * (P2D.x - C2D.x) + (C2D.x - B2D.x) * (P2D.y - C2D.y)) /
      detT;
  GLdouble l2 =
      ((C2D.y - A2D.y) * (P2D.x - C2D.x) + (A2D.x - C2D.x) * (P2D.y - C2D.y)) /
      detT;
...
ccGLWindowInterface::StereoParams::StereoParams()
    : screenWidth_mm(600), screenDistance_mm(800), eyeSeparation_mm(64),
      stereoStrength(50), glassType(RED_CYAN) {}

void ccGLWindowInterface::renderText(int x, int y, const QString &str,
                                     uint16_t uniqueID /*=0*/,
  ...
  // This adjustment and the change to x & y are to work around a crash with
  // Qt 5.9. At the time I (Andy) could not determine if it is a bug in CC or
  // Qt.
  ...
  // ccLog::Print(QString("Texture rect = (%1 ; %2) --> (%3 x
  // %4)").arg(textRect.x()).arg(textRect.y()).arg(textRect.width()).arg(textRect.height()));
  ...
  if (texture->isStorageAllocated()) {
    if (textRect.width() > texture->width() ||
  ...
  QImage textImage(imageSize.width(), imageSize.height(),
                   QImage::Format::Format_RGBA8888);
  QRect imageRect = textImage.rect();
  // ccLog::Print(QString("Image rect = (%1 ; %2) --> (%3 x
  // %4)").arg(imageRect.x()).arg(imageRect.y()).arg(imageRect.width()).arg(imageRect.height()));
  ...
  {
    glFunc->glPushAttrib(GL_COLOR_BUFFER_BIT | GL_TEXTURE_BIT |
    ...
    glFunc->glLoadIdentity();
    {
      ...
      // In order to reduce the time ATI cards take to manage the texture ID
      // generation and switching, we re-use the textures as much as possible.
      // texture->setData(textImage, QOpenGLTexture::DontGenerateMipMaps);
      if (!texture->isStorageAllocated()) {
        // ccLog::Print(QString("New texture allocated: %1 x
        // %2").arg(imageRect.width()).arg(imageRect.height()));
      ...
      texture->setData(QOpenGLTexture::RGBA, QOpenGLTexture::UInt32_RGBA8_Rev,
                       textImage.bits());
      texture->bind();

      ccGL::Color(glFunc, ccColor::bright); // DGM: warning must be float colors
                                            // to work properly?!
      glFunc->glBegin(GL_QUADS);
    ...
    glFunc->glPopAttrib(); // GL_COLOR_BUFFER_BIT | GL_TEXTURE_BIT |
                           // GL_DEPTH_BUFFER_BIT | GL_ENABLE_BIT
  }
}

void ccGLWindowInterface::renderText(double x, double y, double z,
                                     const QString &str,
                                     const QFont &font /*=QFont()*/) {
  doMakeCurrent();
...
bool ccGLWindowInterface::initFBOSafe(ccFrameBufferObject *&fbo, int w, int h) {
  ...
  if (!_fbo->init(w, h) || !_fbo->initColor() || !_fbo->initDepth()) {
    delete _fbo;
    _fbo = nullptr;
    return false;
  }
...
GLfloat ccGLWindowInterface::getGLDepth(int x, int y,
  ...
  if (usePBO && m_pickingPBO.supported && !m_pickingPBO.glBuffer) {
    if (m_pickingPBO.init()) {
      ccLog::Print(
          "[ccGLWindow] Succesfully initialized PBO for faster depth picking");
  ...
  if (usePBO && m_pickingPBO.glBuffer) {
    ...
    if (diff_ms < 100) {
      // we can read the previous frame buffer (faster) as there shouldn't be
      // too much differences between the two frames
      if (m_pickingPBO.glBuffer->read(0, depthPickingBuffer,
                                      kernelSize * sizeof(GLfloat))) {
        bufferRestored = true;
      } else {
        ccLog::Warning(
            "Failed to read the picking PBO contents. We won't use it anymore");
  ...
  glFunc->glReadPixels(x, y, kernel[0], kernel[1], GL_DEPTH_COMPONENT, GL_FLOAT,
                       usePBO && m_pickingPBO.glBuffer ? nullptr
                                                       : depthPickingBuffer);
  ...
  if (usePBO && m_pickingPBO.glBuffer) {
    if (!bufferRestored) {
      ...
      void *_mappedBuffer =
          m_pickingPBO.glBuffer->map(QOpenGLBuffer::QOpenGLBuffer::ReadOnly);
      if (_mappedBuffer) {
        memcpy(depthPickingBuffer, _mappedBuffer, kernelSize * sizeof(GLfloat));
        m_pickingPBO.glBuffer->unmap();
      } else {
        ccLog::Warning(
            "Failed to map the picking PBO contents. We won't use it anymore");
        m_pickingPBO.glBuffer->release();
        m_pickingPBO.release();
        m_pickingPBO.supported = false;
    ...
    if (m_pickingPBO.glBuffer) // may be null if an error occurred when calling
                               // glBuffer->map
    {
      m_pickingPBO.glBuffer->release();
    }
...
bool ccGLWindowInterface::getClick3DPos(int x, int y, CCVector3d &P3D,
                                        bool usePBO) {
  y = glHeight() - 1 - y;
...
float ccGLWindowInterface::computeTrihedronLength() const {
  return (CC_DISPLAYED_TRIHEDRON_AXES_LENGTH + CC_TRIHEDRON_TEXT_MARGIN +
          QFontMetrics(m_font).width('X')) *
         m_captureMode.zoomFactor;
}
...
void ccGLWindowInterface::drawTrihedron() {
  ...
  float trihedronEdgeLength =
      CC_DISPLAYED_TRIHEDRON_AXES_LENGTH * m_captureMode.zoomFactor;
  float trihedronLength = computeTrihedronLength();

  float halfW = glWidth() / 2.0f;
  float halfH = glHeight() / 2.0f;

  float trihedronCenterX =
      halfW - trihedronLength - 10.0f * m_captureMode.zoomFactor;
  float trihedronCenterY =
      halfH - trihedronLength - 5.0f * m_captureMode.zoomFactor;
  ...
  if (m_trihedronGLList == GL_INVALID_LIST_ID) {
    ...
    glFunc->glClear(GL_DEPTH_BUFFER_BIT); // DGM: the trihedron is displayed in
                                          // the foreground but still in 3D!
    glFunc->glEnable(GL_DEPTH_TEST);
    ...
    glFunc->glScalef(m_captureMode.zoomFactor, m_captureMode.zoomFactor,
                     m_captureMode.zoomFactor);
  }
  ...
  // now display the X, Y and Z axis labels
  {
    ...
    QRect rectX = QFontMetrics(m_font).boundingRect(
        'X'); // rect X ignores the rendering zoom!

    double radius = (std::max(rectX.width(), rectX.height()) / 2.0 +
                     CC_TRIHEDRON_TEXT_MARGIN) *
                    m_captureMode.zoomFactor;

    CCVector2d toTrihedronOrigin(trihedronCenterX, -trihedronCenterY);
    CCVector2d toCharOrigin =
        CCVector2d(-(rectX.x() + rectX.width() / 2.0),
                   rectX.y() + rectX.height() / 4.0) *
        m_captureMode.zoomFactor; // rectX.height() should be divided by 2, but
                                  // it looks better with 4 !

    QFont textFont =
        getTextDisplayFont(); // we take rendering zoom into account!
    ...
    ccGL::Color(glFunc, ccColor::red);
    {
      ...
      dX.normalize();
      dX *= radius;
      CCVector2d posX = CCVector2d(tipX2D.x, tipX2D.y) + toTrihedronOrigin +
                        toCharOrigin + dX;
      renderText(static_cast<int>(halfW + posX.x),
                 static_cast<int>(halfH - posX.y), "X",
                 static_cast<uint16_t>(RenderTextReservedIDs::trihedronX),
                 textFont);
    }
    ...
    ccGL::Color(glFunc, ccColor::green);
    {
      ...
      dY.normalize();
      dY *= radius;
      CCVector2d posY = CCVector2d(tipY2D.x, tipY2D.y) + toTrihedronOrigin +
                        toCharOrigin + dY;
      renderText(static_cast<int>(halfW + posY.x),
                 static_cast<int>(halfH - posY.y), "Y",
                 static_cast<uint16_t>(RenderTextReservedIDs::trihedronY),
                 textFont);
    }
    ...
    ccGL::Color(glFunc, ccColor::blueCC);
    {
      ...
      dZ.normalize();
      dZ *= radius;
      CCVector2d posZ = CCVector2d(tipZ2D.x, tipZ2D.y) + toTrihedronOrigin +
                        toCharOrigin + dZ;
      renderText(static_cast<int>(halfW + posZ.x),
                 static_cast<int>(halfH - posZ.y), "Z",
                 static_cast<uint16_t>(RenderTextReservedIDs::trihedronZ),
                 textFont);
    }
  }
}
...
inline double RoundScale(double equivalentWidth) {
  ...
  // decimals)
  int k =
      static_cast<int>(std::floor(std::log(equivalentWidth) / std::log(10.0f)));
...
void ccGLWindowInterface::drawScale(const ccColor::Rgbub &color) {
  ...
  float dH = std::max(fm.height() * 1.25f,
                      trihedronLength + 5.0f * m_captureMode.zoomFactor);
  ...
  glColor3ubv_safe<ccQOpenGLFunctions>(glFunc, color);
  renderText(glWidth() - static_cast<int>(scaleW_pix / 2 + dW) -
                 fm.width(text) / 2,
             glHeight() - static_cast<int>(dH / 2) + fm.height() / 3, text,
             static_cast<uint16_t>(RenderTextReservedIDs::ScaleLabel), font);
}
...
void ccGLWindowInterface::doDragEnterEvent(QDragEnterEvent *event) {
  ...
  //		ccLog::Print(QString("\tdata:
  //%1").arg(QString::fromUtf16((ushort*)byteData.data(), byteData.size() /
  //2)));
  //	}
  //	else
  //	{
  //		ccLog::Print(QString("\tdata:
  //%1").arg(QString(mimeData->data(format))));
  //	}
  //}
...
bool ccGLWindowInterface::processEvents(QEvent *evt) {
  ...
  case QEvent::TouchEnd: {
    ...
    m_touchBaseDist = 0.0;
    ccLog::PrintDebug(
        QString("Touch event %1").arg(m_touchInProgress ? "begins" : "ends"));
  }
    return true;
  ...
  case QEvent::TouchUpdate: {
    ...
    ccLog::PrintDebug(
        QString("Touch update (%1 points)")
            .arg(static_cast<QTouchEvent *>(evt)->touchPoints().size()));
  } break;
...
void ccGLWindowInterface::startFrameRateTest() {
  ...
  QObject::connect(
      &s_frameRateTimer, &QTimer::timeout, asQObject(), [=]() { redraw(); },
      Qt::QueuedConnection);

  displayNewMessage("[Framerate test in progress]",
                    ccGLWindow::UPPER_CENTER_MESSAGE, true, 3600);
...
void ccGLWindowInterface::stopFrameRateTest() {
  ...
  displayNewMessage(QString(),
                    ccGLWindow::UPPER_CENTER_MESSAGE); // clear message in the
                                                       // upper center area
  if (s_frameRateElapsedTime_ms > 0) {
    QString message =
        QString("Framerate: %1 fps")
            .arg((s_frameRateCurrentFrame * 1.0e3) / s_frameRateElapsedTime_ms,
                 0, 'f', 3);
...
void ccGLWindowInterface::updateFrameRateTest() {
  if (s_frameRateTestInProgress) {
    ...
    if (++s_frameRateCurrentFrame > FRAMERATE_TEST_MIN_FRAMES &&
        s_frameRateElapsedTime_ms > FRAMERATE_TEST_DURATION_MSEC) {
      QTimer::singleShot(0,
                         [&]() { ccGLWindowInterface::stopFrameRateTest(); });
    } else {
      // rotate base view matrix
      ccGLMatrixd rotMat;
      rotMat.initFromParameters(2 * M_PI / FRAMERATE_TEST_MIN_FRAMES,
                                CCVector3d(0, 1, 0), CCVector3d(0, 0, 0));
      m_viewportParams.viewMat = rotMat * m_viewportParams.viewMat;
      invalidateVisualization();
    }
  } else {
...
void ccGLWindowInterface::doPaintGL() {
  ...
  // layer
  if (!m_fbo || m_updateFBO || m_captureMode.enabled ||
  ...
  // clean the outdated messages
  {
    ...
    // ccLog::PrintDebug(QString("[paintGL] Current time: %1
    // s.").arg(currentTime_sec));
  ...
  if (m_autoPickPivotAtCenter && !m_mouseMoved &&
      (renderingParams.hasAutoPivotCandidates[0] ||
       (m_stereoModeEnabled && renderingParams.hasAutoPivotCandidates[1])) &&
  ...
  } else // no frame rate test in progress
  {
    ...
    if (m_currentLODState.inProgress) {
      if ((!m_LODPendingRefresh || m_LODPendingIgnore) && !m_mouseMoved &&
        ...
        ccLog::PrintDebug(
            QString("[QPaintGL] New LOD pass scheduled with timer"));
        QTimer::singleShot(
            std::max(baseLODRefreshTime_ms - displayTime_ms, Q_INT64_C(0)),
            [&]() { renderNextLODLevel(); });
      }
    } else {
      // just in case
  ...
  for (size_t i = 1; i < debugTimings.size(); ++i) {
    debugTimingsMessage +=
        QString("[DT%1 = %2]")
            .arg(i)
            .arg((debugTimings[i] - debugTimings[i - 1]) / 1000);
  }
  debugTimingsMessage +=
      QString("[DT TOTAL = %2]")
          .arg((debugTimings.back() - debugTimings.front()) / 1000);
  ccLog::Print(debugTimingsMessage);
#endif
}

void ccGLWindowInterface::draw3D(CC_DRAW_CONTEXT &CONTEXT,
                                 RenderingParams &renderingParams) {
  ccQOpenGLFunctions *glFunc = functions();
  assert(glFunc);
  ...
  // LOD
  if (isLODEnabled() && !isFrameRateTestInProgress() &&
      (!m_stereoModeEnabled ||
    ...
    if (CONTEXT.decimateCloudOnMove) {
      // ccLog::Print(QString("[LOD] Rendering level
      // %1").arg(m_currentLODState.level));
  ...
  if (m_stereoModeEnabled) {
    ...
    if (!setCustomCameraProjection(renderingParams, modelViewMat,
                                   projectionMat)) {
      ...
      // change eye position
      double eyeOffset =
          renderingParams.pass == MONO_OR_LEFT_RENDERING_PASS ? -1.0 : 1.0;

      // update the projection matrix
      projectionMat =
          computeProjectionMatrix(false, nullptr,
  ...
  if (m_customLightEnabled) {
    // DGM: warning, must be enabled/displayed AFTER the 3D MV and MP matrices
    // have been set!)
    glEnableCustomLight();

    if (!m_captureMode.enabled && m_currentLODState.level == 0 &&
  ...
  if (m_autoPickPivotAtCenter &&
      (!m_stereoModeEnabled ||
       renderingParams.pass == MONO_OR_LEFT_RENDERING_PASS)) {
    CCVector3d P;
    if (getClick3DPos(glWidth() / 2, glHeight() / 2, P,
                      !m_stereoModeEnabled)) // can't use PBO in stereo mode
    {
  ...
  if (renderingParams.pass ==
      MONO_OR_LEFT_RENDERING_PASS) // only the first pass is meaningful
                                   //(the second one is just a duplicate of the
                                   //first)
  {
    ...
    if (m_currentLODState.inProgress) {
      if (CONTEXT.moreLODPointsAvailable || CONTEXT.higherLODLevelsAvailable) {
        ...
        if (CONTEXT.moreLODPointsAvailable) {
          ...
          // renderingParams.nextLODState.startIndex +=
  ...
  if (renderingParams.draw3DCross && m_currentLODState.level == 0 &&
      !m_captureMode.enabled && !m_viewportParams.perspectiveView &&
...
void ccGLWindowInterface::fullRenderingPass(CC_DRAW_CONTEXT &CONTEXT,
                                            RenderingParams &renderingParams) {
  // visual traces
  QStringList diagStrings;
  if (m_showDebugTraces) {
    diagStrings << QString("Stereo mode %1 (pass %2)")
                       .arg(m_stereoModeEnabled ? "ON" : "OFF")
                       .arg(renderingParams.pass);
    diagStrings << QString("FBO %1").arg(
        m_fbo && renderingParams.useFBO ? "ON" : "OFF");
    diagStrings << QString("FBO2 %1").arg(
        m_fbo2 && renderingParams.useFBO ? "ON" : "OFF");
    diagStrings << QString("GL filter %1")
                       .arg(m_fbo && renderingParams.useFBO && m_activeGLFilter
                                ? "ON"
                                : "OFF");
    diagStrings << QString("LOD %1 (level %2)")
                       .arg(m_currentLODState.inProgress ? "ON" : "OFF")
                       .arg(m_currentLODState.level);
  }
  ...
  if (m_stereoModeEnabled) {
    if (m_stereoParams.glassType == StereoParams::NVIDIA_VISION ||
      ...
      if (renderingParams.useFBO &&
          renderingParams.pass == RIGHT_RENDERING_PASS) {
        currentFBO = m_fbo2;
      }
    } else {
      modifiedViewport =
          prepareOtherStereoGlassType(CONTEXT, renderingParams, currentFBO);
    }
  }
    ...
    if (renderingParams.drawBackground || renderingParams.draw3DPass) {
      ...
      if (!m_currentLODState.inProgress || m_currentLODState.level == 0) {
        renderingParams.drawBackground = renderingParams.draw3DPass =
            true; // DGM: we must update the FBO completely!
      } else {
        renderingParams.drawBackground = false;
  ...
  } else if (!m_captureMode.enabled) // capture mode doesn't use double
                                     // buffering by default!
  {
    GLboolean isStereoEnabled = GL_FALSE;
    glFunc->glGetBooleanv(GL_STEREO, &isStereoEnabled);
    // ccLog::Warning(QString("[fullRenderingPass:%0][NO FBO] Stereo test:
    // %1").arg(renderingParams.pass).arg(isStereoEnabled));

    if (isStereoEnabled) {
      if (m_stereoModeEnabled &&
          (m_stereoParams.glassType == StereoParams::NVIDIA_VISION ||
           m_stereoParams.glassType == StereoParams::GENERIC_STEREO_DISPLAY)) {
        glFunc->glDrawBuffer(renderingParams.pass == MONO_OR_LEFT_RENDERING_PASS
                                 ? GL_BACK_LEFT
                                 : GL_BACK_RIGHT);
      } else {
  ...
  if (renderingParams.drawBackground) {
    ...
    if (m_currentLODState.level == 0) {
      if (m_stereoModeEnabled && m_stereoParams.isAnaglyph()) {
        // we don't want to clear the color layer between two anaglyph rendering
        // steps!
        renderingParams.clearColorLayer =
            (renderingParams.pass == MONO_OR_LEFT_RENDERING_PASS);
      }
    } else {
  ...
  if (renderingParams.draw3DPass) {
    ...
    if (m_stereoModeEnabled && m_stereoParams.isAnaglyph()) {
      ...
      case StereoParams::RED_BLUE:
        rgbFilter =
            (renderingParams.pass == MONO_OR_LEFT_RENDERING_PASS ? RED : BLUE);
        break;

      case StereoParams::BLUE_RED:
        rgbFilter =
            (renderingParams.pass == MONO_OR_LEFT_RENDERING_PASS ? BLUE : RED);
        break;

      case StereoParams::RED_CYAN:
        rgbFilter =
            (renderingParams.pass == MONO_OR_LEFT_RENDERING_PASS ? RED : CYAN);
        break;

      case StereoParams::CYAN_RED:
        rgbFilter =
            (renderingParams.pass == MONO_OR_LEFT_RENDERING_PASS ? CYAN : RED);
        break;
  ...
  if (currentFBO && renderingParams.useFBO) {
    ...
    if (!oculusMode) {
      ...
      if (m_activeGLFilter &&
          (!m_stereoModeEnabled ||
           m_stereoParams.glassType !=
               StereoParams::OCULUS)) // not supported with Oculus right now!
      {
        ...
        if (!m_captureMode.enabled) {
          ...
          // ccLog::PrintDebug(QString("[QPaintGL] Will use the shader output
          // texture (tex ID = %1)").arg(screenTex));
        }
      } else if (!m_captureMode.enabled) {
        // screenTex = currentFBO->getDepthTexture();
        screenTex = currentFBO->getColorTexture();
        // ccLog::PrintDebug(QString("[QPaintGL] Will use the standard FBO (tex
        // ID = %1)").arg(screenTex));
      }
      ...
      if (glFunc->glIsTexture(screenTex)) {
        ...
        // DGM: as we couldn't call it before (because of the FBO) we have to do
        // it now!
        GLboolean isStereoEnabled = GL_FALSE;
        glFunc->glGetBooleanv(GL_STEREO, &isStereoEnabled);
        // ccLog::Warning(QString("[fullRenderingPass:%0][FBO] Stereo test:
        // %1").arg(renderingParams.pass).arg(isStereoEnabled));
        if (isStereoEnabled) {
          if (m_stereoModeEnabled &&
              (m_stereoParams.glassType == StereoParams::NVIDIA_VISION ||
               m_stereoParams.glassType ==
                   StereoParams::GENERIC_STEREO_DISPLAY)) {
            glFunc->glDrawBuffer(renderingParams.pass ==
                                         MONO_OR_LEFT_RENDERING_PASS
                                     ? GL_BACK_LEFT
                                     : GL_BACK_RIGHT);
          } else {
            glFunc->glDrawBuffer(GL_BACK);
        ...
        ccGLUtils::DisplayTexture2DPosition(screenTex, 0, 0, glWidth(),
                                            glHeight());
...
void ccGLWindowInterface::drawBackground(CC_DRAW_CONTEXT &CONTEXT,
                                         RenderingParams &renderingParams) {
  ccQOpenGLFunctions *glFunc = functions();
  assert(glFunc);
  ...
  // clear background
  {
    ...
    if (renderingParams.clearColorLayer) {
      ...
      if (displayParams.drawBackgroundGradient) {
        ...
        const ccColor::Rgbaf backgroundColor(
            bkgCol.r / 255.0f, bkgCol.g / 255.0f, bkgCol.b / 255.0f, 1.0f);

        glFunc->glClearColor(backgroundColor.r, backgroundColor.g,
...
void ccGLWindowInterface::drawForeground(CC_DRAW_CONTEXT &CONTEXT,
                                         RenderingParams &renderingParams) {
  ccQOpenGLFunctions *glFunc = functions();
  assert(glFunc);
  ...
  /*** overlay entities ***/
  {
    ...
    if (!m_captureMode.enabled) {
      ...
      showGLFilterRibbon &=
          !exclusiveFullScreen(); // we hide it in fullscreen mode!
      if (showGLFilterRibbon) {
        ...
        renderText(
            10,
            borderHeight - CC_GL_FILTER_BANNER_MARGIN -
                CC_GL_FILTER_BANNER_MARGIN / 2,
      ...
      if (!m_messagesToDisplay.empty()) {
        ...
        for (const auto &message : m_messagesToDisplay) {
          ...
          if (message.type != CUSTOM_MESSAGE) {
            textureID = static_cast<uint16_t>(
                            RenderTextReservedIDs::StandardMessagePrefix) +
          ...
          case SCREEN_CENTER_MESSAGE: {
            ...
            // ;)
            renderText((glWidth() - rect.width()) / 2,
                       (glHeight() - rect.height()) / 2, message.message,
                       textureID, newFont);
          } break;
          }
        }
      ...
      if (renderingParams.nextLODState.inProgress) {
        ...
        static const float alpha =
            static_cast<float>((2 * M_PI) / lodIconParts);
        ...
        for (unsigned i = 0; i < lodIconParts; ++i) {
          float intensity =
              static_cast<float>(
                  (i + renderingParams.nextLODState.progressIndicator) %
                  lodIconParts) /
              (lodIconParts - 1);
          ...
          glFunc->glVertex3f(cx + radius * std::cos(i * alpha),
                             cy + radius * std::sin(i * alpha), 0);
        }
        glFunc->glEnd();
...
void ccGLWindowInterface::onItemPickedFast(ccHObject *pickedEntity,
                                           int pickedItemIndex, int x, int y) {
  if (pickedEntity) {
...
QImage ccGLWindowInterface::renderToImage(float zoomFactor /*=1.0f*/,
  ...
  if (!m_glExtFuncSupported) // no FBO support?!
  {
    if (isStereo()) {
      if (!silent) {
        ccLog::Error(
            "Direct screen capture without FBO is not supported anymore!");
      }
      return QImage();
    } else {
  ...
  if (m_fbo && zoomFactor == 1.0f) {
    ...
    bool success = (fbo->init(glWidth(), glHeight()) && fbo->initColor() &&
                    fbo->initDepth());
    if (!success) {
      delete fbo;
      fbo = nullptr;
    ...
    if (m_activeGLFilter) {
      QString error;
      if (!m_activeGLFilter->init(glWidth(), glHeight(), GetShaderPath(),
                                  error)) {
        if (!silent) {
          ccLog::Warning(
              QString("[GL Filter] GL filter can't be used for rendering: %1")
                  .arg(error));
        }
      } else {
        glFilter = m_activeGLFilter;
  ...
  renderingParams.useFBO =
      false; // DGM: make sure that no FBO is used internally!
  ...
  if (m_stereoModeEnabled) // 2 nd pass for single display 'stereo' rendering (=
                           // anaglyphs)
  {
    renderingParams.pass = RIGHT_RENDERING_PASS;
    fullRenderingPass(CONTEXT, renderingParams);
  ...
  if (glFilter) {
    ...
    setStandardOrthoCorner();
    ccGLUtils::DisplayTexture2DPosition(glFilter->getTexture(), 0, 0,
  ...
  if (m_captureMode.renderOverlayItems) {
    // scale: only in ortho mode
    if (!m_viewportParams.perspectiveView) {
      // DGM FIXME: with a zoom > 1, the renderText call inside drawScale will
      // result in the wrong FBO being used?!
  ...
  for (int i = 0; i < glHeight(); ++i) {
    glFunc->glReadPixels(0, i, glWidth(), 1, GL_BGRA, GL_UNSIGNED_BYTE,
                         data + (glHeight() - 1 - i) * glWidth() * 4);
  }
  ...
  if ((m_pickingMode != NO_PICKING) ||
    ...
    if (m_interactionFlags & INTERACT_2D_ITEMS) {
      ...
      if (!m_activeItems.empty()) {
        ...
        // DGM TODO: to activate only if some items take left clicks into
        // account! for (std::set<ccInteractor*>::iterator
        // it=m_activeItems.begin(); it!=m_activeItems.end(); ++it) if
        // ((*it)->acceptClick(x,y,Qt::LeftButton))
        //{
        //	event->accept();
        //	redraw();
        //	return;
...
void ccGLWindowInterface::renderNextLODLevel() {
  ccLog::PrintDebug(
      QString("[renderNextLODLevel] About to draw new LOD level?"));
  m_LODPendingRefresh = false;
  if (m_currentLODState.inProgress && m_currentLODState.level != 0 &&
      !m_LODPendingIgnore) {
    ccLog::PrintDebug(
        QString("[renderNextLODLevel] Level %1 - index %2 confirmed")
            .arg(m_currentLODState.level)
            .arg(m_currentLODState.startIndex));
    QApplication::processEvents();
    requestUpdate();
  } else {
  ...
  if (state) {
    ...
    if (!m_exclusiveFullscreen) {
      ...
      if (widget)
        ...
        doShowFullScreen();
      displayNewMessage("Press F11 to disable full-screen mode",
                        ccGLWindow::UPPER_CENTER_MESSAGE, false, 30,
                        FULL_SCREEN_MESSAGE);
    }
  } else {
    if (m_stereoModeEnabled &&
    ...
    if (m_exclusiveFullscreen) {
      ...
      displayNewMessage(QString(), ccGLWindow::UPPER_CENTER_MESSAGE, false, 0,
                        FULL_SCREEN_MESSAGE); // remove any message
      if (widget) {
        widget->showNormal();
  ...
  if ((event->buttons() & Qt::RightButton)
    ...
    if ((m_interactionFlags & INTERACT_PAN) ||
        ((QApplication::keyboardModifiers() & Qt::ControlModifier) &&
         (m_interactionFlags & INTERACT_CTRL_PAN))) {
...
void ccGLWindowInterface::processMouseMoveEvent(QMouseEvent *event) {
  ...
  if (s_counter == 0) {
    ...
    if (elapsed_ms >= 1000) {
      ccLog::Print("mouseMoveEvent frequency: " +
                   QString::number(s_counter / (elapsed_ms / 1000.0), 'f', 2) +
                   QString(" Hz (mouse tracking: %1)")
                       .arg(hasMouseTracking() ? "ON" : "OFF"));
      s_timer.restart();
      s_counter = 0;
    }
  }
  ...
  if (event->buttons() == Qt::NoButton) {
    if (m_interactionFlags & INTERACT_CLICKABLE_ITEMS) {
      // what would be the size of the 'hot zone' if it was displayed with all
      // options
      if (!m_hotZone) {
        m_hotZone = new HotZone(this);
      }
      QRect areaRect =
          m_hotZone->rect(true, m_bubbleViewModeEnabled, exclusiveFullScreen());

      const int retinaScale = getDevicePixelRatio();
      bool inZone =
          (x * retinaScale * 3 <
               m_hotZone->topCorner.x() + areaRect.width() * 4 // 25% margin
           && y * retinaScale * 2 < m_hotZone->topCorner.y() +
    ...
    if (m_showCursorCoordinates) {
      ...
      this->displayNewMessage(message, LOWER_LEFT_MESSAGE, false, 2,
                              SCREEN_SIZE_MESSAGE);
      redraw(true, false);
    }
  ...
  if ((event->buttons() & Qt::RightButton)
    ...
    if (m_interactionFlags & INTERACT_PAN) {
      ...
      CCVector3d u(dx * pixSize / m_displayScale.x,
                   -dy * pixSize / m_displayScale.y, 0.0);

      const int retinaScale = getDevicePixelRatio();
      u *= retinaScale;

      bool entityMovingMode =
          (m_interactionFlags & INTERACT_TRANSFORM_ENTITIES) ||
          ((QApplication::keyboardModifiers() & Qt::ControlModifier) &&
  ...
  } else if (event->buttons() & Qt::LeftButton) // rotation
  {
    if (m_interactionFlags & INTERACT_2D_ITEMS) {
      ...
      if (!m_mouseMoved) {
        if (m_pickingMode != NO_PICKING
            // DGM: in fact we still need to move labels in those modes below
            // (see the 'Point Picking' tool of CloudCompare for instance)
    ...
    if (!m_activeItems.empty()) {
      ...
      for (auto &activeItem : m_activeItems) {
        if (activeItem->move2D(x * retinaScale, y * retinaScale,
                               dx * retinaScale, dy * retinaScale, glWidth(),
                               glHeight())) {
          invalidateViewport();
      ...
      if (m_allowRectangularEntityPicking &&
        ...
        if (!m_rectPickingPoly) {
          ...
          if (vertices->reserve(4) && m_rectPickingPoly->addPointIndex(0, 4)) {
            ...
            QPointF posA =
                toCenteredGLCoordinates(m_lastMousePos.x(), m_lastMousePos.y());

            CCVector3 A(static_cast<PointCoordinateType>(posA.x()),
                        static_cast<PointCoordinateType>(posA.y()), 0);
            ...
            ccLog::Warning("[ccGLWindow] Failed to create seleciton polyline! "
                           "Not enough memory!");
            delete m_rectPickingPoly;
            m_rectPickingPoly = nullptr;
            vertices = nullptr;
        ...
        if (m_rectPickingPoly) {
          CCCoreLib::GenericIndexedCloudPersist *vertices =
              m_rectPickingPoly->getAssociatedCloud();
          assert(vertices);
          CCVector3 *B =
              const_cast<CCVector3 *>(vertices->getPointPersistentPtr(1));
          CCVector3 *C =
              const_cast<CCVector3 *>(vertices->getPointPersistentPtr(2));
          CCVector3 *D =
              const_cast<CCVector3 *>(vertices->getPointPersistentPtr(3));
      ...
      } else if (m_interactionFlags &
                 INTERACT_ROTATE) // standard rotation around the current pivot
      {
        ...
        case BubbleViewMode: {
          ...
          if (std::abs(posDelta.x()) != 0) {
            double delta_deg =
                (posDelta.x() * static_cast<double>(m_bubbleViewFov_deg)) /
                height();
            // rotation about the sensor Z axis
            CCVector3d axis = m_viewportParams.viewMat.getColumnAsVec3D(2);
            rotMat.initFromParameters(CCCoreLib::DegreesToRadians(delta_deg),
                                      axis, CCVector3d(0, 0, 0));
          }

          if (std::abs(posDelta.y()) != 0) {
            double delta_deg =
                (posDelta.y() * static_cast<double>(m_bubbleViewFov_deg)) /
                height();
            // rotation about the local X axis
            ccGLMatrixd rotX;
            rotX.initFromParameters(CCCoreLib::DegreesToRadians(delta_deg),
        ...
        case StandardMode: {
          ...
          if (QApplication::keyboardModifiers() & Qt::ShiftModifier) {
            ...
            rotMat.initFromParameters(angle_rad, CCVector3d(0, 0, 1),
                                      CCVector3d(0, 0, 0));
          } else {
            if (!m_mouseMoved) {
              // on the first time, we must compute the previous orientation
              // (the camera hasn't moved yet)
              s_lastMouseOrientation = convertMousePositionToOrientation(
                  m_lastMousePos.x(), m_lastMousePos.y());
            }
            // unconstrained rotation following mouse position
            rotMat = ccGLMatrixd::FromToRotation(s_lastMouseOrientation,
        ...
        case LockedAxisMode: {
          ...
          if (topView) {
            ...
            CCVector3d previousMousePos(
                ...
                static_cast<double>(height() - m_lastMousePos.y()), 0.0);
            CCVector3d currentMousePos(static_cast<double>(x),
          ...
          } else // side view
          {
            ...
            if (camera.project(m_viewportParams.getPivotPoint(), A2D) &&
                camera.project(m_viewportParams.getPivotPoint() +
                                   m_viewportParams.zFar * m_lockedRotationAxis,
                               B2D)) {
              ...
              mouseShift -=
                  ...
                  lockedRotationAxis2D; // we only keep the orthogonal part
              double angle_rad =
                  2.0 * M_PI * mouseShift.norm() / (width() + height());
        ...
        if (m_interactionFlags & INTERACT_TRANSFORM_ENTITIES) {
          rotMat = m_viewportParams.viewMat.transposed() * rotMat *
  ...
  if ((event->button() == Qt::RightButton)
    ...
    if (mouseHasMoved) {
      ...
      if (m_rectPickingPoly) {
        ...
        PickingParameters params(ENTITY_RECT_PICKING, pickX + width() / 2,
                                 height() / 2 - pickY, pickW, pickH);
        startPicking(params);
      }
      ...
      // picking?
      if (m_timer.elapsed() <
          m_lastClickTime_ticks + CC_MAX_PICKING_CLICK_DURATION_MS) // in msec
      {
        int x = m_lastMousePos.x();
        int y = m_lastMousePos.y();
        ...
        if (!processClickableItems(x, y)) {
          m_lastMousePos = event->pos(); // just in case (it should be already
                                         // at this position)
          const ccGui::ParamStruct &displayParams = getDisplayParameters();
...
void ccGLWindowInterface::processWheelEvent(QWheelEvent *event) {
  ...
  if (keyboardModifiers & Qt::AltModifier) {
    ...
    double increment =
        (event->delta() < 0 ? -1.0 : 1.0) * computeDefaultIncrement();
    bool shiftPressed = (keyboardModifiers & Qt::ShiftModifier);
    if (shiftPressed) {
      // if ((increment < 0.0) ||
      // (!std::isnan(m_viewportParams.farClippingDepth)))
      {
        double farClippingDepth = (std::isnan(m_viewportParams.farClippingDepth)
                                       ? m_viewportParams.zFar
                                       : m_viewportParams.farClippingDepth);
        if (setFarClippingPlaneDepth(
                std::max(0.0, farClippingDepth + increment))) {
          doRedraw = true;
        }
      }
    } else {
      if ((increment > 0.0) ||
          (!std::isnan(m_viewportParams.nearClippingDepth))) {
        double nearClippingDepth =
            (std::isnan(m_viewportParams.nearClippingDepth)
                 ? m_viewportParams.zNear
                 : m_viewportParams.nearClippingDepth);
        if (setNearClippingPlaneDepth(
                std::max(0.0, nearClippingDepth + increment))) {
          doRedraw = true;
        }
      }
    }
    ...
    // see QWheelEvent documentation ("distance that the wheel is rotated, in
    // eighths of a degree")
...
static void glDrawUnitCircle(QOpenGLContext *context, unsigned char dim,
                             unsigned steps = 64) {
  assert(context);
...
void ccGLWindowInterface::drawPivot() {
  ...
  double symbolRadius =
      CC_DISPLAYED_PIVOT_RADIUS_PERCENT * std::min(glWidth(), glHeight()) / 2.0;
  ...
  if (m_pivotGLList == GL_INVALID_LIST_ID) {
    ...
    const ColorCompType c_alpha =
        static_cast<ColorCompType>(ccColor::MAX * 0.6f);
...
bool ccGLWindowInterface::enableStereoMode(const StereoParams &params) {
  ...
  if (params.glassType == StereoParams::NVIDIA_VISION ||
      params.glassType == StereoParams::GENERIC_STEREO_DISPLAY) {
    if (!isStereo()) {
    ...
    if (!isQuadBufferSupported()) {
      QMessageBox::critical(asWidget(), "Stereo",
...
bool ccGLWindowInterface::isQuadBufferSupported() const {
  ...
  return (surfaceFormat.stereo() &&
          surfaceFormat.swapBehavior() == QSurfaceFormat::DoubleBuffer);
}

void ccGLWindowInterface::Create(ccGLWindowInterface *&window, QWidget *&widget,
...
ccGLWindowInterface *ccGLWindowInterface::FromEmitter(QObject *object) {
  ccGLWindowSignalEmitter *emitter =
      qobject_cast<ccGLWindowSignalEmitter *>(object);
  if (!emitter) {
    ccLog::Warning(QString("[ccGLWindowInterface::FromEmitter] Object %1 is "
                           "not a window signal emitter")
...
ccGLWindowInterface *ccGLWindowInterface::FromQObject(QObject *object) {
  ...
  ccLog::Warning(QString("[ccGLWindowInterface::FromQObject] Object %1 is not "
                         "a valid GL window")
                     .arg(object->objectName()));
  return nullptr;
}
...
bool ccGLWindowInterface::TestStereoSupport(bool forceRetest /*=false*/) {
  ...
  ccLog::Print(QString("Quad Buffered Stereo mode: %1")
                   .arg(isStereoEnabled ? "supported" : "not supported"));

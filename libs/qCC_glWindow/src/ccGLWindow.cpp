...
ccGLWindow::ccGLWindow(QSurfaceFormat *format /*=nullptr*/,
    ...
    : QOpenGLWidget(parent), ccGLWindowInterface(this, silentInitialization) {
  m_font = font();
  ...
  connect(m_signalEmitter, &ccGLWindowSignalEmitter::itemPickedFast, this,
          &ccGLWindow::onItemPickedFastSlot, Qt::DirectConnection);
...
bool ccGLWindow::enableStereoMode(const StereoParams &params) {
  if (params.glassType == StereoParams::OCULUS) {
    QMessageBox::critical(
        asWidget(), "Oculus",
        "The Oculus device is not supported by this type of 3D view");
    return false;
  } else {
...
void ccGLWindow::Create(ccGLWindow *&window, QWidget *&widget,
                        bool silentInitialization /*=false*/) {

...
ccGLWindowStereo::ccGLWindowStereo(QSurfaceFormat *format /*=nullptr*/,
    ...
    : QWindow(parent), ccGLWindowInterface(this, silentInitialization),
      m_context(nullptr), m_parentWidget(nullptr) {
  ...
  connect(m_signalEmitter, &ccGLWindowSignalEmitter::itemPickedFast, this,
          &ccGLWindowStereo::onItemPickedFastSlot, Qt::DirectConnection);
...
void ccGLWindowStereo::grabMouse() { setMouseGrabEnabled(true); }

void ccGLWindowStereo::releaseMouse() { setMouseGrabEnabled(false); }
...
GLuint ccGLWindowStereo::defaultQtFBO() const { return 0; }
...
bool ccGLWindowStereo::initPaintGL() {
  ...
  glFunc->glViewport(m_glViewport.x(), m_glViewport.y(), m_glViewport.width(),
                     m_glViewport.height());
...
bool ccGLWindowStereo::prepareOtherStereoGlassType(
    CC_DRAW_CONTEXT &CONTEXT, RenderingParams &renderingParams,
  ...
  if (m_stereoParams.glassType == StereoParams::OCULUS && s_oculus.session) {
    ...
    if (renderingParams.pass == MONO_OR_LEFT_RENDERING_PASS) {
      ...
      double displayMidpointSeconds =
          ovr_GetPredictedDisplayTime(s_oculus.session, 0);
      // Query the HMD for the current tracking state.
      ovrTrackingState hmdState = ovr_GetTrackingState(
          s_oculus.session, displayMidpointSeconds, ovrTrue);
      if (hmdState.StatusFlags &
          (ovrStatus_OrientationTracked | ovrStatus_PositionTracked)) {
        // compute the eye positions
        ovr_CalcEyePoses(hmdState.HeadPose.ThePose, s_oculus.hmdToEyeViewOffset,
                         s_oculus.layer.RenderPose);
      ...
      ovr_GetTextureSwapChainCurrentIndex(
          s_oculus.session, s_oculus.textureSwapChain, &currentIndex);

      unsigned int colorTexID = 0;
      ovr_GetTextureSwapChainBufferGL(s_oculus.session,
                                      s_oculus.textureSwapChain, currentIndex,
                                      &colorTexID);
...
void ccGLWindowStereo::processOtherStereoGlassType(
    RenderingParams &renderingParams) {
#ifdef CC_OCULUS_SUPPORT
  if (m_stereoModeEnabled && m_stereoParams.glassType == StereoParams::OCULUS &&
      s_oculus.session && renderingParams.pass == RIGHT_RENDERING_PASS) {
    ...
    ovrResult result =
        ovr_SubmitFrame(s_oculus.session, 0, nullptr, &layers, 1);
    ...
    if (s_oculus.mirror.texture) {
      ...
      m_glExtFunc.glBlitFramebuffer(0, oh, ow, 0, sx, sy, sx + sw, sy + sh,
...
bool ccGLWindowStereo::setCustomCameraProjection(
    RenderingParams &renderingParams, ccGLMatrixd &modelViewMat,
  ...
  if (m_stereoParams.glassType == StereoParams::OCULUS && s_oculus.session) {
    ...
    if (s_oculus.hasLastOVRPos) {
      ...
      const ovrVector3f &P =
          s_oculus.layer.RenderPose[renderingParams.pass].Position;
      sensorMat.setTranslation(sensorMat.getTranslationAsVec3D() +
                               CCVector3d(P.x, P.y, P.z));
    ...
    OVR::Matrix4f proj =
        ovrMatrix4f_Projection(s_oculus.layer.Fov[renderingParams.pass],
...
bool ccGLWindowStereo::enableStereoMode(const StereoParams &params) {
  if (params.glassType == StereoParams::OCULUS) {
    ...
    if (!s_oculus.session) {
      ...
      if (OVR_FAILURE(result)) {
        QMessageBox::critical(
            asWidget(), "Oculus",
            "Failed to initialize the Oculus SDK (ovr_Initialize)");
        return false;
      }
      ...
      if (OVR_FAILURE(result)) {
        QMessageBox::critical(
            asWidget(), "Oculus",
            "Failed to initialize the Oculus SDK (ovr_Create)");
        ovr_Shutdown();
        return false;
      }
      ...
      ccLog::Print(QString("[Oculus] HMD '%0' detected (resolution: %1 x %2)")
                       .arg(desc.ProductName)
                       .arg(desc.Resolution.w)
                       .arg(desc.Resolution.h));
    ...
    if (!s_oculus.initTextureSet(context())) {
      QMessageBox::critical(asWidget(), "Oculus",
                            "Failed to initialize the swap texture set "
    ...
    displayNewMessage("Look into your headset",
                      ccGLWindowStereo::SCREEN_CENTER_MESSAGE, false, 3600);
    ...
    QMessageBox::critical(asWidget(), "Oculus",
                          "The Oculus device is not supported by this "
...
void ccGLWindowStereo::disableStereoMode() {
  ...
  if (m_stereoModeEnabled) {
    if (m_stereoParams.glassType == StereoParams::OCULUS) {
      ...
      displayNewMessage(QString(), ccGLWindowStereo::SCREEN_CENTER_MESSAGE,
                        false);
...
void ccGLWindowStereo::Create(ccGLWindowStereo *&window, QWidget *&widget,
                              bool silentInitialization /*=false*/) {

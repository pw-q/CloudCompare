...
QVariant ccAsprsModel::headerData(int section, Qt::Orientation orientation,
                                  int role) const {
...
bool ccAsprsModel::setData(const QModelIndex &index, const QVariant &value,
                           int role) {
  Q_UNUSED(role)
...
static void AddClass(QSettings &settings, const QString &className,
                     int classValue, QColor classColor, bool visible = true) {
...
static void ReadClass(const QSettings &settings, const QString &className,
...
bool ccAsprsModel::removeRows(int position, int rows,
...
int ccAsprsModel::getUnusedCode() const {
  auto it = std::max_element(
      ...
      [](const AsprsItem &a, const AsprsItem &b) { return a.code < b.code; });

...
qAnimationDlg::qAnimationDlg(ccGLWindowInterface *view3d, QWidget *parent)
  ...
  QString defaultOutputFormat;
  {
    ...
    // other parameters
    {
      ...
      int superRes =
          settings.value("superRes", superResolutionSpinBox->value()).toInt();
      int renderingMode =
          settings.value("renderingMode", renderingModeComboBox->currentIndex())
              .toInt();
      int bitRate = settings.value("bitRate", bitrateSpinBox->value()).toInt();
      bool autoStepDuration =
          settings
              .value("autoStepDuration", autoStepDurationCheckBox->isChecked())
              .toBool();
      bool smoothTrajectory =
          settings
              .value("smoothTrajectory", smoothTrajectoryGroupBox->isChecked())
              .toBool();
      double smoothRatio =
          settings.value("smoothRatio", smoothRatioDoubleSpinBox->value())
              .toDouble();
      ...
      autoStepDurationCheckBox->setChecked(
          autoStepDuration); // this might be modified when init will be called!
  ...
  connect(smoothRatioDoubleSpinBox,
          ...
          &qAnimationDlg::onSmoothRatioChanged);

  connect(fpsSpinBox, qOverload<int>(&QSpinBox::valueChanged), this,
          &qAnimationDlg::onFPSChanged);
  connect(totalTimeDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &qAnimationDlg::onTotalTimeChanged);
  connect(stepTimeDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &qAnimationDlg::onStepTimeChanged);
  ...
  connect(outputFormatComboBox, qOverload<int>(&QComboBox::currentIndexChanged),
...
bool qAnimationDlg::init(const std::vector<ExtendedViewport> &viewports) {
  ...
  for (size_t i = 0; i < viewports.size(); ++i) {
    ...
    // check if the (1st) viewport has a duration in meta data (from a previous
    // run)
...
bool qAnimationDlg::smoothTrajectory(double ratio, unsigned iterationCount) {
  ...
  try {
      ...
      for (size_t i = 0; i < segmentCount; ++i) {
        ...
        if (!openPoly || i != 0) {
          ...
          interpolatedStep.cameraCenter =
              (CCCoreLib::PC_ONE - ratio) * sP.cameraCenter +
        ...
        if (!openPoly || i + 1 != segmentCount) {
          ...
          interpolatedStep.cameraCenter =
              ...
              (CCCoreLib::PC_ONE - ratio) * sQ.cameraCenter;
          interpolatedStep.duration_sec =
              sP.duration_sec * (CCCoreLib::PC_ONE - ratio);
    ...
    for (size_t i = 0; i < smoothSegmentCount; ++i) {
      CCVector3d d =
          m_smoothVideoSteps[(i + 1) % m_smoothVideoSteps.size()].cameraCenter -
    ...
    for (size_t i = 0; i < m_smoothVideoSteps.size(); ++i) {
      ...
      if (s.indexInOriginalTrajectory != -1) {
        assert(
            m_videoSteps[s.indexInOriginalTrajectory].indexInSmoothTrajectory <
            0);
        m_videoSteps[s.indexInOriginalTrajectory].indexInSmoothTrajectory =
            static_cast<int>(i);
      }
...
void qAnimationDlg::onAutoStepsDurationToggled(bool state) {
  ...
  while (getNextSegment(vp1Index, vp2Index)) {
    ...
    if (CCCoreLib::LessThanEpsilon(length)) {
      if (CCCoreLib::LessThanEpsilon(referenceLength)) {
        ...
        step1.duration_sec =
            totalTimeDoubleSpinBox->value() / referenceTrajectory->size();
      } else {
        // only this step
      ...
      step1.duration_sec =
          totalTimeDoubleSpinBox->value() * (length / referenceLength);
  ...
  ccLog::PrintDebug(QString("Total time = %1 / Smooth time = %2")
                        .arg(totalTime)
                        .arg(totalTimeSmooth));
...
ccPolyline *qAnimationDlg::getTrajectory() {
  const Trajectory &trajectory =
      smoothTrajectoryGroupBox->isChecked() ? m_smoothVideoSteps : m_videoSteps;
  ...
  for (const Step &step : trajectory) {
    ...
    if (vertices->size() != 0) {
      ...
      if (!CCCoreLib::GreaterThanEpsilon(
              (*vertices->getPoint(vertices->size() - 1) - C).norm())) {
        continue;
      }
    }
    vertices->addPoint(C);
  ...
  polyline->setDisplay_recursive(
      m_videoSteps.front()
          .viewport->getDisplay()); // warning the smoothed version has no valid
                                    // 'viewport'
...
void qAnimationDlg::applyViewport(
    const ExtendedViewportParameters &viewportParameters) {
  if (m_view3d) {
...
void qAnimationDlg::preview() {
  ...
  if (smoothMode) {
    ...
    vp1Index =
        static_cast<size_t>(m_videoSteps[vp1Index].indexInSmoothTrajectory);
  } else {
  ...
  QProgressDialog progressDialog(tr("Frames: %1").arg(frameCount), "Cancel", 0,
                                 frameCount, this);
...
void qAnimationDlg::render(bool asSeparateFrames) {
  ...
  if (!asSeparateFrames) {
    ...
    // hack: as the encoder requires that the video dimensions are multiples of
    // 8, we resize the window a little bit...
    {
    ...
    encoder.reset(
        new QVideoEncoder(outputFilename, m_view3d->glWidth() * animScale,
                          m_view3d->glHeight() * animScale, bitrate, gop,
    ...
    if (!success) {
      QMessageBox::critical(
          ...
          QString("Failed to open file for output: %1")
              .arg(errors.back())); // display the last error message
      setEnabled(true);
      return;
    }
  }
  ...
  for (int frameIndex = 0; frameIndex < frameCount;) {
    ...
    if (deltaTime <= step1.duration_sec) {
      ...
      // render to image
      QImage image =
          m_view3d->renderToImage(superRes, renderingMode == ZOOM, false, true);
      ...
      if (renderingMode == SUPER_RESOLUTION && superRes > 1) {
        image =
            image.scaled(image.width() / superRes, image.height() / superRes,
      ...
      if (asSeparateFrames) {
        ...
        if (!image.save(fullPath)) {
          QMessageBox::critical(
              this, "Error",
              QString("Failed to save frame #%1").arg(frameIndex + 1));
          success = false;
          break;
        }
      } else {
...
void qAnimationDlg::updateCurrentStepDuration() {
  ...
  stepTimeDoubleSpinBox->setValue(index < 0 ? 0.0
                                            : m_videoSteps[index].duration_sec);
...
void qAnimationDlg::onCurrentStepChanged(int index) {
  ...
  if (index >= 0) {
    ...
    applyViewport(
        smoothModeEnabled()
            ? m_smoothVideoSteps[m_videoSteps[index].indexInSmoothTrajectory]
            : m_videoSteps[index]);
  }
  ...
  currentStepGroupBox->setEnabled(
      isEnabled && ((index >= 0 && index + 1 < m_videoSteps.size()) || isLoop));
}

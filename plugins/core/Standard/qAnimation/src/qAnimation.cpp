...
static ViewPortList
GetSelectedViewPorts(const ccHObject::Container &selectedEntities) {
  ViewPortList viewports;

  for (ccHObject *object : selectedEntities) {
    if (object->getClassID() ==
        static_cast<CC_CLASS_ENUM>(CC_TYPES::VIEWPORT_2D_OBJECT)) {
...
qAnimation::qAnimation(QObject *parent)
    : QObject(parent), ccStdPluginInterface(":/CC/plugin/qAnimation/info.json"),
      m_action(nullptr) {}
...
void qAnimation::doAction() {
  ...
  Q_ASSERT(viewports.size() >=
           2); // action will not be active unless we have at least 2 viewports

  m_app->dispToConsole(
      QString("[qAnimation] Selected viewports: %1").arg(viewports.size()));

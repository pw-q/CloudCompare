...
StatisticalOutliersRemover::StatisticalOutliersRemover()
    : BaseFilter(FilterDescription(
          ...
          "Filter the points that are farther of their neighbors than the "
          "average (plus a number of times the standard deviation)",
...
int StatisticalOutliersRemover::compute() {
  ...
  final_cloud->setName(
      QString("%1_k%2_std%3").arg(cloud->getName()).arg(m_kNN).arg(m_std));

...
FastGlobalRegistrationDialog::FastGlobalRegistrationDialog(
    const std::vector<ccPointCloud *> &allClouds, QWidget *parent /*=nullptr*/)
    : QDialog(parent, Qt::Tool), Ui::FastGlobalRegistrationDialog(),
      clouds(allClouds),
...
void FastGlobalRegistrationDialog::updateGUI() {
  ...
  alignedLineEdit->setText(clouds.size() == 2
                               ? alignedCloud->getName()
...
void FastGlobalRegistrationDialog::autoEstimateRadius() {
  ...
  for (ccPointCloud *cloud : clouds) {
    ...
    if (radius < 0) {
      ccLog::Error(tr("Failed to estimate the radius for cloud %1")
                       .arg(cloud->getName()));
      return;
    }

...
MLSDialog::MLSDialog(QWidget *parent) : QDialog(parent), Ui::MLSDialog() {
  ...
  connect(upsampling_method, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &MLSDialog::activateMenu);
  connect(search_radius, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &MLSDialog::updateSquaredGaussian);
}

void MLSDialog::updateCombo() {
  upsampling_method->clear();
  upsampling_method->addItem(tr("None"), QVariant(MLSParameters::NONE));
  upsampling_method->addItem(tr("Sample Local Plane"),
                             QVariant(MLSParameters::SAMPLE_LOCAL_PLANE));
  upsampling_method->addItem(tr("Random Uniform Density"),
                             QVariant(MLSParameters::RANDOM_UNIFORM_DENSITY));
  upsampling_method->addItem(tr("Voxel Grid Dilation"),

...
MLSSmoothingUpsampling::MLSSmoothingUpsampling()
    : BaseFilter(FilterDescription(
          "MLS smoothing", "Smooth using MLS, optionally upsample",
          "Smooth the cloud using Moving Least Sqares algorithm, estimate "
          "normals and optionally upsample",
...
int MLSSmoothingUpsampling::compute() {
  ...
  pcl::PointCloud<pcl::PointNormal>::Ptr rawCloudWithNormals(
      new pcl::PointCloud<pcl::PointNormal>);
#ifdef LP_PCL_PATCH_ENABLED
  pcl::PointIndicesPtr mappingIndices;
  SmoothMLS<pcl::PointXYZ, pcl::PointNormal>(
      xyzCloud, m_parameters, rawCloudWithNormals, mappingIndices);
#else
  SmoothMLS<pcl::PointXYZ, pcl::PointNormal>(xyzCloud, m_parameters,
                                             rawCloudWithNormals);
#endif
  ...
  outputCCCloud->setName(cloud->getName() +
...
int MLSSmoothingUpsampling::getParametersFromDialog() {
  ...
  m_parameters.upsample_method_ =
      static_cast<MLSParameters::UpsamplingMethod>(status);
...
template int SmoothMLS<pcl::PointXYZ, pcl::PointNormal>(
    const pcl::PointCloud<pcl::PointXYZ>::Ptr &incloud,
    const MLSParameters &params,

...
FastGlobalRegistrationFilter::FastGlobalRegistrationFilter()
    : BaseFilter(FilterDescription(
          ...
          "Attempts to automatically register clouds (with normals) without "
          "initial alignment",
          ":/toolbar/PclUtils/icons/fastGlobalRegistration.png")),
      m_alignedClouds(), m_referenceCloud(nullptr), m_featureRadius(0) {}
...
static bool ComputeFeatures(ccPointCloud *cloud, fgr::Features &features,
                            double radius) {
  if (!cloud) {
    assert(false);
    return false;
...
int FastGlobalRegistrationFilter::getParametersFromDialog() {
  ...
  FastGlobalRegistrationDialog dialog(clouds,
  ...
  for (ccPointCloud *alignedCloud : m_alignedClouds) {
    ...
    try {
      ...
      if (!fgrProcess.OptimizePairwise(true)) {
        ccLog::Warning(
            "Failed to perform pair-wise optimization (not enough points)");
        return ComputationError;
      }
    ...
    ccLog::Print(tr("[Fast Global Registration] Resulting matrix for cloud %1")
                     .arg(alignedCloud->getName()));
    ccLog::Print(ccTrans.toString(12, ' ')); // full precision
    ccLog::Print(tr("Hint: copy it (CTRL+C) and apply it - or its inverse - on "

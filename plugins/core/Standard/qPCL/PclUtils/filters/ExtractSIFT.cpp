...
ExtractSIFT::ExtractSIFT()
    : BaseFilter(FilterDescription("Extract SIFT", "Extract SIFT Keypoints",
                                   "Extract SIFT keypoints for clouds with "
                                   "intensity/RGB or any scalar field",
                                   ":/toolbar/PclUtils/icons/sift.png")),
      m_nr_octaves(0), m_min_scale(0), m_nr_scales_per_octave(0),
      m_min_contrast(0), m_use_min_contrast(false), m_mode(RGB) {}
...
int ExtractSIFT::getParametersFromDialog() {
  ...
  m_min_contrast = m_use_min_contrast
                       ? static_cast<float>(dialog.minContrast->value())
                       : 0.0f;
  ...
  m_field_to_use_no_space =
      fieldname.toStdString(); // DGM: warning, toStdString doesn't preserve
                               // "local" characters
...
int ExtractSIFT::compute() {
  ...
  PCLCloud::Ptr sm_cloud =
      cc2smReader(cloud).getAsSM(true, false, m_mode == RGB, scalarFields);
  if (!sm_cloud) {
  ...
  if (m_mode == SCALAR_FIELD) {
    ...
    sm_cloud->fields[field_index].name =
        "intensity"; // we always use intensity as name... even if it is
                     // curvature or another field.
  }
  ...
  if (m_mode == SCALAR_FIELD) {
    ...
    EstimateSIFT<pcl::PointXYZI, pcl::PointXYZ>(
        cloud_i, out_cloud, m_nr_octaves, m_min_scale, m_nr_scales_per_octave,
        m_min_contrast);
  } else if (m_mode == RGB) {
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_rgb(
        new pcl::PointCloud<pcl::PointXYZRGB>);
    FROM_PCL_CLOUD(*sm_cloud, *cloud_rgb);
    EstimateSIFT<pcl::PointXYZRGB, pcl::PointXYZ>(
        cloud_rgb, out_cloud, m_nr_octaves, m_min_scale, m_nr_scales_per_octave,
        m_min_contrast);
  }
  ...
  if (m_mode == RGB)
    name << "SIFT Keypoints_" << m_nr_octaves << "_"
         << "rgb"
         << "_" << m_min_scale << "_" << m_nr_scales_per_octave << "_"
         << m_min_contrast;
  else
    name << "SIFT Keypoints_" << m_nr_octaves << "_" << m_field_to_use_no_space
         << "_" << m_min_scale << "_" << m_nr_scales_per_octave << "_"
         << m_min_contrast;
...
QString ExtractSIFT::getErrorMessage(int errorCode) const {
  ...
  case NoSForRGB:
    return tr("Selected entity does not have any suitable scalar field or RGB. "
              "Intensity scalar field or RGB are needed for computing SIFT");
  case EmptyOutput:
    return tr("SIFT keypoint extraction did not return any point. Try relaxing "
              "your parameters");
  default:
    // see below
    break;

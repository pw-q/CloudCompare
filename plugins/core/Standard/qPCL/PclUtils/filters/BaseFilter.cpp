...
BaseFilter::BaseFilter(FilterDescription desc, ccMainAppInterface *app)
    : m_desc(desc), m_action(new QAction(desc.icon, desc.entryName, this)),
      m_app(app), m_showProgress(true) {
...
void BaseFilter::throwError(int errCode) {
  ...
  if (errCode == CancelledByUser) {
    ...
    // DGM: as libraries shouldn't issue message themselves, it should be sent
    // to the plugin via a signal
    Q_EMIT newErrorMessage(errMsg);
  }
}

void BaseFilter::updateSelectedEntities(
...
bool BaseFilter::checkSelected() const {
  ...
  return (m_selectedEntities.size() == 1 &&
...
QString BaseFilter::getErrorMessage(int errorCode) const {
  ...
  default:
    return tr("Undefined error in filter %1: %2")
        .arg(m_desc.filterName)
        .arg(errorCode);
  }
...
void BaseFilter::getSelectedEntitiesThatAreCCPointCloud(
    ccHObject::Container &entities) const {
  getSelectedEntitiesThatAre(CC_TYPES::POINT_CLOUD, entities);
}

void BaseFilter::getSelectedEntitiesThatAre(
    CC_CLASS_ENUM kind, ccHObject::Container &entities) const {

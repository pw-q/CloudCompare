...
int NormalEstimation::compute() {
  ...
  int result = ComputeNormals<pcl::PointXYZ, pcl::PointNormal>(
      xyzCloud, m_useKnn ? m_knn_radius : m_radius, m_useKnn,
      rawCloudWithNormals);
  if (result < 0) {
    return ComputationError;
  }
  ...
  // FIXME: find a way to avoid converting to a PCLPointCloud2 just to get the
  // curvature values!
  PCLCloud::Ptr cloudWithNormals(new PCLCloud);
  TO_PCL_CLOUD(rawCloudWithNormals, *cloudWithNormals);
  pcl2cc::CopyScalarField(*cloudWithNormals, "curvature", *cloud,

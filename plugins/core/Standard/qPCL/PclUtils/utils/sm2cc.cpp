...
POINT_CLOUD_REGISTER_POINT_STRUCT(PointXYZTpl<std::int32_t>,
                                  (std::int32_t, x, x)(std::int32_t, y,
                                                       y)(std::int32_t, z, z))

POINT_CLOUD_REGISTER_POINT_STRUCT(PointXYZTpl<std::int16_t>,
                                  (std::int16_t, x, x)(std::int16_t, y,
                                                       y)(std::int16_t, z, z))
...
template <class T>
void PCLCloudToCCCloud(
    const PCLCloud &pclCloud, ccPointCloud &ccCloud,
  ...
  for (size_t i = 0; i < pointCount; ++i) {
    CCVector3d P(pcl_cloud.at(i).x, pcl_cloud.at(i).y, pcl_cloud.at(i).z);
    ...
    if (_loadParameters) {
      if (i == 0) // first point
      {
        ...
        if (FileIOFilter::HandleGlobalShift(P, Pshift, preserveCoordinateShift,
          ...
          ccLog::Warning("[PCL-to-CC] PCL cloud has been recentered! "
                         "Translation: (%.2f ; %.2f ; %.2f)",
                         Pshift.x, Pshift.y, Pshift.z);

          if (_transform) {
            // we update the input transform so as to shift the points already
            // 'at the source'
            _transform->setTranslation(_transform->getTranslationAsVec3D() +
                                       Pshift);
...
bool pcl2cc::CopyXYZ(
    const PCLCloud &pclCloud, ccPointCloud &ccCloud, uint8_t coordinateType,
    ccGLMatrixd *_transform /*=nullptr*/,
    FileIOFilter::LoadParameters *_loadParameters /*=nullptr*/) {
  size_t pointCount = GetNumberOfPoints(pclCloud);
  if (pointCount == 0) {
  ...
  case pcl::PCLPointField::INT16:
    PCLCloudToCCCloud<PointXYZTpl<std::int16_t>>(pclCloud, ccCloud, _transform,
                                                 _loadParameters);
    break;
  case pcl::PCLPointField::INT32:
    PCLCloudToCCCloud<PointXYZTpl<std::int32_t>>(pclCloud, ccCloud, _transform,
                                                 _loadParameters);
    break;
  case pcl::PCLPointField::FLOAT32:
    PCLCloudToCCCloud<pcl::PointXYZ>(pclCloud, ccCloud, _transform,
                                     _loadParameters);
    break;
  case pcl::PCLPointField::FLOAT64:
    PCLCloudToCCCloud<PointXYZTpl<double>>(pclCloud, ccCloud, _transform,
                                           _loadParameters);
    break;
  default:
    ccLog::Warning("[PCL] Unsupported coordinate type " +
...
bool pcl2cc::CopyNormals(const PCLCloud &pclCloud, ccPointCloud &ccCloud) {
  ...
  for (size_t i = 0; i < pointCount; ++i) {
    CCVector3 N(
        static_cast<PointCoordinateType>(pcl_cloud_normals->at(i).normal_x),
...
bool pcl2cc::CopyScalarField(const PCLCloud &pclCloud,
                             const std::string &sfName, ccPointCloud &ccCloud,
  ...
  default:
    ccLog::Warning(QString("[PCL] Field with an unmanaged type (= %1)")
...
ccPointCloud *
pcl2cc::Convert(const PCLCloud &pclCloud, ccGLMatrixd *_transform /*=nullptr*/,
                FileIOFilter::LoadParameters *_loadParameters /*=nullptr*/) {
  ...
  if (expectedPointCount != 0) {
    // push points inside
    if (!CopyXYZ(pclCloud, *ccCloud, coordinateType, _transform,
                 _loadParameters)) {
      delete ccCloud;
      return nullptr;
    }

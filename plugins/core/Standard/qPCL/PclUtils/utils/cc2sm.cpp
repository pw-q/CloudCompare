...
cc2smReader::cc2smReader(ccPointCloud *cccloud) : m_ccCloud(cccloud) {
  assert(m_ccCloud);
}
...
PCLCloud::Ptr cc2smReader::getFloatScalarField(const QString &fieldName) const {
  ...
  try {
    ...
    // Now change the name of the scalar field -> we cannot have any space into
    // the field name NOTE this is a little trick to put any number of scalar
    // fields in a message PointCloud2 object We use a point type with a generic
    // scalar field named scalar. we load the scalar field and then we change the
    // name to the needed one
...
PCLCloud::Ptr cc2smReader::getAsSM() const {
  ...
  for (unsigned i = 0; i < m_ccCloud->getNumberOfScalarFields(); ++i) {
    scalarFields << QString::fromStdString(
        m_ccCloud->getScalarField(static_cast<int>(i))->getName());
  }

  return getAsSM(true, m_ccCloud->hasNormals(), m_ccCloud->hasColors(),
                 scalarFields);
}
...
PCLCloud::Ptr cc2smReader::getAsSM(bool xyz, bool normals, bool rgbColors,

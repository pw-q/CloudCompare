...
qPCL::qPCL(QObject *parent /*=nullptr*/)
    : QObject(parent), ccStdPluginInterface(":/toolbar/info.json") {}
...
QList<QAction *> qPCL::getActions() {
  if (m_filters.empty()) {
    ...
    // addFilter( new StatisticalOutliersRemover() ); //Now integrated in CC
...
int qPCL::addFilter(BaseFilter *filter) {
  ...
  connect(filter, &BaseFilter::entityHasChanged, this,
          &qPCL::handleEntityChange);
  connect(filter, &BaseFilter::newErrorMessage, this,

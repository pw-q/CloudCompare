...
bool PcdFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
CC_FILE_ERROR PcdFilter::saveToFile(ccHObject *entity, const QString &filename,
  ...
  if (outputFileFormat == AUTO) {
    if (nullptr == parameters.parentWidget) {
      ...
      if (dialog.exec()) {
        ...
        if (clickedButton == dialog.buttonBox->button(
  ...
  // search for a sensor as child (we take the first if there are several of
  // them)
  ...
  if (ccCloud->getGlobalScale() != 1.0) {
    ccLog::Warning(
        "[PCD] Can't restore the Global scale when exporting to PCD format");
  }
  ...
  if (sensor) {
    ...
    if (!ccGlobalShiftManager::NeedShift(trans - bbCenter)) {
      ...
      ori = Eigen::Quaternionf(eigrot)
                .normalized(); // need to normalize the output in case the input
                               // matrix is not purely 'orthogonal'

      // it happens that the saved pose matrix doesn't capture everything (such
      // as reflections) because of the quaternion form. Therefore we update the
      // input 'poseMat' with what we have actually saved
      {
        CCCoreLib::SquareMatrix transMat(sensorMatrix.data(), true);
        if (transMat.computeDet() < 0) {
          ccLog::Warning("[PCD] Sensor pose matrix seems to be a reflection. "
                         "Can't save it as a quaternion (required by the PCD "
                         "format). We will use the non-reflective form.");
        }
      ...
      ccLog::Warning("Sensor center is too far from the points. Can't restore "
                     "it without losing numerical accuracy...");
    }
  }
  ...
  if (ccCloud->size() == 0) {
    ...
    stream << QString(p.generateHeaderBinary(*pclCloud, pos, ori).c_str())
           << "DATA binary\n";
  ...
  try {
    ...
    case COMPRESSED_BINARY: {
      ...
      std::ofstream ofs(filename.toStdWString(),
                        std::wifstream::out | std::wifstream::binary);
#else
...
CC_FILE_ERROR PcdFilter::loadFile(const QString &filename, ccHObject &container,
  ...
  try {
    ...
    // To avoid issues with local characters, we will open the file 'manually'
    // and then provide the contents to PCL directly
#ifdef _MSC_VER
    std::ifstream ifs(filename.toStdWString(),
    ...
    if (pcdReader.readHeader(ifs, *inputCloud, origin, orientation, pcdVersion,
                             dataType, cloudDataIndex) < 0) {
      return CC_FERR_THIRD_PARTY_LIB_FAILURE;
    }
    ccLog::Print(QString("[PCL] PCD version: %1").arg(pcdVersion));

    unsigned pointCount =
        static_cast<unsigned>(inputCloud->width * inputCloud->height);
    ...
    if (dataType == 0) // ASCII format
    {
      ...
      if (pcdReader.readBodyASCII(ifs, *inputCloud, pcdVersion) <
          0) // DGM: warning, toStdString doesn't preserve "local" characters
      {
        return CC_FERR_THIRD_PARTY_LIB_FAILURE;
      }
    } else // Binary
    {
      ccLog::Print(QObject::tr("[PCL] Reading %1 binary PCD file...")
                       .arg(dataType == 2 ? QObject::tr("compressed")
      ...
      try {
        ...
        if (pcdReader.readBodyBinary(
                reinterpret_cast<unsigned char *>(buffer.data()), *inputCloud,
                pcdVersion, dataType == 2, cloudDataIndex) < 0) {
          return CC_FERR_THIRD_PARTY_LIB_FAILURE;
        }
      ...
      try {
        if (inputCloud->is_dense) {
          ...
          if (xIndex >= 0 && yIndex >= 0 && zIndex >= 0) {
            ...
            ccLog::Warning("Could not find the 'x', 'y' or 'z' field. Can't "
    ...
    // convert to CC cloud
    // DGM: it appears that the sensor orientation/translation has to be applied
    // manually to the points (probably because they just correspond to the raw
    // scan grid in the general case)
    ccPointCloud *ccCloud = pcl2cc::Convert(*inputCloud, &ccRot, &parameters);
    if (!ccCloud) {
      ccLog::Warning("[PCL] An error occurred while converting PCD cloud to "
                     "current cloud!");
    ...
    if (grid) {
      if (grid->validCount !=
          static_cast<unsigned>(inputCloud->width * inputCloud->height)) {
        ccLog::Warning("[PCL] Inconsistency detected between the reconstructed "
                       "scan grid and the converted cloud");
      } else {

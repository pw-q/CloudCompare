...
qRansacSD::qRansacSD(QObject *parent /*=nullptr*/)
    : QObject(parent), ccStdPluginInterface(":/CC/plugin/qRANSAC_SD/info.json"),
      m_action(nullptr) {
  s_app = m_app;
}
...
static double s_proba =
    0.01; // probability that no better candidate was overlooked during sampling
...
void qRansacSD::doAction() {
  ...
  rsdDlg.epsilonDoubleSpinBox->setValue(
      .005 * scale); // set distance threshold to 0.5% of bounding box width
  rsdDlg.bitmapEpsilonDoubleSpinBox->setValue(
      .01 * scale); // set bitmap resolution (= sampling resolution) to 1% of
                    // bounding box width
  ...
  s_minTorusMinorRadiusEnabled =
      rsdDlg.minTorusMinorRadiuscheckBox->isChecked();
  s_minTorusMajorRadiusEnabled =
      rsdDlg.minTorusMajorRadiuscheckBox->isChecked();
  s_maxTorusMinorRadiusEnabled =
      rsdDlg.maxTorusMinorRadiuscheckBox->isChecked();
  s_maxTorusMajorRadiusEnabled =
      rsdDlg.maxTorusMajorRadiuscheckBox->isChecked();
  ...
  RansacParams params;
  {
    ...
    params.createCloudFromLeftOverPoints =
        rsdDlg.saveLeftOverscheckBox->isChecked();
    ...
    if (s_minSphereRadiusEnabled) {
      params.minSphereRadius =
          static_cast<float>(rsdDlg.minSphereRadiusdoubleSpinBox->value());
      s_minSphereRadius = params.minSphereRadius;
    }
    if (s_maxSphereRadiusEnabled) {
      params.maxSphereRadius =
          static_cast<float>(rsdDlg.maxSphereRadiusdoubleSpinBox->value());
      s_maxSphereRadius = params.maxSphereRadius;
    }
    if (s_minCylinderRadiusEnabled) {
      params.minCylinderRadius =
          static_cast<float>(rsdDlg.minCylinderRadiusdoubleSpinBox->value());
      s_minCylinderRadius = params.minCylinderRadius;
    }
    if (s_maxCylinderRadiusEnabled) {
      params.maxCylinderRadius =
          static_cast<float>(rsdDlg.maxCylinderRadiusdoubleSpinBox->value());
      s_maxCylinderRadius = params.maxCylinderRadius;
    }
    if (s_maxConeRadiusEnabled) {
      params.maxConeRadius =
          static_cast<float>(rsdDlg.maxConeRadiusdoubleSpinBox->value());
      s_maxConeRadius = params.maxConeRadius;
    }

    if (s_maxConeLengthEnabled) {
      params.maxConeLength =
          static_cast<float>(rsdDlg.maxConeLengthdoubleSpinBox->value());
      s_maxConeLength = params.maxConeLength;
    }
    if (s_maxConeAngleEnabled) {
      params.maxConeAngle_deg =
          static_cast<float>(rsdDlg.maxConeAngledoubleSpinBox->value());
      s_maxConeAngle_deg = params.maxConeAngle_deg;
    }
    if (s_minTorusMinorRadiusEnabled) {
      params.minTorusMinorRadius =
          static_cast<float>(rsdDlg.minTorusMinorRadiusdoubleSpinBox->value());
      s_minTorusMinorRadius = params.minTorusMinorRadius;
    }
    if (s_minTorusMajorRadiusEnabled) {
      params.minTorusMajorRadius =
          static_cast<float>(rsdDlg.minTorusMajorRadiusdoubleSpinBox->value());
      s_minTorusMajorRadius = params.minTorusMajorRadius;
    }
    if (s_maxTorusMinorRadiusEnabled) {
      params.maxTorusMinorRadius =
          static_cast<float>(rsdDlg.maxTorusMinorRadiusdoubleSpinBox->value());
      s_maxTorusMinorRadius = params.maxTorusMinorRadius;
    }
    if (s_maxTorusMajorRadiusEnabled) {
      params.maxTorusMajorRadius =
          static_cast<float>(rsdDlg.maxTorusMajorRadiusdoubleSpinBox->value());
...
ccHObject *qRansacSD::executeRANSAC(ccPointCloud *ccPC,
                                    const RansacParams &params, bool silent) {
  // consistency check
  {
  ...
  PointCloud cloud;
  {
    try {
      ...
      ccLog::Error(
          "[qRansacSD] Could not create temporary cloud, Not enough memory!");
      return nullptr;
    }
  ...
  RansacShapeDetector::Options ransacOptions;
  {
    ...
    ransacOptions.m_normalThresh = static_cast<float>(
        cos(CCCoreLib::DegreesToRadians(params.maxNormalDev_deg)));
    ...
    ransacOptions.m_fitting = params.allowFitting
  ...
  if (!hasNorms) {
    ...
    if (!silent) {
      pDlg =
          new ccProgressDialog(false, s_app ? s_app->getMainWindow() : nullptr);
  ...
  if (params.primEnabled[RPT_SPHERE])
    detector.Add(new SpherePrimitiveShapeConstructor(params.minSphereRadius,
                                                     params.maxSphereRadius));
  if (params.primEnabled[RPT_CYLINDER])
    detector.Add(new CylinderPrimitiveShapeConstructor(
        params.minCylinderRadius, params.maxCylinderRadius,
        params.maxCylinderLength));
  if (params.primEnabled[RPT_CONE])
    detector.Add(new ConePrimitiveShapeConstructor(
        params.maxConeRadius,
        CCCoreLib::DegreesToRadians(params.maxConeAngle_deg),
        params.maxConeLength));
  if (params.primEnabled[RPT_TORUS])
    detector.Add(new TorusPrimitiveShapeConstructor(
        false, params.minTorusMinorRadius, params.minTorusMajorRadius,
        params.maxTorusMinorRadius,
        params.maxTorusMajorRadius)); // Do not allow apple shaped torus
  ...
  // primitive (the support) the points belonging to the first shape (shapes[0])
  // have been sorted to the end of pc, i.e. into the range [ pc.size() -
  // shapes[0].second, pc.size() ) the points of shape i are found in the range
...

  {
    ...
    ccLog::Print("[qRANSAC] Search Timing: %2.3f s",
  ...
  if (shapes.size() > 0) {
    ...
    for (MiscLib::Vector<DetectedShape>::const_iterator it = shapes.begin();
      ...
      if (shapePointsCount < params.supportPoints) {
        ccLog::Warning("[qRansacSD] Skipping shape, did not meet minimum point "
                       "requirement");
      ...
      bool saveNormals = true;
      {
        ...
        for (unsigned j = 0; j < shapePointsCount; ++j) {
          pcShape->addPoint(
              CCVector3::fromArray(cloud[shapeCloudIndex - j].pos));
          if (saveNormals) {
            pcShape->addNorm(
                CCVector3::fromArray(cloud[shapeCloudIndex - j].normal));
          }
        }
      ...
      case RPT_PLANE: // plane
      {
        ...
        pcShape->setName(
            QString("Plane_%1").arg(planeCount, 4, 10, QChar('0')));
        planeCount++;
      } break;
      ...
      case RPT_SPHERE: // sphere
      {
        ...
        pcShape->setName(
            QString("Sphere_%1").arg(sphereCount, 4, 10, QChar('0')));
        sphereCount++;
      } break;
      ...
      case RPT_CYLINDER: // cylinder
      {
        ...
        prim->setEnabled(false);
        prim->setName(
            QString("Cylinder (r=%1/h=%2)").arg(r, 0, 'f').arg(h, 0, 'f'));
        pcShape->setName(
            QString("Cylinder_%1").arg(cylinderCount, 4, 10, QChar('0')));
        cylinderCount++;
      } break;
      ...
      case RPT_CONE: // cone
      {
        ...
        {
          // the bottom should be the largest part so we inverse the axis
          // direction
          ...
          // radial axis
          CCVector3 X =
              CCVector3::fromArray((maxP - (CC + maxHeight * CA)).getValue());
          X.normalize();
          ...
          prim = new ccCone(maxRadius, minRadius, maxHeight - minHeight, 0, 0,
                            &glMat);
          prim->setEnabled(false);
          prim->setName(
              QString("Cone (alpha=%1 deg / h=%2)")
                  .arg(CCCoreLib::RadiansToDegrees(alpha_rad), 0, 'f')
                  .arg(static_cast<double>(maxHeight) - minHeight, 0, 'f'));
          pcShape->setName(
              QString("Cone_%1").arg(coneCount, 4, 10, QChar('0')));
          coneCount++;
        }
      ...
      case RPT_TORUS: // torus
      {
        ...
        if (torus->Internal().IsAppleShaped()) {
          ccLog::Warning("[qRansacSD] Apple-shaped torus are not handled by "
                         "CloudCompare!");
        } else {
          ...
          // torus primitive
          prim = new ccTorus(maxRadius - minRadius, maxRadius + minRadius,
          ...
          prim->setName(QString("Torus (r=%1/R=%2)")
                            ...
                            .arg(maxRadius, 0, 'f'));
          pcShape->setName(
              QString("Torus_%1").arg(torusCount, 4, 10, QChar('0')));
          torusCount++;
        }
      ...
      if (prim) {
        ...
        if (!group) {
          group = new ccHObject(
              QString("Ransac Detected Shapes (%1)").arg(ccPC->getName()));
        }

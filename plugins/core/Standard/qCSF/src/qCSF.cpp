...
// A mex version for programming in Matlab is at File Exchange of Mathworks
// website:
...
qCSF::qCSF(QObject *parent)
    : QObject(parent), ccStdPluginInterface(":/CC/plugin/qCSF/info.json"),
      m_action(nullptr) {}
...
void qCSF::doAction() {
  ...
  if (selectedEntities.empty()) {
    assert(false);
    m_app->dispToConsole("Select at least one cloud",
  ...
  for (ccHObject *ent : selectedEntities) {
    ...
    if (!CSF::Apply(pc, csfParams, groundCloud, offGroundCloud, ExportClothMesh,
                    clothMesh, m_app)) {

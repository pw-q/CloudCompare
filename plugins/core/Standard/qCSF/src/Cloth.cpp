...
{
  particles.resize(
      static_cast<size_t>(num_particles_width) *
      static_cast<size_t>(
          num_particles_height)); // I am essentially using this vector as an
                                  // array with room for
  ...
  for (int i = 0; i < num_particles_width; i++) {
    for (int j = 0; j < num_particles_height; j++) {
      ...
      particles[j * num_particles_width + i] = Particle(
          pos /*, squareTimeStep*/); // insert particle in column i at j'th row
...
ccMesh *Cloth::toMesh() const {
  ...
  for (int i = 0; i < getSize(); ++i) {
    ...
    vertices->addPoint(
        CCVector3(static_cast<PointCoordinateType>(particle.getPos().x),
...
#pragma omp parallel for
  ...
  //#pragma omp parallel for //DGM: satisfyConstraintSelf is not thread safe at
  //all!
  ...
  for (int i = 0; i < particleCount; i++) {
    if (particles[i].isMovable()) {
      double diff =
          std::abs(particles[i].getPreviousY() - particles[i].getPos().y);
      if (diff > maxDiff) {
        maxDiff = diff;
      }
    }
    ...
    if (x > 0) {
      ...
      if (!ptc_x.isMovable()) {
        ...
        if (std::abs(heightvals[index] - heightvals[index_ref]) <
                smoothThreshold &&
            ptc.getPos().y - heightvals[index] < heightThreshold) {
  ...
  while (!que.empty()) {
    ...
    for (size_t i = 0; i < neibors[index].size(); i++) {
      ...
      if (std::abs(heightvals[index_center] - heightvals[index_neibor]) <
              smoothThreshold &&
          std::abs(particles[index_neibor].getPos().y -
                   heightvals[index_neibor]) < heightThreshold) {
        double offsetY =
            heightvals[index_neibor] - particles[index_neibor].getPos().y;

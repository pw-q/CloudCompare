...
// For each lidar point, we find its neighbors in cloth particles by  Rounding
// operation. use for neighbor particles to do bilinear interpolation.
bool Cloud2CloudDist::Compute(const Cloth &cloth, const wl::PointCloud &pc,
  ...
  // for each lidar point, find the projection in the cloth grid, and the sub
  // grid which contains it.
  ...
  for (unsigned i = 0; i < static_cast<unsigned>(pc.size()); i++) {
    ...
    double fxy =
        cloth.getParticle(col0, row0).getPos().y * (1.0 - subdeltaX) *
            (1.0 - subdeltaZ) +
        cloth.getParticle(col3, row3).getPos().y * (1.0 - subdeltaX) *
            subdeltaZ +
        cloth.getParticle(col2, row2).getPos().y * subdeltaX * subdeltaZ +
        cloth.getParticle(col1, row1).getPos().y * subdeltaX *
            (1.0 - subdeltaZ);

...
//#        GNU General Public License for more details. # # # #        Please
//cite the following paper, If you use this plugin in your work.        # # # #
//Zhang W, Qi J, Wan P, Wang H, Xie D, Wang X, Yan G. An Easy-to-Use Airborne
//LiDAR  # #  Data Filtering Method Based on Cloth Simulation. Remote Sensing.
//2016; 8(6):501.   # # # #                                     Copyright � # #
//RAMM laboratory, School of Geography, Beijing Normal University       # #
//(http://ramm.bnu.edu.cn/)                             # # # # Wuming Zhang;
//Jianbo Qi; Peng Wan; Hongtao Wang                # # # # contact us:
...
bool CSF::Apply(const wl::PointCloud &csfPointCloud, const Parameters &params,
                std::vector<bool> &isGround, bool exportClothMesh,
                ccMesh *&clothMesh, ccMainAppInterface *app /*=nullptr*/,
  ...
  try {
    ...
    int width_num =
        static_cast<int>((bbMax.x - bbMin.x) / params.cloth_resolution) +
        2 * params.clothBuffer; // static_cast is equivalent to floor if value
                                // >= 0
    int height_num =
        static_cast<int>((bbMax.z - bbMin.z) / params.cloth_resolution) +
        2 * params.clothBuffer; // static_cast is equivalent to floor if value
                                // >= 0
    ...
    if (app) {
      app->dispToConsole(
          QString("[CSF] Cloth creation: %1 ms").arg(timer.restart()));
    }

    if (!Rasterization::RasterTerrain(cloth, csfPointCloud,
    ...
    cloth.addForce(-params.gravity *
                   squareTimeStep); // DGM: warning, the force is already
                                    // mutliplied by dt^2, no need to do it
                                    // later (in Particle::timeStep())
    ...
    if (params.smoothSlope) {
      ...
      if (app) {
        app->dispToConsole(
            QString("[CSF] Movable filter: %1 ms").arg(timer.restart()));
      }
    }

    // classification of the points
    bool result = Cloud2CloudDist::Compute(cloth, csfPointCloud,
                                           params.class_threshold, isGround);
    if (app) {
      app->dispToConsole(
          QString("[CSF] Distance computation: %1 ms").arg(timer.restart()));
    }
...
bool CSF::Apply(ccPointCloud *cloud, const Parameters &params,
                ccPointCloud *&groundCloud, ccPointCloud *&offGroundCloud,
                bool exportClothMesh, ccMesh *&clothMesh,
  ...
  if (pointCount == 0) {
    // nothing to do
    if (app) {
      app->dispToConsole(QString("Cloud %1 is empty").arg(cloud->getName()),
  ...
  try {
    ...
    if (!CSF::Apply(csfPC, params, isGround, exportClothMesh, clothMesh, app)) {
      if (app) {
        app->dispToConsole("Process failed",
    ...
    if (app) {
      app->dispToConsole(
          QString("[CSF] %1% of points classified as ground points")
              .arg((groundPointCount * 100.0) / pointCount, 0, 'f', 2),
          ccMainAppInterface::STD_CONSOLE_MESSAGE);
      app->dispToConsole(QString("[CSF] Timing: %1 s.")

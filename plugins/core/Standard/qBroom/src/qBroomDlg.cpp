...
// points)
static bool Intersection(const ccGLMatrix &broomTrans, const CCVector3 &A,
                         const CCVector3 &B, CCVector3 &I) {
...
qBroomDlg::qBroomDlg(ccMainAppInterface *app /*=nullptr*/)
    : QDialog(app ? app->getMainWindow() : nullptr,
      ...
      Ui::BroomDialog(), m_glWindow(nullptr), m_broomBox(new ccBox("Broom")),
      m_selectionBox(new ccBox("Selection")), m_boxes(new ccHObject("Boxes")),
      m_lastMousePos(0, 0), m_lastMousePos3D(0, 0, 0),
      m_hasLastMousePos3D(false), m_broomSelected(false),
      m_selectionMode(ABOVE), m_app(app), m_initialCloud(nullptr) {
  setupUi(this);
  ...
  int selectionMode = selectionModeComboBox->currentIndex();
  {
    ...
    bool stickToTheFloor =
        settings.value("stickToTheFloor", stickCheckBox->isChecked()).toBool();
    bool animateAutomation =
        settings
            .value("animatedAutomation", animateAutomationCheckBox->isChecked())
            .toBool();
...
void qBroomDlg::AutomationArea::clear() {
  ...
  if (polyline) {
    ...
    if (polyline->getDisplay()) {
      static_cast<ccGLWindowInterface *>(polyline->getDisplay())
...
void qBroomDlg::Picking::clear() {
  ...
  for (size_t i = 0; i < labels.size(); ++i) {
    ...
    if (label) {
      // auto remove from display
      if (label->getDisplay()) {
        static_cast<ccGLWindowInterface *>(label->getDisplay())
...
void qBroomDlg::CloudBackup::restore() {
  ...
  if (!hadOctree) {
    // we can only delete the octree if it has not already been added to the DB
    // tree!!!
    if (!ref->getParent()) {
      ref->deleteOctree();
    }
...
bool qBroomDlg::setCloud(ccPointCloud *cloud, bool ownCloud /*=false*/,
  ...
  if (cloud) {
    ...
    // from now on, the cloud will be automatically deleted if it's 'owned' by
    // the dialog
    m_cloud.ownCloud = ownCloud;
    ...
    if (m_cloud.originDisplay) {
      ...
      ccViewportParameters viewport =
          m_cloud.originDisplay->getViewportParameters();
      m_glWindow->setViewportParameters(viewport);

      // by the way, update some parameters of the 3D view (in case they have
      // changed since last call) const ccGui::ParamStruct& displayParams =
      // ccGui::Parameters(); m_glWindow->setDisplayParameters(displayParams);

      // import GL filter so as to get the same rendering aspect!
      ccGlFilter *filter =
          static_cast<ccGLWindowInterface *>(m_cloud.originDisplay)
              ->getGlFilter();
      if (filter) {
...
void qBroomDlg::onSelectionModeChanged(int mode) {
  ...
  if (m_selectionMode != formerMode) {
    removeSelectedPointsCheckBox->setChecked(
        mode !=
        0); // mode '0' is inside (--> we'll probably want to keep these points)
    if (m_glWindow) {
      updateBroomBox();
      updateSelectionBox();
      m_glWindow->redraw();
...
void qBroomDlg::updateAutomationAreaPolyline(int x, int y) {
  ...
  if (!m_autoArea.polyline) {
    ...
    vertices->addPoint(CCVector3(0, 0, 0));
    vertices->addPoint(CCVector3(0, 0, 0));
    m_autoArea.polyline->addPointIndex(0,
                                       2); // we only display a segment at first
    ...
    vertices =
        dynamic_cast<ccPointCloud *>(m_autoArea.polyline->getAssociatedCloud());
  }
  ...
  if (m_autoArea.clickedPoints.size() == 1) {
    ...
    *const_cast<CCVector3 *>(vertices->getPoint(1)) = C12D.toPC();
    ...
    CCVector3 Yp = Zp.cross(Xp); // should be already normal to Zp and Xp as
                                 // P03D and P13D are on the broom plane!
    Yp.normalize();
...
bool qBroomDlg::startAutomation() {
  ...
  if (false) {
    ...
    ccPlane *plane =
        new ccPlane(diagonal.x, diagonal.y, &planeTrans, "automation area");
  ...
  while (true) {
    ...
    bool lastRow = false;
    {
      ...
      if (sign < 0.0 && broomCenter.dot(X) <= BB0.x + broom.length / 2) {
        lastRow = true;
        broomCenter =
            origin + (BB0.x + broom.length / 2) * X + broomCenter.dot(Y) * Y;
        broomTrans.setTranslation(broomCenter);
      } else if (sign > 0.0 && broomCenter.dot(X) >= BB1.x - broom.length / 2) {
        lastRow = true;
        broomCenter =
            origin + (BB1.x - broom.length / 2) * X + broomCenter.dot(Y) * Y;
        broomTrans.setTranslation(broomCenter);
      }
    }
  ...
  if (lostTrack) {
    ccLog::Warning(
        "Holes or too steep slope encountered during automation process");
  }
...
bool qBroomDlg::positionBroom(const CCVector3 &P0, const CCVector3 &P1) {
  ...
  // static_cast<PointCoordinateType>(broomWidthDoubleSpinBox->value() / 100.0) *
  // length; PointCoordinateType thick =
...
#define USE_CYLINDRICAL_NEIGHBORHOOD
  ...
  nn.level =
      octree->findBestLevelForAGivenNeighbourhoodSizeExtraction(nn.radius);
  ...
  m_glWindow->setInteractionMode(ccGLWindowInterface::MODE_TRANSFORM_CAMERA |
                                 ccGLWindowInterface::INTERACT_SIG_LB_CLICKED);

  return true;
}

void qBroomDlg::handlePickedItem(ccHObject *entity, unsigned itemIdx, int x,
                                 int y, const CCVector3 &, const CCVector3d &) {
  assert(m_glWindow &&
         m_glWindow->getPickingMode() == ccGLWindowInterface::POINT_PICKING);
...
void qBroomDlg::onReposition() {
  ...
  // display message
  m_glWindow->displayNewMessage(
      "Pick two points on the cloud", ccGLWindowInterface::UPPER_CENTER_MESSAGE,
      false, 3600, ccGLWindowInterface::CUSTOM_MESSAGE);
...
void qBroomDlg::stopAutomation() {
  ...
  m_glWindow->setInteractionMode(
      ...
      ccGLWindowInterface::INTERACT_SIG_LB_CLICKED); // Standard interaction for
                                                     // manual broom
                                                     // displacement!
  m_picking.mode = Picking::NO_PICKING;
  // m_picking.clear();
...
void qBroomDlg::onAutomate() {
  ...
  m_glWindow->setInteractionMode(ccGLWindowInterface::MODE_PAN_ONLY |
                                 ccGLWindowInterface::INTERACT_SIG_LB_CLICKED);
  // m_glWindow->setPickingMode(ccGLWindowInterface::POINT_PICKING); //We don't
  // actually 'pick' an existing point!
  ...
  // display message
  m_glWindow->displayNewMessage("Pick two corners of the area to clean",
                                ccGLWindowInterface::UPPER_CENTER_MESSAGE,
                                false, 3600,
                                ccGLWindowInterface::CUSTOM_MESSAGE);
...
void qBroomDlg::onLeftButtonClicked(int x, int y) {
  ...
  if (m_picking.mode == Picking::AUTO_AREA_PICKING) {
    ...
    CCVector3 P3D;
    {
      ...
      if (!Intersection(broomTrans, M03D.toPC(), M13D.toPC(), P3D)) {
        ccLog::Warning(
            "Failed to project the clicked point on the bromm plane");
        return;
      }
    ...
    case 1:
      // we switch to mouse tracking in order to display the current area
      // polyline
      m_glWindow->setInteractionMode(
          m_glWindow->getInteractionMode() |
  ...
  } else // selection of the broom
  {
    ...
    m_broomSelected = false;
    {
      ...
      QPointF pos2D = m_glWindow->toCornerGLCoordinates(x, y);
      m_broomSelected =
          m_broomBox->trianglePicking(CCVector2d(pos2D.x(), pos2D.y()), camera,
                                      nearestTriIndex, nearestSquareDist, P);
    }
    ...
    if (m_broomSelected) {
      ...
      if (stickCheckBox->isChecked()) {
        ...
        QPointF pos2D = m_glWindow->toCornerGLCoordinates(m_lastMousePos.x(),
                                                          m_lastMousePos.y());
        ...
        if (camera.unproject(A2D, A3D) && camera.unproject(B2D, B3D)) {
          m_hasLastMousePos3D =
              Intersection(m_boxes->getGLTransformation(), A3D.toPC(),
      ...
      m_glWindow->setInteractionMode(
          ccGLWindowInterface::INTERACT_PAN |
  ...
  QPoint mousePos(x, y);
  {
    ...
    CCVector3d broomDelta(0, 0, 0);
    {
      ...
      if (!stickToTheFloor || !m_hasLastMousePos3D) {
        ...
        m_glWindow->getViewportParameters().viewMat.transposed().applyRotation(
            u);
        broomDelta = u;
      }
      ...
      if (stickToTheFloor) {
        ...
        if (camera.unproject(A2D, A3D) && camera.unproject(B2D, B3D)) {
          hasMousePos3D = Intersection(m_boxes->getGLTransformation(),
        ...
        if (m_hasLastMousePos3D && hasMousePos3D) {
          ...
          // but we can remove the normal component (the broom stays on the
          // floor)
...
bool qBroomDlg::moveBroom(ccGLMatrix &broomTrans, CCVector3d &broomDelta,
  ...
  if (stickToTheFloor) {
    ...
    if (count < 10) {
      ccLog::Warning(
          "Failed to extract enough point inside the broom. Lost track.");
      return false;
    } else {
      ...
      if (N) {
        ...
        ccLog::Error(
            "Failed to detect a planar surface below the broom. Lost track.");
        return false;
      }
    }
  }
...
bool qBroomDlg::selectPoints(const ccGLMatrix &broomTrans,
  ...
  CCCoreLib::DgmOctree::BoxNeighbourhood bn;
  {
    ...
    PointCoordinateType radius =
        std::max(bn.dimensions.x, std::max(bn.dimensions.y, bn.dimensions.z)) /
        5; // emprirical ;)
    bn.level =
        octree->findBestLevelForAGivenNeighbourhoodSizeExtraction(radius);
  }
...
void qBroomDlg::updateSelectionBox() {
  ...
  PointCoordinateType length =
      static_cast<PointCoordinateType>(broomLengthDoubleSpinBox->value());
  PointCoordinateType width = static_cast<PointCoordinateType>(
                                  broomWidthDoubleSpinBox->value() / 100.0) *
                              length;
...
void qBroomDlg::updateBroomBox() {
  ...
  m_broomBox->enableStippling(
      m_selectionMode == INSIDE); // the cleaning area box is already stippled!
  m_broomBox->setTempColor(ccColor::yellow);
}

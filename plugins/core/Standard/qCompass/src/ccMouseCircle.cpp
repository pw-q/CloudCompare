...
ccMouseCircle::ccMouseCircle(ccGLWindowInterface *owner, QString name)
    : cc2DViewportObject(name.isEmpty() ? "label" : name), m_owner(nullptr),
      m_pixelSize(0.0f), m_radius(50), m_radiusStep(4) {
  setVisible(true);
  setEnabled(false);
...
float ccMouseCircle::getRadiusWorld() {
  ...
  ccLog::Print(QString("Radius_w = %1 (= %2 x %3)")
                   .arg(r)
                   .arg(getRadiusPx())
                   .arg(m_pixelSize));
  return r;
...
void ccMouseCircle::draw(CC_DRAW_CONTEXT &context) {
  ...
  m_pixelSize =
      (context.glW != 0 ? params.computeWidthAtFocalDist() / context.glW : 0);

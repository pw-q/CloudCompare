...
void ccTrace::init(ccPointCloud *associatedCloud) {
  setAssociatedCloud(associatedCloud); // the ccPolyline c'tor should do this,
                                       // but just to be sure...
  m_cloud = associatedCloud;                   // store pointer ourselves also
  m_search_r = calculateOptimumSearchRadius(); // estimate the search radius we
                                               // want to use
...
int ccTrace::insertWaypoint(int pointId) {
  if (m_waypoints.size() >= 2) {
    ...
    // check if the point is closer to the start than the end -> i.e. the point
    // should be 'pre-pended'
...
bool ccTrace::inCircle(const CCVector3 *segStart, const CCVector3 *segEnd,
  ...
  for (unsigned i = 1; i < m_waypoints.size(); i++) {
    ...
    if (tID >= m_trace.size()) {
      std::deque<int> segment =
          optimizeSegment(start, end, m_search_r); // calculate segment
    ...
    } else // no... we're somewhere in the middle - update segment if necessary
    {
      if (!m_trace[tID].empty() && (m_trace[tID][0] == start) &&
        ...
        std::deque<int> segment =
            optimizeSegment(start, end, m_search_r); // calculate segment
        success =
            success && !segment.empty(); // if the queue is empty, we failed

        // add trace
        if (m_trace[tID][m_trace[tID].size() - 1] ==
            end) // end matches - we can replace the current trace & things
                 // should be sweet (all prior traces will have been updated
                 // already)
          m_trace[tID] = segment; // end is correct - overwrite this block, then
                                  // hopefully we will match in the next one
...
std::deque<int> ccTrace::optimizeSegment(int start, int end, int offset) {
  ...
  // get location of target node - used to optimise algorithm to stop searching
  // paths leading away from the target
  ...
  std::priority_queue<Node *, std::vector<Node *>, Compare>
      openQueue; // priority queue that stores nodes that haven't yet been
                 // explored/opened
  ...
  visited.resize(
      m_cloud->size(),
      false); // n.b. for 400 million points, this will still only be ~50Mb =)
  ...
  int bufferSize = 500000; // 500k node buffer (~1.5-2Mb). This should be enough
                           // for most small-medium size traces. More buffers
                           // will be created for bigger ones.
  ...
  if (!oct) {
    oct = m_cloud->computeOctree(); // if the user clicked "no" when asked to
  ...
  while (openQueue.size() > 0) // while unvisited nodes exist
  {
    ...
    // backwards (in euclidean space) [essentially stops fracture turning > 90
    // degrees)
    ...
    for (size_t i = 0; i < m_neighbours.size(); i++) {
      ...
      if (visited[m_p.pointIndex]) // Has this node been visited before? If so
                                   // then bail.
        continue;
      ...
      m_cloud->setPointScalarValue(
          ...
          static_cast<ScalarType>(
              cost)); // STORE VISITED NODES (AND COST) FOR DEBUG VISUALISATIONS
#endif
  ...
  // If we're here, then it exhausted all the reachable points without finding
  // the destination point. This can happen if, for example, the user is asking
  // for a path between two "islands".
...
int ccTrace::getSegmentCost(int p1, int p2) {
  int cost =
      1; // n.b. default value is 1 so that if no cost functions are used, the
         // function doesn't crash (and returns the unweighted shortest path)
  ...
  return sqrt(
         ...
         0.25 *
             (
                 //|c1-start|
                 sqrt(
                     (p1_rgb.r - m_start_rgb[0]) * (p1_rgb.r - m_start_rgb[0]) +
                 ...
                 //|c2-start|
                 sqrt(
                     (p2_rgb.r - m_start_rgb[0]) * (p2_rgb.r - m_start_rgb[0]) +
                 ...
                 sqrt((p2_rgb.r - m_end_rgb[0]) * (p2_rgb.r - m_end_rgb[0]) +
                      ...
                      (p2_rgb.b - m_end_rgb[2]) * (p2_rgb.b - m_end_rgb[2]))) /
             3.5; // N.B. the divide by 3.5 scales this cost function to range
                  // between 0 & 255
}
...
int ccTrace::getSegmentCostDark(int p1, int p2) {
  ...
  return (p2_rgb.r + p2_rgb.g + p2_rgb.b); // note: this will naturally give a
                                           // maximum of 765 (=255 + 255 + 255)
}
...
int ccTrace::getSegmentCostCurve(int p1, int p2) {
  ...
  if (isCurvaturePrecomputed()) // scalar field found - return from precomputed
                                // cost]
  {
    // activate SF
    m_cloud->setCurrentScalarField(idx);
  ...
  } else // scalar field not found - do slow calculation...
  {
        ...
        4) // need at least 4 points to calculate curvature....
    {
      ...
      // compute curvature
      CCCoreLib::DgmOctreeReferenceCloud nCloud(
      ...
      m_neighbours.erase(m_neighbours.end() -
                         1); // remove center point from neighbourhood (so as
                             // not to screw up loops)

      // curvature tends to range between 0 (high cost) and 10 (low cost),
      // though it can be greater than 10 in extreme cases hence we need to map
      // to domain 0 - 10 and then transform that to the (integer) domain 0 - 884
      // to meet the cost function spec
      if (c > 10)
        c = 10;
  ...
  } else // not found... do expensive calculation
  {
        ...
        2) // need at least 2 points to calculate gradient....
    {
      ...
      for (int i = 0; i < m_neighbours.size(); i++) {
        ...
        // calculate gradient weighted by distance to the point (i.e. divide by
        // distance^2)
        if (CCCoreLib::GreaterThanSquareEpsilon(norm2)) {
          ...
          // sum stuff
          sum.x +=
              deltaPos.x * deltaValue; // warning: here 'deltaValue'= deltaValue
      ...
      // ensure gradient is lass than a case-specific maximum gradient (colour
      // change from white to black across a distance or search_r,
      //                                                                                  giving a gradient of (255+255+255) / search_r)
      gradient = std::min(gradient, 765 / search_r);
      gradient *= search_r; // scale between 0 and 765
      return 765 -
             gradient; // return inverse gradient (high gradient = low cost)
    }
...
int ccTrace::getSegmentCostScalar(int p1, int p2) {
  ...
  ccScalarField *sf =
      static_cast<ccScalarField *>(m_cloud->getCurrentDisplayedScalarField());
  if (!sf) {
    assert(false);
    return 0;
  }
  return (sf->getValue(p2) - sf->getMin()) *
         (765 /
          (sf->getMax() -
           sf->getMin())); // return scalar field value mapped to range 0 - 765
}
...
int ccTrace::getSegmentCostScalarInv(int p1, int p2) {
  ...
  return (sf->getMax() - sf->getValue(p2)) *
         (765 / (sf->getMax() - sf->getMin())); // return inverted scalar field
...
void ccTrace::buildGradientCost(QWidget *parent) {
  ...
  for (unsigned i = 0; i < m_cloud->size(); i++) {
    ...
    m_cloud->setPointScalarValue(
        i, static_cast<ScalarType>((static_cast<int>(col.r) + col.g) + col.b));
  }
  // compute min/max
  ...
  m_cloud->setCurrentOutScalarField(
      idx); // set out gradient field - values are read from here for calc
  ...
  // calculate gradient
  int result = CCCoreLib::ScalarFieldTools::computeScalarFieldGradient(
      m_cloud,
  ...
  for (unsigned i = 0; i < m_cloud->size(); i++) {
    ...
    if (nVal < 0) // this is caused by isolated points that were assigned "null"
                  // value gradients
...
void ccTrace::buildCurvatureCost(QWidget *parent) {
  ...
  CCCoreLib::GeometricalAnalysisTools::ErrorCode result =
      CCCoreLib::GeometricalAnalysisTools::ComputeCharactersitic(
          ...
          CCCoreLib::Neighbourhood::CurvatureType::MEAN_CURV, m_cloud,
          m_search_r, nullptr, &pDlg, octree.data());
...
ccFitPlane *ccTrace::fitPlane(int surface_effect_tolerance,
  ...
  if (m_cloud->hasNormals()) {
    ...
    if (acos(n_avg.dot(n)) <
        0.01745329251 *
            surface_effect_tolerance) // 0.01745329251 converts deg to rad
    {
...
void ccTrace::drawMeOnly(CC_DRAW_CONTEXT &context) {
  ...
  if (MACRO_Draw3D(context)) {
    ...
    // set draw colour
    ccColor::Rgb color =
        entityPickingMode ? pickingColor : getMeasurementColour();
    ...
    if (m_isActive) {
      for (size_t i = 0; i < m_waypoints.size(); i++) {
        ...
        float scale =
            context.labelMarkerSize * m_relMarkerScale * 0.3 * fmin(pSize, 4);
    ...
    // draw trace points if trace is active OR point size is large (otherwise
    // line gets hidden)
    if (m_isActive || pSize > 8) {
      ...
      for (const std::deque<int> &seg : m_trace) {
        for (int p : seg) {
          ...
          ccGL::Translate(glFunc, P->x, P->y, P->z);
          double scale = context.labelMarkerSize *
                         (m_relMarkerScale * (fmin(pSize, 4) * 0.2));
          if (viewportParams.perspectiveView && viewportParams.zFar > 0) {
            // in perspective view, the actual scale depends on the distance to
            // the camera!
            ...
            double unitD =
                ...
                2; // we consider that the 'standard' scale is at half the depth
            scale *= sqrt(
                d / unitD); // sqrt = empirical (probably because the marker
                            // size is already partly compensated by
                            // ccGLWindowInterface::computeActualPixelSize())
          }
...
bool ccTrace::isTrace(ccHObject *object) // return true if object is a valid
                                         // trace [regardless of it's class type]
{

...
void ccCompass::onNewSelection(const ccHObject::Container &selectedEntities) {
  ...
  if (m_activeTool) {
    m_activeTool->onNewSelection(selectedEntities); // pass on to the active
                                                    // tool
  }
  ...
  for (ccHObject *obj : selectedEntities) {
    // recurse upwards looking for geoObject & relevant part (interior, upper,
    // lower)
    ccHObject *o = obj;
    bool interior = false;
    bool upper = false;
...
void ccCompass::doAction() {
  ...
  if (!m_app->getActiveGLWindow()) {
    m_app->dispToConsole("[ccCompass] Could not find valid 3D window.",
  ...
  if (!m_dlg) {
    ...
    // general
    connect(m_dlg->closeButton, &QAbstractButton::clicked, this,
            &ccCompass::onClose);
    connect(m_dlg->acceptButton, &QAbstractButton::clicked, this,
            &ccCompass::onAccept);
    connect(m_dlg->saveButton, &QAbstractButton::clicked, this,
            &ccCompass::onSave);
    connect(m_dlg->undoButton, &QAbstractButton::clicked, this,
            &ccCompass::onUndo);
    connect(m_dlg->infoButton, &QAbstractButton::clicked, this,
            &ccCompass::showHelp);

    // modes
    connect(m_dlg->mapMode, &QAbstractButton::clicked, this,
            &ccCompass::enableMapMode);
    connect(m_dlg->compassMode, &QAbstractButton::clicked, this,
            &ccCompass::enableMeasureMode);

    // tools
    connect(m_dlg->pickModeButton, &QAbstractButton::clicked, this,
            &ccCompass::setPick);
    connect(m_dlg->pairModeButton, &QAbstractButton::clicked, this,
            &ccCompass::setLineation);
    connect(m_dlg->planeModeButton, &QAbstractButton::clicked, this,
            &ccCompass::setPlane);
    connect(m_dlg->traceModeButton, &QAbstractButton::clicked, this,
            &ccCompass::setTrace);

    // extra tools
    connect(m_dlg->m_pinchTool, &QAction::triggered, this,
            &ccCompass::addPinchNode);
    connect(m_dlg->m_measure_thickness, &QAction::triggered, this,
            &ccCompass::setThickness);
    connect(m_dlg->m_measure_thickness_twoPoint, &QAction::triggered, this,
            &ccCompass::setThickness2);

    connect(m_dlg->m_youngerThan, &QAction::triggered, this,
            &ccCompass::setYoungerThan);
    connect(m_dlg->m_follows, &QAction::triggered, this,
            &ccCompass::setFollows);
    connect(m_dlg->m_equivalent, &QAction::triggered, this,
            &ccCompass::setEquivalent);

    connect(m_dlg->m_mergeSelected, &QAction::triggered, this,
            &ccCompass::mergeGeoObjects);
    connect(m_dlg->m_fitPlaneToGeoObject, &QAction::triggered, this,
            &ccCompass::fitPlaneToGeoObject);
    connect(m_dlg->m_recalculateFitPlanes, &QAction::triggered, this,
            &ccCompass::recalculateFitPlanes);
    connect(m_dlg->m_toPointCloud, &QAction::triggered, this,
            &ccCompass::convertToPointCloud);
    connect(m_dlg->m_distributeSelection, &QAction::triggered, this,
            &ccCompass::distributeSelection);
    connect(m_dlg->m_estimateNormals, &QAction::triggered, this,
            &ccCompass::estimateStructureNormals);
    connect(m_dlg->m_estimateP21, &QAction::triggered, this,
            &ccCompass::estimateP21);
    connect(m_dlg->m_estimateStrain, &QAction::triggered, this,
            &ccCompass::estimateStrain);
    ...
    // settings menu
    connect(m_dlg->m_showNames, &QAction::toggled, this,
            &ccCompass::toggleLabels);
    connect(m_dlg->m_showStippled, &QAction::toggled, this,
            &ccCompass::toggleStipple);
    connect(m_dlg->m_showNormals, &QAction::toggled, this,
            &ccCompass::toggleNormals);
    connect(m_dlg->m_recalculate, &QAction::triggered, this,
            &ccCompass::recalculateSelectedTraces);
  }

  if (!m_mapDlg) {
    m_mapDlg = new ccMapDlg(m_app->getMainWindow());

    connect(m_mapDlg->m_create_geoObject, &QAction::triggered, this,
            &ccCompass::addGeoObject);
    connect(m_mapDlg->m_create_geoObjectSS, &QAction::triggered, this,
            &ccCompass::addGeoObjectSS);
    connect(m_mapDlg->setInteriorButton, &QAbstractButton::clicked, this,
            &ccCompass::writeToInterior);
    connect(m_mapDlg->setUpperButton, &QAbstractButton::clicked, this,
            &ccCompass::writeToUpper);
    connect(m_mapDlg->setLowerButton, &QAbstractButton::clicked, this,
            &ccCompass::writeToLower);
  }
...
void ccCompass::tryLoading() {
  ...
  std::vector<ccHObject *>
      replacements; // pointers to objects that will replace the originals
...
void ccCompass::tryLoading(ccHObject *obj, std::vector<int> &originals,
  ...
  if (dynamic_cast<ccFitPlane *>(obj) || dynamic_cast<ccTrace *>(obj) ||
      dynamic_cast<ccPointPair *>(
          obj) // n.b. several classes inherit from PointPair, so this cast will
  ...
  if (ccGeoObject::isGeoObject(obj)) {
    ...
    // add to originals/duplicates list [these are used later to overwrite the
    // originals]
...
bool ccCompass::startMeasuring() {
  ...
  if (!m_app->getActiveGLWindow()) {
    ...
    m_app->dispToConsole(
        "Error: ccCompass could not find the Cloud Compare window. Abort!",
        ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return false;
  }
...
bool ccCompass::stopMeasuring(bool finalStop /*=false*/) {
  // Check if we were ever loaded. If the plugin wasn't loaded this will be
  // nullptr.
  if (m_app == nullptr) {
    return true;
  }
...
bool ccCompass::startPicking() {
  ...
  {
    m_app->dispToConsole("[ccCompass] Could not retrieve valid picking hub. "
                         "Measurement aborted.",
                         ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return false;
  }
  ...
  if (!m_app->pickingHub()->addListener(this, true, true)) {
    m_app->dispToConsole(
        "Another tool is already using the picking mechanism. Stop it first",
        ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return false;
  }
...
ccHObject *ccCompass::getInsertPoint() {
  ...
  if (ccCompass::mapMode || m_geoObject) {
    ...
    if (!m_geoObject) {
      m_app->dispToConsole(
          "[ccCompass] Error: Please select a GeoObject to digitize to.",
          ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return nullptr;
    }
    ...
    if (!m_app->dbRootObject()->find(m_geoObject_id)) {
      ...
      {
        m_app->dispToConsole("[ccCompass] Warning: Could not retrieve valid "
    ...
    for (unsigned i = 0; i < m_app->dbRootObject()->getChildrenNumber(); i++) {
      if (m_app->dbRootObject()->getChild(i)->getName() == "measurements") {
        ...
        for (unsigned c = 0;
             c < m_app->dbRootObject()->getChild(i)->getChildrenNumber(); c++) {
          if (m_app->dbRootObject()->getChild(i)->getChild(c)->getName() ==
              "measurements") {
...
// Process point picks
void ccCompass::pointPicked(ccHObject *entity, unsigned itemIdx, int x, int y,
                            const CCVector3 &P) {
  if (!entity) // null pick
  {
    return;
  ...
  // ensure what we are writing too is visible (avoids confusion if it is turned
  // off...)
...
void ccCompass::mergeGeoObjects() {
  ...
  {
    m_app->dispToConsole("[Compass] Select several GeoObjects to merge.",
                         ccMainAppInterface::ERR_CONSOLE_MESSAGE);
  ...
  m_app->dispToConsole("[Compass] Merged selected GeoObjects to " +
                           dest->getName(),
...
void ccCompass::fitPlaneToGeoObject() {
  ...
  m_app->dispToConsole("[Compass] fitPlane",
                       ccMainAppInterface::STD_CONSOLE_MESSAGE);
  ...
  for (unsigned i = 0; i < upper->getChildrenNumber(); i++) {
    if (ccTrace::isTrace(upper->getChild(i))) {
      ...
      if (t !=
          nullptr) // can in rare cases be a null ptr (dynamic cast will fail
                   // for traces that haven't been converted to ccTrace objects)
      {
  ...
  if (points->size() > 0) {
    ...
    if (p) {
      ...
      m_app->dispToConsole(
          "[Compass] Not enough 3D information to generate sensible fit plane.",
          ccMainAppInterface::WRN_CONSOLE_MESSAGE);
    }
  }
  ...
  if (!ccGeoObject::isSingleSurfaceGeoObject(obj)) {
    ...
    for (unsigned i = 0; i < lower->getChildrenNumber(); i++) {
      if (ccTrace::isTrace(lower->getChild(i))) {
        ...
        if (t != nullptr) // can in rare cases be a null ptr (dynamic cast will
                          // fail for traces that haven't been converted to
                          // ccTrace objects)
        {
          points->reserve(points->size() + t->size()); // make space
    ...
    if (points->size() > 0) {
      ...
      if (p) {
        ...
        m_app->dispToConsole("[Compass] Not enough 3D information to generate "
                             "sensible fit plane.",
                             ccMainAppInterface::WRN_CONSOLE_MESSAGE);
      }
    }
...
void ccCompass::recalculateFitPlanes() {
  ...
  for (auto &plane : planes) {
    ...
    // otherwise - does the plane have a child that is a trace object (i.e. it
    // was created in Compass mode)
      ...
      if (ccTrace::isTrace(child)) // add to recalculate list
      {
                          ...
                          // ccTrace objects)
        {
          ...
          if (p) {
            ...
            parent->addChild(p); // add fit-plane to the original fit-plane's
...
inline double prior(double phi, double theta, double nx, double ny, double nz) {
  ...
  double alpha = acos(nx * sin(phi) * cos(theta) + ny * cos(phi) * cos(theta) -
                      nz * sin(theta));
  return sin(alpha) /
...
inline double logWishSF(CCCoreLib::SquareMatrixd X, int nobserved) {
  ...
  return (nobserved - 4.0) * 0.5 * log(detX) -
         (nobserved * 3. / 2.) * log(2.0) - // parts of gamma function that do
                                            // not depend on the scale matrix
...
inline double logWishart(CCCoreLib::SquareMatrixd X, int nobserved, double phi,
                         double theta, double alpha, double e1, double e2,
                         double e3, double lsf) {
  //--------------------------------------------------
  ...
  e1 = 1.0 / e1; // N.B. Note that by inverting the eigenvalues we compute the
  ...
  i[1][1] =
      e1 * e[1][0] * e[1][0] + e2 * e[1][1] * e[1][1] + e3 * e[1][2] * e[1][2];
  i[2][2] =
      e1 * e[2][0] * e[2][0] + e2 * e[2][1] * e[2][1] + e3 * e[2][2] * e[2][2];
  i[0][1] = e1 * e[0][0] * e[1][0] + e2 * e[0][1] * e[1][1] +
            e3 * e[0][2] * e[1][2]; // off-axis component
  i[0][2] =
      e1 * e[0][0] * e[2][0] + e2 * e[0][1] * e[2][1] + e3 * e[0][2] * e[2][2];
  i[1][2] =
      e1 * e[1][0] * e[2][0] + e2 * e[1][1] * e[2][1] + e3 * e[1][2] * e[2][2];
...
void ccCompass::estimateStructureNormals() {
  ...
  sampleText.setValidator(new QIntValidator(
      1, 10000)); //>10000 samples per point will break even the best computer!
  ...
  dofText.setToolTip(
      "Sets the degrees of freedom parameter for the Wishart distribution. Due "
      "to non-independent data/errors in traces, this should be low (~10). "
      "Higher give more confident results - use with care!");
  distanceText.setToolTip(
      "The furthest distance to search for points on the opposite surface of a "
      "GeoObject during thickness calculations.");
  sampleText.setToolTip("Sample n orientation estimates at each point in each "
                        "trace to quantify uncertainty.");
  likPowerText.setToolTip(
      "Fudge factor to change the balance between the prior and likelihood "
      "functions. Advanced use only - see docs for details.");
  strideText.setToolTip("Standard deviation of the normal distribution used to "
                        "calculate monte-carlo jumps during sampling. Larger "
                        "numbers sample more widely but are slower to run.");
  ...
  tcDistance =
      distanceText.text().toDouble(); // the square of the maximum distance to
  ...
  // someone is an idiot
  if (maxsize < minsize) {
    m_app->dispToConsole("[ccCompass] Error - provided maxsize is less than "
                         "minsize? Get your shit together...",
                         ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return;
  }

  m_app->dispToConsole(
      "[ccCompass] Estimating structure normals. This may take a while...",
      ccMainAppInterface::STD_CONSOLE_MESSAGE);
  ...
  for (ccHObject *o : m_app->getSelectedEntities()) {
    ...
    } else // otherwise search for all GeoObjects
    {
      o->filterChildren(
          objs, true,
          CC_TYPES::HIERARCHY_OBJECT); // n.b. geoObjects are simpy considered
                                       // to be hierarchy objects by CC
    }
      ...
      if (ccGeoObject::isGeoObject(o2)) {
        ...
        if (g) { // could possibly be null if non-loaded geo-objects exist
          ...
          if (ccGeoObject::isSingleSurfaceGeoObject(
                  g)) { // special case - single surface geoboject (upper and
                        // lower regions will be the same). Set upper to null
            data[1] = nullptr;
          }
          ...
          ccPointCloud *cloud =
              new ccPointCloud(); // points will be written here if the object
                                  // is a GeoObject and if it contains pinch
                                  // nodes
          ...
          g->filterChildren(
              objs, true,
              CC_TYPES::POLY_LINE); // pinch nodes inherit the polyline clas
    ...
    if (ccTrace::isTrace(o)) {
      ...
      o->filterChildren(
          objs, true,
          CC_TYPES::POLY_LINE); // n.b. geoObjects are simpy considered to be
    ...
    for (ccHObject *o2 : objs) {
      if (ccTrace::isTrace(o2) && o2->isEnabled()) {
        ...
        if (t != nullptr) {
          // can in rare cases be a null ptr (dynamic cast will fail for traces
          // that haven't been converted to ccTrace objects)
          datasets.push_back({t, nullptr}); // store data for processing
          pinchClouds.push_back(
              new ccPointCloud()); // push empty cloud (no pinch nodes).
        }
      }
    }
  }
  ...
  if (datasets.empty()) { // no data found
    m_app->dispToConsole(
        "[ccCompass] No GeoObjects or Traces could be found to estimate "
        "structure normals for. Please select some!",
        ccMainAppInterface::ERR_CONSOLE_MESSAGE);
  }
  ...
  for (int _d = 0; _d < datasets.size(); _d++) {
    ...
    ccPointCloud *points[] = {
        ...
        new ccSNECloud()}; // Upper Boundary Points (will remain empty for
                           // everything execept multi-surface GeoObjects)
    ccPointCloud *samples[] = {
        nullptr, nullptr}; // lower and upper boundary samples (will be
                           // populated later if samples are generated).

    // for lower,upper in the case of a GeoObject, otherwise regions[1] will be
    ...
    for (unsigned r = 0; r < 2; r++) {
      ...
      if (ccTrace::isTrace(regions[r])) {
        ...
        // otherwise search for child traces (this is a GeoObject region so
        // traces need to be joined together)
      ...
      for (ccHObject *c : objs) {
        if (ccTrace::isTrace(c) && c->isEnabled()) // is it a trace?
        {
          ...
          if (t != nullptr) // can in rare cases be a null ptr (dynamic cast
                            // will fail for traces that haven't been converted
                            // to ccTrace objects)
          {
            // copy points from this trace across into the relevant point cloud
            // for future access
            points[r]->reserve(points[r]->size() + t->size()); // make space
            points[r]->reserveTheNormsTable(); // make space for normals
            points[r]->copyGlobalShiftAndScale(
                *t); // copy global shift & scale onto new point cloud
            for (unsigned p = 0; p < t->size(); p++) {
              points[r]->addPoint(
                  *t->getPoint(p)); // add point to relevant surface
      ...
      if (points[r]->size() < minsize) {
        m_app->dispToConsole(
            QString::asprintf("[ccCompass] Warning: Region %d contains less "
                              "than minsize points. Region ignored.",
                              regions[r]->getUniqueID()),
            ccMainAppInterface::WRN_CONSOLE_MESSAGE);
        delete points[r];
        points[r] = nullptr;
        regions[r] = nullptr;
      ...
      //*********************************************************
      CCCoreLib::Neighbourhood Z(
          points[r]); // put points for this surface into a neighbourhood and
                      // get the sorting direction (principal eigenvector)
      const CCVector3 *longAxis =
          Z.getLSPlaneX(); // n.b. this is a normal vector
      if (longAxis == nullptr) {
        // fail friendly if eigens could not be computed
        m_app->dispToConsole(
            QString::asprintf("[ccCompass] Warning: Could not compute "
                              "eigensystem for region %u. Region ignored.",
                              regions[r]->getUniqueID()),
            ccMainAppInterface::WRN_CONSOLE_MESSAGE);
      ...
      std::vector<unsigned> pid; // store link to point id in original cloud
                                 // (for later data storage)
        ...
        if (dist[dist.size() - 1] <= d) {
          ...
          for (int n = 0; n < dist.size(); n++) {
            ...
            if (dist[n] > d) // found an insert point from the left
            {
              iid = n;
              break;
            } // TODO - could optimise this by searching backwards from the end
              // also?
          }
      ...
      //**************************************************************************************************
      // CREATE BREAKS AT PINCH NODES (these prevent planes including points
      // from two sides of a pinch node
      //**************************************************************************************************
      std::vector<bool> breaks(px.size(),
                               false); // if point n is a break (closest point
      ...
      for (unsigned p = 0; p < pinchNodes->size(); p++) {
        ...
        nCloud.clear(false);
        oct->findPointNeighbourhood(pinchNodes->getPoint(p), &nCloud, 1, level,
                                    d);
        breaks[nCloud.getPointGlobalIndex(0)] = true; // assign
      }

      //***********************************************************************************************
      // RECURSE THROUGH ALL POSSIBLE COMBINATIONS OF POINTS TO FIND THE BEST
      // STRUCTURE NORMAL ESTIMATE
      //***********************************************************************************************
      ...
      std::vector<double> bestPd(
          px.size(),
          std::numeric_limits<double>::lowest()); // best (log) probability
                                                  // density observed for each
                                                  // point
      ...
      std::vector<CCCoreLib::SquareMatrixd> bestX(
          px.size()); // keep track of best COV matrix for each trace (for
                      // oversampling later)
      std::vector<CCVector3> sne(
          px.size()); // list of the best surface normal estimates found for
                      // each point (corresponds with the MAP above)
      ...
      std::vector<int> segmentID(px.size(),
                                 -1); // unique id for each point segment.
      std::vector<CCVector3> normal(
          px.size()); // list of the surface normals (as opposed to structure
                      // normals stored in SNE).

      // check if valid normals have been retrieved
      if (hasNormals) {
        if (std::abs(nx[0]) <= 0.000001 && std::abs(ny[0]) <= 0.0000001 &&
            std::abs(nz[0]) <=
                0.00000001) // zero normal vector means normals not computed
        {
          m_app->dispToConsole(
              "[ccCompass] Warning: Cannot compensate for outcrop-surface bias "
              "as point cloud has no normals. Structure normal estimates may "
              "be misleading or incorrect.",
              ccMainAppInterface::WRN_CONSOLE_MESSAGE);
          hasNormals = false; // don't bother checking again - if normals are
                              // computed they will exist for all points
        }
      }

      // loop through all possible continuous subsets of the combined trace with
      // minsize < length < maxsize.
      for (unsigned _min = 0; _min < px.size() - minsize; _min++) {
        ...
        for (unsigned _max = _min + minsize;
             _max < std::min(static_cast<unsigned>(px.size()), _min + maxsize);
             _max++) {
          // size of the current subset
          n = _max - _min + 1;

          //-------------------------------------------------------------------------------------------------------------------------------------
          // compute centroid of points between min and max (and the average
          // normal). Also check if break-point exists (if so skip this subset)
          //-------------------------------------------------------------------------------------------------------------------------------------
          cx = 0.0;
          cy = 0.0;
          cz = 0.0;
          ...
          //-----------------------------------------------------------------------------
          // compute the scatter and covariance matrices of this section of the
          // trace
          //-----------------------------------------------------------------------------
          ...
          // update X to reflect the dof (rather than the true number of
          // samples, as these are not truly independent due to spatial
          // covariance)
          ...
          CCCoreLib::Jacobi<double>::ComputeEigenValuesAndVectors(
              cov, eigVectors, eigValues, true); // get eigens
          CCCoreLib::Jacobi<double>::SortEigenValuesAndVectors(
              eigVectors, eigValues); // sort into decreasing order

          //----------------------------------------------------------------------------------------------------
          // Compute the trend and plunge of the best-fit plane (based entirely
          // on the eigensystem). These values will be the maxima of the wishart
          // likelihood distribution and are used to efficiently estimate the
          // maxima a-postiori. This will be incorrect where we are at the
          // low-point in the prior, but it doesn't matter that much....
          //----------------------------------------------------------------------------------------------------

          // calculate trend and plunge of 3rd eigenvector (this represents the
          // "best-fit-plane").
          phi = atan2(
              eigVectors.m_values[0][2],
              eigVectors.m_values[1][2]); // trend of the third eigenvector
          theta = -asin(
              eigVectors.m_values[2][2]); // plunge of the principal eigenvector
          ...
          // calculate third angle (alpha) defining the orientation of the
          // eigensystem
          alpha = asin(eigVectors.m_values[2][1] /
                       cos(theta)); // alpha = arcsin(eigVector2.z / cos(theta))
          ...
          // dof = _max - _min - 1;
          lsf = logWishSF(X, dof);
          lpd = likPower * logWishart(X, dof, phi, theta, alpha, eigValues[0],
                                      eigValues[1], eigValues[2], lsf);
      ...
      if (!hasValidSNE) {
        // if segments between pinch nodes are too small, then we will not get
        // any valid fit-planes
        m_app->dispToConsole(
            QString::asprintf("[ccCompass] Warning: Region %d contains no "
                              "valid points (PinchNodes break the trace into "
                              "small segments?). Region ignored.",
                              regions[r]->getUniqueID()),
            ccMainAppInterface::WRN_CONSOLE_MESSAGE);
        delete points[r];
        points[r] = nullptr;
      ...
      // build scalar fields
      CCCoreLib::ScalarField *startSF = points[r]->getScalarField(
          points[r]->addScalarField(new ccScalarField("StartPoint")));
      CCCoreLib::ScalarField *endSF = points[r]->getScalarField(
          points[r]->addScalarField(new ccScalarField("EndPoint")));
      CCCoreLib::ScalarField *idSF = points[r]->getScalarField(
          points[r]->addScalarField(new ccScalarField("SegmentID")));
      CCCoreLib::ScalarField *weightSF = points[r]->getScalarField(
          points[r]->addScalarField(new ccScalarField("Weight")));
      CCCoreLib::ScalarField *trend = points[r]->getScalarField(
          points[r]->addScalarField(new ccScalarField("Trend")));
      CCCoreLib::ScalarField *plunge = points[r]->getScalarField(
          points[r]->addScalarField(new ccScalarField("Plunge")));
      CCCoreLib::ScalarField *pointID =
          points[r]->getScalarField(points[r]->addScalarField(
              new ccScalarField("PointID"))); // used for linking samples
      ...
      //*************************************************************************************
      // SAMPLE ORIENTATIONS FROM POSTERIORS FOR EACH POINTS SNE TO PROPAGATE
      // UNCERTAINTY
      //*************************************************************************************
      if (oversample > 1) {
        // build point cloud to store MCMC samples in and associated scalar
        // fields
        samples[r] = new ccSNECloud();
        samples[r]->setName("SNE_Samples");
        samples[r]->copyGlobalShiftAndScale(
            *points[r]); // copy global shift & scale onto new point cloud
        samples[r]->reserve(static_cast<unsigned>(px.size()) * oversample);
        samples[r]->reserveTheNormsTable();
        CCCoreLib::ScalarField *startSF = samples[r]->getScalarField(
            samples[r]->addScalarField(new ccScalarField("StartPoint")));
        CCCoreLib::ScalarField *endSF = samples[r]->getScalarField(
            samples[r]->addScalarField(new ccScalarField("EndPoint")));
        CCCoreLib::ScalarField *idSF = samples[r]->getScalarField(
            samples[r]->addScalarField(new ccScalarField("SegmentID")));
        CCCoreLib::ScalarField *weightSF = samples[r]->getScalarField(
            samples[r]->addScalarField(new ccScalarField("Weight")));
        CCCoreLib::ScalarField *trend = samples[r]->getScalarField(
            samples[r]->addScalarField(new ccScalarField("Trend")));
        CCCoreLib::ScalarField *plunge = samples[r]->getScalarField(
            samples[r]->addScalarField(new ccScalarField("Plunge")));
        CCCoreLib::ScalarField *pointID = samples[r]->getScalarField(
            samples[r]->addScalarField(new ccScalarField("PointID")));
        ...
        std::normal_distribution<double> N(0.0,
                                           stride); // construct random samplers
        ...
        for (unsigned p = 0; p < px.size(); p++) {
          // update progress dialog
          prg.setInfo(
              QStringLiteral("Processing %1 of %2 datasets: Sampling points...")
                  .arg(_d + 1)
                  .arg(datasets.size()));
          ...
          // initialise MCMC sampler at likelihood maxima (to avoid need for
          // burn-in period)
          double lpdProposed;
          ...
          while (count < oversample) {
            ...
            lpdProposed = likPower * logWishart(bestX[p], dof, _phi, _theta,
                                                _alpha, e1, e2, e3, lsf);

            // apply prior?
            if (hasNormals) {
              lpdProposed += log(
                  prior(_phi, _theta, normal[p].x, normal[p].y, normal[p].z));
            }
            ...
            if (log(U(generator)) <= lpdProposed - lpdCurrent) {
              ...
              CCVector3 norm(sin(phi) * cos(theta), cos(phi) * cos(theta),
                             -sin(theta));
            ...
            if (iter > 1000000 * oversample) {
              m_app->dispToConsole("[ccCompass] Warning - MCMC sampler failed "
                                   "so sampling will be incomplete.",
                                   ccMainAppInterface::WRN_CONSOLE_MESSAGE);
              break;
            }
            iter++;
        ...
        // get rid of excess space in arrays (if samples were not generated in
        // sections)
    ...
    {
      if (points[0]->size() > 0 &&
          points[1]->size() > 0) { // do both surfaces have points in them?
        prg.setInfo(QStringLiteral(
                        "Processing %1 of %2 datasets: Estimating thickness...")
                        .arg(_d + 1)
                        .arg(datasets.size()));
        for (int r = 0; r < 2; r++) {
          // make scalar field
          CCCoreLib::ScalarField *thickSF = points[r]->getScalarField(
              points[r]->addScalarField(new ccScalarField("Thickness")));
          ...
          if (samples[r] != nullptr) {
            thickSF_sample = samples[r]->getScalarField(
                samples[r]->addScalarField(new ccScalarField("Thickness")));
            thickSF_sample->reserve(samples[r]->size());
            idSF_sample = samples[r]->getScalarField(
                samples[r]->getScalarFieldIndexByName("PointID"));
            samples[r]->setCurrentDisplayedScalarField(
                samples[r]->getScalarFieldIndexByName("Thickness"));
            samples[r]->showSF(true);
          }
          ...
          CCCoreLib::ReferenceCloud *nCloud =
              new CCCoreLib::ReferenceCloud(points[compID]);
          unsigned char level =
          ...
          for (unsigned p = 0; p < points[r]->size(); p++) {
            ...
            if (r == 0) {
              prg.update((50.0f * p) /
                         points[r]->size()); // first 50% from lower surface
            } else {
              prg.update(
                  50.0f +
                  (50.0f * p) /
                      points[r]->size()); // second 50% from upper surface
            }
            if (prg.isCancelRequested()) {
              // cleanup
              for (int i = 0; i < pinchClouds.size(); i++) {
            ...
            nCloud->clear();
            oct->findPointNeighbourhood(points[r]->getPoint(p), nCloud, 1,
                                        level, d);
            ...
            PointCoordinateType pEq[4]{
                points[r]->getPointNormal(p).x, points[r]->getPointNormal(p).y,
                points[r]->getPointNormal(p).z,
                points[r]->getPoint(p)->dot(points[r]->getPointNormal(p))};

            // calculate point to plane distance
            d = CCCoreLib::DistanceComputationTools::computePoint2PlaneDistance(
            ...
            points[r]->setPointNormal(p, points[r]->getPointNormal(p) *
                                             (d / std::abs(d)));

            // if samples have been generated, also calculate thicknesses for
            ...
            if (samples[r] != nullptr) {
              for (unsigned s = 0; s < samples[r]->size(); s++) {
                    ...
                    p) // find samples matching this point
                {
                  ...
                  PointCoordinateType pEq[4]{
                      samples[r]->getPointNormal(s).x,
                      samples[r]->getPointNormal(s).y,
                      samples[r]->getPointNormal(s).z,
                      samples[r]->getPoint(s)->dot(
                          samples[r]->getPointNormal(s))};
                  d = CCCoreLib::DistanceComputationTools::
                      computePoint2PlaneDistance(nCloud->getPoint(0), pEq);
                  thickSF_sample->setValue(s, std::abs(d));
                  samples[r]->setPointNormal(s, samples[r]->getPointNormal(s) *
                                                    (d / std::abs(d)));
                }
              }
            }
  ...
  // notify finish
  prg.stop();
  m_app->dispToConsole("[ccCompass] Structure normal estimation complete.",
                       ccMainAppInterface::STD_CONSOLE_MESSAGE);
...
void ccCompass::estimateStrain() {
  ...
  if (lines.empty()) {
    m_app->dispToConsole("[ccCompass] Error - no traces or SNEs found to "
                         "compute estimate strain with.",
                         ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return;
  }
  ...
  boxSizeText.setValidator(
      new QDoubleValidator(0.00001, std::numeric_limits<double>::max(), 6));
  ...
  boxSizeText.setToolTip("The voxel size for computing strain. This should be "
                         "large enough that most boxes contain SNEs.");
  externalSNEChk.setToolTip("Use SNE orientation estimates for outside the "
                            "current cell if none are avaliable within it.");
      ...
      for (unsigned p = 0; p < lines[i]->size(); p++) {
        ...
        if (idx < geoObjectBins.size()) {
          ...
          const QString message =
              QStringLiteral("[ccCompass] Error: cell %1 is outside of mesh "
  ...
  //*********************************************
  std::vector<int> nStructures(
      nx * ny * nz,
      0); // number of structures used to compute the strain tensor per cell
  std::vector<int> nIgnored(
      nx * ny * nz,
      0); // number of structured ignored during the above calculation (as they
          // did not have orientation/thickness estimates)
  ...
  I.toIdentity();
  std::vector<CCCoreLib::SquareMatrixd> F(
      nx * ny * nz,
      CCCoreLib::SquareMatrixd(I)); // deformation gradient tensors
  ...
  for (size_t x = 0; x < nx; x++) {
    for (size_t y = 0; y < ny; y++) {
      for (size_t z = 0; z < nz; z++) {
        ...
        // build graphics objects. These are deleted later if no graphics were
        // built.
        ...
        for (ccGeoObject *g : geoObjectBins[idx]) {
          // calculate average dilation vector for this structure (based on the
          // SNEs within this cell)
          ...
          int n_upper = 0;
          ccHObject::Container objs;
          g->filterChildren(objs, true, CC_TYPES::POINT_CLOUD,
                            true); // look for SNE
          for (ccHObject *c : objs) {
            if (ccSNECloud::isSNECloud(c)) {
              ...
              if (s != nullptr) {
                ...
                if (thickSF != -1) {
                  ...
                  if (!(region == ccGeoObject::LOWER_BOUNDARY ||
                        region == ccGeoObject::UPPER_BOUNDARY)) {
                    continue;
                  }

                  // loop through points and only pick those that fall in this
                  // bin
                  for (unsigned p = 0; p < s->size(); p++) {
                    ...
                    if (_idx == idx) {
                      ...
                      if (average_direction.norm2() == 0.0) {
                        ...
                        // avoid vectors pointing in opposite directions (as
                        // opposite normal directions are equivalent)
          ...
          // check that an upper SNE has been observed, otherwise we ignore this
          // structure
          ...
          // increment number of structures that have contributed to the strain
          // in this cell and number of valid cells for which strain can be
          // estimated
          validCells++;
          nStructures[idx]++;

          // build local coordinate system relative to the dyke opening vector
          // (opening vector N, strike vector S, dip vector D)
          ...
          // compute transform matrix that apply's a scaling/stretching equal to
          // the dyke thickness and in the direction of its normal
          CCCoreLib::SquareMatrixd e(3);
          e.toIdentity();
          e.setValue(0, 0,
                     (binSize + average_thickness) /
                         binSize); // stretch matrix in local coordinates
          CCCoreLib::SquareMatrixd F_increment =
              B * (e * B.transposed()); // transform to global coords
          ...
          if (buildGraphics) {
            ...
            gl16[15] = 1.0;
            ccGLMatrix gl(gl16);
            ccGenericPrimitive *box = new ccBox(
                CCVector3d(average_thickness, binSize / 2, binSize / 2).toPC(),
                &gl, "BlockStrain");
  ...
  points->copyGlobalShiftAndScale(
      *lines[0]); // copy global shift & scale from one of the polylines (N.B.
                  // we assume here that all features have the same shift/scale)
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      eSF[i][j] = new ccScalarField(
          QString::asprintf("E%d%d", i + 1, j + 1).toStdString().c_str());
    ...
    for (size_t y = 0; y < ny; y++) {
      for (size_t z = 0; z < nz; z++) {
        ...
        if (nStructures[idx] > 0) {
          // build point
          CCVector3d p(minx + ((x + 0.5) * binSize),
                       miny + ((y + 0.5) * binSize),
                       minz + ((z + 0.5) * binSize));
          ...
          U_local.toIdentity(); // calculate stretch matrix in local (un-rotated
                                // coordinates)
          U_local.setValue(0, 0, sqrt(eigValues[0]));
          U_local.setValue(1, 1, sqrt(eigValues[1]));
          U_local.setValue(2, 2, sqrt(eigValues[2]));
          CCCoreLib::SquareMatrixd U =
              eigVectors.transposed() *
              (U_local * eigVectors); // transform back into global coordinates
          ...
          if (buildGraphics) {
            ...
            eigValues.clear();
            CCCoreLib::Jacobi<double>::ComputeEigenValuesAndVectors(
                F[idx], eigVectors, eigValues, true); // get eigens
            CCCoreLib::Jacobi<double>::SortEigenValuesAndVectors(eigVectors,
                                                                 eigValues);

            // apply exaggeration to eigenvalues (exaggerate shape of the strain
            // ellipse)
            ...
            ccGenericPrimitive *ellipse =
                new ccSphere(static_cast<PointCoordinateType>(binSize / 3), &gl,
                             "StrainEllipse");
            ...
            ccGenericPrimitive *box =
                new ccBox(CCVector3(binSize, binSize, binSize), &T, "GridCell");
            grid->addChild(box);
            box->showWired(true);
...
void ccCompass::estimateP21() {
  ...
  if (lines.empty()) {
    m_app->dispToConsole(
        "[ccCompass] Error - no polylines or traces found to compute P21.",
        ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return;
  }
  ...
  for (ccPolyline *p : lines) {
    ...
    if (outcrop != p->getAssociatedCloud()) {
      m_app->dispToConsole(
          "[ccCompass] Error - cannot calculate P21 intensity for structures "
          "digitised from different point clouds.",
          ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return;
    }

    int sID = ccGeoObject::getGeoObjectRegion(
        p); // get the region of any associated geo-object this polyline relates
            // too.
    double w = 1.0;
    if (sID == ccGeoObject::UPPER_BOUNDARY || ccGeoObject::LOWER_BOUNDARY) {
      w = 0.5; // upper/lower boundaries only count for 0.5 as they should be
               // represented/counted twice.
    }
  ...
  subsampleText.setToolTip(
      "Only sample P21 on the each n'th point in the original outcrop model "
  ...
  m_app->dispToConsole(QString::asprintf("[ccCompass] Estimating P21 Intensity "
                                         "using a search radius of of %f.",
                                         searchR),
                       ccMainAppInterface::STD_CONSOLE_MESSAGE);
  ...
  unsigned char trace_level =
      trace_oct->findBestLevelForAGivenNeighbourhoodSizeExtraction(searchR);
  ...
  for (unsigned p = 0; p < outputCloud->size(); p++) {
    ...
    region.clear();
    trace_oct->getPointsInSphericalNeighbourhood(*outputCloud->getPoint(p),
                                                 searchR, region, trace_level);
  ...
  // loop through points in output cloud again, but this time calculate surface
  // area in regions where n_trace wasn't zero
...
void ccCompass::convertToPointCloud() {
  ...
  for (ccHObject *o : m_app->getSelectedEntities()) {
    if (ccGeoObject::isGeoObject(o)) {
      ...
      o->filterChildren(objs, true,
                        CC_TYPES::POLY_LINE | CC_TYPES::HIERARCHY_OBJECT);
      for (ccHObject *c : objs) {
  ...
  for (ccGeoObject *o : objs) {
    ...
    ccPointCloud *points = new ccPointCloud(
        "ConvertedLines"); // create point cloud for storing points
    int sfid = points->addScalarField(
        new ccScalarField("Region")); // add scalar field containing region info
    ...
    if (points->size() > 0) {
      ...
      m_app->dispToConsole(
          "[Compass] No polylines or traces converted - none found.",
          ccMainAppInterface::WRN_CONSOLE_MESSAGE);
      delete points;
    }
  }
...
void ccCompass::distributeSelection() {
  ...
  if (selection.empty()) {
    m_app->dispToConsole("[Compass] No objects selected.",
                         ccMainAppInterface::WRN_CONSOLE_MESSAGE);
  }
  ...
  m_app->dbRootObject()->filterChildren(search, true,
                                        CC_TYPES::HIERARCHY_OBJECT, false);
  ...
  for (ccHObject *obj : selection) {
    ...
    for (ccGeoObject *g : geoObjs) {
      // find geoObject with biggest matching name (this avoids issues with
      // Object_1 and Object_11 matching)
    ...
    } else // a best match was not found...
    {
      m_app->dispToConsole(
          QString::asprintf(
              "[Compass] Warning: No GeoObject could be found that matches %s.",
              obj->getName().toLatin1().data()),
          ccMainAppInterface::WRN_CONSOLE_MESSAGE);
    }
  }
...
void ccCompass::toggleStipple(bool checked) {
  ccCompass::drawStippled = checked; // change stippling for newly created
                                     // planes
...
{
  // m_app->dispToConsole("ccCompass: Changing to Map mode. Measurements will be
  // associated with GeoObjects.", ccMainAppInterface::STD_CONSOLE_MESSAGE);
...
void ccCompass::addGeoObject(bool singleSurface) // creates a new GeoObject
{
  ...
  // get name
  name = QInputDialog::getText(m_app->getMainWindow(), "New GeoObject",
                               "GeoObject Name:", QLineEdit::Normal, name);
  ...
  for (unsigned i = 0; i < m_app->dbRootObject()->getChildrenNumber(); i++) {
    if (m_app->dbRootObject()->getChild(i)->getName() == "interpretation") {
      ...
      // also search first-level children of root node (when files are re-loaded
      // this is where things will sit)
      for (unsigned c = 0;
           c < m_app->dbRootObject()->getChild(i)->getChildrenNumber(); c++) {
        if (m_app->dbRootObject()->getChild(i)->getChild(c)->getName() ==
            "interpretation") {
...
void ccCompass::exportToSVG() {
  ...
  QString filename = QFileDialog::getSaveFileName(m_dlg, tr("SVG Output file"),
                                                  "", tr("SVG files (*.svg)"));
  if (filename.isEmpty()) {
...
void ccCompass::onSave() {
  ...
  QString filename = QFileDialog::getSaveFileName(
      m_dlg, tr("Output file"), "", tr("CSV files (*.csv *.txt);;XML (*.xml)"));
  if (filename.isEmpty()) {
    // process cancelled by the user
    return;
  }

...
void ccThicknessTool::onNewSelection(
  ...
  for (ccHObject *h : selectedEntities) {
                               ...
                               // confusion]
    {
      ...
      // store plane
      m_referencePlane =
          p; // set the reference plane used to calculate the thickness
      ...
      m_app->getActiveGLWindow()->displayNewMessage(
          "Select measurement point.", ccGLWindowInterface::LOWER_LEFT_MESSAGE);
...
bool ccThicknessTool::pointPicked(ccHObject *insertPoint, unsigned itemIdx,
                                  ccHObject *pickedObject, const CCVector3 &P) {
...
// called when a point in a point cloud gets picked while this tool is active
void ccThicknessTool::pointPicked(ccHObject *insertPoint, unsigned itemIdx,
                                  ccPointCloud *cloud, const CCVector3 &P) {
  // no plane, no deal
  if (!m_referencePlane) {
    ccLog::Error("[Compass] Please select a fit-plane to constrain "
  ...
  if (!ccThicknessTool::TWO_POINT_MODE) // one point mode - calculate plane to
                                        // point distance and finish
  {
...
void ccThicknessTool::recurseChildren(ccHObject *obj, bool hidePointClouds,
                                      bool hidePlanes) {
...
float ccThicknessTool::planeToPointDistance(ccPlane *plane, CCVector3 P) {
  ...
  // return distance
  return CCCoreLib::DistanceComputationTools::computePoint2PlaneDistance(&P,
                                                                         pEq);
}

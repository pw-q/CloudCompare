...
void ccPinchNodeTool::pointPicked(ccHObject *insertPoint, unsigned itemIdx,
  ...
  {
    // throw error
    ccLog::Error("[Compass] PinchNodes can only be added to GeoObjects. Please "
                 "select one!");
    return;
  }

  ccHObject *region = geoObj->getRegion(
      ccGeoObject::INTERIOR); // add pinch-points to GeoObject interior
  if (!region) {

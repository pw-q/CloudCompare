...
static int WritePlanes(ccHObject *rootObject, ccHObject *object,
                       QTextStream &out,
  ...
  // n.b. ccPlanes do not store a global shift/scale like point clouds do, hence
  // this hack. Will only cause issues if CC is being used with multiple point
  // clouds that have different underlying coordinate systems (and hence shift
  // and scales) - which I can't see happening too often. (in any case, 99% of
  //the time the biggest point cloud in the project will be the model being
  //interpreted)
  ...
  if (ccFitPlane::isFitPlane(object)) {
    ...
    ++n;
                 ...
                 CC_TYPES::PLANE)) // not one of our planes, but a plane anyway
                                   // (so we'll export it)
  {
...
static int WriteTraces(ccHObject *object, QTextStream &out,
                       const QString &parentName = QString()) {
  // get object name
  QString name;
  if (parentName.isEmpty()) {
  ...
  if (ccTrace::isTrace(object)) // ensure this is a trace
  {
    ...
    if (p->size() >= 2) {
      ...
      for (unsigned i = 1; i < p->size(); i++) {
        ...
        // write data
        // n.b. csv columns are
        // name,trace_id,seg_id,start_x,start_y,start_z,end_x,end_y,end_z, cost,
        // cost_mode
        out << name << ","; // name
        out << tID << ",";
        out << i - 1 << ",";
...
static int WriteLineations(ccHObject *object, QTextStream &out,
                           const QString &parentName = QString(),
...
int WriteTracesSVG(const ccGLCameraParameters &cameraParams, ccHObject *object,
                   QTextStream &out, int height, float zoom) {
  int n = 0;
  ...
  if (object->isA(CC_TYPES::POLY_LINE) || ccTrace::isTrace(object)) {
    ...
    for (unsigned i = 0; i < line->size(); i++) {
      ...
      out << QString::asprintf(
          ...
          height - (coords2D.y * zoom)); // n.b. we need to flip y-axis
    }
...
int WriteObjectXML(ccHObject *object, QXmlStreamWriter &out) {
  ...
  if (ccGeoObject::isGeoObject(object)) {
    ...
    // write polyline (note that this will ignore "trace" polylines as they have
    // been grabbed earlier)
    out.writeStartElement("POLYLINE");
  } else if (object->isA(CC_TYPES::HIERARCHY_OBJECT)) {
    // write container
    out.writeStartElement(
        "CONTAINER"); // QString::asprintf("CONTAINER name = '%s' id = %d",
                      // object->getName(), object->getUniqueID())
  ...
  if (object->isA(CC_TYPES::PLANE)) {
    ...
    out.writeTextElement(
        "Length",
        QString::asprintf("%f", std::max(P->getXWidth(), P->getYWidth())));

    // if this is just an ordinary plane, make a corresponding fitplane object
    // and then steal metadata
  ...
  if (object->isA(CC_TYPES::POLY_LINE)) {
    ...
    if (poly->size() >= 2) {
      ...
      for (unsigned i = 1; i < poly->size(); i++) {
        ...
        if (trace) {
          int c = trace->getSegmentCost(trace->getPointGlobalIndex(i - 1),
          ...
          if (!hasNormals && !(n1.x == 0 && n1.y == 0 && n1.z == 0)) {
            hasNormals = true; // this was a non-null normal estimate - we will
                               // write normals now
          }
        }
      }
  ...
  if (object->isA(CC_TYPES::POINT_CLOUD)) {
    ...
    out.writeTextElement("GLOBAL_X",
                         QString::asprintf("%f", cloud->getGlobalShift().x));
    out.writeTextElement("GLOBAL_Y",
                         QString::asprintf("%f", cloud->getGlobalShift().y));
    out.writeTextElement("GLOBAL_Z",
                         QString::asprintf("%f", cloud->getGlobalShift().z));
    ...
    if (ccSNECloud::isSNECloud(object)) {
      ...
      QString trend;
      QString plunge;
      CCCoreLib::ScalarField *wSF =
          cloud->getScalarField(cloud->getScalarFieldIndexByName("Weight"));
      CCCoreLib::ScalarField *trendSF =
          cloud->getScalarField(cloud->getScalarFieldIndexByName("Trend"));
      CCCoreLib::ScalarField *plungeSF =
          cloud->getScalarField(cloud->getScalarFieldIndexByName("Plunge"));

      CCCoreLib::ScalarField *tSF =
          cloud->getScalarField(cloud->getScalarFieldIndexByName("Thickness"));
...
void ccCompassExport::SaveCSV(ccMainAppInterface *app,
                              const QString &filename) {
  // write a whole bunch of .csv files

  int planes = 0; // keep track of how many objects are being written (used to
                  // delete empty files)
  int traces = 0;
  int lineations = 0;
  int thicknesses = 0;
  ...
  if (plane_file.open(QIODevice::WriteOnly) &&
    ...
    // write headers
    plane_stream << "Name,Strike,Dip,Dip_Dir,Cx,Cy,Cz,Nx,Ny,Nz,Sample_Radius,"
                    "RMS,Gx,Gy,Gz,Length"
                 << endl;
    trace_stream << "Name,Trace_id,Point_id,Start_x,Start_y,Start_z,End_x,End_"
                    "y,End_z,Cost,Cost_Mode"
                 << endl;
    ...
    if (lineations) {
      app->dispToConsole("[ccCompass] Successfully exported lineation data.",
                         ccMainAppInterface::STD_CONSOLE_MESSAGE);
    } else {
    ...
    if (thicknesses) {
      app->dispToConsole("[ccCompass] Successfully exported thickness data.",
                         ccMainAppInterface::STD_CONSOLE_MESSAGE);
    } else {
    ...
    app->dispToConsole("[ccCompass] Could not open output files... ensure CC "
                       "has write access to this location.",
                       ccMainAppInterface::ERR_CONSOLE_MESSAGE);
  }
}

void ccCompassExport::SaveSVG(ccMainAppInterface *app, const QString &filename,
                              float zoom) {
  ...
  app->dbRootObject()->filterChildren(objects, true, CC_TYPES::OBJECT,
  ...
  if (svg_file.open(QIODevice::WriteOnly)) {
    ...
    int width = std::abs(static_cast<int>(
        app->getActiveGLWindow()->glWidth() *
        zoom)); // glWidth and glHeight are negative on some machines??
    int height =
        std::abs(static_cast<int>(app->getActiveGLWindow()->glHeight() * zoom));

    // write svg header
    svg_stream << QString::asprintf("<svg width=\"%d\" height=\"%d\">", width,
                                    height)
               << endl;

    // write the image
    svg_stream << QString::asprintf("<image height = \"%d\" width = \"%d\" "
    ...
    if (params.perspective) {
      ...
      // m_app->getActiveGLWindow()->redraw(false, false); //not sure if this is
      // needed or not?
      app->getActiveGLWindow()->getGLCameraParameters(
          params); // get updated params
    }

    int count =
        WriteTracesSVG(params, app->dbRootObject(), svg_stream, height, zoom);
    ...
    if (count > 0) {
      app->dispToConsole(QString::asprintf(
          "[ccCompass] Successfully saved %d polylines to .svg file.", count));
    } else {
      // remove file
      svg_file.remove();
      app->dispToConsole(
          "[ccCompass] Could not write polylines to .svg - no polylines found!",
          ccMainAppInterface::WRN_CONSOLE_MESSAGE);
    }
  }
}

void ccCompassExport::SaveXML(ccMainAppInterface *app,
                              const QString &filename) {
  // find root node
  ccHObject *rootObject = app->dbRootObject();
  if (rootObject && rootObject->getChildrenNumber() ==
                        1) // HACK - often the root only has one child (a .bin
  ...
  if (file.open(QIODevice::WriteOnly)) // open the file
  {
    ...
    rootObject->filterChildren(&pointClouds, true, CC_TYPES::POINT_CLOUD,
    true);*/
    ...
    app->dispToConsole("[ccCompass] Successfully exported data-tree to xml.",
                       ccMainAppInterface::STD_CONSOLE_MESSAGE);
  } else {
    app->dispToConsole("[ccCompass] Could not open output files... ensure CC "
                       "has write access to this location.",

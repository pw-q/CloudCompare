...
void ccGeoObject::assignGID() {
  ...
  _gID = static_cast<unsigned>(std::hash<std::string>{}(
...
ccHObject *ccGeoObject::getRegion(int mappingRegion) {
  if (ccGeoObject::isSingleSurfaceGeoObject(this)) {
    return this; // SingleSurface GeoObjects only have a single region; this is
                 // essentially a combined upper, lower and interior
  }
...
int ccGeoObject::getRelationTo(ccGeoObject *obj, ccTopologyRelation **out) {
  ccTopologyRelation *r =
      getRelation(this, getUniqueID(),
  ...
  if (!r) // not found - search for a relation belonging to obj
  {
    ...
    invert = true; // we need to invert backwards relationships (i.e. Obj OLDER
  ...
  if (r) // a relation was found
  {
    *out = r; // set out pointer
    ...
    if (!invert) {
      return r->getType();
    } else { // we need to invert backwards relationships (i.e. Obj OLDER THAN
             // this, so this YOUNGER THAN Obj)
...
ccTopologyRelation *ccGeoObject::addRelationTo(ccGeoObject *obj2, int type,
  ...
  ccTopologyRelation *t = new ccTopologyRelation(verts, older->getUniqueID(),
                                                 younger->getUniqueID(), type);
...
void ccGeoObject::recurseChildren(ccHObject *par, bool highlight) {
  ...
  if (m) {
    ...
    while (p &&
           highlight) // if highlight is set to false, we don't need to bother
    {
    ...
    if (upperBoundary) {
      ...
      m->setAlternate(
          false); // disable alternate colour if it was previously active
    ...
    {
      par->setVisible(true);
    } else // hide annoying graphics on leaving traceMode (we basically only
           // want traces to be visible)
    {

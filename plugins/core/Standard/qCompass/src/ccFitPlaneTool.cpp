...
void ccFitPlaneTool::pointPicked(ccHObject *insertPoint, unsigned itemIdx,
  ...
  if (pPlane) // valid fit
  {
    ...
    if (m_app->getActiveGLWindow()) {
      CCVector3d viewDir =
          m_app->getActiveGLWindow()->getViewportParameters().getViewDir();
    ...
    pPlane->prepareDisplayForRefresh_recursive(); // not sure what this does,
    ...
    m_app->dispToConsole(QString("[ccCompass] Surface orientation estimate = " +
                                 pPlane->getName()),

...
ccCompassDlg::ccCompassDlg(QWidget *parent /*=nullptr*/)
  ...
  m_recalculate = new QAction(
      "Recalculate selection",
      this); // used to recalculate selected traces with new cost function
  ...
  m_rgb->setToolTip(
      "Traces follow points that have a similar color to the start and end "
      "points. Useful if structures being traced have a distinct color.");
  m_grad->setToolTip("Traces follow points in neighbourhoods with high colour "
                     "gradients. Good for contacts.");
  m_curve->setToolTip(
      "Traces follow ridges and valleys. Good for fractures with high relief, "
      "especially in combination with \"Darkness\".");
  m_dist->setToolTip(
      "Traces take the shortest euclidean path (how booring...)");
  m_scalar->setToolTip("Use the active scalar field to define path cost (i.e. "
                       "the path follows low scalar values).");
  m_scalar_inv->setToolTip(
      "Use the inverse of the active scalar field to define path cost (i.e. "
      "the path follows high scalar values).");
  ...
  connect(m_scalar_inv, &QAction::triggered, this,
          &ccCompassDlg::setInvScalarCost);
  ...
  m_plane_fit->setToolTip(
      "If checked, a plane will automatically be fitted to traces matching the "
      "criteria defined in the ccCompass description.");
  m_showStippled->setToolTip(
      "If checked, planes will be drawn partially transparent (stippled).");
  ...
  m_pinchTool->setToolTip("Add Pinch Node objects to record features such as "
                          "dyke tips or sedimentary units that pinch-out.");
  m_loadFoliations->setToolTip(
      "Converts a point cloud containing points (measurement location) and "
      "dip/dip-direction scalar fields to planes.");
  m_loadLineations->setToolTip(
      "Convert a point cloud containing measurement points and trend->plunge "
      "scalar fields into foliation objects.");
  m_toSVG->setToolTip(
      "Export the currently visible trace to a SVG vector graphic using an "
      "orthographic projection of the current view.");
  m_noteTool->setToolTip(
      "Add short notes to a point in a point cloud for future reference.");
  m_measure_thickness->setToolTip("Select a plane and then a point to measure "
                                  "plane-perpendicular thickness.");
  m_measure_thickness_twoPoint->setToolTip(
      "Measure the plane-perpendicular distance between two points.");
  m_youngerThan->setToolTip("Pick two GeoObjects to assign a \"younger-than\" "
                            "(i.e. crosscutting, superposition) relationshi.p");
  m_follows->setToolTip("Select two GeoObjects to assign a \"follows\" (i.e. "
                        "conformable) relationship.");
  m_equivalent->setToolTip("Select two GeoObjects to assign an \"equivalent\" "
                           "(i.e. coeval) relationship.");
  m_fitPlaneToGeoObject->setToolTip("Calculates best fit planes for the entire "
                                    "upper/lower surfaces of the GeoObject.");
  m_mergeSelected->setToolTip(
      "Merge all selected GeoObjects into a single GeoObject.");
  m_estimateNormals->setToolTip("Estimate trace structure normals with maximum "
                                "a-postiori plane fitting algorithm.");
  ...
  // m_pairpicking_menu->addAction(m_youngerThan); //TODO - reenable these once
  // topology code has been fixed m_pairpicking_menu->addAction(m_follows);
  ...
  m_recalculateFitPlanes->setToolTip(
      "Recalculates all fit-planes deriving from traces and GeoObjects (but "
      "not those calculated with the Plane Tool).");
  m_toPointCloud->setToolTip(
      "Converts the selected GeoObject(s) or individual traces to a point "
      "cloud (typically for proximity analysis).");
  m_distributeSelection->setToolTip("Distributes the selected objects into "
                                    "GeoObjects that have matching names.");
  m_estimateP21->setToolTip("Uses structure traces and the mesh octree to "
                            "measure fracture intensity.");
  m_estimateStrain->setToolTip("Estimate bulk strain tensor from Mode-I dykes "
                               "and veins with structure normal estimates.");
  ...
  addOverriddenShortcut(Qt::Key_Space);  // space key also hits the "apply"
                                        // button (easier for some)

  connect(this, &ccOverlayDialog::shortcutTriggered, this,
          &ccCompassDlg::onShortcutTriggered);
}
...
int ccCompassDlg::getCostMode() {
  ...
  if (out == 0)
    return ccTrace::MODE::DISTANCE; // default to distance if everything has
                                    // been unchecked

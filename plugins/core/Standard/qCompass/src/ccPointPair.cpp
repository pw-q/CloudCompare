...
void ccPointPair::drawMeOnly(CC_DRAW_CONTEXT &context) {
  ...
  if (MACRO_Draw3D(context)) {
    ...
    if (!c_headMarker) {
      c_headMarker.reset(
          new ccCone(2.5f, 0.0f, 0.1f, 0, 0, nullptr, "UnitNormalHead", 12));
    ...
    for (unsigned i = 0; i < size(); i++) {
      ...
      float scale =
          context.labelMarkerSize * m_relMarkerScale * 0.2 * fmin(pSize, 4);
      if (viewportParams.perspectiveView && viewportParams.zFar > 0) {
        // in perspective view, the actual scale depends on the distance to the
        // camera!
    ...
    if (size() == 2) // two points
    {
      ...
      ccGLMatrix mat = ccGLMatrix::FromToRotation(
          CCVector3(0, 0, CCCoreLib::PC_ONE), dir); // end = 0,0,1

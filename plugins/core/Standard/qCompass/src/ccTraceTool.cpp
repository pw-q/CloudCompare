...
void ccTraceTool::pointPicked(ccHObject *insertPoint, unsigned itemIdx,
                              ccPointCloud *cloud, const CCVector3 &P) {
  ...
  // if cost function is gradient/curvature then check appropriate SFs have been
  // defined
  if (ccTrace::COST_MODE & ccTrace::GRADIENT) // gradient cost is active
  {
        ...
        !t->isGradientPrecomputed()) // not already computed
    {
      ...
      q = QMessageBox::question(
          ...
          "Precompute Gradient? This can be slow, but once complete will "
          "greatly decrease future computation times.",
  ...
  if (ccTrace::COST_MODE & ccTrace::CURVE) // curvature cost is active
  {
        ...
        !t->isCurvaturePrecomputed()) // not already computed?
    {
      ...
      q = QMessageBox::question(
          ...
          "Precompute Curvature? This can be slow, but once complete will "
          "greatly decrease future computation times.",
          QMessageBox::Yes | QMessageBox::No);
      if (q == QMessageBox::Yes) // do compute
      {
  ...
  if (t->waypoint_count() >= 2) {
    if (!t->optimizePath()) // optimize the path!
    {
      //... problem?
      ccLog::Warning(
          "[Compass] Failed to optimize trace path... please try again.");
  ...
  if (t) {
    ...
    if ((fitPlane && m_changed) ||
      ...
      if (p) {
        ...
        bool child =
            ccCompass::mapMode; // in map mode, plane is stored as a child; in
                                // compass mode, plane is stored as parent
        ...
        m_app->dispToConsole(
            QString("[ccCompass] Trace orientation estimate = " + p->getName()),
            ccMainAppInterface::STD_CONSOLE_MESSAGE);
      } else {
        m_app->dispToConsole(
            QString("[ccCompass] Not enough topography to fit plane to trace."),
...
bool ccTraceTool::pickupTrace(ccHObject *obj) {
  ...
  if (t)                                     // the object is a ccTrace!
  {
    ...
    // check if trace has previously fitted planes before optimizing (and delete
    // them if so)
    if (ccFitPlane::isFitPlane(t->getParent())) {
      ...
      for (unsigned idx = 0; idx < t->getChildrenNumber(); idx++) {
        ...
        if (ccFitPlane::isFitPlane(child)) {
          m_app->removeFromDB(child); // n.b. removeFromDB also deletes the
                                      // child

...
qFacets::qFacets(QObject *parent)
    : QObject(parent), ccStdPluginInterface(":/CC/plugin/qFacets/info.json"),
      m_doFuseKdTreeCells(nullptr), m_fastMarchingExtraction(nullptr),
      m_doExportFacets(nullptr), m_doExportFacetsInfo(nullptr),
      m_doClassifyFacetsByAngle(nullptr), m_doShowStereogram(nullptr),
...
QList<QAction *> qFacets::getActions() {
  ...
  if (!m_doFuseKdTreeCells) {
    ...
    m_doFuseKdTreeCells->setIcon(
        QIcon(QString::fromUtf8(":/CC/plugin/qFacets/images/extractKD.png")));
    // connect signal
  ...
  if (!m_fastMarchingExtraction) {
    m_fastMarchingExtraction =
        new QAction("Extract facets (Fast Marching)", this);
    m_fastMarchingExtraction->setToolTip(
        "Detect planar facets with Fast Marching");
    m_fastMarchingExtraction->setIcon(
        QIcon(QString::fromUtf8(":/CC/plugin/qFacets/images/extractFM.png")));
    // connect signal
  ...
  if (!m_doExportFacets) {
    ...
    m_doExportFacets->setIcon(
        QIcon(QString::fromUtf8(":/CC/plugin/qFacets/images/shpFile.png")));
    // connect signal
  ...
  if (!m_doExportFacetsInfo) {
    ...
    m_doExportFacetsInfo->setToolTip(
        "Exports various information on a set of facets (ASCII CSV file)");
    m_doExportFacetsInfo->setIcon(
        QIcon(QString::fromUtf8(":/CC/plugin/qFacets/images/csvFile.png")));
    // connect signal
  ...
  if (!m_doClassifyFacetsByAngle) {
    m_doClassifyFacetsByAngle =
        new QAction("Classify facets by orientation", this);
    m_doClassifyFacetsByAngle->setToolTip(
        "Classifies facets based on their orienation (dip & dip direction)");
    m_doClassifyFacetsByAngle->setIcon(
        QIcon(QString::fromUtf8(":/CC/plugin/qFacets/images/classifIcon.png")));
    // connect signal
  ...
  if (!m_doShowStereogram) {
    ...
    m_doShowStereogram->setToolTip(
        "Computes and displays a stereogram (+ interactive filtering)");
    m_doShowStereogram->setIcon(
        QIcon(QString::fromUtf8(":/CC/plugin/qFacets/images/stereogram.png")));
    // connect signal
...
void qFacets::extractFacets(CellsFusionDlg::Algorithm algo) {
  ...
  ccPointCloud *pc =
      (m_app->haveOneSelection()
           ...
           : nullptr);
  if (!pc) {
    m_app->dispToConsole("Select one and only one point cloud!",
  ...
  if (algo != CellsFusionDlg::ALGO_FAST_MARCHING &&
    ...
    m_app->dispToConsole("Internal error: invalid algorithm type!",
                         ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return;
  }
  ...
  CCCoreLib::DistanceComputationTools::ERROR_MEASURES errorMeasure =
      CCCoreLib::DistanceComputationTools::RMS;
  ...
  if (sfIdx < 0) {
    m_app->dispToConsole("Couldn't allocate a new scalar field for computing "
                         "fusion labels! Try to free some memory ...",
                         ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return;
  }
  ...
  if (algo == CellsFusionDlg::ALGO_KD_TREE) {
    ...
    if (kdtree.build(s_errorMaxPerFacet / 2, errorMeasure, s_minPointsPerFacet,
                     1000, &pDlg)) {
      qint64 elapsedTime_ms = eTimer.elapsed();
      m_app->dispToConsole(
          QString("[qFacets] Kd-tree construction timing: %1 s")
              .arg(static_cast<double>(elapsedTime_ms) / 1.0e3, 0, 'f', 3),
          ccMainAppInterface::STD_CONSOLE_MESSAGE);

      success = ccKdTreeForFacetExtraction::FuseCells(
          &kdtree, s_errorMaxPerFacet, errorMeasure, s_kdTreeFusionMaxAngle_deg,
          static_cast<PointCoordinateType>(s_kdTreeFusionMaxRelativeDistance),
          true, &pDlg);
    } else {
      m_app->dispToConsole("Failed to build Kd-tree! (not enough memory?)",
                           ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      success = false;
    }
  ...
  if (success) {
    pc->setCurrentScalarField(
        sfIdx); // for AutoSegmentationTools::extractConnectedComponents

    CCCoreLib::ReferenceCloudContainer components;
    if (!CCCoreLib::AutoSegmentationTools::extractConnectedComponents(
            pc, components)) {
      m_app->dispToConsole(
          "Failed to extract fused components! (not enough memory?)",
          ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    } else {
      // we remove the temporary scalar field (otherwise it will be copied to
      // the sub-clouds!)
      ...
      indexSF
          ->link(); // to prevent deletion when calling deleteScalarField below
      ...
      if (group) {
        ...
        m_app->dispToConsole(
            QString("[qFacets] %1 facet(s) where created from cloud '%2'")
                .arg(count)
                .arg(pc->getName()));

        if (error) {
          m_app->dispToConsole("Error(s) occurred during the generation of "
        ...
        m_app->dispToConsole(
            "An error occurred during the generation of facets!",
            ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      } else {
        m_app->dispToConsole(
            "No facet remains! Check the parameters (min size, etc.)",
            ccMainAppInterface::WRN_CONSOLE_MESSAGE);
      }
    }
  } else {
    m_app->dispToConsole("An error occurred during the fusion process!",
                         ccMainAppInterface::ERR_CONSOLE_MESSAGE);
  }
...
ccHObject *qFacets::createFacets(ccPointCloud *cloud,
                                 ...
                                 double maxEdgeLength, bool randomColors,
                                 bool &error) {
  if (!cloud) {
    return nullptr;
  }
    ...
    if (compIndexes && compIndexes->size() >= minPointsPerComponent) {
      ...
      if (!facetCloud) {
        ...
        ccFacet *facet = ccFacet::Create(
            facetCloud, static_cast<PointCoordinateType>(maxEdgeLength), true);
        if (facet) {
          QString facetName = QString("facet %1 (RMS=%2)")
          ...
          if (randomColors) {
            ...
            darkCol.r = static_cast<ColorCompType>(static_cast<double>(col.r) *
                                                   c_darkColorRatio);
            darkCol.g = static_cast<ColorCompType>(static_cast<double>(col.g) *
                                                   c_darkColorRatio);
            darkCol.b = static_cast<ColorCompType>(static_cast<double>(col.b) *
                                                   c_darkColorRatio);
          } else {
            ...
            FacetsClassifier::GenerateSubfamilyColor(col, dip, dipDir, 0, 1,
                                                     &darkCol);
          }
          facet->setColor(col);
          if (facet->getContour()) {
  ...
  for (ccHObject *entity : m_app->getSelectedEntities()) {
    if (entity->isA(CC_TYPES::FACET)) {
      ...
      if (facet->getContour()) // if no contour, we won't be able to save it?!
        facets.insert(facet);
    } else // if (entity->isA(CC_TYPES::HIERARCHY_OBJECT)) //recursively tests
           // group's children
    {
...
struct FacetMetaData {
  ...
  //! Default constructor
  FacetMetaData()
      : facetIndex(-1), center(0, 0, 0), globalCenter(0, 0, 0), normal(0, 0, 1),
        surface(0.0), dip_deg(0), dipDir_deg(0), rms(0.0), familyIndex(0),
        subfamilyIndex(0) {}
};
...
void GetFacetMetaData(ccFacet *facet, FacetMetaData &data) {
  ...
  if (facet->filterChildren(clouds, false, CC_TYPES::POINT_CLOUD, true) != 0) {
    // any cloud directly under the facet should have the right Global Shift
    // information now (either the 'countour points' or the 'origin points')
    data.globalCenter = static_cast<const ccGenericPointCloud *>(clouds.front())
...
void ComputeFacetExtensions(CCVector3 &N, ccPolyline *facetContour,
                            double &horizExt, double &vertExt) {
  // horizontal and vertical extensions
  horizExt = vertExt = 0;

  CCCoreLib::GenericIndexedCloudPersist *vertCloud =
  ...
  if (vertCloud) {
    ...
    if (CCCoreLib::GreaterThanSquareEpsilon(
...
void qFacets::exportFacets() {
  ...
  if (facets.empty()) {
    m_app->dispToConsole("Couldn't find any facet in the current selection!",
                         ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return;
  }
  assert(!facets.empty());
  ...
  if (QFile(filename).exists()) {
    ...
    if (QMessageBox::warning(
            m_app->getMainWindow(), "File already exists!",
  ...
  if (!useNativeOrientation) {
    if (useCustomOrientation) {
      Z = CCVector3(
          static_cast<PointCoordinateType>(fDlg.nXLineEdit->text().toDouble()),
  ...
  for (FacetSet::iterator it = facets.begin(); it != facets.end(); ++it) {
    ...
    if (!useNativeOrientation) {
      ...
      ccPointCloud *pc =
          (newPoly ? dynamic_cast<ccPointCloud *>(newPoly->getAssociatedCloud())
                   : nullptr);
      if (pc) {
        pc->applyGLTransformation_recursive(&oriRotMat);
      } else {
        m_app->dispToConsole(QString("Failed to change the orientation of "
  ...
  if (toSave.getChildrenNumber()) {
    ...
    if (filter.saveToFile(&toSave, fields, filename, params) ==
        CC_FERR_NO_ERROR) {
      m_app->dispToConsole(
          QString("[qFacets] File '%1' successfully saved").arg(filename),
          ccMainAppInterface::STD_CONSOLE_MESSAGE);
    } else {
      m_app->dispToConsole(
          QString("[qFacets] Failed to save file '%1'!").arg(filename),
          ccMainAppInterface::WRN_CONSOLE_MESSAGE);
    }
  }
}
...
void qFacets::classifyFacetsByAngle(ccHObject *group, double angleStep_deg,
                                    double maxDist) {
  assert(m_app);
  if (!m_app)
    return;
  ...
  if (group->isA(CC_TYPES::HIERARCHY_OBJECT)) {
    ...
    if (!success) {
      m_app->dispToConsole("An error occurred while classifying the facets! "
                           "(not enough memory?)",
                           ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return;
    }
...
void qFacets::exportFacetsInfo() {
  ...
  if (!outFile.open(QFile::WriteOnly | QFile::Text)) {
    m_app->dispToConsole(QString("Failed to open file for writing! Check "

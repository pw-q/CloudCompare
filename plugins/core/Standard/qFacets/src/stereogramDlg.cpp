...
public:
  //! Default constructor
  FacetDensityGrid()
      : grid(nullptr), rSteps(0), ddSteps(0), step_R(0), step_deg(0) {
...
StereogramWidget::StereogramWidget(QWidget *parent)
    : QLabel(QString(), parent), m_angularStep_deg(0), m_densityGrid(nullptr),
      m_meanDipDir_deg(-1.0), m_meanDip_deg(-1.0), m_densityColorScale(nullptr),
      m_densityColorScaleSteps(ccColorScale::MAX_STEPS < 256
                                   ? ccColorScale::MAX_STEPS
                                   : 256) // DGM: we can't pass a constant
                                          // initializer (MAX_STEPS) by reference
      ,
      m_ticksFreq(3), m_showHSVRing(false), m_trackMouseClick(false),
      m_clickDip_deg(0.0), m_clickDipDir_deg(0.0), m_clickDipSpan_deg(30.0),
      m_clickDipDirSpan_deg(30.0), m_center(0, 0), m_radius(0) {
  setVisible(true);
...
bool StereogramWidget::init(double angularStep_deg, ccHObject *entity,
  ...
  if (densityGrid->grid) {
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      // unsigned iDip = static_cast<unsigned>(static_cast<double>(dip) /
      // densityGrid->step_deg); //static_cast is equivalent to floor if value >=
      // 0 if (iDip == densityGrid->dSteps) 	iDip--;
      unsigned iDipDir = static_cast<unsigned>(
          dipDir /
          densityGrid
              ->step_deg); // static_cast is equivalent to floor if value >= 0
      ...
      unsigned iR = static_cast<unsigned>(
          R / densityGrid
                  ->step_R); // static_cast is equivalent to floor if value >= 0
    ...
    if (e->button() == Qt::LeftButton) {
      ...
      if (squareDistToCenter <= m_radius * m_radius) {
        ...
        m_clickDip_deg = std::min(
            90.0, 90.0 * sqrt(static_cast<double>(squareDistToCenter)) /
                      static_cast<double>(m_radius));
        m_clickDipDir_deg = CCCoreLib::RadiansToDegrees(
            atan2(static_cast<double>(AB.y()), static_cast<double>(AB.x())));
...
void StereogramWidget::paintEvent(QPaintEvent *event) {
  ...
  if (m_showHSVRing) {
    ...
    if (hsvThickness > 0) {
      QRect rectangle(center.x() - diameter / 2 + 1,
                      ...
                      diameter - 2);
      int angle_span =
          static_cast<int>(m_angularStep_deg * 16.0); // see QPainter::drawPie
      QBrush brush;
      ...
      // dip direction steps (dip dir. in [0,360])
      unsigned ddSteps =
          static_cast<unsigned>(ceil(360.0 / std::max(m_angularStep_deg, 1.0)));
      for (unsigned j = 0; j < ddSteps; ++j) {
        ...
        brush.setColor(QColor(static_cast<int>(col.r), static_cast<int>(col.g),
                              static_cast<int>(col.b), 255));
        painter.setBrush(brush);

        int angle_start =
            static_cast<int>((360.0 - dipDir_deg - m_angularStep_deg + 90.0) *
  ...
  if (m_densityGrid && m_densityColorScale && m_densityGrid->grid &&
    ...
    const double step_rad =
        CCCoreLib::DegreesToRadians(m_densityGrid->step_deg);
    ...
    for (unsigned j = 0; j < m_densityGrid->ddSteps; ++j) {
      ...
      for (unsigned i = 0; i < m_densityGrid->rSteps; ++i, ++d) {
        if (*d != 0) {
          ...
          const ccColor::Rgb *col = m_densityColorScale->getColorByRelativePos(
  ...
  if (m_trackMouseClick) {
    ...
    double R1 =
        radius *
        (std::min(90.0, m_clickDip_deg + m_clickDipSpan_deg / 2) / 90.0);

    const double angle_rad = CCCoreLib::DegreesToRadians(
        m_clickDipDir_deg - m_clickDipDirSpan_deg / 2);
    ...
    // draw concentric limits
    {
      int angle_start = static_cast<int>(
          (360.0 - m_clickDipDir_deg - m_clickDipDirSpan_deg / 2 + 90.0) *
          16.0); // see QPainter::drawPie
...
StereogramDialog::StereogramDialog(ccMainAppInterface *app)
    : QDialog(app->getMainWindow()), Ui::StereogramDialog(),
      m_classifWidget(nullptr), m_colorScaleSelector(nullptr), m_app(app),
  ...
  if (m_app) {
    ...
    ccColorScale::Shared scale =
        csManager->getDefaultScale(ccColorScalesManager::HIGH_CONTRAST);
    if (scale) {
  ...
  connect(colorScaleStepsSpinBox, qOverload<int>(&QSpinBox::valueChanged), this,
          &StereogramDialog::onDensityColorStepsChanged);
  connect(ticksFreqSpinBox, qOverload<int>(&QSpinBox::valueChanged), this,
          &StereogramDialog::onTicksFreqChanged);
  connect(showHSVColorsCheckBox, &QAbstractButton::toggled, this,
          &StereogramDialog::onHSVColorsToggled);

  // interactive filtering mechanism
  connect(filterFacetsGroupBox, &QGroupBox::toggled, this,
          &StereogramDialog::onFilterEnabled);
  connect(dipSpanDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &StereogramDialog::onFilterSizeChanged);
  connect(dipDirSpanDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &StereogramDialog::onFilterSizeChanged);
  connect(dipDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &StereogramDialog::onFilterCenterChanged);
  connect(dipDirDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &StereogramDialog::onFilterCenterChanged);
  connect(m_classifWidget, &StereogramWidget::pointClicked, this,
          &StereogramDialog::onPointClicked);
  connect(exportPushButton, &QAbstractButton::clicked, this,
          &StereogramDialog::exportCurrentSelection);
}

bool StereogramDialog::init(double angularStep_deg, ccHObject *facetGroup,
                            double resolution_deg /*=2.0*/) {
  if (!m_classifWidget)
    return false;
...
void StereogramDialog::onFilterEnabled(bool state) {
  if (m_classifWidget) {
    m_classifWidget->enableMouseTracking(state, dipSpanDoubleSpinBox->value(),
...
void StereogramDialog::updateFacetsFilter(bool enable) {
  ...
  PointCoordinateType halfDipDirSpan =
      static_cast<PointCoordinateType>(dipDirSpanDoubleSpinBox->value()) / 2;
  ...
  // or a cloud?
    ...
    if (enable) {
      ...
      for (unsigned i = 0; i < static_cast<unsigned>(count); ++i) {
        ...
        visTable[i] =
            (visible ? CCCoreLib::POINT_VISIBLE : CCCoreLib::POINT_HIDDEN);
      }
    } else {

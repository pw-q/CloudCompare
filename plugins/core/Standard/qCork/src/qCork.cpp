...
qCork::qCork(QObject *parent /*=nullptr*/)
    : QObject(parent), ccStdPluginInterface(":/CC/plugin/qCork/info.json"),
      m_action(nullptr) {}
...
bool ToCorkMesh(const ccMesh *in, CorkMesh &out,
  ...
  if (outVerts.empty() || outTris.empty()) {
    if (app)
      app->dispToConsole(
          QString("[Cork] Input mesh '%1' is empty?!").arg(in->getName()),
...
struct BoolOpParameters {
  BoolOpParameters()
      : operation(ccCorkDlg::UNION), corkA(nullptr), corkB(nullptr),
        app(nullptr), meshesAreOk(false) {}
...
static bool DoPerformBooleanOp() {
  ...
  try {
      ...
      if (s_params.corkA->isSelfIntersecting()) {
        if (s_params.app)
          s_params.app->dispToConsole(
              QString("[Cork] Mesh '%1' is self-intersecting! Result may be "
                      "jeopardized!")
                  .arg(s_params.nameA),
              ccMainAppInterface::WRN_CONSOLE_MESSAGE);
        ...
        if (s_params.app)
          s_params.app->dispToConsole(
              QString(
                  "[Cork] Mesh '%1' is not closed! Result may be jeopardized!")
                  .arg(s_params.nameA),
              ccMainAppInterface::WRN_CONSOLE_MESSAGE);
        s_params.meshesAreOk = false;
      }
    ...
    if (s_params.app) {
      ...
      s_params.app->dispToConsole(
          QString("[Cork] CSG operation duration: %1 s")
              .arg(timer.elapsed() / 1000.0, 0, 'f', 2));
    }
  } catch (const std::exception &e) {
    if (s_params.app)
      s_params.app->dispToConsole(
          QString("[Cork] Exception caught: %1").arg(e.what()),
          ccMainAppInterface::WRN_CONSOLE_MESSAGE);
    return false;
  }
...
void qCork::doAction() {
  ...
  // launch process
  {
    ...
    if (!future.result()) {
      if (m_app)
        m_app->dispToConsole(s_params.meshesAreOk
                                 ? "Computation failed!"
  ...
  if (m_app) {
    // display the duration time
    m_app->dispToConsole(QString("[Cork] Total duration: %1 s")
                             .arg(timer.elapsed() / 1000.0, 0, 'f', 2));
  }
}

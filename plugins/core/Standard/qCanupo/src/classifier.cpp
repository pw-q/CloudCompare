...
Classifier::Classifier()
    : class1(0), class2(0), absMaxXY(0), axisScaleRatio(1.0f),
      descriptorID(DESC_INVALID), dimPerScale(0) {}
...
float Classifier::classify2D_checkcondnum(const Point2D &P, const Point2D &R,
  ...
  for (size_t i = 0; i < segCount; ++i) {
    ...
    if (denom != 0) {
      // 1. check whether the given pt and the refpt are on different sides of
      // the classifier line
      ...
      if (pathIntersects) {
        ...
        bool refSegIntersects =
            ((i == 0 || beta >= 0) &&
             (i + 1 == segCount ||
              beta * beta < AB.norm2())); // not "beta*beta <= AB.norm2()"
                                          // because the equality case will be
...
bool Classifier::Load(QString filename, std::vector<Classifier> &classifiers,
                      std::vector<float> &scales, QString &error,
  ...
  try {
    ...
    for (unsigned ci = 0; ci < nclassifiers; ++ci) {
      ...
      classifier.scales =
          scales; // all classifiers inside a file have the same scales!
      ...
      classifier.weightsAxis1.resize(fdim + 1);
      {
        for (unsigned i = 0; i <= fdim; ++i)
          file.read(reinterpret_cast<char *>(&classifier.weightsAxis1[i]),
                    sizeof(float));
      }
      classifier.weightsAxis2.resize(fdim + 1);
      {
        for (unsigned i = 0; i <= fdim; ++i)
          file.read(reinterpret_cast<char *>(&classifier.weightsAxis2[i]),
                    sizeof(float));
      }

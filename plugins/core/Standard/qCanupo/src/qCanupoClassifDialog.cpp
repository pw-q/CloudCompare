...
qCanupoClassifDialog::qCanupoClassifDialog(ccPointCloud *cloud,
    ...
    : QDialog(app ? app->getMainWindow() : nullptr), Ui::CanupoClassifDialog(),
      m_app(app), m_cloud(cloud) {
  setupUi(this);
  ...
  if (cloud) {
    ...
    useSFCheckBox->setEnabled(cloud->getCurrentDisplayedScalarField() !=
                              nullptr);
  }
  ...
  if (m_app) {
    ...
    for (size_t i = 0; i < clouds.size(); ++i) {
      if (clouds[i]->isA(
        ...
        cpOtherCloudComboBox->addItem(qCanupoTools::GetEntityName(clouds[i]),
  ...
  connect(mscBrowseToolButton, &QAbstractButton::clicked, this,
...
void qCanupoClassifDialog::loadParamsFromPersistentSettings() {
  ...
  // read out parameters
  // double minScale =
  // settings.value("MinScale",minScaleDoubleSpinBox->value()).toDouble(); double
  // step = settings.value("Step",stepScaleDoubleSpinBox->value()).toDouble();
  // double maxScale =
  ...
  bool useConfThreshold =
      settings.value("UseConfThreshold", useConfThresholdGroupBox->isChecked())
          .toBool();
  ...
  bool additionalSF =
      settings.value("AdditionalSF", generateAdditionalSFsCheckBox->isChecked())
          .toBool();
  bool roughnessSF =
      settings.value("RoughnessSF", generateRoughnessSFsCheckBox->isChecked())
          .toBool();
  int maxThreadCount =
      settings.value("MaxThreadCount", ccQtHelpers::GetMaxThreadCount())
          .toInt(); // always leave one thread/core to let the application
                    // breath
...
void qCanupoClassifDialog::browseMscFile() {
  ...
  settings.beginGroup("Classif");
  QString currentPath =
      settings.value("MscCurrentPath", mscFileLineEdit->text()).toString();

  QString filename =
      QFileDialog::getOpenFileName(this, "Load MSC file", currentPath, "*.msc");
  if (filename.isEmpty())
    return;
...
void qCanupoClassifDialog::browseClassifierFile() {
  ...
  settings.beginGroup("Classif");
  QString currentPath =
      settings.value("CurrentPath", classifFileLineEdit->text()).toString();
  ...
  if (Classifier::Load(filename, classifiers, scales, error, &header, true)) {
    ...
    fileDesc
        << QString("Classifier(s) in file: %1").arg(header.classifierCount);
    ...
    for (size_t i = 0; i < scales.size(); ++i)
      scalesStr.append(QString("%1 ").arg(
          scales[scales.size() - 1 - i])); // reverse order as scales are sorted

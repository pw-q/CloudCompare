...
static float RefinePointClassif(
    const Classifier &classifier, const float confidence, float &distToBoundary,
    ccPointCloud *cloud, ccOctree *octree, unsigned char octreeLevel,
    CCCoreLib::GenericIndexedCloudPersist *corePoints,
    CCCoreLib::DgmOctree *corePointsOctree, unsigned char coreOctreeLevel,
    unsigned coreIndex, PointCoordinateType largestRadius,
  ...
  try {
    ...
    // for each scene data point, find the corresponding core point and check if
    // it is reliable
    ...
    for (int j = 0; j < n; ++j) {
      ...
      if (corePoints == cloud) {
        // if we used the whole cloud as core points, then each point is a core
        // point!
        ...
        if (corePointsOctree->findPointNeighbourhood(
                cloud->getPoint(currentPointIndex), &Yk, 1, coreOctreeLevel,
                maxSquareDist) == 1) {
    ...
    if (class1SFValues.empty()) {
      ...
      distToBoundary =
          -static_cast<float>(class1SFValues.size()) / n; // dist(class 1) < 0
      if (distToBoundary > -0.5f) {
        // too close
        return confidence;
      }
      ...
      for (size_t k = 0; k < smallestSet->size(); ++k) {
        ...
        // classification on either side, take largest and reverse roles if
        // necessary
        ...
        if (qCanupoTools::Fpeq<float>(bestConfidence, conf)) {
          bestSplit.push_back(
              (smallestSet->at(k) +
               largestSet->at(std::min(dichomed, largestSet->size() - 1))) /
              2);
    ...
    // update confidence
    float newConfidence =
        1.0f / (exp(-std::abs(distToBoundary)) + 1.0f); // in [0.5 ; 1]
...
bool qCanupoProcess::Classify(QString classifierFilename,
                              const ClassifyParams &params, ccPointCloud *cloud,
                              CCCoreLib::GenericIndexedCloudPersist *corePoints,
  ...
  unsigned descriptorID = DESC_DIMENSIONALITY;
  {
    ...
    for (size_t i = 0; i < classifiers.size(); ++i) {
      if (!ScaleParamsComputer::GetByID(classifiers[i].descriptorID)) {
        if (app)
          app->dispToConsole(QString("Unhandled descriptor type! (ID = %1)")
      ...
      if (i != 0 &&
        ...
        if (app)
          app->dispToConsole(QString("Can't handle mixed descriptor types!"),
                             ccMainAppInterface::ERR_CONSOLE_MESSAGE);
        return false;
      }
    }
  ...
  if (realCorePoints) {
    ...
    if (mscMetaData.isValid()) {
      bool validMetaData =
          (mscMetaData.type() == QVariant::ByteArray &&
      ...
      if (validMetaData) {
        ...
        if (app)
          app->dispToConsole("[qCanupo] Failed to read core point cloud "
    ...
    for (int step = 0; step < 1; ++step) // fake loop for easy break
    {
      ...
      if (!corePointsDescriptors.scales().empty()) {
        if (qCanupoTools::TestVectorsOverlap(
                scales, corePointsDescriptors.scales()) < scales.size()) {
          if (!silent && QMessageBox::question(
                             parentWidget, "Scales mismatch",
                             "Available descriptors/scales data mismatch with "
                             "classifier's! Compute new descriptors or cancel?",
                             QMessageBox::Yes,
          ...
          computeDescriptors = false; // no need to compute the descriptors as
        ...
        if (!computeDescriptors &&
          ...
          if (!silent &&
              QMessageBox::question(
                  ...
                  "Available descriptors have been computed with another "
                  "descriptor type! Compute new descriptors or cancel?",
                  QMessageBox::Yes,
                  QMessageBox::Cancel) == QMessageBox::Cancel) {
            // cancel process
            break;
      ...
      if (!octree) {
        ...
        if (!octree) {
          if (app)
            app->dispToConsole("Failed to compute input cloud octree!",
                               ccMainAppInterface::ERR_CONSOLE_MESSAGE);
          break;
      ...
      if (computeDescriptors) {
        // check that the selected descriptor (computer) is valid
        {
          ...
          if (!computer) {
            if (app)
              app->dispToConsole(
                  QString("Internal error: unhandled descriptor ID (%1)!")
                      .arg(descriptorID),
                  ccMainAppInterface::ERR_CONSOLE_MESSAGE);
            break;
          }
          ...
          if (computer->needSF() &&
              ((realCorePoints &&
                realCorePoints->getCurrentDisplayedScalarField() == nullptr) ||
               (!realCorePoints &&
                cloud->getCurrentDisplayedScalarField() ==
                    nullptr) // if realCorePoints == nullptr, it means that the
                             // subsampled cloud couldn't be converted to a real
                             // cloud!
               )) {
            if (app)
              app->dispToConsole(
                  QString("To compute this type of descriptor, the core points "
                          "cloud must have an active scalar field!"),
                  ccMainAppInterface::ERR_CONSOLE_MESSAGE);
            break;
          }
        }
        ...
        if (generateRoughnessSF) {
          ...
          for (size_t s = 0; s < scaleCount; ++s) {
            QString sfName = QString(CANUPO_PER_LEVEL_ROUGHNESS_SF_NAME) +
                             QString(" @ scale %1").arg(scales[s]);

            coreRoughnessSFs[s] = new ccScalarField(sfName.toStdString());
            if (!coreRoughnessSFs[s]->resizeSafe(corePoints->size(),
                                                 CCCoreLib::NAN_VALUE)) {
              if (app)
                app->dispToConsole(
                    "Not enough memory to store per-level roughness!",
                    ccMainAppInterface::ERR_CONSOLE_MESSAGE);
        ...
        if (app)
          app->dispToConsole(
              QString("[Canupo] Will use %1 threads")
                  .arg(params.maxThreadCount == 0
                           ? "the max number of"
                           : QString::number(params.maxThreadCount)),
        ...
        if (!qCanupoTools::ComputeCorePointsDescriptors(
          ...
          if (app)
            app->dispToConsole(
                QString("Failed to compute core points descriptors: %1")
                    .arg(errorStr),
                ccMainAppInterface::ERR_CONSOLE_MESSAGE);
          break;
        } else if (invalidDescriptors) {
          if (app)
            app->dispToConsole("[qCanupo] Some descriptors couldn't be "
      ...
      // main classification process
      {
        ...
        if (corePoints == cloud) {
          ...
          if (!corePointsOctree->build(&pDlg)) {
            if (app)
              app->dispToConsole(
                  "Failed to compute core points octree! (not enough memory?)",
                  ccMainAppInterface::ERR_CONSOLE_MESSAGE);
        ...
        const PointCoordinateType &largestRadius =
            scales.front() /
            2; // largest scale is the first by construction in canupo
        unsigned char coreOctreeLevel =
            corePointsOctree->findBestLevelForAGivenNeighbourhoodSizeExtraction(
                params.samplingDist != 0 ? params.samplingDist : largestRadius);
        ...
        while (!pendingPoints.empty()) {
          ...
          pDlg.reset();
          pDlg.setInfo(
              QObject::tr("Remaining points to classify: %1\nSource points: %2")
          ...
          for (size_t i = 0; i < pendingPoints.size(); ++i) {
            ...
            if (classifiers.size() == 1) {
              ...
              float confidence = 1.0f / (exp(-std::abs(distToBoundary)) +
                                         1.0f);     // in [0.5 ; 1]
              ...
              if (confidence < params.confidenceThreshold) {
                ...
                if (params.useActiveSFForConfidence) {
                  ...
                  float newConfidence = RefinePointClassif(
                      classifier, confidence, distToBoundary, cloud,
                      octree.data(), octreeLevel, corePoints, corePointsOctree,
                      coreOctreeLevel, coreIndex, largestRadius,
                      corePointClasses);

                  if (newConfidence < 0) {
                    if (app)
                      app->dispToConsole(
                          "Internal error: failed to refine the classification "
                          "on the boundary (not enough memory)",
                          ccMainAppInterface::ERR_CONSOLE_MESSAGE);
                    return false;
              ...
              if (!unreliable) {
                int theClass = (distToBoundary >= 0 ? classifier.class2
                                                    : classifier.class1);
            ...
            } else // more than one classifier
            {
              ...
              for (std::vector<Classifier>::const_iterator classifierIt =
                       classifiers.begin();
                   classifierIt != classifiers.end(); ++classifierIt) {
                const Classifier &classifier = *classifierIt;

                // uniformize the order, distToBoundary>0 selects the larger
                // class of both
                float distToBoundary = classifier.classify(
                    coreDesc); // DGM: the descriptors may have more values than
                               // the number of scales!
                // if (classifier.class1 > classifier.class2)
                //	distToBoundary = -distToBoundary;

                // int minclass = std::min(classifier.class1,
                // classifier.class2); int maxclass = std::max(classifier.class1,
                // classifier.class2);
                ...
                if (confidence < params.confidenceThreshold) {
                  ...
                  if (params.useActiveSFForConfidence) {
                    ...
                    float newConfidence = RefinePointClassif(
                        classifier, confidence, distToBoundary, cloud,
                        octree.data(), octreeLevel, corePoints,
                        corePointsOctree, coreOctreeLevel, coreIndex,
                        largestRadius, corePointClasses);

                    if (newConfidence < 0) {
                      if (app)
                        app->dispToConsole(
                            "Internal error: failed to refine the "
                            "classification on the boundary (not enough "
                            "memory)",
                            ccMainAppInterface::ERR_CONSOLE_MESSAGE);
                      return false;
                    } else if (newConfidence > confidence) {
              ...
              if (!votes.empty()) {
                ...
                // in case of equality, use the distances to the decision
                // boundary
        ...
        if (processCanceled) {
          if (app)
            app->dispToConsole("[qCanupo] Process cancelled by user!",
                               ccMainAppInterface::WRN_CONSOLE_MESSAGE);
          break;
        }
        ...
        // eventually label the points
        {
          ...
          int classLabelSFIdx = -1;
          {
            ...
            if (classLabelSFIdx >= 0) {
              ...
              if (app)
                app->dispToConsole(
                    "Not enough memory (class label can't be created)!",
                    ccMainAppInterface::ERR_CONSOLE_MESSAGE);
              break;
            }
          }
          ...
          {
            confidenceSFIdx =
                cloud->getScalarFieldIndexByName("CANUPO.confidence");
            ...
            if (confidenceSFIdx >= 0) {
              ...
              app->dispToConsole(
                  "[qCanupo] Not enough memory to store the confidence values!",
                  ccMainAppInterface::WRN_CONSOLE_MESSAGE);
            }
          }
          ...
          if (generateAdditionalSF &&
            ...
            if (app) {
              app->dispToConsole(
                  "[qCanupo] Per-level 'x-y' values can only be extracted from "
                  "descriptor with 2 dimensions per scale!",
                  ccMainAppInterface::WRN_CONSOLE_MESSAGE);
            }
          ...
          if (generateAdditionalSF) {
            // remove any previously generated scalar field starting by
            // CANUPO_PER_LEVEL_ADDITIONAL_SF_NAME
            {
              QStringList toDelete;
              for (unsigned i = 0; i < cloud->getNumberOfScalarFields(); ++i) {
                QString sfName =
                    QString::fromStdString(cloud->getScalarField(i)->getName());
              ...
              for (int j = 0; j < toDelete.size(); ++j) {
                cloud->deleteScalarField(cloud->getScalarFieldIndexByName(
                    toDelete[j].toStdString()));
              }
            }
            ...
            // for each scale
            for (size_t s = 0; s < scaleCount; ++s) {
              QString sfName = QString(CANUPO_PER_LEVEL_ADDITIONAL_SF_NAME) +
                               QString(" @ scale %1").arg(scales[s]);

              // SF with same name (if any) should have already been removed!
              assert(cloud->getScalarFieldIndexByName(sfName.toStdString()) <
                     0);
              ...
              if (!scaleSFs[s]->resizeSafe(cloud->size(), true,
                ...
                if (app)
                  app->dispToConsole(
                      "Not enough memory to store per-level 'x-y' values!",
                      ccMainAppInterface::ERR_CONSOLE_MESSAGE);
          ...
          if (generateRoughnessSF) {
            ...
            // if the output cloud has the same number of points as the core
            // points cloud, no need to duplicate the scalar fields!
            if (corePoints->size() == cloud->size()) {
              for (size_t s = 0; s < coreRoughnessSFs.size(); ++s) {
                // SF with same name (if any) should have already been removed!
                assert(cloud->getScalarFieldIndexByName(
                           coreRoughnessSFs[s]->getName()) < 0);
              ...
              for (size_t s = 0; s < scaleCount; ++s) {
                ...
                roughnessSFs[s] =
                    new ccScalarField(coreRoughnessSFs[s]->getName());

                // SF with same name (if any) should have already been removed!
                assert(cloud->getScalarFieldIndexByName(
                           roughnessSFs[s]->getName()) < 0);

                if (!roughnessSFs[s]->resizeSafe(cloud->size(),
                                                 CCCoreLib::NAN_VALUE)) {
                  if (app)
                    app->dispToConsole(
                        "Not enough memory to store per-level roughness!",
          ...
          for (unsigned i = 0; i < cloud->size(); ++i) {
            ...
            if (corePoints == cloud) {
              // if we used the whole cloud as core points, then each point is a
              // core point
              ...
              assert(corePointsOctree);
              if (corePointsOctree->findPointNeighbourhood(
                      P, &Yk, 1, coreOctreeLevel, maxSquareDist) == 1) {
            ...
            if (confidenceSF) {
              ScalarType confVal = static_cast<ScalarType>(
                  corePointConfidences[nearestCorePointIndex]);
            ...
            if (generateAdditionalSF) {
              ...
              const CorePointDesc &desc =
                  corePointsDescriptors[nearestCorePointIndex];
              assert(desc.params.size() == scaleSFs.size() * dimPerScale);
              for (size_t s = 0; s < scaleSFs.size(); ++s) {
                ScalarType val = (desc.params[s * dimPerScale] -
            ...
            if (generateRoughnessSF) {
              ...
              for (size_t s = 0; s < roughnessSFs.size(); ++s) {
                const ScalarType &val =
                    coreRoughnessSFs[s]->getValue(nearestCorePointIndex);
          ...
          if (error) {
            if (app)
              app->dispToConsole(
                  "Internal error: failed to get nearest core points?!",
                  ccMainAppInterface::ERR_CONSOLE_MESSAGE);
        ...
        if (realCorePoints && !useExistingMetaData) {
          bool proceed = true;
          if (mscMetaData.isValid()) {
            proceed = (silent || QMessageBox::question(
                                     parentWidget, "Overwrite MSC meta-data?",
                                     "Core points cloud already has associated "
                                     "MSC meta-data, should we overwrite them?",
                                     QMessageBox::Yes,
          ...
          if (proceed) {
            ...
            if (mscMetaData.isValid()) {
              ...
              if (app)
                app->dispToConsole(
                    QString("[qCanupo] MSC descriptors have been saved as "
                            "meta-data (cloud '%1')")
                        .arg(realCorePoints->getName()));
            } else {
              if (app)
                app->dispToConsole("[qCanupo] Failed to save MSC meta-data "
                                   "(not enough memory?)",
                                   ccMainAppInterface::WRN_CONSOLE_MESSAGE);
            }
          }

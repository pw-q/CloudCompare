...
void ComputeCorePointDescriptor(unsigned index) {
  ...
  int n =
      s_computeCorePointsDescParams.octree->getPointsInSphericalNeighbourhood(
          *P, maxRadius, neighbours, s_computeCorePointsDescParams.octreeLevel);
  ...
  if (n != 0) {
    ...
    for (size_t i = 0; i < scaleCount; ++i) {
      ...
      if (i != 0) {
        ...
        CCCoreLib::DgmOctree::PointDescriptor fakeDesc(nullptr, 0,
                                                       squareRadius);
        CCCoreLib::DgmOctree::NeighboursSet::iterator up =
            std::upper_bound(neighbours.begin(), neighbours.end(), fakeDesc,
                             CCCoreLib::DgmOctree::PointDescriptor::distComp);
      ...
      if (s_computeCorePointsDescParams.roughnessSFs) {
        ...
        if (subset.size() >= 3) {
          // to compute we take the nearest point to the query point as
          // 'central' point warning: it should work in most of the cases, apart
          // if the core points have nothing to do with the global cloud!!!
          ...
          if (lsPlane) {
            ...
            const CCVector3 *centralPoint =
                s_computeCorePointsDescParams.sourceCloud->getPoint(
                    globalIndex);
            roughness = std::abs(
      ...
      s_computeCorePointsDescParams.computer->computeScaleParams(
          subset, radius, &(desc.params[i * dimPerScale]), invalidScale);
      ...
      if (invalidScale) {
        ...
        for (size_t j = i + 1; j < scaleCount; ++j) {
          ...
          memcpy(&(desc.params[j * dimPerScale]),
                 &(desc.params[i * dimPerScale]), sizeof(float) * dimPerScale);
        }
        // neighbours.clear();
        // subset.clear(true);
        break;
...
bool qCanupoTools::ComputeCorePointsDescriptors(
    CCCoreLib::GenericIndexedCloud *corePoints,
    CorePointDescSet &corePointsDescriptors, ccGenericPointCloud *sourceCloud,
    const std::vector<float> &sortedScales, bool &invalidDescriptors,
    QString &error, // if any
    unsigned descriptorID /*=DESC_DIMENSIONALITY*/, int maxThreadCount /*=0*/,
  ...
  if (success)
    success = corePointsDescriptors.setScales(
        sortedScales); // automatically resizes the 'params' structure for each
                       // core point
  if (!success) {
...
bool qCanupoTools::TrainClassifier(
    Classifier &classifier, const CorePointDescSet &descriptors1,
    const CorePointDescSet &descriptors2, const std::vector<float> &scales,
  ...
  try {
    ...
    if (app)
      app->dispToConsole("Not enough memory to prepare the samples!",
                         ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return false;
  }
  ...
  // add class #1 data
  {
    for (size_t i = 0; i < nsamples1; ++i) {
      ...
      size_t shift = (paramsCount - scaleCount) *
  ...
  LDATrainer trainer;
  {
    ...
    try {
      ...
      if (app)
        app->dispToConsole("Oops, it seems the LDA classifier just crashed!",
                           ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return false;
    }
  }
  ...
  std::vector<float> proj1;
  {
    try {
      ...
      if (app)
        app->dispToConsole("Not enough memory to project the samples!",
                           ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return false;
    }
  ...
  std::vector<LDATrainer::sample_type> samples_reduced;
  {
    try {
      ...
      if (app)
        app->dispToConsole("Not enough memory to compute 'reduced' samples!",
                           ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return false;
    }
  ...
  // convert back the classifier weights into the original space
  {
    try {
      ...
      if (app)
        app->dispToConsole(
            "Not enough memory to compute the classifier weights!",
            ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return false;
    }
  ...
  std::vector<float> proj2;
  {
    try {
      ...
      if (app)
        app->dispToConsole("Not enough memory to project samples!",
                           ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return false;
    }
  ...
  ComputeReferencePoints(classifier.refPointPos, classifier.refPointNeg, proj1,
                         proj2, labels);
  ...
  if (true) {
    ...
    if (!DilateClassifier(classifier, proj1, proj2, labels, samples, trainer,
                          orthoTrainer)) {
      if (app)
        app->dispToConsole("Not enough memory to 'dilate' the classifier!",
                           ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return false;
    }
  }
  ...
  // proceed to boundary evaluation
  {
    ...
    if (!mscCloud->reserve(cloudSize)) {
      if (app)
        app->dispToConsole(
            "Not enough memory to determine the classifier behavior!",
            ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return false;
    }
...
// DGM: we only use the evaluation cloud for representation now!
...
#endif
    {
      ...
      for (int sd = 1; sd < c_searchSteps; ++sd) {
        Classifier::Point2D v(tableCosSin.cosines[sd], tableCosSin.sines[sd]);
        ...
        for (size_t i = 0; i < nsamples; ++i) {
          ...
          P = basis * P;
          const double &d =
              P(0); // projection on w_vect along the slanted direction
          if (labels[i] < 0) {
            p1.push_back(d);
            m1 += d;
          } else {
        ...
        for (int i = 0; i <= 100; ++i) {
          ...
          size_t idx1 =
              std::lower_bound(p1.begin(), p1.end(), pos) - p1.begin();
          size_t idx2 =
              std::lower_bound(p2.begin(), p2.end(), pos) - p2.begin();

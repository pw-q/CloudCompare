...
public:
  ...
  // inherited from ScaleParamsComputer
  void computeScaleParams(CCCoreLib::ReferenceCloud &neighbors, double radius,
                          float params[], bool &invalidScale) override {
    // PCA analysis
    if (neighbors.size() >= 3) {
      ...
      if (CCCoreLib::Jacobi<double>::ComputeEigenValuesAndVectors(
        ...
        CCCoreLib::Jacobi<double>::SortEigenValuesAndVectors(
            eigVectors,
            eigValues); // decreasing order of their associated eigenvalues
        ...
        double a =
            std::min<double>(1.0, std::max<double>(0.0, sValues.x - sValues.y));
        double b = std::min<double>(
            1.0, std::max<double>(0.0, 2 * sValues.x + 4 * sValues.y - 2.0));
        double c = 1.0 - a - b;
...
public:
  ...
  void computeScaleParams(CCCoreLib::ReferenceCloud &neighbors, double radius,
    ...
    if (neighbors.size() >= 6) {
      CCCoreLib::Neighbourhood Z(&neighbors);
      params[0] = Z.computeCurvature(*neighbors.getPoint(0),
...
public:
        ...
        //inherited from ScaleParamsComputer
        void computeScaleParams(CCCoreLib::ReferenceCloud& neighbors, double
radius, float params[], bool& invalidScale) override
        {
                //Curvature
                if (neighbors.size() >= ?)
                {
...
QByteArray CorePointDescSet::toByteArray() const {
  ...
  try {
    ...
    *reinterpret_cast<int *>(buffer) = static_cast<int>(m_dimPerScale);
    ...
    // descriptors
    {
      for (int j = 0; j < descCount; ++j) {
        ...
        assert(desc.params.size() ==
               static_cast<size_t>(scaleCount) * m_dimPerScale);
    ...
    ccLog::Warning("Not enough memory to convert the core point descriptors to "
                   "a QByteArray");
    return {};
  }
}
...
bool CorePointDescSet::fromByteArray(const QByteArray &data) {
  ...
  // descriptors
  {
    for (int j = 0; j < descCount; ++j) {
      ...
      assert(desc.params.size() ==
             static_cast<size_t>(scaleCount) * m_dimPerScale);
      for (size_t i = 0; i < static_cast<size_t>(scaleCount) * m_dimPerScale;
           ++i) {
...
bool CorePointDescSet::loadFromMSC(QString filename, QString &error,
                                   ccPointCloud *corePoints /*=nullptr*/) {
  error.clear();

  std::ifstream mscfile(
      qPrintable(filename),
      std::ifstream::binary); // DGM: warning, toStdString doesn't preserve
                              // "local" characters
  ...
  // read scales
  {
    ...
    if (!setScales(
            scales)) // automatically resize the descriptors 'params' structure
    {
      error = "Not enough memory to load scales";
      return false;
    }
  ...
  // Put the points in the cloud, keep the multiscale information in a separate
  // vector matched by point index
  int ptnparams;
  ...
  if (corePoints) {
    ...
    for (int i = 3; i < ptnparams; ++i) {
      int sfIdx = corePoints->addScalarField(
          QString("scalar #%1").arg(i - 2).toStdString());
      paramsSf.push_back(sfIdx >= 0 ? corePoints->getScalarField(sfIdx)
                                    : nullptr);

      if (sfIdx < 0) {
        // just a warning in fact
        error = "Not enough memory to import additional scalars! (they are not "
                "used for classification anyway)";
      }
    }
  }
      ...
      if (paramsSf[i]) {
        ...
        if (first) {
          corePoints->setCurrentDisplayedScalarField(
              corePoints->getScalarFieldIndexByName(paramsSf[i]->getName()));

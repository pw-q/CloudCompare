...
qCanupoTrainingDialog::qCanupoTrainingDialog(ccMainAppInterface *app)
    : QDialog(app ? app->getMainWindow() : nullptr), Ui::CanupoTrainingDialog(),
      m_app(app) {
  setupUi(this);
  ...
  if (m_app) {
    ...
    // if 3 clouds are loaded, then there's chances that the first one is the
    // global  cloud!
    ...
    if (cloudCount < 2 && app)
      app->dispToConsole("You need at least 2 loaded clouds to train a "
                         "classifier (one per class)",
  ...
  connect(cloud1ClassSpinBox, qOverload<int>(&QSpinBox::valueChanged), this,
          &qCanupoTrainingDialog::onClassChanged);
  connect(cloud2ClassSpinBox, qOverload<int>(&QSpinBox::valueChanged), this,
          &qCanupoTrainingDialog::onClassChanged);

  connect(class1CloudComboBox, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &qCanupoTrainingDialog::onCloudChanged);
  connect(class2CloudComboBox, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &qCanupoTrainingDialog::onCloudChanged);
  ...
  try {
    if (scalesRampRadioButton->isChecked()) {
      ...
      if (maxScale < minScale || maxScale < 0 || step < 1.0e-6)
        return false;
      unsigned stepCount =
          static_cast<unsigned>((maxScale - minScale) / step + 1.0e-6) +
      ...
      QStringList scaleList =
          scalesListLineEdit->text().split(' ', QString::SkipEmptyParts);
...
void qCanupoTrainingDialog::loadParamsFromPersistentSettings() {
  ...
  bool scalesRampEnabled =
      settings.value("ScalesRampEnabled", scalesRampRadioButton->isChecked())
          .toBool();

  unsigned maxPoints =
      settings.value("MaxPoints", maxPointsSpinBox->value()).toUInt();
  int classifParam =
      settings.value("ClassifParam", paramComboBox->currentIndex()).toInt();
  int maxThreadCount =

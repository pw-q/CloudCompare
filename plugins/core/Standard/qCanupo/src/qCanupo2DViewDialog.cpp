...
qCanupo2DViewDialog::qCanupo2DViewDialog(
    const CorePointDescSet *descriptors1, const CorePointDescSet *descriptors2,
    QString cloud1Name, QString cloud2Name, int class1 /*=1*/,
    int class2 /*=2*/,
    const CorePointDescSet *evaluationDescriptors /*=nullptr*/,
    ccMainAppInterface *app /*=nullptr*/)
    : QDialog(app ? app->getMainWindow() : nullptr), Ui::Canupo2DViewDialog(),
      m_app(app), m_glWindow(nullptr), m_classifierSaved(false),
      m_descriptors1(descriptors1), m_descriptors2(descriptors2),
      m_evaluationDescriptors(evaluationDescriptors), m_class1(class1),
      m_cloud1Name(cloud1Name), m_cloud2Name(cloud2Name), m_class2(class2),
      m_cloud(nullptr), m_poly(nullptr), m_polyVertices(nullptr),
      m_selectedPointIndex(-1), m_pickingRadius(5) {
  setupUi(this);
  ...
  // setup 2D view
  {
    ...
    m_glWindow->setInteractionMode(
        ccGLWindowInterface::MODE_PAN_ONLY |
    ...
    connect(m_glWindow->signalEmitter(), &ccGLWindowSignalEmitter::mouseMoved,
            this, &qCanupo2DViewDialog::moveSelectedPoint);
    connect(m_glWindow->signalEmitter(),
  ...
  connect(resetToolButton, &QAbstractButton::clicked, this,
          &qCanupo2DViewDialog::resetBoundary);
  connect(statisticsToolButton, &QAbstractButton::clicked, this,
          &qCanupo2DViewDialog::computeStatistics);
  connect(savePushButton, &QAbstractButton::clicked, this,
          &qCanupo2DViewDialog::saveClassifier);
  connect(donePushButton, &QAbstractButton::clicked, this,
          &qCanupo2DViewDialog::checkBeforeAccept);

  connect(pointSizeSpinBox, qOverload<int>(&QSpinBox::valueChanged), this,
          &qCanupo2DViewDialog::setPointSize);
  connect(scalesCountSpinBox, qOverload<int>(&QSpinBox::valueChanged), this,
          &qCanupo2DViewDialog::onScalesCountSpinBoxChanged);
}
...
void qCanupo2DViewDialog::getActiveScales(std::vector<float> &scales) const {
  ...
  for (int i = 0; i < currentScaleCount; ++i) {
    // take only the smallest values (scales are sorted from the biggest to the
    // smallest)
...
bool qCanupo2DViewDialog::trainClassifier() {
  ...
  // show reference points as spheres
  {
    ...
    ccGLMatrix matPos;
    matPos.setTranslation(
        CCVector3(m_classifier.refPointPos.x, m_classifier.refPointPos.y, 0));
    ...
    ccGLMatrix matNeg;
    matNeg.setTranslation(
        CCVector3(m_classifier.refPointNeg.x, m_classifier.refPointNeg.y, 0));
...
void qCanupo2DViewDialog::checkBeforeAccept() {
  if (!m_classifierSaved) {
    if (QMessageBox::warning(this, "Classifier has not been saved!",
                             "Do you really want to close the dialog before "
...
void qCanupo2DViewDialog::resetBoundary() {
  ...
  if (pathLength > 1) {
    if (!m_poly->reserve(pathLength) || !m_polyVertices->reserve(pathLength))
      return;

    for (unsigned i = 0; i < pathLength; ++i)
      m_polyVertices->addPoint(
          CCVector3(m_classifier.path[i].x, m_classifier.path[i].y,
...
void qCanupo2DViewDialog::updateZoom() {
  ccBBox box =
      m_glWindow->getOwnDB()->getDisplayBB_recursive(false, m_glWindow);
...
void qCanupo2DViewDialog::addOrSelectPoint(int x, int y) {
  ...
  double maxPickingDist = static_cast<double>(m_pickingRadius) *
                          m_glWindow->computeActualPixelSize();
...
void qCanupo2DViewDialog::removePoint(int x, int y) {
  ...
  double maxPickingDist =
      m_pickingRadius * m_glWindow->computeActualPixelSize();
...
void qCanupo2DViewDialog::moveSelectedPoint(int x, int y,

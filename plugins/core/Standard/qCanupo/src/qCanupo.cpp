...
qCanupoPlugin::qCanupoPlugin(QObject *parent /*=nullptr*/)
    : QObject(parent),
      ...
      m_classifyAction(nullptr), m_trainAction(nullptr) {}
...
void qCanupoPlugin::onNewSelection(
  ...
  if (m_trainAction) {
    m_trainAction->setEnabled(
        m_app && m_app->dbRootObject() &&
        m_app->dbRootObject()->getChildrenNumber() !=
            0); // need some loaded entities to train the classifier!
  }
...
QList<QAction *> qCanupoPlugin::getActions() {
  ...
  if (!m_trainAction) {
    ...
    m_trainAction->setIcon(QIcon(
        QString::fromUtf8(":/CC/plugin/qCanupoPlugin/images/iconCreate.png")));
    connect(m_trainAction, &QAction::triggered, this,
  ...
  if (!m_classifyAction) {
    ...
    connect(m_classifyAction, &QAction::triggered, this,
...
void qCanupoPlugin::doClassifyAction() {
  ...
  if (m_selectedEntities.empty() ||
      !m_selectedEntities.front()->isA(CC_TYPES::POINT_CLOUD)) {
  ...
  ccPointCloud *realCorePoints =
      nullptr; // the core point cloud (as a real point cloud, if available)
  CCCoreLib::GenericIndexedCloudPersist *corePoints =
      nullptr; // the core points, potentially as references!
  ...
  case qCanupoClassifDialog::OTHER: {
    ...
    if (!realCorePoints) {
      assert(false);
      m_app->dispToConsole("Internal error: failed to access core pointss?!",
                           ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return;
    }
  ...
  case qCanupoClassifDialog::SUBSAMPLED: {
    ...
    CCCoreLib::ReferenceCloud *refCloud =
        CCCoreLib::CloudSamplingTools::resampleCloudSpatially(
            cloud, samplingDist, modParams, nullptr, &pDlg);
    if (!refCloud) {
      m_app->dispToConsole("Failed to compute sub-sampled core points!",
                           ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return;
    }
    ...
    // try to convert the references into a real point cloud (not mandatory
    // though!)
    realCorePoints = cloud->partialClone(refCloud);
    if (realCorePoints) {
      realCorePoints->setName(
          cloud->getName() +
          QString(".core points (subsampled @ %1)").arg(samplingDist));
  ...
  case qCanupoClassifDialog::MSC_FILE: {
    ...
    if (!corePointsDescriptors.loadFromMSC(filenmae, error, realCorePoints)) {
      ...
      m_app->dispToConsole(QString("[qCanupo] ") + error,
  ...
  default: {
    assert(false);
    m_app->dispToConsole("Internal error: no core point source specified?!",
                         ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return;
  }
  }
...
void qCanupoPlugin::doTrainAction() {
  ...
  //clouds!",ccMainAppInterface::ERR_CONSOLE_MESSAGE); 	return;
  //}
  //
  ...
  std::vector<float> scales;
  {
    if (!ctDlg.getScales(scales)) {
      m_app->dispToConsole("Invalid scale parameters!",
                           ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return;
    }
  ...
  // check that the selected descriptor (computer) is valid
  {
    ...
    if (computer->needSF() &&
        (cloud1->getCurrentDisplayedScalarField() == nullptr ||
         ...
         (evaluationCloud &&
          evaluationCloud->getCurrentDisplayedScalarField() == nullptr))) {
      if (m_app)
        m_app->dispToConsole(
            QString("To compute this type of descriptor, all clouds must have "
                    "an active scalar field!"),
            ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return;
    }
  }
  ...
  while (true) {
    // sub-sample clouds (if necessary)
    {
        ...
        if (evaluationCloud->size() > maxCorePoints)
          evaluationPoints =
              CCCoreLib::CloudSamplingTools::subsampleCloudRandomly(
                  evaluationCloud, maxCorePoints, &pDlg);
        else
          evaluationPoints = evaluationCloud;

        if (!evaluationPoints) {
          m_app->dispToConsole(
              "Failed to compute sub-sampled version of evaluation points!",
              ccMainAppInterface::ERR_CONSOLE_MESSAGE);
          break;
        }
      }
      ...
      if (cloud1->size() > maxCorePoints)
        corePoints1 = CCCoreLib::CloudSamplingTools::subsampleCloudRandomly(
      ...
      if (!corePoints1) {
        m_app->dispToConsole(
            "Failed to compute sub-sampled version of cloud #1!",
            ccMainAppInterface::ERR_CONSOLE_MESSAGE);
        break;
      }

      if (cloud2->size() > maxCorePoints)
        corePoints2 = CCCoreLib::CloudSamplingTools::subsampleCloudRandomly(
    ...
    CorePointDescSet descriptors1;
    {
      ...
      if (!qCanupoTools::ComputeCorePointsDescriptors(corePoints1,
        ...
        m_app->dispToConsole("[qCanupo] Some descriptors couldn't be computed "
                             "on cloud#1 (min scale may be too small)!",
                             ccMainAppInterface::WRN_CONSOLE_MESSAGE);
      }
    }
    ...
    CorePointDescSet descriptors2;
    {
      ...
      if (!qCanupoTools::ComputeCorePointsDescriptors(corePoints2,
        ...
        m_app->dispToConsole("[qCanupo] Some descriptors couldn't be computed "
                             "on cloud#2 (min scale may be too small)!",
                             ccMainAppInterface::WRN_CONSOLE_MESSAGE);
      }
    }
    ...
    if (evaluationPoints) {
      ...
      if (!qCanupoTools::ComputeCorePointsDescriptors(
              evaluationPoints, evaluationDescriptors, evaluationCloud, scales,
      ...
      if (invalidDescriptors) {
        m_app->dispToConsole(
            "[qCanupo] Some descriptors couldn't be computed on evaluation "
            "cloud (min scale may be too small)!",
            ccMainAppInterface::WRN_CONSOLE_MESSAGE);
      }
    }
    ...
    // now for the Classifier training!
    {
      qCanupo2DViewDialog c2DDlg(
          &descriptors1, &descriptors2, cloud1->getName(), cloud2->getName(),
          ctDlg.cloud1ClassSpinBox->value(), ctDlg.cloud2ClassSpinBox->value(),
          evaluationPoints ? &evaluationDescriptors : nullptr, m_app);
      ...
      if (!c2DDlg.trainClassifier()) {
        ...
        m_app->dispToConsole("[qCanupo] Classifier training failed...",
                             ccMainAppInterface::WRN_CONSOLE_MESSAGE);
        break;
      }

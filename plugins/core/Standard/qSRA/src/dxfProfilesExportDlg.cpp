...
void DxfProfilesExportDlg::initFromPersistentSettings() {
  ...
  const QString defaultVertProfile(ccFileUtils::defaultDocPath() +
                                   "/vert_profiles.dxf");
  const QString defaultHorizProfile(ccFileUtils::defaultDocPath() +
                                    "/horiz_profiles.dxf");
  ...
  QString theoTitle =
      settings.value("legendTheoTitle", theoNameLineEdit->text()).toString();
  QString realTitle =
      settings.value("legendRealTitle", realNameLineEdit->text()).toString();
  int angularSteps =
      settings.value("angularSteps", angularStepsSpinBox->value()).toInt();
  int heightSteps =
...
void DxfProfilesExportDlg::browseVertFile() {
  ...
  QString filename = QFileDialog::getSaveFileName(
      nullptr, "Select output file", vertOutputFileLineEdit->text(), filter);
...
void DxfProfilesExportDlg::browseHorizFile() {
  ...
  // open file loading dialog
  QString filename = QFileDialog::getSaveFileName(
      nullptr, "Select output file", horizOutputFileLineEdit->text(), filter);

...
bool DxfProfilesExporter::SaveVerticalProfiles(
    ...
    ccPolyline *profile, QString filename, unsigned angularStepCount,
    double heightStep, double heightShift, const Parameters &params,
  ...
  double yMin =
      std::max(map->yMin, static_cast<double>(profileBBMin.y) + heightShift);
  ...
  if (!dw) {
    if (app)
      app->dispToConsole(
          QString("Failed to open '%1' file for writing!").arg(filename),
          ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return false;
  }
  ...
  QStringList profileNames;
  {
    ...
    dxf.writeLayer(*dw, DL_LayerData(PROFILE_LAYER, 0),
                   DL_Attributes(std::string(""), DL_Codes::red, s_lineWidth,
                                 "CONTINUOUS", 1.0));

    // legend layer
    dxf.writeLayer(*dw, DL_LayerData(LEGEND_LAYER, 0),
                   DL_Attributes(std::string(""), DL_Codes::black, s_lineWidth,
                                 "CONTINUOUS", 1.0));
      ...
      dxf.writeLayer(
          *dw,
          ...
          DL_Attributes(std::string(""),
                        i == 0 ? DL_Codes::green
                               : -DL_Codes::green, // invisible if negative!
  ...
  // Writing the Entities Section
  {
    ...
    // write theoretical profile (polyline)
    {
      ...
      dxf.writePolyline(
          *dw, DL_PolylineData(static_cast<int>(vertexCount), 0, 0, 0),
          DL_Attributes(PROFILE_LAYER, DL_Codes::bylayer, -1, "BYLAYER", 1.0));

      for (unsigned i = 0; i < vertexCount; ++i) {
        const CCVector3 *P = profile->getPoint(i);
        dxf.writeVertex(
            *dw, DL_VertexData(x0 + (P->x - xMin) * scale,
                               y0 + (P->y + heightShift - yMin) * scale, 0.0));
      }
    ...
    // write legend
    {
      DL_Attributes DefaultLegendMaterial(LEGEND_LAYER, DL_Codes::bylayer, -1,
                                          "BYLAYER", 1.0);
      ...
      // write page contour
      {
        ...
        dxf.writeVertex(*dw,
                        DL_VertexData(c_pageMargin_mm, c_pageMargin_mm, 0.0));
        dxf.writeVertex(*dw,
        ...
        dxf.writeVertex(*dw,
                        DL_VertexData(c_pageWidth_mm - c_pageMargin_mm,
                                      c_pageHeight_mm - c_pageMargin_mm, 0.0));
        dxf.writeVertex(*dw, DL_VertexData(c_pageWidth_mm - c_pageMargin_mm,
                                           c_pageMargin_mm, 0.0));
      ...
      dxf.writeText(
          *dw,
          DL_TextData(xLegend, yLegend, 0.0, xLegend, yLegend, 0.0,
                      ...
                      qPrintable(QString("Deviation magnification factor: ") +
                                 magnifyStr),
                      "STANDARD", 0.0), // DGM: warning, toStdString doesn't
      ...
      dxf.writeLine(
          *dw,
          DL_LineData(xLegend, yLegend, 0, xLegend + legendWidth_mm, yLegend,
                      0.0),
          DL_Attributes(LEGEND_LAYER, DL_Codes::green, -1, "BYLAYER", 1.0));

      dxf.writeText(
          *dw,
          DL_TextData(xLegend + legendWidth_mm + c_textMargin_mm, yLegend, 0.0,
                      xLegend + legendWidth_mm + c_textMargin_mm, yLegend, 0.0,
                      c_textHeight_mm, 1.0, 0, 0, 0,
                      qPrintable(params.legendRealProfileTitle), "STANDARD",
                      0.0), // DGM: warning, toStdString doesn't preserve
                            // "local" characters
          DefaultLegendMaterial);
      ...
      dxf.writeText(
          *dw,
          DL_TextData(xLegend + legendWidth_mm + c_textMargin_mm, yLegend, 0.0,
                      xLegend + legendWidth_mm + c_textMargin_mm, yLegend, 0.0,
                      c_textHeight_mm, 1.0, 0, 0, 0,
                      qPrintable(params.legendTheoProfileTitle), "STANDARD",
                      0.0), // DGM: warning, toStdString doesn't preserve
                            // "local" characters
          DefaultLegendMaterial);
    }
    ...
    for (unsigned angleStep = 0; angleStep < angularStepCount; ++angleStep) {
      ...
      unsigned iMap = static_cast<unsigned>(
          static_cast<double>(angleStep * map->xSteps) / angularStepCount);
      ...
      const DL_Attributes DefaultMaterial(
          qPrintable(profileNames[angleStep]), DL_Codes::bylayer, -1, "BYLAYER",
          1.0); // DGM: warning, toStdString doesn't preserve "local" characters
      const DL_Attributes GrayMaterial(qPrintable(profileNames[angleStep]),
      ...
      if (static_cast<int>(angleStep) < params.profileTitles.size()) {
        ...
        CCVector3d Ptop(c_pageWidth_mm / 2.0,
                        y0 + ySpan * scale + c_profileMargin_mm / 2.0, 0.0);
      ...
      {
        dxf.writePolyline(
            *dw, DL_PolylineData(static_cast<int>(polySteps.size()), 0, 0, 0),
            DefaultMaterial);

        for (size_t i = 0; i < polySteps.size(); ++i) {
          const VertStepData &step = polySteps[i];
          dxf.writeVertex(
              *dw, DL_VertexData(x0 + (step.radius_th +
      ...
      CCVector3d pageShift(x0 - xMin * scale, y0 - yMin * scale, 0.0);
      {
        ...
        for (size_t i = 0; i < polySteps.size(); ++i) {
          ...
          if (!displayIt) {
            ...
            double next_dh =
                polySteps[i + 1].height - polySteps[lastStep].height;
            if (dh >= heightStep ||
                (next_dh > heightStep &&
                 std::abs(dh - heightStep) < std::abs(next_dh - heightStep))) {
              displayIt = true;
            }
          }
          ...
          if (displayIt) {
            ...
            CCVector3d Pdev(step.radius_th +
                                step.deviation * params.devMagnifyCoef,
                            step.height, 0.0);
            ...
            // deviation bar
            dxf.writeLine(*dw,
                          DL_LineData(Pheight.x, Pheight.y, Pheight.z, Pdev.x,
                                      Pdev.y, Pdev.z),
                          GrayMaterial);
            ...
            // Vertical justification: 0 = Baseline, 1 = Bottom, 2 = Middle, 3 =
            // Top
            ...
            QString devText = QString::number(polySteps[i].deviation *
                                                  params.devLabelMultCoef,
                                              'f', params.precision);
            dxf.writeText(*dw,
                          DL_TextData(Pdev.x, Pdev.y, Pdev.z, Pdev.x, Pdev.y,
                                      Pdev.z, c_textHeight_mm, 1.0, 0,
                                      hJustification, vJustification,
                                      qPrintable(devText), "STANDARD",
                                      0.0), // DGM: warning, toStdString doesn't
                                            // preserve "local" characters
                          DefaultMaterial);

            QString heightText =
                QString::number(polySteps[i].height, 'f', params.precision);
            dxf.writeText(*dw,
                          DL_TextData(Pheight.x, Pheight.y, Pheight.z,
                                      ...
                                      qPrintable(heightText), "STANDARD", 0.0),
                          GrayMaterial);
...
bool DxfProfilesExporter::SaveHorizontalProfiles(
    const QSharedPointer<DistanceMapGenerationTool::Map> &map,
    ccPolyline *profile, QString filename, unsigned heightStepCount,
    double heightShift, double angularStep_rad, double radToUnitConvFactor,
    QString angleUnit, const Parameters &params,
    ccMainAppInterface *app /*=nullptr*/) {
#ifdef CC_DXF_SUPPORT
  assert(c_pageMargin_mm < c_profileMargin_mm);
  assert(2.0 * c_profileMargin_mm < std::min(c_pageWidth_mm, c_pageHeight_mm));
  ...
  // Mix with the map's boundaries along 'Y'
  double yMin =
      std::max(map->yMin + 0.5 * map->xStep, // central height of first row
               static_cast<double>(profileBBMin.y) + heightShift);
  double yMax =
      std::min(map->yMin + (static_cast<double>(map->ySteps) - 0.5) *
  ...
  QStringList profileNames;
  {
    ...
    for (unsigned i = 0; i < heightStepCount; ++i) {
      ...
      //(heightStepCount-1)) * ySpan; 	layerName =
  ...
  // Writing the Entities Section
  {
    ...
    // write legend
    {
      ...
      // Y axis tip as triangle
      {
        ...
        dxf.writePolyline(*dw, DL_PolylineData(3, 0, 0, 1), // closed polyline!
                          DefaultLegendMaterial);
        dxf.writeVertex(*dw,
                        DL_VertexData(xc, yc - (axisTip + axisTipSize), 0.0));
        dxf.writeVertex(
            *dw, DL_VertexData(xc - axisTipSize / 2.0, yc - axisTip, 0.0));
        dxf.writeVertex(
            *dw, DL_VertexData(xc + axisTipSize / 2.0, yc - axisTip, 0.0));
        dxf.writePolylineEnd(*dw);
      ...
      dxf.writeText(
          *dw,
          DL_TextData(
              xLegend, yLegend, 0.0, xLegend, yLegend, 0.0, c_textHeight_mm,
              1.0, 0, 0, 0,
              qPrintable(QString("Deviation units: ") + params.scaledDevUnits),
              "STANDARD", 0.0), // DGM: warning, toStdString doesn't preserve
                                // "local" characters
          DefaultLegendMaterial);
    ...
    for (unsigned heightStep = 0; heightStep < heightStepCount; ++heightStep) {
      ...
      double height =
          yMin +
          (static_cast<double>(heightStep) / (heightStepCount - 1)) * ySpan;

      // corresponding index in map
      if (height < map->yMin ||
          height >= map->yMin + static_cast<double>(map->ySteps) * map->yStep) {
        assert(false); // we have computed yMin and yMax so that those values
                       // are totally included inside the map's boundaries...
        continue;
      }
      ...
      double currentRadius = 0.0;
      {
        ...
        if (!found) {
          assert(false); // we have computed yMin and yMax so that 'height' is
                         // totally included inside the profile's boundaries...
          continue;
        }
      }
      ...
      if (params.profileTitles.size() == 1) {
        ...
        CCVector3d Ptop(
            xc, y0 + 2.0 * maxRadius * scale + c_profileMargin_mm / 2.0, 0.0);
      ...
      {
        dxf.writeCircle(*dw, DL_CircleData(xc, yc, 0.0, currentRadius * scale),
      ...
      {
        const DistanceMapGenerationTool::MapCell *cell =
            &map->at(static_cast<size_t>(jMap) * map->xSteps);
      ...
      // write profile polyline
      {
        ...
        for (size_t i = 0; i < polySteps.size(); ++i) {
          ...
          double radius =
              currentRadius + step.deviation * params.devMagnifyCoef;
          dxf.writeVertex(
              *dw,
              DL_VertexData(
                  pageShift.x - (cwSign * radius * sin(step.angle_rad)) * scale,
                  pageShift.y - (radius * cos(step.angle_rad)) * scale, 0.0));
        }
      ...
      // write corresponding 'deviation bars' and labels
      {
        ...
        for (size_t i = 0; i < polySteps.size(); ++i) {
          const HorizStepData &step = polySteps[i];
          bool displayIt =
              (i == 0 /*|| i+1 == polySteps.size()*/); // warning: cycle
          if (i != 0) {
            double dAngle =
                polySteps[i].angle_rad - polySteps[lastStep].angle_rad;
            double next_dAngle =
                (i + 1 == polySteps.size() ? polySteps[0].angle_rad + 2.0 * M_PI
                                           : polySteps[i + 1].angle_rad) -
                polySteps[lastStep].angle_rad;
            if (dAngle >= angularStep_rad ||
                (next_dAngle > angularStep_rad &&
                 std::abs(dAngle - angularStep_rad) <
                     std::abs(next_dAngle - angularStep_rad))) {
              displayIt = true;
            }
          }

          if (displayIt &&
              i != 0) // we skip 0 as we always display a vertical axis!
          {
            ...
            // deviation bar
            dxf.writeLine(*dw,
                          DL_LineData(Pangle.x, Pangle.y, Pangle.z, Pdev.x,
                                      Pdev.y, Pdev.z),
                          GrayMaterial);
            ...
            QString angleText =
                QString::number(polySteps[i].angle_rad * radToUnitConvFactor,
                                'f', params.precision) +
                angleUnit;
            dxf.writeText(*dw,
                          DL_TextData(Pangle.x, Pangle.y, Pangle.z, Pangle.x,
                                      Pangle.y, Pangle.z, c_textHeight_mm, 1.0,
                                      0, hJustificationAng, vJustificationAng,
                                      qPrintable(angleText), "STANDARD", 0.0),
                          GrayMaterial);

...
// helper
static void SetPoylineMetaVector(ccPolyline *polyline, const QString &key,
                                 const CCVector3 &P) {
  assert(polyline);
  if (polyline) {
...
DistanceMapGenerationTool::ProfileMetaData::computeCloudToSurfaceOriginTrans()
  ...
  // we must take the axis of the Surface of Revolution into account (if any and
  // if it is not colinear with X, Y or Z)
  if (hasAxis &&
...
bool DistanceMapGenerationTool::ComputeRadialDist(
    ccPointCloud *cloud, ccPolyline *profile, bool storeRadiiAsSF /*=false*/,
    ccMainAppInterface *app /*=nullptr*/) {
  // check input cloud and profile/polyline
  if (!cloud || !profile) {
    if (app)
      app->dispToConsole(QString("Internal error: invalid input parameters"),
  ...
  if (vertexCount < 2) {
    if (app)
      app->dispToConsole(QString("Invalid polyline (not enough vertices)"),
                         ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return false;
  }
  ...
  if (!GetPoylineMetaData(profile, profileDesc)) {
    if (app)
      app->dispToConsole(QString("Invalid polyline (bad or missing meta-data)"),
                         ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return false;
  }
  ...
  if (sfIdx < 0) {
    if (app)
      app->dispToConsole(
          QString("Failed to allocate a new scalar field for computing "
  ...
  if (storeRadiiAsSF) {
    ...
    if (sfIdxRadii < 0) {
      if (app)
        app->dispToConsole(
            QString("Failed to allocate a new scalar field for storing radii! "
                    "You should try to free some memory ..."),
  ...
  // revolution
  {
    ...
    CCCoreLib::NormalizedProgress nProgress(
        static_cast<CCCoreLib::GenericProgressCallback *>(&dlg), pointCount);
    ...
    for (unsigned i = 0; i < pointCount; ++i) {
      ...
      double radius =
          sqrt(Prel.u[dim1] * Prel.u[dim1] + Prel.u[dim2] * Prel.u[dim2]);
...
bool DistanceMapGenerationTool::ComputeMinAndMaxLatitude_rad(
    ccPointCloud *cloud, double &minLat_rad, double &maxLat_rad,
    const ccGLMatrix
        &cloudToSurfaceOrigin, // e.g. translation to the revolution origin
...
double DistanceMapGenerationTool::ConicalProject(double phi, double phi1,
                                                 double n) {
...
DistanceMapGenerationTool::CreateMap(
    ccPointCloud *cloud, ccScalarField *sf, const ccGLMatrix &cloudToSurface,
    unsigned char revolutionAxisDim, double xStep_rad, double yStep,
    double yMin, double yMax, bool conical, bool counterclockwise,
    FillStrategyType fillStrategy, EmptyCellFillOption emptyCellfillOption,
    ccMainAppInterface *app /*=nullptr*/) {
  assert(cloud && sf);
  if (!cloud || !sf) {
    if (app)
      app->dispToConsole(QString("[DistanceMapGenerationTool] Internal error: "
  ...
  if (count == 0) {
    if (app)
      app->dispToConsole(
          QString("[DistanceMapGenerationTool] Cloud is empty! Nothing to do!"),
          ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return QSharedPointer<Map>(nullptr);
  }
  ...
  unsigned xSteps = 0;
  {
    ...
    if (xSteps == 0) {
      if (app)
        app->dispToConsole(
            QString("[DistanceMapGenerationTool] Invalid longitude "
                    "step/boundaries! Can't generate a proper map!"),
            ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return QSharedPointer<Map>(nullptr);
    }
  }
  ...
  unsigned ySteps = 0;
  {
    ...
    if (ySteps == 0) {
      if (app)
        app->dispToConsole(
            QString("[DistanceMapGenerationTool] Invalid latitude "
                    "step/boundaries! Can't generate a proper map!"),
            ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return QSharedPointer<Map>(nullptr);
    }
  ...
  for (unsigned n = 0; n < count; ++n) {
    ...
    double y = 0.0;
    if (conical) {
      y = ComputeLatitude_rad(relativePos.u[X], relativePos.u[Y],
  ...
  if (emptyCellfillOption == FILL_WITH_ZERO) {
    ...
    if (fillCount) {
      ...
      try {
        ...
        // out of memory
        if (app)
          app->dispToConsole(QString("[DistanceMapGenerationTool] Not enough "
      ...
      if (the2DPoints.capacity() == fillCount) {
        ...
        if (!dm->buildMesh(the2DPoints,
                           CCCoreLib::Delaunay2dMesh::USE_ALL_POINTS,
                           errorStr)) {
          if (app) {
            app->dispToConsole(
                QStringLiteral("[DistanceMapGenerationTool] Interpolation "
          ...
          for (unsigned k = 0; k < triNum; ++k) {
            ...
            // now scan the cells
            {
              ...
              const double &valA =
                  cells[P[0][0] + P[0][1] * grid->xSteps].value;
              const double &valB =
                  cells[P[1][0] + P[1][1] * grid->xSteps].value;
              const double &valC =
                  cells[P[2][0] + P[2][1] * grid->xSteps].value;
              int det = (P[1][1] - P[2][1]) * (P[0][0] - P[2][0]) +
              ...
              for (int j = yMin; j <= yMax; ++j) {
                ...
                for (int i = xMin; i <= xMax; ++i) {
                  ...
                  if (!cell[i].count) {
                    ...
                    // Point Inclusion in Polygon Test (inspired from W.
...
ccMesh *DistanceMapGenerationTool::ConvertConicalMapToMesh(
    const QSharedPointer<Map> &map, bool counterclockwise,
  ...
  // create vertices
  {
    ...
    for (unsigned j = 0; j < map->xSteps; ++j) {
      ...
      double theta =
          nProj * (lon_rad - M_PI); //-Pi shift so that the map is well centered
      ...
      for (unsigned i = 0; i < map->ySteps; ++i) {
        ...
        CCVector3 Pxyz(static_cast<PointCoordinateType>(cwSign * r * sin_theta),
                       static_cast<PointCoordinateType>(-r * cos_theta), 0);
        cloud->addPoint(Pxyz);
      }
    }
  }
...
bool DistanceMapGenerationTool::ComputeSurfacesAndVolumes(
    const QSharedPointer<Map> &map, ccPolyline *profile, Measures &surface,
    Measures &volume) {
  if (!map || !profile)
    // invalid input!
    return false;
  ...
  const double surfPart =
      map->xStep / 2.0; // perimeter of a portion of circle of angle alpha =
                        // alpha * r (* height to get the external surface)
  ...
  for (unsigned j = 0; j < map->ySteps; ++j) {
    ...
    for (unsigned k = 1; k < vertexCount; ++k) {
      ...
      if (alpha >= 0.0 && alpha <= 1.0) {
        ...
        break; // FIXME: we hope that there's only one segment facing this
               // particular height?!
      }
    }
    ...
    if (r_th1 >= 0.0 /* && r_th2 >= 0.0*/) {
      ...
      for (unsigned i = 0; i < map->xSteps; ++i, ++cell) {
        ...
        //"real" radius
        double r1 = r_th1 + d; // see ComputeRadialDist --> << double dist =
                               // radius - radius_th; >>
        double r2 = r_th2 + d; // FIXME: works only if the "scalar field" used
        ...
        // volume of the element
        {
          ...
          double diffVolume = /*volPart * */ std::abs(
              3.0 * d *
              (r_th1 + r_th2 + d)); // = (r*r) * part - (r_th*r_th) * part =
...
bool DistanceMapGenerationTool::ConvertCloudToCylindrical(
    ...
    unsigned char revolutionAxisDim, bool counterclockwise /*=false*/) {
  assert(cloud);
  ...
  // motion direction
  PointCoordinateType ccw =
      (counterclockwise ? -CCCoreLib::PC_ONE : CCCoreLib::PC_ONE);
...
bool DistanceMapGenerationTool::ConvertCloudToConical(
    ...
    const ccGLMatrix
        &cloudToSurface, // e.g. translation to the revolution origin
    unsigned char revolutionAxisDim, double latMin_rad, double latMax_rad,
    double conicalSpanRatio /*=1.0*/, bool counterclockwise /*=false*/) {
  assert(cloud);
  if (!cloud || cloud->size() == 0)
    return false;
...
bool DistanceMapGenerationTool::SaveMapAsCSVMatrix(
    const QSharedPointer<Map> &map, QString filename, QString xUnit,
    QString yUnit, double xConversionFactor /*=1.0*/,
    double yConversionFactor /*=1.0*/, ccMainAppInterface *app /*=nullptr*/) {
  if (!map) {
    if (app)
      app->dispToConsole(
          QString("[SaveMapAsCSVMatrix] Internal error: invalid input map!"),
          ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return false;
  }
  ...
  if (!file.open(QFile::WriteOnly | QFile::Text)) {
    if (app)
      app->dispToConsole(QString("[SaveMapAsCSVMatrix] Failed to open file for "
                                 "writing! Check access rights"),
                         ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return false;
  }
  ...
  for (unsigned j = 0; j < map->ySteps; ++j) {
    ...
    double minY =
        yConversionFactor * (map->yMin + (map->ySteps - 1 - j) * map->yStep);
    double maxY =
        yConversionFactor * (map->yMin + (map->ySteps - j) * map->yStep);
...
ccMesh *DistanceMapGenerationTool::ConvertProfileToMesh(
    ccPolyline *profile, const ccGLMatrix &cloudToProfile,
    bool counterclockwise, unsigned angularSteps /*=36*/,
    QImage mapTexture /*=QImage()*/) {
  ...
  // profile vertices
  CCCoreLib::GenericIndexedCloudPersist *profileVertices =
      profile->getAssociatedCloud();
...
ccPointCloud *DistanceMapGenerationTool::ConvertMapToCloud(
    const QSharedPointer<Map> &map, ccPolyline *profile,
    double baseRadius /*=1.0*/, bool keepNaNPoints /*=true*/) {
  if (!map || !profile)
    return nullptr;
  ...
  // number of vertices
  CCCoreLib::GenericIndexedCloudPersist *polyVertices =
      profile->getAssociatedCloud();
  ...
  for (unsigned j = 0; j < map->ySteps; ++j) {
    CCVector3 P(0, 0, 0);
    P.u[Z] =
        static_cast<PointCoordinateType>(map->yMin + (j + 0.5) * map->yStep);

    // for each column
    for (unsigned i = 0; i < map->xSteps; ++i, ++cell) {
      if (keepNaNPoints || cell->count != 0) {
        P.u[X] =
            static_cast<PointCoordinateType>(map->xMin + (i + 0.5) * xStep);
        ...
        for (unsigned k = 1; k < polyVertCount; ++k) {
          ...
          double alpha =
              (P.u[Z] - profileDesc.heightShift - A->y) / (B->y - A->y);
          if (alpha >= 0.0 && alpha <= 1.0) {
            // we deduce the right radius by linear interpolation
            double radius_th = A->x + alpha * (B->x - A->x);
            // TODO: we take the first radius (even if there are other segments
            // at this particular height, because we can't guess which one is the
            // 'right' one!
        ...
        ScalarType val = cell->count ? static_cast<ScalarType>(cell->value)
                                     : CCCoreLib::NAN_VALUE;
        sf->addElement(val);
      }
    }
...
QImage DistanceMapGenerationTool::ConvertMapToImage(
    const QSharedPointer<Map> &map, ccColorScale::Shared colorScale,
    ...
    for (unsigned j = 0; j < map->ySteps; ++j) {
      ...
      for (unsigned i = 0; i < map->xSteps; ++i, ++cell) {
        ...
        if (cell->count != 0) {
          double relativePos =
              csIsRelative
                  ? (cell->value - map->minVal) / (map->maxVal - map->minVal)
                  : colorScale->getRelativePosition(cell->value);
          if (relativePos < 0.0)
            relativePos = 0.0;
          else if (relativePos > 1.0)
            relativePos = 1.0;
          rgb = colorScale->getColorByRelativePos(relativePos, colorScaleSteps,

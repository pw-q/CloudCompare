...
qSRA::qSRA(QObject *parent /*=nullptr*/)
    : QObject(parent), ccStdPluginInterface(":/CC/plugin/qSRA/info.json"),
      m_doLoadProfile(nullptr), m_doCompareCloudToProfile(nullptr),
...
QList<QAction *> qSRA::getActions() {
  ...
  if (!m_doLoadProfile) {
    ...
    m_doLoadProfile->setToolTip("Loads the 2D profile of a Surface of "
                                "Revolution (from a dedicated ASCII file)");
    m_doLoadProfile->setIcon(QIcon(
        QString::fromUtf8(":/CC/plugin/qSRA/images/loadProfileIcon.png")));
    // connect signal
  ...
  if (!m_doCompareCloudToProfile) {
    m_doCompareCloudToProfile =
        new QAction("Cloud-SurfRev radial distance", this);
    m_doCompareCloudToProfile->setToolTip(
        "Computes the radial distances between a cloud and a Surface of "
        "Revolution (polyline/profile, cone or cylinder)");
    m_doCompareCloudToProfile->setIcon(QIcon(
        QString::fromUtf8(":/CC/plugin/qSRA/images/distToProfileIcon.png")));
    // connect signal
  ...
  if (!m_doProjectCloudDists) {
    ...
    m_doProjectCloudDists->setToolTip(
        "Creates the 2D deviation map (radial distances) from a Surface or "
        "Revolution (unroll)");
    m_doProjectCloudDists->setIcon(
        QIcon(QString::fromUtf8(":/CC/plugin/qSRA/images/createMapIcon.png")));
    // connect signal
  ...
  if (selectedEntities.size() == 2) {
    ...
    int cloudIndex =
        selectedEntities[0]->isA(CC_TYPES::POINT_CLOUD)
            ? 0
            : selectedEntities[1]->isA(CC_TYPES::POINT_CLOUD) ? 1 : -1;
    if (cloudIndex != -1) {
      //... and either a polyline or a cone/cylinder
      validSelection =
          (selectedEntities[1 - cloudIndex]->isA(CC_TYPES::POLY_LINE) ||
           selectedEntities[1 - cloudIndex]->isKindOf(CC_TYPES::CONE));
    }
  }
...
void qSRA::loadProfile() const {
  ...
  if (!polyline) {
    if (m_app)
      m_app->dispToConsole(QString("Failed to load file '%1'!").arg(filename),
  ...
  m_app->dispToConsole(
      QString("[qSRA] File '%1' successfully loaded").arg(filename),
  ...
  bool tempPolyline = false;
  {
    for (unsigned i = 0; i < 2; ++i) {
      if (selectedEntities[i]->isA(CC_TYPES::POINT_CLOUD)) {
        ...
        // special case: we can deduce the polyline from the cone/cylinder
        // parameters
...
#ifdef _DEBUG
        // test: apply a visual transformation to see the polyline in the right
        // place
        {
          polyline->set2DMode(false);
          polyline->setWidth(2);
  ...
  if (cloud && polyline) {
    if (doComputeRadialDists(cloud, polyline)) {
      ...
      if (QMessageBox::question(
              m_app ? m_app->getMainWindow() : nullptr, "Generate map",
              "Do you want to generate a 2D deviation map?", QMessageBox::Yes,
    ...
    if (m_app)
      m_app->dispToConsole(
          QString("Select exactly one cloud and one Surface of Revolution "
...
void qSRA::doProjectCloudDistsInGrid(ccPointCloud *cloud,
  ...
  ccScalarField *sf = nullptr;
  {
    ...
    if (sfIdx < 0) {
      ...
      if (sf) {
        if (QMessageBox::question(
                m_app ? m_app->getMainWindow() : nullptr, "Distance field",
                QString("Cloud has no '%1' field. Do you want to use the "
                        "active scalar field instead?")
        ...
        QString message =
            QString("Cloud has no '%1' field and no active scalar field!")
                .arg(RADIAL_DIST_SF_NAME);
        ccLog::Error(message);
        ...
        if (m_doCompareCloudToProfile) {
          ccLog::Warning(
              QString(
                  "You can compute the radial distances with the '%1' method")

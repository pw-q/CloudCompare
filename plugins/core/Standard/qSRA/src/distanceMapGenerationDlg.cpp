...
static double
ConvertAngleFromRad(double angle_rad,
                    DistanceMapGenerationDlg::ANGULAR_UNIT destUnit) {
  switch (destUnit) {
...
static double
ConvertAngleToRad(double angle,
                  DistanceMapGenerationDlg::ANGULAR_UNIT srcUnit) {
  switch (srcUnit) {
  case DistanceMapGenerationDlg::ANG_DEG: // degrees
...
DistanceMapGenerationDlg::DistanceMapGenerationDlg(
    ccPointCloud *cloud, ccScalarField *sf, ccPolyline *polyline,
    ccMainAppInterface *app /*=nullptr*/)
    : QDialog(app ? app->getMainWindow() : nullptr), m_app(app), m_cloud(cloud),
      m_profile(polyline), m_sf(sf), m_map(nullptr), m_angularUnits(ANG_GRAD),
      m_window(nullptr), m_colorScaleSelector(nullptr), m_xLabels(nullptr),
      m_yLabels(nullptr), m_gridColor(Qt::gray), m_symbolColor(Qt::black) {
  setupUi(this);
  ...
  if (m_sf) {
    ...
    m_colorScaleSelector = new ccColorScaleSelector(
        ...
        QString::fromUtf8(":/CC/plugin/qSRA/images/gearIcon.png"));
    m_colorScaleSelector->init();
    m_colorScaleSelector->setSelectedScale(
        ccColorScalesManager::GetDefaultScale()->getUuid());
    connect(m_colorScaleSelector, &ccColorScaleSelector::colorScaleSelected,
            this, &DistanceMapGenerationDlg::colorScaleChanged);
    connect(m_colorScaleSelector,
            &ccColorScaleSelector::colorScaleEditorSummoned, this,
  ...
  if (m_profile) {
    ...
    if (validProfile) {
      ...
      if (dY < scaleHStepDoubleSpinBox->value()) {
        ...
        ccLog::Warning(
            "[qSRA] The vertical step of the grid has been automatically "
            "reduced to match the map height (you can change it if required)");
      }
    } else {
      if (m_app)
        m_app->dispToConsole(
            QString("Invalid profile: can't generate a proper map!"),
  ...
  // add window
  {
    ...
    m_window->setInteractionMode(ccGLWindowInterface::INTERACT_PAN |
                                 ccGLWindowInterface::INTERACT_CLICKABLE_ITEMS |
  ...
  // create labels "clouds" (empty)
  {
    ...
    m_xLabels->setLabelAlignmentFlags(ccGenericGLDisplay::ALIGN_HMIDDLE |
                                      ccGenericGLDisplay::ALIGN_VBOTTOM);
    ...
    m_yLabels->setLabelAlignmentFlags(ccGenericGLDisplay::ALIGN_HRIGHT |
                                      ccGenericGLDisplay::ALIGN_VMIDDLE);
    m_window->addToOwnDB(m_yLabels, false);
  }

  connect(projectionComboBox, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &DistanceMapGenerationDlg::projectionModeChanged);
  connect(angularUnitComboBox, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &DistanceMapGenerationDlg::angularUnitChanged);
  connect(xStepDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &DistanceMapGenerationDlg::updateGridSteps);
  connect(hStepDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &DistanceMapGenerationDlg::updateGridSteps);
  connect(latStepDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &DistanceMapGenerationDlg::updateGridSteps);
  connect(xMinDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &DistanceMapGenerationDlg::updateGridSteps);
  connect(xMaxDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &DistanceMapGenerationDlg::updateGridSteps);
  connect(hMinDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &DistanceMapGenerationDlg::updateGridSteps);
  connect(hMaxDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &DistanceMapGenerationDlg::updateGridSteps);
  connect(latMinDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &DistanceMapGenerationDlg::updateGridSteps);
  connect(latMaxDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &DistanceMapGenerationDlg::updateGridSteps);
  connect(axisDimComboBox, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &DistanceMapGenerationDlg::updateProfileRevolDim);
  connect(xOriginDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &DistanceMapGenerationDlg::updateProfileOrigin);
  connect(yOriginDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &DistanceMapGenerationDlg::updateProfileOrigin);
  connect(zOriginDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &DistanceMapGenerationDlg::updateProfileOrigin);
  connect(baseRadiusDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &DistanceMapGenerationDlg::baseRadiusChanged);

  connect(heightUnitLineEdit, &QLineEdit::editingFinished, this,
          &DistanceMapGenerationDlg::updateHeightUnits);

  connect(exportCloudPushButton, &QAbstractButton::clicked, this,
          &DistanceMapGenerationDlg::exportMapAsCloud);
  connect(exportMeshPushButton, &QAbstractButton::clicked, this,
          &DistanceMapGenerationDlg::exportMapAsMesh);
  connect(exportMatrixPushButton, &QAbstractButton::clicked, this,
          &DistanceMapGenerationDlg::exportMapAsGrid);
  connect(exportImagePushButton, &QAbstractButton::clicked, this,
          &DistanceMapGenerationDlg::exportMapAsImage);
  connect(loadLabelsPushButton, &QAbstractButton::clicked, this,
          &DistanceMapGenerationDlg::loadOverlaySymbols);
  connect(clearLabelsPushButton, &QAbstractButton::clicked, this,
          &DistanceMapGenerationDlg::clearOverlaySymbols);

  connect(symbolSizeSpinBox, qOverload<int>(&QSpinBox::valueChanged), this,
  ...
  connect(precisionSpinBox, qOverload<int>(&QSpinBox::valueChanged), this,
          &DistanceMapGenerationDlg::labelPrecisionChanged);
  connect(colorScaleStepsSpinBox, qOverload<int>(&QSpinBox::valueChanged), this,
          &DistanceMapGenerationDlg::colorRampStepsChanged);

  connect(overlayGridGroupBox, &QGroupBox::toggled, this,
          &DistanceMapGenerationDlg::toggleOverlayGrid);
  connect(scaleXStepDoubleSpinBox, &QAbstractSpinBox::editingFinished, this,
          &DistanceMapGenerationDlg::updateOverlayGrid);
  connect(scaleHStepDoubleSpinBox, &QAbstractSpinBox::editingFinished, this,
          &DistanceMapGenerationDlg::updateOverlayGrid);
  connect(scaleLatStepDoubleSpinBox, &QAbstractSpinBox::editingFinished, this,
          &DistanceMapGenerationDlg::updateOverlayGrid);
  connect(xScaleCheckBox, &QAbstractButton::clicked, this,
          &DistanceMapGenerationDlg::updateOverlayGrid);
  connect(yScaleCheckBox, &QAbstractButton::clicked, this,
          &DistanceMapGenerationDlg::updateOverlayGrid);
  connect(gridColorButton, &QAbstractButton::clicked, this,
          &DistanceMapGenerationDlg::changeGridColor);
  connect(symbolColorButton, &QAbstractButton::clicked, this,
          &DistanceMapGenerationDlg::changeSymbolColor);
  connect(displayColorScaleCheckBox, &QAbstractButton::toggled, this,
          &DistanceMapGenerationDlg::toggleColorScaleDisplay);
  connect(updateVolumesPushButton, &QAbstractButton::clicked, this,
  ...
  if (DxfProfilesExporter::IsEnabled()) {
    connect(exportImageDXFButton, &QAbstractButton::clicked, this,
...
void DistanceMapGenerationDlg::updateZoom(ccBBox &box) {
  ...
  // equivalent to 'ccGLWindowInterface::updateConstellationCenterAndZoom' but
  // we take aspect ratio into account
  ...
  if (CCCoreLib::GreaterThanEpsilon(bbDiag)) {
    ...
    // we compute the focal distance so that the map appears at the right
    // 'scale'
    {
      ...
      if (sfDisplayed) {
        scaleWidth = m_window->getDisplayParameters().colorScaleRampWidth +
                     fm.width("123.456789");
      ...
      float mapPart =
          static_cast<float>(mapWidth) / static_cast<float>(screenWidth);

      // we must also center the camera on the right position so that the map
      // appears in between the scale and the color ramp
      float mapStart =
          static_cast<float>(labelsWidth) / static_cast<float>(screenWidth);
      centerPos = (0.5f - mapStart) / mapPart;

      // update pixel size accordingly
      float screenHeight = m_window->glHeight();
      double pixelSize = std::max(
          box.getDiagVec().x * m_window->getDisplayScale().x / mapWidth,
          box.getDiagVec().y * m_window->getDisplayScale().y / screenHeight);
...
void DistanceMapGenerationDlg::update() {
  ...
  if (m_map && m_window) {
    ...
    if (mode == PROJ_CYLINDRICAL) {
      ...
      transMat.setTranslation(
          CCVector3(static_cast<PointCoordinateType>(dx / 2 + m_map->xMin),
                    static_cast<PointCoordinateType>(dy / 2 + m_map->yMin), 0));
      ccPlane *mapPlane =
          new ccPlane(static_cast<PointCoordinateType>(dx),
    ...
    if (mapMesh) {
      ...
      m_app->dispToConsole(QString("Not enough memory to display the map!"),
                           ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    }
  }
  ...
  // update zoom
  ccBBox box =
      m_window ? m_window->getOwnDB()->getDisplayBB_recursive(false, m_window)
               : ccBBox();
  updateZoom(box);
...
void DistanceMapGenerationDlg::updateHeightUnits() {
  ...
  if (m_window && m_window->getAssociatedScalarField()) {
    m_window->getAssociatedScalarField()->setName(
        QString("Distance (%1)").arg(getHeightUnitString()).toStdString());
  }
...
void DistanceMapGenerationDlg::updateMapTexture() {
  ...
  if (mode == PROJ_CYLINDRICAL) {
    ...
    if (m_window->getOwnDB()->filterChildren(texturedEntities, false,
                                             CC_TYPES::PLANE) == 0)
      return;
  ...
  // spawn "update" dialog
  QProgressDialog progressDlg(tr("Updating..."), QString(), 0, 0, nullptr,
                              Qt::Popup);
  ...
  for (ccHObject *texturedEntity : texturedEntities) {
    ...
    if (mode == PROJ_CYLINDRICAL && texturedEntity->isA(CC_TYPES::PLANE)) {
      if (!static_cast<ccPlane *>(texturedEntity)->setAsTexture(mapImage)) {
        if (m_app)
          m_app->dispToConsole(QString("Not enough memory to update the map!"),
                               ccMainAppInterface::ERR_CONSOLE_MESSAGE);
        return;
      }
    }
...
void DistanceMapGenerationDlg::spawnColorScaleEditor() {
  ...
  ccColorScale::Shared colorScale =
      (m_colorScaleSelector ? m_colorScaleSelector->getSelectedScale()
...
// helper
static void SetSpinBoxValues(QDoubleSpinBox *spinBox, int decimals,
...
void DistanceMapGenerationDlg::getGridYValues(
    double &minY, double &maxY, double &step,
    ANGULAR_UNIT unit /*=ANG_RAD*/) const {
...
double
DistanceMapGenerationDlg::getScaleYStep(ANGULAR_UNIT unit /*=ANG_RAD*/) const {
...
void DistanceMapGenerationDlg::updateProfileOrigin() {
  ...
  if (dShift != 0) {
    clearOverlaySymbols(); // symbols placement depend on the origin position
...
void DistanceMapGenerationDlg::updateGridSteps() {
  ...
  QString xStepsStr;
  {
    ...
    xStepsStr =
        (step > 0 ? QString::number(ceil(std::max(maxX - minX, 0.0) / step))
                  : "inf");
  }
  ...
  QString yStepsStr;
  {
    ...
    yStepsStr =
        (step > 0 ? QString::number(ceil(std::max(maxY - minY, 0.0) / step))
                  : "inf");
  }
...
double DistanceMapGenerationDlg::getBaseRadius() const {
  return getProjectionMode() == PROJ_CONICAL ? 1.0
...
DistanceMapGenerationDlg::updateMap() {
  ...
  return DistanceMapGenerationTool::CreateMap(
      m_cloud, m_sf, cloudToSurface, profileDesc.revolDim, angStep_rad, yStep,
      yMin, yMax, getProjectionMode() == PROJ_CONICAL, ccw,
...
void DistanceMapGenerationDlg::exportMapAsMesh() {
  ...
  if (mapImage.isNull()) {
    if (m_app)
      m_app->dispToConsole(
          QString("Failed to generate mesh texture! Not enough memory?"),
          ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return;
  }
...
void DistanceMapGenerationDlg::exportMapAsGrid() {
  ...
  QString path =
      settings.value("exportPath", ccFileUtils::defaultDocPath()).toString();

  QString filter("Grid file (*.csv)");

  // open file saving dialog
  QString filename =
      QFileDialog::getSaveFileName(nullptr, "Select output file", path, filter);
  if (filename.isEmpty())
    return;
  ...
  if (DistanceMapGenerationTool::SaveMapAsCSVMatrix(m_map, filename, xUnit,
                                                    yUnit, xConversionFactor,
...
void DistanceMapGenerationDlg::exportMapAsImage() {
  ...
  ccRenderToFileDlg rtfDlg(m_window->glWidth(), m_window->glHeight(),
                           m_app->getMainWindow());
  ...
  if (rtfDlg.exec()) {
    QApplication::processEvents();
    m_window->renderToFile(rtfDlg.getFilename(), rtfDlg.getZoom(),
...
void DistanceMapGenerationDlg::exportProfilesAsDXF() {
  ...
  if (!vertFilename.isNull()) {
    ...
    if (!DxfProfilesExporter::SaveVerticalProfiles(
            m_map, m_profile, vertFilename, angularStepCount, heightStep,
            heightShift, params, m_app)) {
      if (m_app)
        m_app->dispToConsole(
      ...
      if (m_app)
        m_app->dispToConsole(
            QString("File '%1' saved successfully").arg(vertFilename),
            ccMainAppInterface::STD_CONSOLE_MESSAGE);
    }
  }
  ...
  if (!horizFilename.isNull()) {
    ...
    if (!DxfProfilesExporter::SaveHorizontalProfiles(
            m_map, m_profile, horizFilename, heightStepCount, heightShift,
            angleStep_rad, ConvertAngleFromRad(1.0, m_angularUnits),
      ...
      if (m_app)
        m_app->dispToConsole(
            QString("File '%1' saved successfully").arg(horizFilename),
            ccMainAppInterface::STD_CONSOLE_MESSAGE);
    }
  }
}
...
void DistanceMapGenerationDlg::loadOverlaySymbols() {
  ...
  settings.beginGroup("qSRA");
  QString path =
      settings.value("importPath", ccFileUtils::defaultDocPath()).toString();

  QString filter("Symbols (*.txt)");

  // open file loading dialog
  QString filename = QFileDialog::getOpenFileName(
      nullptr, "Select symbols file", path, filter);
  if (filename.isEmpty())
    return;
  ...
  // try to load the file (as a "symbol" point cloud)
  {
    ...
    if (!file.open(QFile::ReadOnly)) {
      if (m_app)
        m_app->dispToConsole(
            QString("Failed to open symbol file '%1'!").arg(filename),
            ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return;
    }
    ...
    while (!currentLine.isNull()) {
      QStringList tokens =
          currentLine.simplified().split(QChar(' '), QString::SkipEmptyParts);
      if (tokens.size() == 4) {
        bool okX = false;
        bool okY = false;
        bool okZ = false;
        ...
        if (symbolCloud->size() == symbolCloud->capacity()) {
          if (!symbolCloud->reserveThePointsTable(symbolCloud->size() + 64) ||
              !symbolCloud->reserveLabelArray(symbolCloud->size() + 64)) {
            if (m_app)
              m_app->dispToConsole(QString("Not enough memory!"),
                                   ccMainAppInterface::ERR_CONSOLE_MESSAGE);
            error = true;
            break;
          }
        ...
        // DGM: warning, for historical reasons height values are expressed
        // relative to the profile origin!
    ...
    if (error) {
      ...
      if (m_app)
        m_app->dispToConsole(QString("An error occurred while loading the "
    ...
    if (m_window) {
      ...
      } else /*if (getProjectionMode() == PROJ_CONICAL)*/
      {
        ...
        DistanceMapGenerationTool::ConvertCloudToConical(
            symbolCloud, cloudToSurface, profileDesc.revolDim, m_map->yMin,
...
void DistanceMapGenerationDlg::overlaySymbolsSizeChanged(int size) {
  ...
  for (unsigned i = 0; i < db->getChildrenNumber(); ++i) {
    ...
    if (child->isA(CC_TYPES::POINT_CLOUD) && child != m_xLabels &&
...
void DistanceMapGenerationDlg::toggleOverlayGrid(bool state) {
  ...
  if (state && m_map) // on
  {
    ...
    if (scaleXStep_rad == 0 || scaleYStep == 0) {
      if (m_app)
        m_app->dispToConsole(QString("Internal error: invalid step values?!"),
                             ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      m_window->redraw();
      return;
    }
    ...
    unsigned yStepCount =
        static_cast<unsigned>(ceil(std::max(yMax - yMin, 0.0) / scaleYStep));
    ...
    if (mode == PROJ_CONICAL) {
      double conicalSpanRatio = conicSpanRatioDoubleSpinBox->value();
      nProj =
          DistanceMapGenerationTool::ConicalProjectN(m_map->yMin, m_map->yMax) *
          conicalSpanRatio;
    }
    bool ccw = ccwCheckBox->isChecked();
    ...
    // create vertical polylines
    {
      ...
      if (m_xLabels->isVisible()) {
        if (!m_xLabels->reserve(xStepCount + 1) ||
          ...
          if (m_app)
            m_app->dispToConsole(
                QString("Not engouh memory to display the 'X' scale?!"),
                ccMainAppInterface::ERR_CONSOLE_MESSAGE);
          m_xLabels->clear();
          m_xLabels->setVisible(false);
        }
      ...
      for (unsigned i = 0; i <= xStepCount; ++i) {
        ...
        if (m_xLabels->isVisible()) {
          ...
          m_xLabels->addLabel(
              QString("%1%2")
                  .arg(ConvertAngleFromRad(angle_rad, m_angularUnits), 0, 'f',
    ...
    // create horizontal polylines
    {
      if (m_yLabels->isVisible()) {
        if (!m_yLabels->reserve(yStepCount + 1) ||
          ...
          if (m_app)
            m_app->dispToConsole(
                QString("Not enough memory to display the 'Y' scale?!"),
                ccMainAppInterface::ERR_CONSOLE_MESSAGE);
          m_yLabels->clear();
          m_yLabels->setVisible(false);
        }
        ...
        if (mode == PROJ_CONICAL) {
          ...
          if (vertices->reserve(polySteps + 1))
            for (unsigned j = 0; j <= polySteps; ++j) {
              double angle_rad =
                  xMin_rad +
                  static_cast<double>(j) / (polySteps * (xMax_rad - xMin_rad));
              CCVector3 P = DistanceMapGenerationTool::ProjectPointOnCone(
                  angle_rad, y, m_map->yMin, nProj, ccw);
              P.z = 1.0;
              vertices->addPoint(P);
            }
        } else {
...
void DistanceMapGenerationDlg::initFromPersistentSettings() {
  ...
  // read parameters
  double conicSpanRatio =
      settings.value("conicSpanRatio", conicSpanRatioDoubleSpinBox->value())
          .toDouble();
  int angularUnit =
  ...
  double angularStep =
      settings.value("angularStep", xStepDoubleSpinBox->value()).toDouble();
  double heightStep =
      settings.value("heightStep", hStepDoubleSpinBox->value()).toDouble();
  double latitudeStep =
      settings.value("latitudeStep", latStepDoubleSpinBox->value()).toDouble();
  ...
  double scaleLatitudeStep =
      settings.value("scaleLatitudeStep", scaleLatStepDoubleSpinBox->value())
          .toDouble();
  bool ccw = settings.value("CCW", ccwCheckBox->isChecked()).toBool();
  int fillStrategy =
      settings.value("fillStrategy", fillingStrategyComboxBox->currentIndex())
          .toBool();
  int emptyCells =
      settings.value("emptyCells", emptyCellsComboBox->currentIndex()).toInt();
  ...
  bool showColorScale =
      settings.value("showColorScale", displayColorScaleCheckBox->isChecked())
          .toBool();

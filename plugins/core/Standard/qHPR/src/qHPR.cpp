...
qHPR::qHPR(QObject *parent)
    : QObject(parent), ccStdPluginInterface(":/CC/plugin/qHPR/info.json"),
      m_action(nullptr) {}
...
qHPR::removeHiddenPoints(CCCoreLib::GenericIndexedCloudPersist *theCloud,
                         const CCVector3d &viewPoint, double fParam) {
  assert(theCloud);
  ...
  if (nbPoints < 4) {
    CCCoreLib::ReferenceCloud *visiblePoints =
        new CCCoreLib::ReferenceCloud(theCloud);
    if (!visiblePoints->addPointIndex(
            0, nbPoints)) // well even for less than 4 points we never know ;)
    {
      // not enough memory!
  ...
  if (!qh_new_qhull(3, nbPoints + 1, pt_array, False, qHullCommand, nullptr,
                    stderr)) {
    try {
    ...
    FORALLfacets {
      // if (!facet->simplicial)
      //	error("convhulln: non-simplicial facet"); // should never happen
      //with QJ
...
void qHPR::doAction() {
  ...
  if (!win) {
    m_app->dispToConsole("No active window!",
                         ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return;
  }
  ...
  if (!params.perspectiveView) {
    m_app->dispToConsole(
        "[Hidden Point Removal] for improved results use Perspective mode",
  ...
  if (!theOctree) {
    m_app->dispToConsole("Couldn't compute octree!",
                         ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    return;
  }
  ...
  QScopedPointer<CCCoreLib::ReferenceCloud> visibleCells;
  {
    ...
    QScopedPointer<CCCoreLib::ReferenceCloud> theCellCenters(
        CCCoreLib::CloudSamplingTools::subsampleCloudWithOctreeAtLevel(
            ...
            &progressCb, theOctree.data()));
    if (!theCellCenters) {
      m_app->dispToConsole("Error while simplifying point cloud with octree!",
                           ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return;
    }

    visibleCells.reset(
        removeHiddenPoints(theCellCenters.data(), viewPoint, 3.5));

    m_app->dispToConsole(QString("[HPR] Cells: %1 - Time: %2 s")
                             .arg(theCellCenters->size())
                             .arg(eTimer.elapsed() / 1.0e3));
  ...
  if (visibleCells) {
    // DGM: we generate a new cloud now, instead of playing with the points
    // visiblity! (too confusing for the user)
    /*if (!cloud->isVisibilityTableInstantiated() &&
    !cloud->resetVisibilityArray())
    {
            m_app->dispToConsole("Visibility array allocation failed! (Not
    enough memory?)",ccMainAppInterface::ERR_CONSOLE_MESSAGE); return;
    }

    ccPointCloud::VisibilityTableType* pointsVisibility =
    cloud->getTheVisibilityArray(); assert(pointsVisibility);
    ...
    */
    ...
    if (!theOctree->getCellIndexes(static_cast<unsigned char>(octreeLevel),
                                   cellIndexes)) {
      m_app->dispToConsole("Couldn't fetch the list of octree cell indexes! "
                           "(Not enough memory?)",
                           ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return;
    }
    ...
    for (unsigned i = 0; i < visibleCellsCount; ++i) {
      ...
      theOctree->getPointsInCellByCellIndex(
          &Yk, cellIndexes[index], static_cast<unsigned char>(octreeLevel));
      //...are all visible
    ...
    m_app->dispToConsole(
        QString("[HPR] Visible points: %1").arg(visiblePointCount));

    if (visiblePoints.size() == cloud->size()) {
      m_app->dispToConsole("No points were removed!",
                           ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    } else {
      ...
      if (newCloud) {
        ...
        cc2DViewportObject *viewportObject =
            new cc2DViewportObject(QString("Viewport"));

...
void qMeshBoolean::onNewSelection(
...
bool ToIGLMesh(const ccMesh *in, IGLMesh &out,
               ccMainAppInterface *app = nullptr) {
  if (!in || !in->getAssociatedCloud()) {
    if (app)
      app->dispToConsole("Invalid input mesh!",
...
ccMesh *FromIGLMesh(const IGLMesh &in, ccMainAppInterface *app = nullptr) {
  if (in.F.rows() == 0 || in.V.rows() == 0) {
    if (app)
      app->dispToConsole("[Mesh boolean] Output mesh is empty?!",
  ...
  if (!vertices->reserve(vertCount)) {
    if (app)
      app->dispToConsole("[Mesh boolean] Not enough memory!",
                         ccMainAppInterface::ERR_CONSOLE_MESSAGE);
    delete vertices;
    return nullptr;
  }
...
static bool DoPerformBooleanOp() {
  ...
  try {
    ...
    default:
      ...
      if (s_params.app)
        s_params.app->dispToConsole(
            "[Mesh boolean] Unhandled operation?!",
            ccMainAppInterface::
                WRN_CONSOLE_MESSAGE); // DGM: can't issue an error message (i.e.
                                      // with dialog) in another thread!
      return false;
    }

    s_params.output = IGLMesh();
    if (!igl::copyleft::cgal::mesh_boolean(s_params.meshA->V, s_params.meshA->F,
                                           s_params.meshB->V, s_params.meshB->F,
                                           booleanType, s_params.output.V,
                                           s_params.output.F)) {
      if (s_params.app)
        s_params.app->dispToConsole("[Mesh boolean] CSG operation failed",
                                    ccMainAppInterface::WRN_CONSOLE_MESSAGE);
      return false;
    }
    ...
    if (s_params.app) {
      ...
      s_params.app->dispToConsole(
          QString("[Mesh boolean] CSG operation duration: %1 s")
    ...
    if (s_params.app)
      s_params.app->dispToConsole(
          QString("[Mesh boolean] Exception caught: %1").arg(e.what()),
          ccMainAppInterface::WRN_CONSOLE_MESSAGE);
    return false;
  }
...
void qMeshBoolean::doAction() {
  ...
  if (selNum != 2 || !selectedEntities[0]->isKindOf(CC_TYPES::MESH) ||
      !selectedEntities[1]->isKindOf(CC_TYPES::MESH)) {
    assert(false);
  ...
  // launch process
  {
    ...
    if (!future.result()) {
      // an error occurred
      if (m_app)
        m_app->dispToConsole("Computation failed",
                             ccMainAppInterface::ERR_CONSOLE_MESSAGE);
      return;
    }
  }
  ...
  if (m_app) {
    // display the duration time
    m_app->dispToConsole(QString("[Mesh boolean] Total duration: %1 s")
                             .arg(timer.elapsed() / 1000.0, 0, 'f', 2));
  }
}

...
qPCV::qPCV(QObject *parent /*=nullptr*/)
    : QObject(parent), ccStdPluginInterface(":/CC/plugin/qPCV/info.json"),
      m_action(nullptr) {}
...
void qPCV::doAction() {
  ...
  if (root) {
    ...
    for (auto &pointCloud : clouds) {
      ...
      ccGenericPointCloud *cloud =
          ccHObjectCaster::ToGenericPointCloud(pointCloud);

      if (cloud && cloud->hasNormals()) {
        cloudsWithNormals.push_back(cloud);
        QString cloudTitle = QStringLiteral("%1 - %2 points")
                                 .arg(cloud->getName())
                                 .arg(cloud->size());
        if (cloud->getParent() &&
            cloud->getParent()->isKindOf(CC_TYPES::MESH)) {
          cloudTitle.append(
              QStringLiteral(" (%1)").arg(cloud->getParent()->getName()));
        }
  ...
  if (!cloudsWithNormals.empty() && dlg.useCloudRadioButton->isChecked()) {
    ...
    try {
      ...
      m_app->dispToConsole("Not enough memory to generate the set of rays",

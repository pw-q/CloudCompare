...
// fields)
static double ComputePMUncertainty(CCCoreLib::DgmOctree::NeighboursSet &set,
                                   const CCVector3 &N,
...
void ComputeM3C2DistForPoint(unsigned index) {
  ...
  try {
      ...
      while (cn1.currentHalfLength < cn1.maxHalfLength) {
        size_t neighbourCount =
            s_M3C2Params.cloud1Octree
                ->getPointsInCylindricalNeighbourhoodProgressive(cn1);
    ...
    if (n1 != 0) {
      ...
      if (!validStats1) {
        qM3C2Tools::ComputeStatistics(cn1.neighbours, s_M3C2Params.useMedian,
                                      mean1, stdDev1);
      }

      if (s_M3C2Params.usePrecisionMaps &&
          (s_M3C2Params.computeConfidence || s_M3C2Params.stdDevCloud1SF)) {
        // compute the Precision Maps derived sigma
        stdDev1 =
            ComputePMUncertainty(cn1.neighbours, N, s_M3C2Params.cloud1PM);
      }
    ...
    if (n1 != 0 ||
        ...
        while (cn2.currentHalfLength < cn2.maxHalfLength) {
          size_t neighbourCount =
              s_M3C2Params.cloud2Octree
                  ->getPointsInCylindricalNeighbourhoodProgressive(cn2);
          if (neighbourCount != previousNeighbourCount) {
            // do we have enough points for computing stats?
            if (neighbourCount >= s_M3C2Params.minPoints4Stats) {
              qM3C2Tools::ComputeStatistics(
                  cn2.neighbours, s_M3C2Params.useMedian, mean2, stdDev2);
              validStats2 = true;
              // do we have a sharp enough 'mean' to stop?
              if (std::abs(mean2) + 2 * stdDev2 <
      ...
      if (n2 != 0) {
        ...
        if (!validStats2) {
          qM3C2Tools::ComputeStatistics(cn2.neighbours, s_M3C2Params.useMedian,
                                        mean2, stdDev2);
        }
        assert(stdDev2 != stdDev2 ||
               stdDev2 >= 0); // first inequality fails if stdDev2 is NaN ;)
        ...
        if (s_M3C2Params.usePrecisionMaps &&
            (s_M3C2Params.computeConfidence || s_M3C2Params.stdDevCloud2SF)) {
          // compute the Precision Maps derived sigma
          stdDev2 =
              ComputePMUncertainty(cn2.neighbours, N, s_M3C2Params.cloud2PM);
        }

        if (n1 != 0) {
          // m3c2 dist = distance between i1 and i2 (i.e. either the mean or the
          // median of both neighborhoods)
          ...
          if (s_M3C2Params.computeConfidence) {
            ...
            if (s_M3C2Params.usePrecisionMaps) {
              LODStdDev = stdDev1 * stdDev1 +
                          stdDev2 * stdDev2; // equation (2) in M3C2-PM article
            }
            // standard M3C2 algortihm: have we enough points for computing the
            // confidence interval?
            ...
            if (!std::isnan(LODStdDev)) {
              ...
              ScalarType LOD = static_cast<ScalarType>(
                  1.96 * (sqrt(LODStdDev) + s_M3C2Params.registrationRms));
              ...
              if (s_M3C2Params.sigChangeSF) {
                ...
                if (significant) {
                  s_M3C2Params.sigChangeSF->setValue(
                      index,
                      SCALAR_ONE); // already equal to SCALAR_ZERO otherwise
                }
              }
            }
            // else //DGM: scalar fields have already been initialized with the
            // right 'default' values
            //{
            //	if (distUncertaintySF)
            //		distUncertaintySF->setValue(index,
            //CCCoreLib::NAN_VALUE); 	if (sigChangeSF)
...
bool qM3C2Process::Compute(const qM3C2Dialog &dlg, QString &errorMessage,
                           ccPointCloud *&outputCloud, bool allowDialogs,
  ...
  ccScalarField *normalScaleSF = nullptr; // normal scale (multi-scale mode
                                          // only)
  ...
  s_M3C2Params.projectionDepth =
      static_cast<PointCoordinateType>(dlg.cylHalfHeightDoubleSpinBox->value());
  ...
  // precision maps
  {
    s_M3C2Params.usePrecisionMaps = dlg.precisionMapsGroupBox->isEnabled() &&
                                    dlg.precisionMapsGroupBox->isChecked();
    if (s_M3C2Params.usePrecisionMaps) {
      if (allowDialogs &&
          QMessageBox::question(parentWidget, "Precision Maps",
                                "Are you sure you want to compute the M3C2 "
                                "distances with precision maps?",
                                QMessageBox::Yes,
  ...
  if (app)
    app->dispToConsole(QString("[M3C2] Will use %1 threads")
  ...
  if (!s_M3C2Params.corePoints && samplingDist > 0) {
    ...
    CCCoreLib::ReferenceCloud *subsampled =
        CCCoreLib::CloudSamplingTools::resampleCloudSpatially(
            cloud1, static_cast<PointCoordinateType>(samplingDist), modParams,
            s_M3C2Params.cloud1Octree.data(), &pDlg);

    if (subsampled) {
      s_M3C2Params.corePoints =
          static_cast<ccPointCloud *>(cloud1)->partialClone(subsampled);
    ...
    if (s_M3C2Params.corePoints) {
      s_M3C2Params.corePoints->setName(QString("%1.subsampled [min dist. = %2]")
                                           .arg(cloud1->getName())
                                           .arg(samplingDist));
      ...
      if (app) {
        app->dispToConsole(
            QString("[M3C2] Sub-sampled cloud has been saved ('%1')")
  ...
  if (!error) {
    ...
    if (s_M3C2Params.keepOriginalCloud) {
      ...
      s_M3C2Params.outputCloud =
          new ccPointCloud(/*outputName*/); // setName will be called at the end
      if (!s_M3C2Params.outputCloud->resize(
              s_M3C2Params.corePoints
                  ->size())) // resize as we will 'set' the new points positions
                             // in 'ComputeM3C2DistForPoint'
      {
  ...
  if (!error) {
    ...
    case qM3C2Normals::MULTI_SCALE_MODE: {
      ...
      s_M3C2Params.coreNormals
          ->link(); // will be released anyway at the end of the process
      ...
      ccOctree *baseOctree =
          (baseCloud == cloud1 ? s_M3C2Params.cloud1Octree.data() : nullptr);

      // dedicated core points method
      normalsAreOk = qM3C2Normals::ComputeCorePointsNormals(
          s_M3C2Params.corePoints, s_M3C2Params.coreNormals, baseCloud, radii,
          invalidNormals, maxThreadCount, normalScaleSF, &pDlg, baseOctree);

      // now fix the orientation
      if (normalsAreOk) {
        // some invalid normals?
        if (invalidNormals && app) {
          app->dispToConsole("[M3C2] Some normals are invalid! You may have to "
                             "increase the scale.",
        ...
        if (usePreferredOrientation) {
          ...
          assert(preferredOrientation >= ccNormalVectors::PLUS_X &&
                 preferredOrientation <= ccNormalVectors::MINUS_SENSOR_ORIGIN);
          if (!ccNormalVectors::UpdateNormalOrientations(
            ...
            errorMessage =
                "[M3C2] Failed to re-orient the normals (invalid parameter?)";
            error = true;
          }
        } else // or use external points
        {
          ...
          if (!qM3C2Normals::UpdateNormalOrientationsWithCloud(
                  s_M3C2Params.corePoints, *s_M3C2Params.coreNormals,
                  orientationCloud, maxThreadCount, &pDlg)) {
            errorMessage = "[M3C2] Failed to re-orient the normals with input "
                           "point cloud!";
            error = true;
          }
        }
    ...
    case qM3C2Normals::USE_CLOUD1_NORMALS: {
      ...
      ccPointCloud *sourceCloud =
          (corePointsHaveBeenSubsampled ? s_M3C2Params.corePoints : cloud1);
      s_M3C2Params.coreNormals = sourceCloud->normals();
      if (s_M3C2Params.coreNormals) {
        normalsAreOk =
            (s_M3C2Params.coreNormals->currentSize() == sourceCloud->size());
        s_M3C2Params.coreNormals
            ->link(); // will be released anyway at the end of the process
      } else {
        normalsAreOk = false;
    ...
    case qM3C2Normals::USE_CORE_POINTS_NORMALS: {
      normalsAreOk =
          s_M3C2Params.corePoints && s_M3C2Params.corePoints->hasNormals();
      if (normalsAreOk) {
  ...
  if (!error && s_M3C2Params.coreNormals && corePointsHaveBeenSubsampled) {
    if (s_M3C2Params.corePoints->hasNormals() ||
      ...
      app->dispToConsole("Failed to allocate memory for core points normals!",
                         ccMainAppInterface::WRN_CONSOLE_MESSAGE);
    }
  }
  ...
  while (!error) // fake loop for easy break
  {
    ...
    if (dlg.exportStdDevInfoCheckBox->isChecked()) {
      ...
      s_M3C2Params.stdDevCloud1SF =
          new ccScalarField(stdDevSFName1.toStdString());
      s_M3C2Params.stdDevCloud1SF->link();
      if (!s_M3C2Params.stdDevCloud1SF->resizeSafe(corePointCount, true,
                                                   CCCoreLib::NAN_VALUE)) {
        if (app)
          app->dispToConsole(
              "Failed to allocate memory for cloud #1 std. dev. values!",
              ccMainAppInterface::WRN_CONSOLE_MESSAGE);
      ...
      s_M3C2Params.stdDevCloud2SF =
          new ccScalarField(stdDevSFName2.toStdString());
      s_M3C2Params.stdDevCloud2SF->link();
      if (!s_M3C2Params.stdDevCloud2SF->resizeSafe(corePointCount, true,
                                                   CCCoreLib::NAN_VALUE)) {
        if (app)
          app->dispToConsole(
              "Failed to allocate memory for cloud #2 std. dev. values!",
              ccMainAppInterface::WRN_CONSOLE_MESSAGE);
    ...
    if (dlg.exportDensityAtProjScaleCheckBox->isChecked()) {
      ...
      if (!s_M3C2Params.densityCloud1SF->resizeSafe(corePointCount, true,
        ...
        if (app)
          app->dispToConsole(
              "Failed to allocate memory for cloud #1 density values!",
              ccMainAppInterface::WRN_CONSOLE_MESSAGE);
      ...
      if (!s_M3C2Params.densityCloud2SF->resizeSafe(corePointCount, true,
        ...
        if (app)
          app->dispToConsole(
              "Failed to allocate memory for cloud #2 density values!",
              ccMainAppInterface::WRN_CONSOLE_MESSAGE);
    ...
    PointCoordinateType equivalentRadius = static_cast<PointCoordinateType>(
        pow((static_cast<double>(s_M3C2Params.projectionDepth) *
             s_M3C2Params.projectionDepth) *
    ...
    s_M3C2Params.exportNormal =
        s_M3C2Params.updateNormal && !s_M3C2Params.outputCloud->hasNormals();
    if (s_M3C2Params.exportNormal &&
        !s_M3C2Params.outputCloud
             ->resizeTheNormsTable()) // resize because we will 'set' the normal
                                      // in ComputeM3C2DistForPoint
    {
      if (app)
        app->dispToConsole("Failed to allocate memory for exporting normals!",
                           ccMainAppInterface::WRN_CONSOLE_MESSAGE);
      ...
      if (app)
        app->dispToConsole(
            QString("[M3C2] Distances computation: %1 s.")
                .arg(static_cast<double>(distTime_ms) / 1000.0, 0, 'f', 3),
            ccMainAppInterface::STD_CONSOLE_MESSAGE);
    }
  ...
  if (!error) {
    ...
    if (normalScaleSF) {
      ...
      // in case the output cloud is the original cloud, we must remove the
      // former SF
    ...
    if (s_M3C2Params.stdDevCloud1SF) {
      ...
      sfIdx =
          s_M3C2Params.outputCloud->addScalarField(s_M3C2Params.stdDevCloud1SF);
    }
    if (s_M3C2Params.stdDevCloud2SF) {
      ...
      sfIdx =
          s_M3C2Params.outputCloud->addScalarField(s_M3C2Params.stdDevCloud2SF);
    }
    ...
    if (s_M3C2Params.sigChangeSF) {
      ...
      sfIdx =
          s_M3C2Params.outputCloud->addScalarField(s_M3C2Params.sigChangeSF);
    }
    ...
    if (s_M3C2Params.outputCloud != cloud1 &&
      ...
      s_M3C2Params.outputCloud->setDisplay(
          s_M3C2Params.corePoints->getDisplay());

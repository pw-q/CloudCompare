...
qM3C2Dialog::qM3C2Dialog(ccPointCloud *cloud1, ccPointCloud *cloud2,
                         ccMainAppInterface *app)
    : QDialog(app ? app->getMainWindow() : nullptr), Ui::M3C2Dialog(),
      m_app(app), m_cloud1(nullptr), m_cloud2(nullptr),
  ...
  connect(showCloud1CheckBox, &QAbstractButton::toggled, this,
          &qM3C2Dialog::setCloud1Visibility);
  connect(showCloud2CheckBox, &QAbstractButton::toggled, this,
          &qM3C2Dialog::setCloud2Visibility);

  connect(loadParamsToolButton, &QAbstractButton::clicked, this,
          &qM3C2Dialog::getParamsFromFile);
  connect(saveParamsToolButton, &QAbstractButton::clicked, this,
          &qM3C2Dialog::saveParamsToFile);
  connect(swapCloudsToolButton, &QAbstractButton::clicked, this,
          &qM3C2Dialog::swapClouds);
  connect(guessParamsPushButton, &QAbstractButton::clicked, this,
          &qM3C2Dialog::guessParamsSlow);

  connect(projDestComboBox, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &qM3C2Dialog::projDestIndexChanged);

  connect(cpOtherCloudComboBox, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &qM3C2Dialog::updateNormalComboBox);
  connect(normalSourceComboBox, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &qM3C2Dialog::onUpdateNormalComboBoxChanged);
  connect(cpUseCloud1RadioButton, &QAbstractButton::toggled, this,
          &qM3C2Dialog::updateNormalComboBox);
  connect(cpSubsampleRadioButton, &QAbstractButton::toggled, this,
          &qM3C2Dialog::updateNormalComboBox);
  connect(cpUseOtherCloudRadioButton, &QAbstractButton::toggled, this,
          &qM3C2Dialog::updateNormalComboBox);
  ...
  if (m_app) {
    ...
    for (size_t i = 0; i < clouds.size(); ++i) {
      if (clouds[i]->isA(
              CC_TYPES::POINT_CLOUD)) // as filterChildren only test 'isKindOf'
      {
        cpOtherCloudComboBox->addItem(GetEntityName(clouds[i]),
                                      QVariant(clouds[i]->getUniqueID()));
        normOriCloudComboBox->addItem(GetEntityName(clouds[i]),
                                      QVariant(clouds[i]->getUniqueID()));
      }
    }
  } else {
  ...
  loadParamsFromPersistentSettings(); // must be done after updating the 'normal
                                      // source' combox-box!
}

bool PopulateSFCombo(QComboBox *combo, const ccPointCloud &cloud,
...
bool PopulatePMFields(QComboBox *sx, QComboBox *sy, QComboBox *sz,
...
void qM3C2Dialog::setupPrecisionMapsTab() {
  ...
  if (m_cloud1->hasScalarFields() && m_cloud2->hasScalarFields()) {
    ...
    bool auto1 =
        PopulatePMFields(c1SxComboBox, c1SyComboBox, c1SzComboBox, *m_cloud1);
    bool auto2 =
        PopulatePMFields(c2SxComboBox, c2SyComboBox, c2SzComboBox, *m_cloud2);
...
void qM3C2Dialog::updateNormalComboBox() {
  int previouslySelectedItem = normalSourceComboBox->currentIndex() >= 0
                                   ? normalSourceComboBox->currentData().toInt()
                                   : -1;
  int lastIndex = -1;
  ...
  if (m_cloud1 && m_cloud1->hasNormals()) {
    normalSourceComboBox->addItem("Use cloud #1 normals",
  ...
  if (cpUseOtherCloudRadioButton->isChecked()) {
    ...
    if (otherCloud && otherCloud->hasNormals()) {
      ...
      ++lastIndex;
      if (previouslySelectedItem == qM3C2Normals::USE_CORE_POINTS_NORMALS ||
          previouslySelectedItem < 0) {
        normalSourceComboBox->setCurrentIndex(lastIndex);
...
qM3C2Normals::ComputationMode qM3C2Dialog::getNormalsComputationMode() const {
  ...
  if (!m_app) {
    if (getRequestedComputationMode() == qM3C2Normals::VERT_MODE) {
      ccLog::Print(
          "[M3C2] force vertical mode as requested in the parameter file");
...
void qM3C2Dialog::loadParamsFrom(const QSettings &settings) {
  ...
  int normModeInt =
      settings
          .value("NormalMode", static_cast<int>(getNormalsComputationMode()))
          .toInt();
  double normMinScale =
      settings.value("NormalMinScale", minScaleDoubleSpinBox->value())
          .toDouble();
  double normStep =
      settings.value("NormalStep", stepScaleDoubleSpinBox->value()).toDouble();
  double normMaxScale =
      settings.value("NormalMaxScale", maxScaleDoubleSpinBox->value())
          .toDouble();
  bool normUseCorePoints =
      settings
          .value("NormalUseCorePoints", normUseCorePointsCheckBox->isChecked())
          .toBool();
  int normPreferredOri =
      settings
          .value("NormalPreferedOri", normOriPreferredComboBox->currentIndex())
          .toInt();
  ...
  double subsampleRadius =
      settings.value("SubsampleRadius", cpSubsamplingDoubleSpinBox->value())
          .toDouble();
  bool subsampleEnabled =
      settings.value("SubsampleEnabled", cpSubsampleRadioButton->isChecked())
          .toBool();

  double registrationError =
      settings.value("RegistrationError", rmsDoubleSpinBox->value()).toDouble();
  ...
  bool useSinglePass4Depth =
      settings
          .value("UseSinglePass4Depth",
                 useSinglePass4DepthCheckBox->isChecked())
          .toBool();
  bool positiveSearchOnly =
      settings
          .value("PositiveSearchOnly", positiveSearchOnlyCheckBox->isChecked())
          .toBool();
  bool useMedian =
      settings.value("UseMedian", useMedianCheckBox->isChecked()).toBool();

  bool useMinPoints4Stat =
      settings
          .value("UseMinPoints4Stat", useMinPoints4StatCheckBox->isChecked())
          .toBool();
  int minPoints4Stat =
      settings.value("MinPoints4Stat", minPoints4StatSpinBox->value()).toInt();

  int projDestIndex =
      settings.value("ProjDestIndex", projDestComboBox->currentIndex()).toInt();
  bool useOriginalCloud =
      settings.value("UseOriginalCloud", useOriginalCloudCheckBox->isChecked())
          .toBool();

  bool exportStdDevInfo =
      settings.value("ExportStdDevInfo", exportStdDevInfoCheckBox->isChecked())
          .toBool();
  bool exportDensityAtProjScale =
      settings
          .value("ExportDensityAtProjScale",
                 exportDensityAtProjScaleCheckBox->isChecked())
          .toBool();

  int maxThreadCount =
      settings.value("MaxThreadCount", ccQtHelpers::GetMaxThreadCount())
          .toInt();

  bool usePrecisionMaps =
      settings.value("UsePrecisionMaps", precisionMapsGroupBox->isChecked())
          .toBool();
  double pm1Scale =
      settings.value("PM1Scale", pm1ScaleDoubleSpinBox->value()).toDouble();
  double pm2Scale =
      settings.value("PM2Scale", pm2ScaleDoubleSpinBox->value()).toDouble();
  ...
  case qM3C2Normals::USE_CORE_POINTS_NORMALS: {
    ...
    if (!found) {
      ccLog::Warning("Can't restore the previous normal computation method "
...
bool qM3C2Dialog::loadParamsFromFile(QString filename) {
  ...
  // load the requested normal mode from the parameter file (used in command
  // line calls)
  int normalMode =
      fileSettings
          .value("NormalMode", static_cast<int>(qM3C2Normals::DEFAULT_MODE))
          .toInt();
  m_requestedComputationMode =
      static_cast<qM3C2Normals::ComputationMode>(normalMode);
...
void qM3C2Dialog::saveParamsToFile() {
  ...
  QString filename;
  {
    ...
    filename = QFileDialog::getSaveFileName(
        this, "Save M3C2 parameters", currentPath + QString("/m3c2_params.txt"),
        "*.txt");
    if (filename.isEmpty())
      return;
...
void qM3C2Dialog::guessParams(bool fastMode /*=false*/) {
  ...
  unsigned minPoints4Stats =
      getMinPointsForStats() * 6; // see article: ideal = 30 while default = 5
  ...
  if (qM3C2Tools::GuessBestParams(m_cloud1, m_cloud2, minPoints4Stats, params,
    ...
    if (params.preferredDimension >= 0 && params.preferredDimension < 3) {
      normOriPreferredComboBox->setCurrentIndex(
          params.preferredDimension *
          2); //*2 because for each dimension, the preferred orientation is
              //either + or -
    }

...
public:
  explicit MeshWrapper(ccMesh &mesh, ccPointCloud &vertices,
                       CCCoreLib::ScalarField *densitySF = nullptr)
      : m_mesh(mesh), m_vertices(vertices), m_densitySF(densitySF),
        m_error(false) {}
  ...
  virtual void addColor(const Real *rgb) override {
    ...
    m_vertices.addColor(static_cast<ColorCompType>(
                            std::min((Real)255, std::max((Real)0, rgb[0]))),
                        static_cast<ColorCompType>(
                            std::min((Real)255, std::max((Real)0, rgb[1]))),
                        static_cast<ColorCompType>(
                            std::min((Real)255, std::max((Real)0, rgb[2]))));
  }
...
public:
  explicit PoissonReconParamDlg(QWidget *parent = nullptr)
    ...
    threadSpinBox->setRange(1,
                            PoissonReconLib::Parameters::GetMaxThreadCount());
  }
};
...
bool doReconstruct() {
  ...
  ccLog::Print(QString("[PoissonRecon] Duration: %1 s")
...
void qPoissonRecon::doAction() {
  ...
  newMesh->setName(
      QString("Mesh[%1] (level %2)").arg(pc->getName()).arg(s_params.depth));

...
ccEDLFilter::ccEDLFilter()
    : ccGlFilter("EyeDome Lighting (disable normals and increase points size "
                 "for a better result!)"),
      m_screenWidth(0), m_screenHeight(0), m_EDLShader(nullptr),
      m_fboMix(nullptr), m_mixShader(nullptr), m_expScale(100.0f),
...
bool ccEDLFilter::init(unsigned width, unsigned height,
                       const QString &shadersPath, QString &error) {
  return init(width, height, GL_RGBA, GL_LINEAR, shadersPath, error);
}

bool ccEDLFilter::init(unsigned width, unsigned height, GLenum internalFormat,
                       GLenum minMagFilter, const QString &shadersPath,
                       QString &error) {
  if (width == 0 || height == 0) {
  ...
  for (unsigned i = 0; i < FBO_COUNT; ++i) {
    ...
    if (!fbo->init(w, h) ||
      ...
      error =
          QString("[EDL Filter] FBO 1:%1 initialization failed!").arg(scale);
      reset();
      return false;
    }
...
void ccEDLFilter::shade(GLuint texDepth, GLuint texColor,
  ...
  m_glFunc.glOrtho(0.0, static_cast<GLdouble>(m_screenWidth), 0.0,
                   static_cast<GLdouble>(m_screenHeight), 0.0, 1.0);
  ...
  float lightMod = parameters.perspectiveMode
                       ? 3.0f
                       : 1.2f; // FIXME: we would need to be smarter and depend
                               // on the actual 'zoom' (= the focal distance now)
  lightMod *= parameters.zoomFactor;
  // ccLog::Print(QString("Zm = %1 / ZM = %2
  // (%3)").arg(parameters.zNear).arg(parameters.zFar).arg(parameters.zFar -
  // parameters.zNear));
  ...
  for (unsigned i = 0; i < FBO_COUNT; ++i) {
    ...
    m_EDLShader->setUniformValue("PerspectiveMode",
                                 parameters.perspectiveMode ? 1 : 0);
    ...
    m_EDLShader->setUniformValueArray(
        "Light_dir", reinterpret_cast<const GLfloat *>(m_lightDir), 1, 3);
    m_EDLShader->setUniformValueArray(
        "Neigh_pos_2D", reinterpret_cast<const GLfloat *>(m_neighbours), 8, 2);

    m_glFunc.glActiveTexture(GL_TEXTURE1);
    m_glFunc.glBindTexture(GL_TEXTURE_2D, texColor);

    m_glFunc.glActiveTexture(GL_TEXTURE0);
    ccGLUtils::DisplayTexture2DPosition(texDepth, 0, 0, m_screenWidth / scale,
                                        m_screenHeight / scale);
  ...
  if (m_fboMix) {
    ...
    GLuint texCol0 = m_bilateralFilters[0].filter
                         ? m_bilateralFilters[0].filter->getTexture()
                         : m_fbos[0]->getColorTexture();
    GLuint texCol1 = m_bilateralFilters[1].filter
                         ? m_bilateralFilters[1].filter->getTexture()
                         : m_fbos[1]->getColorTexture();
    GLuint texCol2 = m_bilateralFilters[2].filter
                         ? m_bilateralFilters[2].filter->getTexture()
                         : m_fbos[2]->getColorTexture();
    ...
    m_glFunc.glActiveTexture(GL_TEXTURE0);
    ccGLUtils::DisplayTexture2DPosition(texCol0, 0, 0, m_screenWidth,
                                        m_screenHeight);

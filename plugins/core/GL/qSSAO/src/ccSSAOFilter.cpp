...
ccSSAOFilter::ccSSAOFilter()
    : ccGlFilter("Screen Space Ambient Occlusion"), m_w(0), m_h(0),
      m_fbo(nullptr), m_shader(nullptr), m_texReflect(0), m_glFuncIsValid(0) {
...
ccSSAOFilter::~ccSSAOFilter() { reset(); }
...
bool ccSSAOFilter::init(unsigned width, unsigned height,
                        const QString &shadersPath, QString &error) {
  return init(width, height, true, true, shadersPath, error);
}

bool ccSSAOFilter::init(unsigned width, unsigned height,
                        bool enableBilateralFilter, bool useReflectTexture,
                        const QString &shadersPath, QString &error) {
  if (width == 0 || height == 0) {
...
void ccSSAOFilter::sampleSphere() {
  ...
  rk_sobol_state s;
  if (rk_sobol_init(3, &s, nullptr, rk_sobol_Ldirections, nullptr) !=
      RK_SOBOL_OK) {
    return;
  }
...
void ccSSAOFilter::shade(GLuint texDepth, GLuint texColor,
  ...
  m_glFunc.glOrtho(0.0, static_cast<GLdouble>(m_w), 0.0,
                   static_cast<GLdouble>(m_h), 0.0, 1.0);
  ...
  if (m_bilateralFilter) {
    m_bilateralFilter->setParams(m_bilateralGHalfSize, m_bilateralGSigma,
                                 m_bilateralGSigmaZ);
...
inline double frand() { return static_cast<double>(rand()) / RAND_MAX; }
...
void ccSSAOFilter::initReflectTexture() {
  ...
  m_glFunc.glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, m_w, m_h, 0, GL_RGB,
                        GL_FLOAT, &reflectTexture[0]);
  m_glFunc.glBindTexture(GL_TEXTURE_2D, 0);

  m_glFunc.glPopAttrib();
  assert(m_glFunc.glGetError() == GL_NO_ERROR);

  // According to Wikipedia, noise is made of 4*4 repeated tiles	to have only
  // high frequency
}

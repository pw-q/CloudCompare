...
CC_FILE_ERROR CSVMatrixFilter::loadFile(const QString &filename,
  ...
  while (file.error() == QFile::NoError && !file.atEnd()) {
    ...
    if (line.size() == 0) {
      ccLog::Warning(QString("[CSVMatrixFilter] Line %1 is empty (ignored)")
                         .arg(lineIndex));
      continue;
    }
    ...
    if (width < 0) {
      ...
      ccLog::Warning(QString("[CSVMatrixFilter] Line %1 has not the same width "
    ...
    bool ok = true;
    CCVector3 P(0,
                static_cast<PointCoordinateType>((s_inverseRows ? -row : row) *
                                                 s_ySpacing),
                0);
  ...
  if (result == CC_FERR_NO_ERROR && rowCount != 0 && colCount != 0) {
    ...
    if (s_loadAsMesh) {
      ...
      if (!mesh->reserve(triCount)) {
        ...
        if (mesh->computePerVertexNormals()) {
          ...
          ccLog::Warning(QString("[CSVMatrixFilter] Failed to compute normals "
                                 "on mesh (not enough memory?)"));
        }
      }
    }
    ...
    if (s_useTexture) {
      ...
      if (!texture.load(filename)) {
        ccLog::Warning(
            QString("[CSVMatrixFilter] Failed to load texture from file '%1'")
                .arg(filename));
      } else {
        if (mesh) {
          ...
          if (texCoords->reserveSafe(cloud->size()) &&
            ...
            ccLog::Warning("[CSVMatrixFilter] Not enough memory to map the "
                           "texture on the mesh!");
            texCoords->release();
            texCoords = nullptr;
          }
          ...
          if (texture.width() < static_cast<int>(colCount) ||
            ...
            ccLog::Warning(QString("[CSVMatrixFilter] To map a texture on the "
                                   "cloud, the image size should equal or "
                                   "greater than the grid size (%1x%2)")
                               .arg(colCount)
                               .arg(rowCount));
            ...
            for (unsigned j = 0; j < rowCount; ++j) {
              for (unsigned i = 0; i < colCount; ++i) {
                QRgb col =
                    texture.pixel(static_cast<int>(i), static_cast<int>(j));
    ...
    if (s_inverseRows) {
      CCVector3 T(
          0, static_cast<PointCoordinateType>((rowCount - 1) * s_ySpacing), 0);
      ccGLMatrix trans;

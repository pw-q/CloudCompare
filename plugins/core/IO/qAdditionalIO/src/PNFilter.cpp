...
PNFilter::PNFilter()
    : FileIOFilter(
          {"_Point+Normal Filter",
           ...
           QStringList{"pn"}, "pn", QStringList{"Point+Normal cloud (*.pn)"},
           QStringList{"Point+Normal cloud (*.pn)"}, Import | Export}) {}

bool PNFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
CC_FILE_ERROR PNFilter::saveToFile(ccHObject *entity, const QString &filename,
  ...
  if (theCloud->isShifted())
    ccLog::Warning(QString("[PNFilter::save] Can't recenter or rescale cloud "
                           "'%1' when saving it in a PN file!")
                       .arg(theCloud->getName()));

  bool hasNorms = theCloud->hasNormals();
  if (!hasNorms)
    ccLog::Warning(QString("[PNFilter::save] Cloud '%1' has no normal (we will "
                           "save points with a default normal)!")
                       .arg(theCloud->getName()));

...
bool SalomeHydroFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
CC_FILE_ERROR SalomeHydroFilter::saveToFile(ccHObject *entity,
  ...
  for (size_t i = 0; i < candidates.size(); ++i) {
    ...
    if (vertCount < 2) {
      // invalid size
      ccLog::Warning(
          QString("[Salome Hydro] Polyline '%1' does not have enough vertices")
    ...
    if (currentLine.isNull() || currentLine.isEmpty()) {
      ...
      if (currentVertices) {
        if (currentVertices->size() < 2) {
          ...
          ccLog::Warning("[Salome Hydro] An invalid polyline (single vertex) "
                         "will be ignored");
        } else {
          bool isClosed = false;
          if (currentVertices->size() > 2) {
            const CCVector3 *firstVertex = currentVertices->getPoint(0);
            const CCVector3 *lastVertex =
                currentVertices->getPoint(currentVertices->size() - 1);

            // close the polyline
            if (CCCoreLib::LessThanEpsilon(
                    (*lastVertex - *firstVertex).norm2())) {
              isClosed = true;
      ...
      if (parts.size() == 3) {
        ...
        if (firstPoint) {
          if (HandleGlobalShift(P, Pshift, preserveCoordinateShift,
            ...
            ccLog::Warning("[Salome Hydro] Polylines will be recentered! "
        ...
        ccLog::Warning(
            "[Salome Hydro] Malformed file: 3 values per line expected");

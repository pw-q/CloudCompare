...
bool PVFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
CC_FILE_ERROR PVFilter::saveToFile(ccHObject *entity, const QString &filename,
  ...
  if (theCloud->isShifted())
    ccLog::Warning(QString("[PVFilter::save] Can't recenter or rescale cloud "
                           "'%1' when saving it in a PN file!")
                       .arg(theCloud->getName()));

  // for point clouds with multiple SFs, we must set the currently displayed one
  // as 'input' SF if (theCloud->isA(CC_TYPES::POINT_CLOUD))
  //{
  ...
  if (!hasSF)
    ccLog::Warning(
        QString("[PVFilter::save] Cloud '%1' has no displayed scalar field (we "
                "will save points with a default scalar value)!")
            .arg(theCloud->getName()));
...
CC_FILE_ERROR PVFilter::loadFile(const QString &filename, ccHObject &container,
  ...
  for (unsigned i = 0; i < numberOfPoints; i++) {
    if (pointsRead == fileChunkPos + fileChunkSize) {
      ...
      fileChunkSize = std::min<unsigned>(numberOfPoints - pointsRead,
                                         CC_MAX_NUMBER_OF_POINTS_PER_CLOUD);
      loadedCloud =
          new ccPointCloud(QString("unnamed - Cloud #%1").arg(++chunkIndex));
      sf = nullptr;
      if (!loadedCloud || !loadedCloud->reserveThePointsTable(fileChunkSize) ||

...
bool PovFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
CC_FILE_ERROR PovFilter::saveToFile(ccHObject *entity, const QString &filename,
    ...
    for (unsigned i = 0; i < hClouds.size(); ++i) {
      ...
      if (!cloudSensors.empty()) {
        ...
        if (cloudSensors.size() > 1)
          ccLog::Warning(QString("Found more than one GBL sensor associated to "
                                 "entity '%1'. Only the first will be saved!")
  ...
  // save sensor(s) info
  {
    for (unsigned i = 0; i < clouds.size(); ++i) {
      ...
      CC_FILE_ERROR error = FileIOFilter::SaveToFile(
          clouds[i], thisFilename, parameters, BinFilter::GetFileFilter());
...
CC_FILE_ERROR PovFilter::loadFile(const QString &filename, ccHObject &container,
  ...
  if (strcmp(sensorType,
          ...
          0 ||
      strcmp(sensorType,
             CC_SENSOR_ROTATION_ORDER_OLD_NAMES[ccGBLSensor::YAW_THEN_PITCH]) ==
          0) {
    rotationOrder = ccGBLSensor::YAW_THEN_PITCH;
  } else if (strcmp(
                 sensorType,
                 CC_SENSOR_ROTATION_ORDER_NAMES[ccGBLSensor::PITCH_THEN_YAW]) ==
                 0 ||
    ...
    ccLog::Warning(
        "[PovFilter::loadFile] Unhandled rotation order description! (%s)",
        sensorType);
    fclose(fp);
    return CC_FERR_READING;
  }
  ...
  while (fgets(line, MAX_ASCII_FILE_LINE_LENGTH, fp)) {
    if ((line[0] == '#') && (line[1] == 'P')) {
      ...
      ccHObject *entities = FileIOFilter::LoadFromFile(
          QString("%1/%2").arg(path, subFileName), parameters, filter, result);
      if (entities) {
        ccGLMatrix rot;
        rot.toIdentity();
        ...
        for (size_t i = 0; i < clouds.size(); ++i) {
          ...
          // DGM: the base simply corresponds to a shift of the center along the
          // X axis!
          sensorCenter.x -= base;
          // DGM: sensor center is now integrated in rigid transformation (=
          // inverse of former rotation matrix + center as translation)
          ...
          if (gls->computeAutoParameters(theCloud)) {
            ...
            ccLog::Warning(QString("[PovFilter::loadFile] failed to create "
                                   "sensor on cloud #%1 (%2)")
                               .arg(i)

...
CC_FILE_ERROR BundlerFilter::loadFileExtended(
    ...
    bool _undistortImages /*=false*/, bool _generateColoredDTM /*=false*/,
  ...
  // Read Bundler '.out' file
  {
    ...
    if (parameters.parentWidget) {
      ...
      pDlg->setInfo(
          QObject::tr("Cameras: %1\nPoints: %2").arg(camCount).arg(ptsCount));
      pDlg->start();
    }
    CCCoreLib::NormalizedProgress nprogress(
        pDlg.data(),
        camCount + (importKeypoints || orthoRectifyImages || generateColoredDTM
                        ? ptsCount
                        : 0));
    ...
    if (!useAltKeypoints &&
      ...
      for (unsigned i = 0; i < ptsCount; ++i) {
        ...
        if (currentLine.startsWith(
                "--")) // skip lines starting with '--' (yes it happens in some
                       // weird version of Bundler?!)
          currentLine = stream.readLine();
        if (currentLine.isEmpty()) {
          delete keypointsCloud;
          return CC_FERR_READING;
        ...
        if (i == 0) {
          ...
          if (HandleGlobalShift(Pd, Pshift, preserveCoordinateShift,
            ...
            ccLog::Warning("[Bundler] Cloud has been recentered! Translation: "
                           "(%.2f ; %.2f ; %.2f)",
        ...
        if (colorParts.size() == 3) {
          if (hasColors) {
            ...
            keypointsCloud->addColor(
                static_cast<ColorCompType>(std::min<int>(R, ccColor::MAX)),
          ...
          // so we skip the line and assume it's in fact the keypoint
          // description...
        ...
        if (storeKeypoints || !camUsage.empty()) {
          ...
          if (!parts.isEmpty()) {
            ...
            if (!ok || nviews * 4 + 1 > static_cast<unsigned>(parts.size())) {
              ccLog::Warning("[Bundler] View list for point #%i is invalid!",
                             i);
            } else {
              unsigned pos = 1;
              for (unsigned n = 0; n < nviews; ++n) {
                int cam = parts[pos++].toInt(); // camera index
                ++pos; // int key = parts[pos++].toInt();		//index of
                       // the SIFT keypoint where the point was detected in that
                       // camera (not used)
                ...
                if (storeKeypoints) {
                  float x =
                      parts[pos++]
                          .toFloat(); // detected positions of that keypoint (x)
                  float y =
                      parts[pos++]
                          .toFloat(); // detected positions of that keypoint (y)
                  // add key point
                  ...
                  lastKeyPoint.second.x =
                      x * scaleFactor; // the origin is the center of the image,
                                       // the x-axis increases to the right
                  lastKeyPoint.second.y =
                      -y * scaleFactor; // and the y-axis increases towards the
                                        // top of the image
                  try {
                    keypointsDescriptors.push_back(lastKeyPoint);
                  } catch (const std::bad_alloc &) {
                    ccLog::Warning(
                        "[Bundler] Not enough memory to store keypoints!");
      ...
      if (!camUsage.empty()) {
        for (size_t i = 0; i < camUsage.size(); ++i) {
          if (!camUsage[i])
            ccLog::Warning(
                QString("[Bundler] Camera #%1 has no associated keypoints!")
                    .arg(i + 1));
        }
      }
      ...
      if (applyOptMatrix) {
        ...
        ccLog::Print("[Bundler] Keypoints cloud has been transformed with "
                     "input matrix!");
  ...
  if (useAltKeypoints) {
    ...
    if (!altKeypointsContainer ||
        ...
        (!altKeypointsContainer->getChild(0)->isKindOf(CC_TYPES::POINT_CLOUD) &&
         !altKeypointsContainer->getChild(0)->isKindOf(CC_TYPES::MESH))) {
      if (!altKeypointsContainer)
        ccLog::Error(
            QString(
                "[Bundler] Failed to load alternative keypoints file:\n'%1'")
                .arg(altKeypointsFilename));
      else
        ccLog::Error("[Bundler] Can't use this kind of entities as keypoints "
                     "(need one and only one cloud or mesh)");
  ...
  // load images
  QDir imageDir =
      QFileInfo(f).dir(); // by default we look in the Bundler file folder
  ...
  QStringList imageFilenames;
  {
    ...
    if (!imageListFile.exists() || !imageListFile.open(QIODevice::ReadOnly)) {
      ccLog::Error(QString("[Bundler] Failed to open image list file! (%1)")
    ...
    for (unsigned lineIndex = 0; lineIndex < camCount; ++lineIndex) {
      ...
      QStringList parts =
          nextLine.simplified().split(QChar(' '), QString::SkipEmptyParts);
      if (!parts.empty()) {
        imageFilenames << parts[0];
      } else {
        ccLog::Error(QString("[Bundler] Couldn't extract image name from line "
                             "%1 in file '%2'!")
                         .arg(lineIndex)
                         .arg(imageListFilename));
        break;
  ...
  if (imageFilenames.size() < static_cast<int>(camCount)) // not enough images!
  {
    if (imageFilenames.isEmpty())
      ccLog::Error(
          QString("[Bundler] No filename could be extracted from file '%1'!")
              .arg(imageListFilename));
    else
      ccLog::Error(QString("[Bundler] Only %1 filenames (out of %2) could be "
                           "extracted\nfrom file '%3'!")
  ...
  if (parameters.parentWidget) {
    ipDlg.reset(new ccProgressDialog(
        true, parameters.parentWidget)); // cancel available
  ...
  if (generateColoredDTM) {
    ...
    if (parameters.parentWidget) {
      toDlg.reset(new ccProgressDialog(
          true, parameters.parentWidget)); // cancel available
    ...
    ccGenericMesh *baseDTMMesh =
        (altEntity ? ccHObjectCaster::ToGenericMesh(altEntity) : nullptr);
    ...
    if (!baseDTMMesh) {
      ...
      ccGenericPointCloud *altKeypoints =
          (altEntity ? ccHObjectCaster::ToGenericPointCloud(altEntity)
                     : nullptr);
      std::string errorStr;
      dummyMesh = CCCoreLib::PointProjectionTools::computeTriangulation(
          altKeypoints ? altKeypoints : keypointsCloud,
          CCCoreLib::DELAUNAY_2D_BEST_LS_PLANE,
          CCCoreLib::PointProjectionTools::IGNORE_MAX_EDGE_LENGTH, 0, errorStr);
      if (!dummyMesh) {
    ...
    if (dummyMesh) {
      ...
      mntSamples.reset(CCCoreLib::MeshSamplingTools::samplePointsOnMesh(
          (CCCoreLib::GenericMesh *)dummyMesh, coloredDTMVerticesCount));
      if (!baseDTMMesh)
        delete dummyMesh;
      dummyMesh = nullptr;
      ...
      if (mntSamples) {
        ...
        try {
          ...
          // not enough memory
          ccLog::Error("Not enough memory to store DTM colors! DTM generation "
                       "cancelled");
  ...
  if (orthoRectifyImages) {
    altKeypoints =
        (altEntity ? ccHObjectCaster::ToGenericPointCloud(altEntity) : nullptr);
  ...
  for (unsigned i = 0; i < camCount; ++i) {
    ...
    if (!image->load(imageDir.absoluteFilePath(imageFilenames[i]), errorStr)) {
      ccLog::Error(QString("[Bundler] %1 (image '%2')")
    ...
    ccCameraSensor *sensor = nullptr;
    {
      ...
      // dist to cloud
      PointCoordinateType dist = keypointsCloud
                                     ? (transf.getTranslationAsVec3D() -
      ...
      sensor->setGraphicScale(
          keypointsCloud ? keypointsCloud->getOwnBB().getDiagNorm() / 10
                         : CCCoreLib::PC_ONE);
      ...
      if (cameras[i].k1 != 0 || cameras[i].k2 != 0) {
        ...
        sensor->setDistortionParameters(
            ccCameraSensor::LensDistortionParameters::Shared(distParams));
      }

      // apply optional matrix (if any)
      if (applyOptMatrix) {
        sensor->applyGLTransformation_recursive(&orthoOptMatrix);
        // ccLog::Print("[Bundler] Camera cloud has been transformed with input
        // matrix!"); this transformation is of no interest for the user
    ...
    if (orthoRectifyImages) {
      ...
      if (keypointsImage.size() < 4) {
        ccLog::Warning(
            QString(
                "[Bundler] Not enough keypoints descriptors for image '%1'!")
                .arg(image->getName()));
      } else if (!keypointsImageBB.isValid() ||
                 keypointsImageBB.getDiagNorm() < CCCoreLib::PC_ONE) {
        ccLog::Warning(QString("[Bundler] Keypoints descriptors for image '%1' "
                               "are invalid (= all the same)")
                           .arg(image->getName()));
      } else {
        if (orthoRectifyImagesAsImages) {
          ...
          if (orthoRectMethod == BundlerImportDlg::OPTIMIZED) {
            orthoImage = sensor->orthoRectifyAsImage(
                image, _keypointsCloud, keypointsImage, OR_pixelSize, info.minC,
                info.maxC, corners);
          }
          ...
          if (orthoImage) {
            ...
            info.name = QString("ortho_%1.png")
                            .arg(QFileInfo(imageFilenames[i]).baseName());
            info.w = orthoImage->getW();
            info.h = orthoImage->getH();
            orthoImage->data().save(imageDir.absoluteFilePath(info.name));
            ccLog::Print(QString("[Bundler] Ortho-rectified version of image "
                                 "'%1' (%2 x %3) saved to '%4'")
                             .arg(imageFilenames[i])
                             .arg(info.w)
                             .arg(info.h)
                             .arg(imageDir.absoluteFilePath(info.name)));
...
#define TBI_DEFAULT_TILE_POW 10 // 2^10 = 1024
            ...
            ccLog::Warning(
                QString("[Bundler] Failed to ortho-rectify image '%1'!")
                    .arg(image->getName()));
          }
        }
        ...
        if (orthoRectifyImagesAsClouds) {
          ...
          if (orthoCloud) {
            ...
            ccLog::Warning(
                QString(
                    "[Bundler] Failed to ortho-rectify image '%1' as a cloud!")
                    .arg(image->getName()));
          }
        }
      }
    ...
    if (sensor && undistortImages)
      if (!sensor->undistort(image, true))
        ccLog::Warning(QString("[Bundler] Failed to undistort image '%1'!")
                           .arg(image->getName()));
    ...
    if (sensor && generateColoredDTM) {
      ...
      for (unsigned k = 0; k < sampleCount; ++k) {
        ...
        if (CCCoreLib::GreaterThanEpsilon(std::abs(P.z))) {
          ...
          // float rp = 1.0+norm_p2*(cam.k1+cam.k2*norm_p2); //images are
          // already undistorted
          float rp = 1.0f;
    ...
    if (f.open(QIODevice::WriteOnly | QIODevice::Text)) {
      ...
      for (unsigned i = 0; i < OR_infos.size(); ++i) {
        ...
        int xShiftGlobal = static_cast<int>(
            (OR_infos[i].minC[0] - OR_globalCorners[0]) / OR_pixelSize);
        int yShiftGlobal = static_cast<int>(
            (OR_globalCorners[3] - OR_infos[i].maxC[1]) / OR_pixelSize);
      ...
      ccLog::Warning("Failed to save orthorectification log file! "
    ...
    if (!cancelledByUser) {
      ...
      if (mntCloud->reserve(sampleCount) && mntCloud->reserveTheRGBTable()) {
        ...
        if (realCount != 0) {
          ...
          ccLog::Warning(
              "[Bundler] DTM vertices successfully generated: clean it if "
              "necessary then use 'Edit > Mesh > Compute Delaunay 2D (Best LS "
              "plane)' then 'Smooth' to get a proper mesh");

          if (!parameters.alwaysDisplayLoadDialog) {
            // auto save DTM vertices
            BinFilter bf;
            QString outputFile =
                imageDir.absoluteFilePath("colored_dtm_vertices.bin");
            ...
            if (bf.saveToFile(mntCloud, outputFile, parameters) ==
                CC_FERR_NO_ERROR)
              ccLog::Print(QString("[Bundler] Color DTM vertices automatically "
                                   "saved to '%2'")
                               .arg(outputFile));
            else
              ccLog::Warning(
                  QString("[Bundler] Failed to save DTM vertices to '%2'")
                      .arg(outputFile));
          }
        } else {
          ccLog::Warning(
              "[Bundler] Failed to generate DTM! (no point viewed in images?)");
        }
      } else {
        ccLog::Warning("[Bundler] Failed to generate DTM vertices cloud! (not "
                       "enough memory?)");
        delete mntCloud;
        mntCloud = nullptr;
      }

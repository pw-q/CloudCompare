...
bool BundlerCommand::process(ccCommandLineInterface &cmd) {
  ...
  while (!cmd.arguments().empty()) {
    ...
    if (ccCommandLineInterface::IsCommand(argument,
      ...
      if (cmd.arguments().empty())
        return cmd.error(QObject::tr("Missing parameter: value after \"-%1\"")
                             .arg(COMMAND_BUNDLER_SCALE_FACTOR));
      bool conversionOk = false;
      scaleFactor = cmd.arguments().takeFirst().toFloat(&conversionOk);
      if (!conversionOk)
        return cmd.error(QObject::tr("Invalid parameter: value after \"-%1\"")
                             .arg(COMMAND_BUNDLER_SCALE_FACTOR));
      ...
      if (cmd.arguments().empty())
        return cmd.error(
            QObject::tr("Missing parameter: vertices count after \"-%1\"")
      ...
      if (!conversionOk)
        return cmd.error(
            QObject::tr("Invalid parameter: vertices count after \"-%1\"")
                .arg(COMMAND_BUNDLER_COLOR_DTM));
      generateColoredDTM = true;
    } else {
      break; // as soon as we encounter an unrecognized argument, we break the
             // local loop to go back to the main one!
    }
  }

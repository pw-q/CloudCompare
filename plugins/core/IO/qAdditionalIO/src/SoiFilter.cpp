...
CC_FILE_ERROR SoiFilter::loadFile(const QString &filename, ccHObject &container,
  ...
  if ((nbScansTotal == 0) || (nbPointsTotal == 0)) {
    ccLog::Warning(
        "[SoiFilter::loadFile] No points or scans defined in this file!");
    fclose(fp);
  ...
  for (unsigned k = 0; k < nbScansTotal; k++) {
    ...
    if (strcmp(line.substr(0, 4).c_str(), "#pt#") == 0) {
      ...
      // ccLog::Print("[SoiFilter::loadFile] Scan %i - points:
      // %i",k+1,nbOfPoints);
    } else {

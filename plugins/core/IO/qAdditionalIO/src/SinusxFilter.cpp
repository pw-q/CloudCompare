...
bool SinusxFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
CC_FILE_ERROR SinusxFilter::saveToFile(ccHObject *entity,
  ...
  // write header
  outFile << "C Generated by Zoomlion" << endl;
  ...
  for (size_t i = 0; i < profiles.size(); ++i) {
    ...
    if (vertCount < 2) {
      // invalid size
      ccLog::Warning(
          QString("[Sinusx] Polyline '%1' does not have enough vertices")
  ...
  while (!currentLine.isEmpty() && file.error() == QFile::NoError) {
    ...
    if (currentLine.startsWith("C ")) {
      ...
      if (tokens.size() < 2 || tokens[1].length() > 1) {
        ccLog::Warning(
            QString("[SinusX] Line %1 is corrupted").arg(lineNumber));
      ...
      if (curveType == INVALID) {
        ccLog::Warning(
            QString("[SinusX] Unhandled curve type '%1' on line '%2'!")
                .arg(curveTypeChar)
                .arg(lineNumber));
        result = CC_FERR_MALFORMED_FILE;
        continue;
      ...
      if (currentLine.startsWith("CN")) {
        ...
        case 0: // first 'CP' line
        {
          ...
          if (!ok) {
            ccLog::Warning(QString("[SinusX] Line %1 is corrupted (expected: "
                                   "'CP connected_flag closed_flag')")
                               .arg(lineNumber));
            result = CC_FERR_MALFORMED_FILE;
            continue;
          }
        ...
        case 1: // second 'CP' line
        {
          if (curveType == CUREV_S) {
            ...
            // nothing particular for profiles (they are not handled in the same
            // way in CC!)
            ++cpIndex;
            break;
            ...
            if (ok) {
              ...
              if (ok)
                currentPoly->setMetaData(ccPolyline::MetaKeyConstAltitude(),
                                         QVariant(z));
            }
            if (!ok) {
              ccLog::Warning(QString("[SinusX] Line %1 is corrupted (expected: "
            ...
            while (skipped < 16 && !currentLine.isEmpty() &&
              ...
              currentLine = stream.readLine();
              ++lineNumber;
              tokens = currentLine.simplified().split(QChar(' '),
        ...
        case 2: {
          ...
          if (ok) {
            ...
            if (basePlaneChar == '0')
              ...
              ok = false;
            if (ok)
              currentPoly->setMetaData(ccPolyline::MetaKeyUpDir(),
                                       QVariant(vertDir));
          }
          if (!ok) {
            ccLog::Warning(
                QString(
                    "[SinusX] Line %1 is corrupted (expected: 'CP base_plane')")
                    .arg(lineNumber));
            result = CC_FERR_MALFORMED_FILE;
            continue;
          }
              ...
              if (ok) {
                ...
                if (firstVertex /*currentVertices->size() == 0*/) {
                  ...
                  if (HandleGlobalShift(Pd, Pshift, preserveCoordinateShift,
                    ...
                    ccLog::Warning(
                        "[SinusX::loadFile] Polyline has been recentered! "
        ...
        if (!ok) {
          ccLog::Warning(
              QString(
                  "[SinusX] Line %1 is corrupted (expected: 'X Y Z Key ...')")
                  .arg(lineNumber));
          result = CC_FERR_MALFORMED_FILE;
          continue;
        }

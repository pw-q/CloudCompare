...
CC_FILE_ERROR IcmFilter::loadFile(const QString &filename, ccHObject &container,
                                  LoadParameters &parameters) {
  if (CheckForSpecialChars(filename)) {
    ccLog::Warning(QString("[ICM] Input filename contains special characters. "
  ...
  if (!filter) {
    ccLog::Warning(
        QString("[ICM] No I/O filter found for loading file '%1' (type = '%2')")
  ...
  ccHObject *entities = FileIOFilter::LoadFromFile(
      QString("%1/%2").arg(path, cloudFileName), parameters, filter, result);
  if (!entities) {
    fclose(fp);
    return CC_FERR_READING;
  }
...
int IcmFilter::LoadCalibratedImages(ccHObject *entities, const QString &path,
  ...
  if (CheckForSpecialChars(completeImageDescFilename)) {
    ccLog::Warning(QString("[ICM] File '%1' contains special characters. It "
  ...
  if (fp == nullptr) {
    ccLog::Error(
        QString("[IcmFilter::loadCalibratedImages] Error opening file %1!")
            .arg(completeImageDescFilename));
    return -1;
  }
  ...
  while (fgets(line, MAX_ASCII_FILE_LINE_LENGTH, fp) != nullptr) {
    if (line[0] == 'D' && line[1] == 'E' && line[2] == 'F') {
      ...
      if (!CI->load(QString("%1/%2").arg(path, imageFileName), errorStr)) {
        ccLog::Warning(
            QString(
                "[IcmFilter] Failed to load image %1 (%2)! Process stopped...")
      ...
      ccLog::Print("\t Camera orientation=(%f,%f,%f)+[%f]", axis[0], axis[1],
                   axis[2], angle_rad);
      ...
      params.vertFocal_pix =
          params.arrayHeight / (2 * tan(params.vFOV_rad / 2));

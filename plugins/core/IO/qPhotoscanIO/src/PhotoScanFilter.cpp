...
static void DisplayCurrentNodeInfo(QXmlStreamReader &stream) {
  ccLog::Warning("--> " + stream.name().toString() +
        ...
        if (hasDistortion) {
          sensor->setDistortionParameters(
              ccCameraSensor::LensDistortionParameters::Shared(
                  new ccCameraSensor::ExtendedRadialDistortionParameters(
                      distParams)));
        }
      }
...
static QString CreateTempFile(QuaZip &zip, QString zipFilename) {
  if (!zip.setCurrentFile(zipFilename)) {
    ccLog::Warning(
        QString("[Photoscan] Failed to locate '%1' in the Photoscan archive")
            .arg(zipFilename));
    return QString();
  }
  ...
  if (!zipFile.open(QFile::ReadOnly)) {
    ccLog::Warning(
        QString("[Photoscan] Failed to extract '%1' from Photoscan archive")
            .arg(zipFilename));
    return QString();
  }
  ...
  if (!tempFile.open(QFile::WriteOnly)) {
    ccLog::Warning(QString("[Photoscan] Failed to create temp file '%1'")
                       .arg(tempFilename));
    return QString();
  }
...
CC_FILE_ERROR PhotoScanFilter::loadFile(const QString &filename,
  ...
  for (CameraDesc &camera : cameras) {
    ...
    //{
    //	ccLog::Warning(QString("[Photoscan] Failed to locate image '%1' in the
    ...
    //{
    //	ccLog::Warning(QString("[Photoscan] Failed to extract '%1' from
    //Photoscan archive").arg(camera.imageFilename)); 	continue;
    //}
    ...
    if (!qImage.load(absoluteImageFilename)) {
      ccLog::Warning(QString("[Photoscan] Failed to load image '%1'")
  ...
  if (!wasCanceled) {
    ...
    for (CloudDesc &desc : clouds) {
      ...
      if (desc.filename.endsWith(".oc3", Qt::CaseInsensitive)) {
        ccLog::Warning(QString("[Photoscan] OC3 format not supported. Can't "

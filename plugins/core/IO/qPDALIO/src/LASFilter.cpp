...
static const char s_LAS_SRS_Key[] =
    "LAS.spatialReference.nosave"; // DGM: added the '.nosave' suffix because
                                   // this custom type can't be streamed properly

//! Custom ("Extra bytes") field (EVLR)
struct ExtraLasField : LasField {
  //! Default constructor
  ExtraLasField(QString name, Id id, double defaultVal = 0.0, double min = 0.0,
                double max = -1.0)
      : LasField(LAS_EXTRA, defaultVal, min, max),
        fieldName(SanitizeString(name)), pdalId(id), scale(1.0), offset(0.0) {
    if (fieldName != name) {
      ccLog::Warning(
          QString(
...
pdal::Dimension::Id typeToId(LAS_FIELDS sfType, uint8_t pointFormat) {
  ...
  case LAS_FIELDS::LAS_CLASSIF_OVERLAP:
    return pointFormat < 6
               ...
               : pdal::Dimension::Id::ClassFlags; // only for point formats >= 6
    // Invalid flag
...
CC_FILE_ERROR LASFilter::saveToFile(ccHObject *entity, const QString &filename,
  ...
  if (theCloud->isA(CC_TYPES::POINT_CLOUD)) {
    ...
    for (unsigned i = 0; i < pc->getNumberOfScalarFields(); ++i) {
      ...
      auto pos =
          std::find_if(fieldsToSave.begin(), fieldsToSave.end(), name_matches);
  ...
  if (!hasOffsetMetaData) {
    ...
    if (theCloud->isShifted()) {
      lasOffset = -theCloud->getGlobalShift(); //'global shift' is the opposite
                                               //of LAS offset ;)
    ...
    if (ccGlobalShiftManager::NeedShift(bbMax - lasOffset)) {
      ccLog::Warning("[LAS] The former LAS_OFFSET doesn't seem to be optimal. "
                     "Using the minimum bounding-box corner instead.");
      CCVector3d globaShift =
          theCloud->getGlobalShift(); //'global shift' is the opposite of LAS
                                      //offset ;)
    ...
    if (hasScaleMetaData) {
      ...
      if (!canUseOriginalScale) {
        s_saveDlg->labelOriginal
            ->setText(QObject::tr("Original scale is too small for this cloud "
  ...
  minPointFormat =
      LasField::UpdateMinPointFormat(minPointFormat, hasColors, false, true);
          ...
          LAS_POINT_FORMAT_META_DATA)) // DGM: is it really necessary?
  {
    ...
    if (ok && previousPointFormat < 256) {
      minPointFormat =
          std::max(static_cast<uint8_t>(previousPointFormat), minPointFormat);
    } else {
  ...
  auto convertOne = [&](PointRef &point) {
    ...
    for (const LasField &lasField : fieldsToSave) {
      ...
      case LAS_CLASSIFICATION:
        classification = static_cast<uint8_t>(
            static_cast<int>(lasField.getSafeValue(ptsWritten)) & 255);
        break;
      case LAS_CLASSIF_VALUE:
        classification = static_cast<uint8_t>(
            static_cast<int>(lasField.getSafeValue(ptsWritten)) &
    ...
    for (const ExtraLasField::Shared &extraField : extraFieldsToSave) {
      point.setField(extraField->pdalId, extraField->getSafeValue(ptsWritten) +
  ...
  try {
    ...
    if (theCloud->hasMetaData(LAS_GLOBAL_ENCODING_META_DATA)) {
      ...
      unsigned global_encoding =
          theCloud->getMetaData(LAS_GLOBAL_ENCODING_META_DATA).toUInt(&ok);
      if (ok) {
    ...
    if (theCloud->hasMetaData(LAS_PROJECT_UUID_META_DATA)) {
      QString uuid =
          theCloud->getMetaData(LAS_PROJECT_UUID_META_DATA).toString();
    ...
    if (theCloud->hasMetaData(LAS_VERSION_MINOR_META_DATA)) {
      bool ok = false;
      int minor_version =
          theCloud->getMetaData(LAS_VERSION_MINOR_META_DATA).toInt(&ok);
...
public:
  ...
  bool init(unsigned width, unsigned height, unsigned Zdim,
            const QString &absoluteBaseFilename, const CCVector3d &bbMin,
            const CCVector3d &bbMax, PointTableRef table,
    ...
    for (unsigned i = 0; i < width; ++i) {
      for (unsigned j = 0; j < height; ++j) {
        ...
        QString filename =
            absoluteBaseFilename +
            QString("_%1_%2.%3")
                .arg(QString::number(i), QString::number(j), ext);
  ...
  void createFieldsToLoad(const IdList &extraFieldsToLoad,
    ...
    if (s_lasOpenDlg->doLoad(LAS_CLASSIFICATION))
      lasFields.push_back(LasField::Shared(new LasField(
          LAS_CLASSIFICATION, 0, 0, 255))); // unsigned char: between 0 and 255
  ...
  void addLasFieldsToCloud() {
    ...
    while (!lasFields.empty()) {
      ...
      if (field && field->sf) {
        ...
        ccLog::Warning(
            QString(
                "[LAS] All '%1' values were the same (%2)! We ignored them...")
...
CC_FILE_ERROR LASFilter::loadFile(const QString &filename, ccHObject &container,
  ...
  try {
    ...
    if (nullptr != layout) {
      for (const pdal::Dimension::Id pdalId : layout->dims()) {
        ...
        default:
          ...
          QString desanitizedSFName =
              LasField::DesanitizeString(QString::fromStdString(dim.name));
    ...
    if (parameters.sessionStart) {
      ...
      // good that the previous configuration is restored even though the user
      // needs to confirm it
      s_lasOpenDlg->resetApplyAll();
    }

    if (parameters.alwaysDisplayLoadDialog && !s_lasOpenDlg->autoSkipMode() &&
        !s_lasOpenDlg->exec()) {
    ...
    bool loadColor =
        (loadRGBComponent[0] || loadRGBComponent[1] || loadRGBComponent[2]);

    // by default we read colors as triplets of 8 bits integers but we might
    // dynamically change this if we encounter values using 16 bits (16 bits is
    // the standard!)
    ...
    if (!extraNamesToLoad.empty()) {
      // If extra fields are requested, reload the file with the new extra_dims
      // parameters
      Options las_opts2;
    ...
    auto ccProcessOne = [&](PointRef &point) {
      ...
      if (pointChunk.getLoadedCloud() == nullptr) {
        ...
        fileChunkSize =
            std::min(pointsToRead, CC_MAX_NUMBER_OF_POINTS_PER_CLOUD);
      ...
      if (nbPointsRead == 0) {
        CCVector3d P(
            static_cast<PointCoordinateType>(point.getFieldAs<int>(Id::X)),
      ...
      if (loadColor) {
        uint16_t red =
            loadRGBComponent[0] ? point.getFieldAs<uint16_t>(Id::Red) : 0;
        uint16_t green =
            loadRGBComponent[1] ? point.getFieldAs<uint16_t>(Id::Green) : 0;
        uint16_t blue =
            loadRGBComponent[2] ? point.getFieldAs<uint16_t>(Id::Blue) : 0;

        // if we don't have reserved a color field yet, we check that color is
        // not black
        bool pushColor = true;
        if (!loadedCloud->hasColors()) {
          if (red || green || blue) {
            if (loadedCloud->reserveTheRGBTable()) {
              ...
              ccLog::Warning(
                  "[LAS]: Not enough memory, color field will be ignored!");
              loadColor = false; // no need to retry with the other chunks
                                 // anyway
              pushColor = false;
            }
          } else // otherwise we ignore it for the moment (we'll add it later if
                 // necessary)
          {
            pushColor = false;
          }
        ...
        if (pushColor) {
          // we test if the color components are coded on 16 bits (standard) or
          // only on 8 bits (it happens ;)
          if (!forced8bitRgbMode && colorCompBitShift == 0) {
            if ((red & 0xFF00) || (green & 0xFF00) || (blue & 0xFF00)) {
      ...
      for (LasField::Shared &field : fieldsToLoad) {
        ...
        case LAS_CLASSIF_VALUE:
          if (pointFormat <= 5)
            value = (point.getFieldAs<int>(pdalId) &
                     31); // bit #0-4 of the 'Classification' field
          else
            value = point.getFieldAs<int>(pdalId);
          break;
        case LAS_CLASSIF_SYNTHETIC:
          if (pointFormat <= 5)
            value = (point.getFieldAs<int>(pdalId) & 32)
                        ? 1.0
                        : 0.0; // bit #5 of the 'Classification' field
          else
            value = (point.getFieldAs<int>(pdalId) & 1)
                        ? 1.0
                        : 0.0; // bit #0 of the 'Classification Flags' field
          break;
        case LAS_CLASSIF_KEYPOINT:
          if (pointFormat <= 5)
            value = (point.getFieldAs<int>(pdalId) & 64)
                        ? 1.0
                        : 0.0; // bit #6 of the 'Classification' field
          else
            value = (point.getFieldAs<int>(pdalId) & 2)
                        ? 1.0
                        : 0.0; // bit #1 of the 'Classification Flags' field
          break;
        case LAS_CLASSIF_WITHHELD:
          if (pointFormat <= 5)
            value = (point.getFieldAs<int>(pdalId) & 128)
                        ? 1.0
                        : 0.0; // bit #7 of the 'Classification' field
          else
            value = (point.getFieldAs<int>(pdalId) & 4)
                        ? 1.0
                        : 0.0; // bit #2 of the 'Classification Flags' field
          break;
        case LAS_CLASSIF_OVERLAP:
          if (pointFormat <= 5) {
            ...
            value = (point.getFieldAs<int>(pdalId) & 8)
                        ? 1.0
                        : 0.0; // bit #3 of the 'Classification Flags' field
          }
          break;
        default:
        ...
        if (field->sf) {
          ...
          if (!ignoreDefaultFields || value != field->firstValue ||
            ...
            field->sf = new ccScalarField(
                qPrintable(LasField::DesanitizeString(field->getName())));
            if (field->sf->reserveSafe(fileChunkSize)) {
              ...
              if (field->type == LAS_TIME) {
                ...
                if (!s_lasOpenDlg->getTimeShift(timeShift)) {
                  ...
                  // lose accuracy)
                  timeShift =
                      static_cast<int64_t>(field->firstValue / 10000.0) *
                      10000.0;
                }
                ...
                if (value < 1.0e5) {
                  ...
                  ccLog::Warning("[LAS] Time SF has been shifted but accuracy "
                                 "may not be preserved (%.2f)",
                                 timeShift);
                } else {
                  ccLog::Warning("[LAS] Time SF has not been shifted. Accuracy "
    ...
    for (auto &chunk : chunks) {
      ...
      if (loadedCloud) {
        if (loadedCloud->size()) {
          ...
          loadedCloud->setMetaData(LAS_GLOBAL_ENCODING_META_DATA,
          ...
          loadedCloud->setMetaData(LAS_VERSION_MAJOR_META_DATA,
                                   QVariant(lasHeader.versionMajor()));
          loadedCloud->setMetaData(LAS_VERSION_MINOR_META_DATA,

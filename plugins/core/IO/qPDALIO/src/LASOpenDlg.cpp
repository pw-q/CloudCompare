...
bool LASOpenDlg::doLoad(LAS_FIELDS field) const {
  ...
  case LAS_CLASSIFICATION:
    return classifCheckBox->isEnabled() && classifCheckBox->isChecked() &&
...
void LASOpenDlg::setInfos(QString filename, unsigned pointCount,
                          const CCVector3d &bbMin, const CCVector3d &bbMax) {

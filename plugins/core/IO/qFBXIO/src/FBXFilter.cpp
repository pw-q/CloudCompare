...
bool FBXFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
static FbxNode *ToFbxMesh(ccGenericMesh *mesh, FbxScene *pScene,
                          QString filename, size_t meshIndex) {
  if (!mesh)
    return 0;
  ...
  // Create control points.
  {
    ...
    for (unsigned i = 0; i < vertCount; ++i) {
      ...
      // lControlPoints[i] = FbxVector4(P->x,P->z,-P->y); //DGM: see loadFile (Y
      // and Z are inverted)
    }
  }
  ...
  if (mesh->hasNormals()) {
    ...
    if (mesh->hasTriNormals()) {
      ...
      if (asCCMesh) {
        ...
        for (unsigned j = 0; j < faceCount; ++j) {
          // we can't use the 'NormsIndexesTable' so we save all the normals of
          // all the vertices
          CCVector3 Na, Nb, Nc;
          ...
          lGeometryElementNormal->GetIndexArray().SetAt(
              static_cast<int>(j) * 3 + 0, static_cast<int>(j) * 3 + 0);
          lGeometryElementNormal->GetIndexArray().SetAt(
              static_cast<int>(j) * 3 + 1, static_cast<int>(j) * 3 + 1);
          lGeometryElementNormal->GetIndexArray().SetAt(
              static_cast<int>(j) * 3 + 2, static_cast<int>(j) * 3 + 2);
        }
      }
    } else {
    ...
    ccLog::Warning("[FBX] Mesh has no normal! You can manually compute them "
  ...
  if (asCCMesh && asCCMesh->hasMaterials()) {
    ...
    if (hasTextures) {
      ...
      if (texCoords) {
        ...
        for (unsigned i = 0; i < count; ++i) {
          ...
          lUVDiffuseElement->GetDirectArray().SetAt(i,
                                                    FbxVector2(uv.tx, uv.ty));
        }
      }
    ...
    for (size_t i = 0; i < matCount; ++i) {
      ...
      if (hasTextures && mat->hasTexture()) {
        ...
        if (!texFilenames.contains(texFilename)) {
          ...
          if (!texDir.exists()) {
            ...
            if (texDir.mkdir(textDirName)) {
              ...
              textDirName = QString();
              ccLog::Warning(
                  "[FBX] Failed to create subfolder '%1' to store texture "
                  "files (files will be stored next to the .fbx file)");
            }
          }
          ...
          ccLog::PrintDebug(QString("[FBX] Material '%1' texture: %2")
                                .arg(mat->getName())
        ...
        lTexture->UVSet.Set(
            FbxString(gDiffuseElementName)); // Connect texture to the proper UV

        // don't forget to connect the texture to the corresponding property of
        // the material
    ...
    {
      for (QMap<QString, QString>::ConstIterator it = texFilenames.begin();
  ...
  if (cloud->hasColors()) {
    ...
    if (!hasMaterial) {
      // it seems that we have to create a fake material in order for the colors
      // to be displayed (in Unity and FBX Review at least)!
      FbxSurfacePhong *lMaterial =
...
static bool SaveScene(FbxManager *pManager, FbxDocument *pScene,
                      const char *pFilename, int pFileFormat = -1,
  ...
  if (lExporter->Initialize(pFilename, pFileFormat,
    ...
    ccLog::Warning("[FBX] Error returned: %s",
...
CC_FILE_ERROR FBXFilter::saveToFile(ccHObject *entity, const QString &filename,
        ...
        if (child->hasMetaData(FBX_SCALE_METADATA_KEY)) {
          ...
          if (ok) {
            if (scaleFactor == 0.0) {
              ...
              ccLog::Warning("[FBX] Attempt to save mutliple meshes with "
                             "different units!");
            }
          } else {
            ccLog::Warning(
                "[FBX] Internal error: invalid FBX scale meta-data?!");
            scaleFactor = 1.0;
          }
        } else {
  ...
  if (!lSdkManager) {
    ...
    ccLog::Print("[FBX] Autodesk FBX SDK version %s",
  ...
  try {
    ...
    // create scene info
    {
      ...
      sceneInfo->mTitle =
          qPrintable(QString("Mesh: ") + (meshes.size() == 1
                                              ? meshes[0]->getName()
    ...
    FbxNode *lRootNode = lScene->GetRootNode();
    {
      for (size_t i = 0; i < meshes.size(); ++i) {
        ...
        if (meshNode)
          ...
          ccLog::Warning(
              QString("[FBX] Failed to convert mesh '%1' to FBX mesh/node!")
    ...
    // Display a combox box to let the user choose the export file format
    {
      ...
      if (lFormatCount > 0) {
        if (s_defaultOutputFormat.isEmpty()) {
          try {
            ...
            for (int lFormatIndex = 0; lFormatIndex < lFormatCount;
              ...
              if (pSdkManager->GetIOPluginRegistry()->WriterIsFBX(
                ...
                QPushButton *button =
                    msgBox.addButton(lDesc.Buffer(), QMessageBox::AcceptRole);
          ...
          for (int lFormatIndex = 0; lFormatIndex < lFormatCount;
            ...
            if (pSdkManager->GetIOPluginRegistry()->WriterIsFBX(lFormatIndex)) {
              ...
              if (s_defaultOutputFormat == sanitizedDesc) {
                ccLog::Print(QString("[FBX] Default output file format: %1")
                                 .arg(sanitizedDesc));
          ...
          // if we failed to find the specified file format, warn the user and
          // display the list of supported formats
          if (fileFormat < 0) {
            ccLog::Warning(QString("[FBX] File format '%1' not supported")
    ...
    if (CheckForSpecialChars(filename)) {
      ccLog::Warning(
          QString("[FBX] Output filename contains special characters. It might "
                  "be rejected by the third party library..."));
    }

    // Save the scene.
    bool lResult =
        SaveScene(lSdkManager, lScene, qPrintable(filename), fileFormat);
...
static ccMesh *FromFbxMesh(FbxMesh *fbxMesh,
    ...
    if (triCount == 0) {
      ccLog::Warning(
          QString("[FBX] No triangle or quad found in mesh '%1'! (polygons "
                  "with more than 4 vertices are not supported for the moment)")
              .arg(fbxMesh->GetName()));
      return 0;
    } else if (skipped != 0) {
      ccLog::Warning(
          QString(
              "[FBX] Some polygons in mesh '%1' were ignored (%2): polygons "
              "with more than 4 vertices are not supported for the moment)")
              .arg(fbxMesh->GetName())
              .arg(skipped));
      return 0;
  ...
  if (vertCount <= 0) {
    ccLog::Warning(QString("[FBX] Mesh '%1' has no vetex or no polygon?!")
                       .arg(fbxMesh->GetName()));
    return 0;
  }
  ...
  if (!mesh->reserve(static_cast<unsigned>(triCount)) ||
    ...
    ccLog::Warning(QString("[FBX] Not enough memory to load mesh '%1'!")
                       .arg(fbxMesh->GetName()));
    delete mesh;
    return 0;
  }
          ...
          if (vertices->reserveTheRGBTable()) {
            ...
            case FbxGeometryElement::eDirect: {
              for (int i = 0; i < vertCount; ++i) {
                ...
                vertices->addColor(
                    static_cast<ColorCompType>(c.mRed * ccColor::MAX),
            ...
            break; // no need to look for other color fields (we won't be able
                   // to handle them!
          } else {
            ccLog::Warning(
                QString("[FBX] Not enough memory to load mesh '%1' colors!")
                    .arg(fbxMesh->GetName()));
          }
        } else {
          ccLog::Warning(QString("[FBX] Color field #%1 of mesh '%2' will be "
                                 "ignored (unhandled type)")
                             .arg(l)
                             .arg(fbxMesh->GetName()));
        }
      } else {
  ...
  if (perPointNormals >= 0) {
    ...
    if (normals.GetCount() != vertCount) {
      ccLog::Warning(QString("[FBX] Wrong number of normals on mesh '%1'!")
                         .arg(fbxMesh->GetName()));
      perPointNormals = -1;
    } else if (!vertices->reserveTheNormsTable()) {
      ccLog::Warning(
          QString("[FBX] Not enough memory to load mesh '%1' normals!")
              .arg(fbxMesh->GetName()));
      perPointNormals = -1;
    } else {
      // import normals
    ...
    for (int i = 0; i < lMaterialCount; i++) {
      ...
      if (isLambert || isPhong) {
        ...
        FbxSurfacePhong *lPhongMat =
            isPhong ? static_cast<FbxSurfacePhong *>(lBaseMaterial) : 0;
        ...
        // import associated texture (if any)
        {
          ...
          FBXSDK_FOR_EACH_TEXTURE(lTextureIndex) {
            ...
            if (lProperty.IsValid()) {
              ...
              FbxTexture *texture =
                  0; // we can handle only one texture per material! We'll take
                     // the non layered one by default (if any)
              for (int j = 0; j < lTextureCount; ++j) {
                // Here we have to check if it's layeredtextures, or just
                // textures:
                FbxLayeredTexture *lLayeredTexture =
                    lProperty.GetSrcObject<FbxLayeredTexture>(j);
                if (lLayeredTexture) {
                  // we don't handle layered textures!
                  /*int lNbTextures =
                  lLayeredTexture->GetSrcObjectCount<FbxTexture>(); for (int
                  k=0; k<lNbTextures; ++k)
                  {
                  FbxTexture* lTexture =
                  lLayeredTexture->GetSrcObject<FbxTexture>(k); if(lTexture)
                  {
                  }
                  }
                  //*/
        ...
        ccLog::Warning(QString("[FBX] Material '%1' has an unhandled type")
    ...
    for (int l = 0; l < fbxMesh->GetElementUVCount(); ++l) {
      ...
      if (leUV->GetMappingMode() == FbxGeometryElement::eByPolygonVertex) {
        ...
        if (!vertTexUVTable->reserveSafe(uvCount) ||
          ...
          ccLog::Warning(
              QString(
                  "[FBX] Not enough memory to load mesh '%1' UV coordinates!")
                  .arg(fbxMesh->GetName()));
        } else {
          ...
          if (refMode == FbxGeometryElement::eIndexToDirect) {
            ...
            ccLog::Warning(QString("[FBX] UV coordinates for mesh '%1' are "
                                   "encoded in an unhandled mode!")
                               .arg(fbxMesh->GetName()));
            vertTexUVTable->release();
            vertTexUVTable = 0;
          }
  ...
  // import polygons
  {
    ...
    for (int i = 0; i < polyCount; ++i) {
      ...
      if (vertTexUVTable) {
        ...
        if (uvIndex >= static_cast<int>(vertTexUVTable->currentSize())) {
          ccLog::Warning(
              QString("[FBX] Mesh '%1': UV coordinates indexes mismatch!")
                  .arg(fbxMesh->GetName()));
          vertTexUVTable->release();
          vertTexUVTable = 0;
        }
    ...
    if (mesh->size() == 0) {
      ccLog::Warning(QString("[FBX] No triangle found in mesh '%1'! (only "
  ...
  // import vertices
  {
    ...
    for (int i = 0; i < vertCount; ++i, ++fbxVertices) {
      ...
      if (i == 0) {
        ...
        if (FileIOFilter::HandleGlobalShift(P, Pshift, preserveCoordinateShift,
                                            parameters)) {
          if (preserveCoordinateShift) {
            vertices->setGlobalShift(Pshift);
          }
          ccLog::Warning("[FBX] Mesh has been recentered! Translation: (%.2f ; "
                         "%.2f ; %.2f)",
    ...
    if (mesh->reservePerTriangleMtlIndexes()) {
      for (int i = 0; i < polyCount; ++i) {
        ...
        for (int l = 0; l < fbxMatCount; l++) {
          FbxGeometryElementMaterial *lMaterialElement =
              fbxMesh->GetElementMaterial(l);
          FbxSurfaceMaterial *lMaterial = nullptr;
          lMaterial = fbxMesh->GetNode()->GetMaterial(
...
CC_FILE_ERROR FBXFilter::loadFile(const QString &filename, ccHObject &container,
  ...
  try {
    ...
    if (CheckForSpecialChars(filename)) {
      ccLog::Warning(
          QString("[FBX] Input filename contains special characters. It might "
                  "be rejected by the third party library..."));
    }
    ...
    if (!lImporter->Initialize(qPrintable(filename), -1,
      ...
      ccLog::Warning(QString("[FBX] Error: %1")
...
#ifdef QT_DEBUG
          ccLog::Print(QString("Node: %1 - %2 properties")
                           .arg(nodeName)
...
#ifdef QT_DEBUG
            ccLog::Print(
                QString("\tProp. #%1").arg(GetAttributeTypeName(type)));
#endif

            switch (type) {
            case FbxNodeAttribute::eMesh: {
              ccMesh *mesh =
                  FromFbxMesh(static_cast<FbxMesh *>(pAttribute), parameters);
              if (mesh) {
                ...
                if (scaleFactor != 1.0) {
                  // save this info for later (in case the user exports the mesh
                  // as FBX later)

...
// header flag
constexpr quint16 s_headerFlagSBF =
...
bool SimpleBinFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
CC_FILE_ERROR SimpleBinFilter::saveToFile(ccHObject *root,
  ...
  // header
  {
    ...
    dataStream.setFloatingPointPrecision(
        QDataStream::DoublePrecision); // we write real 'double' values in the
                                       // header
  ...
  dataStream.setFloatingPointPrecision(
      QDataStream::SinglePrecision); // we wave only 'float' values in the data
...
CC_FILE_ERROR SimpleBinFilter::loadFile(const QString &filename,
        ...
        if (!tokens.empty()) {
          ...
          for (int k = 1; k < tokens.size(); ++k) {
            ...
            if (token.startsWith("s=")) {
              ...
              if (!ok) {
                ccLog::Error(
                    QString("[SBF] Invalid %1 description (shift/offset)")
                        .arg(key));
                return CC_FERR_MALFORMED_FILE;
              }
              ...
              if (!ok) {
                ccLog::Error(QString("[SBF] Invalid %1 description (precision)")
                                 .arg(key));
                return CC_FERR_MALFORMED_FILE;
              }
            }
  ...
  // header
  {
    size_t readBytes = 0;
    dataStream.setFloatingPointPrecision(
        QDataStream::DoublePrecision); // we expect real 'double' values in the
                                       // header
    ...
    // 8 bytes = point count
    {
      ...
      if (descriptor.pointCount != pointCount) {
        ccLog::Warning("[SBF] Inconsistent number of points between the header "
                       "and the data file!");
      }
    ...
    // 2 bytes = sf count
    {
      ...
      if (sfCount != descriptor.SFs.size()) {
        ccLog::Warning("[SBF] Inconsistent number of scalar fields between the "
  ...
  size_t sizePerPoint =
      (3 + descriptor.SFs.size()) *
      4; // 3 * 4 bytes (float) for coordinates + 4 bytes (float) for scalars
  ...
  dataStream.setFloatingPointPrecision(
      QDataStream::SinglePrecision); // we expect only 'float' values in the
                                     // data
  for (size_t i = 0; i < descriptor.pointCount; ++i) {
    ...
    if (i == 0) {
      ...
      if (HandleGlobalShift(Pd, Pshift, preserveCoordinateShift, parameters,
        ...
        ccLog::Warning("[SBF] Cloud has been recentered! Translation: (%.2f ; "
                       "%.2f ; %.2f)",

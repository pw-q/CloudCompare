...
CC_FILE_ERROR PTXFilter::loadFile(const QString &filename, ccHObject &container,
  ...
  for (unsigned cloudIndex = 0;
       result == CC_FERR_NO_ERROR || result == CC_FERR_NO_LOAD; cloudIndex++) {
    unsigned width = 0;
    unsigned height = 0;
    ...
    // read header
    {
      ...
      // read the width (number of columns) and the height (number of rows) on
      // the two first lines (DGM: we transpose the matrix right away)
      bool ok;
      ...
      ccLog::Print(QString("[PTX] Scan #%1 - grid size: %2 x %3")
                       .arg(cloudIndex + 1)
                       .arg(height)
                       .arg(width));
      ...
      if (cloudIndex == 0) {
        if (HandleGlobalShift(cloudTransD.getTranslationAsVec3D(), PshiftTrans,
                              preserveCoordinateShift, parameters)) {
          ccLog::Warning("[PTXFilter::loadFile] Cloud has be recentered! "
    ...
    if (container.getChildrenNumber() == 0) {
      ...
      cloud->setName(
          QString("unnamed - Cloud %1").arg(container.getChildrenNumber() + 1));
    }
      ...
      for (unsigned j = 0; j < height; ++j) {
        for (unsigned i = 0; i < width; ++i, ++gridIndex) {
          ...
          if (firstPoint) {
            ...
            if (hasColors) {
              ...
              if (!loadColors) {
                ...
                // we also load the colors into the grid (as invalid/missing
                // points can have colors!)
                try {
          ...
          if (pointIsValid) {
            ...
            if (firstPoint) {
              if (cloudIndex == 0 &&
                  !cloud->isShifted()) // in case the trans. matrix was ok!
              {
                CCVector3d P(Pd);
                if (HandleGlobalShift(P, PshiftCloud, preserveCoordinateShift,
                                      parameters)) {
                  if (preserveCoordinateShift) {
                    cloud->setGlobalShift(PshiftCloud);
                  }
                  ccLog::Warning(
                      "[PTXFilter::loadFile] Cloud has been recentered! "
                      "Translation: (%.2f ; %.2f ; %.2f)",
                      PshiftCloud.x, PshiftCloud.y, PshiftCloud.z);
                }
              }
              firstPoint = false;
            }
            ...
            if (hasIndexGrid) {
              grid->indexes[gridIndex] = static_cast<int>(
                  cloud
                      ->size()); // = index (default value = -1, means no point)
            }

            // add point
            cloud->addPoint(CCVector3(
                static_cast<PointCoordinateType>(Pd[0] + PshiftCloud.x),
    ...
    if (cloud->size() == 0) {
      ...
      if (result == CC_FERR_NO_LOAD) {
        result = CC_FERR_NO_ERROR; // to make clear that we have loaded at least
                                   // something!
      }
      ...
      if (hasIndexGrid && result != CC_FERR_CANCELED_BY_USER) {
        ...
        sensor =
            ccGriddedTools::ComputeBestSensor(cloud, grid, &cloudToSensorTrans);
      }
      ...
      if (sensor) {
        ...
        sensor->setRigidTransformation(
            sensorTrans); // after cloud->applyGLTransformation_recursive!
  ...
  // update scalar fields saturation (globally!)
  {
    ...
    if (minIntensity < 0 || maxIntensity > 1.0) {
      ccLog::Warning("[PTX] Intensity values are invalid (they should all fall "
                     "in [0 ; 1])");
    ...
    for (unsigned i = 0; i < container.getChildrenNumber(); ++i) {
      ...
      if (sf) {
        ...
        ccSF->setColorScale(ccColorScalesManager::GetDefaultScale(
            validIntensityRange ? ccColorScalesManager::ABS_NORM_GREY

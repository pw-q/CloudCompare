...
static void
ReleaseEdgeList(edge **&theEdges, unsigned numberOfVertexes,
...
bool MAFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
CC_FILE_ERROR MAFilter::saveToFile(ccHObject *entity, const QString &filename,
  ...
  int hard = 0; // Maya edges cab be "hard" or "soft" ...
  {
    ...
    for (unsigned i = 0; i < numberOfTriangles; ++i) {
      const CCCoreLib::VerticesIndexes *tsi =
          theMesh
              ->getNextTriangleVertIndexes(); // DGM: getNextTriangleVertIndexes
      ...
      for (unsigned char k = 0; k < 3; ++k) {
        ...
        if (currentEdgeIndex < 0) // create a new edge
        {
          ...
          // newEdge->edgeIndex = ++lastEdgeIndexPushed; //don't write the edge
          // right now
          newEdge->edgeIndex = 0;
          ++lastEdgeIndexPushed;
  ...
  theMesh->placeIteratorAtBeginning();
  {
    for (unsigned i = 0; i < numberOfTriangles; ++i) {
      ...
      for (unsigned char k = 0; k < 3; ++k) {
        ...
        if (fprintf(fp, " %i",
                    ((e->positif && a == ind[k]) || (!e->positif && a == ind[l])
                         ? e->edgeIndex
                         : -(e->edgeIndex + 1))) < 0) {
          fclose(fp);
          ReleaseEdgeList(theEdges, numberOfVertexes);
  ...
  // bonus track
  if (fprintf(fp, "\tsetAttr \".cd\" -type \"dataPolyComponent\" Index_Data "
                  "Edge 0 ;\n") < 0 ||
  ...
  if (hasColors) {
    ...
    if (fprintf(fp,
                "createNode polyColorPerVertex -n \"polyColorPerVertex%i\";\n",
                currentMesh + 1) < 0) {
      fclose(fp);
      return CC_FERR_WRITING;
    }
      ...
      for (unsigned i = 0; i < numberOfVertexes; ++i) {
        ...
        if (nf > 0) {
          if (fprintf(fp, "\tsetAttr -s %i \".vclr[%u].vfcl\";\n", nf, i) < 0) {
            fclose(fp);
            delete[] theFacesIndexes; // DGM: we are missing some faces here,
                                      // aren't we?
            return CC_FERR_WRITING;
          }
          ...
          while (f) {
            if (fprintf(fp,
                        "\tsetAttr \".vclr[%u].vfcl[%i].frgb\" -type "
                        "\"float3\" %f %f %f;\n",
  ...
  if (hasColors) {
    if (fprintf(fp,
                "connectAttr \"polyColorPerVertex%i.out\" \"MeshShape%i.i\";\n",
                currentMesh + 1, currentMesh + 1) < 0 ||
        fprintf(fp,
  ...
  if (fprintf(
          fp,
          "connectAttr \"MeshShape%i.iog\" \":initialShadingGroup.dsm\" -na;\n",
          currentMesh + 1) < 0) {
    fclose(fp);
    return CC_FERR_WRITING;
  }

...
PdmsLexer::PdmsLexer()
    : loadedObject(nullptr), currentToken(PDMS_INVALID_TOKEN), stop(false),
      metaGroupMask(0) {
  tokenBuffer[0] = 0;
  nextBuffer[0] = 0;
}
...
PdmsFileSession::PdmsFileSession(const QString &filename)
    : m_filename(filename), m_currentLine(-1), m_eol(false), m_eof(false) {}
...
void PdmsFileSession::skipComment() {
  ...
  case PDMS_COMMENT_LINE:
    ...
    break;
  case PDMS_COMMENT_BLOCK: {
    // comment block opening symbol has been met. Search for comment block
    // ending symbol don't forget that some other comments could be embedded in
    // this comment
...
void PdmsFileSession::printWarning(const char *str) {
  if (currentToken == PDMS_EOS)
    std::cerr << "[" << qPrintable(m_filename) << "]@postprocessing : " << str
              << std::endl;
  else
    std::cerr << "[" << qPrintable(m_filename) << "]@[line " << m_currentLine
...
PdmsParser::PdmsParser()
    : session(nullptr), currentCommand(nullptr), currentItem(nullptr),
      root(nullptr) {}
...
bool PdmsParser::processCurrentToken() {
  ...
  default:
    ...
    if (currentCommand) {
      ...
      // Else, the token must be a new command. We execute the active command
      // and delete it
      ...
      if (!success) {
        assert(false);
        session->printWarning("Unable to resolve previous command (this token "
      ...
      if (item) {
        ...
        if (!root) {
          ...
          session->printWarning(
              "Trying to create a second root for elements hierarchy");
          return false;
        }
      }
    }
    if (currentToken == PDMS_RETURN) {
      ...
      // Here, we must create a new command from the token speciffied by the
      // lexer
      currentCommand = PdmsCommands::Command::Create(currentToken);
      if (!currentCommand) {
        if (currentToken == PDMS_INVALID_TOKEN ||
            currentToken == PDMS_UNKNOWN) {

...
bool Reference::execute(PdmsObjects::GenericItem *&item) const {
  ...
  // Request for an element (hierarchical or design element only)
    ...
    if (PdmsToken::isGroupElement(token)) {
      // Go up in the hierarchy to find a matching, or the first group which can
      // own the request item
      result = item;
  ...
  // If the reference command is PDMS_OWNER, then we have to change the request
  // item owner
...
bool Orientation::axisFromCoords(const Coordinates &coords, CCVector3 &u) {
  ...
  if (coords.getNbComponents(true) == 2) {
    PointCoordinateType alpha =
        static_cast<PointCoordinateType>(CCCoreLib::DegreesToRadians(u[0]));
    PointCoordinateType beta =
        static_cast<PointCoordinateType>(CCCoreLib::DegreesToRadians(u[1]));
...
bool ElementEnding::execute(PdmsObjects::GenericItem *&item) const {
  ...
  switch (command) {
  case PDMS_OWNER:
    // If the ending command is PDMS_OWNER, then we simply go back to the item
    // owner
  ...
  case PDMS_END:
    ...
    if (end.isTokenReference() &&
      ...
      return true;
    // In the general case, we have to find the references item (default : this
    // one), and go back to its creator
    result = item;
...
bool HierarchyNavigation::execute(PdmsObjects::GenericItem *&item) const {
  ...
  // Go back to the first creator object that matches or can contain the command
  // hierarchy level
  while (result && command < result->getType())
    result = result->creator;

  // If we went to the root, we have to create a new hierarchy level and set it
  // as the new root
  if (!result) {
    try {
...
GenericItem::GenericItem()
    : owner(nullptr), creator(nullptr), position(0, 0, 0),
      isCoordinateSystemUpToDate(false), positionReference(nullptr) {
...
bool GenericItem::setOrientation(const CCVector3 &x, const CCVector3 &y,
                                 const CCVector3 &z) {
  orientation[0] = x;
  orientation[1] = y;
  orientation[2] = z;
  ...
  for (unsigned k = 0; k < 3; k++) {
    ...
    if (orientationReferences[k]) {
      ...
      }
      {
        for (unsigned j = 0; j < 3; j++)
          for (unsigned i = 0; i < 3; i++)
            orientation[j][i] = ref->orientation[0][i] * axis[j][0] +
...
bool GroupElement::push(GenericItem *i) {
  ...
  if (PdmsToken::isGroupElement(i->getType())) {
    // If this group can contain the request item, then insert it in the group
    // list
...
PointCoordinateType CTorus::surface() const {
  PointCoordinateType r =
      static_cast<PointCoordinateType>(0.5) * (outside_radius - inside_radius);
...
PointCoordinateType RTorus::surface() const {
  PointCoordinateType inside =
      static_cast<PointCoordinateType>(2.0 * M_PI) * inside_radius * height;
  PointCoordinateType outside =
      static_cast<PointCoordinateType>(2.0 * M_PI) * outside_radius * height;
  ...
  if (CCCoreLib::GreaterThanEpsilon(radius)) {
    ...
    if (2 * height > diameter) {
      ...
      return static_cast<PointCoordinateType>(
          M_PI * (PDMS_SQR(r) + (a * r * height / sin(a))));
    } else {

...
VTKFilter::VTKFilter()
    : FileIOFilter(
          {"_VTK Filter",
           9.0f, // priority
           QStringList{"vtk"}, "vtk", QStringList{"VTK cloud or mesh (*.vtk)"},
           QStringList{"VTK cloud or mesh (*.vtk)"}, Import | Export}) {}

bool VTKFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
CC_FILE_ERROR VTKFilter::saveToFile(ccHObject *entity, const QString &filename,
  ...
  // write the points
  {
    ...
    for (unsigned i = 0; i < ptsCount; ++i) {
      ...
      outFile << Pglobal.x << " " << Pglobal.y << " " << Pglobal.z << endl;
    }
  }
  ...
  if (vertices->hasColors()) {
    ...
    for (unsigned i = 0; i < ptsCount; ++i) {
      ...
      outFile << static_cast<float>(C.r) / ccColor::MAX << " "
              << static_cast<float>(C.g) / ccColor::MAX << " "
              << static_cast<float>(C.b) / ccColor::MAX << endl;
    }
  }
  ...
  if (vertices->isA(CC_TYPES::POINT_CLOUD)) {
    ...
    for (unsigned i = 0; i < sfCount; ++i) {
      ...
      outFile << "POINT_DATA " << ptsCount << endl;
      outFile << "SCALARS "
              << QString::fromStdString(sf->getName()).replace(" ", "_")
  ...
  } else // virtual point cloud, we only have access to its currently displayed
         // scalar field
  {
...
CC_FILE_ERROR VTKFilter::loadFile(const QString &filename, ccHObject &container,
  ...
  std::vector<int> indexes; // global so as to avoid unnecessary mem.
                            // allocations
  QString lastSfName;
  ...
  while (error == CC_FERR_NO_ERROR) {
    ...
    if (nextline.startsWith("POINTS")) {
      ...
      //{
      //	ccLog::Error(QString("Non floating point data (%1) is not
      //supported!").arg(dataFormat)); 	error = CC_FERR_WRONG_FILE_TYPE; 	break;
      //}
      ...
      while (iPt < ptsCount) {
        ...
        for (int i = 0; i < parts.size(); ++i) {
          ...
          if (!ok) {
            ccLog::Warning("[VTK] Element #%1 of POINTS data is corrupted!",
                           iPt);
            error = CC_FERR_MALFORMED_FILE;
            iPt = ptsCount;
            break;
          ...
          if (coordIndex == 2) {
            ...
            if (iPt == 0) {
              ...
              if (HandleGlobalShift(Pd, Pshift, preserveCoordinateShift,
                ...
                ccLog::Warning("[VTKFilter::loadFile] Cloud has been "
      ...
      assert(mesh);
      if (!mesh) {
        ccLog::Warning(QString("[VTK] We found %1 data while file is not "
                               "composed of POLYDATA!")
                           .arg(typeName));
      ...
      for (unsigned i = 0; i < elemCount; ++i) {
        ...
        if (!ok || static_cast<int>(vertCount) >= parts.size()) {
          ...
          ccLog::Warning(QString("[VTK] Element #%1 of %2 data is corrupted! "
                                 "(not enough indexes)")
                             .arg(i)
                             .arg(typeName));
        }

        if (isPolygon && (vertCount != 3 &&
                          vertCount != 4)) // quads are easy to handle as well!
        {
          ccLog::Warning(QString("[VTK] POLYGON element #%1 has an unhandled "
                                 "size (> 4 vertices)")
                             .arg(i));
          continue;
        }
        ...
        // add the triangles
        {
          ...
          if (mesh->size() + triCount > mesh->capacity()) {
            if (!mesh->reserve(
                    mesh->size() + triCount +
                    256)) // take some advance to avoid too many allocations
            {
              error = CC_FERR_NOT_ENOUGH_MEMORY;
              break;
            }
      ...
      while (iNorm < lastDataSize) {
        ...
        for (int i = 0; i < parts.size(); ++i) {
          bool ok;
          N.u[coordIndex] =
              static_cast<PointCoordinateType>(parts[i].toDouble(&ok));
          if (!ok) {
            ccLog::Warning("[VTK] Element #%1 of NORMALS data is corrupted!",
                           iNorm);
            error = CC_FERR_MALFORMED_FILE;
            iNorm = lastDataSize;
            break;
      ...
      while (iCol < lastDataSize) {
        ...
        for (int i = 0; i < parts.size(); ++i) {
          bool ok;
          rgb.rgb[coordIndex] =
              static_cast<ColorCompType>(parts[i].toDouble(&ok) * ccColor::MAX);
          if (!ok) {
            ccLog::Warning(
                "[VTK] Element #%1 of COLOR_SCALARS data is corrupted!", iCol);
            error = CC_FERR_MALFORMED_FILE;
            iCol = lastDataSize;
            break;
          }
      ...
      assert(!acceptLookupTables || expected); // i.e. lastDataSize shouldn't be
                                               // 0 for 'accepted' lookup tables
      ...
      if (acceptLookupTables && !createSF) {
        ccLog::Warning(QString("[VTK] field %1 has not the right number of "
                               "points (will be ignored)")
                           .arg(itemName));
      }
      createSF &= (acceptLookupTables || expected);
      if (createSF && lastSfName.isNull()) {
        ccLog::Warning(QString("[VTK] field %1 has no name (will be ignored)")
                           .arg(itemName));
        createSF = false;
      } else if (!expected) {
        ccLog::Warning(
            QString("[VTK] field %1 was not expected (will be ignored)")
                .arg(itemName));
      }
      ...
      if (createSF) {
        ...
        if (!sf->reserveSafe(lastDataSize)) {
          ccLog::Warning(QString("[VTK] Not enough memory to load scalar "
      ...
      while (iScal < lastDataSize) {
        ...
        if (expected) {
          for (int i = 0; i < parts.size(); ++i) {
            ...
            if (!ok) {
              ccLog::Warning("[VTK] Element #%1 of LOOKUP_TABLE/VECTORS data "
                             "is corrupted!",
                             iScal);
              error = CC_FERR_MALFORMED_FILE;
              if (sf) {
          ...
          // hard to guess the right format, but an unexpected field seem to
          // always be organized as 'one element per line'
          ++iScal;
        }
      }
      // lastDataSize = 0; //lastDataSize is "consumed"
    ...
    } else // unhandled property (CELLS, CELL_TYPES, etc.)
    {
      ...
      if (nextline.startsWith("CELL_DATA")) {
        ...
        if (nextline.startsWith("SCALARS") || nextline.startsWith("NORMALS") ||
  ...
  if (mesh) {
    ...
    // vertices->setLocked(false); //DGM: no need to lock it as it is only used
    // by one mesh!

    // DGM: normals can be per-vertex or per-triangle so it's better to let the
    // user do it himself later Moreover it's not always good idea if the user
    // doesn't want normals (especially in ccViewer!)
    if (!mesh->hasNormals()) {
      //	mesh->computeNormals();
      ccLog::Warning("[VTK] Mesh has no normal! You can manually compute them "

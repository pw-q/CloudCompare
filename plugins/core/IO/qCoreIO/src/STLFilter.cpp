...
bool STLFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
CC_FILE_ERROR STLFilter::saveToFile(ccHObject *entity, const QString &filename,
  ...
  if (!mesh || mesh->size() == 0) {
    ccLog::Warning(
        QString("[STL] No facet in mesh '%1'!")
...
CC_FILE_ERROR STLFilter::saveToBINFile(ccGenericMesh *mesh, QFile &theFile,
  ...
  // header
  {
    char header[80];
    memset(header, 0, 80);
    strcpy(header, "Binary STL file generated by zoomlion!");
  ...
  if (vertices->isShifted()) {
    ccLog::Warning("[STL] Global shift information can't be restored in STL "
  ...
  for (unsigned i = 0; i < faceCount; ++i) {
    ...
    CCVector3f buffer = N.toFloat(); // convert to an explicit float array (as
    ...
    buffer = A->toFloat(); // convert to an explicit float array (as
                           // PointCoordinateType may be a double!)
    if (theFile.write((const char *)buffer.u, 12) < 12)
      return CC_FERR_WRITING;
    buffer = B->toFloat(); // convert to an explicit float array (as
                           // PointCoordinateType may be a double!)
    if (theFile.write((const char *)buffer.u, 12) < 12)
      return CC_FERR_WRITING;
    buffer = C->toFloat(); // convert to an explicit float array (as
                           // PointCoordinateType may be a double!)
    if (theFile.write((const char *)buffer.u, 12) < 12)
      return CC_FERR_WRITING;
...
CC_FILE_ERROR STLFilter::saveToASCIIFile(ccGenericMesh *mesh, QFile &theFile,
                                         QWidget *parentWidget /*=nullptr*/) {
  assert(theFile.isOpen() && mesh && mesh->size() != 0);
  unsigned faceCount = mesh->size();
  ...
  for (unsigned i = 0; i < faceCount; ++i) {
    ...
    stream << "vertex " << Aglobal.x << ' ' << Aglobal.y << ' ' << Aglobal.z
           << endl;
    CCVector3d Bglobal = vertices->toGlobal3d<PointCoordinateType>(*B);
    stream << "vertex " << Bglobal.x << ' ' << Bglobal.y << ' ' << Bglobal.z
           << endl;
    CCVector3d Cglobal = vertices->toGlobal3d<PointCoordinateType>(*C);
    stream << "vertex " << Cglobal.x << ' ' << Cglobal.y << ' ' << Cglobal.z
           << endl;
    stream << "endloop" << endl;
...
CC_FILE_ERROR STLFilter::loadFile(const QString &filename, ccHObject &container,
  ...
  if (mesh->size() != 0 &&
      meshVertices) // their might not remain anymore triangle after
                    // 'mergeDuplicatedVertices'
  {
    ...
    if (normals) {
      ...
      // user doesn't want normals (especially in ccViewer!) if
      // (mesh->computeNormals()) 	mesh->showNormals(true); else
      //	ccLog::Warning("[STL] Failed to compute per-vertex normals...");
      ccLog::Warning("[STL] Mesh has no normal! You can manually compute them "
                     "(select it then call \"Edit > Normals > Compute\")");
    }

    meshVertices->setEnabled(false);
    // meshVertices->setLocked(true); //DGM: no need to lock it as it is only
    // used by one mesh!
...
CC_FILE_ERROR STLFilter::loadASCIIFile(QFile &fp, ccMesh *mesh,
  ...
  while (true) {
    ...
    // 1st line of a 'facet': "facet normal ni nj nk" / or 'endsolid' (i.e. end
    // of file)
    {
      QString currentLine = stream.readLine();
      ...
      if (tokens.empty() || tokens[0].toUpper() != "FACET") {
        if (tokens[0].toUpper() != "ENDSOLID") {
          ccLog::Warning(
              "[STL] Error on line #%i: line should start by 'facet'!",
              lineCount);
          return CC_FERR_MALFORMED_FILE;
        }
        break;
      ...
      if (normals && tokens.size() >= 5) {
        ...
        if (tokens[1].toUpper() == "NORMAL") {
          N.x =
              static_cast<PointCoordinateType>(tokens[2].toDouble(&normalIsOk));
          if (normalIsOk) {
          ...
          if (!normalIsOk && !normalWarningAlreadyDisplayed) {
            ccLog::Warning(
                "[STL] Error on line #%i: failed to read 'normal' values!",
                lineCount);
            normalWarningAlreadyDisplayed = true;
          }
        } else if (!normalWarningAlreadyDisplayed) {
          ccLog::Warning(
              "[STL] Error on line #%i: expecting 'normal' after 'facet'!",
              lineCount);
          normalWarningAlreadyDisplayed = true;
        }
    ...
    // 2nd line: 'outer loop'
    {
      ...
      if (currentLine.isEmpty() || fp.error() != QFile::NoError ||
        ...
        ccLog::Warning("[STL] Error: expecting 'outer loop' on line #%i",
                       lineCount + 1);
        result = CC_FERR_READING;
        break;
      }
    ...
    for (unsigned i = 0; i < 3; ++i) {
      ...
      if (currentLine.isEmpty() || fp.error() != QFile::NoError ||
        ...
        ccLog::Warning(
            "[STL] Error: expecting a line starting by 'vertex' on line #%i",
            lineCount + 1);
      ...
      QStringList tokens = QString(currentLine)
                               .simplified()
                               .split(QChar(' '), QString::SkipEmptyParts);
      if (tokens.size() < 4) {
        ccLog::Warning(
            "[STL] Error on line #%i: incomplete 'vertex' description!",
            lineCount);
        result = CC_FERR_MALFORMED_FILE;
        break;
      }
      ...
      CCVector3d Pd(0, 0, 0);
      {
        ...
        if (!vertexIsOk) {
          ccLog::Warning(
              "[STL] Error on line #%i: failed to read 'vertex' coordinates!",
              lineCount);
          result = CC_FERR_MALFORMED_FILE;
          break;
        }
      ...
      if (pointCount == 0) {
        ...
        if (HandleGlobalShift(Pd, Pshift, preserveCoordinateShift,
          ...
          ccLog::Warning("[STLFilter::loadFile] Cloud has been recentered! "
    ...
    // let's add a new triangle
    {
      ...
      if (mesh->capacity() == faceCount) {
        ...
        if (normals) {
          ...
          if (success && faceCount == 0) // specific case: allocate per triangle
    ...
    // 6th line: 'endloop'
    {
      ...
      if (currentLine.isEmpty() || fp.error() != QFile::NoError ||
        ...
        ccLog::Warning("[STL] Error: expecting 'endnloop' on line #%i",
                       lineCount + 1);
        result = CC_FERR_MALFORMED_FILE;
        break;
      }
    ...
    // 7th and last line: 'endfacet'
    {
      ...
      if (currentLine.isEmpty() || fp.error() != QFile::NoError ||
        ...
        ccLog::Warning("[STL] Error: expecting 'endfacet' on line #%i",
                       lineCount + 1);
        result = CC_FERR_MALFORMED_FILE;
        break;
      }
...
CC_FILE_ERROR STLFilter::loadBinaryFile(QFile &fp, ccMesh *mesh,
                                        ccPointCloud *vertices,
                                        LoadParameters &parameters) {
  assert(fp.isOpen() && mesh && vertices);

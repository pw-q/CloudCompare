...
ObjFilter::ObjFilter()
    : FileIOFilter(
          {"_OBJ Filter",
           8.0f, // priority
           QStringList{"obj"}, "obj", QStringList{"OBJ cloud or mesh (*.obj)"},
           QStringList{"OBJ cloud or mesh (*.obj)"}, Import | Export}) {}

bool ObjFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
CC_FILE_ERROR ObjFilter::saveToFile(ccHObject *entity, const QString &filename,
  ...
  if (!entity->isKindOf(CC_TYPES::MESH) &&
    ...
    ccLog::Warning("[OBJ] This filter can only save one mesh (optionally with "
                   "sub-meshes) or one cloud at a time!");
  ...
  if (parameters.parentWidget) {
    ...
    if (mesh) {
      ...
      assert(vertices);
      pDlg->setMethodTitle(
          QObject::tr("Saving cloud [%1]").arg(vertices->getName()));
    }
  ...
  stream.setRealNumberPrecision(
      sizeof(PointCoordinateType) == 4 && !vertices->isShifted() ? 8 : 12);
  ...
  for (unsigned i = 0; i < nbPoints; ++i) {
    ...
    if (file.error() != QFile::NoError)
      return CC_FERR_WRITING;
    if (pDlg &&
        !nprogress.oneStep()) // update progress bar, check cancel requested
  ...
  if (withNormals) {
    ...
    if (withTriNormals) {
      ...
      if (pDlg)
        pDlg->setInfo(
            QObject::tr("Writing %1 triangle normals").arg(numTriangleNormals));
  ...
  if (mesh) {
    ...
    if (withTexCoordinates) {
      ...
      if (texCoords) {
        ...
        if (pDlg) {
          pDlg->setInfo(
              QObject::tr("Writing %1 texture coordinates").arg(numTexCoords));
        }
    ...
    // check that the number of facets is the same as the full mesh!
    {
      ...
      // if there's no face (i.e. no sub-mesh) or less face than the total mesh,
      // we save the full mesh!
...
CC_FILE_ERROR ObjFilter::loadFile(const QString &filename, ccHObject &container,
  ...
  try {
    ...
    while (!currentLine.isNull()) {
      ...
      if (tokens.front() == "v") {
        ...
        if (pointsRead == 0) {
          ...
          if (HandleGlobalShift(Pd, Pshift, preserveCoordinateShift,
            ...
            ccLog::Warning("[OBJ] Cloud has been recentered! Translation: "
                           "(%.2f ; %.2f ; %.2f)",
               ...
               "vn") //--> in fact it can also be a facet normal!!!
      {
        ...
        normals->addElement(nIndex); // we don't know yet if it's per-vertex or
                                     // per-triangle normal...
        ++normsRead;
      }
      /*** new group ***/
        ...
        // get the group name
        QString groupName =
            (tokens.size() > 1 && !tokens[1].isEmpty() ? tokens[1] : "default");
        ...
        if (!groups.empty() && groups.back().first == totalFacesRead)
          groups.back().second = groupName; // simply replace the group name if
                                            // the previous group was empty!
        else
      ...
      /*** new face ***/
        ...
        if (currentFace.size() < 3) {
          ccLog::Warning("[OBJ] Malformed file: polygon on line %1 has less "
                         "than 3 vertices!",
                         lineCount);
          error = true;
          break;
        ...
        if (shouldTesselate) {
          for (const facetElement &fe : currentFace) {
            if (fe.vIndex < 0 ||
                vertices->size() <= static_cast<unsigned>(fe.vIndex)) {
              // we haven't loaded all the vertices?! Too bad, we can't
              // tesselate properly :(
        ...
        if (shouldTesselate) {
          try {
            ...
            CCCoreLib::Delaunay2dMesh *dMesh =
                CCCoreLib::Delaunay2dMesh::TesselateContour(&contour);
            if (dMesh) {
              // need more space?
              unsigned triCount = dMesh->size();
              if (baseMesh->size() + triCount >= baseMesh->capacity()) {
                if (!baseMesh->reserve(baseMesh->size() +
              ...
              bool flip = false;
              {
                for (unsigned i = 0; i < triCount; ++i, _triIndexes += 3) {
                  ...
                  // should be in the same 'direction' of the triangle that uses
                  // it
                  if ((i1 == 0 || i2 == 0 || i3 == 0) &&
                      (i1 == 1 || i2 == 1 || i3 == 1)) {
                    if ((i1 == 1 && i2 == 0) || (i2 == 1 && i3 == 0) ||
                        (i3 == 1 && i1 == 0)) {
                      flip = true;
                    }
                    break;
              ...
              for (unsigned i = 0; i < triCount; ++i, _triIndexes += 3) {
                ...
                if (hasTexCoords)
                  baseMesh->addTriangleTexCoordIndexes(f1.tcIndex, f2.tcIndex,
                                                       f3.tcIndex);

                if (normalsPerFacet)
                  baseMesh->addTriangleNormalIndexes(f1.nIndex, f2.nIndex,
                                                     f3.nIndex);
        ...
        if (!shouldTesselate) {
          ...
          for (; C != currentFace.end(); ++B, ++C) {
            ...
            if (hasTexCoords)
              baseMesh->addTriangleTexCoordIndexes(A->tcIndex, B->tcIndex,
                                                   C->tcIndex);

            if (normalsPerFacet)
              baseMesh->addTriangleNormalIndexes(A->nIndex, B->nIndex,
                                                 C->nIndex);
          }
        }
      }
      /*** polyline ***/
        ...
        for (int i = 1; i < tokens.size(); ++i) {
          ...
          if (vertexTokens.empty() || vertexTokens[0].isEmpty()) {
            ...
            int index =
                vertexTokens[0].toInt(); // we ignore normal index (if any!)
        ...
        QString name = groups.empty() ? QString("Line")
                                      : groups.back().second + QString(".line");
      ...
      else if (tokens.front() == "usemtl") // see 'MTL file' below
      {
        if (materials) // otherwise we have failed to load MTL file!!!
        {
          ...
          // DGM: in case there's space characters in the material name, we must
          // read it again from the original line buffer QString mtlName =
          // (tokens.size() > 1 && !tokens[1].isEmpty() ? tokens[1] : "");
          currentMaterial =
              (!mtlName.isEmpty() ? materials->findMaterialByName(mtlName)
                                  : -1);
      ...
      /*** material file (MTL) ***/
        ...
        if (tokens.size() < 2 || tokens[1].isEmpty()) {
          ...
          // DGM: in case there's space characters in the filename, we must read
          // it again from the original line buffer QString mtlFilename =
          // tokens[1];
          ...
          if (ccMaterialSet::ParseMTL(mtlPath, mtlFilename, *materials,
                                      errors)) {
            ccLog::Print("[OBJ] %i materials loaded",
            ...
            ccLog::Error(
                QString(
                    ...
                    .arg(mtlPath + '/' + QString(mtlFilename)));
  ...
  if (!error) {
    ...
    if (texCoordsRead > 0 || normsRead > 0)
      ccLog::Print("[OBJ] %i tex. coords, %i normals", texCoordsRead,
                   normsRead);
    ...
    if (maxVertexIndex >= pointsRead || maxTexCoordIndex >= texCoordsRead ||
      ...
      ccLog::Warning("[OBJ] Malformed file: indexes go higher than the number "
    ...
    if (!error && baseMesh) {
      ...
      if (texCoords) {
        if (materials) {
          ...
          ccLog::Warning("[OBJ] Texture coordinates were defined but no "
      ...
      if (!normals) {
        ccLog::Warning(
            "[OBJ] Mesh has no normals! You can manually compute them (select "
            "it then call \"Edit > Normals > Compute\")");
      }
      ...
      if (groups.size() > 1) {
        for (size_t i = 0; i < groups.size(); ++i) {
          ...
          unsigned endIndex =
              (i + 1 == groups.size() ? baseMesh->size() : groups[i + 1].first);
    ...
    if (normals && !normalsPerFacet) {
      if (normsRead == pointsRead) // must be 'per-vertex' normals
      {
        ...
        ccLog::Warning("File contains normals which seem to be neither "
  ...
  if (objWarnings[INVALID_NORMALS])
    ccLog::Warning(
        "[OBJ] Some normals in file were invalid. You should re-compute them "
        "(select entity, then \"Edit > Normals > Compute\")");

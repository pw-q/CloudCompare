...
bool OFFFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
CC_FILE_ERROR OFFFilter::saveToFile(ccHObject *entity, const QString &filename,
...
CC_FILE_ERROR OFFFilter::loadFile(const QString &filename, ccHObject &container,
    ...
    for (unsigned i = 0; i < vertCount; ++i) {
      ...
      if (i == 0) {
        ...
        if (HandleGlobalShift(Pd, Pshift, preserveCoordinateShift,
          ...
          ccLog::Warning("[OFF] Cloud has been recentered! Translation: (%.2f "
                         "; %.2f ; %.2f)",
  ...
  // load triangles
  {
    ...
    for (unsigned i = 0; i < triCount; ++i) {
      ...
      if (polyVertCount == 3 || polyVertCount == 4) {
        ...
        if (mesh->size() + polyTriCount > mesh->capacity()) {
          if (!mesh->reserve(
                  mesh->size() + polyTriCount +
                  256)) // use some margin to avoid too many allocations
          {
            delete mesh;
            return CC_FERR_NOT_ENOUGH_MEMORY;
          }
    ...
    if (ignoredPolygons) {
      ccLog::Warning("[OFF] Some polygons with an unhandled size (i.e. > 4) "
                     "were ignored!");
    }
  }
  ...
  if (mesh->size() == 0) {
    ...
    // doesn't want normals (especially in ccViewer!) if (mesh->computeNormals())
    //	mesh->showNormals(true);
    // else
    //	ccLog::Warning("[OFF] Failed to compute per-vertex normals...");
    ccLog::Warning("[OFF] Mesh has no normal! You can manually compute them "
                   "(select it then call \"Edit > Normals > Compute\")");

    vertices->setEnabled(false);
    // vertices->setLocked(true); //DGM: no need to lock it as it is only used
    // by one mesh!

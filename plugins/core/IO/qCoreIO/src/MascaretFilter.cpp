...
bool MascaretFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
inline void ToLocalAbscissa(const CCVector3 &P, const CCVector3 &C,
                            const CCVector3 &U, unsigned char upDir,
                            CCVector2 &localP) {
...
CC_FILE_ERROR MascaretFilter::saveToFile(ccHObject *entity,
  ...
  try {
    ...
    if (entity->isA(CC_TYPES::POLY_LINE)) {
      ...
      for (unsigned i = 0; i < entity->getChildrenNumber(); ++i)
        if (entity->getChild(i) &&
            entity->getChild(i)->isA(CC_TYPES::POLY_LINE))
    ...
    for (size_t i = 0; i < candidates.size(); ++i) {
      ...
      if (!poly->hasMetaData(ccPolyline::MetaKeyUpDir()) ||
        ...
        ccLog::Warning(QString("[Mascaret] Polyline '%1' is not a valid "
                               "profile (missing meta-data)")
                           .arg(poly->getName()));
        break;
      } else {
  ...
  if (profiles.size() > 1) {
    for (size_t i = 0; i < profiles.size() - 1; ++i) {
      ...
      double smallestAbscissa =
          profiles[i]->getMetaData(ccPolyline::MetaKeyAbscissa()).toDouble();
      for (size_t j = i + 1; j < profiles.size(); ++j) {
        double a =
            profiles[j]->getMetaData(ccPolyline::MetaKeyAbscissa()).toDouble();
  ...
  for (size_t i = 0; i < profiles.size(); ++i) {
    ...
    if (vertCount < 2) {
      // invalid size
      ccLog::Warning(
          QString("[Mascaret] Polyline '%1' does not have enough vertices")
    ...
    while (true) // fake loop for easy break
    {
      ...
      if (!ok)
        break;
      Ud.x = poly->getMetaData(ccPolyline::MetaKeyPrefixDirection() + ".x")
                 .toDouble(&ok);
      if (!ok)
        break;
      Ud.y = poly->getMetaData(ccPolyline::MetaKeyPrefixDirection() + ".y")
                 .toDouble(&ok);
      if (!ok)
        break;
      Ud.z = poly->getMetaData(ccPolyline::MetaKeyPrefixDirection() + ".z")
                 .toDouble(&ok);
      break;
    }
    if (!ok) {
      ccLog::Warning(QString("[Mascaret] At least one of the meta-data entry "
                             "of polyline '%1' is invalid?!")
                         .arg(poly->getName()));
      continue;
    }

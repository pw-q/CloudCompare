...
QString GetStringFromNode(const e57::StructureNode &node, const char *fieldName,
...
void AddToE57NodeMap(E57NodeMap &nodeMap, const e57::StructureNode &node,
  ...
  if (node.isDefined(fieldName)) {
    ...
    case e57::TypeInteger:
      nodeDesc.type = "INT";
      nodeDesc.content =
          QString::number(e57::IntegerNode(node.get(fieldName)).value());
      break;
    case e57::TypeFloat:
      nodeDesc.type = "FLT";
      nodeDesc.content =
          QString::number(e57::FloatNode(node.get(fieldName)).value(), 'f', 12);
      break;
    case e57::TypeString:
      nodeDesc.type = "STR";
      nodeDesc.content =
          QString::fromStdString(e57::StringNode(node.get(fieldName)).value());
      break;
    default:
      // unhandled node type
      return;
...
bool E57Filter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
static void SavePoseInformation(e57::StructureNode &parentNode,
                                const e57::ImageFile &imf, ccGLMatrixd &poseMat,
  ...
  if (transMat.computeDet() < 0) {
    ccLog::Warning("[E57] Pose matrix seems to be a reflection. Can't save it "
                   "as a quaternion (required by the E57 format). We will use "
                   "the non-reflective form.");
  }
  ...
  // it happens that the saved pose matrix doesn't capture everything (such as
  // reflections) because of the quaternion form. Therefore we update the input
  // 'poseMat' with what we have actually saved
  poseMat = ccGLMatrixd::FromQuaternion(q);
  poseMat.setTranslation(tr);
}

static void SaveNodeInfo(const E57NodeMap &nodeMap,
                         e57::StructureNode &outputNode, e57::ImageFile &imf) {
  for (auto it = nodeMap.begin(); it != nodeMap.end(); ++it) {
    if (!it.value().content.isEmpty()) {
      if (it.value().type == "STR") {
        outputNode.set(it.key().toStdString(),
                       e57::StringNode(imf, it.value().content.toStdString()));
        ...
        if (ok) {
          ...
          ccLog::Warning("[E57] Invalid value in meta-data for field " +
                         it.key());
        }
        ...
        ccLog::Warning(
            QString("[E57] Unkown E57 type %1 in meta-data for field %2")
                .arg(it.value().type)
                .arg(it.key()));
      }
    }
...
static bool SaveScan(ccPointCloud *cloud, e57::StructureNode &scanNode,
                     e57::ImageFile &imf, e57::VectorNode &data3D,
                     QString &guidStr,
  ...
  if (pointCount == 0) {
    ccLog::Error(QString("[E57Filter::SaveScan] Cloud '%1' is empty!")
                     .arg(cloud->getName()));
    return false;
  }
  ...
  if (cloud->hasMetaData(s_e57NodeInfoKey)) {
    QStringList stringList =
        cloud->getMetaData(s_e57NodeInfoKey).toString().split("\n");
  ...
  ccGLMatrixd
      toSensorCS; // 'Sensor' output coordinate system (no Global Shift applied)
  ...
  bool isScaled = false;
  {
    ...
    if (!poseStr.isEmpty()) {
      ...
      if (sensors.size() != 0) {
        ...
        toSensorCS =
            ccGLMatrixd(sensors.front()->getGLTransformationHistory().data());
      }
    }
  ...
  if (hasSensorPoseMat) {
    ...
    for (unsigned i = 0; i < pointCount; ++i) {
      // we apply the Global Scale but not the Global Shift (already
      // incorporated in the 'pose' matrix above)
      CCVector3d Psensor =
          cloud->getPointPersistentPtr(i)->toDouble() / globalScale;
    ...
    if (!cloud->getOwnGlobalBB(bbMin, bbMax)) {
      ccLog::Error(QString("[E57Filter::SaveScan] Internal error: cloud '%1' "
  ...
  if (!cloud->getName().isEmpty())
    ...
    scanNode.set(
        "name",
        e57::StringNode(
            imf, QString("Scan %1").arg(s_absoluteScanIndex).toStdString()));
  ...
  // e57::StringNode(imf,sensorSerialNumber)); scan.set("sensorHardwareVersion",
  ...
  // e57::FloatNode(imf,relativeHumidity)); scanNode.set("atmosphericPressure",
  ...
  {
    int returnIndexSFIndex =
        cloud->getScalarFieldIndexByName(CC_E57_RETURN_INDEX_FIELD_NAME);
      ...
      assert(sf->getMin() >= 0);
      {
        ...
        if (fracMin == 0 && fracMax == 0 &&
          ...
          if (maxReturnIndex > minReturnIndex) {
            ...
            // DGM FIXME: should we really save this for an unstructured point
            // cloud?
  ...
  bool hasInvalidIntensities = false;
  {
    ...
    if (intensitySFIndex < 0) {
      ...
      if (intensitySFIndex >= 0)
        ccLog::Print("[E57] No 'intensity' scalar field found, we'll use the "
                     "currently displayed one instead (%s)",
                     cloud->getScalarFieldName(intensitySFIndex).c_str());
    }
    if (intensitySFIndex >= 0) {
      intensitySF =
          static_cast<ccScalarField *>(cloud->getScalarField(intensitySFIndex));
      assert(intensitySF);
  ...
  // acquisitionStart.set("dateTimeValue",			e57::FloatNode(imf,
  // dateTimeValue)); acquisitionStart.set("isAtomicClockReferenced",
  // e57::IntegerNode(imf, isAtomicClockReferenced)); e57::StructureNode
  // acquisitionEnd = e57::StructureNode(imf); scanNode.set("acquisitionEnd",
  // acquisitionEnd); acquisitionEnd.set("dateTimeValue",
  ...
  const unsigned chunkSize = std::min<unsigned>(
      pointCount, (1 << 20)); // we save the file in several steps to limit the
                              // memory consumption
  TempArrays arrays;
  ...
  // Cartesian field
  {
    e57::FloatPrecision precision = sizeof(PointCoordinateType) == 8 || isScaled
                                        ? e57::E57_DOUBLE
                                        : e57::E57_SINGLE;
  ...
  if (returnIndexSF) {
    ...
    proto.set("returnIndex", e57::IntegerNode(imf, minReturnIndex,
                                              minReturnIndex, maxReturnIndex));
  ...
  if (intensitySF) {
    ...
    dbufs.emplace_back(imf, "intensity", arrays.intData.data(), chunkSize, true,
                       true);
    ...
    if (hasInvalidIntensities) {
      ...
      dbufs.emplace_back(imf, "isIntensityInvalid",
                         arrays.isInvalidIntData.data(), chunkSize, true, true);
    }
  }
  ...
  if (hasColors) {
    ...
    dbufs.emplace_back(imf, "colorRed", arrays.redData.data(), chunkSize, true,
                       true);
    proto.set("colorGreen", e57::IntegerNode(imf, 0, 0, 255));
    arrays.greenData.resize(chunkSize);
    dbufs.emplace_back(imf, "colorGreen", arrays.greenData.data(), chunkSize,
                       true, true);
  ...
  while (remainingPointCount != 0) {
    ...
    for (unsigned i = 0; i < thisChunkSize; ++i, ++index) {
      ...
      // incorporated in the 'pose' matrix above)
      CCVector3d Psensor =
          cloud->getPointPersistentPtr(index)->toDouble() / globalScale;

      // DGM: according to E57 specifications, the points are saved in the local
      // CS (i.e. in the sensor 'input' coordinate system)
      CCVector3d Plocal =
          (hasSensorPoseMat ? fromSensorToLocalCS * Psensor : Psensor);
      ...
      if (intensitySF) {
        ...
        if (!arrays.isInvalidIntData.empty()) {
          arrays.isInvalidIntData[i] =
              ccScalarField::ValidValue(sfVal) ? VALID_DATA : INVALID_DATA;
        }
      }
...
void SaveImage(const ccImage *image, const QString &scanGUID,
               e57::ImageFile &imf, e57::VectorNode &images2D,
               const CCVector3d &globalShift) {
  assert(image);
  ...
  if (!image->getName().isEmpty()) {
    ...
    imageNode.set(
        "name",
        e57::StringNode(
            imf, QString("Image %1").arg(s_absoluteImageIndex).toStdString()));
  }
  ...
  {
    ccGLMatrixd toSensorCS; // 'Sensor' output coordinate system (no Global
                            // Shift applied)
    toSensorCS.toIdentity();
    bool hasSensorPoseMat = false;
    {
      ...
      if (sensor) {
        ...
        if (!poseStr.isEmpty()) {
          toSensorCS = ccGLMatrixd::FromString(poseStr, hasSensorPoseMat);
          if (hasSensorPoseMat) {
            // apply transformation history (i.e. transformations applied after
            // the entity has been loaded)
            //--> we assume they have been implicitly applied to the sensor as
            //well
            ccGLMatrixd historyTransform(
                image->getGLTransformationHistory().data());
            toSensorCS = historyTransform * toSensorCS;
          } else {
            ccLog::Warning(
                "[E57Filter::saveFile] Sensor pose meta-data is invalid");
  ...
  if (image->hasMetaData(s_e57CameraInfoKey)) {
    QStringList stringList =
        image->getMetaData(s_e57CameraInfoKey).toString().split("\n");
    if (stringList.size() != 0 && stringList.front().startsWith("type=")) {
      ...
      if (sensorType == VisualReferenceRepresentation::GetName()) {
        ...
        ccLog::Warning(
            "[E57] Invalid 'E57 camera info' meta-data (unkown sensor type)");
      }
    } else {
      ccLog::Warning("[E57] Invalid 'E57 camera info' meta-data (invalid "
                     "format: no 'type' key)");
    }

    if (cameraRepresentation &&
        !cameraRepresentation->fromStringList(stringList)) {
      // failed to unserialize the meta-data
      ccLog::Warning(
          "[E57] Invalid 'E57 camera info' meta-data (invalid format)");
    }
  }
  ...
  if (!cameraRepresentation && sensor) {
    ...
    PinholeRepresentation *pinholeRepresentation = new PinholeRepresentation;
    {
      ...
      pinholeRepresentation->imageSize =
          static_cast<int64_t>(pinholeRepresentation->imageWidth) *
          pinholeRepresentation->imageHeight;
    }
  ...
  if (cameraRepresentation) {
    ...
    if (cameraRepresentation->getType() == E57_VISUAL ||
      ...
      // cameraRepresentationNode.set("imageWidth", e57::IntegerNode(imf,
      // cameraRepresentation->imageWidth));		//overwritten later with
      // the real image size cameraRepresentationNode.set("imageHeight",
      // e57::IntegerNode(imf, cameraRepresentation->imageHeight));
      // //overwritten later with the real image size
      ...
      if (cameraRepresentation->getType() == E57_SPHERICAL ||
        ...
        cameraRepresentationNode.set(
            "pixelWidth", e57::FloatNode(imf, spherical->pixelWidth));
        cameraRepresentationNode.set(
            "pixelHeight", e57::FloatNode(imf, spherical->pixelHeight));
        ...
        if (cameraRepresentation->getType() == E57_PINHOLE) {
          ...
          cameraRepresentationNode.set(
              "focalLength", e57::FloatNode(imf, pinhole->focalLength));
          cameraRepresentationNode.set(
              "principalPointX", e57::FloatNode(imf, pinhole->principalPointX));
          cameraRepresentationNode.set(
              "principalPointY", e57::FloatNode(imf, pinhole->principalPointY));
      ...
      ccLog::Warning(
          "[E57] Invalid 'E57 camera info' meta-data (wrong camera type)");
    }
  ...
  cameraRepresentationNode.set("imageHeight",
                               e57::IntegerNode(imf, image->getH()));
  cameraRepresentationNode.set("imageWidth",
                               e57::IntegerNode(imf, image->getW()));

  imageNode.set(cameraRepresentationStr.toStdString(),
                cameraRepresentationNode);
  images2D.append(imageNode);
  blob.write((uint8_t *)ba.data(), 0, static_cast<size_t>(imageSize));
}

CC_FILE_ERROR E57Filter::saveToFile(ccHObject *entity, const QString &filename,
  ...
  try {
    ...
    root.set("e57LibraryVersion",
             e57::StringNode(imf, e57::Version::library()));

    // Save a dummy string for coordinate system.
    /// Really should be a valid WKT string identifying the coordinate reference
    /// system (CRS).
    ...
    if (result == CC_FERR_NO_ERROR) {
      ...
      for (size_t i = 0; i < scanCount; ++i) {
        ...
        unsigned imageCount =
            cloud->filterChildren(images, false, CC_TYPES::IMAGE);

        if (imageCount != 0) {
          // progress bar
          CCCoreLib::NormalizedProgress nprogress(progressDlg.data(),
                                                  imageCount);
          if (progressDlg) {
            progressDlg->setMethodTitle(QObject::tr("Write E57 file"));
            progressDlg->setInfo(
                QObject::tr("Cloud #%1 - Images: %2").arg(i).arg(imageCount));
            progressDlg->start();
            QApplication::processEvents();
          }
          ...
          for (unsigned j = 0; j < imageCount; ++j) {
            ...
            SaveImage(static_cast<ccImage *>(images[j]), scanGUID, imf,
                      images2D, cloud->getGlobalShift());
    ...
    if (!e.context().empty()) {
      ccLog::Warning(QStringLiteral("    context: %1")
...
static bool NodeStructureToTree(ccHObject *currentTreeNode,
  ...
  QString infoStr =
      QString(name.c_str() == nullptr || name.c_str()[0] == 0 ? "No name"
                                                              : name.c_str());
  ...
  case e57::E57_COMPRESSED_VECTOR: {
    ...
    infoStr +=
        QString(" [COMPRESSED VECTOR (%1 elements)]").arg(cv.childCount());
  } break;
...
static bool ChildNodeToConsole(const e57::Node &node, const char *childName) {
  ...
  if (node.type() == e57::E57_STRUCTURE) {
    ...
    ccLog::Warning(
        "[E57] Element '%s' has no child (not a structure nor a vector!)",
...
static void DecodePrototype(const e57::StructureNode &scan,
  ...
  if (proto.isDefined("cartesianX")) {
    ...
    if (proto.get("sphericalRange").type() == e57::E57_SCALED_INTEGER) {
      ...
      double offset =
          e57::ScaledIntegerNode(proto.get("sphericalRange")).offset();
      int64_t minimum =
          e57::ScaledIntegerNode(proto.get("sphericalRange")).minimum();
      int64_t maximum =
          e57::ScaledIntegerNode(proto.get("sphericalRange")).maximum();
      header.pointFields.pointRangeMinimum = minimum * scale + offset;
      header.pointFields.pointRangeMaximum = maximum * scale + offset;
      header.pointFields.pointRangeScaledInteger = scale;
  ...
  if (proto.isDefined("sphericalAzimuth")) {
    if (proto.get("sphericalAzimuth").type() == e57::E57_SCALED_INTEGER) {
      double scale =
          e57::ScaledIntegerNode(proto.get("sphericalAzimuth")).scale();
      double offset =
          e57::ScaledIntegerNode(proto.get("sphericalAzimuth")).offset();
      int64_t minimum =
          e57::ScaledIntegerNode(proto.get("sphericalAzimuth")).minimum();
      int64_t maximum =
          e57::ScaledIntegerNode(proto.get("sphericalAzimuth")).maximum();
  ...
  if (proto.isDefined("timeStamp")) {
    if (proto.get("timeStamp").type() == e57::E57_INTEGER)
      ...
      header.pointFields.timeMaximum =
          static_cast<double>(e57::FloatNode(proto.get("timeStamp")).maximum());
  }
  ...
  if (scan.isDefined("intensityLimits")) {
    ...
    if (intbox.get("intensityMaximum").type() == e57::E57_SCALED_INTEGER) {
      header.intensityLimits.intensityMaximum =
          e57::ScaledIntegerNode(intbox.get("intensityMaximum")).scaledValue();
      header.intensityLimits.intensityMinimum =
          e57::ScaledIntegerNode(intbox.get("intensityMinimum")).scaledValue();
  ...
  if (proto.isDefined("intensity")) {
    if (proto.get("intensity").type() == e57::E57_INTEGER) {
      ...
      if (header.intensityLimits.intensityMaximum == 0.) {
        int64_t minimum =
            e57::ScaledIntegerNode(proto.get("intensity")).minimum();
        int64_t maximum =
            e57::ScaledIntegerNode(proto.get("intensity")).maximum();
  ...
  if (scan.isDefined("colorLimits")) {
    ...
    if (colorbox.get("colorRedMaximum").type() == e57::E57_SCALED_INTEGER) {
      header.colorLimits.colorRedMaximum =
          e57::ScaledIntegerNode(colorbox.get("colorRedMaximum")).scaledValue();
      header.colorLimits.colorRedMinimum =
          e57::ScaledIntegerNode(colorbox.get("colorRedMinimum")).scaledValue();
      ...
      header.colorLimits.colorGreenMaximum = static_cast<double>(
          e57::IntegerNode(colorbox.get("colorGreenMaximum")).value());
      header.colorLimits.colorGreenMinimum = static_cast<double>(
          e57::IntegerNode(colorbox.get("colorGreenMinimum")).value());
...
static LoadedScan LoadScan(const e57::Node &node, QString &guidStr,
                           ccProgressDialog *progressDlg = nullptr) {
  if (node.type() != e57::E57_STRUCTURE) {
  ...
  // log
  ccLog::Print(QString("[E57] Reading new scan node (%1) - %2")
                   .arg(scanNode.elementName().c_str())
                   .arg(scanName));

  if (!scanNode.isDefined("points")) {
    ccLog::Warning(QString("[E57Filter] No point in scan '%1'!")
                       .arg(scanNode.elementName().c_str()));
    return {};
  }
  ...
  if (!header.pointFields.cartesianXField &&
    ...
    if (!header.pointFields.sphericalRangeField &&
        ...
        !header.pointFields.sphericalElevationField) {
      ccLog::Warning(
          QString("[E57Filter] No readable point in scan '%1'! (only cartesian "
                  "and spherical coordinates are supported right now)")
              .arg(scanNode.elementName().c_str()));
      return {};
    }
    sphericalMode = true;
  ...
  if (scanNode.isDefined("description")) {
    ccLog::Print(
        QStringLiteral("[E57] Internal description: %1")
            .arg(GetStringFromNode(scanNode, "description", QString())));
  }
  ...
  int64_t gridRowCount = 0, gridColumnCount = 0;
  {
    if (scanNode.isDefined("indexBounds")) {
      ...
      if (indexBounds.isDefined("columnMaximum")) {
        gridColumnCount =
            e57::IntegerNode(indexBounds.get("columnMaximum")).value() -
            e57::IntegerNode(indexBounds.get("columnMinimum")).value() + 1;
      }

      if (indexBounds.isDefined("rowMaximum")) {
        gridRowCount = e57::IntegerNode(indexBounds.get("rowMaximum")).value() -
                       e57::IntegerNode(indexBounds.get("rowMinimum")).value() +
                       1;
      }
    }
    ...
    if (scanNode.isDefined("pointGroupingSchemes")) {
      e57::StructureNode pointGroupingSchemes(
          scanNode.get("pointGroupingSchemes"));
      if (pointGroupingSchemes.isDefined("groupingByLine")) {
        e57::StructureNode groupingByLine(
            pointGroupingSchemes.get("groupingByLine"));
        ...
        if (lineGroupRecord.isDefined("idElementValue"))
          elementCount = e57::IntegerNode(lineGroupRecord.get("idElementValue"))
                             .maximum() -
                         e57::IntegerNode(lineGroupRecord.get("idElementValue"))
                             .minimum() +
                         1;
        ...
        if (lineGroupRecord.isDefined("pointCount"))
          groupPointCount =
              e57::IntegerNode(lineGroupRecord.get("pointCount")).maximum();
        else if (bColumnIndex)
  ...
  if (validPoseMat) {
    ...
    if (FileIOFilter::HandleGlobalShift(
            T, poseMatShift, preserveCoordinateShift, s_loadParameters)) {
      ...
      ccLog::Warning("[E57Filter::loadFile] Cloud %s has been recentered! "
                     "Translation: (%.2f ; %.2f ; %.2f)",
                     qPrintable(guidStr), poseMatShift.x, poseMatShift.y,
                     poseMatShift.z);
    }
  ...
  // prepare temporary structures
  const unsigned chunkSize = std::min<unsigned>(
      pointCount, (1 << 20)); // we load the file in several steps to limit the
                              // memory consumption
  TempArrays arrays;
  std::vector<e57::SourceDestBuffer> dbufs;
  ...
  if (header.pointFields.rowIndexField && header.pointFields.columnIndexField &&
      gridRowCount != 0 && gridColumnCount != 0) {
    scanGrid.reset(new ccPointCloud::Grid);
    if (scanGrid->init(static_cast<unsigned>(gridRowCount),
                       static_cast<unsigned>(gridColumnCount))) {
      arrays.rowIndex.resize(chunkSize);
      dbufs.emplace_back(node.destImageFile(), "rowIndex",
                         arrays.rowIndex.data(), chunkSize, true);
      arrays.columnIndex.resize(chunkSize);
      dbufs.emplace_back(node.destImageFile(), "columnIndex",
                         arrays.columnIndex.data(), chunkSize, true);
    } else {
  ...
  if (sphericalMode) {
    ...
    if (header.pointFields.sphericalRangeField) {
      arrays.xData.resize(chunkSize);
      dbufs.emplace_back(
          node.destImageFile(), "sphericalRange", arrays.xData.data(),
          chunkSize, true,
          (prototype.get("sphericalRange").type() == e57::E57_SCALED_INTEGER));
    }
    ...
    if (header.pointFields.cartesianXField) {
      arrays.xData.resize(chunkSize);
      dbufs.emplace_back(
          node.destImageFile(), "cartesianX", arrays.xData.data(), chunkSize,
          true,
          (prototype.get("cartesianX").type() == e57::E57_SCALED_INTEGER));
    }
    if (header.pointFields.cartesianYField) {
      arrays.yData.resize(chunkSize);
      dbufs.emplace_back(
          node.destImageFile(), "cartesianY", arrays.yData.data(), chunkSize,
          true,
          (prototype.get("cartesianY").type() == e57::E57_SCALED_INTEGER));
    }
    if (header.pointFields.cartesianZField) {
      arrays.zData.resize(chunkSize);
      dbufs.emplace_back(
          node.destImageFile(), "cartesianZ", arrays.zData.data(), chunkSize,
          true,
          (prototype.get("cartesianZ").type() == e57::E57_SCALED_INTEGER));
    }
  ...
  if (hasNormals) {
    ...
    if (header.pointFields.normXField) {
      arrays.xNormData.resize(chunkSize);
      dbufs.emplace_back(
          node.destImageFile(), "nor:normalX", arrays.xNormData.data(),
          chunkSize, true,
          (prototype.get("nor:normalX").type() == e57::E57_SCALED_INTEGER));
    }
    if (header.pointFields.normYField) {
      arrays.yNormData.resize(chunkSize);
      dbufs.emplace_back(
          node.destImageFile(), "nor:normalY", arrays.yNormData.data(),
          chunkSize, true,
          (prototype.get("nor:normalY").type() == e57::E57_SCALED_INTEGER));
    }
    if (header.pointFields.normZField) {
      arrays.zNormData.resize(chunkSize);
      dbufs.emplace_back(
          node.destImageFile(), "nor:normalZ", arrays.zNormData.data(),
          chunkSize, true,
          (prototype.get("nor:normalZ").type() == e57::E57_SCALED_INTEGER));
    }
  }
  ...
  if (header.pointFields.intensityField) {
    ...
    arrays.intData.resize(chunkSize);
    dbufs.emplace_back(
        node.destImageFile(), "intensity", arrays.intData.data(), chunkSize,
        true, (prototype.get("intensity").type() == e57::E57_SCALED_INTEGER));
  ...
  if (hasColors) {
    ...
    if (header.pointFields.colorRedField) {
      ...
      if (colorRedRange <= 0.0)
        colorRedRange = 1.0;
      dbufs.emplace_back(
          node.destImageFile(), "colorRed", arrays.redData.data(), chunkSize,
          true, (prototype.get("colorRed").type() == e57::E57_SCALED_INTEGER));
    }
    if (header.pointFields.colorGreenField) {
      ...
      if (colorGreenRange <= 0.0)
        colorGreenRange = 1.0;
      dbufs.emplace_back(
          node.destImageFile(), "colorGreen", arrays.greenData.data(),
          chunkSize, true,
          (prototype.get("colorGreen").type() == e57::E57_SCALED_INTEGER));
    }
    if (header.pointFields.colorBlueField) {
      ...
      if (colorBlueRange <= 0.0)
        colorBlueRange = 1.0;
      dbufs.emplace_back(
          node.destImageFile(), "colorBlue", arrays.blueData.data(), chunkSize,
          true, (prototype.get("colorBlue").type() == e57::E57_SCALED_INTEGER));
    }
  }
  ...
  if (header.pointFields.returnIndexField &&
    ...
    arrays.scanIndexData.resize(chunkSize);
    dbufs.emplace_back(
        node.destImageFile(), "returnIndex", arrays.scanIndexData.data(),
        chunkSize, true,
        (prototype.get("returnIndex").type() == e57::E57_SCALED_INTEGER));
  }
  ...
  while ((size = dataReader.read())) {
    for (unsigned i = 0; i < size; ++i) {
      ...
      // DGM TODO: not handled yet (-->what are the standard cylindrical field
      // names?)
      ...
      if (realCount == 0 && !poseMatWasShifted) {
        if (FileIOFilter::HandleGlobalShift(Pd, Pshift, preserveCoordinateShift,
                                            s_loadParameters)) {
          globalShiftApplied = true;
          if (preserveCoordinateShift) {
      ...
      if (!arrays.intData.empty()) {
        assert(intensitySF);
        if (!header.pointFields.isIntensityInvalidField ||
            arrays.isInvalidIntData[i] != INVALID_DATA) {
      ...
      if (hasColors) {
        ...
        if (!arrays.redData.empty())
          C.r = static_cast<ColorCompType>(
              ((arrays.redData[i] - colorRedOffset) * 255) / colorRedRange);
        ...
        if (!arrays.blueData.empty())
          C.b = static_cast<ColorCompType>(
              ((arrays.blueData[i] - colorBlueOffset) * 255) / colorBlueRange);
  ...
  if (zeroCount > 1) {
    ccLog::Warning(QString("[E57] Number of points clustured at the origin: %1 "
                           "(consider removing duplicate points)")
                       .arg(zeroCount));
  }

  if (realCount == 0) {
    ccLog::Warning(QString("[E57] No valid point in scan '%1'!")
                       .arg(scanNode.elementName().c_str()));
    delete cloud;
    return {};
    ...
    if ((realCount + invalidCount) != pointCount) {
      ccLog::Warning(
          QString(
              "[E57] We read fewer points than expected for scan '%1' (%2/%3)")
              .arg(scanNode.elementName().c_str())
              .arg(realCount)
              .arg(pointCount));
    }
  ...
  if (scanGrid) {
    ...
    ccLog::Print(QString("[E57] Scan grid loaded for scan '%1' (%2 x %3)")
                     .arg(scanNode.elementName().c_str())
                     .arg(scanGrid->w)
                     .arg(scanGrid->h));
  }
  ...
  if (returnIndexSF) {
    ...
    ccLog::Warning("[E57] Cloud has multiple echoes: use 'Edit > Scalar Fields "
  ...
  return {cloud, globalShiftApplied, poseMatWasShifted ? poseMatShift : Pshift,
...
static LoadedImage LoadImage(const e57::Node &node,
  ...
  // log
  ccLog::Print(QString("[E57] Reading new image node (%1) - %2")
                   .arg(imageNode.elementName().c_str())
                   .arg(imageName));

  if (imageNode.isDefined("description")) {
    ccLog::Print(
        QString("[E57] Description: %1")
            .arg(
                e57::StringNode(imageNode.get("description")).value().c_str()));
  }

  if (imageNode.isDefined("associatedData3DGuid")) {
    associatedData3DGuid =
        e57::StringNode(imageNode.get("associatedData3DGuid")).value().c_str();
  } else {
  ...
  if (imageNode.isDefined(VisualReferenceRepresentation::GetName())) {
    ...
    ccLog::Warning(QString("[E57] Image %1 has no or a unknown associated "
  ...
  e57::StructureNode cameraRepresentationNode(
      imageNode.get(e57::ustring(cameraRepresentation->getName())));
  ...
  if (cameraRepresentationNode.isDefined("jpegImage")) {
    ...
    cameraRepresentation->imageSize =
        e57::BlobNode(cameraRepresentationNode.get("jpegImage")).byteCount();
  } else if (cameraRepresentationNode.isDefined("pngImage")) {
    cameraRepresentation->imageType = E57_PNG_IMAGE;
    cameraRepresentation->imageSize =
        e57::BlobNode(cameraRepresentationNode.get("pngImage")).byteCount();
  } else {
    ccLog::Warning("[E57] Unhandled image format (only JPG and PNG are "
                   "currently supported)");
    delete cameraRepresentation;
    return {};
  }
  ...
  if (cameraRepresentationNode.isDefined("imageMask")) {
    cameraRepresentation->imageMaskSize =
        e57::BlobNode(cameraRepresentationNode.get("imageMask")).byteCount();
  }

  cameraRepresentation->imageHeight = static_cast<int32_t>(
      e57::IntegerNode(cameraRepresentationNode.get("imageHeight")).value());
  cameraRepresentation->imageWidth = static_cast<int32_t>(
      e57::IntegerNode(cameraRepresentationNode.get("imageWidth")).value());
  ...
  case E57_SPHERICAL: {
    ...
    spherical->pixelHeight =
        e57::FloatNode(cameraRepresentationNode.get("pixelHeight")).value();
    spherical->pixelWidth =
        e57::FloatNode(cameraRepresentationNode.get("pixelWidth")).value();
  } break;
  ...
  if (cameraType == E57_PINHOLE) {
    ...
    pinhole->focalLength =
        e57::FloatNode(cameraRepresentationNode.get("focalLength")).value();
    pinhole->principalPointX =
        e57::FloatNode(cameraRepresentationNode.get("principalPointX")).value();
    pinhole->principalPointY =
        e57::FloatNode(cameraRepresentationNode.get("principalPointY")).value();
  ...
  case E57_JPEG_IMAGE: {
    ...
    jpegImage.read(imageBits, 0,
                   static_cast<size_t>(cameraRepresentation->imageSize));
    break;
  }
  case E57_PNG_IMAGE: {
    ...
    pngImage.read((uint8_t *)imageBits, 0,
                  static_cast<size_t>(cameraRepresentation->imageSize));
    break;
  }
  default:
  ...
  bool loadResult = qImage.loadFromData(
      imageBits, static_cast<int>(cameraRepresentation->imageSize),
      imageFormat);

  // a bug in some 2.13.alpha versions was causing CC to save JPEG images
  // declared as PNG images :(
  if (!loadResult && cameraRepresentation->imageType == E57_PNG_IMAGE) {
    loadResult = qImage.loadFromData(
        imageBits, static_cast<int>(cameraRepresentation->imageSize),
        "jpegImage");
    ccLog::Warning("[E57] JPG image was wrongly tagged as PNG. You should save "
                   "this E57 again to fix this...");
  }
  ...
  case E57_PINHOLE: {
    ...
    params.vertFocal_pix =
        ccCameraSensor::ConvertFocalMMToPix(focal_mm, pixelHeight_mm);
    ...
    params.vFOV_rad =
        ccCameraSensor::ComputeFovRadFromFocalMm(focal_mm, ccdHeight_mm);

    output.sensor = new ccCameraSensor(params);
    if (output.validPoseMat) {
      ccGLMatrix poseMatf(
          output.poseMat.data()); // we'll use it as is for now (will be updated
                                  // later with a potential Global Shift)
  ...
  if (output.entity) {
    ...
    output.entity->setMetaData(s_e57NodeInfoKey,
                               nodeInfo.toStringList().join("\n"));
    output.entity->setMetaData(s_e57CameraInfoKey,
                               cameraRepresentation->toStringList().join("\n"));

    // save the original pose matrix as meta-data
    if (output.validPoseMat) {
      output.entity->setMetaData(
          s_e57PoseKey,
          output.poseMat.toString(
              12, ' ')); // we'll use it as is for now (will be updated later
                         // with a potential Global Shift)
    }
    ...
    if (cameraType == E57_CYLINDRICAL || cameraType == E57_PINHOLE ||
      ...
      SphericalRepresentation *spherical =
          static_cast<SphericalRepresentation *>(cameraRepresentation);
      output.entity->setAspectRatio(
          static_cast<float>(spherical->pixelWidth / spherical->pixelHeight) *
          output.entity->getAspectRatio());
    }
  }
...
CC_FILE_ERROR E57Filter::loadFile(const QString &filename, ccHObject &container,
  ...
  try {
    ...
    // unroll structure in tree (it's a quick to check structure + informative
    // for user)
    ...
    if (root.isDefined("/data3D")) {
      e57::Node n =
          root.get("/data3D"); // E57 standard: "data3D is a vector for storing
                               // an arbitrary number of 3D data sets "
      ...
      for (unsigned i = 0; i < scanCount; ++i) {
        ...
        if ((showGlobalProgress && progressDlg && !nprogress.oneStep()) ||
    ...
    if (!s_cancelRequestedByUser && root.isDefined("/images2D")) {
      e57::Node n =
          root.get("/images2D"); // E57 standard: "images2D is a vector for
      ...
      if (imageCount) {
        ...
        if (parameters.parentWidget) {
          progressDlg.reset(
              new ccProgressDialog(true, parameters.parentWidget));
          progressDlg->setMethodTitle(QObject::tr("Read E57 file"));
        ...
        for (unsigned i = 0; i < imageCount; ++i) {
          ...
          if (image.entity) {
            ...
            if (!associatedData3DGuid.isEmpty() &&
                scans.contains(associatedData3DGuid)) {
            ...
            if (image.validPoseMat) {
              ...
              } else // no parent scan
              {
                ...
                applySensorGlobalShift = FileIOFilter::HandleGlobalShift(
                    T, poseMatShift, preserveCoordinateShift, s_loadParameters);
              ...
              if (applySensorGlobalShift) {
                image.poseMat.setTranslation(
                    (image.poseMat.getTranslationAsVec3D() + poseMatShift).u);
                ...
                if (image.sensor) {
                  ...
                  ccLog::Warning(
                      "[E57Filter::loadFile] The sensor of image %s has been "
                      "recentered! Translation: (%.2f ; %.2f ; %.2f)",
                      qPrintable(image.entity->getName()), poseMatShift.x,
                      poseMatShift.y, poseMatShift.z);
                }
              }

              // save the potentially shifted pose matrix as meta-data
              image.entity->setMetaData(s_e57PoseKey,
                                        image.poseMat.toString(12, ' '));
            }
          }

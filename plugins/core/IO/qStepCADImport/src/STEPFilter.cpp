...
CC_FILE_ERROR STEPFilter::loadFile(const QString &fullFilename,
  ...
  if (parameters.sessionStart && parameters.parentWidget) {
    bool ok = false;
    double linearDeflection = QInputDialog::getDouble(
        parameters.parentWidget, "Linear deflection", "Linear deflection",
        s_defaultLinearDeflection, 1.0e-6, 1.0e-2, 6, &ok);
    if (!ok) {
  ...
  CC_FILE_ERROR error;
  try {
    error = importStepFile(container, fullFilename, s_defaultLinearDeflection,
                           parameters);
...
CC_FILE_ERROR STEPFilter::importStepFile(ccHObject &container,
                                         ...
                                         LoadParameters &parameters) {
  // Interface_Static::SetCVal("xstep.cascade.unit", "M"); // DGM: seems to mess
  // things completely, having various impacts during the next run (smaller
  // scale, etc.)
  ...
  BRepMesh_IncrementalMesh incrementalMesh(
      ...
      Standard_True); // not explicitly used, but still needs to be instantiated
  ...
  ccLog::Print("[STEP] Number of triangles (after tesselation) = " +
               QString::number(triCount));
  ccLog::Print("[STEP] Number of vertices (after tesselation)  = " +
               QString::number(vertCount));

  mesh->mergeDuplicatedVertices(ccMesh::DefaultMergeDuplicateVerticesLevel,
                                parameters.parentWidget);
  vertices =
      nullptr; // warning, after this point, 'vertices' is not valid anymore

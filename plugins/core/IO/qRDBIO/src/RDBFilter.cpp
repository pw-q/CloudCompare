...
RDBFilter::RDBFilter()
    : FileIOFilter(
          {"RDB 2.0 Filter",
           ...
           QStringList{"RDB Pointcloud file (*.rdbx)", "RDB Voxelfile (*.vxls)",
...
void updateTable(RDBOpenDialog &openDlg, riegl::rdb::Pointcloud &rdb) {
  ...
  struct SpecialAttribute {
    ...
    SpecialAttribute(const std::string &arg_name,
        ...
        : name(arg_name), comboBoxEntry(arg_comboBoxEntry), icon(arg_icon) {}
  };
  const std::vector<SpecialAttribute> known_special_attributes{
      ...
      {"riegl.surface_normal", RDB_OPEN_DLG_TYPES_NAMES[RDB_OPEN_DLG_NORM],
       NormIcon},
  ...
  };
  {
    ...
    auto list_known_attribute = [](RDBOpenDialog &openDlg, int &idx,
      ...
      if (std::find(attributes.begin(), attributes.end(), att) !=
          attributes.end()) {
        attributes.erase(std::remove(attributes.begin(), attributes.end(), att),
                         attributes.end());
    ...
    auto add_qcombobox_known_scalar = [add_qcombobox_scalar](
                                          RDBOpenDialog &openDlg, int &idx,
                                          std::vector<std::string> &attributes,
                                          const std::string &att, bool active) {
      if (std::find(attributes.begin(), attributes.end(), att) !=
          attributes.end()) {
    ...
    // list non-scalar known attributes, remove them from attributes list, as
    ...
    // some attributes have multiple entries, and each entry should have its own
    // row to present them we need to know how many separate row entries we
    // have, so count them
    ...
    // add row entry for known scalar attributes and remove them from remaining
    // attributes list
    add_qcombobox_known_scalar(openDlg, idx, attributes, "riegl.id", false);
    add_qcombobox_known_scalar(openDlg, idx, attributes, "riegl.reflectance",
                               true);
      ...
      if (attribute_length == 1) {
        ...
        for (uint32_t att_idx = 0; att_idx < attribute_length; att_idx++) {
          add_qcombobox_scalar(
              openDlg, idx, att + "[" + std::to_string(att_idx) + "]", false);
        }
      }
    }
  }
...
CC_FILE_ERROR RDBFilter::loadFile(const QString &filename, ccHObject &container,
  ...
  for (int i = 0; i < openDlg.rdbTableWidget->rowCount(); ++i) {
    QComboBox *cbox =
        static_cast<QComboBox *>(openDlg.rdbTableWidget->cellWidget(i, 1));
  ...
  }
  {
    ...
    CCCoreLib::NormalizedProgress nprogress(&pdlg,
                                            root.pointCountTotal / BUFFER_SIZE);
    {
      std::stringstream ss;
    ...
    if (rdb_has_pca_axis_min)
      select.bind("riegl.pca_axis_min",
                  ...
                  buffer_normals.data());
    if (rdb_has_plane_up)
      select.bind("riegl.plane_up", riegl::rdb::pointcloud::DataType::SINGLE,
                  buffer_plane_up.data());

    riegl::rdb::pointcloud::DataType ScalarTypeRiegl =
        (sizeof(ScalarType) == 4 ? riegl::rdb::pointcloud::DataType::SINGLE
    ...
    { // pre allocate memory for cloud to load
      ...
      if (cloud->reserve(total_entries)) {
        if (rdb_hasRGBA && !cloud->reserveTheRGBTable()) {
          ccLog::Print(
              QString("[RDBFilter] Not enough memory to reserve RGB table for "
                      "points. Try to load as many as possible. Total: %1")
                  .arg(total_entries));
        ...
        if ((rdb_hasNormals || rdb_has_pca_axis_min) &&
          ...
          ccLog::Print(
              QString("[RDBFilter] Not enough memory to reserve normal table "
                      "for points. Try to load as many as possible. Total: %1")
                  .arg(total_entries));
          result = CC_FERR_NOT_ENOUGH_MEMORY;
        }
      ...
      {
        ccLog::Print(
            QString("[RDBFilter] Not enough memory to load all points. Try to "
      ...
      for (uint32_t i = 0; i < count; ++i) {
        ...
        if (create_planes) {
          ...
          ccPlane *plane = new ccPlane(
              ...
              static_cast<PointCoordinateType>(buffer_plane_height[i]), &mat);
          if (rdb_hasRGBA) {
            col.r = static_cast<unsigned char>(buffer_rgba[i][0]);
            col.g = static_cast<unsigned char>(buffer_rgba[i][1]);
            col.b = static_cast<unsigned char>(buffer_rgba[i][2]);
...
bool RDBFilter::canSave(CC_CLASS_ENUM type, bool &multiple,

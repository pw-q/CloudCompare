...
DRCFilter::DRCFilter()
    : FileIOFilter(
          {"_Draco DRC Filter",
           12.0f, // priority
           QStringList{"drc"}, "drc", QStringList{"DRC cloud or mesh (*.drc)"},
           QStringList{"DRC cloud or mesh (*.drc)"}, Import | Export}) {}

bool DRCFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
static CC_FILE_ERROR CCCloudToDraco(const ccGenericPointCloud &ccCloud,
  ...
  if (ccCloud.hasNormals()) {
    ...
    if (nullptr != normalAttribute) {
      for (draco::PointIndex::ValueType i = 0; i < pointCount; ++i) {
        normalAttribute->SetAttributeValue(draco::AttributeValueIndex(i),
  ...
  if (ccCloud.hasColors()) {
    draco::GeometryAttribute ga;
    ga.Init(draco::GeometryAttribute::COLOR, nullptr, 4, draco::DT_UINT8, false,
            DataTypeLength(draco::DT_UINT8) * 4, 0);
  ...
  if (ccCloud.hasScalarFields()) {
    if (ccCloud.isA(CC_TYPES::POINT_CLOUD)) {
      ...
      if (cc.getNumberOfScalarFields() > 1) {
        ccLog::Warning("[DRACO] Cloud %1 has multiple scalar fields, however, "
                       "only one can be saved (the active one by default)");
      }
    }

    draco::GeometryAttribute ga;
    ga.Init(draco::GeometryAttribute::GENERIC, nullptr, 1, draco::DT_FLOAT32,
...
CC_FILE_ERROR DRCFilter::saveToFile(ccHObject *entity, const QString &filename,
  ...
  // texCoordQuantization = XXX; //not available yet since we don't know how to
  // save the texture!
    ...
    for (draco::AttributeValueIndex i(0);
      ...
      if (i == 0) {
        if (FileIOFilter::HandleGlobalShift(P, Pshift, preserveCoordinateShift,
          ...
          ccLog::Warning("[DRACO] Cloud has been recentered! Translation: "
                         "(%.2f ; %.2f ; %.2f)",
    ...
    if (rgb || rgba) {
      if (ccCloud.reserveTheRGBTable()) {
        if (rgb) {
          ...
          for (draco::AttributeValueIndex i(0);
               i < static_cast<uint32_t>(colorAttribute->size()); ++i) {
...
CC_FILE_ERROR DRCFilter::loadFile(const QString &filename, ccHObject &container,
  ...
  if (geomType == draco::TRIANGULAR_MESH) {
    ...
    const draco::PointAttribute *const pointAttribute =
        meshDraco->GetNamedAttribute(draco::GeometryAttribute::POSITION);
    if (!pointAttribute) {
      return CC_FERR_THIRD_PARTY_LIB_FAILURE;
    }
    ccLog::Print(
        "[DRACO] Mesh size: " + QString::number(meshDraco->num_faces()) +

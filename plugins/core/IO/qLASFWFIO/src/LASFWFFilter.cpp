...
struct ExtraLasField : LasField {
  ...
  ExtraLasField(QString name = "Undefined", ccScalarField *_sf = nullptr)
      : LasField(LAS_EXTRA, 0.0, 0.0, 0.0), fieldName(name),
        sanitizedName(SanitizeString(name)), isShifted(false), startIndex(0) {
    if (fieldName != sanitizedName) {
      ccLog::Warning(
          QString(
              "Extra field '%1' renamed '%2' to comply to LAS specifications")
              .arg(fieldName)
              .arg(sanitizedName));
    }
...
bool LASFWFFilter::canSave(CC_CLASS_ENUM type, bool &multiple,
...
CC_FILE_ERROR LASFWFFilter::saveToFile(ccHObject *entity,
  ...
  try {
    ...
    bool hasIntensity =
        (cloud->getScalarFieldIndexByName(LAS_FIELD_NAMES[LAS_INTENSITY]) >= 0);
    ...
    if (hasFWF) {
      ...
      if (fwfFile.error() != QFile::NoError) {
        ccLog::Warning(QString("[LAS_FWF] An error occurred while writing the "
                               "FWF data file!\n(%1)")
                           .arg(fwFilename));
        hasFWF = false;
      } else {
        ccLog::Print(
            QString("[LAS_FWF] FWF data file written: %1").arg(fwFilename));
      }
    }
    ...
    LASheader lasheader;
    {
      ...
      // let the user choose between the original scale and the 'optimal' one
      // (for accuracy, not for compression ;)
      bool hasScaleMetaData = false;
      CCVector3d originalLasScale(0, 0, 0);
      originalLasScale.x =
          cloud->getMetaData(LAS_SCALE_X_META_DATA).toDouble(&hasScaleMetaData);
      if (hasScaleMetaData) {
        originalLasScale.y = cloud->getMetaData(LAS_SCALE_Y_META_DATA)
                                 .toDouble(&hasScaleMetaData);
        if (hasScaleMetaData) {
          originalLasScale.z = cloud->getMetaData(LAS_SCALE_Z_META_DATA)
                                   .toDouble(&hasScaleMetaData);
        }
      }
      ...
      if (!hasOffsetMetaData) {
        ...
        if (isShifted) {
          lasOffset = -cloud->getGlobalShift(); //'global shift' is the opposite
                                                //of LAS offset ;)
        ...
        if (ccGlobalShiftManager::NeedShift(bbMax - lasOffset)) {
          ccLog::Warning(
              "[LAS] The former LAS_OFFSET doesn't seem to be optimal. Using "
              "the minimum bounding-box corner instead.");
          CCVector3d globaShift =
              cloud->getGlobalShift(); //'global shift' is the opposite of LAS
                                       //offset ;)
          ...
          if (ccGlobalShiftManager::NeedShift(bbMax + globaShift)) {
            ...
            lasOffset.y = bbMin.y;
            lasOffset.z = 0;
            isShifted = false; // let's not use the current shift (but the bbox
                               // min corner)
          } else {
          ...
          isShifted =
              false; // let's not use the current shift (but the LAS offset)
        }
      }
      ...
      {
        double maxScale =
            std::max(optimalScale.x, std::max(optimalScale.y, optimalScale.z));
      ...
      if (parameters.alwaysDisplaySaveDialog) {
        ...
        if (hasScaleMetaData) {
          s_saveDlg->origAccuracyLabel->setText(QString("(%1, %2, %3)")
                                                    .arg(originalLasScale.x)
                                                    .arg(originalLasScale.y)
                                                    .arg(originalLasScale.z));

          if (!canUseOriginalScale) {
            s_saveDlg->labelOriginal
                ->setText(
                    QObject::tr("Original scale is too small for this cloud "
      ...
      lasheader.point_data_record_length =
          LasField::GetFormatRecordLength(lasheader.point_data_format);
      ...
      if (hasFWF) {
        ...
        //LASvlr_wave_packet_descr*[256]; 	for (int i = 0; i < 256; ++i)
        //	{
        ...
        for (ccPointCloud::FWFDescriptorSet::const_iterator it =
          ...
          lasheader.add_vlr("LASF_Spec", 99 + static_cast<U16>(it.key()), 26,
        ...
        if (!buffer.isEmpty()) {
          ...
          if (FromQByteArray(buffer, vlr)) {
            lasheader.add_vlr(ProjectionVLR, vlr.record_id,
                              vlr.record_length_after_header, vlr.data, FALSE,
                              vlr.description);
          } else {
            // warning message already issued
          }
        } else {
          ccLog::Warning("[LAS_FWF] Invalid projection VLR meta-data. Can't "
      ...
      // additional fields
      {
        for (ExtraLasField &f : extraFieldsToSave) {
          ...
          LASattribute attribute(
              f.isShifted || sizeof(ScalarType) == 8 ? LAS_ATTRIBUTE_F64
                                                     : LAS_ATTRIBUTE_F32,
              qPrintable(f.sanitizedName), "additional attributes");
          lasheader.point_data_record_length +=
              (attribute.data_type == LAS_ATTRIBUTE_F32 + 1
                   ? 4
                   : 8); // strangely, LASlib shifts the official type indexes
                         // :|
          ...
          // U8* data = new U8[192];
          // memset(data, 0, 192);
          // data[2] = (f.isShifted || sizeof(ScalarType) == 8 ? 10 : 9);
          // //double if shifted or ScalarType is also double, float otherwise
          // data[3] = 0; //no options
          // assert(f.sanitizedName.length() <= 32);
          // strncpy(reinterpret_cast<char*>(data + 4),
          // qPrintable(f.sanitizedName), f.sanitizedName.length()); //name
          // strncpy(reinterpret_cast<char*>(data + 160), "CloudCompare scalar
          // field", 24); //description lasheader.add_vlr("LASF_Spec", 4, 192,
          // data, TRUE);
        }
    ...
    if (parameters.parentWidget) {
      progressDialog.reset(
          new ccProgressDialog(false, parameters.parentWidget));
    ...
    CCCoreLib::NormalizedProgress nProgress(progressDialog.data(),
                                            cloud->size());
    ...
    for (unsigned i = 0; i < cloud->size(); ++i) {
      ...
      if (isShifted) {
        ...
        laspoint.set_X(static_cast<U32>((Pglobal.x - lasheader.x_offset) /
                                        lasheader.x_scale_factor));
        laspoint.set_Y(static_cast<U32>((Pglobal.y - lasheader.y_offset) /
                                        lasheader.y_scale_factor));
        laspoint.set_Z(static_cast<U32>((Pglobal.z - lasheader.z_offset) /
                                        lasheader.z_scale_factor));
      }

      // additional fields
      for (std::vector<LasField>::const_iterator it = fieldsToSave.begin();
        ...
        case LAS_RETURN_NUMBER:
          if (pointFormatSixOrAbove) {
            laspoint.set_extended_return_number(
                static_cast<U8>(it->getSafeValue(i)));
          } else {
        ...
        case LAS_NUMBER_OF_RETURNS:
          if (pointFormatSixOrAbove) {
            laspoint.set_extended_number_of_returns(
                static_cast<U8>(it->getSafeValue(i)));
          } else {
        ...
        case LAS_CLASSIFICATION:
          if (pointFormatSixOrAbove) {
            laspoint.set_extended_classification(
                static_cast<U8>(it->getSafeValue(i)));
          } else {
        ...
        case LAS_SCAN_ANGLE_RANK:
          if (pointFormatSixOrAbove) {
            laspoint.set_extended_scan_angle(
                static_cast<I16>(it->getSafeValue(i)));
          } else {
        ...
        case LAS_CLASSIF_VALUE:
          if (pointFormatSixOrAbove) {
            laspoint.set_extended_classification(
                static_cast<U8>(it->getSafeValue(i))); // 8 bits
          } else {
            laspoint.set_classification(static_cast<U8>(it->getSafeValue(i)) &
                                        31); // 5 first bits
          }
          break;
      ...
      if (hasFWF) {
        ...
        if (proxy.isValid()) {
          ...
          laspoint.wavepacket.setOffset(static_cast<U64>(w.dataOffset()) +
                                        60); // EVLR header
...
bool PrepareLASField(ccScalarField *&sf, LasField *lasField,
                     unsigned totalCount, unsigned currentCount,
  ...
  if (!sf->reserveSafe(totalCount)) {
    ccLog::Warning(QString("[LAS] Not enough memory to load a field: '%1'")
                       .arg(sf->getName()));
    sf->release();
    sf = nullptr;
    return false;
...
CC_FILE_ERROR LASFWFFilter::loadFile(const QString &filename,
  ...
  try {
    ...
    ccLog::Print(QString("[LASLib] File version: %1.%2")
                     .arg(lasreader.header.version_major)
                     .arg(lasreader.header.version_minor));
    ccLog::Print(QString("[LASLib] Point format: %1")
                     .arg(lasreader.header.point_data_format));
    const bool pointFormatSixOrAbove =
        (lasreader.header.point_data_format >= 6);

    unsigned pointCount = static_cast<unsigned>(lasreader.npoints);
    ccLog::Print(QString("[LASLib] " +
    ...
    if (parameters.parentWidget) {
      ...
      progressDialog->setRange(
          0,
          0 /*static_cast<int>(pointCount)*/); // DGM FIXME: the progress
                                               // doesn't update! Is it because
                                               // it's in a separate DLL/plugin?
      progressDialog->show();
      QCoreApplication::processEvents();
    }
    ...
    std::vector<LasField::Shared> fieldsToLoad;
    {
      if (pointFormatSixOrAbove) {
        ...
        fieldsToLoad.push_back(LasField::Shared(
            new LasField(LAS_CLASSIF_OVERLAP, 0, 0, 1))); // 1 bit: 0 or 1
      } else {
        fieldsToLoad.push_back(LasField::Shared(new LasField(
            LAS_CLASSIF_VALUE, 0, 0, 31))); // 5 bits: between 0 and 31
      }
      fieldsToLoad.push_back(LasField::Shared(
          new LasField(LAS_CLASSIF_SYNTHETIC, 0, 0, 1))); // 1 bit: 0 or 1
      fieldsToLoad.push_back(LasField::Shared(
          new LasField(LAS_CLASSIF_KEYPOINT, 0, 0, 1))); // 1 bit: 0 or 1
      fieldsToLoad.push_back(LasField::Shared(
          new LasField(LAS_CLASSIF_WITHHELD, 0, 0, 1))); // 1 bit: 0 or 1
      fieldsToLoad.push_back(LasField::Shared(new LasField(
          LAS_INTENSITY, 0, 0, 65535))); // 16 bits: between 0 and 65536
      fieldsToLoad.push_back(LasField::Shared(new LasField(
          LAS_TIME, 0, 0, -1.0))); // 8 bytes (double) --> we use global shift!
      fieldsToLoad.push_back(LasField::Shared(
          new LasField(LAS_RETURN_NUMBER, 1, 1, 7))); // 3 bits: between 1 and 7
      fieldsToLoad.push_back(LasField::Shared(new LasField(
          LAS_NUMBER_OF_RETURNS, 1, 1, 7))); // 3 bits: between 1 and 7
      fieldsToLoad.push_back(LasField::Shared(
          new LasField(LAS_SCAN_DIRECTION, 0, 0, 1))); // 1 bit: 0 or 1
      fieldsToLoad.push_back(LasField::Shared(
          new LasField(LAS_FLIGHT_LINE_EDGE, 0, 0, 1))); // 1 bit: 0 or 1
      fieldsToLoad.push_back(LasField::Shared(
          new LasField(LAS_SCAN_ANGLE_RANK, 0, -90,
                       90))); // signed char: between -90 and +90
      fieldsToLoad.push_back(LasField::Shared(new LasField(
          LAS_USER_DATA, 0, 0, 255))); // unsigned char: between 0 and 255
      fieldsToLoad.push_back(LasField::Shared(new LasField(
          LAS_POINT_SOURCE_ID, 0, 0, 65535))); // 16 bits: between 0 and 65536
    }
    ...
    for (int fakeIteration = 0; hasFWF && fakeIteration < 1; ++fakeIteration) {
      try {
        ...
        ccLog::Warning(
            QString("Not enough memory to import the waveform data"));
        hasFWF = false;
        break;
      }
      ...
      if (lasreader.header.start_of_waveform_data_packet_record != 0) {
        ...
        if (!fwfDataSource.open(QFile::ReadOnly)) {
          ccLog::Warning(
              QString("Failed to read the associated waveform data packets"));
          hasFWF = false;
          break;
        }
        // seek for the waveform EVLR
        if (!fwfDataSource.seek(
                lasreader.header.start_of_waveform_data_packet_record)) {
          ccLog::Warning(QString(
              "Failed to find the associated waveform data packets header"));
          hasFWF = false;
          break;
        }
        ...
        unsigned short reserved = *reinterpret_cast<const unsigned short *>(
            evlrHeader.constData() + 0);
        // char userID[16];
        // memcpy(userID, evlrHeader.constData() + 2, 16);
        unsigned short recordID = *reinterpret_cast<const unsigned short *>(
        ...
        if (fwfDataCount == 0) {
          ccLog::Warning(QString("Invalid waveform data packet size (0). We'll "
                                 "load all the remaining part of the file!"));
        ...
        if (!fwfDataSource.open(QFile::ReadOnly)) {
          ccLog::Warning(QString("Failed to read the associated waveform data "
                                 "packets file (looking for '%1')")
                             .arg(wdpFilename));
          hasFWF = false;
          break;
        }
    ...
    CCVector3d lasOffset =
        CCVector3d(lasreader.header.x_offset, lasreader.header.y_offset,
    ...
    for (size_t pointIndex = 0; lasreader.read_point(); ++pointIndex) {
        ...
        // if (fwfReader->read_waveform(&point))
        {
          ...
          if (!descriptors.contains(packetIndex)) {
            LASvlr_wave_packet_descr *descriptor =
                lasreader.header.vlr_wave_packet_descr[packetIndex];
      ...
      if (cloud->size() == 0) // first point
      {
        ...
        if (lasOffsetXY.norm2() != 0 &&
            ((nullptr == parameters._coordinatesShiftEnabled) ||
             (false == *parameters._coordinatesShiftEnabled))) {
          if (csModeBackup !=
        ...
        if (hasColors) {
          if (colorBitDec == 0) {
            ...
            if (mergedColorComp > 255) {
              ...
              // As soon as we detect a value higher than 255, we shift to 16
              // bits mode!
              colorBitDec = 8;
              for (unsigned i = 0; i < cloud->size(); ++i) {
          ...
          ccColor::Rgb color(
              static_cast<unsigned char>((point.rgb[0] >> colorBitDec) & 255),
      ...
      for (LasField::Shared &field : fieldsToLoad) {
        ...
        case LAS_CLASSIFICATION:
          if (pointFormatSixOrAbove) {
            ...
            // warning: compared to the other LAS filters, the
            // 'LAS_CLASSIFICATION' field corresponds to the full 8 bits (for
            // point format < 6)
        ...
        case LAS_CLASSIF_VALUE:
          if (pointFormatSixOrAbove)
            value = static_cast<double>(
                point.get_extended_classification()); // 8 bits
          else
            value =
                static_cast<double>(point.get_classification() & 31); // 5 bits
          break;
        case LAS_CLASSIF_SYNTHETIC:
          if (pointFormatSixOrAbove)
            value =
                static_cast<double>(point.get_synthetic_flag()
                                    << 5); // shift the value so as to give the
                                           // same result as with older versions
          else
            value =
                static_cast<double>(point.get_classification() & 32); // bit #6
          break;
        case LAS_CLASSIF_KEYPOINT:
          if (pointFormatSixOrAbove)
            value =
                static_cast<double>(point.get_keypoint_flag()
                                    << 6); // shift the value so as to give the
                                           // same result as with older versions
          else
            value =
                static_cast<double>(point.get_classification() & 64); // bit #7
          break;
        case LAS_CLASSIF_WITHHELD:
          if (pointFormatSixOrAbove)
            value =
                static_cast<double>(point.get_withheld_flag()
                                    << 7); // shift the value so as to give the
                                           // same result as with older versions
          else
            value =
                static_cast<double>(point.get_classification() & 128); // bit #8
          break;
        case LAS_CLASSIF_OVERLAP:
          if (pointFormatSixOrAbove)
        ...
        case LAS_EXTRA:
          value = point.get_attribute_as_float(
              static_cast<ExtraLasField *>(field.data())->startIndex);
          break;
        default:
          // ignored
          continue;
            ...
            if (PrepareLASField(field->sf, field.data(), pointCount,
              ...
              if (field->type == LAS_TIME ||
                  (field->type == LAS_EXTRA &&
                   static_cast<ExtraLasField *>(field.data())->isShifted)) {
                // we use the first value as 'global shift' (otherwise we will
                // lose accuracy)
                double timeShift =
                    static_cast<int64_t>(field->firstValue / 10000.0) * 10000.0;
                field->sf->setGlobalShift(timeShift);
                value -= timeShift;
                ccLog::Warning("[LAS] Time SF has been shifted to prevent a "
                               "loss of accuracy (%.2f)",
                               timeShift);
                field->firstValue = value;
              }
              ...
              ccLog::Warning(
                  QString(
                      "[LAS] Not enough memory: '%1' field will be ignored!")
    ...
    for (uint32_t i = 0; i < lasreader.header.number_of_variable_length_records;
         ++i) {
      ...
      if (QString(vlr.user_id) == ProjectionVLR) {
        ...
        if (buffer.size() != 0) {
          ...
          ccLog::Warning("[LAS] Failed to save projection information (not "
                         "enough memory)");
        }
        break;
      }
      ...
      for (LasField::Shared &field : fieldsToLoad) {
        if (field->sf) {
          ...
          if (field->type == LAS_CLASSIFICATION ||
            ...
            field->sf->setColorScale(ccColorScalesManager::GetDefaultScale(
          ...
          ccLog::Warning(QString("[LAS] All '%1' values were the same (%2)! We "
                                 "ignored them...")
                             .arg(field->type == LAS_EXTRA
                                      ? field->getName()
                                      : QString(LAS_FIELD_NAMES[field->type]))

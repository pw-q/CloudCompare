...
// First:
//	Replace all occurrences of 'ExamplePlugin' by your own plugin class name
//in this file. 	This includes the resource path to info.json in the constructor.

// Second:
//	Open ExamplePlugin.qrc, change the "prefix" and the icon filename for
...
//file) 	 "description" is used as a tootip if the plugin has actions and is
//displayed in the plugin dialog 	 "authors", "maintainers", and "references" show
//up in the plugin dialog as well
...
QList<QAction *> ExamplePlugin::getActions() {
  ...
  if (!m_action) {
    ...
    connect(m_action, &QAction::triggered, this,
            [this]() { Example::performActionA(m_app); });
  }

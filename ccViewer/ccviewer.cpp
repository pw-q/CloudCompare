...
ccViewer::ccViewer(QWidget *parent, Qt::WindowFlags flags)
    : QMainWindow(parent, flags), m_glWindow(nullptr),
      m_selectedObject(nullptr), m_3dMouseInput(nullptr),
  ...
  setStyleSheet(
      "QStatusBar{background-color: qlineargradient(x1:0, y1:0, x2:0, "
      "y2:1,stop:0 rgb(200,200,200), stop:1 rgb(255,255,255));}");
#endif
  ...
  m_glWindow->setRectangularPickingAllowed(
      false); // multiple entities picking not supported
  ...
  ui.menuOptions->insertMenu(ui.menu3DMouse->menuAction(),
                             m_gamepadManager->menu());
#endif

  // Signals & slots connection
  connect(m_glWindow->signalEmitter(), &ccGLWindowSignalEmitter::filesDropped,
          this, qOverload<QStringList>(&ccViewer::addToDB),
          Qt::QueuedConnection);
  connect(m_glWindow->signalEmitter(),
          &ccGLWindowSignalEmitter::entitySelectionChanged, this,
          &ccViewer::selectEntity);
  connect(m_glWindow->signalEmitter(),
          &ccGLWindowSignalEmitter::exclusiveFullScreenToggled, this,
          &ccViewer::onExclusiveFullScreenToggled);

  //"Options" menu
  connect(ui.actionDisplayParameters, &QAction::triggered, this,
          &ccViewer::showDisplayParameters);
  connect(ui.actionEditCamera, &QAction::triggered, this,
          &ccViewer::doActionEditCamera);
  //"Display > Standard views" menu
  connect(ui.actionSetViewTop, &QAction::triggered, this,
          &ccViewer::setTopView);
  connect(ui.actionSetViewBottom, &QAction::triggered, this,
          &ccViewer::setBottomView);
  connect(ui.actionSetViewFront, &QAction::triggered, this,
          &ccViewer::setFrontView);
  connect(ui.actionSetViewBack, &QAction::triggered, this,
          &ccViewer::setBackView);
  connect(ui.actionSetViewLeft, &QAction::triggered, this,
          &ccViewer::setLeftView);
  connect(ui.actionSetViewRight, &QAction::triggered, this,
          &ccViewer::setRightView);
  connect(ui.actionSetViewIso1, &QAction::triggered, this,
          &ccViewer::setIsoView1);
  connect(ui.actionSetViewIso2, &QAction::triggered, this,
          &ccViewer::setIsoView2);

  //"Options > Perspective" menu
  connect(ui.actionSetOrthoView, &QAction::triggered, this,
          &ccViewer::setOrthoView);
  connect(ui.actionSetCenteredPerspectiveView, &QAction::triggered, this,
          &ccViewer::setCenteredPerspectiveView);
  connect(ui.actionSetViewerPerspectiveView, &QAction::triggered, this,
          &ccViewer::setViewerPerspectiveView);
  //"Options > Rotation symbol" menu
  connect(ui.actionSetPivotAlwaysOn, &QAction::triggered, this,
          &ccViewer::setPivotAlwaysOn);
  connect(ui.actionSetPivotRotationOnly, &QAction::triggered, this,
          &ccViewer::setPivotRotationOnly);
  connect(ui.actionSetPivotOff, &QAction::triggered, this,
          &ccViewer::setPivotOff);
  //"Options > 3D mouse" menu
  connect(ui.actionEnable3DMouse, &QAction::toggled, this,
          &ccViewer::enable3DMouse);
  //"Display > Lights & Materials" menu
  connect(ui.actionToggleSunLight, &QAction::toggled, this,
          &ccViewer::toggleSunLight);
  connect(ui.actionToggleCustomLight, &QAction::toggled, this,
          &ccViewer::toggleCustomLight);
  //"Options" menu
  connect(ui.actionGlobalZoom, &QAction::triggered, this,
          &ccViewer::setGlobalZoom);
  connect(ui.actionEnableStereo, &QAction::toggled, this,
          &ccViewer::toggleStereoMode);
  connect(ui.actionFullScreen, &QAction::toggled, this,
          &ccViewer::toggleFullScreen);
  connect(ui.actionLockRotationVertAxis, &QAction::triggered, this,
          &ccViewer::toggleRotationAboutVertAxis);

  //"Options > Selected" menu
  connect(ui.actionShowColors, &QAction::toggled, this,
          &ccViewer::toggleColorsShown);
  connect(ui.actionShowNormals, &QAction::toggled, this,
          &ccViewer::toggleNormalsShown);
  connect(ui.actionShowMaterials, &QAction::toggled, this,
          &ccViewer::toggleMaterialsShown);
  connect(ui.actionShowScalarField, &QAction::toggled, this,
          &ccViewer::toggleScalarShown);
  connect(ui.actionShowColorRamp, &QAction::toggled, this,
          &ccViewer::toggleColorbarShown);
  connect(ui.actionZoomOnSelectedEntity, &QAction::triggered, this,
          &ccViewer::zoomOnSelectedEntity);
  connect(ui.actionDelete, &QAction::triggered, this,
          &ccViewer::doActionDeleteSelectedEntity);

  //"Shaders" menu
  connect(ui.actionNoFilter, &QAction::triggered, this,
          &ccViewer::doDisableGLFilter);

  //"Help" menu
  connect(ui.actionAbout, &QAction::triggered, this, &ccViewer::doActionAbout);
  connect(ui.actionHelpShortcuts, &QAction::triggered, this,
          &ccViewer::doActionDisplayShortcuts);
...
void ccViewer::loadPlugins() {
  ...
  for (ccPluginInterface *plugin : ccPluginManager::Get().pluginList()) {
    ...
    if (plugin->getType() == CC_GL_FILTER_PLUGIN) {
      ccGLPluginInterface *glPlugin =
          static_cast<ccGLPluginInterface *>(plugin);
      ...
      ccLog::Print(
          QStringLiteral("Plugin name: [%1] (GL filter)").arg(pluginName));

      QAction *action = new QAction(pluginName, this);
      action->setToolTip(glPlugin->getDescription());
      action->setIcon(glPlugin->getIcon());

      // store the plugin's interface pointer in the QAction data so we can
      // access it in doEnableGLFilter()
      QVariant v;
      v.setValue(glPlugin);
...
void ccViewer::doEnableGLFilter() {
  ...
  if (filter != nullptr) {
    if (m_glWindow->areGLFiltersEnabled()) {
      ...
      ccLog::Print("Note: go to << Display > Shaders & Filters > No filter >> "
                   "to disable GL filter");
    } else {
...
void ccViewer::doActionDeleteSelectedEntity() {
  ...
  while (!toCheck.empty()) {
    ...
    if (obj->isSelected()) {
      if (obj->getParent()) {
        obj->getParent()->addDependency(
            obj, ccHObject::DP_DELETE_OTHER); // we force deletion!
...
void ccViewer::selectEntity(ccHObject *toSelect) {
  ...
  if (toSelect) {
    ...
    ui.actionShowColorRamp->setChecked(
        cloud ? cloud->sfColorScaleShown() && cloud->sfShown() : false);
    ...
    if (hasSF && sfCount > 1) {
      ...
      for (unsigned i = 0; i < sfCount; ++i) {
        QAction *action = ui.menuSelectSF->addAction(
            QString::fromStdString(cloud->getScalarFieldName(i)));
        action->setData(i);
        action->setCheckable(true);
        if (currentSFIndex == static_cast<int>(i))
          action->setChecked(true);
        connect(action, &QAction::toggled, this,
                &ccViewer::changeCurrentScalarField);
      }
    }
...
bool ccViewer::checkForLoadedEntities() {
  ...
  m_glWindow->displayNewMessage(
      QString(),
      ccGLWindowInterface::SCREEN_CENTER_MESSAGE); // clear (any) message in the
                                                   // middle area

  if (!m_glWindow->getSceneDB()) {
    m_glWindow->displayNewMessage(
        "Drag & drop files on the 3D window to load them!",
        ccGLWindowInterface::SCREEN_CENTER_MESSAGE, true, 3600);
...
void ccViewer::updateGLFrameGradient() {
  ...
  const ccColor::Rgbub &bkgCol =
      stereoModeEnabled ? s_black
                        : m_glWindow->getDisplayParameters().backgroundCol;
  const ccColor::Rgbub &forCol =
      stereoModeEnabled ? s_white
                        : m_glWindow->getDisplayParameters().pointsDefaultCol;

  QString styleSheet =
      QString("QFrame#GLframe{border: 2px solid white; border-radius: 10px; "
...
ccHObject *ccViewer::addToDB(QStringList filenames) {
  ...
  for (int i = 0; i < filenames.size(); ++i) {
    ...
    ccHObject *newGroup =
        FileIOFilter::LoadFromFile(filenames[i], parameters, result);
...
void ccViewer::addToDB(ccHObject *entity, bool updateZoom /*=false*/,
...
void ccViewer::showDisplayParameters() {
  ...
  connect(&clmDlg, &ccDisplaySettingsDlg::aspectHasChanged, this,
          &ccViewer::updateDisplay);
...
void ccViewer::reflectPerspectiveState() {
  ...
  ui.actionSetCenteredPerspectiveView->setChecked(perspectiveEnabled &&
                                                  objectCentered);
  ui.actionSetViewerPerspectiveView->setChecked(perspectiveEnabled &&
                                                !objectCentered);
}

bool ccViewer::checkStereoMode() {
  if (m_glWindow && m_glWindow->getViewportParameters().perspectiveView &&
      m_glWindow->stereoModeIsEnabled()) {
    if (QMessageBox::question(this, "Stereo mode",
                              "Stereo-mode only works in perspective mode. Do "
...
void ccViewer::reflectPivotVisibilityState() {
  ...
  ui.actionSetPivotAlwaysOn->setChecked(vis ==
                                        ccGLWindowInterface::PIVOT_ALWAYS_SHOW);
  ui.actionSetPivotRotationOnly->setChecked(
      vis == ccGLWindowInterface::PIVOT_SHOW_ON_MOVE);
...
void ccViewer::toggleStereoMode(bool state) {
  ...
  if (isActive) {
    ...
    if (m_glWindow->getStereoParams().glassType ==
            ccGLWindowInterface::StereoParams::NVIDIA_VISION ||
        m_glWindow->getStereoParams().glassType ==
  ...
  if (m_glWindow) {
    if (m_glWindow->stereoModeIsEnabled() &&
        (m_glWindow->getStereoParams().glassType ==
         ...
         m_glWindow->getStereoParams().glassType ==
             ccGLWindowInterface::StereoParams::GENERIC_STEREO_DISPLAY)) {
      // auto disable stereo mode as NVidia Vision only works in full screen
      // mode!
      ui.actionEnableStereo->setChecked(false);
    }
...
void ccViewer::onExclusiveFullScreenToggled(bool state) {
  ...
  ui.actionFullScreen->setChecked(m_glWindow ? m_glWindow->exclusiveFullScreen()
                                             : false);
  ui.actionFullScreen->blockSignals(false);

  if (!state && m_glWindow && m_glWindow->stereoModeIsEnabled() &&
      (m_glWindow->getStereoParams().glassType ==
           ccGLWindowInterface::StereoParams::NVIDIA_VISION ||
       m_glWindow->getStereoParams().glassType ==
           ccGLWindowInterface::StereoParams::GENERIC_STEREO_DISPLAY)) {
...
void ccViewer::doActionDisplayShortcuts() {
  ...
  // text += "Alt + left button hold: Select multiple entities (rectangular
  // area)\n";
...
void ccViewer::changeCurrentScalarField(bool state) {
  ...
  if (sfIndex < static_cast<int>(cloud->getNumberOfScalarFields())) {
    ...
    // when 'setCurrentDisplayedScalarField' is called, scalar field is
    // automatically shown!
    ui.actionShowScalarField->blockSignals(true);
...
void ccViewer::enable3DMouse(bool state) {
  ...
  if (state) {
    ...
    if (m_3dMouseInput->connect(this, "ccViewer")) {
      QObject::connect(m_3dMouseInput, &Mouse3DInput::sigMove3d, this,
                       &ccViewer::on3DMouseMove);
      QObject::connect(m_3dMouseInput, &Mouse3DInput::sigReleased, this,
                       &ccViewer::on3DMouseReleased);
      QObject::connect(m_3dMouseInput, &Mouse3DInput::sigOn3dmouseKeyDown, this,
                       &ccViewer::on3DMouseKeyDown);
      QObject::connect(m_3dMouseInput, &Mouse3DInput::sigOn3dmouseKeyUp, this,
                       &ccViewer::on3DMouseKeyUp);
      QObject::connect(m_3dMouseInput, &Mouse3DInput::sigOn3dmouseCMDKeyDown,
                       this, &ccViewer::on3DMouseCMDKeyDown);
      QObject::connect(m_3dMouseInput, &Mouse3DInput::sigOn3dmouseCMDKeyUp,
                       this, &ccViewer::on3DMouseCMDKeyUp);
    } else {
      delete m_3dMouseInput;
      m_3dMouseInput = 0;
...
// ANY CHANGE/BUG FIX SHOULD BE REFLECTED TO THE EQUIVALENT METHODS IN QCC
// "MainWindow.cpp" FILE!
...
void ccViewer::on3DMouseReleased() {
  // active window?
  if (m_glWindow && m_glWindow->getPivotVisibility() ==
                        ccGLWindowInterface::PIVOT_SHOW_ON_MOVE) {
...
void ccViewer::redrawAll(bool only2D /*=false*/) { m_glWindow->redraw(only2D); }
...
void ccViewer::toggleActiveWindowCustomLight() {
  ui.actionToggleCustomLight->setChecked(
      !ui.actionToggleCustomLight->isChecked());
}
...
void ccViewer::increasePointSize() {
  m_glWindow->setPointSize(
      m_glWindow->getViewportParameters().defaultPointSize + 1);
  m_glWindow->redraw();
}

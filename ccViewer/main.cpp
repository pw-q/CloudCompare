...
int main(int argc, char *argv[]) {
  ...
  // reading filenmaes from the command line)
...
#ifdef Q_OS_MAC
  // This makes sure that our "working directory" is not within the application
  // bundle
  ...
  if (!QGLFormat::hasOpenGL()) {
    QMessageBox::critical(nullptr, "Error",
                          "This application needs OpenGL to run!");
    return EXIT_FAILURE;
  }
  ...
  if (argc > 1) {
    ...
    while (i < argc) {
      ...
      if (upperArgument == "-WIN") {
        ...
        if (!ok) {
          ccLog::Warning("Invalid arguments! 4 integer values are expected "
                         "after '-win' (X Y W H)");
          break;
        }
        ...
        w.show();
      } else // any other argument is assumed to be a filename (that will we try
             // to load)
      {
        filenames << argument;
      }

...
ccScaleDlg::ccScaleDlg(QWidget *parent /*=nullptr*/)
    : QDialog(parent), m_ui(new Ui::ScaleDialog) {
  m_ui->setupUi(this);

  connect(m_ui->sameForAllCheckBox, &QCheckBox::toggled, this,
          &ccScaleDlg::allDimsAtOnceToggled);
  connect(m_ui->fxSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
...
CCVector3d ccScaleDlg::getScales() const {
  return CCVector3d(m_ui->fxSpinBox->value(), m_ui->fySpinBox->value(),

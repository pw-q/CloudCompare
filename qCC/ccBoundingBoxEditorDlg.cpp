...
ccBoundingBoxEditorDlg::ccBoundingBoxEditorDlg(bool showBoxAxes,
                                               bool showRasterGridImage,
                                               QWidget *parent /*=nullptr*/)
    : QDialog(parent, Qt::Tool), Ui::BoundingBoxEditorDialog(),
      m_baseBoxIsMinimal(false), m_showInclusionWarning(true) {
  setupUi(this);

  oriGroupBox->setVisible(showBoxAxes);
  rasterGridImageLabel->setVisible(showRasterGridImage);
  resize(QSize(width(),
               computeBestDialogHeight(showBoxAxes, showRasterGridImage)));
  ...
  connect(pointTypeComboBox, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &ccBoundingBoxEditorDlg::reflectChanges);

  connect(xDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccBoundingBoxEditorDlg::updateCurrentBBox);
  connect(yDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccBoundingBoxEditorDlg::updateCurrentBBox);
  connect(zDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccBoundingBoxEditorDlg::updateCurrentBBox);

  connect(dxDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccBoundingBoxEditorDlg::updateXWidth);
  connect(dyDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccBoundingBoxEditorDlg::updateYWidth);
  connect(dzDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccBoundingBoxEditorDlg::updateZWidth);

  connect(xOriXDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccBoundingBoxEditorDlg::onAxisValueChanged);
  connect(xOriYDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccBoundingBoxEditorDlg::onAxisValueChanged);
  connect(xOriZDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccBoundingBoxEditorDlg::onAxisValueChanged);
  connect(yOriXDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccBoundingBoxEditorDlg::onAxisValueChanged);
  connect(yOriYDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccBoundingBoxEditorDlg::onAxisValueChanged);
  connect(yOriZDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccBoundingBoxEditorDlg::onAxisValueChanged);
  connect(zOriXDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccBoundingBoxEditorDlg::onAxisValueChanged);
  connect(zOriYDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccBoundingBoxEditorDlg::onAxisValueChanged);
  connect(zOriZDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccBoundingBoxEditorDlg::onAxisValueChanged);
...
void ccBoundingBoxEditorDlg::saveBoxAndAccept() {
  if (oriGroupBox->isVisible()) {
    ...
    if (X.norm2d() == 0 || Y.norm2d() == 0 || Z.norm2d() == 0) {
      ccLog::Error(
          "Invalid axes definition: at least two vectors are colinear");
      return;
    }
...
void ccBoundingBoxEditorDlg::reflectChanges(int dummy) {
  ...
  // right column
  {
    ...
    assert(!keepSquare() || std::abs(W.x - W.y) * 1.0e-6 < 1.0 &&
                                std::abs(W.x - W.z) * 1.0e-6 < 1.0);
...
int ccBoundingBoxEditorDlg::computeBestDialogHeight(
    bool showBoxAxes, bool showRasterGridImage) const {
...
void ccBoundingBoxEditorDlg::setBoxAxes(const CCVector3 &X, const CCVector3 &Y,
                                        const CCVector3 &Z) {
...
void ccBoundingBoxEditorDlg::getBoxAxes(CCVector3d &X, CCVector3d &Y,
                                        CCVector3d &Z) {
  ...
  if (clipboard) {
    ...
    if (!clipText.isEmpty()) {
      ...
      if (success) {
        ...
        reflectChanges();
        // change axes
        setBoxAxes(matrix.getColumnAsVec3D(0), matrix.getColumnAsVec3D(1),

...
static void MakeComboBoxOptionInaccessible(QComboBox *comboBox, int index) {
  ...
  const QStandardItemModel *model =
      qobject_cast<const QStandardItemModel *>(comboBox->model());
  QStandardItem *item = model ? model->item(index) : 0;
  if (item) {
    item->setFlags(item->flags() & ~(Qt::ItemIsSelectable | Qt::ItemIsEnabled));
    // visually disable by greying out - works only if combobox has been painted
    // already and palette returns the wanted color
    item->setData(
        comboBox->palette().color(QPalette::Disabled, QPalette::Text),
        Qt::TextColorRole); // clear item data in order to use default color
  }
}

ccRasterizeTool::ccRasterizeTool(ccGenericPointCloud *cloud, QWidget *parent)
    : QDialog(parent, Qt::WindowMaximizeButtonHint | Qt::WindowCloseButtonHint),
      cc2Point5DimEditor(), m_UI(new Ui::RasterizeToolDialog), m_cloud(cloud),
  ...
  if (gridBBox.isValid()) {
    ...
    connect(m_UI->editGridToolButton, &QAbstractButton::clicked, this,
  ...
  connect(m_UI->gridStepDoubleSpinBox,
          ...
          &ccRasterizeTool::updateGridInfo);
  connect(m_UI->gridStepDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccRasterizeTool::gridOptionChanged);
  connect(m_UI->emptyValueDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccRasterizeTool::gridOptionChanged);

  connect(m_UI->dimensionComboBox,
          qOverload<int>(&QComboBox::currentIndexChanged), this,
          &ccRasterizeTool::projectionDirChanged);
  connect(m_UI->heightProjectionComboBox,
          qOverload<int>(&QComboBox::currentIndexChanged), this,
          &ccRasterizeTool::projectionTypeChanged);
  connect(m_UI->scalarFieldProjection,
          qOverload<int>(&QComboBox::currentIndexChanged), this,
          &ccRasterizeTool::sfProjectionTypeChanged);
  connect(m_UI->fillEmptyCellsComboBox,
          qOverload<int>(&QComboBox::currentIndexChanged), this,
          &ccRasterizeTool::fillEmptyCellStrategyChanged);
  connect(m_UI->stdDevLayerComboBox,
          qOverload<int>(&QComboBox::currentIndexChanged), this,
          &ccRasterizeTool::stdDevLayerChanged);
  connect(m_UI->activeLayerComboBox,
          qOverload<int>(&QComboBox::currentIndexChanged), this,
          [this](int index) { activeLayerChanged(index); });

  connect(m_UI->resampleCloudCheckBox, &QAbstractButton::toggled, this,
          &ccRasterizeTool::resampleOptionToggled);
  connect(m_UI->updateGridPushButton, &QAbstractButton::clicked, this,
          &ccRasterizeTool::updateGridAndDisplay);
  connect(m_UI->generateCloudPushButton, &QAbstractButton::clicked, this,
          [this]() { generateCloud(true); });
  connect(m_UI->generateImagePushButton, &QAbstractButton::clicked, this,
          &ccRasterizeTool::generateImage);
  connect(m_UI->generateRasterPushButton, &QAbstractButton::clicked, this,
          &ccRasterizeTool::generateRaster);
  connect(m_UI->generateASCIIPushButton, &QAbstractButton::clicked, this,
          &ccRasterizeTool::generateASCIIMatrix);
  connect(m_UI->generateMeshPushButton, &QAbstractButton::clicked, this,
          &ccRasterizeTool::generateMesh);
  connect(m_UI->generateContoursPushButton, &QAbstractButton::clicked, this,
          &ccRasterizeTool::generateContours);
  connect(m_UI->exportContoursPushButton, &QAbstractButton::clicked, this,
          &ccRasterizeTool::exportContourLines);
  connect(m_UI->clearContoursPushButton, &QAbstractButton::clicked, this,
          &ccRasterizeTool::removeContourLines);
  connect(m_UI->generateHillshadePushButton, &QAbstractButton::clicked, this,
          &ccRasterizeTool::generateHillshade);
  connect(m_UI->interpParamsToolButton, &QAbstractButton::clicked, this,
          &ccRasterizeTool::showInterpolationParamsDialog);

  connect(m_UI->exportHeightStatsCheckBox, &QCheckBox::toggled, this,
          &ccRasterizeTool::onStatExportTargetChanged);
  connect(m_UI->exportSFStatsCheckBox, &QCheckBox::toggled, this,
          &ccRasterizeTool::onStatExportTargetChanged);

  if (m_cloud) {
    // populate layer box
    m_UI->activeLayerComboBox->addItem(
    ...
    if (cloud->isA(CC_TYPES::POINT_CLOUD) && cloud->hasScalarFields()) {
      ...
      for (unsigned i = 0; i < pc->getNumberOfScalarFields(); ++i) {
        m_UI->activeLayerComboBox->addItem(
            QString::fromStdString(pc->getScalarField(i)->getName()),
            QVariant(LAYER_SF));
        // populate std. dev. layer box as well
        m_UI->stdDevLayerComboBox->addItem(
            QString::fromStdString(pc->getScalarField(i)->getName()));
      }
  ...
  if (!m_cloudHasScalarFields) {
    ...
    MakeComboBoxOptionInaccessible(m_UI->heightProjectionComboBox,
                                   ccRasterGrid::PROJ_INVERSE_VAR_VALUE);
    MakeComboBoxOptionInaccessible(m_UI->scalarFieldProjection,
                                   ccRasterGrid::PROJ_INVERSE_VAR_VALUE);
  }

  m_UI->projectSFCheckBox->setEnabled(m_cloudHasScalarFields);
  m_UI->scalarFieldProjection->setEnabled(m_cloudHasScalarFields);
  m_UI->stdDevLayerComboBox->setEnabled(
      m_cloudHasScalarFields); // real state will be set later (-->
...
void ccRasterizeTool::updateCloudName(bool withNonEmptyCells) {
  QString str;
  if (m_cloud) {
    str = QString("<b>%1</b> (%2 points")
              .arg(m_cloud->getName(),
                   QLocale::system().toString(m_cloud->size()));
    if (withNonEmptyCells)
      str += QString(" - %1 non-empty cells)")
                 .arg(QLocale::system().toString(m_grid.validCellCount));
    else
      str += ')';
  } else {
    str = "No cloud loaded";
...
void ccRasterizeTool::getExportedStats(
    std::vector<ccRasterGrid::ExportableFields> &stats) const {
  stats.clear();
...
bool ccRasterizeTool::resampleOriginalCloud() const {
  return m_UI->resampleCloudCheckBox->isEnabled() &&
...
void ccRasterizeTool::updateStdDevLayerComboBox() {
  m_UI->stdDevLayerComboBox->setEnabled(
      m_UI->stdDevLayerComboBox->count() != 0 &&
      (m_UI->heightProjectionComboBox->currentIndex() ==
           ccRasterGrid::PROJ_INVERSE_VAR_VALUE ||
       m_UI->scalarFieldProjection->currentIndex() ==
           ccRasterGrid::PROJ_INVERSE_VAR_VALUE));
}

void ccRasterizeTool::projectionTypeChanged(int index) {
  // we can't use the 'resample origin cloud' option with 'average height'
  // projection resampleCloudCheckBox->setEnabled(index != PROJ_AVERAGE_VALUE &&
  // index != PROJ_INVERSE_VAR_VALUE); DGM: now we can! We simply display a
  // warning message
  m_UI->warningResampleWithAverageLabel->setVisible(
      m_UI->resampleCloudCheckBox->isChecked() &&
...
void ccRasterizeTool::stdDevLayerChanged(int index) { gridIsUpToDate(false); }

void ccRasterizeTool::activeLayerChanged(int layerIndex,
                                         bool autoRedraw /*=true*/) {
  if (m_UI->activeLayerComboBox->itemData(layerIndex).toInt() == LAYER_SF) {
    if (m_UI->activeLayerComboBox->itemText(layerIndex) !=
        HILLSHADE_FIELD_NAME) {
      m_UI->projectSFCheckBox->setChecked(
      ...
      // m_UI->projectSFCheckBox->setChecked(false); //we shouldn't change that
      // as it only impacts the other fields
      // m_UI->projectSFCheckBox->setEnabled(false);
    ...
    // m_UI->projectSFCheckBox->setChecked(false); //DGM: we can't force that,
    // just let the user decide
    m_UI->projectSFCheckBox->setEnabled(
        m_cloudHasScalarFields); // we need SF fields!
    m_UI->generateImagePushButton->setEnabled(true);
  ...
  if (m_rasterCloud) {
    ...
    if (m_UI->activeLayerComboBox->currentData().toInt() == LAYER_RGB) {
      ...
      int sfIndex = m_rasterCloud->getScalarFieldIndexByName(
          m_UI->activeLayerComboBox->itemText(layerIndex).toStdString());
      ...
      if (sfIndex >= 0) {
        ...
        if (activeLayer) {
          ...
          m_UI->gridLayerRangeLabel->setText(QString("%1 [%2 ; %3]")
...
void ccRasterizeTool::fillEmptyCellStrategyChanged(int) {
  ...
  {
    m_UI->interpParamsToolButton->setEnabled(
        fillEmptyCellsStrategy == ccRasterGrid::INTERPOLATE_DELAUNAY ||
        fillEmptyCellsStrategy == ccRasterGrid::INTERPOLATE_KRIGING);
  }
...
ccRasterGrid::ProjectionType ccRasterizeTool::getTypeOfSFProjection() const {
  if (/*!m_UI->projectSFCheckBox->isEnabled() || */ !m_UI->projectSFCheckBox
          ->isChecked()) // DGM: the check-box might be disabled to actually
                         // 'force' the user to choose a projection type
  {
    return ccRasterGrid::INVALID_PROJECTION_TYPE; // means that we don't want to
                                                  // project SF values
  }
...
void ccRasterizeTool::loadSettings() {
  ...
  int projType = settings
                     ...
                     .toInt();
  int projDim =
      settings.value("ProjectionDim", m_UI->dimensionComboBox->currentIndex())
          .toInt();
  bool sfProj =
      settings.value("SfProjEnabled", m_UI->projectSFCheckBox->isChecked())
          .toBool();
  int sfProjStrategy =
      settings
          .value("SfProjStrategy", m_UI->scalarFieldProjection->currentIndex())
          .toInt();
  int fillStrategy =
      settings
          .value("FillStrategy", m_UI->fillEmptyCellsComboBox->currentIndex())
          .toInt();
  m_delaunayInterpParams.maxEdgeLength =
      settings.value("MaxEdgeLength", m_delaunayInterpParams.maxEdgeLength)
          .toDouble();
  m_krigingParams.kNN =
      settings.value("KrigingKNN", m_krigingParams.kNN).toDouble();
  double step = settings.value("GridStep", m_UI->gridStepDoubleSpinBox->value())
                    .toDouble();
  double emptyHeight =
      settings.value("EmptyCellsHeight", m_UI->emptyValueDoubleSpinBox->value())
          .toDouble();
  bool resampleCloud =
      settings
          .value("ResampleOrigCloud", m_UI->resampleCloudCheckBox->isChecked())
          .toBool();
  int minVertexCount =
      settings.value("MinVertexCount", m_UI->minVertexCountSpinBox->value())
          .toInt();
  bool ignoreBorders =
      settings
          .value("IgnoreBorders",
                 m_UI->ignoreContourBordersCheckBox->isChecked())
          .toBool();
  bool projectContoursOnAlt =
      settings
          .value("projectContoursOnAlt",
                 m_UI->projectContoursOnAltCheckBox->isChecked())
          .toBool();

  // Statistics checkboxes
  bool generateHeightStatistics =
      settings
          .value("GenerateHeightStatistics",
                 m_UI->exportHeightStatsCheckBox->isChecked())
          .toBool();
  bool generateSFStatistics =
      settings
          .value("GenerateSFStatistics",
                 m_UI->exportSFStatsCheckBox->isChecked())
          .toBool();
  bool generateStatisticsPopulation =
      settings
          .value("GenerateStatisticsPopulation",
                 m_UI->generateStatisticsPopulationCheckBox->isChecked())
          .toBool();
  bool generateStatisticsMin =
      settings
          .value("GenerateStatisticsMin",
                 m_UI->generateStatisticsMinCheckBox->isChecked())
          .toBool();
  bool generateStatisticsMax =
      settings
          .value("GenerateStatisticsMax",
                 m_UI->generateStatisticsMaxCheckBox->isChecked())
          .toBool();
  bool generateStatisticsAverage =
      settings
          .value("GenerateStatisticsAverage",
                 m_UI->generateStatisticsAverageCheckBox->isChecked())
          .toBool();
  bool generateStatisticsStdDev =
      settings
          .value("GenerateStatisticsStdDev",
                 m_UI->generateStatisticsStdDevCheckBox->isChecked())
          .toBool();
  bool generateStatisticsRange =
      settings
          .value("GenerateStatisticsRange",
                 m_UI->generateStatisticsRangeCheckBox->isChecked())
          .toBool();
  bool generateStatisticsMedian =
      settings
          .value("GenerateStatisticsMedian",
                 m_UI->generateStatisticsMedianCheckBox->isChecked())
          .toBool();
  bool generateStatisticsUnique =
      settings
          .value("GenerateStatisticsUnique",
                 m_UI->generateStatisticsUniqueCheckBox->isChecked())
          .toBool();
  bool generateStatisticsPercentile =
      settings
          .value("GenerateStatisticsPercentile",
                 m_UI->generateStatisticsPercentileCheckBox->isChecked())
          .toBool();
  double generateStatisticsPercentileValue =
      settings
          .value("GenerateStatisticsPercentileValue",
                 m_UI->generateStatisticsPercentileDoubleSpinBox->value())
          .toDouble();

  settings.endGroup();

  m_UI->gridStepDoubleSpinBox->setValue(step);
  m_UI->heightProjectionComboBox->setCurrentIndex(
      m_cloudHasScalarFields || projType != ccRasterGrid::PROJ_INVERSE_VAR_VALUE
          ? projType
          : 0);
  ...
  m_UI->scalarFieldProjection->setCurrentIndex(
      m_cloudHasScalarFields ||
              sfProjStrategy != ccRasterGrid::PROJ_INVERSE_VAR_VALUE
          ? sfProjStrategy
          : 0);
  ...
  m_UI->exportSFStatsCheckBox->setChecked(generateSFStatistics &&
                                          m_cloudHasScalarFields);
  m_UI->generateStatisticsPopulationCheckBox->setChecked(
      generateStatisticsPopulation);
  ...
  m_UI->generateStatisticsAverageCheckBox->setChecked(
      generateStatisticsAverage);
  ...
  m_UI->generateStatisticsPercentileCheckBox->setChecked(
      generateStatisticsPercentile);
  m_UI->generateStatisticsPercentileDoubleSpinBox->setValue(
      generateStatisticsPercentileValue);
}
...
bool ccRasterizeTool::canClose() {
  if (!m_contourLines.empty()) {
    ...
    if (QMessageBox::question(this, "Unsaved contour lines",
                              "Contour lines have not been exported! Do you "
                              "really want to close the tool?",
                              QMessageBox::Yes,
...
void ccRasterizeTool::saveSettings() {
  ...
  settings.setValue("SfProjStrategy",
                    m_UI->scalarFieldProjection->currentIndex());
  settings.setValue("FillStrategy",
                    m_UI->fillEmptyCellsComboBox->currentIndex());
  ...
  settings.setValue("ResampleOrigCloud",
                    m_UI->resampleCloudCheckBox->isChecked());
  ...
  // SF Statistics checkboxes
  settings.setValue("generateStatisticsPopulation",
                    m_UI->generateStatisticsPopulationCheckBox->isChecked());
  settings.setValue("generateStatisticsMin",
                    m_UI->generateStatisticsMinCheckBox->isChecked());
  settings.setValue("generateStatisticsMax",
                    m_UI->generateStatisticsMaxCheckBox->isChecked());
  settings.setValue("generateStatisticsAverage",
                    m_UI->generateStatisticsAverageCheckBox->isChecked());
  settings.setValue("generateStatisticsStdDev",
                    m_UI->generateStatisticsStdDevCheckBox->isChecked());
  settings.setValue("generateStatisticsRange",
                    m_UI->generateStatisticsRangeCheckBox->isChecked());
  settings.setValue("generateStatisticsMedian",
                    m_UI->generateStatisticsMedianCheckBox->isChecked());
  settings.setValue("generateStatisticsUnique",
                    m_UI->generateStatisticsUniqueCheckBox->isChecked());
  settings.setValue("generateStatisticsPercentile",
                    m_UI->generateStatisticsPercentileCheckBox->isChecked());
  settings.setValue("generateStatisticsPercentileValue",
                    m_UI->generateStatisticsPercentileDoubleSpinBox->value());
...
void ccRasterizeTool::gridIsUpToDate(bool state) {
  if (state) {
    ...
    // red button
    m_UI->updateGridPushButton->setStyleSheet(
...
ccPointCloud *ccRasterizeTool::convertGridToCloud(
    ...
    bool projectSFs, bool projectColors, bool copyHillshadeSF,
    const QString &activeSFName, double percentileValue,
  ...
  ccPointCloud *cloudGrid = cc2Point5DimEditor::convertGridToCloud(
      exportHeightStats, exportSFStats, exportedStatistics, projectSFs,
      projectColors,
      /*resampleInputCloudXY=*/resampleOriginalCloud(),
      /*resampleInputCloudZ=*/getTypeOfProjection() !=
          ccRasterGrid::PROJ_AVERAGE_VALUE,
      /*inputCloud=*/m_cloud, percentileValue, exportToOriginalCS,
      appendGridSizeToSFNames, progressDialog);
  ...
  if (cloudGrid) {
    ...
    if (copyHillshadeSF) {
      int hillshadeSFIdx =
          m_rasterCloud->getScalarFieldIndexByName(HILLSHADE_FIELD_NAME);
      if (hillshadeSFIdx >= 0) {
        CCCoreLib::ScalarField *hillshadeField =
        ...
        if (hillshadeField->currentSize() == cloudGrid->size()) {
          try {
            ...
            ccLog::Warning(
                "[Rasterize] Not enough memory to export the hillshade field");
          }
        }
      }
    }
    ...
    int activeSFIndex =
        cloudGrid->getScalarFieldIndexByName(activeSFName.toStdString());
...
void ccRasterizeTool::updateGridAndDisplay() {
  ...
  int hillshadeIndex =
      m_UI->activeLayerComboBox->findText(HILLSHADE_FIELD_NAME);
  if (hillshadeIndex >= 0) {
    if (m_UI->activeLayerComboBox->currentIndex() == hillshadeIndex &&
        m_UI->activeLayerComboBox->count() > 1) {
  ...
  bool activeLayerIsSF =
      (m_UI->activeLayerComboBox->currentData().toInt() == LAYER_SF);
  bool projectSFs =
      (getTypeOfSFProjection() != ccRasterGrid::INVALID_PROJECTION_TYPE) ||
      activeLayerIsSF;
  ...
  if (success && m_glWindow) {
    ...
    try {
      ...
      m_rasterCloud = convertGridToCloud(
          true, false, exportedStatistics, projectSFs, projectColors,
          /*copyHillshadeSF=*/false, activeLayerName,
          getStatisticsPercentileValue(), false, false, nullptr);

      // Special case: the 'LAYER_HEIGHT' field has now a dynamic name
      if (m_UI->activeLayerComboBox->currentIndex() == 0 && m_rasterCloud &&
          m_rasterCloud->getNumberOfScalarFields() != 0) {
        assert(m_UI->activeLayerComboBox->itemData(0).toInt() == LAYER_HEIGHT);
        m_UI->activeLayerComboBox->setItemText(
            0, QString::fromStdString(
                   m_rasterCloud->getScalarField(0)->getName()));
      }
    } catch (const std::bad_alloc &) {
      // see below
    }
...
bool ccRasterizeTool::updateGrid(bool projectSFs /*=false*/) {
  ...
  ccRasterGrid::ProjectionType sfProjectionType =
      projectSFs ? getTypeOfSFProjection()
  ...
  if (gridTotalSize == 1) {
    if (QMessageBox::question(this, "Unexpected grid size",
                              "The generated grid will only have 1 cell! Do "
    ...
    if (QMessageBox::question(
            this, "Big grid size",
            "The generated grid will have more than 10.000.000 cells! Do you "
  ...
  if (!m_grid.fillWith(m_cloud, Z, projectionType, interpolationType,
                       interpolationParams, sfProjectionType, &pDlg,
                       zStdDevSfIndex)) {
    return false;
  }
  ...
  // update volume estimate
  {
    ...
    if (filledCellCount) {
      ...
      m_UI->filledCellsPercentageLabel->setText(
...
ccPointCloud *ccRasterizeTool::generateCloud(bool autoExport /*=true*/) {
  ...
  bool exportSFStats = m_UI->exportSFStatsCheckBox->isEnabled() &&
                       m_UI->exportSFStatsCheckBox->isChecked();
  ...
  ccPointCloud *rasterCloud =
      convertGridToCloud(exportHeightStats, exportSFStats, exportedStatistics,
                         projectSFs, projectColors,
                         /*copyHillshadeSF=*/true, activeLayerName,
                         getStatisticsPercentileValue(), true,
  ...
  if (rasterCloud && autoExport) {
    ...
    if (m_cloud->isEnabled()) {
      ...
      ccLog::Warning("[Rasterize] Previously selected entity (source cloud) "
                     "has been hidden!");
    }
    ...
    if (mainWindow) {
      ...
      ccLog::Print(QString("[Rasterize] Cloud '%1' successfully exported")
  ...
  if (rasterCloud) {
    ...
    CCCoreLib::GenericIndexedMesh *baseMesh =
        CCCoreLib::PointProjectionTools::computeTriangulation(
            rasterCloud, CCCoreLib::DELAUNAY_2D_AXIS_ALIGNED,
    ...
    if (rasterMesh) {
      ...
      ccLog::Print(QString("[Rasterize] Mesh '%1' successfully exported")
...
void ccRasterizeTool::generateRaster() const {
  ...
  if (m_UI->activeLayerComboBox->currentData().toInt() == LAYER_SF &&
      m_cloud->isA(CC_TYPES::POINT_CLOUD)) {
    ...
    visibleSfIndex =
        static_cast<ccPointCloud *>(m_cloud)->getScalarFieldIndexByName(
            m_UI->activeLayerComboBox->currentText().toStdString());
  }
  ...
  while (true) {
    ...
    if (reoDlg.exportRGBCheckBox->isEnabled() &&
      ...
      if (QMessageBox::warning(
              nullptr, "Mixed raster",
              "Mixing colors and other layers will result in\na strange raster "
              "file with 64 bits color bands\n(some applications won't handle "
              "them properly)",
              QMessageBox::Ignore, QMessageBox::Retry) == QMessageBox::Ignore) {
  ...
  QString outputFilename;
  {
    ...
    QString imageSavePath =
        settings.value("savePathImage", ccFileUtils::defaultDocPath())
            .toString();
  ...
  ExportGeoTiff(outputFilename, exportBands,
                getFillEmptyCellsStrategy(m_UI->fillEmptyCellsComboBox), m_grid,
  ...
  assert(false);
  ccLog::Error("[Rasterize] GDAL not supported by this version! Can't generate "
               "a raster...");
...
bool ccRasterizeTool::ExportGeoTiff(
    const QString &outputFilename, const ExportBands &exportBands,
    ccRasterGrid::EmptyCellFillOption fillEmptyCellsStrategy,
    const ccRasterGrid &grid, const ccBBox &gridBBox, unsigned char Z,
    double
        customHeightForEmptyCells /*=std::numeric_limits<double>::quiet_NaN()*/,
    ccGenericPointCloud *originCloud /*=nullptr*/, int visibleSfIndex /*=-1*/) {
#ifdef CC_GDAL_SUPPORT
  ...
  double shiftX =
      ...
      stepX / 2; // we will declare the raster grid as 'Pixel-is-area'!
  double shiftY =
      gridBBox.maxCorner().u[Y] +
      stepY / 2; // we will declare the raster grid as 'Pixel-is-area'!
  double shiftZ = 0.0;
  ...
  if (totalBands == 0) {
    ccLog::Error(
        "Can't output a raster with no band! (check export parameters)");
    return false;
  }

  GDALAllRegister();
  ccLog::PrintDebug("(GDAL drivers: %i)",
  ...
  GDALDataset *poDstDS = poDriver->Create(
      qUtf8Printable(outputFilename), static_cast<int>(grid.width),
  ...
  if (exportBands.rgb) {
    ...
    unsigned char *cLine = static_cast<unsigned char *>(
        CPLMalloc(sizeof(unsigned char) * grid.width));
    if (!cLine) {
    ...
    if (!error && rgbaMode) {
      ...
      aBand->SetStatistics(
          0, 255, 255, 0); // warning: arbitrary average and std. dev. values
  ...
  double *scanline =
      static_cast<double *>(CPLMalloc(sizeof(double) * grid.width));
  if (!scanline) {
    ccLog::Error("[GDAL] Not enough memory");
    GDALClose(poDstDS);
    return false;
  ...
  if (exportBands.height) {
    ...
    for (unsigned j = 0; j < grid.height; ++j) {
      ...
      for (unsigned i = 0; i < grid.width; ++i) {
        scanline[i] =
            std::isfinite(row[i].h) ? row[i].h + shiftZ : emptyCellHeight;
      }

      if (poBand->RasterIO(GF_Write, 0, static_cast<int>(j),
                           static_cast<int>(grid.width), 1, scanline,
                           static_cast<int>(grid.width), 1, GDT_Float64, 0,
                           0) != CE_None) {
  ...
  if (exportBands.density) {
    ...
    for (unsigned j = 0; j < grid.height; ++j) {
      ...
      if (poBand->RasterIO(GF_Write, 0, static_cast<int>(j),
                           ...
                           0) != CE_None) {
        ccLog::Error(
            "[GDAL] An error occurred while writing the density band!");
        if (scanline) {
          CPLFree(scanline);
        }
        GDALClose(poDstDS);
  ...
  if (exportBands.allSFs || (exportBands.visibleSF && visibleSfIndex >= 0)) {
    for (size_t k = 0; k < grid.scalarFields.size(); ++k) {
      ...
      if (exportBands.allSFs ||
          (exportBands.visibleSF && visibleSfIndex == static_cast<int>(k))) {
        const double *sfGrid = grid.scalarFields[k].data();
        GDALRasterBand *poBand = poDstDS->GetRasterBand(++currentBand);

        double sfNanValue =
            std::numeric_limits<ccRasterGrid::SF::value_type>::quiet_NaN();
        ...
        for (unsigned j = 0; j < grid.height; ++j) {
          ...
          if (poBand->RasterIO(GF_Write, 0, static_cast<int>(j),
            ...
            ccLog::Error(QString(
                "[GDAL] An error occurred while writing a scalar field band!"));
  ...
  ccLog::Print(QString("[Rasterize] Raster '%1' successfully saved")
                   .arg(outputFilename));
  return true;
...
void ccRasterizeTool::generateHillshade() {
  ...
  if (sfIdx >= 0) {
    hillshadeLayer =
        static_cast<ccScalarField *>(m_rasterCloud->getScalarField(sfIdx));
  } else {
    ...
    m_UI->activeLayerComboBox->addItem(HILLSHADE_FIELD_NAME,
                                       QVariant(LAYER_SF));
  ...
  double zenith_rad =
      CCCoreLib::DegreesToRadians(static_cast<double>(zenith_deg));
  ...
  double azimuth_rad =
      CCCoreLib::DegreesToRadians(static_cast<double>(azimuth_math));
      ...
      if (std::isfinite(cell.h)) {
        if (j != 0 && i != 0 && i + 1 != m_grid.width) {
          ...
          for (int di = -1; di <= 1; ++di) {
            for (int dj = -1; dj <= 1; ++dj) {
              const ccRasterCell &n =
                  m_grid.rows[j - dj]
                             [i + di]; //-dj (instead of + dj) because we
                                       // scan the grid in the reverse
          ...
          if (dz_dx_count == 8 && dz_dy_count == 8) {
            ...
            double slope_rad =
                atan(/*z_factor **/ sqrt(dz_dx * dz_dx + dz_dy * dz_dy));
            ...
            ScalarType hillshade = static_cast<ScalarType>(
                std::max(0.0, cos_zenith_rad * cos(slope_rad) +
                                  sin_zenith_rad * sin(slope_rad) *
                                      cos(azimuth_rad - aspect_rad)));
            if (!resampleInputCloudXY) {
              hillshadeLayer->setValue(sparseSF ? nonEmptyCellIndex
                                                : i + j * m_grid.width,
                                       hillshade);
            } else // resampling mode
            {
              if (cell.nbPoints != 0) {
                ...
                hillshadeLayer->setValue(m_grid.nonEmptyCellCount +
        ...
        if (cell.nbPoints) {
          // with inv. var. projection mode, it's possible to have a non empty
          // cell with NaN height!
          ++nonEmptyCellIndex;
        }
      }
    }
  ...
  m_rasterCloud->showSF(true);
  m_UI->activeLayerComboBox->setCurrentIndex(
      m_UI->activeLayerComboBox->findText(HILLSHADE_FIELD_NAME));
...
void ccRasterizeTool::addNewContour(ccPolyline *poly, double height) {
  ...
  if (poly->size() > 1) {
    ...
    poly->setWidth(
        m_UI->contourWidthSpinBox->value() < 2
            ? 0
            : m_UI->contourWidthSpinBox
                  ->value()); // size 1 is equivalent to the default size
...
void ccRasterizeTool::generateContours() {
  ...
  ccContourLinesGenerator::Parameters params;
  {
    ...
    // the parameters below are only required if GDAL is not supported (but we
    // can set them anyway)
  ...
  if (!ccContourLinesGenerator::GenerateContourLines(&m_grid, gridMinCorner,
                                                     params, contourLines)) {
  ...
  for (ccPolyline *poly : contourLines) {
    addNewContour(
        poly, poly->getMetaData(ccPolyline::MetaKeyConstAltitude()).toUInt());
  }
...
void ccRasterizeTool::exportContourLines() {
  ...
  ccHObject *group =
      new ccHObject(QString("Contour plot(%1) [step=%2]")
                        .arg(m_cloud->getName())
                        .arg(m_UI->contourStepDoubleSpinBox->value()));
  ...
  ccLog::Print(QString("Contour lines have been successfully exported to DB "
                       "(group name: %1)")
                   .arg(group->getName()));
}
...
ccRasterGrid::EmptyCellFillOption ccRasterizeTool::getFillEmptyCellsStrategyExt(
  ...
  case ccRasterGrid::FILL_AVERAGE_HEIGHT:
    //'average height' is a kind of 'custom height' so we can fall back to this
    // mode!
    fillEmptyCellsStrategy = ccRasterGrid::FILL_CUSTOM_HEIGHT;
...
void ccRasterizeTool::generateImage() const {
  ...
  bool exportRGB =
      (m_UI->activeLayerComboBox->currentData().toInt() == LAYER_RGB);
  ...
  if (!exportRGB &&
    ...
    int visibleSfIndex = pc->getScalarFieldIndexByName(
        m_UI->activeLayerComboBox->currentText().toStdString());
    if (visibleSfIndex >= 0 &&
        static_cast<size_t>(visibleSfIndex) < m_grid.scalarFields.size()) {
  ...
  if (cloudSF) {
    ...
    fillEmptyCellsStrategy =
        getFillEmptyCellsStrategyExt(emptyCellsValue, minValue, maxValue);
  }

  double valueRange = maxValue - minValue;
  if (!exportRGB && CCCoreLib::LessThanEpsilon(valueRange)) {
    ccLog::Warning(
        "[Rasterize::generateImage] Exported field has a flat range");
    valueRange = 1.0; // to simplify tests below
  }

  QImage outputImage(m_grid.width, m_grid.height,
                     exportRGB ? QImage::Format_ARGB32
  ...
  if (!outputImage.isNull()) {
    ...
    if (!exportRGB) {
      bool addTransparentColor =
          (cloudSF || fillEmptyCellsStrategy == ccRasterGrid::LEAVE_EMPTY);
      ...
      if (addTransparentColor) {
        palette[255] =
            qRgba(255, 0, 255, 0); // magenta/transparent color for empty cells
      ...
      if (!cloudSF) // we are using height values
      {
        ...
        case ccRasterGrid::FILL_CUSTOM_HEIGHT: {
          ...
          emptyCellColorIndex = static_cast<unsigned>(
              normalizedHeight *
              maxColorComp); // static_cast is equivalent to floor if value >= 0
        } break;
        case ccRasterGrid::FILL_AVERAGE_HEIGHT:
        default:
          assert(false);
    ...
    for (unsigned j = 0; j < m_grid.height; ++j) {
      const ccRasterGrid::Row &row = m_grid.rows[j];
      const double *sfRow =
          (gridSF ? gridSF->data() + j * m_grid.width : nullptr);
      for (unsigned i = 0; i < m_grid.width; ++i) {
        if (std::isfinite(row[i].h)) {
          if (exportRGB) {
            int r = static_cast<int>(
                std::max(0.0, std::min(255.0, row[i].color.u[0])));
            int g = static_cast<int>(
                std::max(0.0, std::min(255.0, row[i].color.u[1])));
            int b = static_cast<int>(
                std::max(0.0, std::min(255.0, row[i].color.u[2])));
            ...
            unsigned char val = static_cast<unsigned char>(
                normalizedHeight * maxColorComp); // static_cast is equivalent
                                                  // to floor if value >= 0
        ...
        } else // NaN
        {
          outputImage.setPixel(
              i, m_grid.height - 1 - j,
              emptyCellColorIndex); // in RGBA mode, it should be 0
        }
      }
    }
    ...
    // open file saving dialog
    {
      ...
      QString outputFilename = ImageFileFilter::GetSaveFilename(
          "Save raster as image", "image", imageSavePath,
      ...
      if (!outputFilename.isNull()) {
        ...
        if (outputImage.save(outputFilename)) {
          ccLog::Print(QString("[Rasterize] Image '%1' successfully saved")
                           .arg(outputFilename));
        } else {
...
void ccRasterizeTool::generateASCIIMatrix() const {
  ...
  QString asciiGridSavePath =
      settings.value("savePathASCIIGrid", ccFileUtils::defaultDocPath())
          .toString();

  // open file saving dialog
  QString filter("ASCII file (*.txt)");
  QString outputFilename = QFileDialog::getSaveFileName(
      nullptr, "Save grid as ASCII file",
  ...
  ccLog::Print(QString("[Rasterize] Raster matrix '%1' successfully saved")
                   .arg(outputFilename));
}

void ccRasterizeTool::onStatExportTargetChanged(bool) {
  m_UI->exportStatisticsFrame->setEnabled(
      m_UI->exportHeightStatsCheckBox->isChecked() ||
      (m_UI->exportSFStatsCheckBox->isEnabled() &&
       m_UI->exportSFStatsCheckBox->isChecked()));
}
...
void ccRasterizeTool::showInterpolationParamsDialog() {
  ...
  case ccRasterGrid::EmptyCellFillOption::INTERPOLATE_DELAUNAY: {
    ...
    double value = QInputDialog::getDouble(
        this, tr("Delaunay triangulation"),
        tr("Triangles max edge length (0 = no limit)"),
        m_delaunayInterpParams.maxEdgeLength, 0, 1.0e6, 6, &ok);
    if (ok) {
  ...
  case ccRasterGrid::EmptyCellFillOption::INTERPOLATE_KRIGING: {

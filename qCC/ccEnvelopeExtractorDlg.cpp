...
ccEnvelopeExtractorDlg::ccEnvelopeExtractorDlg(QWidget *parent /*=nullptr*/)
    : QDialog(parent, Qt::WindowMaximizeButtonHint | Qt::WindowCloseButtonHint),
      Ui::EnvelopeExtractorDlg(), m_skipped(false), m_glWindow(nullptr) {
  setupUi(this);
}
...
void ccEnvelopeExtractorDlg::init() {
  ...
  connect(skipPushButton, &QAbstractButton::clicked, this,
          &ccEnvelopeExtractorDlg::onSkipButtonClicked);
...
void ccEnvelopeExtractorDlg::zoomOn(const ccBBox &box) {
  ...
  double pixSize =
      std::max(box.getDiagVec().x / std::max(20, m_glWindow->glWidth() - 20),
               box.getDiagVec().y / std::max(20, m_glWindow->glHeight() - 20));
...
void ccEnvelopeExtractorDlg::addToDisplay(ccHObject *obj,
...
void ccEnvelopeExtractorDlg::displayMessage(

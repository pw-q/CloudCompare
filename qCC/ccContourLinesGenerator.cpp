...
static QString GetPolylineName(double value, unsigned mainIndex,
                               unsigned partNumber) {
  QString contourLineName = QObject::tr("Contour line [value = %1]").arg(value);
  if (partNumber != 0) {
    contourLineName +=
        QObject::tr(" (poly %1-%2)").arg(mainIndex).arg(partNumber);
  } else {
...
static CPLErr ContourWriter(double dfLevel, int nPoints, double *padfX,
  ...
  ContourGenerationParameters *params =
      reinterpret_cast<ContourGenerationParameters *>(userData);
      ...
      if (std::isfinite(h)) {
        ...
        if (poly) {
          if (poly->size() < 2) {
            ...
            ++partCount;
            poly->setName(GetPolylineName(
                dfLevel, contourIndex,
                partCount == 1 && i + 2 >= nPoints
                    ? 0
                    : partCount)); // if not enough points to add another
                                   // polyline, we don't need to add the
                                   // sub-part index

            if (params->contourLines.insert(dfLevel, poly) ==
                params->contourLines.end()) {
              // not enough memory?
              delete poly;
              return CE_Failure;
            }
  ...
  if (poly) {
    poly->shrinkToFit();
    vertices->shrinkToFit();
    poly->setName(GetPolylineName(dfLevel, contourIndex,
                                  partCount == 0 ? 0 : partCount + 1));
    if (params->contourLines.insert(dfLevel, poly) ==
        params->contourLines.end()) {
      // not enough memory?
      delete poly;
...
bool ccContourLinesGenerator::GenerateContourLines(
    ccRasterGrid *rasterGrid, const CCVector2d &gridMinCornerXY,
    const Parameters &params, std::vector<ccPolyline *> &contourLines) {
  ...
  if (params.minVertexCount < 3) {
    ccLog::Error("Invalid input parameter: can't have less than 3 vertices per "
                 "contour line");
    assert(false);
    return false;
  }
  ...
  bool sparseLayer =
      (params.altitudes && params.altitudes->currentSize() !=
                               rasterGrid->height * rasterGrid->width);
  ...
  if (CCCoreLib::GreaterThanEpsilon(params.step)) {
    levelCount += static_cast<unsigned>(
        (params.maxAltitude - params.startAltitude) /
        params.step); // static_cast is equivalent to floor if value >= 0
  }

  try {
#ifdef CC_GDAL_SUPPORT // use GDAL (more robust) - otherwise we will use an old
    ...
    GDALContourGeneratorH hCG =
        GDAL_CG_Create(rasterGrid->width, rasterGrid->height,
                       std::isnan(params.emptyCellsValue) ? FALSE : TRUE,
                       params.emptyCellsValue, params.step,
                       params.startAltitude, ContourWriter, &gdalParams);
    if (!hCG) {
    ...
    {
      double *scanline =
          static_cast<double *>(CPLMalloc(sizeof(double) * rasterGrid->width));
      if (!scanline) {
      ...
      for (unsigned j = 0; j < rasterGrid->height; ++j) {
        ...
        for (unsigned i = 0; i < rasterGrid->width; ++i) {
          if (cellRow[i].nbPoints || !sparseLayer) {
            if (params.altitudes) {
              ...
              scanline[i] = std::isfinite(cellRow[i].h)
                                ? cellRow[i].h
                                : params.emptyCellsValue;
            }
          } else {
        ...
        if (error != CE_None) {
          ccLog::Error(
              "[GDAL] An error occurred during contour lines generation");
          break;
        }
      }
          ...
          for (unsigned i = 0; i < poly->size(); ++i) {
            ...
            CCVector3 P(
                static_cast<PointCoordinateType>(
                    (P2D->x - 0.5) * rasterGrid->gridStep + gridMinCornerXY.x),
                static_cast<PointCoordinateType>(
                    (P2D->y - 0.5) * rasterGrid->gridStep + gridMinCornerXY.y),
                P2D->z);
            *P2D = P;
          }
      ...
      for (unsigned j = 0; j < rasterGrid->height; ++j) {
        ...
        for (unsigned i = 0; i < rasterGrid->width; ++i) {
          if (cellRow[i].nbPoints || !sparseLayer) {
            if (params.altitudes) {
              ScalarType value = params.altitudes->getValue(layerIndex++);
              row[i] = ccScalarField::ValidValue(value)
                           ? value
                           : params.emptyCellsValue;
            } else {
              row[i] = std::isfinite(cellRow[i].h) ? cellRow[i].h
                                                   : params.emptyCellsValue;
            }
          } else {
    ...
    // generate contour lines
    {
      ...
      pDlg.setInfo(QObject::tr("Levels: %1\nCells: %2 x %3")
                       .arg(levelCount)
                       .arg(rasterGrid->width)
                       .arg(rasterGrid->height));
      pDlg.start();
      pDlg.show();
      QCoreApplication::processEvents();
      CCCoreLib::NormalizedProgress nProgress(&pDlg, levelCount);

      for (double v = params.startAltitude; v <= params.maxAltitude;
           v += params.step) {
        // extract contour lines for the current level
        iso.setThreshold(v);
        int lineCount = iso.find(grid.data());

        ccLog::PrintDebug(QString("[Rasterize][Isolines] value=%1 : %2 lines")
                              .arg(v)
                              .arg(lineCount));
        ...
        for (int i = 0; i < lineCount; ++i) {
          ...
          if (vertCount >= params.minVertexCount) {
            int startVi =
                0; // we may have to split the polyline in multiple chunks
            while (startVi < vertCount) {
              ...
              if (poly->reserve(vertCount - startVi) &&
                ...
                for (int vi = startVi; vi < vertCount; ++vi) {
                  ...
                  //(otherwise the contour lines appear in the wrong orientation
                  //compared to the grid/raster which
                  // is in the XY plane by default!)
                  /*P.u[X] = */ P.x = static_cast<PointCoordinateType>(
                      (x + 0.5) * rasterGrid->gridStep + gridMinCornerXY.x);
                  /*P.u[Y] = */ P.y = static_cast<PointCoordinateType>(
                      (y + 0.5) * rasterGrid->gridStep + gridMinCornerXY.y);
                  if (params.projectContourOnAltitudes) {
                    int xi = std::min(std::max(static_cast<int>(x), 0),
                                      static_cast<int>(rasterGrid->width) - 1);
                    int yi = std::min(std::max(static_cast<int>(y), 0),
                                      static_cast<int>(rasterGrid->height) - 1);
                ...
                if (poly->size() > 1) {
                  poly->setClosed(
                      isClosed); // if we have less vertices, it means we have
                                 // 'chopped' the original contour
                  vertices->setEnabled(false);
                  ...
                  ++subPartCount;
                  poly->setName(
                      GetPolylineName(v, static_cast<unsigned>(lineCount + 1),
                                      isClosed ? 0 : subPartCount));
                  try {
                    contourLines.push_back(poly);
                  } catch (const std::bad_alloc &) {
                    ccLog::Warning(
                        "[ccContourLinesGenerator] Not enough memory");
                    return false;
                  }
                } else {
                  delete poly;
  ...
  ccLog::Print(
      QString("[ccContourLinesGenerator] %1 iso-lines generated (%2 levels)")
          .arg(contourLines.size())
          .arg(levelCount));
  return true;
}

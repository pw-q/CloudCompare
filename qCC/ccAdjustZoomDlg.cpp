...
ccAdjustZoomDlg::ccAdjustZoomDlg(ccGLWindowInterface *win,
                                 QWidget *parent /*=nullptr*/)
    : QDialog(parent, Qt::Tool), Ui::AdjustZoomDialog(), m_windowWidth_pix(0),
      m_distanceToWidthRatio(0.0) {
  setupUi(this);

  if (win) {
    windowLabel->setText(QString("%1 [%2 x %3]")
                             .arg(win->getWindowTitle())
                             .arg(win->glWidth())
                             .arg(win->glHeight()));
    ...
    pixelSizeDoubleSpinBox->setValue(focalDist * m_distanceToWidthRatio /
                                     m_windowWidth_pix);
  } else {
    windowLabel->setText("Error");
  }

  connect(focalDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccAdjustZoomDlg::onFocalChanged);
  connect(pixelSizeDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccAdjustZoomDlg::onPixelSizeChanged);
  connect(pixelCountSpinBox, qOverload<int>(&QSpinBox::valueChanged), this,
          &ccAdjustZoomDlg::onPixelCountChanged);
}
...
void ccAdjustZoomDlg::onFocalChanged(double focalDist) {
  ...
  double pixelSizeNPixels =
      (focalDist * m_distanceToWidthRatio * pixelCountSpinBox->value()) /
      m_windowWidth_pix;
...
void ccAdjustZoomDlg::onPixelSizeChanged(double pixelSizeNPixels) {
  ...
  double focalDist = (pixelSizeNPixels * m_windowWidth_pix) /
                     (pixelCountSpinBox->value() * m_distanceToWidthRatio);
  focalDoubleSpinBox->setValue(focalDist);
...
void ccAdjustZoomDlg::onPixelCountChanged(int pixelCount) {
  ...
  pixelSizeDoubleSpinBox->blockSignals(true);
  double pixelSizeNPixels =
      (focalDoubleSpinBox->value() * m_distanceToWidthRatio * pixelCount) /
      m_windowWidth_pix;
  pixelSizeDoubleSpinBox->setValue(pixelSizeNPixels);
  pixelSizeDoubleSpinBox->blockSignals(false);
}

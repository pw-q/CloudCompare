...
struct Edge {
  ...
  Edge(const VertexIterator &A, unsigned _nearestPointIndex,
       float _nearestPointSquareDist)
      : itA(A), nearestPointIndex(_nearestPointIndex),
...
static PointCoordinateType FindNearestCandidate(
    unsigned &minIndex, const VertexIterator &itA, const VertexIterator &itB,
    const std::vector<Vertex2D> &points,
    const std::vector<HullPointFlags> &pointFlags,
    PointCoordinateType minSquareEdgeLength, bool allowLongerChunks = false,
  ...
  for (unsigned i = 0; i < pointCount; ++i) {
    ...
    if (dot >= 0 && dot <= squareLengthAB) {
      ...
      if (minDist2 < 0 || dist2 < minDist2) {
        // the 'nearest' point must also be a valid candidate
        //(i.e. at least one of the created edges is smaller than the original
        //one and we don't create too small edges!)
...
bool ccEnvelopeExtractor::ExtractConcaveHull2D(
    std::vector<Vertex2D> &points, std::list<Vertex2D *> &hullPoints,
    EnvelopeType envelopeType, bool allowMultiPass,
    PointCoordinateType maxSquareEdgeLength /*=0*/,
    bool enableVisualDebugMode /*=false*/, double maxAngleDeg /*=0.0*/) {
  ...
  PointCoordinateType minSquareEdgeLength = 0;
  {
    ...
    if (envelopeType != FULL) {
      // we will now try to determine which part of the envelope is the 'upper'
      // one and which one is the 'lower' one

      // search for the min and max vertices
      VertexIterator itLeft = hullPoints.begin();
      VertexIterator itRight = hullPoints.begin();
      {
        for (VertexIterator it = hullPoints.begin(); it != hullPoints.end();
             ++it) {
          if ((*it)->x < (*itLeft)->x ||
      ...
      // find the right way to go
      {
        ...
        bool forward =
            ((**itBefore - **itLeft).cross(**itAfter - **itLeft) < 0 &&
             envelopeType == LOWER);
        if (!forward) {
  ...
  if (enableVisualDebugMode) {
    ...
    debugDialog.show();
    QCoreApplication::processEvents(); // make sure the dialog is visible or the
                                       // call to zoomOn below won't be
                                       // effective!
    ...
    // create point cloud with all (2D) input points
    {
      ...
      debugDialog.addToDisplay(
          debugCloud,
          false); // the window will take care of deleting this entity!
    }
    ...
    // create polyline
    {
      ...
      if (debugEnvelope->reserve(hullSize)) {
        ...
        ccLog::Warning(
            "Not enough memory to create the debug envelope polyline");
  ...
  while (somethingHasChanged) {
    try {
      ...
      for (size_t i = 0; i < initEdgeCount; ++i) {
        ...
        // we will only process the edges that are longer than the maximum
        // specified length
      ...
      while (!edges.empty()) {
        ...
        assert(pointFlags[P.index] ==
               POINT_NOT_USED); // we don't consider already used points!
        ...
        ////at least one of the new segments must be smaller than the initial
        ///one!
        ...
        // if (false)
        {
          for (VertexIterator itJ = hullPoints.begin(), itI = itJ++;
            ...
            if (((*itI)->index != (*itA)->index &&
                 (*itJ)->index != (*itA)->index &&
                 CCCoreLib::PointProjectionTools::segmentIntersect(**itI, **itJ,
                                                                   **itA, P)) ||
                ((*itI)->index != (*itB)->index &&
                 (*itJ)->index != (*itB)->index &&
                 CCCoreLib::PointProjectionTools::segmentIntersect(**itI, **itJ,
        ...
        if (!intersect) {
          ...
          if (enableVisualDebugMode && !debugDialog.isSkipped()) {
            ...
            debugDialog.displayMessage("point has been added to envelope",
                                       true);
          }

          // update all edges that were having 'P' as their nearest candidate as
          // well
          if (!edges.empty()) {
            ...
            for (size_t i = 0; i < removed.size(); ++i) {
              ...
              unsigned nearestPointIndex = 0;
              PointCoordinateType minSquareDist = FindNearestCandidate(
                  nearestPointIndex, itC, itD, points, pointFlags,
                  minSquareEdgeLength, false, minCosAngle);
          ...
          if ((P - **itA).norm2() > maxSquareEdgeLength) {
            unsigned nearestPointIndex = 0;
            PointCoordinateType minSquareDist = FindNearestCandidate(
                nearestPointIndex, itA, itP, points, pointFlags,
                minSquareEdgeLength, false, minCosAngle);
          ...
          if ((**itB - P).norm2() > maxSquareEdgeLength) {
            unsigned nearestPointIndex = 0;
            PointCoordinateType minSquareDist = FindNearestCandidate(
                nearestPointIndex, itP, itB, points, pointFlags,
                minSquareEdgeLength, false, minCosAngle);
          ...
          if (enableVisualDebugMode)
            debugDialog.displayMessage(
                "[rejected] new edge would intersect the current envelope!",
                true);
        }
...
ccPolyline *ccEnvelopeExtractor::ExtractFlatEnvelope(
    CCCoreLib::GenericIndexedCloudPersist *points, bool allowMultiPass,
  ...
  CCCoreLib::Neighbourhood::InputVectorsUsage vectorsUsage =
      CCCoreLib::Neighbourhood::None;
  ...
  // if the user has specified a default direction, we'll use it as 'projecting
  // plane'
  PointCoordinateType preferredPlaneEq[4] = {0, 0, 1, 0};

  if (preferredNormDim != nullptr) {
    const CCVector3 *G =
        points->getPoint(0); // any point through which the plane passes is ok
  ...
  if (!Yk.projectPointsOn2DPlane<Vertex2D>(points2D, planeEq, &O, &X, &Y,
                                           vectorsUsage)) {
    ccLog::Warning("[ExtractFlatEnvelope] Failed to project the points on the "
                   "LS plane (not enough memory?)!");
    return nullptr;
  }
  ...
  // try to get the points on the convex/concave hull to build the envelope and
  // the polygon
  Hull2D hullPoints;
  if (!ExtractConcaveHull2D(points2D, hullPoints, envelopeType, allowMultiPass,
                            maxEdgeLength * maxEdgeLength,
                            enableVisualDebugMode, maxAngleDeg)) {
    ccLog::Warning("[ExtractFlatEnvelope] Failed to compute the convex hull of "
                   "the input points!");
    return nullptr;
  }
  ...
  if (originalPointIndexes) {
    ...
    unsigned i = 0;
    for (Hull2D::const_iterator it = hullPoints.begin(); it != hullPoints.end();
         ++it, ++i) {
  ...
  if (envelopePolyline->reserve(hullPtsCount)) {
    ...
    ccLog::Warning("[ExtractFlatEnvelope] Not enough memory to create the "
                   "envelope polyline!");
  }
...
bool ccEnvelopeExtractor::ExtractFlatEnvelope(
    CCCoreLib::GenericIndexedCloudPersist *points, bool allowMultiPass,
    PointCoordinateType maxEdgeLength, std::vector<ccPolyline *> &parts,
    EnvelopeType envelopeType /*=FULL*/, bool allowSplitting /*=true*/,
  ...
  ccPolyline *basePoly = ExtractFlatEnvelope(
      points, allowMultiPass, maxEdgeLength, preferredNormDir, preferredUpDir,
      envelopeType, nullptr, enableVisualDebugMode);
  if (!basePoly) {
    return false;

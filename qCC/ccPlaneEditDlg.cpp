...
ccPlaneEditDlg::ccPlaneEditDlg(ccPickingHub *pickingHub, QWidget *parent)
    : QDialog(parent), Ui::PlaneEditDlg(), m_pickingWin(nullptr),
      m_associatedPlane(nullptr), m_pickingHub(pickingHub) {
  assert(pickingHub);
  ...
  connect(dipDirDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccPlaneEditDlg::onDipDirChanged);
  connect(nxDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccPlaneEditDlg::onNormalChanged);
  connect(nyDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccPlaneEditDlg::onNormalChanged);
  connect(nzDoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &ccPlaneEditDlg::onNormalChanged);
...
void ccPlaneEditDlg::updatePlane(ccPlane *plane) {
  ...
  ccGLMatrix trans;
  bool needToApplyRot = (std::abs(N.dot(Nd) - CCCoreLib::PC_ONE) >
  ...
  if (needToApplyRot) {
    ...
    if (std::abs(N.z) >
        CCCoreLib::PC_ONE -
            std::numeric_limits<PointCoordinateType>::epsilon()) {
      ccGLMatrix rotX;
      rotX.initFromParameters(CCCoreLib::DegreesToRadians(-dip),
                              CCVector3(1, 0, 0), CCVector3(0, 0, 0)); // plunge
      ccGLMatrix rotZ;
      rotZ.initFromParameters(CCCoreLib::DegreesToRadians(dipDir),

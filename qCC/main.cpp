...
static bool IsCommandLine(int argc, char **argv) {
  ...
  // serial number". We need to recognize this and discount it when determining
  // if we are running on the command line or not.
...
int main(int argc, char **argv) {
  ...
  if (AttachConsole(ATTACH_PARENT_PROCESS)) {
    if (stdout_type == FILE_TYPE_UNKNOWN) // this will allow std redirection
                                          // (./executable > out.txt)
    {
  ...
  // Convert the input arguments to QString before the application is
  // initialized (as it will force utf8, which might prevent from properly
  ...
  if (commandLine) {
    ...
    if (lastArgumentIndex < argumentsLocal8Bit.size() &&
      ...
      if (lastArgumentIndex >= argumentsLocal8Bit.size()) {
        ccLog::Error(QObject::tr("Missing argument after %1: language file")
                         .arg("-LANG"));
        return EXIT_FAILURE;
      }

      // remove verified arguments so that -SILENT will be the first one (if
      // present)...
    ...
    if (lastArgumentIndex < argumentsLocal8Bit.size() &&
      ...
      if (lastArgumentIndex >= argumentsLocal8Bit.size()) {
        ccLog::Error(QObject::tr("Missing argument after %1: verbosity level")
                         .arg("-VERBOSITY"));
        return EXIT_FAILURE;
      }
      ...
      if (!ok || verbosityLevel < 0) {
        ccLog::Warning(
            QObject::tr("Invalid verbosity level: %1").arg(verbosityLevelStr));
      } else {
  ...
  FileIOFilter::InitInternalFilters();  // load all known I/O filters (plugins
                                        // will come later!)
  ccNormalVectors::GetUniqueInstance(); // force pre-computed normals array
                                        // initialization
  ccColorScalesManager::GetUniqueInstance(); // force pre-computed color tables
                                             // initialization
  ...
  // restore some global parameters
  {
    ...
    double maxAbsCoord =
        settings
            .value(ccPS::MaxAbsCoord(),
                   ccGlobalShiftManager::MaxCoordinateAbsValue())
            .toDouble();
    double maxAbsDiag = settings
                            .value(ccPS::MaxAbsDiag(),
                                   ccGlobalShiftManager::MaxBoundgBoxDiagonal())
                            .toDouble();
    settings.endGroup();

    ccLog::Print(
        QString("[Global Shift] Max abs. coord = %1 / max abs. diag = %2")
  ...
  if (commandLine) {
    ...
    result = ccCommandLineParser::Parse(argumentsLocal8Bit,
    ...
    if (argc > lastArgumentIndex) {
      // any additional argument is assumed to be a filename --> we try to load
      // it/them
        ...
        if (arg.startsWith(":start-plugin:")) {
          ...
          for (ccPluginInterface *plugin : pluginManager.pluginList()) {
            if (plugin->getName().replace(' ', '_').toUpper() ==
              ...
              if (!success) {
                ccLog::Error(QString("Failed to start the plugin '%1'")
                                 .arg(plugin->getName()));
              }
              break;
            }
          ...
          if (!found) {
            ccLog::Error(QString("Couldn't find the plugin '%1'")
    ...
    // change the default path to the application one (do this AFTER processing
    // the command line)
    ...
    try {
      ...
      QMessageBox::warning(
          nullptr, "CC crashed!",
          QString(
              "Hum, it seems that CC has crashed... Sorry about that :)\n") +
              e.what());
    } catch (...) {
      QMessageBox::warning(
          nullptr, "CC crashed!",
  ...
  ccPointCloud::ReleaseShaders(); // must be done before the OpenGL context is
                                  // released (i.e. before the windows is
                                  // destroyed)
  ...
  if (alive > 1) {
    printf("Error: some shared objects (%u) have not been released on program "
           "end!",
           alive);
    system("PAUSE");
  }

...
ccPointListPickingDlg::ccPointListPickingDlg(ccPickingHub *pickingHub,
    ...
    : ccPointPickingGenericInterface(pickingHub, parent),
      Ui::PointListPickingDlg(), m_associatedEntity(nullptr),
      m_lastPreviousID(0), m_orderedLabelsContainer(nullptr) {
  setupUi(this);
  ...
  connect(markerSizeSpinBox, qOverload<int>(&QSpinBox::valueChanged), this,
          &ccPointListPickingDlg::markerSizeChanged);
  connect(startIndexSpinBox, qOverload<int>(&QSpinBox::valueChanged), this,
...
unsigned
ccPointListPickingDlg::getPickedPoints(std::vector<cc2DLabel *> &pickedPoints) {
  pickedPoints.clear();
  ...
  if (m_orderedLabelsContainer) {
    ...
    unsigned count = m_orderedLabelsContainer->filterChildren(
        labels, false, CC_TYPES::LABEL_2D);
    ...
    for (unsigned i = 0; i < count; ++i) {
      if (labels[i]->isA(
              CC_TYPES::LABEL_2D)) // Warning: cc2DViewportLabel is also a kind
...
void ccPointListPickingDlg::linkWithEntity(ccHObject *entity) {
  ...
  if (m_associatedEntity) {
    ...
    for (unsigned i = 0; i < count; ++i) {
      m_lastPreviousID =
          std::max(m_lastPreviousID, previousPickedPoints[i]->getUniqueID());
    }
  }
...
void ccPointListPickingDlg::exportToNewCloud() {
  ...
  if (count != 0) {
    ...
    if (cloud->reserve(count)) {
      ...
      ccLog::Error(
          "Can't export picked points as point cloud: not enough memory!");
      delete cloud;
      cloud = nullptr;
    }
  } else {
...
void ccPointListPickingDlg::applyAndExit() {
  if (m_associatedEntity && !m_toBeDeleted.empty()) {
    // apply modifications
    MainWindow::TheInstance()->db()->removeElements(
        m_toBeDeleted); // no need to redraw as they should already be invisible
    m_associatedEntity = nullptr;
  }
...
void ccPointListPickingDlg::removeLastEntry() {
  ...
  if (lastVisibleLabel->getUniqueID() <= m_lastPreviousID) {
    // old label: hide it and add it to the 'to be deleted' list (will be
    // restored if process is cancelled)
...
void ccPointListPickingDlg::markerSizeChanged(int size) {
  ...
  if (guiParams.labelMarkerSize != static_cast<unsigned>(size)) {
    ...
    m_associatedWin->setDisplayParameters(
        guiParams, m_associatedWin->hasOverriddenDisplayParameters());
    m_associatedWin->redraw();
  }
}
...
void ccPointListPickingDlg::updateList() {
  ...
  const int precision =
      m_associatedWin
          ? static_cast<int>(
                m_associatedWin->getDisplayParameters().displayedNumPrecision)
          : 6;

  const bool showAbsolute = showGlobalCoordsCheckBox->isEnabled() &&
  ...
  for (int i = 0; i < count; ++i) {
    ...
    tableWidget->verticalHeaderItem(i)->setText(
        QStringLiteral("%1").arg(i + startIndex));

    // update name as well
    if (label->getUniqueID() > m_lastPreviousID ||
        label->getName().startsWith(
            s_defaultLabelBaseName)) // DGM: we don't change the name of old
                                     // labels that have a non-default name
    {
    ...
    for (int j = 0; j < 3; ++j) {
      tableWidget->item(i, j + 1)->setText(
          QStringLiteral("%1").arg(Pd.u[j], 0, 'f', precision));
    }
  }
...
void ccPointListPickingDlg::processPickedPoint(const PickedItem &picked) {
  ...
  if (picked.entity->isKindOf(CC_TYPES::POINT_CLOUD)) {
    newLabel->addPickedPoint(static_cast<ccGenericPointCloud *>(picked.entity),
                             picked.itemIndex, picked.entityCenter);
  } else if (picked.entity->isKindOf(CC_TYPES::MESH)) {
    newLabel->addPickedPoint(
        static_cast<ccGenericMesh *>(picked.entity), picked.itemIndex,
        CCVector2d(picked.uvw.x, picked.uvw.y), picked.entityCenter);
  } else {
    delete newLabel;
    assert(false);
    return;
  ...
  if (!m_orderedLabelsContainer) {
    ...
    MainWindow::TheInstance()->addToDB(m_orderedLabelsContainer, false, true,
                                       false, false);
  }
  ...
  if (clipboard) {
    ...
    int indexInList =
        ...
        static_cast<int>(m_orderedLabelsContainer->getChildrenNumber()) - 1;

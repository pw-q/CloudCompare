...
void ccClippingBoxTool::editBox() {
  ...
  ccBoundingBoxEditorDlg bbeDlg(true, false,
                                this); // show the box 'axes' (orientation)
...
bool ccClippingBoxTool::addAssociatedEntity(ccHObject *entity) {
  ...
  if (!m_associatedWin || !m_clipBox) {
    ccLog::Error(QString(
        "[Clipping box] No associated 3D view or no valid clipping box!"));
    return false;
  }

  // we don't handle entities associated to another context
  if (entity->getDisplay() != m_associatedWin) {
    ccLog::Warning(QString("[Clipping box] Can't use entity '%1' cause it's "
                           "not displayed in the active 3D view!")
                       .arg(entity->getName()));
    return false;
  }

  bool firstEntity =
      (m_clipBox && m_clipBox->getContainer().getChildrenNumber() == 0);
  if (firstEntity) {
  ...
  if (win) {
    if (!m_clipBox) {
      m_clipBox = new ccClipBox(
          QString(), static_cast<unsigned>(ReservedIDs::CLIPPING_BOX));
      m_clipBox->setVisible(true);
      m_clipBox->setEnabled(true);
      m_clipBox->setSelected(showInteractorsToolButton->isChecked());
      connect(m_clipBox, &ccClipBox::boxModified, this,
              &ccClippingBoxTool::onBoxModified);
    }
...
bool ccClippingBoxTool::start() {
  ...
  // m_associatedWin->displayNewMessage(QString(),
  // ccGLWindowInterface::UPPER_CENTER_MESSAGE); //clear the area
  // m_associatedWin->displayNewMessage("[Rotation/Translation mode]",
...
void ccClippingBoxTool::stop(bool state) {
  if (m_clipBox) {
    if (state && m_clipBox->getContainer().getChildrenNumber()) {
      ...
      for (unsigned ci = 0; ci != m_clipBox->getContainer().getChildrenNumber();
           ++ci) {
        s_lastBoxParams[m_clipBox->getContainer().getChild(ci)->getUniqueID()] =
            params;
      }
    }
...
static unsigned ComputeGridDimensions(const ccBBox &localBox,
                                      ...
                                      int indexMins[3], int indexMaxs[3],
                                      int gridDim[3],
  ...
  for (unsigned char d = 0; d < 3; ++d) {
    if (processDim[d]) {
      if (CCCoreLib::LessThanEpsilon(cellSizePlusGap.u[d])) {
        ccLog::Error(
            "Box size (plus gap) is null! Can't apply repetitive process!");
        return 0;
      }
      ...
      PointCoordinateType a =
          ...
          cellSizePlusGap
              .u[d]; // don't forget the user defined gap between 'cells'
      PointCoordinateType b =
          (localBox.maxCorner().u[d] - gridOrigin.u[d]) / cellSizePlusGap.u[d];

      indexMins[d] =
          static_cast<int>(floor(a + static_cast<PointCoordinateType>(1.0e-6)));
      indexMaxs[d] =
          static_cast<int>(ceil(b - static_cast<PointCoordinateType>(1.0e-6))) -
          1;
...
bool ccClippingBoxTool::ExtractSlicesAndContours(
    ...
    const std::vector<ccGenericMesh *> &meshes, ccClipBox &clipBox,
    bool singleSliceMode, bool repeatDimensions[3],
    std::vector<ccHObject *> &outputSlices,

    bool extractEnvelopes, PointCoordinateType maxEdgeLength,
    ccEnvelopeExtractor::EnvelopeType envelopeType,
    std::vector<ccPolyline *> &outputEnvelopes,

    bool extractLevelSet, double levelSetGridStep, int levelSetMinVertCount,
    std::vector<ccPolyline *> &levelSet,

    PointCoordinateType gap /*=0*/, bool multiPass /*=false*/,
    bool splitEnvelopes /*=false*/, bool projectOnBestFitPlane /*=false*/,
    bool visualDebugMode /*=false*/, bool generateRandomColors /*=false*/,
  ...
  if (extractLevelSet && repeatDimensionsSum != 1) {
    assert(false);
    ccLog::Error("Only one repeat/flat dimension should be defined for level "
                 "set extraction");
  ...
  try {
    ...
    } else // repeat mode
    {
      if (!clouds.empty()) // extract sections from clouds
      {
        ...
        unsigned cellCount = ComputeGridDimensions(
            localBox, repeatDimensions, indexMins, indexMaxs, gridDim,
            gridOrigin, cellSizePlusGap);

        // we'll potentially create up to one (ref.) cloud per input loud and
        // per cell
        ...
        for (size_t ci = 0; ci != clouds.size(); ++ci) {
          ...
          for (unsigned i = 0; i < pointCount; ++i) {
            ...
            if (gap == 0 || ((P.x - static_cast<PointCoordinateType>(xi)) *
                                     cellSizePlusGap.x <=
                                 cellSize.x &&
                             ...
                             (P.z - static_cast<PointCoordinateType>(zi)) *
                                 ...
                                 cellSize.z)) {
              int cloudIndex =
                  ((zi - indexMins[2]) * static_cast<int>(gridDim[1]) +
                   (yi - indexMins[1])) *
                      static_cast<int>(gridDim[0]) +
                  (xi - indexMins[0]);
              assert(cloudIndex >= 0 &&
                     static_cast<size_t>(cloudIndex) * clouds.size() + ci <
                         refClouds.size());

              CCCoreLib::ReferenceCloud *&destCloud =
        ...
        for (int i = indexMins[0]; i <= indexMaxs[0]; ++i) {
          for (int j = indexMins[1]; j <= indexMaxs[1]; ++j) {
            for (int k = indexMins[2]; k <= indexMaxs[2]; ++k) {
              int cloudIndex =
                  ((k - indexMins[2]) * static_cast<int>(gridDim[1]) +
                   (j - indexMins[1])) *
                      static_cast<int>(gridDim[0]) +
                  (i - indexMins[0]);
              assert(cloudIndex >= 0 &&
                     static_cast<size_t>(cloudIndex) * clouds.size() <
                         refClouds.size());
              ...
              for (size_t ci = 0; ci != clouds.size(); ++ci) {
                ...
                if (destCloud) // some slices can be empty!
                {
                  ...
                  ccPointCloud *sliceCloud =
                      cloud->isA(CC_TYPES::POINT_CLOUD)
                          ? static_cast<ccPointCloud *>(cloud)->partialClone(
                                destCloud, &warnings)
                          : ccPointCloud::From(destCloud, cloud);
                  ...
                  if (sliceCloud) {
                    ...
                    QString slicePosStr = QString("(%1 ; %2 ; %3)")
                                              .arg(cellOrigin.x)
                                              .arg(cellOrigin.y)
                                              .arg(cellOrigin.z);
                    sliceCloud->setName(cloud->getName() +
                                        QString(".slice @ ") + slicePosStr);

                    // set meta-data
                    sliceCloud->setMetaData(s_originEntityUUID,
                                            cloud->getUniqueID());
                    sliceCloud->setMetaData(s_sliceID, slicePosStr);
                    sliceCloud->setMetaData("slice.origin.dim(0)",
                                            cellOrigin.x);
                    sliceCloud->setMetaData("slice.origin.dim(1)",
                                            cellOrigin.y);
                    sliceCloud->setMetaData("slice.origin.dim(2)",
                                            cellOrigin.z);
        ...
        for (int i = indexMins[0]; i <= indexMaxs[0]; ++i) {
          for (int j = indexMins[1]; j <= indexMaxs[1]; ++j) {
            for (int k = indexMins[2]; k <= indexMaxs[2]; ++k) {
              ...
              CCVector3 C = gridOrigin + CCVector3(i * cellSizePlusGap.x,
                                                   j * cellSizePlusGap.y,
                                                   k * cellSizePlusGap.z);
              ...
              for (size_t mi = 0; mi != meshes.size(); ++mi) {
                ...
                ccHObject *croppedEnt =
                    ccCropTool::Crop(mesh, cropBox, true, _transformation);
                if (croppedEnt) {
                  if (generateRandomColors) {
                  ...
                  QString slicePosStr =
                      QString("(%1 ; %2 ; %3)").arg(C.x).arg(C.y).arg(C.z);
                  croppedEnt->setName(mesh->getName() + QString(".slice @ ") +
                                      slicePosStr);

                  // set meta-data
                  croppedEnt->setMetaData(s_originEntityUUID,
                                          mesh->getUniqueID());
    ...
    if (!error && extractLevelSet && cloudSliceCount != 0) {
           ...
           ++iteration) // fake loop for easy break
      {
        if (progressDialog) {
          ...
          progressDialog->setInfo(
              QObject::tr("Level(s): %L1").arg(cloudSliceCount));
        ...
        unsigned gridWidth =
            1 + static_cast<unsigned>(gridSize.u[X] / levelSetGridStep + 0.5);
        unsigned gridHeight =
            1 + static_cast<unsigned>(gridSize.u[Y] / levelSetGridStep + 0.5);
        ...
        ccRasterGrid grid;
        if (!grid.init(gridWidth, gridHeight, levelSetGridStep,
                       CCVector3d(0, 0, 0))) {
          ccLog::Error("Not enough memory!");
          error = true;
          break;
        ...
        for (size_t i = 0; i < cloudSliceCount; ++i) {
          ccPointCloud *sliceCloud =
              ccHObjectCaster::ToPointCloud(outputSlices[i]);
          assert(sliceCloud);

          double sliceZ =
              sliceCloud->getMetaData(QString("slice.origin.dim(%1)").arg(Z))
                  .toDouble();
          ...
          for (unsigned pi = 0; pi != sliceCloud->size(); ++pi) {
            ...
            if (i < 0 || i >= static_cast<int>(gridWidth) || j < 0 ||
                j >= static_cast<int>(gridHeight)) {
          ...
          if (ccContourLinesGenerator::GenerateContourLines(
                  &grid, CCVector2d(gridOrigin.u[X], gridOrigin.u[Y]), params,
                  contours)) {
            for (size_t k = 0; k < contours.size(); ++k) {
              ...
              for (unsigned pi = 0; pi < vertices->size(); ++pi) {
                // convert the vertices from the local coordinate system to the
                // global one
              ...
              poly->setName(QString("Contour line %1=%2 (#%3)")
                                .arg(s_dimNames[Z])
                                .arg(sliceZ)
                                .arg(k + 1));
              ...
              poly->setMetaData(
                  ...
                  QVariant(sliceZ)); // replace the 'altitude' meta-data by the
                                     // right value

              // set meta-data
              poly->setMetaData(s_originEntityUUID,
                                sliceCloud->getMetaData(s_originEntityUUID));
              poly->setMetaData(s_sliceID, sliceCloud->getMetaData(s_sliceID));
              poly->setMetaData("slice.origin.dim(0)",
                                sliceCloud->getMetaData("slice.origin.dim(0)"));
              poly->setMetaData("slice.origin.dim(1)",
                                sliceCloud->getMetaData("slice.origin.dim(1)"));
              poly->setMetaData("slice.origin.dim(2)",
                                sliceCloud->getMetaData("slice.origin.dim(2)"));

              levelSet.push_back(poly);
            }
          } else {
            ccLog::Warning(tr("Failed to generate contour lines for cloud #%1")
                               .arg(i + 1));
          }
          ...
          if (progressDialog) {
            if (progressDialog->wasCanceled()) {
              error = true;
              ccLog::Warning(
                  tr("[ExtractSlicesAndContours] Process canceled by user"));
              break;
            }
    ...
    if (!error && extractEnvelopes && cloudSliceCount != 0) {
      ...
      if (repeatDimensionsSum == 1) {
        for (int i = 0; i < 3; ++i) {
          if (repeatDimensions[i]) {
            ...
            if (!projectOnBestFitPlane) // otherwise the normal will be
      ...
      for (size_t i = 0; i < cloudSliceCount; ++i) {
        ...
        if (ccEnvelopeExtractor::ExtractFlatEnvelope(
                sliceCloud, multiPass, maxEdgeLength, polys, envelopeType,
                splitEnvelopes, preferredNormDir, preferredUpDir,
                visualDebugMode)) {
          if (!polys.empty()) {
            ...
            ccLog::Warning(tr("%1: points are too far from each other! "
                              "Increase the max edge length")
                               .arg(sliceCloud->getName()));
            warningsIssued = true;
          }
        } else {
          ccLog::Warning(
              tr("%1: envelope extraction failed!").arg(sliceCloud->getName()));
          warningsIssued = true;
        }
    ...
    if (error) {
      ...
      ccLog::Warning(tr("[ExtractSlicesAndContours] Warnings were issued "
                        "during the process! (result may be incomplete)"));
    }
  } catch (const std::bad_alloc &) {
...
void ccClippingBoxTool::extractSlicesAndContours(bool singleSliceMode) {
  ...
  repeatDlg.extractSliceEntitiesGroupBox->setChecked(
      s_extractSliceCloudsOrMeshes);
  ...
  repeatDlg.maxEnvelopeEdgeLengthDoubleSpinBox->setValue(
      s_maxEnvelopeEdgeLength);
  repeatDlg.multiPassEnvelopeCheckBox->setChecked(s_multiPassEnvelope);
  repeatDlg.envProjectPointsOnBestFitCheckBox->setChecked(
  ...
  s_extractSliceCloudsOrMeshes =
      repeatDlg.extractSliceEntitiesGroupBox->isChecked();
  ...
  s_maxEnvelopeEdgeLength =
      repeatDlg.maxEnvelopeEdgeLengthDoubleSpinBox->value();
  s_multiPassEnvelope = repeatDlg.multiPassEnvelopeCheckBox->isChecked();
  s_envProjectPointsOnBestFitPlane =
      repeatDlg.envProjectPointsOnBestFitCheckBox->isChecked();
  ...
  ccEnvelopeExtractor::EnvelopeType envelopeType =
      ccEnvelopeExtractor::EnvelopeType::FULL;
  ...
  if (!ExtractSlicesAndContours(
          clouds, meshes, *m_clipBox, singleSliceMode, processDim, outputSlices,

          s_extractEnvelopes,
          static_cast<PointCoordinateType>(s_maxEnvelopeEdgeLength),
          envelopeType, outputEnvelopes,

          s_extractLevelSet, s_levelSetGridStep, s_levelSetMinVertCount,
          outputLevelSet,

          static_cast<PointCoordinateType>(s_defaultGap), s_multiPassEnvelope,
          s_splitEnvelopes, s_envProjectPointsOnBestFitPlane,
          s_envelopeDebugMode, s_generateRandomColors, &pDlg)) {
    // process failed (error message has already been issued)
    return;
  }

  ccLog::Print("[ccClippingBoxTool] Processed finished in %.2f s.",
    ...
    case OutputFormat::BY_SLICE_THEN_ENTITY: {
      ...
      if (s_groupByIndex == OutputFormat::BY_SLICE_THEN_ENTITY) {
        ...
        if (perEntityGroupNames.contains(entityUUID)) {
          destGroup =
              FindOrCreateChildren(destGroup, perEntityGroupNames[entityUUID]);
        }
      }
    } break;
  ...
  if (sliceCount) {
    if (s_extractSliceCloudsOrMeshes) {
      if (s_groupByIndex == OutputFormat::BY_TYPE) {
        ...
        sliceGroup =
            new ccHObject(baseName.isEmpty() ? QString("Slices")
                                             : baseName + QString(".slices"));
      }
  ...
  if (!outputEnvelopes.empty()) {
    if (s_groupByIndex == OutputFormat::BY_TYPE) {
      // we have to create the destination group
      envelopeGroup =
          new ccHObject(baseName.isEmpty() ? QString("Envelopes")
                                           : baseName + QString(".envelopes"));
    }
  ...
  if (!outputLevelSet.empty()) {
    if (s_groupByIndex == OutputFormat::BY_TYPE) {
      // we have to create the destination group
      levelSetGroup =
          new ccHObject(baseName.isEmpty() ? QString("Level set")
                                           : baseName + QString(".levelSet"));
    }
  ...
  if (sliceCount != 0) {
    QMessageBox::warning(
        nullptr, tr("Process finished"),
        tr("%1 slices have been generated.\n(you may have to close the tool "
           "and hide the initial cloud to see them...)")
            .arg(sliceCount));
  } else if (s_extractSliceCloudsOrMeshes) {
    QMessageBox::warning(nullptr, tr("Process finished"),
                         tr("The process has generated no output"));
  }
}
...
void ccClippingBoxTool::shiftBox(unsigned char dim, bool minus) {
  ...
  PointCoordinateType width =
      (m_clipBox->getBox().maxCorner() - m_clipBox->getBox().minCorner())
          .u[dim];
...
void ccClippingBoxTool::closeDialog() { stop(true); }

void ccClippingBoxTool::setTopView() { setView(CC_TOP_VIEW); }

void ccClippingBoxTool::setBottomView() { setView(CC_BOTTOM_VIEW); }

void ccClippingBoxTool::setFrontView() { setView(CC_FRONT_VIEW); }

void ccClippingBoxTool::setBackView() { setView(CC_BACK_VIEW); }

void ccClippingBoxTool::setLeftView() { setView(CC_LEFT_VIEW); }

void ccClippingBoxTool::setRightView() { setView(CC_RIGHT_VIEW); }

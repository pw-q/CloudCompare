...
ccInnerRect2DFinder::ccInnerRect2DFinder()
    : m_maxArea(0), m_cloud(nullptr), m_X(0), m_Y(1) {}
...
ccBox *ccInnerRect2DFinder::process(ccGenericPointCloud *cloud,
  ...
  if (m_maxArea > 0) {
    ...
    boxCenter.u[m_X] =
        static_cast<PointCoordinateType>((m_maxRect.x0 + m_maxRect.x1) / 2);
    boxCenter.u[m_Y] =
        static_cast<PointCoordinateType>((m_maxRect.y0 + m_maxRect.y1) / 2);

...
ccHObject *ccCropTool::Crop(ccHObject *entity, const ccBBox &box,
                            bool inside /*=true*/,
  ...
  if (entity->isA(CC_TYPES::POINT_CLOUD)) {
    ...
    if (!selection) {
      // process failed!
      ccLog::Warning(
          QString("[Crop] Failed to crop cloud '%1'!").arg(cloud->getName()));
      return nullptr;
    }
    ...
    if (selection->size() == 0) {
      ...
      ccLog::Warning(
          QString(
              "[Crop] No point of the cloud '%1' falls %2side the input box!")
    ...
    params.trackOrigIndexes =
        mesh->hasColors() || mesh->hasScalarFields() || mesh->hasMaterials();
    ...
    if (meshRotation) {
      ...
      if (!rotatedVertices) {
        ccLog::Warning(
            QString("[Crop] Failed to crop mesh '%1'! (not enough memory)")
                .arg(mesh->getName()));
        return nullptr;
      }
    ...
    if (!CCCoreLib::ManualSegmentationTools::segmentMeshWithAABox(
            mesh, cropVertices, params)) {
      // process failed!
      ccLog::Warning(
          QString("[Crop] Failed to crop mesh '%1'!").arg(mesh->getName()));
    }
    ...
    if (tempMesh) {
      ...
      if (croppedVertices) {
        ...
        if (croppedMesh->size() == 0) {
          // no points fall inside selection!
          ccLog::Warning(QString("[Crop] No triangle of the mesh '%1' falls "
                                 "%2side the input box!")
                             .arg(mesh->getName(), (inside ? "in" : "out")));
          delete croppedMesh;
          croppedMesh = nullptr;
        } else {
          assert(origVertices);
          ...
          if (mesh->hasNormals()) {
            ...
            if (!success) {
              ccLog::Warning("[Crop] Failed to compute normals on the output "
          ...
          if (params.trackOrigIndexes) {
            ...
            try {
              ...
              if (origVertices->hasColors() ||
                ...
                // we use flags to avoid processing the same vertex multiple
                // times
                ...
                if (origVertices->hasColors()) {
                  ...
                  if (!importColors)
                    ccLog::Warning("[Crop] Failed to transfer RGB colors on "
                                   "the output mesh (not enough memory)");
                }
                ...
                if (origVertices->hasScalarFields()) {
                  origVertices_pc =
                      origVertices->isA(CC_TYPES::POINT_CLOUD)
                          ? static_cast<ccPointCloud *>(origVertices)
                          : nullptr;
                  unsigned sfCount =
                      origVertices_pc
                          ? origVertices_pc->getNumberOfScalarFields()
                          : 1;
                  ...
                  for (unsigned i = 0; i < sfCount; ++i) {
                    int sfIdx = croppedVertices->addScalarField(
                        origVertices_pc
                            ? origVertices_pc->getScalarField(i)->getName()
                            : "Scalar field");
                    if (sfIdx >= 0) {
                      ccScalarField *sf = static_cast<ccScalarField *>(
                          croppedVertices->getScalarField(i));
                      sf->fill(CCCoreLib::NAN_VALUE);
                      if (origVertices_pc) {
                        // import display parameters if possible
                        ccScalarField *originSf = static_cast<ccScalarField *>(
                            origVertices_pc->getScalarField(i));
                        assert(originSf);
                      ...
                      ccLog::Warning(
                          "[Crop] Failed to transfer one or several scalar "
                          "fields on the output mesh (not enough memory)");
                  ...
                  if (origVertices_pc)
                    croppedVertices->setCurrentDisplayedScalarField(std::max(
                        static_cast<int>(
                            croppedVertices->getNumberOfScalarFields()) -
                            1,
                        origVertices_pc
                ...
                if (importColors || importSFs) {
                  ...
                  for (unsigned i = 0; i < croppedMesh->size(); ++i) {
                    ...
                    const CCCoreLib::VerticesIndexes *tsio =
                        mesh->getTriangleVertIndexes(origTriIndex);

                    // get the new triangle
                    const CCCoreLib::VerticesIndexes *tsic =
                        croppedMesh->getTriangleVertIndexes(i);
                    ...
                    for (unsigned j = 0; j < 3; ++j) {
                      ...
                      const CCVector3 *Vcj =
                          croppedVertices->getPoint(vertIndex);
                      ...
                      if (importSFs) {
                        CCVector3d w;
                        mesh->computeInterpolationWeights(origTriIndex, *Vcj,
                                                          w);

                        // import SFs
                        for (unsigned s = 0;
                             s < static_cast<unsigned>(importedSFs.size());
                             ++s) {
                          CCVector3d scalarValues(0, 0, 0);
                          if (origVertices_pc) {
                            const CCCoreLib::ScalarField *sf =
                                origVertices_pc->getScalarField(s);
                            ...
                            assert(s == 0);
                            scalarValues.x =
                                origVertices->getPointScalarValue(tsio->i1);
                            scalarValues.y =
                                origVertices->getPointScalarValue(tsio->i2);
                            scalarValues.z =
                                origVertices->getPointScalarValue(tsio->i3);
                          }

                          ScalarType sVal =
                              static_cast<ScalarType>(scalarValues.dot(w));
                  ...
                  croppedVertices->showColors(importColors &&
              ...
              if (mesh->hasMaterials()) {
                ...
                if (origMaterialSet && !origMaterialSet->empty() &&
                    croppedMesh->reservePerTriangleMtlIndexes()) {
                  ...
                  // import materials
                  {
                    ...
                    if (materialUsedCount == materialUsed.size()) {
                      ...
                      ccMaterialSet *matSet =
                          new ccMaterialSet(origMaterialSet->getName());
                      {
                        matSet->reserve(materialUsedCount);
                        for (size_t i = 0; i < materialUsed.size(); ++i) {
                          if (materialUsed[i] >= 0) {
                            matSet->push_back(ccMaterial::Shared(
                                new ccMaterial(*origMaterialSet->at(i))));
                            // update index
                            materialUsed[i] =
                                static_cast<int>(matSet->size()) - 1;
                          }
                        }
                      }
                      ...
                      for (unsigned i = 0; i < croppedMesh->size(); ++i) {
                        ...
                        if (mtlIndex >= 0) {
                          assert(mtlIndex <
                                 static_cast<int>(materialUsed.size()));
                          croppedMesh->setTriangleMtlIndex(
                              i, materialUsed[mtlIndex]);
                        }
                      }
                    }
                  }
                  ...
                  ccLog::Warning("[Crop] Failed to transfer materials on the "
                ...
                if (mesh->hasPerTriangleTexCoordIndexes()) {
                  ...
                  if (croppedMesh->reservePerTriangleTexCoordIndexes() &&
                    ...
                    for (unsigned i = 0; i < croppedMesh->size(); ++i) {
                      ...
                      mesh->getTriangleTexCoordinates(origTriIndex, tx1, tx2,
                                                      tx3);
                      ...
                      for (unsigned j = 0; j < 3; ++j) {
                        ...
                        if ((tx1 || CCCoreLib::LessThanEpsilon(w.u[0])) &&
                          ...
                          TexCoords2D t(static_cast<float>(
                                            (tx1 ? tx1->tx * w.u[0] : 0.0) +
                                            (tx2 ? tx2->tx * w.u[1] : 0.0) +
                                            (tx3 ? tx3->tx * w.u[2] : 0.0)),
                                        static_cast<float>(
                                            (tx1 ? tx1->ty * w.u[0] : 0.0) +
                                            (tx2 ? tx2->ty * w.u[1] : 0.0) +
                                            (tx3 ? tx3->ty * w.u[2] : 0.0)));

                          texCoords->addElement(t);
                          texIndexes[j] =
                              static_cast<int>(texCoords->currentSize()) - 1;
                        }
                      }

                      croppedMesh->addTriangleTexCoordIndexes(
                          texIndexes[0], texIndexes[1], texIndexes[2]);
                    }
                    croppedMesh->setTexCoordinatesTable(texCoords);
                  } else {
                    ccLog::Warning(
                        "[Crop] Failed to transfer texture coordinates on the "
                        "output mesh (not enough memory)");
                    delete texCoords;
                    texCoords = nullptr;
                  }
              ...
              ccLog::Warning(
                  "[Crop] Failed to transfer per-vertex features (color, SF "
                  "values, etc.) on the output mesh (not enough memory)");
        ...
        ccLog::Warning(
            "[Crop] Failed to create output mesh vertices (not enough memory)");
      }
    }

...
//! Map between a file version, and the first version of CloudCompare that was
//! able to load it
...
ccApplication::ccApplication(int &argc, char **argv, bool isCommandLine)
    : ccApplicationBase(argc, argv, isCommandLine,
                        QString("2.14.alpha (%1)").arg(__DATE__)) {
...
bool ccApplication::event(QEvent *inEvent) {
  ...
  case QEvent::FileOpen: {
    ...
    mainWindow->addToDB(
        QStringList(static_cast<QFileOpenEvent *>(inEvent)->file()));
    return true;
  }

...
MatrixDisplayDlg::MatrixDisplayDlg(QWidget *parent /*=nullptr*/)
    : QWidget(parent), m_ui(new Ui::MatrixDisplayDlg) {
  m_ui->setupUi(this);

  connect(m_ui->exportToAsciiPushButton, &QAbstractButton::clicked, this,
          &MatrixDisplayDlg::exportToASCII);
  connect(m_ui->exportToClipboardPushButton, &QAbstractButton::clicked, this,
          &MatrixDisplayDlg::exportToClipboard);

  show();
}

MatrixDisplayDlg::~MatrixDisplayDlg() { delete m_ui; }
...
void MatrixDisplayDlg::fillDialogWith(const CCVector3d &axis, double angle_rad,
                                      const CCVector3d &T, int precision) {
  // center position
  QString centerStr = QStringLiteral("%0 ; %1 ; %2")
                          .arg(T.x, 0, 'f', precision)
                          .arg(T.y, 0, 'f', precision)
                          .arg(T.z, 0, 'f', precision);
  // rotation axis
  QString axisStr = QStringLiteral("%0 ; %1 ; %2")
                        .arg(axis.x, 0, 'f', precision)
                        .arg(axis.y, 0, 'f', precision)
                        .arg(axis.z, 0, 'f', precision);
  // rotation angle
  QString angleStr = QStringLiteral("%1 deg.").arg(
      CCCoreLib::RadiansToDegrees(angle_rad), 0, 'f', precision);

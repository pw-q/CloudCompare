...
sfEditDlg::sfEditDlg(QWidget *parent /*=nullptr*/)
    : QWidget(parent), m_associatedSF(nullptr), m_associatedSFHisto(nullptr),
  ...
  connect(m_ui->minValSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &sfEditDlg::minValSBChanged);
  connect(m_ui->maxValSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &sfEditDlg::maxValSBChanged);
  connect(m_ui->minSatSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
          this, &sfEditDlg::minSatSBChanged);
  connect(m_ui->maxSatSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged),
...
void sfEditDlg::symmetricalScaleChanged(bool state) {
  ...
  if (m_associatedSF->symmetricalScale() != state) {
    ...
    fillDialogWith(
        m_associatedSF); // the saturation sliders may need to be updated!
    Q_EMIT entitySFHasChanged();
...
void sfEditDlg::logScaleChanged(bool state) {
  ...
  if (m_associatedSF->logScale() != state) {
    ...
    fillDialogWith(m_associatedSF); // the saturation sliders + the symmetrical

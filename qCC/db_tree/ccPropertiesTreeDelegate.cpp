...
CHECKABLE_ITEM(bool checkState,
               ccPropertiesTreeDelegate::CC_PROPERTY_ROLE role) {
...
static QStandardItem *
PERSISTENT_EDITOR(ccPropertiesTreeDelegate::CC_PROPERTY_ROLE role) {
...
ccPropertiesTreeDelegate::ccPropertiesTreeDelegate(QStandardItemModel *model,
                                                   ...
                                                   QObject *parent)
    : QStyledItemDelegate(parent), m_currentObject(nullptr), m_model(model),
      m_view(view), m_lastFocusItemRole(OBJECT_NO_PROPERTY) {
...
void ccPropertiesTreeDelegate::fillModel(ccHObject *hObject) {
  ...
  if (m_currentObject->isHierarchy()) {
    if (!m_currentObject->isA(
            CC_TYPES::VIEWPORT_2D_LABEL)) // don't need to display this kind of
                                          // info for viewport labels!
    {
...
void ccPropertiesTreeDelegate::addSeparator(const QString &title) {
  if (m_model) {
    // DGM: we can't use the 'text' of the item as it will be displayed under
    // the associated editor (label)! So we simply use the 'accessible
    // description' field
...
void ccPropertiesTreeDelegate::fillWithHObject(ccHObject *_obj) {
  ...
  // Bounding-box
  {
    ...
    if (box.isValid()) {
      // name in 3D (we can't display a 3D name if the bounding-box is not
      // valid!
      ...
      appendRow(
          ITEM(fitBBox ? tr("Local box dimensions") : tr("Box dimensions")),
          ITEM(QStringLiteral(
                   "X: %1 (%2 : %3)\nY: %4 (%5 : %6)\nZ: %7 (%8 : %9)")
                   .arg(bboxDiag.x)
                   .arg(box.minCorner().x)
...
void ccPropertiesTreeDelegate::fillWithShifted(const ccShiftedObject *_obj) {
  ...
  appendRow(ITEM(tr("Global shift")), ITEM(QStringLiteral("(%1;%2;%3)")
                                               .arg(shift.x, 0, 'f', 2)
                                               .arg(shift.y, 0, 'f', 2)
                                               .arg(shift.z, 0, 'f', 2)));
  ...
  appendRow(ITEM(tr("Global scale")),
            ITEM(QStringLiteral("%1").arg(scale, 0, 'f', 6)));
}
...
void ccPropertiesTreeDelegate::fillWithPointCloud(ccGenericPointCloud *_obj) {
  ...
  // custom point size
  appendRow(ITEM(tr("Point size")), PERSISTENT_EDITOR(OBJECT_CLOUD_POINT_SIZE),
            true);
  ...
  if (_obj->isA(CC_TYPES::POINT_CLOUD)) {
    ...
    if (gridCount != 0) {
      ...
      for (size_t i = 0; i < gridCount; ++i) {
        ...
        appendRow(ITEM(tr("Scan #%1").arg(i + 1)),
                  ITEM(tr("%1 x %2 (%3 points)")
                           .arg(grid->w)
                           .arg(grid->h)
                           .arg(QLocale(QLocale::English)
                                    .toString(grid->validCount))));
      }
    }
...
void ccPropertiesTreeDelegate::fillWithDrawNormals(ccGenericPointCloud *_obj) {
  ...
  // visibility
  const ccPointCloud *cloud = static_cast<const ccPointCloud *>(_obj);
  appendRow(ITEM(tr("Draw")), CHECKABLE_ITEM(cloud->normalsAreDrawn(),
                                             OBJECT_CLOUD_DRAW_NORMALS));

  // normals length
  appendRow(ITEM(tr("Length")), PERSISTENT_EDITOR(OBJECT_CLOUD_NORMAL_LENGTH),
            true);

  // normals color
  appendRow(ITEM(tr("Color")), PERSISTENT_EDITOR(OBJECT_CLOUD_NORMAL_COLOR),
            true);
}
...
void ccPropertiesTreeDelegate::fillSFWithPointCloud(ccGenericPointCloud *_obj) {
  ...
  if (sfCount != 0) {
    ...
    if (sf) {
      ...
      // color scale steps
      appendRow(ITEM(tr("Steps")), PERSISTENT_EDITOR(OBJECT_COLOR_RAMP_STEPS),
                true);

      // scale visible?
      appendRow(ITEM(tr("Visible")), CHECKABLE_ITEM(cloud->sfColorScaleShown(),
...
void ccPropertiesTreeDelegate::fillWithPrimitive(
    const ccGenericPrimitive *_obj) {
  assert(_obj && m_model);
  if (!_obj || !m_model) {
    return;
  }
  ...
  } else if (_obj->isKindOf(CC_TYPES::CONE)) // cylinders are also cones!
  {
    ...
    appendRow(ITEM(tr("Half angle")),
              ITEM(QStringLiteral("%1 deg.").arg(angle_deg)));
...
void ccPropertiesTreeDelegate::fillWithFacet(const ccFacet *_obj) {
  ...
  // center
  appendRow(ITEM(tr("Center")), ITEM(QStringLiteral("(%1 ; %2 ; %3)")
  ...
  if (_obj->getContour()) {
    appendRow(
        ITEM(tr("Show contour")),
        CHECKABLE_ITEM(_obj->getContour()->isVisible(), OBJECT_FACET_CONTOUR));
  }

  // polygon visibility
  if (_obj->getPolygon()) {
    appendRow(
        ITEM(tr("Show polygon")),
        CHECKABLE_ITEM(_obj->getPolygon()->isVisible(), OBJECT_FACET_MESH));
  }
}

void ccPropertiesTreeDelegate::fillWithPlanarEntity(
    const ccPlanarEntityInterface *_obj) {
  if (!_obj || !m_model) {
    return;
  }
  // normal
  ...
  appendRow(ITEM(tr("Normal")),
            ITEM(QStringLiteral("(%1 ; %2 ; %3)").arg(N.x).arg(N.y).arg(N.z)));
  ...
  appendRow(ITEM(tr("Dip / Dip dir. (integer)")),
            ITEM(QStringLiteral("(%1 ; %2) deg.")
                     .arg(static_cast<int>(std::round(dip_deg)))
                     .arg(static_cast<int>(std::round(dipDir_deg)))));
  appendRow(ITEM(tr("Dip / Dip dir.")), ITEM(QStringLiteral("(%1; %2) deg.")
                                                 .arg(dip_deg, 0, 'f', 2)
                                                 .arg(dipDir_deg, 0, 'f', 2)));

  // normal vector visibility
  appendRow(
      ITEM(tr("Show normal vector")),
      CHECKABLE_ITEM(_obj->normalVectorIsShown(), OBJECT_PLANE_NORMAL_VECTOR));
}
...
void ccPropertiesTreeDelegate::fillWithPointOctree(const ccOctree *_obj) {
  ...
  const double cellSize =
      static_cast<double>(_obj->getCellSize(static_cast<unsigned char>(level)));
  ...
  appendRow(ITEM(tr("Filled volume")),
            ITEM(QString::number(cellCount * pow(cellSize, 3.0))));
}
...
void ccPropertiesTreeDelegate::fillWithLabel(const cc2DLabel *_obj) {
  ...
  // Body
  QStringList body =
      _obj->getLabelContent(ccGui::Parameters().displayedNumPrecision);
...
void ccPropertiesTreeDelegate::fillWithViewportObject(
  ...
  appendRow(
      ITEM(tr("Name")),
      ITEM(_obj->getName().isEmpty() ? tr("undefined") : _obj->getName()));
...
void ccPropertiesTreeDelegate::fillWithTransBuffer(
    const ccIndexedTransformationBuffer *_obj) {
  assert(_obj && m_model);
  if (!_obj || !m_model) {
    return;
  }
  ...
  appendRow(ITEM(tr("Show path")),
            CHECKABLE_ITEM(_obj->isPathShownAsPolyline(),
                           OBJECT_SHOW_TRANS_BUFFER_PATH));

  // Show trihedrons
  appendRow(ITEM(tr("Show trihedrons")),
            CHECKABLE_ITEM(_obj->trihedronsShown(),
...
void ccPropertiesTreeDelegate::fillWithSensor(const ccSensor *_obj) {
  ...
  if (minIndex != maxIndex) {
    // Index span
    appendRow(ITEM(tr("Indices")),
              ITEM(QStringLiteral("%1 - %2").arg(minIndex).arg(maxIndex)));

    // Current index
    appendRow(ITEM(tr("Active index")), PERSISTENT_EDITOR(OBJECT_SENSOR_INDEX),
              true);
  }
}
...
void ccPropertiesTreeDelegate::fillWithGBLSensor(const ccGBLSensor *_obj) {
  ...
  addSeparator(tr("Angular viewport (degrees)"));
  {
    ...
    appendRow(ITEM(tr("Yaw step")),
              ITEM(QStringLiteral("%1").arg(
                  CCCoreLib::RadiansToDegrees(yawStep), 0, 'f', 4)));
    ...
    appendRow(ITEM(tr("Pitch step")),
              ITEM(QStringLiteral("%1").arg(
                  CCCoreLib::RadiansToDegrees(pitchStep), 0, 'f', 4)));
  }
...
void ccPropertiesTreeDelegate::fillWithCameraSensor(
    const ccCameraSensor *_obj) {
  assert(_obj && m_model);
  if (!_obj || !m_model) {
    return;
  }
  ...
  // Array size
  appendRow(ITEM(tr("Array size")), ITEM(QStringLiteral("%1 x %2")
  ...
  if (params.pixelSize_mm[0] != 0 || params.pixelSize_mm[1] != 0) {
    appendRow(ITEM(tr("Pixel size")), ITEM(QStringLiteral("%1 x %2")
  ...
  appendRow(ITEM(tr("Field of view")),
            ITEM(QString::number(CCCoreLib::RadiansToDegrees(params.vFOV_rad)) +
                 " deg."));
...
void ccPropertiesTreeDelegate::fillWithShareable(const CCShareable *_obj) {
  ...
  // Link count
  unsigned linkCount =
      _obj->getLinkCount(); // if we display it, it means it is a member of the
                            // DB --> i.e. link is already >1
...
void ccPropertiesTreeDelegate::fillWithCCArray(
    const ccArray<Type, N, ComponentType> *_obj) {
  assert(_obj && m_model);
  if (!_obj || !m_model) {
    return;
  }
  ...
  // Capacity
  appendRow(ITEM(tr("Capacity")),
            ITEM(QLocale(QLocale::English)
                     .toString(static_cast<qulonglong>(_obj->capacity()))));

  // Memory
  appendRow(ITEM(tr("Memory")),
            ITEM(QStringLiteral("%1 Mb").arg(
...
QWidget *
ccPropertiesTreeDelegate::createEditor(QWidget *parent,
  ...
  case OBJECT_CURRENT_DISPLAY: {
    ...
    connect(comboBox,
            ...
            &ccPropertiesTreeDelegate::objectDisplayChanged);
  ...
  case OBJECT_CURRENT_SCALAR_FIELD: {
    ...
    connect(comboBox, qOverload<int>(&QComboBox::activated), this,
            &ccPropertiesTreeDelegate::scalarFieldChanged);
  ...
  case OBJECT_CURRENT_COLOR_RAMP: {
    ccColorScaleSelector *selector = new ccColorScaleSelector(
        ...
        QString::fromUtf8(":/CC/images/ccGear.png"));
    ...
    connect(selector, &ccColorScaleSelector::colorScaleEditorSummoned, this,
            &ccPropertiesTreeDelegate::spawnColorRampEditor);
  ...
  case OBJECT_COLOR_RAMP_STEPS: {
    ...
    connect(spinBox, qOverload<int>(&QSpinBox::valueChanged), this,
            &ccPropertiesTreeDelegate::colorRampStepsChanged);
  ...
  case OBJECT_OCTREE_TYPE: {
    ...
    connect(comboBox, qOverload<int>(&QComboBox::activated), this,
            &ccPropertiesTreeDelegate::octreeDisplayModeChanged);
  ...
  case OBJECT_PRIMITIVE_PRECISION: {
    ...
    connect(spinBox, qOverload<int>(&QSpinBox::valueChanged), this,
            &ccPropertiesTreeDelegate::primitivePrecisionChanged);
  ...
  case OBJECT_SPHERE_RADIUS: {
    ...
    connect(spinBox, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
            &ccPropertiesTreeDelegate::sphereRadiusChanged);
  ...
  case OBJECT_CONE_HEIGHT: {
    ...
    connect(spinBox, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
            &ccPropertiesTreeDelegate::coneHeightChanged);
  ...
  case OBJECT_CONE_BOTTOM_RADIUS: {
    ...
    connect(spinBox, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
            &ccPropertiesTreeDelegate::coneBottomRadiusChanged);
  ...
  case OBJECT_CONE_TOP_RADIUS: {
    ...
    connect(spinBox, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
            &ccPropertiesTreeDelegate::coneTopRadiusChanged);
  ...
  case OBJECT_SENSOR_INDEX: {
    ...
    connect(spinBox, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
            &ccPropertiesTreeDelegate::sensorIndexChanged);
  ...
  case OBJECT_TRANS_BUFFER_TRIHDERONS_SCALE: {
    ...
    connect(spinBox, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
            &ccPropertiesTreeDelegate::trihedronsScaleChanged);
  ...
  case OBJECT_APPLY_IMAGE_VIEWPORT: {
    ...
    connect(button, &QAbstractButton::clicked, this,
            &ccPropertiesTreeDelegate::applyImageViewport);
  ...
  case OBJECT_SENSOR_DISPLAY_SCALE: {
    ...
    connect(spinBox, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
            &ccPropertiesTreeDelegate::sensorScaleChanged);
  ...
  case OBJECT_CLOUD_POINT_SIZE: {
    ...
    for (int i = static_cast<int>(ccGLWindowInterface::MIN_POINT_SIZE_F);
         i <= static_cast<int>(ccGLWindowInterface::MAX_POINT_SIZE_F); ++i) {
      comboBox->addItem(QString::number(i));
    }

    connect(comboBox, qOverload<int>(&QComboBox::currentIndexChanged), this,
            &ccPropertiesTreeDelegate::cloudPointSizeChanged);
  ...
  case OBJECT_POLYLINE_WIDTH: {
    ...
    for (int i = static_cast<int>(ccGLWindowInterface::MIN_LINE_WIDTH_F);
         i <= static_cast<int>(ccGLWindowInterface::MAX_LINE_WIDTH_F); ++i) {
      comboBox->addItem(QString::number(i));
    }

    connect(comboBox, qOverload<int>(&QComboBox::currentIndexChanged), this,
            &ccPropertiesTreeDelegate::polyineWidthChanged);
    ...
    if (m_currentObject) {
      if (m_currentObject->hasColors()) {
        ...
        comboBox->setItemIcon(
            comboBox->count() - 1,
            QIcon(QString::fromUtf8(":/CC/images/typeRgbCcolor.png")));
      }
      if (m_currentObject->hasScalarFields()) {
        comboBox->addItem(tr(s_sfColor));
        comboBox->setItemIcon(
            comboBox->count() - 1,
            QIcon(QString::fromUtf8(":/CC/images/typeSF.png")));
      }
      connect(comboBox,
              qOverload<const QString &>(&QComboBox::currentIndexChanged), this,
              &ccPropertiesTreeDelegate::colorSourceChanged);
    }
  ...
  case OBJECT_COORDINATE_SYSTEM_AXES_WIDTH: {
    ...
    for (int i = static_cast<int>(ccCoordinateSystem::MIN_AXIS_WIDTH_F);
         i <= static_cast<int>(ccCoordinateSystem::MAX_AXIS_WIDTH_F); ++i) {
      comboBox->addItem(QString::number(i));
    }
    ...
    connect(comboBox, qOverload<int>(&QComboBox::currentIndexChanged), this,
            &ccPropertiesTreeDelegate::coordinateSystemAxisWidthChanged);
  ...
  case OBJECT_COORDINATE_SYSTEM_DISP_SCALE: {
    ...
    connect(spinBox, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
            &ccPropertiesTreeDelegate::coordinateSystemDisplayScaleChanged);
  ...
  case OBJECT_CLOUD_NORMAL_COLOR: {
    ...
    connect(comboBox, qOverload<int>(&QComboBox::currentIndexChanged), this,
            &ccPropertiesTreeDelegate::normalColorChanged);
  ...
  case OBJECT_CLOUD_NORMAL_LENGTH: {
    ...
    connect(spinBox, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
            &ccPropertiesTreeDelegate::normalLengthChanged);
  ...
  if (outputWidget) {
    outputWidget->setFocusPolicy(
        Qt::StrongFocus); // Qt doc: << The returned editor widget should have
                          // Qt::StrongFocus >>
    outputWidget->installEventFilter(
        new FilterMouseWheelNoFocusEvent(outputWidget));
...
void SetDoubleSpinBoxValue(QWidget *editor, double value,
...
void SetSpinBoxValue(QWidget *editor, int value,
                     bool keyboardTracking = false) {
...
void ccPropertiesTreeDelegate::setEditorData(QWidget *editor,
  ...
  case OBJECT_CURRENT_DISPLAY: {
    ...
    ccGLWindowInterface *win =
        static_cast<ccGLWindowInterface *>(m_currentObject->getDisplay());
  ...
  case OBJECT_SENSOR_UNCERTAINTY: {
    ...
    assert(sensor);
    lineEdit->setText(
        QString::number(sensor ? sensor->getUncertainty() : 0, 'g', 8));
    break;
  }
  case OBJECT_SENSOR_DISPLAY_SCALE: {
    ccSensor *sensor = ccHObjectCaster::ToSensor(m_currentObject);
  ...
  case OBJECT_TRANS_BUFFER_TRIHDERONS_SCALE: {
    ...
    SetDoubleSpinBoxValue(editor,
                          buffer ? buffer->trihedronsDisplayScale() : 0.0);
    break;
  }
  case OBJECT_CLOUD_POINT_SIZE: {
  ...
  case OBJECT_CLOUD_NORMAL_COLOR: {
    ...
    SetComboBoxIndex(editor,
                     static_cast<ccPointCloud *>(cloud)->getNormalLineColor());
    break;
  }
  case OBJECT_CLOUD_NORMAL_LENGTH: {
    ccGenericPointCloud *cloud =
    ...
    SetDoubleSpinBoxValue(
        editor, static_cast<ccPointCloud *>(cloud)->getNormalLength());
    break;
  }
  default:
...
void ccPropertiesTreeDelegate::updateDisplay() {
  ...
  if (!objectIsDisplayed) {
    ...
    if (object->isKindOf(CC_TYPES::POINT_CLOUD)) {
      ...
      if (parent && parent->isKindOf(CC_TYPES::MESH) &&
...
void ccPropertiesTreeDelegate::spawnColorRampEditor() {
  ...
  if (sf) {
    ...
    ccColorScaleEditorDialog *editorDialog = new ccColorScaleEditorDialog(
        ccColorScalesManager::GetUniqueInstance(), MainWindow::TheInstance(),
        sf->getColorScale(), glWindow ? glWindow->asWidget() : nullptr);
...
void ccPropertiesTreeDelegate::coneBottomRadiusChanged(double val) {
  ...
  if (cone->getBottomRadius() != radius) {
    ...
    cone->setBottomRadius(
        radius); // works for both the bottom and top radii for cylinders!
    cone->setVisible(wasVisible);
...
void ccPropertiesTreeDelegate::applyImageViewport() {
  ...
  if (image->getAssociatedSensor() &&
      image->getAssociatedSensor()->applyImageViewport(image)) {
...
void ccPropertiesTreeDelegate::updateLabelViewport() {
  ...
  ccLog::Print(
      QString("Viewport '%1' has been updated").arg(viewport->getName()));
}

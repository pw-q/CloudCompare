...
static bool CanDetachCloud(const ccHObject *obj) {
  ...
  bool blocked =
      ((parent->isKindOf(CC_TYPES::MESH) &&
        (ccHObjectCaster::ToGenericMesh(parent)->getAssociatedCloud() ==
         obj)) ||
       (parent->isKindOf(CC_TYPES::POLY_LINE) &&
        (dynamic_cast<ccPointCloud *>(
             ccHObjectCaster::ToPolyline(parent)->getAssociatedCloud()) ==
         obj)));
...
ccDBRoot::ccDBRoot(ccCustomQTreeView *dbTreeWidget,
                   QTreeView *propertiesTreeWidget, QObject *parent)
  ...
  m_selectByTypeAndName =
      new QAction("Select children by type and/or name", this);
  ...
  m_ccPropDelegate =
      new ccPropertiesTreeDelegate(m_propertiesModel, m_propertiesTreeWidget);
  ...
  connect(m_ccPropDelegate,
          &ccPropertiesTreeDelegate::ccObjectAndChildrenAppearanceChanged, this,
          &ccDBRoot::redrawCCObjectAndChildren);
}
...
void ccDBRoot::addElement(ccHObject *object, bool autoExpand /*=true*/) {
  ...
  if (!parentObject) {
    ...
    // DGM TODO: how could we check that the object is not already inserted in
    // the DB tree? The double insertion can cause serious damage to it (not sure
    // why excatly though).

    // The code below doesn't work because the 'index' method will always return
    // a valid index as soon as the object has a parent (index creation is a
    // purely 'logical' approach) QModelIndex nodeIndex = index(object); if
    // (nodeIndex.isValid()) 	return;
  }
...
void ccDBRoot::removeElements(ccHObject::Container &objects) {
  ...
  for (ccHObject *object : objects) {
    ...
    if (!parent) {
      ccLog::Warning(QString("[ccDBRoot::removeElements] Internal error: "
...
void ccDBRoot::removeElement(ccHObject *object) {
  ...
  if (!parent) {
    ccLog::Warning(
        "[ccDBRoot::removeElement] Internal error: object has no parent");
    return;
  }
...
void ccDBRoot::deleteSelectedEntities() {
  ...
  for (unsigned i = 0; i < selCount; ++i) {
    ...
    if (!obj->getParent() || obj->isLocked()) {
      ccLog::Warning(QString("Object '%1' can't be deleted this way (locked)")
                         .arg(obj->getName()));
      continue;
    }
    ...
    for (unsigned j = 0; j < selCount; ++j) {
      if (i != j) {
        ccHObject *otherObj =
            static_cast<ccHObject *>(selectedIndexes[j].internalPointer());
  ...
  while (!toBeDeleted.empty()) {
    ...
    if (object->isKindOf(CC_TYPES::MESH)) {
      ...
      if (object->getParent() &&
          object->getParent() ==
              ccHObjectCaster::ToGenericMesh(object)->getAssociatedCloud()) {
...
QVariant ccDBRoot::data(const QModelIndex &idx, int role) const {
  ...
  case Qt::CheckStateRole: {
    // Don't include checkboxes for hierarchy objects if they have no children
    // or only contain hierarchy objects (recursively)
    if (item->getClassID() == CC_TYPES::HIERARCHY_OBJECT) {
      ...
      unsigned int count = item->filterChildren(
          drawableObjects, true, CC_TYPES::HIERARCHY_OBJECT, true);
...
bool ccDBRoot::setData(const QModelIndex &idx, const QVariant &value,
                       int role) {
  if (idx.isValid()) {
    if (role == Qt::EditRole) {
...
QModelIndex ccDBRoot::index(int row, int column,
...
QModelIndex ccDBRoot::index(ccHObject *object) {
  ...
  ccHObject *parent = object->getParent();
  if (!parent) {
    // DGM: actually, it can happen (for instance if the entity is displayed in
    // the local DB of a 3D view) ccLog::Error(QString("An error occurred while
    // creating DB tree index: object '%1' has no
...
int ccDBRoot::columnCount(const QModelIndex &parent) const { return 1; }
...
void ccDBRoot::changeSelection(const QItemSelection &selected,
  ...
  QModelIndexList deselectedItems = deselected.indexes();
  {
    for (int i = 0; i < deselectedItems.count(); ++i) {
      ccHObject *element =
          static_cast<ccHObject *>(deselectedItems.at(i).internalPointer());
      assert(element);
      if (element) {
  ...
  QModelIndexList selectedItems = selected.indexes();
  {
    for (int i = 0; i < selectedItems.count(); ++i) {
      ccHObject *element =
          static_cast<ccHObject *>(selectedItems.at(i).internalPointer());
      assert(element);
      if (element) {
          ...
          if (!selectedIndexes.empty()) {
            ...
            if (obj->isA(CC_TYPES::LABEL_2D) !=
                static_cast<ccHObject *>(selectedIndexes[0].internalPointer())
                    ->isA(CC_TYPES::LABEL_2D)) {
              ccLog::Warning(
                  "[Selection] Labels and other entities can't be mixed "
                  "(release the CTRL key to start a new selection)");
              return;
            }
          }
        }
...
void ccDBRoot::selectEntities(const ccHObject::Container &entities,
  ...
  bool keepLabels = false;
  {
    ...
    if (formerSelectedIndexes.isEmpty() || !incremental)
      keepLabels =
          (labelCount ==
           entities.size()); // yes if they are the only selected entities
    else if (incremental)
      keepLabels =
          static_cast<ccHObject *>(formerSelectedIndexes[0].internalPointer())
              ->isA(CC_TYPES::LABEL_2D); // yes if previously selected entities
                                         // were already labels
  }
  ...
  for (auto entity : entities) {
    ...
    // filter input selection (can't keep both labels and standard entities -->
    // we can't mix them!)
    ...
    if (isLabel == keepLabels && (!incremental || !entity->isSelected())) {
      ...
      if (selectedIndex.isValid()) {
        newSelection.merge(QItemSelection(selectedIndex, selectedIndex),
...
Qt::ItemFlags ccDBRoot::flags(const QModelIndex &idx) const {
  ...
  if (item && !item->isLocked()) // locked items cannot be drag-dropped
  {
    if (item->isA(CC_TYPES::HIERARCHY_OBJECT) ||
        ...
        (item->isKindOf(CC_TYPES::MESH) &&
         !item->isA(
             CC_TYPES::SUB_MESH)) || // a sub-mesh can't leave its parent mesh
...
QMap<int, QVariant> ccDBRoot::itemData(const QModelIndex &idx) const {
  ...
  if (idx.isValid()) {
    const ccHObject *object =
        static_cast<const ccHObject *>(idx.internalPointer());
    if (object)
...
bool ccDBRoot::dropMimeData(const QMimeData *data, Qt::DropAction action,
  ...
  while (!stream.atEnd()) {
    ...
    // ccLog::Print(QString("[Drag & Drop] Parent: %1").arg(oldParent ?
    ...
    if (oldParent) {
      if (item->isKindOf(CC_TYPES::POINT_CLOUD)) {
        ...
        if (oldParent->isKindOf(CC_TYPES::MESH) &&
            ccHObjectCaster::ToGenericMesh(oldParent)->getAssociatedCloud() ==
                item) {
        ...
        else if (oldParent->isKindOf(CC_TYPES::POINT_CLOUD) &&
                 ccHObjectCaster::ToGenericMesh(item)->getAssociatedCloud() ==
                     oldParent) {
          if (oldParent != newParent) {
            ccLog::Error(
                "Meshes can't leave their associated cloud (vertices set)");
            return false;
          }
        }
      } else if (/*item->isKindOf(CC_TYPES::PRIMITIVE) || */ item->isKindOf(
          CC_TYPES::IMAGE)) {
        if (oldParent != newParent) {
        ...
        // for (ccHObject::Container::const_iterator it = labels.begin(); it !=
        // labels.end(); ++it)
        //{
        //	if ((*it)->isA(CC_TYPES::LABEL_2D)) //Warning: cc2DViewportLabel
        //is also a kind of 'CC_TYPES::LABEL_2D'!
        //	{
        ...
        //			if (item->isAncestorOf(label->getPoint(j).cloud)
        ////label's cloud is inside sub-tree
        //				|| newParent == label->getPoint(j).cloud
        ////destination is label's cloud
        //				||
        //label->getPoint(j).cloud->isAncestorOf(newParent)) //destination is
        //below label's cloud
        //			{
        //				canMove = true;
        //				break;
        ...
        //		if (!canMove)
        //		{
        //			ccLog::Error("Labels (or group of) can't leave their
        //parent"); 			return false;
        //		}
        //	}
        //}
    ...
    int itemDependencyFlags =
        item->getDependencyFlagsWith(oldParent); // works even with nullptr
    ...
    assert(newParent);
    newParent->addChild(
        item, fatherDependencyFlags & ccHObject::DP_PARENT_OF_OTHER, destRow);
    item->addDependency(newParent,
                        itemDependencyFlags & ccHObject::DP_PARENT_OF_OTHER);
    // restore other flags on old parent (as all flags have been removed when
    // calling removeElement!)
    if (oldParent) {
      oldParent->addDependency(item, fatherDependencyFlags &
                                         (~ccHObject::DP_PARENT_OF_OTHER));
      item->addDependency(oldParent, itemDependencyFlags &
                                         (~ccHObject::DP_PARENT_OF_OTHER));
    }
...
void ccDBRoot::alignCameraWithEntity(bool reverse) {
  ...
  if (!display) {
    ccLog::Warning(
        "[alignCameraWithEntity] Selected entity has no associated display");
    return;
  }
  ...
  if (obj->isA(CC_TYPES::LABEL_2D)) // 2D label with 3 points?
  {
    ...
    if (label->size() == 3) {
      ...
      if (pp._mesh) {
        ...
        ccLog::Error(
            "Works only with 3-points labels or labels picked on a triangle");
        return;
      }
    } else {
      ccLog::Error(
  ...
  } else if (obj->isA(CC_TYPES::FACET)) // facet
  {
    ...
    CCCoreLib::CCMiscTools::ComputeBaseVectors(planeNormal, planeHorizDir,
                                               planeVertDir);
  ...
  // output the transformation matrix that would make this normal points towards
  // +Z
  {
    ...
    ccLog::Print("[Orientation] You can copy this matrix values (CTRL+C) and "
...
void ccDBRoot::gatherRecursiveInformation() {
  ...
  while (!toProcess.empty()) {
    ...
    if (ent->isA(CC_TYPES::POINT_CLOUD)) {
      ...
      info.materialCount +=
          (mesh->getMaterialSet()
               ? static_cast<unsigned>(mesh->getMaterialSet()->size())
               : 0);
  ...
  // output information
  {
    ...
    // display info box
    QMessageBox::information(MainWindow::TheInstance(), "Information",
                             infoStr.join("\n"));
  }
}
        ...
        for (unsigned j = k + 1; j < childCount; ++j) {
          ...
          case SORT_BY_TYPE:
            if (firstChild->getClassID() == item->getChild(j)->getClassID())
              ...
              swap =
                  (firstChild->getClassID() > item->getChild(j)->getClassID());
            break;
          }
...
void ccDBRoot::selectByTypeAndName() {
  ...
  if (scDlg.getTypeIsUsed()) // we are using type checking
  {
    ...
    switch (type) {
    case CC_TYPES::HIERARCHY_OBJECT: // returned if no type is selected (i.e.
...
void ccDBRoot::editLabelScalarValue() {
  ...
  bool ok = false;
  double newValue =
      QInputDialog::getDouble(MainWindow::TheInstance(), "Edit scalar value",
                              QString("%1 (%2) =")
                                  .arg(QString::fromStdString(sf->getName()))
                                  .arg(P.index),
                              s, -2147483647, 2147483647, 6, &ok);
  if (!ok) {
            ...
            if (item->isA(CC_TYPES::LABEL_2D)) {
              ...
              if (label->size() == 1) {
                ...
                canEditLabelScalarValue =
                    (pp._cloud && pp._cloud->hasScalarFields() &&
                     pp._cloud->isA(CC_TYPES::POINT_CLOUD) &&
                     static_cast<ccPointCloud *>(pp._cloud)
                             ->getCurrentDisplayedScalarField() != nullptr);
              }
            } else if (item->isA(CC_TYPES::PLANE) ||
                       item->isA(CC_TYPES::FACET)) {
              ++planarEntityCount;
...
ccCustomQTreeView::selectionCommand(const QModelIndex &idx,
  ...
  if (idx.isValid()) {
    ...
    if (!selectedIndexes.empty() && !selectionModel()->isSelected(idx)) {
      ...
      if (selectedItem &&
          selectedItem->isA(CC_TYPES::LABEL_2D) !=
              static_cast<ccHObject *>(selectedIndexes[0].internalPointer())
                  ->isA(CC_TYPES::LABEL_2D))

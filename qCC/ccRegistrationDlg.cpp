...
ccRegistrationDlg::ccRegistrationDlg(ccHObject *data, ccHObject *model,
...
CCCoreLib::ICPRegistrationTools::NORMALS_MATCHING
ccRegistrationDlg::normalsMatchingOption() const {
  if (normalsComboBox->isEnabled()) {
    return static_cast<CCCoreLib::ICPRegistrationTools::NORMALS_MATCHING>(
...
ccRegistrationDlg::ConvergenceMethod
ccRegistrationDlg::getConvergenceMethod() const {
...
void ccRegistrationDlg::updateGUI() {
  ...
  checkBoxUseModelSFAsWeights->setEnabled(
      modelEntity->isKindOf(CC_TYPES::POINT_CLOUD) &&
      modelEntity->hasDisplayedScalarField()); // only supported for clouds

  bool hasRefMesh = modelEntity->isKindOf(CC_TYPES::MESH);
  useC2MSignedDistancesCheckBox->setEnabled(
      hasRefMesh); // only supported if a mesh is the reference cloud
  robustC2MDistsCheckBox->setEnabled(hasRefMesh);
  normalsComboBox->setEnabled(
      dataEntity->hasNormals() &&
      modelEntity->hasNormals()); // only supported if both the to-be-aligned
                                  // and the reference entities have normals

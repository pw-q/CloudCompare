...
ccUnrollDlg::ccUnrollDlg(ccHObject *dbRootEntity, QWidget *parent /*=nullptr*/)
    : QDialog(parent), m_ui(new Ui::UnrollDialog),
      m_dbRootEntity(dbRootEntity) {
  m_ui->setupUi(this);

  connect(m_ui->checkBoxAuto, &QCheckBox::stateChanged, this,
          &ccUnrollDlg::axisAutoStateChanged);
  connect(m_ui->comboBoxUnrollShapeType,
          qOverload<int>(&QComboBox::currentIndexChanged), this,
          &ccUnrollDlg::shapeTypeChanged);
  connect(m_ui->comboBoxProjectionType,
          qOverload<int>(&QComboBox::currentIndexChanged), this,
          &ccUnrollDlg::projectionTypeChanged);
  connect(m_ui->comboBoxAxisDimension,
          qOverload<int>(&QComboBox::currentIndexChanged), this,
          &ccUnrollDlg::axisDimensionChanged);
  ...
  connect(m_ui->fromEntityToolButton, &QToolButton::clicked, this,
          &ccUnrollDlg::loadParametersFromEntity);
  connect(m_ui->pasteAxisToolButton, &QToolButton::clicked, this,
          &ccUnrollDlg::axisFromClipboard);
  connect(m_ui->pasteCenterToolButton, &QToolButton::clicked, this,
          &ccUnrollDlg::centerFromClipboard);
...
ccUnrollDlg::~ccUnrollDlg() { delete m_ui; }
...
CCVector3d ccUnrollDlg::getAxis() const {
  ...
  case 3:
  default:
    return CCVector3d(m_ui->axisXDoubleSpinBox->value(),
                      m_ui->axisYDoubleSpinBox->value(),
                      m_ui->axisZDoubleSpinBox->value());
  }
...
CCVector3 ccUnrollDlg::getAxisPosition() const {
  return CCVector3(
      static_cast<PointCoordinateType>(m_ui->axisCenterXDoubleSpinBox->value()),
      static_cast<PointCoordinateType>(m_ui->axisCenterYDoubleSpinBox->value()),
      static_cast<PointCoordinateType>(
          m_ui->axisCenterZDoubleSpinBox->value()));
}
...
void ccUnrollDlg::shapeTypeChanged(int index) {
  ...
  case 1: // cone
  {
    ...
    // m_ui->radiusFrame->setVisible(false); // will depend on the projection
    // type
...
void ccUnrollDlg::toPersistentSettings() const {
  ...
  {
    settings.setValue("shapeType",
                      m_ui->comboBoxUnrollShapeType->currentIndex());
    settings.setValue("projectionType",
                      m_ui->comboBoxProjectionType->currentIndex());
    settings.setValue("spanRatio",
                      m_ui->conicalProjSpanRatioDoubleSpinBox->value());
    settings.setValue("axisDimension",
                      m_ui->comboBoxAxisDimension->currentIndex());
    ...
    settings.setValue("exportDeviationSF",
                      m_ui->exportDeviationSFCheckBox->isChecked());
...
void ccUnrollDlg::fromPersistentSettings() {
  ...
  settings.beginGroup("Unroll");
  {
    int shapeType =
        settings
            .value("shapeType", m_ui->comboBoxUnrollShapeType->currentIndex())
            .toInt();
    int projectionType = settings.value("projectionType", -1).toInt();
    int axisDim =
        settings
            .value("axisDimension", m_ui->comboBoxAxisDimension->currentIndex())
            .toInt();
    double angle =
        settings.value("angle", m_ui->halfAngleDoubleSpinBox->value())
            .toDouble();
    double radius =
        settings.value("radius", m_ui->radiusDoubleSpinBox->value()).toDouble();
    bool autoCenter =
        settings.value("autoCenter", m_ui->checkBoxAuto->isChecked()).toBool();
    bool exportDeviationSF =
        settings
            .value("exportDeviationSF",
                   m_ui->exportDeviationSFCheckBox->isChecked())
            .toBool();
    bool spanRatio =
        settings
    ...
    CCVector3d axis;
    axis.x =
        settings.value("axis.x", m_ui->axisXDoubleSpinBox->value()).toDouble();
    axis.y =
        settings.value("axis.y", m_ui->axisYDoubleSpinBox->value()).toDouble();
    axis.z =
        settings.value("axis.z", m_ui->axisZDoubleSpinBox->value()).toDouble();

    CCVector3d axisCenter;
    axisCenter.x =
        settings.value("axisCenter.x", m_ui->axisCenterXDoubleSpinBox->value())
            .toDouble();
    axisCenter.y =
        settings.value("axisCenter.y", m_ui->axisCenterYDoubleSpinBox->value())
            .toDouble();
    axisCenter.z =
        settings.value("axisCenter.z", m_ui->axisCenterZDoubleSpinBox->value())
            .toDouble();
...
void ccUnrollDlg::loadParametersFromEntity() {
  ...
  if (m_ui->comboBoxUnrollShapeType->currentIndex() == 0) // Cylinder
  {
    ...
    int selectedIndex = ccEntitySelectionDialog::SelectEntity(
        cylinders, -1, this, tr("Select a cylinder entity"));
    if (selectedIndex < 0) {
    ...
    const ccCylinder *cylinder =
        static_cast<const ccCylinder *>(cylinders[selectedIndex]);
    CCVector3 axis = cylinder->getTransformation().getColumnAsVec3D(
        2); // Z axis is the cylinder axis
  ...
  } else if (m_ui->comboBoxUnrollShapeType->currentIndex() == 1) // Cone
  {
    ...
    int selectedIndex = ccEntitySelectionDialog::SelectEntity(
        cones, -1, this, tr("Select a cone entity"));
    if (selectedIndex < 0) {
      // process cancelled by the user
      return;
    }
    ...
    CCVector3 axis = cone->getTransformation().getColumnAsVec3D(
        2); // Z axis is the cylinder axis

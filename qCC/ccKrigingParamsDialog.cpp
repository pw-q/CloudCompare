...
ccKrigingParamsDialog::ccKrigingParamsDialog(QWidget *parent /*=nullptr*/)
    : QDialog(parent, Qt::Tool), m_ui(new Ui_KrigingParamsDialog) {
  m_ui->setupUi(this);
}
...
void ccKrigingParamsDialog::setParameters(
    const ccRasterGrid::KrigingParams &krigingParams) {
  // altitudes
  ...
  // common
  m_ui->modelComboBox->setCurrentIndex(
      static_cast<int>(krigingParams.params.model != Kriging::Invalid
                           ? krigingParams.params.model
                           : Kriging::Spherical));
  m_ui->knnSpinBox->setValue(krigingParams.kNN);
}

void ccKrigingParamsDialog::getParameters(
    ccRasterGrid::KrigingParams &krigingParams) {
  krigingParams.autoGuess = !m_ui->krigeParamsGroupBox->isChecked();
  {
    krigingParams.params.model =
        static_cast<Kriging::Model>(m_ui->modelComboBox->currentIndex());

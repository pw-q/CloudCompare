...
ccNormalComputationDlg::getPreferredOrientation() const {
  int index = preferredOrientRadioButton->isChecked()
                  ? preferredOrientationComboBox->currentIndex()
                  : -1;

  return (index >= 0 && index <= ccNormalVectors::MINUS_SENSOR_ORIGIN
...
void ccNormalComputationDlg::autoEstimateRadius() {
  ...
  PointCoordinateType radius =
      ccOctree::GuessBestRadiusAutoComputeOctree(m_cloud, params, this);
  if (radius > 0) {

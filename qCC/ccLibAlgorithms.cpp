...
static QString
GetDensitySFName(CCCoreLib::GeometricalAnalysisTools::Density densityType,
                 bool approx, double densityKernelSize = 0.0) {
  QString sfName;
...
double GetDefaultCloudKernelSize(ccGenericPointCloud *cloud,
                                 unsigned knn /*=12*/) {
  assert(cloud);
  if (cloud && cloud->size() != 0) {
    // we get 1% of the cloud bounding box
    // and we divide by the number of points / 10e6 (so that the kernel for a 20
    // M. points cloud is half the one of a 10 M. cloud)
...
double GetDefaultCloudKernelSize(const ccHObject::Container &entities,
                                 unsigned knn /*=12*/) {
...
bool ComputeGeomCharacteristics(const GeomCharacteristicSet &characteristics,
  ...
  if (characteristics.size() == 1) {
    return ComputeGeomCharacteristic(characteristics.front().charac,
                                     characteristics.front().subOption, radius,
                                     entities, roughnessUpDir, parent);
  }
...
bool ComputeGeomCharacteristic(
    CCCoreLib::GeometricalAnalysisTools::GeomCharacteristic c, int subOption,
    PointCoordinateType radius, ccHObject::Container &entities,
    const CCVector3 *roughnessUpDir /*=nullptr*/, QWidget *parent /*= nullptr*/,
  ...
  case CCCoreLib::GeometricalAnalysisTools::LocalDensity:
    sfName = GetDensitySFName(
        static_cast<CCCoreLib::GeometricalAnalysisTools::Density>(subOption),
        false, radius);
    break;
    ...
    if (entities[i]->isKindOf(CC_TYPES::POINT_CLOUD)) {
      ...
      if (cloud->isA(CC_TYPES::POINT_CLOUD)) {
        ...
        if (sfIdx >= 0)
          ...
          ccConsole::Error(QString("Failed to create scalar field on cloud "
                                   "'%1' (not enough memory?)")
                               .arg(pc->getName()));
          continue;
        }
      ...
      if (!octree) {
        ...
        if (!octree) {
          ccConsole::Error(QString("Couldn't compute octree for cloud '%1'!")
                               .arg(cloud->getName()));
          break;
        }
      }
      ...
      CCCoreLib::GeometricalAnalysisTools::ErrorCode result =
          CCCoreLib::GeometricalAnalysisTools::ComputeCharactersitic(
              c, subOption, cloud, radius, roughnessUpDir, pDlg, octree.data());
      ...
      if (result == CCCoreLib::GeometricalAnalysisTools::NoError) {
        if (pc && sfIdx >= 0) {
          ...
          if (c == CCCoreLib::GeometricalAnalysisTools::Roughness &&
              roughnessUpDir != nullptr) {
            // signed roughness should be displayed with a symmetrical color
            // scale
            ccScalarField *sf =
                dynamic_cast<ccScalarField *>(pc->getCurrentInScalarField());
            if (sf) {
        ...
        ccConsole::Warning(QString("Failed to apply processing to cloud '%1'")
                               .arg(cloud->getName()));
...
bool ApplyCCLibAlgorithm(CC_LIB_ALGORITHM algo, ccHObject::Container &entities,
                         QWidget *parent /*=nullptr*/,
  ...
  for (size_t i = 0; i < selNum; ++i) {
    ...
    case CCLIB_ALGO_SF_GRADIENT:
      ...
      bool lockedVertices;
      cloud =
          ccHObjectCaster::ToGenericPointCloud(entities[i], &lockedVertices);
      if (lockedVertices) {
        ccUtils::DisplayLockedVerticesWarning(entities[i]->getName(),
                                              selNum == 1);
        cloud = nullptr;
      }
      if (cloud) {
        ...
        if (cloud->isA(CC_TYPES::POINT_CLOUD)) {
          ...
          if (outSfIdx < 0) {
            ...
            sfName = QString("%1(%2)").arg(
                ...
                QString::fromStdString(pc->getScalarFieldName(outSfIdx)));
          }
        } else // if (!cloud->hasDisplayedScalarField()) //TODO: displayed but
               // not necessarily set as OUTPUT!
        {
          cloud = nullptr;
        }
      }
    ...
    if (cloud) {
      ...
      switch (algo) {
      case CCLIB_ALGO_SF_GRADIENT:
        result = CCCoreLib::ScalarFieldTools::computeScalarFieldGradient(
            cloud,
            0, // auto --> FIXME: should be properly set by the user!
            euclidean, false, pDlg.data(), octree.data());
        break;
      ...
      if (result == 0) {
        ...
        cloud->prepareDisplayForRefresh();
        ccConsole::Print("[Algorithm] Timing: %3.2f s.",
...
bool ApplyScaleMatchingAlgorithm(ScaleMatchingAlgorithm algo,
                                 ccHObject::Container &entities,
                                 double icpRmsDiff, int icpFinalOverlap,
  ...
  if (!refEntity->isKindOf(CC_TYPES::POINT_CLOUD) &&
    ...
    ccLog::Warning(
        "[Scale Matching] The reference entity must be a cloud or a mesh!");
    return false;
  }
  ...
  for (unsigned i = 0; i < count; ++i) {
    ...
    if (cloud && !lockedVertices) {
      ...
      case BB_VOLUME: {
        ...
        if (box.isValid())
          scales[i] =
              algo == BB_MAX_DIM ? box.getMaxBoxDim() : box.computeVolume();
        else
          ccLog::Warning(
              QString(
                  "[Scale Matching] Entity '%1' has an invalid bounding-box!")
                  .arg(ent->getName()));
      } break;
      ...
      case PCA_MAX_DIM: {
        ...
        if (!Yk.getLSPlane()) {
          ccLog::Warning(
              QString("[Scale Matching] Failed to perform PCA on entity '%1'!")
                  .arg(ent->getName()));
          break;
        }
        // deduce the scale
      ...
      case ICP_SCALE: {
        ...
        CCCoreLib::ICPRegistrationTools::Parameters parameters;
        {
          ...
          parameters.transformationFilters =
              CCCoreLib::RegistrationTools::SKIP_NONE;
          ...
          parameters.normalsMatching =
              CCCoreLib::ICPRegistrationTools::NO_NORMAL;
        }

        if (ccRegistrationTools::ICP(ent, refEntity, transMat, finalScale,
                                     finalError, finalPointCount, parameters,
                                     false, false, parent)) {
          scales[i] = finalScale;
        } else {
          ccLog::Warning(
              QString("[Scale Matching] Failed to register entity '%1'!")
                  .arg(ent->getName()));
        }
      ...
      ccLog::Warning(
          QString("[Scale Matching] Entity '%1' can't be rescaled this way!")
              .arg(ent->getName()));
    }
  ...
  ccLog::Print(QString("[Scale Matching] Reference entity scale: %1")
  ...
  pDlg.setMethodTitle(QObject::tr("Rescaling entities"));
  {
    for (unsigned i = 0; i < count; ++i) {
      ...
      ccLog::Print(QString("[Scale Matching] Entity '%1' scale: %2")

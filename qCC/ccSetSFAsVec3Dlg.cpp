...
ccSetSFsAsVec3Dialog::ccSetSFsAsVec3Dialog(
    const ccPointCloud *cloud, const QString &xLabel, const QString &yLabel,
    const QString &zLabel, bool allowUnchanged, QWidget *parent /*=nullptr*/)
    : QDialog(parent, Qt::Tool), Ui::SetSFsAsVec3Dialog(), m_constFields(0) {
  setupUi(this);
...
int ccSetSFsAsVec3Dialog::toComboBoxIndex(int index) const {
  if (index >= 0) {
    ...
    if (m_constFields == 3) {
      ...
      // we don't have a 'Unchanged' field anymore, let's switch to Zero by
      // default
      return 0;
    }
  } else {
    ccLog::Warning("[ccSetSFsAsVec3Dialog] Invalid SF index " +
...
void ccSetSFsAsVec3Dialog::setSFIndexes(int sf1Index, int sf2Index,
                                        int sf3Index) {
...
void ccSetSFsAsVec3Dialog::getSFIndexes(int &sf1Index, int &sf2Index,
                                        int &sf3Index) const {

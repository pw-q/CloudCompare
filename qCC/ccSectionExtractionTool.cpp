...
ccSectionExtractionTool::ccSectionExtractionTool(QWidget *parent)
    : ccOverlayDialog(parent), m_UI(new Ui::SectionExtractionDlg),
      m_selectedPoly(nullptr), m_state(0), m_editedPoly(nullptr),
      m_editedPolyVertices(nullptr) {
  m_UI->setupUi(this);

  connect(m_UI->undoToolButton, &QAbstractButton::clicked, this,
          &ccSectionExtractionTool::undo);
  connect(m_UI->validToolButton, &QAbstractButton::clicked, this,
          &ccSectionExtractionTool::apply);
  connect(m_UI->cancelToolButton, &QAbstractButton::clicked, this,
          &ccSectionExtractionTool::cancel);
  connect(m_UI->polylineToolButton, &QAbstractButton::toggled, this,
          &ccSectionExtractionTool::enableSectionEditingMode);
  connect(m_UI->importFromDBToolButton, &QAbstractButton::clicked, this,
          &ccSectionExtractionTool::doImportPolylinesFromDB);
  connect(m_UI->vertAxisComboBox,
          qOverload<int>(&QComboBox::currentIndexChanged), this,
          &ccSectionExtractionTool::setVertDimension);

  connect(m_UI->generateOrthoSectionsToolButton, &QAbstractButton::clicked,
          this, &ccSectionExtractionTool::generateOrthoSections);
  connect(m_UI->extractPointsToolButton, &QAbstractButton::clicked, this,
          &ccSectionExtractionTool::extractPoints);
  connect(m_UI->unfoldToolButton, &QAbstractButton::clicked, this,
          &ccSectionExtractionTool::unfoldPoints);
  connect(m_UI->exportSectionsToolButton, &QAbstractButton::clicked, this,
          &ccSectionExtractionTool::exportSections);
  ...
  addOverriddenShortcut(
      Qt::Key_Delete); // delete key to delete the selected polyline

  connect(this, &ccOverlayDialog::shortcutTriggered, this,
          &ccSectionExtractionTool::onShortcutTriggered);
}
...
bool ccSectionExtractionTool::linkWith(ccGLWindowInterface *win) {
  ...
  if (oldWin) {
    ...
    if (MainWindow::TheInstance()) {
      QMdiSubWindow *subWindow =
          MainWindow::TheInstance()->getMDISubWindow(oldWin);
      if (subWindow) {
        subWindow->close();
      }
    }
  ...
  if (m_associatedWin) {
    connect(m_associatedWin->signalEmitter(),
            &ccGLWindowSignalEmitter::leftButtonClicked, this,
            &ccSectionExtractionTool::addPointToPolyline);
    connect(m_associatedWin->signalEmitter(),
            &ccGLWindowSignalEmitter::rightButtonClicked, this,
            &ccSectionExtractionTool::closePolyLine);
    connect(m_associatedWin->signalEmitter(),
            &ccGLWindowSignalEmitter::mouseMoved, this,
            &ccSectionExtractionTool::updatePolyLine);
    connect(m_associatedWin->signalEmitter(),
            &ccGLWindowSignalEmitter::entitySelectionChanged, this,
            &ccSectionExtractionTool::entitySelected);
...
void ccSectionExtractionTool::selectPolyline(
    Section *poly, bool autoRefreshDisplay /*=true*/) {
  bool redraw = false;
  ...
  if (m_selectedPoly) {
    ...
    m_selectedPoly->entity->setSelected(
        false); // as the window selects it by default (with bounding-box, etc.)
...
void ccSectionExtractionTool::undo() {
  ...
  } while (static_cast<int>(count) >= m_sections.size() &&
           !m_undoCount.empty());

  // ask for a confirmation
  if (QMessageBox::question(
          MainWindow::TheInstance(), "Undo",
          QString("Remove %1 polylines?").arg(m_sections.size() - count),
...
bool ccSectionExtractionTool::reset(bool askForConfirmation /*=true*/) {
  ...
  if (askForConfirmation) {
    // if we found at least one temporary polyline, we display a confirmation
    // message
    for (auto &section : m_sections) {
      if (section.entity && !section.isInDB) {
        if (QMessageBox::question(
                MainWindow::TheInstance(), "Reset",
                "You'll lose all manually defined polylines: are you sure?",
                QMessageBox::Yes, QMessageBox::No) == QMessageBox::No)
          return false;
        else
          break;
...
bool ccSectionExtractionTool::addPolyline(ccPolyline *inputPoly,
...
bool ccSectionExtractionTool::addCloud(ccGenericPointCloud *inputCloud,
                                       bool alreadyInDB /*=true*/) {
  assert(inputCloud);
  ...
  for (CloudPool::iterator it = m_clouds.begin(); it != m_clouds.end(); ++it) {
    ...
    if (!s_mixedShiftAndScaleInfo && it == m_clouds.begin()) {
      if (cloud.entity->getGlobalScale() != inputCloud->getGlobalScale() ||
          CCCoreLib::LessThanEpsilon(
              (cloud.entity->getGlobalShift() - inputCloud->getGlobalShift())
                  .norm())) {
        ccLog::Warning("[ccSectionExtractionTool] Clouds have different shift "
                       "& scale information! Only the first one will be used");
...
void ccSectionExtractionTool::updatePolyLine(int x, int y,
  ...
  CCVector3 *lastP = const_cast<CCVector3 *>(
      m_editedPolyVertices->getPointPersistentPtr(vertCount - 1));
  *lastP = P;
...
void ccSectionExtractionTool::addPointToPolyline(int x, int y) {
  ...
  if (!m_editedPoly) {
    ...
    m_editedPoly->addChild(
        m_editedPolyVertices); // make sure the polyline is a parent of the
                               // polyline before copying the Global Shift &
                               // Scale info!
  ...
  } else // next points
  {
    ...
    CCVector3 *lastQ = const_cast<CCVector3 *>(
        m_editedPolyVertices->getPointPersistentPtr(vertCount - 2));
    ...
    *lastP = P;
    ...
    PointCoordinateType defaultArrowSize =
        std::min(s_defaultArrowSize, tipLength / 2);
...
void ccSectionExtractionTool::closePolyLine(int, int) {
  ...
  if (vertCount < 3) {
    ...
    if (!m_clouds.isEmpty()) {
      m_editedPoly->setDisplay_recursive(
          m_clouds.front()
              .originalDisplay); // set the same 'default' display as the cloud
    }
...
void ccSectionExtractionTool::cancelCurrentPolyline() {
  if ((m_state & STARTED) == 0 || !m_editedPoly) {
    return;
  }
...
void ccSectionExtractionTool::enableSectionEditingMode(bool state) {
  ...
  if (!state /*=activate pause mode*/) {
    ...
    m_associatedWin->displayNewMessage(
        QString(), ccGLWindowInterface::UPPER_CENTER_MESSAGE, false, 0,
        ccGLWindowInterface::MANUAL_SEGMENTATION_MESSAGE);
    m_associatedWin->setPickingMode(
        ccGLWindowInterface::ENTITY_PICKING); // to be able to select polylines!
  } else {
    ...
    m_associatedWin->setInteractionMode(
        ccGLWindowInterface::INTERACT_SEND_ALL_SIGNALS);
    m_associatedWin->displayNewMessage(
        "Section edition mode", ccGLWindowInterface::UPPER_CENTER_MESSAGE,
        false, 3600, ccGLWindowInterface::MANUAL_SEGMENTATION_MESSAGE);
    m_associatedWin->displayNewMessage(
        "Left click: add section points / Right click: stop",
...
void ccSectionExtractionTool::addUndoStep() {
  if (m_undoCount.empty() ||
      (static_cast<int>(m_undoCount.back()) < m_sections.size())) {
...
void ccSectionExtractionTool::generateOrthoSections() {
  ...
  // show arrow
  {
    ...
    PointCoordinateType tipLength = (*lastQ - *lastP).norm();
    PointCoordinateType defaultArrowSize =
        m_associatedWin->computeActualPixelSize() * s_defaultArrowSize;
    ...
    if (s_autoSaveAndRemoveGeneratrix) {
      ...
      if (!m_selectedPoly->isInDB) {
        ccHObject *destEntity =
            getExportGroup(s_polyExportGroupID, "Exported sections");
        if (!destEntity) {
          assert(false);
          return;
        }
        ...
        MainWindow::TheInstance()->addToDB(m_selectedPoly->entity, false,
                                           false);
      }
      // and remove
      deleteSelectedPolyline();
    ...
    for (unsigned i = 0; i < maxCount; ++i) {
      ...
      while (s < l + lAB) {
        ...
        if (vertices->reserve(2) && orthoPoly->reserve(2)) {
          ...
          CCVector3 I1 = I + nAB * static_cast<PointCoordinateType>(
                                       s_orthoSectionWidth / 2);
          CCVector3 I2 = I - nAB * static_cast<PointCoordinateType>(
                                       s_orthoSectionWidth / 2);
          ...
          if (!m_clouds.isEmpty())
            orthoPoly->setDisplay_recursive(
                m_clouds.front().originalDisplay); // set the same 'default'
                                                   // display as the cloud
          orthoPoly->setName(
              QString("%1.%2").arg(poly->getName()).arg(++polyIndex));

          // add meta data (for Mascaret export)
          {
            orthoPoly->setMetaData(ccPolyline::MetaKeyUpDir(),
                                   QVariant(vertDim));
...
ccHObject *ccSectionExtractionTool::getExportGroup(unsigned &defaultGroupID,
  ...
  ccHObject *destEntity =
      (defaultGroupID != 0 ? root->find(defaultGroupID) : nullptr);
  if (!destEntity) {
...
void ccSectionExtractionTool::exportSections() {
  ...
  if (!exportCount) {
    // nothing to do
    ccLog::Warning(
        "[ccSectionExtractionTool] All active sections are already in DB");
    return;
  }
  ...
  ccLog::Print(QString("[ccSectionExtractionTool] %1 sections exported")
                   .arg(exportCount));

  // m_associatedWin->redraw();
}

bool ccSectionExtractionTool::extractSectionEnvelope(
    const ccPolyline *originalSection, const ccPointCloud *originalSectionCloud,
    ccPointCloud *unrolledSectionCloud, unsigned sectionIndex,
    ccEnvelopeExtractor::EnvelopeType envelopeType,
    PointCoordinateType maxEdgeLength, bool multiPass, bool splitEnvelope,
    bool &envelopeGenerated, bool visualDebugMode /*=false*/) {
  envelopeGenerated = false;

  if (!originalSectionCloud || !unrolledSectionCloud) {
    ccLog::Warning("[ccSectionExtractionTool][extract envelope] Internal "
  ...
  if (originalSectionCloud->size() < 2) {
    // nothing to do
    ccLog::Warning(
        QString("[ccSectionExtractionTool][extract envelope] Section #%1 "
                "contains less than 2 points and will be ignored")
            .arg(sectionIndex));
    return true;
  }

  // by default, the points in 'unrolledSectionCloud' are 2D (X = curvilinear
  // coordinate, Y = height, Z = 0)
  CCVector3 N(0, 0, 1);
  CCVector3 Y(0, 1, 0);

  std::vector<unsigned> vertIndexes;
  ccPolyline *envelope = ccEnvelopeExtractor::ExtractFlatEnvelope(
      unrolledSectionCloud, multiPass, maxEdgeLength, N.u, Y.u, envelopeType,
      &vertIndexes, visualDebugMode);
  ...
  if (envelope) {
    // update vertices (to replace 'unrolled' points by 'original' ones
    {
      ...
      if (vertIndexes.size() == static_cast<size_t>(vertices->size())) {
        for (unsigned i = 0; i < vertices->size(); ++i) {
          ...
          *const_cast<CCVector3 *>(P) =
              *originalSectionCloud->getPoint(vertIndexes[i]);
        }
        ...
        ccLog::Warning("[ccSectionExtractionTool][extract envelope] Internal "
                       "error (couldn't fetch original points indexes?!)");
        delete envelope;
        return false;
      }
    }
    ...
    if (splitEnvelope) {
      ...
      // compute some stats on the envelope
      {
        ...
        ccLog::PrintDebug(QString("Envelope: min = %1 / avg = %2 / max = %3")
                              .arg(minLength)
                              .arg(sumLength / count)
                              .arg(maxLength));
      }
    ...
    ccHObject *destEntity =
        getExportGroup(s_profileExportGroupID, "Extracted profiles");
    if (!destEntity) {
      assert(false);
      return false;
    }
    ...
    for (size_t p = 0; p < parts.size(); ++p) {
      ...
      // copy meta-data (import for Mascaret export!)
      {
        ...
        for (QVariantMap::const_iterator it = metaData.begin();
             it != metaData.end(); ++it) {
...
bool ccSectionExtractionTool::extractSectionCloud(
    const std::vector<CCCoreLib::ReferenceCloud *> &refClouds,
    unsigned sectionIndex, bool &cloudGenerated) {
  ...
  for (int i = 0; i < static_cast<int>(refClouds.size()); ++i) {
    if (!refClouds[i])
      continue;
    assert(m_clouds[i]
               .entity); // a valid ref. cloud must have a valid counterpart!
    ...
    if (part) {
      if (i == 0) {
        ...
        if (sectionCloud->size() != cloudSizeBefore + partSize) {
          // not enough memory
          ccLog::Warning(
              "[ccSectionExtractionTool][extract cloud] Not enough memory");
          delete sectionCloud;
          return false;
        }
      }
  ...
  if (sectionCloud) {
    // create output group if necessary
    ccHObject *destEntity =
        getExportGroup(s_cloudExportGroupID, "Extracted section clouds");
    if (!destEntity) {
      assert(false);
      return false;
    }
...
struct Segment {
  Segment() : A(0, 0), B(0, 0), u(0, 0), d(0), curvPos(0) {}
...
void ccSectionExtractionTool::unfoldPoints() {
  if (!m_selectedPoly) {
    if (QMessageBox::No ==
        QMessageBox::question(
            this, "No polyline selected",
            "Do you want to unfold all the polylines at once?")) {
      return;
    }
  ...
  double thickness = QInputDialog::getDouble(
      MainWindow::TheInstance(), "Thickness",
      "Distance to polyline:", s_defaultThickness, 1.0e-6, 1.0e6, 6, &ok);
  if (!ok)
    return;
  ...
  CCCoreLib::NormalizedProgress nprogress(
      &pdlg, polylines.size() > 1 ? static_cast<unsigned>(polylines.size())
                                  : totalPointCount);
  ...
  for (ccPolyline *poly : polylines) {
    ...
    unsigned polySegmentCount =
        poly->isClosed() ? polyVertCount : polyVertCount - 1;
    {
      try {
    ...
    for (auto &pc : m_clouds) {
      ...
      // now test each point and see if it's close to the current polyline (in
      // 2D)
      for (unsigned i = 0; i < cloud->size(); ++i) {
        ...
        if (closestSegment >= 0) {
          ...
          // we use the curvilinear position of the point in the X dimension
          // (and Y is 0)
          CCVector3 Q;
          {
            CCVector2 AP2D = P2D - s.A;
            ...
            Q.u[yDim] = crossprod < 0
                            ? -d
                            : d; // signed orthogonal distance to the polyline
        ...
        } else // multiple polylines: we don't display the progress, but we
               // still check if the cancel button has been pressed
        {
      ...
      if (unfoldedIndexes.size() != 0) {
        ...
        ccPointCloud *unfoldedCloud = nullptr;
        {
          if (cloud->isA(CC_TYPES::POINT_CLOUD))
            unfoldedCloud = static_cast<ccPointCloud *>(cloud)->partialClone(
                &unfoldedIndexes);
          else
        ...
        for (unsigned i = 0; i < unfoldedCloud->size(); ++i) {
          ...
          *const_cast<CCVector3 *>(unfoldedCloud->getPoint(i)) =
              unfoldedPoints[i] + C;
        }
        ...
        ccLog::Warning(QString("[Unfold] No point of the cloud '%1' were "
                               "unfolded (check parameters)")
                           .arg(cloud->getName()));
      }
...
void ccSectionExtractionTool::extractPoints() {
  ...
  static ccEnvelopeExtractor::EnvelopeType s_extractSectionsType =
      ccEnvelopeExtractor::LOWER;
  ...
  sesDlg.doExtractEnvelopes(s_extractSectionsAsEnvelopes,
  ...
  // progress dialog
  ccProgressDialog pdlg(true);
  CCCoreLib::NormalizedProgress nprogress(&pdlg,
      ...
      if (poly) {
        ...
        for (int c = 0; c < cloudCount; ++c) {
          ...
          if (cloud) {
            ...
            // compute the distance of each point to the current polyline
            // segment
            for (unsigned i = 0; i < cloud->size(); ++i) {
              const CCVector3 *P = cloud->getPoint(i);
              CCVector2 P2D(P->u[xDim], P->u[yDim]);
              ...
              if (minSquareDist >= 0 && minSquareDist < sectionThicknessSq) {
                // if we extract the section as cloud(s), we add the point to
                // the (current) ref. cloud
                if (s_extractSectionsAsClouds) {
                  assert(refCloud);
                  ...
                  if (refCloudSize == refCloud->capacity()) {
                    ...
                    if (!refCloud->reserve(refCloudSize)) {
                      // not enough memory
                      ccLog::Warning(
                          "[ccSectionExtractionTool] Not enough memory");
                      error = true;
                      break;
                    }
                  }
                ...
                // if we extract the section as envelope(s), we add it to the 2D
                // points set
                if (s_extractSectionsAsEnvelopes) {
                  assert(originalSlicePoints && unrolledSlicePoints);
                  assert(originalSlicePoints->size() ==
                         unrolledSlicePoints->size());
                  ...
                  unrolledSlicePoints->addPoint(
                      CCVector3(seg2D.s + curvilinearPos, P->u[vertDim], 0));
                }
              }
        ...
        if (!error) {
          ...
          if (/*!error && */ s_extractSectionsAsEnvelopes) {
            ...
            error = !extractSectionEnvelope(
                poly, originalSlicePoints, unrolledSlicePoints, s + 1,
                s_extractSectionsType, s_envelopeMaxEdgeLength, s_multiPass,
                s_splitEnvelope, envelopeGenerated, visualDebugMode);
  ...
  if (error) {
    ...
    ccLog::Print(QString("[ccSectionExtractionTool] Job done (%1 envelope(s) "
                         "and %2 cloud(s) were generated)")
                     .arg(generatedEnvelopes)
                     .arg(generatedClouds));
  }
}

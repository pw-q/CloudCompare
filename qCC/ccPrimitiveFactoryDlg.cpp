...
ccPrimitiveFactoryDlg::ccPrimitiveFactoryDlg(MainWindow *win)
  ...
  connect(createPushButton, &QAbstractButton::clicked, this,
          &ccPrimitiveFactoryDlg::createPrimitive);
  ...
  connect(spherePosFromClipboardButton, &QPushButton::clicked, this,
          &ccPrimitiveFactoryDlg::setSpherePositionFromClipboard);
  connect(spherePosToOriginButton, &QPushButton::clicked, this,
          &ccPrimitiveFactoryDlg::setSpherePositionToOrigin);
  connect(csSetMatrixBasedOnSelectedObjectButton, &QPushButton::clicked, this,
          &ccPrimitiveFactoryDlg::setCoordinateSystemBasedOnSelectedObject);
  connect(csMatrixTextEdit, &QPlainTextEdit::textChanged, this,
          &ccPrimitiveFactoryDlg::onMatrixTextChange);
  connect(csClearMatrixButton, &QPushButton::clicked, this,
          &ccPrimitiveFactoryDlg::setCSMatrixToIdentity);
...
void ccPrimitiveFactoryDlg::createPrimitive() {
  ...
  // Plane
  case 0: {
    primitive = new ccPlane(
        static_cast<PointCoordinateType>(planeWidthDoubleSpinBox->value()),
        static_cast<PointCoordinateType>(planeHeightDoubleSpinBox->value()));
  } break;
  // Box
  case 1: {
    CCVector3 dims(
        static_cast<PointCoordinateType>(boxDxDoubleSpinBox->value()),
  ...
  case 2: {
    ccGLMatrix transMat;
    transMat.setTranslation(CCVector3f(spherePosXDoubleSpinBox->value(),
                                       spherePosYDoubleSpinBox->value(),
                                       spherePosZDoubleSpinBox->value()));
    primitive = new ccSphere(
        static_cast<PointCoordinateType>(sphereRadiusDoubleSpinBox->value()),
        &transMat);
  } break;
  // Cylinder
  case 3: {
    primitive = new ccCylinder(
        static_cast<PointCoordinateType>(cylRadiusDoubleSpinBox->value()),
  ...
  case 4: {
    primitive = new ccCone(
        ...
        static_cast<PointCoordinateType>(snoutGroupBox->isChecked()
  ...
  case 5: {
    primitive = new ccTorus(
        ...
        static_cast<PointCoordinateType>(
            CCCoreLib::DegreesToRadians(torusAngleDoubleSpinBox->value())),
        torusRectGroupBox->isChecked(),
        static_cast<PointCoordinateType>(
            torusRectGroupBox->isChecked()
                ? torusRectSectionHeightDoubleSpinBox->value()
                : 0));
  } break;
  // Dish
  case 6: {
    primitive = new ccDish(
        static_cast<PointCoordinateType>(dishRadiusDoubleSpinBox->value()),
        static_cast<PointCoordinateType>(dishHeightDoubleSpinBox->value()),
        static_cast<PointCoordinateType>(dishEllipsoidGroupBox->isChecked()

...
ccAlignDlg::ccAlignDlg(ccGenericPointCloud *data, ccGenericPointCloud *model,
                       QWidget *parent)
  ...
  connect(m_ui->modelSamplingRate,
          ...
          &ccAlignDlg::modelSamplingRateChanged);
  connect(m_ui->dataSamplingRate,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccAlignDlg::dataSamplingRateChanged);
  connect(m_ui->deltaEstimation, &QPushButton::clicked, this,
          &ccAlignDlg::estimateDelta);
  connect(m_ui->samplingMethod, qOverload<int>(&QComboBox::currentIndexChanged),
...
CCCoreLib::ReferenceCloud *ccAlignDlg::getSampledModel() {
  ...
  case SPACE: {
    ...
    sampledCloud = CCCoreLib::CloudSamplingTools::resampleCloudSpatially(
        modelObject,
  ...
  case OCTREE:
    if (modelObject->getOctree()) {
      ...
      ccLog::Error(
          "[ccAlignDlg::getSampledModel] Failed to get/compute model octree!");
    }
    break;
  case RANDOM: {
    sampledCloud = CCCoreLib::CloudSamplingTools::subsampleCloudRandomly(
        modelObject, static_cast<unsigned>(m_ui->modelSamplingRate->value()));
  } break;
  default: {
...
CCCoreLib::ReferenceCloud *ccAlignDlg::getSampledData() {
  ...
  case OCTREE:
    if (dataObject->getOctree()) {
      ...
      ccLog::Error(
          "[ccAlignDlg::getSampledData] Failed to get/compute data octree!");
    }
    break;
  case RANDOM: {
    sampledCloud = CCCoreLib::CloudSamplingTools::subsampleCloudRandomly(
        dataObject, (unsigned)(m_ui->dataSamplingRate->value()));
  } break;
  default: {
...
void ccAlignDlg::modelSamplingRateChanged(double value) {
  ...
  switch (method) {
  case SPACE: {
    CCCoreLib::ReferenceCloud *tmpCloud =
        getSampledModel(); // DGM FIXME: wow! you generate a spatially sampled
                           // cloud just to display its size?!
    if (tmpCloud) {
      message =
          QString("distance units (%1 remaining points)").arg(tmpCloud->size());
      delete tmpCloud;
    }
  } break;
  case RANDOM: {
...
void ccAlignDlg::dataSamplingRateChanged(double value) {
  ...
  case SPACE: {
    CCCoreLib::ReferenceCloud *tmpCloud =
        getSampledData(); // DGM FIXME: wow! you generate a spatially sampled
                          // cloud just to display its size?!
    if (tmpCloud) {
      message =
          QString("distance units (%1 remaining points)").arg(tmpCloud->size());
...
void ccAlignDlg::estimateDelta() {
  ...
  if (CCCoreLib::GeometricalAnalysisTools::ComputeLocalDensityApprox(
          &cloud, CCCoreLib::GeometricalAnalysisTools::DENSITY_KNN, &pDlg) !=

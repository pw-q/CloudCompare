...
// operation names
constexpr char s_opNames[s_opCount][8]{
    "add",  "sub",  "mult", "div",     "min", "max", "sqrt", "pow2",
    "pow3", "exp",  "log",  "log10",   "cos", "sin", "tan",  "acos",
    "asin", "atan", "int",  "inverse", "set", "abs"};
...
ccScalarFieldArithmeticsDlg::ccScalarFieldArithmeticsDlg(
    ccPointCloud *cloud, QWidget *parent /*=nullptr*/)
    : QDialog(parent, Qt::Tool), m_ui(new Ui::SFArithmeticsDlg) {
  assert(cloud);
  ...
  connect(m_ui->operationComboBox,
          qOverload<int>(&QComboBox::currentIndexChanged), this,
          &ccScalarFieldArithmeticsDlg::onOperationIndexChanged);
  connect(m_ui->sf2ComboBox, qOverload<int>(&QComboBox::currentIndexChanged),
...
void ccScalarFieldArithmeticsDlg::onOperationIndexChanged(int index) {
  if (index == Operation::SET) {
    ...
    m_ui->sf2ComboBox->setEnabled(
        index <=
        MAX); // only the 6 first operations are	applied with 2 SFs
...
void ccScalarFieldArithmeticsDlg::onSF2IndexChanged(int index) {
  ...
  m_ui->constantDoubleSpinBox->setEnabled(
      m_ui->sf2ComboBox->currentIndex() + 1 == m_ui->sf2ComboBox->count());
}
...
ccScalarFieldArithmeticsDlg::Operation
ccScalarFieldArithmeticsDlg::GetOperationByName(const QString &name) {
...
QString ccScalarFieldArithmeticsDlg::GetOperationName(
    Operation op, const QString &sf1, const QString &sf2 /*=QString()*/) {
  switch (op) {
  case PLUS:
...
bool ccScalarFieldArithmeticsDlg::apply(ccPointCloud *cloud) {
  ...
  SF2 sf2Desc;
  sf2Desc.isConstantValue =
      m_ui->constantDoubleSpinBox->isEnabled() || (sf1Idx == Operation::SET);
...
bool ccScalarFieldArithmeticsDlg::Apply(ccPointCloud *cloud, Operation op,
                                        int sf1Idx, bool inplace,
                                        SF2 *sf2Desc /*=nullptr*/,
                                        QWidget *parent /*=nullptr*/) {
  assert(cloud);

  if (!cloud || !cloud->hasScalarFields()) {
    ccLog::Warning("[ccScalarFieldArithmeticsDlg::apply] No input cloud, or "
                   "cloud has no SF?!");
    assert(false);
    return false;
  }
  ...
  if (op == INVALID) {
    ccLog::Warning(
        "[ccScalarFieldArithmeticsDlg::apply] Invalid/unhandled operation");
    assert(false);
    return false;
  }
  ...
  double sf1PreviousOffset =
      sf1->getOffset(); // remember the offset, as we may have to change it
  ...
  if (op <= MAX) {
    if (!sf2Desc || (!sf2Desc->isConstantValue &&
      ...
      ccLog::Warning(
          "[ccScalarFieldArithmeticsDlg::apply] Invalid SF2 index/descriptor!");
      assert(false);
      return false;
    }
  ...
  if (!inplace) {
    ...
    if (sfIdx >= 0) {
      if (sfIdx == sf1Idx || (sf2Desc && sfIdx == sf2Desc->sfIndex)) {
        ccLog::Warning(QString("[ccScalarFieldArithmeticsDlg::apply] Resulting "
                               "scalar field would have the same name as one "
                               "of the operand (%1)! Rename it first...")
                           .arg(sfName));
        return false;
      }
    ...
    if (sfIdx < 0) {
      ccLog::Warning("[ccScalarFieldArithmeticsDlg::apply] Failed to create "
  ...
  } else // we keep the original scalar field
  {
    ...
    case PLUS: {
      if (sf2Desc && sf2Desc->isConstantValue) {
        ...
        ccLog::PrintVerbose(
            QString("SF %1 offset has been changed: from %2 to %3")
    ...
    case MULTIPLY: {
      if (sf2Desc && sf2Desc->isConstantValue) {
        sf1->setOffset(
            sf1->getOffset() *
            sf2Desc
                ->constantValue); // it's safe was long as we use
                                  // sf1PreviousOffset to read the values later
      } else {
        sf1->setOffset(
            sf1->getOffset() *
            sf2->getOffset()); // it's safe was long as we use sf1PreviousOffset
                               // to read the values later
      }
      break;
    }
    case DIVIDE: {
      if (sf2Desc && sf2Desc->isConstantValue) {
        if (sf2Desc->constantValue != 0) {
          sf1->setOffset(sf1->getOffset() /
                         sf2Desc->constantValue); // it's safe was long as we
                                                  // use sf1PreviousOffset to
                                                  // read the values later
        }
      } else {
        // not obvious...
      }
    ...
    case SQRT: {
      sf1->setOffset(sqrt(std::max(
          0.0,
          sf1->getOffset()))); // it's safe was long as we use sf1PreviousOffset
                               // to read the values later
      break;
    }
    case POW2: {
    ...
    case ASIN:
    case ATAN: {
      sf1->setOffset(0.0); // it's safe was long as we use sf1PreviousOffset to
                           // read the values later
      break;
    }
    case INT: {
    ...
    if (ccScalarField::ValidValue(val1)) {
      ...
      case DIVIDE: {
        ...
        if (sf2Desc->isConstantValue) {
          ...
          const ScalarType &val2 = sf2->getValue(i);
          if (ccScalarField::ValidValue(val2) &&
              CCCoreLib::GreaterThanEpsilon(std::abs(val2))) {
            val = val1 / val2;
          }
        }
      } break;
      ...
      case INT:
        ...
        break;
      case INVERSE:
        val = CCCoreLib::LessThanEpsilon(std::abs(val1))

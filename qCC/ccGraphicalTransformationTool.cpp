...
ccGraphicalTransformationTool::ccGraphicalTransformationTool(QWidget *parent)
    : ccOverlayDialog(parent), Ui::GraphicalTransformationDlg(),
  ...
  connect(TxCheckBox, &QCheckBox::clicked, this,
          &ccGraphicalTransformationTool::incrementalTranslationToggle);
  connect(TyCheckBox, &QCheckBox::clicked, this,
          &ccGraphicalTransformationTool::incrementalTranslationToggle);
  connect(TzCheckBox, &QCheckBox::clicked, this,
          &ccGraphicalTransformationTool::incrementalTranslationToggle);

  connect(advPushButton, &QPushButton::toggled, this,
          &ccGraphicalTransformationTool::advModeToggle);
  connect(refAxisRadio, &QRadioButton::toggled, this,
          &ccGraphicalTransformationTool::advRefAxisRadioToggled);
  connect(objCenterRadio, &QRadioButton::toggled, this,
          &ccGraphicalTransformationTool::advObjectAxisRadioToggled);
  connect(advTranslateComboBox, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &ccGraphicalTransformationTool::advTranslateRefUpdate);
  connect(advRotateComboBox, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &ccGraphicalTransformationTool::advRotateRefUpdate);
  connect(rotComboBox, qOverload<int>(&QComboBox::activated), this,
          &ccGraphicalTransformationTool::advRotateComboBoxUpdate);
  connect(incrementalForwardButton, &QAbstractButton::clicked, this,
          &ccGraphicalTransformationTool::incrementalTransform);
  connect(incrementalBackwardButton, &QAbstractButton::clicked, this,
          &ccGraphicalTransformationTool::incrementalTransform);
...
void ccGraphicalTransformationTool::pause(bool state) {
  ...
  if (state) {
    m_associatedWin->setInteractionMode(
        ccGLWindowInterface::MODE_TRANSFORM_CAMERA);
    m_associatedWin->displayNewMessage(
        "Transformation [PAUSED]", ccGLWindowInterface::UPPER_CENTER_MESSAGE,
        false, 3600, ccGLWindowInterface::MANUAL_TRANSFORMATION_MESSAGE);
    m_associatedWin->displayNewMessage(
        "Unpause to transform again", ccGLWindowInterface::UPPER_CENTER_MESSAGE,
        true, 3600, ccGLWindowInterface::MANUAL_TRANSFORMATION_MESSAGE);
  } else {
    m_associatedWin->setInteractionMode(
        ccGLWindowInterface::MODE_TRANSFORM_ENTITIES);
  ...
  m_associatedWin->redraw(false,
                          false); // we have to redraw the 3D layer for labels!
}
...
void ccGraphicalTransformationTool::advModeToggle(bool state) {
  ...
  if (state) {
    ...
    this->adjustSize(); // adjust size will minimize the display height with the
                        // dropdowns not visible
    this->setGeometry(this->x() + (wPrev - 250), this->y(), 250,
                      this->height());
  ...
  if (mainWindow) {
    ...
    if (!m_advancedModeObjectList.empty()) {
      for (size_t i = 0; i < m_advancedModeObjectList.size(); ++i) {
        QString item = QString("%1 (ID=%2)")
                           .arg(m_advancedModeObjectList[i]->getName())
                           .arg(m_advancedModeObjectList[i]->getUniqueID());
        advTranslateComboBox->insertItem(
            static_cast<int>(i) + 1, item,
            QVariant(m_advancedModeObjectList[i]->getUniqueID()));
        advRotateComboBox->insertItem(
            static_cast<int>(i) + 1, item,
            QVariant(m_advancedModeObjectList[i]->getUniqueID()));
      }
    }
...
ccGLMatrixd ccGraphicalTransformationTool::arbitraryVectorTranslation(
    const CCVector3 &vec) {
  double theta = 0;
  ...
  // special case
  if (CCCoreLib::LessThanEpsilon(std::abs(vec.x)) &&
      CCCoreLib::LessThanEpsilon(std::abs(vec.y)) && vec.z < 0) {
    arbitraryVectorTranslationAdjust.scaleRotation(-1);
  }
  return arbitraryVectorTranslationAdjust;
}

ccGLMatrixd ccGraphicalTransformationTool::arbitraryVectorRotation(
    double angle, const CCVector3d &arbitraryVector) {
  // advRotationTransform = (cos(theta)*I) + ((1-cos(theta)*u) (X) u) +
  // (sin(theta)*u_skewsym) (X) represents tensor product Reference: Ch 4.7.3 in
  // Geometric Tools for Computer Graphics - P. Schneider & D. Eberly
  ...
  thirdTerm.setColumn(0,
                      CCVector3d(0, arbitraryVector[2], -arbitraryVector[1]));
  thirdTerm.setColumn(1,
                      CCVector3d(-arbitraryVector[2], 0, arbitraryVector[0]));
  thirdTerm.setColumn(2,
                      CCVector3d(arbitraryVector[1], -arbitraryVector[0], 0));
...
bool ccGraphicalTransformationTool::setAdvTranslationTransform(
    ccHObject *translateRef) {
  if (!m_associatedWin) {
    assert(false);
    return false;
  }
...
bool ccGraphicalTransformationTool::setAdvRotationAxis(
    ccHObject *rotateRef, rotComboBoxItems selectedAxis) {
  ...
  if (rotComboBox->findData(selectedAxis) == -1) {
    rotComboBox->setCurrentIndex(rotComboBox->findData(
        rotComboBoxItems::Z)); // Default to Z axis if passed an invalid axis
                               // selection
  } else {
  ...
  if (refAxisRadio->isChecked()) {
    if (m_advRotateRefIsChild) {
      ...
      newCenter = (m_rotation.inverse() * m_advRotationRefObjCenter -
                   m_rotation.inverse() * m_position.getTranslationAsVec3D());
    }
...
bool ccGraphicalTransformationTool::entityInTransformList(ccHObject *entity) {
  for (unsigned j = 0; j < m_toTransform.getChildrenNumber(); j++) {
    if (entity == m_toTransform.getChild(j) ||
        m_toTransform.getChild(j)->isAncestorOf(entity)) {
      return true;
    }
  }
  return false;
...
void ccGraphicalTransformationTool::advTranslateRefUpdate(int index) {
  if (index == 0 ||
      m_advancedModeObjectList.empty()) // index 0 is always the origin
  {
    TxCheckBox->setEnabled(true);
    TyCheckBox->setEnabled(true);
  ...
  for (size_t i = 0; i < m_advancedModeObjectList.size(); i++) {
    if (id == m_advancedModeObjectList[i]->getUniqueID()) {
      MainWindow *mainWindow = MainWindow::TheInstance();
      if (mainWindow) {
        if (m_advTranslateRef != nullptr &&
            m_advTranslateRef != m_advRotateRef) // leave selected if rotate ref
        {
          mainWindow->db()->unselectEntity(m_advTranslateRef);
        }
      ...
      if (!setAdvTranslationTransform(m_advTranslateRef)) {
        ccLog::Error("Error calculating adv translation transform, cannot "
                     "translate along selected item");
        advTranslateComboBox->setCurrentIndex(0);
      }
      return;
    }
  }
  ccLog::Error("Error finding the selected object in DB Tree, cannot translate "
               "along selected object");
  advTranslateComboBox->setCurrentIndex(0);
}

void ccGraphicalTransformationTool::advRotateComboBoxUpdate(int index) {
  rotComboBoxItems selectedAxis =
      static_cast<rotComboBoxItems>(rotComboBox->itemData(index).toInt());
  ...
  if (!setAdvRotationAxis(m_advRotateRef, selectedAxis)) {
    ccLog::Error(
        "Error setting adv rotation axis, cannot rotate around selected item");
    advRotateComboBox->setCurrentIndex(0);
  }
}
  ...
  for (size_t i = 0; i < m_advancedModeObjectList.size(); i++) {
    if (id == m_advancedModeObjectList[i]->getUniqueID()) {
      ...
      if (mainWindow) {
        if (m_advRotateRef != nullptr &&
            m_advTranslateRef !=
                m_advRotateRef) // leave selected if translate ref
        {
          mainWindow->db()->unselectEntity(m_advRotateRef);
        }
      ...
      if (!setAdvRotationAxis(m_advRotateRef,
                              static_cast<rotComboBoxItems>(
                                  rotComboBox->itemData(index).toInt()))) {
        ccLog::Error("Error setting adv rotation axis, cannot rotate around "
                     "selected item");
        advRotateComboBox->setCurrentIndex(0);
      }
      return;
...
void ccGraphicalTransformationTool::incrementalTranslationToggle() {
  const bool isIncrementalActive = TxCheckBox->isChecked() ||
                                   TyCheckBox->isChecked() ||
                                   TzCheckBox->isChecked();
  incrementalTransLabel->setEnabled(isIncrementalActive);
  incrementalTransSpin->setEnabled(isIncrementalActive);
}

void ccGraphicalTransformationTool::incrementalRotationToggle(
    const rotComboBoxItems &selectedRotationItem) {
  if (selectedRotationItem == rotComboBoxItems::NONE ||
      selectedRotationItem == rotComboBoxItems::XYZ) {
...
bool ccGraphicalTransformationTool::addEntity(ccHObject *entity) {
  ...
  if (entity->getDisplay() != m_associatedWin) {
    ccLog::Warning(
        QString("[Graphical Transformation Tool] Can't use entity '%1' cause "
                "it's not displayed in the active 3D view!")
            .arg(entity->getName()));
    return false;
  }
  ...
  if (entity->isLocked()) {
    ccLog::Warning(QString("[Graphical Transformation Tool] Can't transform "
                           "entity '%1' cause it's locked!")
                       .arg(entity->getName()));
    return false;
  }

  // we can't transform child meshes
  if (entity->isA(CC_TYPES::MESH) && entity->getParent() &&
      entity->getParent()->isKindOf(CC_TYPES::MESH)) {
    ccLog::Warning(
        QString("[Graphical Transformation Tool] Entity '%1' can't be modified "
                "as it is part of a mesh group. You should 'clone' it first.")
            .arg(entity->getName()));
    return false;
  }
...
void ccGraphicalTransformationTool::updateDisplayMessage() {
  ...
  if (advPushButton->isChecked()) {
    m_associatedWin->displayNewMessage(
        "[Advanced mode]", ccGLWindowInterface::UPPER_CENTER_MESSAGE, false,
        3600, ccGLWindowInterface::MANUAL_TRANSFORMATION_MESSAGE);
    m_associatedWin->displayNewMessage(
        "[Select Plane or Line to translate along/rotate around]",
        ccGLWindowInterface::UPPER_CENTER_MESSAGE, true, 3600,
        ccGLWindowInterface::MANUAL_TRANSFORMATION_MESSAGE);
    m_associatedWin->displayNewMessage(
        "[If plane selected, rotation will be around normal vector]",
        ccGLWindowInterface::UPPER_CENTER_MESSAGE, true, 3600,
        ccGLWindowInterface::MANUAL_TRANSFORMATION_MESSAGE);
  } else {
    m_associatedWin->displayNewMessage(
        "[Rotation/Translation mode]",
        ccGLWindowInterface::UPPER_CENTER_MESSAGE, false, 3600,
        ccGLWindowInterface::MANUAL_TRANSFORMATION_MESSAGE);
  }
...
bool ccGraphicalTransformationTool::start() {
  ...
  m_translation = CCVector3d(0, 0, 0);
  m_rotationCenter =
      m_toTransform.getBB_recursive()
          .getCenter(); // m_rotation center == selected entities center
  ...
  connect(m_associatedWin->signalEmitter(), &ccGLWindowSignalEmitter::rotation,
          this, &ccGraphicalTransformationTool::glRotate);
  connect(m_associatedWin->signalEmitter(),
          &ccGLWindowSignalEmitter::translation, this,
          &ccGraphicalTransformationTool::glTranslate);
  m_associatedWin->displayNewMessage(
      QString(), ccGLWindowInterface::UPPER_CENTER_MESSAGE); // clear the area
...
void ccGraphicalTransformationTool::stop(bool state) {
  if (m_associatedWin) {
    ...
    m_associatedWin->displayNewMessage(
        "[Rotation/Translation mode OFF]",
        ccGLWindowInterface::UPPER_CENTER_MESSAGE, false, 2,
        ccGLWindowInterface::MANUAL_TRANSFORMATION_MESSAGE);
    m_associatedWin->redraw(
        false, false); // we have to redraw the 3D layer for labels!
  }
...
void ccGraphicalTransformationTool::glTranslate(const CCVector3d &realT) {
  ...
  if (t.norm2() != 0) {
    ...
    if (m_advMode && m_advRotateRef != nullptr && refAxisRadio->isChecked() &&
        !m_advRotateRefIsChild) {
      // If we translate we have to update the rotation center when using the
      // ref axis as center of rotation. Dont update the rotation center if the
      // rotation ref is contained in m_toTranslate.
      CCVector3d centerUpdate =
          m_rotation.inverse() * m_advRotationRefObjCenter -
          m_rotation.inverse() * m_position.getTranslationAsVec3D();
      m_translation += (m_rotationCenter - centerUpdate) -
                       m_rotation * (m_rotationCenter - centerUpdate);
...
void ccGraphicalTransformationTool::glRotate(const ccGLMatrixd &rotMat) {
  if (m_advMode && m_advRotateRef != nullptr) {
    rotComboBoxItems rotAxis = static_cast<rotComboBoxItems>(
        rotComboBox->itemData(rotComboBox->currentIndex()).toInt());
    double angle = 0;
    switch (rotAxis) {
    case rotComboBoxItems::X:
...
void ccGraphicalTransformationTool::incrementalTransform() {
  ...
  // The rotation part:
  // No incremental rotation if rotComboBoxItems == XYZ or rotComboBoxItems ==
  // NONE because it has no sense it could be handled in the subsequent switch
  // cases but it is somehiow more explicit to check if the SpinBox is enabled
  if (incrementalRotSpin->isEnabled()) {
    const double alpha =
        isForwardTransform
            ? CCCoreLib::DegreesToRadians(incrementalRotSpin->value())
            : -CCCoreLib::DegreesToRadians(incrementalRotSpin->value());
    if (m_advMode && m_advRotateRef != nullptr) {
      m_rotation =
          m_rotation * arbitraryVectorRotation(alpha, m_advRotationAxis);
    } else {
      ccGLMatrixd rotMat;
      // compute rotation matrix from spinbox
      switch (static_cast<rotComboBoxItems>(
          rotComboBox->itemData(rotComboBox->currentIndex()).toInt())) {
      case rotComboBoxItems::X:
        rotMat.initFromParameters(alpha, CCVector3d(1.0, 0.0, 0.0),
                                  CCVector3d(0.0, 0.0, 0.0));
        break;
      case rotComboBoxItems::Y:
        rotMat.initFromParameters(alpha, CCVector3d(0.0, 1.0, 0.0),
                                  CCVector3d(0.0, 0.0, 0.0));
        break;
      case rotComboBoxItems::Z:
        rotMat.initFromParameters(alpha, CCVector3d(0.0, 0.0, 1.0),
                                  CCVector3d(0.0, 0.0, 0.0));
        break;
      default:
        return;
  ...
  // we pass a vector with equal magnitude in the three components; gltranslate
  // will handle the correct transform for us.
  const double transMagnitude = isForwardTransform
                                    ? incrementalTransSpin->value()
                                    : -incrementalTransSpin->value();
...
void ccGraphicalTransformationTool::reset() {
  ...
  updateAllGLTransformations();
  advRotateRefUpdate(advRotateComboBox->currentIndex()); // force an update
  advTranslateRefUpdate(advTranslateComboBox->currentIndex()); // force an
                                                               // update
}

void ccGraphicalTransformationTool::setRotationCenter(CCVector3d &center) {
  m_translation +=
      (m_rotationCenter - center) - m_rotation * (m_rotationCenter - center);
...
void ccGraphicalTransformationTool::updateAllGLTransformations() {
  ...
  m_position +=
      m_rotationCenter + m_translation - m_rotation * m_rotationCenter;
  ...
  if (m_advTranslateRefIsChild && m_advTranslateRef != nullptr) {
    // update m_advTranslationTransform if the ref object is translated/rotated
    // by virtue of being in m_toTransform
    if (m_advTranslateRef->isA(CC_TYPES::PLANE)) {
      m_advTranslationTransform =
          m_position *
          ccGLMatrixd(m_advTranslateRef->getGLTransformationHistory().data());
    } else if (m_advTranslateRef->isA(CC_TYPES::POLY_LINE)) {
      ccPolyline *line = static_cast<ccPolyline *>(m_advTranslateRef);
      CCVector3 arbitraryVec = line->getGLTransformation() *
                               (*line->getPoint(1) - *line->getPoint(0));
      m_advTranslationTransform =
          m_position * arbitraryVectorTranslation(arbitraryVec);
    }
  }
...
#define NORMALIZE_TRANSFORMATION_MATRIX_WITH_EULER
  ...
  {
    // convert matrix back and forth so as to be sure to get a 'true' rotation
    // matrix DGM: we use Euler angles, as the axis/angle method (formerly used)
    // is not robust enough! Shifts could be perceived by the user.
    double phi_rad = 0.0;
...
#ifdef QT_DEBUG
    ccLog::Print("[GraphicalTransformationTool] Final transformation (before "
                 "correction):");
    ccLog::Print(finalTrans.toString(12, ' ')); // full precision
    ccLog::Print(QString("Angles(%1,%2,%3) T(%5,%6,%7)")
                     .arg(phi_rad)
                     .arg(theta_rad)
                     .arg(psi_rad)
                     .arg(t3D.x)
...
#ifdef QT_DEBUG
  // test: compute rotation "norm" (as it may not be exactly 1 due to numerical
  // (in)accuracy!)
  {
  ...
  for (unsigned i = 0; i < m_toTransform.getChildrenNumber(); ++i) {
    ...
    // special case: if the object is a mesh vertices set, we may have to update
    // the mesh normals!
    if (toTransform->isA(CC_TYPES::POINT_CLOUD) && toTransform->getParent() &&
        toTransform->getParent()->isKindOf(CC_TYPES::MESH)) {

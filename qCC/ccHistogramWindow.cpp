...
ccHistogramWindow::ccHistogramWindow(QWidget *parent /*=nullptr*/)
    : QCustomPlot(parent), m_titlePlot(nullptr), m_colorScheme(USE_SOLID_COLOR),
      m_solidColor(Qt::blue),
      m_colorScale(ccColorScalesManager::GetDefaultScale()),
      m_associatedSF(nullptr), m_numberOfClassesCanBeChanged(false),
      m_histogram(nullptr), m_minVal(0), m_maxVal(0), m_maxHistoVal(0),
      m_overlayCurve(nullptr), m_vertBar(nullptr),
      m_drawVerticalIndicator(false), m_verticalIndicatorPositionPercent(0),
      m_sfInteractionModes(SFInteractionMode::None),
      m_axisDisplayOptions(AxisDisplayOption::All), m_selectedItem(NONE),
      m_areaLeft(nullptr),
      ...
      m_lastMouseClick(0, 0), m_refreshAfterResize(true) {
...
bool ccHistogramWindow::computeBinArrayFromSF(size_t binCount) {
  ...
  if (!m_associatedSF) {
    assert(false);
    ccLog::Error(
        "[ccHistogramWindow::computeBinArrayFromSF] Need an associated SF!");
    return false;
  }

  if (binCount == 0) {
    assert(false);
    ccLog::Error("[ccHistogramWindow::computeBinArrayFromSF] Invalid number of "
                 "classes!");
    return false;
  }
  ...
  if (binCount == m_associatedSF->getHistogram().size()) {
    try {
      ...
      ccLog::Warning(
          "[ccHistogramWindow::computeBinArrayFromSF] Not enough memory!");
      return false;
    }
    return true;
  }
  ...
  if (range > 0.0) {
    ...
    for (unsigned i = 0; i < count; ++i) {
      ...
      // we ignore values outside of [m_minVal,m_maxVal] (works fro NaN values
      // as well)
  ...
  if (m_histogram && m_colorScheme == USE_SF_SCALE && m_associatedSF &&
    ...
    for (int i = 0; i < histoSize; ++i) {
      ...
      const ccColor::Rgb *col =
          m_associatedSF->getColor(static_cast<ScalarType>(keyData[i]));
...
void ccHistogramWindow::refresh() {
  ...
  xAxis->setRange(
      minVal,
      std::max(minVal + std::numeric_limits<ScalarType>::epsilon(), maxVal));
  ...
  if (!m_titleStr.isEmpty()) {
    ...
    m_titlePlot = new QCPTextElement(
        this, QStringLiteral("%0 [%1 classes]")
                  .arg(m_titleStr, QString::number(m_histoValues.size())));
    // title font
  ...
  ccColorScale::Shared colorScale =
      (m_colorScale ? m_colorScale : ccColorScalesManager::GetDefaultScale());
      ...
      if (colorScheme != USE_SOLID_COLOR) {
        ...
        if (colorScheme == USE_SF_SCALE) {
          ...
          if (colorScale->isRelative()) {
            ...
            col = colorScale->getColorByRelativePos(
                std::max(0.0, std::min(1.0, relativePos)));
          }
        }
        if (!col) // hidden values may have no associated color!
        {
  ...
  } else if (m_drawVerticalIndicator) // vertical hint
  {
    ...
    // horizontal position
    keyData[0] =
        m_minVal + (m_maxVal - m_minVal) * m_verticalIndicatorPositionPercent;
    ...
    unsigned bin = static_cast<unsigned>(m_verticalIndicatorPositionPercent *
                                         m_histoValues.size());
    QString valueStr = QString("bin %0").arg(bin);
    m_vertBar->setText(valueStr);
    valueStr =
        QString("< %0 %").arg((100.0 * partialSum) / totalSum, 0, 'f', 3);
    m_vertBar->appendText(valueStr);
    valueStr = QString("val = %0")
                   .arg(m_minVal + (m_maxVal - m_minVal) *
...
ccHistogramWindowDlg::ccHistogramWindowDlg(QWidget *parent /*=nullptr*/)
    : QDialog(parent, Qt::WindowMaximizeButtonHint | Qt::WindowCloseButtonHint),
      m_win(new ccHistogramWindow(this)), m_gui(new Ui_HistogramDialog) {
  m_gui->setupUi(this);
...
bool ccHistogramWindowDlg::exportToCSV(QString filename) const {
  if (!m_win || m_win->histoValues().empty()) {
    ccLog::Warning(
        "[Histogram] Histogram has no associated values (can't save file)");
    return false;
  }

  QFile file(filename);
  if (!file.open(QFile::WriteOnly | QFile::Text)) {
    ccLog::Warning(QString("[Histogram] Failed to save histogram to file '%1'")
                       .arg(filename));
    return false;
  }
...
void ccHistogramWindowDlg::onExportToImage() {
  ...
  if (image.save(outputFilename)) {
    ccLog::Print(QString("[Histogram] Image '%1' successfully saved")
                     .arg(outputFilename));
  } else {

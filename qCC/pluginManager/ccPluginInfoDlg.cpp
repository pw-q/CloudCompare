...
static QString
sFormatReferenceList(const ccPluginInterface::ReferenceList &list) {
...
static QString sFormatContactList(const ccPluginInterface::ContactList &list,
  ...
  const QString emailFormat(
      "&lt;<a href=\"mailto:%1?Subject=CloudCompare %2\">%1</a>&gt;");
  QString formattedText;
...
private:
  static void _init() {
    ...
    sIconMap[CC_GL_FILTER_PLUGIN] =
        QIcon(":/CC/pluginManager/images/gl_plugin.png");
    sIconMap[CC_IO_FILTER_PLUGIN] =
...
ccPluginInfoDlg::ccPluginInfoDlg(QWidget *parent)
    : QDialog(parent), m_UI(new Ui::ccPluginInfoDlg),
  ...
  m_UI->mWarningLabel->setText(
      tr("Enabling/disabling plugins will take effect next time you run %1")
          .arg(QApplication::applicationName()));
  m_UI->mWarningLabel->setStyleSheet(
      QStringLiteral("QLabel { background-color : #FFFF99; color : black; }"));
...
const ccPluginInterface *
ccPluginInfoDlg::pluginFromItemData(const QStandardItem *item) const {
...
void ccPluginInfoDlg::updatePluginInfo(const ccPluginInterface *plugin) {
  ...
  case CC_STD_PLUGIN: {
    if (iconPixmap.isNull()) {
      iconPixmap =
          QPixmap(":/CC/pluginManager/images/std_plugin.png").scaled(iconSize);
    }
  ...
  case CC_GL_FILTER_PLUGIN: {
    if (iconPixmap.isNull()) {
      iconPixmap =
          QPixmap(":/CC/pluginManager/images/gl_plugin.png").scaled(iconSize);
    }
  ...
  case CC_IO_FILTER_PLUGIN: {
    if (iconPixmap.isNull()) {
      iconPixmap =
          QPixmap(":/CC/pluginManager/images/io_plugin.png").scaled(iconSize);
    }

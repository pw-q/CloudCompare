...
ccPluginUIManager::ccPluginUIManager(ccMainAppInterface *appInterface,
                                     QWidget *parent)
    : QObject(parent), m_parentWidget(parent), m_appInterface(appInterface),
      m_pluginMenu(nullptr), m_glFilterMenu(nullptr),
      m_actionRemoveFilter(nullptr), m_glFilterActions(this),
      m_mainPluginToolbar(nullptr), m_showPluginToolbar(nullptr),
      m_glFiltersToolbar(nullptr), m_showGLFilterToolbar(nullptr) {
  setupActions();
  setupMenus();
  setupToolbars();
}
...
void ccPluginUIManager::init() {
  ...
  for (ccPluginInterface *plugin : plugins) {
    ...
    case CC_GL_FILTER_PLUGIN: // GL filter
    {
      ...
      connect(action, &QAction::triggered, this,
              &ccPluginUIManager::enableGLFilter);
  ...
  m_glFiltersToolbar->setEnabled(
      !m_glFilterMenu->isEmpty()); // [sic] we have toolbar actions if we have
                                   // them in the menu
...
QMenu *ccPluginUIManager::shaderAndFilterMenu() const { return m_glFilterMenu; }
...
QToolBar *ccPluginUIManager::glFiltersToolbar() { return m_glFiltersToolbar; }
...
void ccPluginUIManager::setupActions() {
  m_actionRemoveFilter =
      new QAction(QIcon(":/CC/images/noFilter.png"), tr("Remove Filter"), this);
  m_actionRemoveFilter->setEnabled(false);

  connect(m_actionRemoveFilter, &QAction::triggered, this,
          &ccPluginUIManager::disableGLFilter);
...
void ccPluginUIManager::setupToolbars() {
  ...
  connect(m_showGLFilterToolbar, &QAction::toggled, m_glFiltersToolbar,
          &QToolBar::setVisible);
}
...
void ccPluginUIManager::addActionsToToolBar(ccStdPluginInterface *stdPlugin,
  ...
  if (actions.size() > 1) {
    QToolBar *toolBar =
        new QToolBar(pluginName + QStringLiteral(" toolbar"), m_parentWidget);
...
void ccPluginUIManager::enableGLFilter() {
  ...
  if (win->areGLFiltersEnabled()) {
    ...
    if (filter != nullptr) {
      ...
      ccConsole::Print("Note: go to << Display > Shaders & Filters > No filter "

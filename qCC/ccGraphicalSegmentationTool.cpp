...
ccGraphicalSegmentationTool::ccGraphicalSegmentationTool(QWidget *parent)
    : ccOverlayDialog(parent), Ui::GraphicalSegmentationDlg(),
      m_somethingHasChanged(false), m_state(0), m_segmentationPoly(nullptr),
      m_polyVertices(nullptr), m_rectangularSelection(false),
  ...
  connect(pauseButton, &QToolButton::toggled, this,
          &ccGraphicalSegmentationTool::pauseSegmentationMode);
  ...
  addOverriddenShortcut(Qt::Key_Tab); // tab key to switch between rectangular
  ...
  m_polyVertices = new ccPointCloud(
      ...
      static_cast<unsigned>(
          ReservedIDs::INTERACTIVE_SEGMENTATION_TOOL_POLYLINE_VERTICES));
  m_segmentationPoly = new ccPolyline(
      m_polyVertices, static_cast<unsigned>(
                          ReservedIDs::INTERACTIVE_SEGMENTATION_TOOL_POLYLINE));
...
bool ccGraphicalSegmentationTool::linkWith(ccGLWindowInterface *win) {
  ...
  if (m_associatedWin) {
    ...
    connect(m_associatedWin->signalEmitter(),
            &ccGLWindowSignalEmitter::buttonReleased, this,
...
void ccGraphicalSegmentationTool::stop(bool accepted) {
  ...
  if (m_associatedWin) {
    m_associatedWin->displayNewMessage(
        "Segmentation [OFF]", ccGLWindowInterface::UPPER_CENTER_MESSAGE, false,
        2, ccGLWindowInterface::MANUAL_SEGMENTATION_MESSAGE);
  ...
  for (auto item :
       m_enableOnClose) // in export mode, all parts are enabled at the close
  {
    if (item != nullptr) {
  ...
  for (auto item : m_disableOnClose) // in export mode, the original entities
                                     // are disabled on close to make sure the
                                     // newly created parts are visible
  {
    if (item != nullptr) {
...
bool ccGraphicalSegmentationTool::addEntity(ccHObject *entity,
  ...
  if (!entity->isDisplayedIn(m_associatedWin) && !silent) {
    ccLog::Warning(QString("[Graphical Segmentation Tool] Entity [%1] is not "
                           "visible in the active 3D view!")
                       .arg(entity->getName()));
  }
  ...
  if (entity->isKindOf(CC_TYPES::POINT_CLOUD)) {
    ...
    if (ccGenericMesh::IsCloudVerticesOfMesh(cloud, &associatedMesh)) {
      ...
      if (m_toSegment.contains(associatedMesh)) {
        if (!silent) {
          ccLog::Warning(QString("[Graphical Segmentation Tool] The mesh "
                                 "associated to cloud %1 is already selected")
                             .arg(cloud->getName()));
        }
        return false;
      }
    ...
    if (ccPolyline::IsCloudVerticesOfPolyline(cloud, &associatedPolyline)) {
      ...
      if (m_toSegment.contains(associatedPolyline)) {
        if (!silent) {
          ccLog::Warning(QString("[Graphical Segmentation Tool] The polyline "
                                 "associated to cloud %1 is already selected")
                             .arg(cloud->getName()));
        }
        return false;
    ...
    //	if (child != associatedMesh && child != associatedPolyline) // we don't
    //add the associated mesh or polyline (if any)
    //	{
    ...
    if (entity->isKindOf(CC_TYPES::PRIMITIVE)) {
      if (!silent) {
        ccLog::Warning("[ccGraphicalSegmentationTool] Can't segment primitives "
                       "yet! Sorry...");
      }
      return false;
    }
    if (entity->isKindOf(CC_TYPES::SUB_MESH)) {
      if (!silent) {
        ccLog::Warning("[ccGraphicalSegmentationTool] Can't segment "
                       "sub-meshes! Select the parent mesh...");
      }
      return false;
    }
    ...
    // DGM: the code below is useless since we don't allow CC_TYPES::SUB_MESH
    // entities (see above)
    //
    // first, we must check that there's no mesh and at least one of its
    // sub-mesh mixed in the current selection!
    // for (QSet<ccHObject*>::const_iterator p = m_toSegment.constBegin(); p !=
    // m_toSegment.constEnd(); ++p)
    //{
    ...
    //		{
    //			if ((otherMesh->isA(CC_TYPES::SUB_MESH) &&
    //mesh->isA(CC_TYPES::MESH))
    //				|| (otherMesh->isA(CC_TYPES::MESH) &&
    //mesh->isA(CC_TYPES::SUB_MESH)))
    //			{
    //				if (!silent)
    //				{
    //					ccLog::Warning("[Graphical Segmentation Tool] Can't mix
    ...
    // Make sure the vertices of this mesh are not already in the 'to segment'
    // list
    if (m_toSegment.contains(vertices)) {
      // let's remove the vertices
      mesh->pushDisplayState(); // just in case the vertices were inserted
                                // before the mesh)
    ...
    if (verticesCloud && m_toSegment.contains(verticesCloud)) {
      // let's remove the vertices
      poly->pushDisplayState(); // just in case the vertices were inserted
                                // before the polyline)
    ...
    if (!silent) {
      ccLog::Warning("[ccGraphicalSegmentationTool] Can't segment entity " +
                     entity->getName());
    }
    return false;
  }
...
void ccGraphicalSegmentationTool::updatePolyLine(int x, int y,
  ...
  if (m_state & RECTANGLE) {
    ...
    CCVector3 *B =
        const_cast<CCVector3 *>(m_polyVertices->getPointPersistentPtr(1));
    CCVector3 *C =
        const_cast<CCVector3 *>(m_polyVertices->getPointPersistentPtr(2));
    CCVector3 *D =
        const_cast<CCVector3 *>(m_polyVertices->getPointPersistentPtr(3));
...
void ccGraphicalSegmentationTool::addPointToPolylineExt(
    int x, int y, bool allowClicksOutside) {
  ...
  if (!allowClicksOutside &&
      (x < 0 || y < 0 || x >= m_associatedWin->qtWidth() ||
       y >= m_associatedWin->qtHeight())) {
  ...
  bool ctrlKeyPressed =
      m_rectangularSelection || ((QApplication::keyboardModifiers() &
  ...
  // DGM: to increase the poll rate of the mouse movements in
  // ccGLWindow::mouseMoveEvent we have to completely grab the mouse focus! (the
  //only way to take back the control is to right-click now...)
...
void ccGraphicalSegmentationTool::segment(
    bool keepPointsInside,
    ScalarType classificationValue /*=CCCoreLib::NAN_VALUE*/,
  ...
  if (!m_segmentationPoly->isClosed()) {
    ccLog::Error("Define and/or close the segmentation polygon first! (right "
                 "click to close)");
    return;
  }
  ...
  if ((m_state & POLYLINE) != 0 && (m_state & RUNNING) != 0) {
    ...
    ccLog::Warning(QString("Polyline was not closed - we'll close it with the "
                           "current mouse cursor position: (%1 ; %2)")
                       .arg(mousePos.x())
                       .arg(mousePos.y()));
  ...
  bool polyInsideViewport = true;
  {
    ...
    for (int i = 0; i < vertexCount; ++i) {
      ...
      if (P2D->x < -half_w || P2D->x > half_w || P2D->y < -half_h ||
          P2D->y > half_h) {
  ...
  ccLog::PrintDebug("Polyline is fully inside viewport: " +
                    QString(polyInsideViewport ? "Yes" : "No"));

  bool classificationMode =
      CCCoreLib::ScalarField::ValidValue(classificationValue);
  ...
  for (QSet<ccHObject *>::const_iterator p = m_toSegment.constBegin();
    ...
    // if a classification value is set as input, this means that we want to
    // label the set of points, and we don't want to segment it
    CCCoreLib::ScalarField *classifSF = nullptr;
    if (classificationMode) {
      ccPointCloud *pc = ccHObjectCaster::ToPointCloud(*p);
      if (!pc) {
        ccLog::Warning("Can't apply classification to entity " +
                       (*p)->getName());
        continue;
      }
    ...
    // we project each point and we check if it falls inside the segmentation
    // polyline
#if defined(_OPENMP)
#pragma omp parallel for num_threads(omp_get_max_threads())
    ...
    for (int i = 0; i < cloudSize; ++i) {
      if (visibilityArray[i] == CCCoreLib::POINT_VISIBLE) {
        ...
        if (pointInFrustum ||
            !polyInsideViewport) // we can only skip the test if the point is
                                 // outside the viewport/frustum AND the polyline
                                 // is fully inside the viewport
        {
          ...
          pointInside = CCCoreLib::ManualSegmentationTools::isPointInsidePoly(
        ...
        if (classifSF) // classification mode
        {
          ...
          visibilityArray[i] = (pointInside ? CCCoreLib::POINT_VISIBLE
                                            : CCCoreLib::POINT_HIDDEN);

          if (pointInside) {
            // (exported points or triangles will be hidden until the Segment
            // tool is closed)
          ...
          visibilityArray[i] =
              (keepPointsInside != pointInside ? CCCoreLib::POINT_HIDDEN
      ...
      if (entity->isKindOf(CC_TYPES::POINT_CLOUD)) {
        ...
        if (segmentedCloud != nullptr) {
          if (segmentedCloud->size() != 0) {
            ...
            MainWindow::TheInstance()->addToDB(segmentedCloud, false, true,
                                               false, false);
        ...
        if (segmentedMesh != nullptr) {
          if (segmentedMesh->size() != 0) {
            ...
            MainWindow::TheInstance()->addToDB(segmentedMesh, false, true,
                                               false, false);
        ...
        ccLog::Warning("Entity type is not supported in 'export selection' "
                       "mode, only points clouds and meshes are accepted");
        return;
      }
...
void ccGraphicalSegmentationTool::stopRunning() {
  m_state &= (~RUNNING);
  buttonsFrame->setEnabled(
      true); // we restore the buttons when running is stopped
}
...
void ccGraphicalSegmentationTool::pauseSegmentationMode(bool state) {
  ...
  if (state /*=activate pause mode*/) {
    ...
    m_associatedWin->setInteractionMode(
        ccGLWindowInterface::MODE_TRANSFORM_CAMERA);
    m_associatedWin->displayNewMessage(
        "Segmentation [PAUSED]", ccGLWindowInterface::UPPER_CENTER_MESSAGE,
        false, 3600, ccGLWindowInterface::MANUAL_SEGMENTATION_MESSAGE);
    m_associatedWin->displayNewMessage(
        "Unpause to segment again", ccGLWindowInterface::UPPER_CENTER_MESSAGE,
        true, 3600, ccGLWindowInterface::MANUAL_SEGMENTATION_MESSAGE);
  } else {
    m_state = STARTED;
    m_associatedWin->setInteractionMode(
...
void ccGraphicalSegmentationTool::setClassificationValue() {
  ...
  int iValue = QInputDialog::getInt(
      m_associatedWin->asWidget(), QT_TR_NOOP("Classification"),
      QT_TR_NOOP("value"), s_classValue, -1000000, 1000000, 1, &ok);
  if (!ok) {
    return;
  }
  s_classValue = iValue;
...
void ccGraphicalSegmentationTool::doActionUseExistingPolyline() {
  ...
  if (mainWindow) {
    ...
    if (!polylines.empty()) {
      ...
      if (poly->filterChildren(viewports, false, CC_TYPES::VIEWPORT_2D_OBJECT,
                               true) == 1) {
        // shall we apply this viewport?
        if (QMessageBox::question(
                m_associatedWin->asWidget(), "Associated viewport",
                "The selected polyline has an associated viewport: do you want "
                "to apply it?",
                QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes) {
          m_associatedWin->setViewportParameters(
      ...
      if (m_polyVertices->reserve(vertices->size() +
        ...
        if (m_segmentationPoly->isClosed()) {
          // stop (but we can't call pauseSegmentationMode as it would remove
...
void ccGraphicalSegmentationTool::doExportSegmentationPolyline() {
  ...
  if (mainWindow && m_segmentationPoly) {
    ...
    // if the polyline is 2D and we export the polyline in 3D, we must project
    // its vertices
    if (!mode2D) {
      ...
      if (verticesPC) {
        for (unsigned i = 0; i < vertices->size(); ++i) {
          ...
          CCVector3d Pd(half_w + Pscreen->x, half_h + Pscreen->y,
                        0 /*Pscreen->z*/);
          CCVector3d Q3D;
        ...
        assert(false);
        ccLog::Warning("[Segmentation] Failed to convert 2D polyline to 3D! "
      ...
      {
        for (QSet<ccHObject *>::const_iterator it = m_toSegment.constBegin();
      ...
      if (hasGlobalShift && m_toSegment.size() != 1) {
        hasGlobalShift =
            (QMessageBox::question(
                 MainWindow::TheInstance(), "Apply Global Shift",
                 "At least one of the segmented entity has been shifted. Apply "
                 "the same shift to the polyline?",
                 QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes);
      }
    ...
    QString polyName =
        QString("Segmentation polyline #%1").arg(++s_polylineExportCount);
    poly->setName(polyName);
    poly->setEnabled(
        false); // we don't want it to appear while the segmentation mode is
                // enabled! (anyway it's 2D only...)
    poly->set2DMode(mode2D);
    poly->setColor(
        ccColor::yellow); // we use a different color so as to differentiate
                          // them from the active polyline!
    ...
    ccLog::Print(QString("[Segmentation] Polyline exported (%1 vertices)")
                     .arg(poly->size()));
  }
}
...
static void RemoveUnusedLabelsAndUpdateTheOthers(
    std::set<cc2DLabel *> &watchedLabels, ccHObject *entity,
    ...
    for (unsigned i = 0; i < label->size(); ++i) {
      ...
      if (pp.entity() == entity) {
        if (pp.index < newIndexesOfRemainingPointsOrTriangles.size() &&
          ...
          bool saveContext =
              (labelParent != entity && !entity->isAncestorOf(labelParent));
          if (saveContext)
...
bool ccGraphicalSegmentationTool::applySegmentation(
    ccMainAppInterface *app, ccHObject::Container &newEntities) {
  if (!app) {
    assert(false);
    return false;
  }
      ...
      for (ccHObject *labelEntity : loadedLabels) {
        ...
        for (unsigned i = 0; i < label->size(); ++i) {
          ...
          if (m_toSegment.contains(pp.entity())) {
            // we will watch this label as it may be deprecated by the
  ...
  for (QSet<ccHObject *>::iterator p = m_toSegment.begin();
    ...
    // check first if we can modify this entity directly or if there might be
    // dire consequences...
    bool canModify = true;
    if (entity->isLocked()) {
      // we can't delete this entity
      ccLog::Warning("Entity " + entity->getName() +
    ...
    if (entity->isKindOf(CC_TYPES::POINT_CLOUD)) {
      ...
      if (cloud->size() == 0) {
        // ignore this cloud
        ccLog::Warning("Cloud " + cloud->getName() +
                       " is empty. We will ignore it");
        continue;
      }
      if (canModify) {
        // check that the point cloud is not the vertices of a mesh or of a
        // polyline
        if (ccGenericMesh::IsCloudVerticesOfMesh(cloud)) {
          // we can't delete this cloud
          ccLog::Warning("Cloud " + cloud->getName() +
                         " seems to be the vertices of a mesh. We won't be "
                         "able to modify it");
          canModify = false;
        } else if (ccPolyline::IsCloudVerticesOfPolyline(cloud)) {
          // we can't delete this cloud
          ccLog::Warning("Cloud " + cloud->getName() +
                         " seems to be the vertices of a polyine. We won't be "
                         "able to modify it");
          canModify = false;
        }
      }
    } else if (entity->isA(CC_TYPES::MESH)) // TODO: sub-meshes and primitives
                                            // are not handled for now
    {
      ccGenericMesh *mesh = static_cast<ccGenericMesh *>(entity);
      if (mesh->size() == 0 || mesh->getAssociatedCloud()->size() == 0) {
        // ignore this mesh
        ccLog::Warning("Mesh " + mesh->getName() +
                       " is empty. We will ignore it");
        continue;
      }
    } else if (entity->isKindOf(CC_TYPES::POLY_LINE)) {
      ...
      if (poly->size() == 0 || poly->getAssociatedCloud()->size() == 0) {
        ...
        ccLog::Warning("Polyline " + poly->getName() +
                       " is empty. We will ignore it");
        continue;
      }
      ...
      if (!cantModifyPolylinesWarningIssued) {
        ccLog::Warning(
            "Can't modify polylines. A new polyline will be created.");
    ...
    if (entity->isKindOf(CC_TYPES::POINT_CLOUD) ||
        ...
        if (segmentedCloud) {
          ...
          } else // we have a new entity
          {
            ...
            if (removeSelectedElementsFromEntity &&
                !deleteOriginalEntity) // if we have removed points from the
                                       // original entity
            {
              // be smart and keep only the necessary labels
              RemoveUnusedLabelsAndUpdateTheOthers(
                  watchedLabels, cloud, newIndexesOfRemainingPoints, app);
            }
          }
        }
        ...
        if (segmentatedMesh) {
          ...
          } else // we have a new entity
          {
            ...
            if (removeSelectedElementsFromEntity && !deleteOriginalEntity) {
              // be smart and keep only the necessary labels
              RemoveUnusedLabelsAndUpdateTheOthers(
                  watchedLabels, mesh, newIndexesOfRemainingTriangles, app);
            }
          }
        }
      } else {
      ...
      if (segmentationResult) // we have a result (= a new entity)
      {
        // update suffix
        {
          ...
          if (segmentationResult->isKindOf(CC_TYPES::MESH) &&
              entity->isKindOf(CC_TYPES::MESH)) {
        ...
        if (removeSelectedElementsFromEntity &&
            !deleteOriginalEntity) // if we were able to modify the original
                                   // entity
        {
          ...
          if (entity->isKindOf(CC_TYPES::MESH)) {
            ...
            if (!verticesName.endsWith(remainingSuffix)) {
              mesh->getAssociatedCloud()->setName(verticesName +
                                                  remainingSuffix);
            }
          }
          ...
          for (ccHObject *child : gblSensors) {
            ...
            // clear the associated depth buffer of the original sensor
            // (deprecated)
        ...
        while (resultParent &&
               (resultParent->isKindOf(CC_TYPES::MESH) ||
                resultParent->isKindOf(CC_TYPES::POINT_CLOUD))) {
      ...
      if (!deleteOriginalEntity) {
        ...
        while (it != watchedLabels.end()) {
          ...
          for (unsigned i = 0; i < label->size(); ++i) {
            ...
            if (pp.entity() == entity) {
              ...
              if (saveContext)
                parentContext =
                    app->removeObjectTemporarilyFromDBTree(labelParent);
              labelParent->removeChild(label);
              if (saveContext)
                app->putObjectBackIntoDBTree(labelParent, parentContext);
        ...
        delete entity; // TODO: should we wait that all entities are processed
                       // before removing it?
        entity = nullptr;
      }
    } else if (entity->isKindOf(CC_TYPES::POLY_LINE)) {

...
ccComparisonDlg::ccComparisonDlg(ccHObject *compEntity, ccHObject *refEntity,
                                 CC_COMPARISON_TYPE cpType,
                                 QWidget *parent /*=nullptr*/,
                                 bool noDisplay /*=false*/)
    : QDialog(parent, Qt::Tool), Ui::ComparisonDialog(), m_compEnt(compEntity),
      m_compCloud(nullptr), m_compOctree(nullptr), m_compOctreeIsPartial(false),
      m_compSFVisibility(false), m_refEnt(refEntity), m_refCloud(nullptr),
      m_refMesh(nullptr), m_refOctree(nullptr), m_refOctreeIsPartial(false),
      m_refVisibility(false), m_compType(cpType), m_noDisplay(noDisplay),
      m_bestOctreeLevel(0) {
  setupUi(this);
  ...
  if (m_refMesh) {
    ...
    filterVisibilityCheckBox->setEnabled(
        m_refCloud && m_refCloud->isA(CC_TYPES::POINT_CLOUD) &&
  ...
  connect(localModelComboBox, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &ccComparisonDlg::locaModelChanged);
  connect(maxSearchDistSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccComparisonDlg::maxDistUpdated);
  connect(split3DCheckBox, &QCheckBox::toggled, this,
          &ccComparisonDlg::enableCompute2D);
}
...
bool ccComparisonDlg::prepareEntitiesForComparison() {
  ...
  // compared entity
  if (!m_compEnt->isA(CC_TYPES::POINT_CLOUD)) // TODO --> pas possible avec des
                                              // GenericPointCloud ? :(
  {
    if (m_compType == CLOUDCLOUD_DIST ||
        (m_compType == CLOUDMESH_DIST &&
         !m_compEnt->isKindOf(CC_TYPES::MESH))) {
    ...
    {
      ccLog::Error("Dialog initialization error! (bad entity type - works only "
                   "with real point clouds [todo])");
      return false;
    }
  ...
  if (oldSfIdx >= 0) {
    m_oldSfName =
        QString::fromStdString(m_compCloud->getScalarFieldName(oldSfIdx));
  }

  // reference entity
  if ((m_compType == CLOUDMESH_DIST && !m_refEnt->isKindOf(CC_TYPES::MESH)) ||
      (m_compType == CLOUDCLOUD_DIST &&
       !m_refEnt->isA(CC_TYPES::POINT_CLOUD))) {
    ccLog::Error("Dialog initialization error! (bad entity type)");
    return false;
  }
  ...
  } else /*if (m_compType == CLOUDCLOUD_DIST)*/
  {
    ...
    // for computing cloud/cloud distances we need also the reference cloud's
    // octree
...
bool ccComparisonDlg::computeApproxDistances() {
  ...
  if (sfIdx < 0) {
    sfIdx =
        m_compCloud->addScalarField(CC_TEMP_APPROX_DISTANCES_DEFAULT_SF_NAME);
    if (sfIdx < 0) {
      ccLog::Error("Failed to allocate a new scalar field for computing "
  ...
  case CLOUDCLOUD_DIST: // cloud-cloud
  {
    approxResult =
        CCCoreLib::DistanceComputationTools::computeApproxCloud2CloudDistance(
            ...
            progressDlg.data(), m_compOctree.data(), m_refOctree.data());
  } break;

  case CLOUDMESH_DIST: // cloud-mesh
  {
    CCCoreLib::DistanceComputationTools::Cloud2MeshDistancesComputationParams
        c2mParams;
    {
    ...
    approxResult =
        CCCoreLib::DistanceComputationTools::computeCloud2MeshDistances(
            m_compCloud, m_refMesh, c2mParams, progressDlg.data(),
            m_compOctree.data());
  } break;
  ...
  if (approxResult < 0) {
    ...
    ccLog::Print("[computeApproxDistances] Time: %3.2f s.",
...
int ccComparisonDlg::determineBestOctreeLevel(double maxSearchDist) {
  ...
  const CCCoreLib::ScalarField *approxDistances =
  ...
  const int MAX_OCTREE_LEVEL =
      m_refMesh
          ? 9
          : CCCoreLib::DgmOctree::MAX_OCTREE_LEVEL; // DGM: can't go higher than
  ...
  if (parentWidget()) {
    ...
    progressDlg->setInfo(
        tr("Testing %1 levels...").arg(MAX_OCTREE_LEVEL)); // we lie here ;)
    progressDlg->start();
  }
  ...
  uint64_t maxNeighbourhoodVolume = static_cast<uint64_t>(1)
                                    << (3 * MAX_OCTREE_LEVEL);
  ...
  for (int level = s_minOctreeLevel; level < MAX_OCTREE_LEVEL; ++level) {
    ...
    // we compute a 'correction factor' that converts an approximate distance
    // into an approximate size of the neighborhood (in terms of cells)
    ...
    const CCCoreLib::DgmOctree::cellsContainer &compCodes =
        m_compOctree->pointsAndTheirCellCodes();
    for (CCCoreLib::DgmOctree::cellsContainer::const_iterator c =
             compCodes.begin();
         c != compCodes.end(); ++c) {
      ...
      if (truncatedCode != tempCode) {
        ...
        if (numberOfPointsInCell != 0) {
          // if 'maxSearchDist' has been defined by the user, we must take it
          // into account! (in this case we skip the cell if its approx. distance
          //is superior)
          if (maxSearchDist <= 0 || cellDist <= maxSearchDist) {
            ...
            // if the reference is a mesh
            if (mesh) {
              //(integer) approximation of the neighborhood size (in terms of
              //cells)
              ...
              double neighbourSize3 =
                  (neighbourSize * neighbourSize * neighbourSize) /
                  maxNeighbourhoodVolume;

              // TIME = NEIGHBORS SEARCH + proportional factor *
              // POINTS/TRIANGLES COMPARISONS
              timings[level] +=
                  neighbourSize3 +
                  ((0.5 * numberOfPointsInCell) / maxNeighbourhoodVolume) *
                      (crossingMeshSurface / meanTriangleSurface);
            } else {
              ...
              double lastSliceCellCount =
                  (cellDist > 0 ? cellDist * cellDist * 24.0 + 2.0 : 1.0);
              // TIME = NEIGHBORS SEARCH + proportional factor *
              // POINTS/TRIANGLES COMPARISONS (we admit that the filled cells
              //roughly correspond to the sqrt of the total number of cells)
              timings[level] +=
                  neighbourSize3 +
                  0.1 * ((numberOfPointsInCell * sqrt(lastSliceCellCount) *
                          refListDensity) /
                         maxNeighbourhoodVolume);
            }
          }
    ...
    //	ccLog::PrintDebug(QString("[Distances] Level %1 - timing = %2 (modifier
    //= %3)").arg(level).arg(timings[level]).arg(levelModifier));
    //}

    // ccLog::Print("[Timing] Level %i --> %2.12f", level, timings[level]);

    if (timings[level] * 1.05 <
        timings[theBestOctreeLevel]) // avoid increasing the octree level for
                                     // super small differences (which is
  ...
  ccLog::PrintDebug("[Distances] Best level: %i (maxSearchDist = %f)",
...
bool ccComparisonDlg::computeDistances() {
  ...
  if (octreeLevel == 0) {
    ...
    ccLog::Print(
        QString("[Distances] Octree level (auto): %1").arg(octreeLevel));
  }
  ...
  int sfIdx =
      m_compCloud->getScalarFieldIndexByName(CC_TEMP_DISTANCES_DEFAULT_SF_NAME);
  if (sfIdx < 0) {
    // we need to create a new scalar field
    sfIdx = m_compCloud->addScalarField(CC_TEMP_DISTANCES_DEFAULT_SF_NAME);
    if (sfIdx < 0) {
      ccLog::Error("Couldn't allocate a new scalar field for computing "
  ...
  CCCoreLib::DistanceComputationTools::Cloud2MeshDistancesComputationParams
      c2mParams;
  s_maxThreadCount = c2cParams.maxThreadCount = c2mParams.maxThreadCount =
      maxThreadCountSpinBox->value();
  ccLog::Print(
      QString("[Distances] Will use %1 threads").arg(s_maxThreadCount));
  ...
  case CLOUDCLOUD_DIST: // cloud-cloud
    ...
    if (split3D) {
      ...
      if (!success) {
        ccLog::Error("[ComputeDistances] Not enough memory to generate 3D "
                     "split fields!");
      ...
      if (filterVisibility) {
          ...
          if (child && child->isA(CC_TYPES::GBL_SENSOR)) {
            ...
            if (sensor->getDepthBuffer().zBuff.empty()) {
              ...
              if (!sensor->computeDepthBuffer(pc, errorCode)) {
                ccLog::Warning(QString("[ComputeDistances] ") +
        ...
        if (validDB == 0) {
          ...
          if (QMessageBox::warning(
                  this, "Error",
                  "Failed to find/init the depth buffer(s) on the associated "
                  "sensor! Do you want to continue?",
    ...
    // setup parameters
    {
      ...
      if (localModelingTab->isEnabled()) {
        c2cParams.localModel =
            (CCCoreLib::LOCAL_MODEL_TYPES)localModelComboBox->currentIndex();
        if (c2cParams.localModel != CCCoreLib::NO_MODEL) {
          c2cParams.useSphericalSearchForLocalModel =
              lmRadiusRadioButton->isChecked();
          c2cParams.kNNForLocalModel =
              static_cast<unsigned>(std::max(0, lmKNNSpinBox->value()));
          c2cParams.radiusForLocalModel =
              static_cast<ScalarType>(lmRadiusDoubleSpinBox->value());
    ...
    result = CCCoreLib::DistanceComputationTools::computeCloud2CloudDistances(
        m_compCloud, m_refCloud, c2cParams, progressDlg.data(),
        m_compOctree.data(), m_refOctree.data());
    break;
  ...
  if (result >= 0) {
    ...
    sf->computeMinAndMax();
    sf->computeMeanAndVariance(mean, &variance);
    ccLog::Print("[ComputeDistances] " +
                 tr("Mean distance = %1 / std deviation = %2")
                     .arg(mean)
                     .arg(sqrt(variance)));
    ...
    case CLOUDCLOUD_DIST: // hausdorff
      ...
      break;
    case CLOUDMESH_DIST: // cloud-mesh
      m_sfName = QString(signedDistances
    ...
    if (split3D) {
      ...
      for (unsigned j = 0; j < 3; ++j) {
        ...
        if (sf) {
          ...
          int sfEnter =
              m_compCloud->addScalarField(static_cast<ccScalarField *>(sf));
          assert(sfEnter >= 0);
        }
      }
      ccLog::Warning(
          "[ComputeDistances] Result has been split along each dimension "
          "(check the 3 other scalar fields with '_X', '_Y' and '_Z' suffix!)");
      if (mergeXY) {
        ccLog::Warning("[ComputeDistances] compute 2D distances (xy plane)");
        QString sfNameXY = m_sfName + " (XY)";
        int sf2D =
            m_compCloud->getScalarFieldIndexByName(sfNameXY.toStdString());
        if (sf2D < 0) {
        ...
        for (unsigned idx = 0; idx < m_compCloud->size(); idx++) {
          float d2D =
              pow(pow(c2cParams.splitDistances[0]->getValue(idx), 2) +
                      pow(c2cParams.splitDistances[1]->getValue(idx), 2),
                  0.5);
          sf->setValue(idx, d2D);
        }
        sf->computeMinAndMax();
...
void ccComparisonDlg::showHisto() {
  ...
  hDlg->setWindowTitle(QString("Histogram [%1]").arg(m_compCloud->getName()));
  {
    ...
    histogram->setTitle(
        QString("Approximate distances (%1 values)").arg(m_compCloud->size()));
  ...
  if (m_compCloud) {
    ...
    if (sfIdx >= 0) {
      if (m_sfName.isEmpty()) // hum,hum
      {
        ...
        int _sfIdx =
            m_compCloud->getScalarFieldIndexByName(m_sfName.toStdString());
        if (_sfIdx >= 0) {
...
void ccComparisonDlg::cancelAndExit() {
  if (m_compCloud) {
    ...
    if (!m_oldSfName.isEmpty()) {
      int oldSfIdx =
          m_compCloud->getScalarFieldIndexByName(m_oldSfName.toStdString());
      if (oldSfIdx) {

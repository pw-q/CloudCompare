...
ccPointPairRegistrationDlg::ccPointPairRegistrationDlg(
    ccPickingHub *pickingHub, ccMainAppInterface *app,
    QWidget *parent /*=nullptr*/)
    : ccOverlayDialog(parent), m_alignedPoints("to-be-aligned points"),
      m_refPoints("reference points"), m_paused(false),
      m_pickingHub(pickingHub), m_app(app) {
  assert(m_pickingHub);
  ...
  // restore from persistent settings
  {
    ...
    bool pickSpheres =
        settings.value("PickSpheres", useSphereToolButton->isChecked())
            .toBool();
    double sphereRadius =
        settings.value("SphereRadius", radiusDoubleSpinBox->value()).toDouble();
    int maxRMS = settings.value("MaxRMS", maxRmsSpinBox->value()).toInt();
    bool adjustScale =
        settings.value("AdjustScale", adjustScaleCheckBox->isChecked())
            .toBool();
    bool autoUpdateZoom =
        settings.value("AutoUpdateZom", autoZoomCheckBox->isChecked()).toBool();
    settings.endGroup();
  ...
  connect(TzCheckBox, &QCheckBox::toggled, this,
          &ccPointPairRegistrationDlg::updateAlignInfo);
  connect(rotComboBox, qOverload<int>(&QComboBox::currentIndexChanged), this,
          &ccPointPairRegistrationDlg::updateAlignInfo);
...
ccPointPairRegistrationDlg::EntityContext::EntityContext(ccHObject *ent)
    : entity(ent), originalDisplay(entity ? entity->getDisplay() : nullptr),
  ...
  for (ccHObject *entity : entities) {
    ...
    if (!isShifted) {
      ...
      if (shiftedEntity && shiftedEntity->isShifted()) {
        isShifted = true;
        shift = shiftedEntity
                    ->getGlobalShift(); // we can only consider the first shift!
...
bool ccPointPairRegistrationDlg::linkWith(ccGLWindowInterface *win) {
  ...
  if (m_associatedWin) {
    if (!m_pickingHub->addListener(
            this, true, true,
            ccGLWindowInterface::POINT_OR_TRIANGLE_OR_LABEL_PICKING)) {
      ccLog::Error("Picking mechanism is already in use! Close the other tool "
    ...
    m_associatedWin->displayNewMessage(
        ...
        ccGLWindowInterface::LOWER_LEFT_MESSAGE, true, 3600);
    m_associatedWin->displayNewMessage(
...
bool ccPointPairRegistrationDlg::init(
    ccGLWindowInterface *win, const ccHObject::Container &alignedEntities,
  ...
  if (alignedEntities.empty()) {
    ccLog::Error(
        "[PointPairRegistration] Need at least one to-be-aligned entity!");
    return false;
  }
  ...
  if (!m_associatedWin) {
    // import GL filter so as to get the same rendering aspect!
    {
      ...
      if (!sourceDisplay && !m_referenceEntities.empty()) {
        for (auto it = m_referenceEntities.begin();
             it != m_referenceEntities.end(); ++it) {
          sourceDisplay = it.key()->getDisplay();
          if (sourceDisplay)
            // we found a display
            break;
      ...
      if (sourceDisplay) {
        ccGlFilter *filter =
            static_cast<ccGLWindowInterface *>(sourceDisplay)->getGlFilter();
        if (filter)
...
static QString s_aligned_tooltip =
    QObject::tr("Whether the point is expressed in the entity original "
                "coordinate system (before being shifted by CC) or not");
...
void ccPointPairRegistrationDlg::addManualAlignedPoint() {
  ccAskThreeDoubleValuesDlg ptsDlg("x", "y", "z", -1.0e12, 1.0e12, s_last_ax,
                                   s_last_ay, s_last_az, 8, "Add aligned point",
                                   this);

  // if the aligned entity is shifted, the user has the choice to input virtual
  // point either in the original coordinate system or the shifted one
...
bool ccPointPairRegistrationDlg::convertToSphereCenter(
  ...
  if (!entity || !useSphereToolButton->isChecked() ||
      !entity->isKindOf(
          CC_TYPES::POINT_CLOUD)) // only works with cloud right now
  {
    // nothing to do
    return true;
  }
  ...
  if (part && part->size() > 16) {
    ...
    if (CCCoreLib::GeometricalAnalysisTools::DetectSphereRobust(
      ...
      if (radius / searchRadius < 0.5 || radius / searchRadius > 2.0) {
        ccLog::Warning(QString("[ccPointPairRegistrationDlg] Detected sphere "
        ...
        // now look again (more precisely)
        {
          ...
          box.add(C - CCVector3(1, 1, 1) * radius *
                          static_cast<PointCoordinateType>(1.05)); // add 5%
          box.add(C + CCVector3(1, 1, 1) * radius *
                          static_cast<PointCoordinateType>(1.05)); // add 5%
          part = cloud->crop(box, true);
          if (part && part->size() > 16)
            CCCoreLib::GeometricalAnalysisTools::DetectSphereRobust(
                part, 0.5, C, radius, rms, &pDlg, 0.99);
        }
        ccLog::Print(QString("[ccPointPairRegistrationDlg] Detected sphere "
                             "radius = %1 (rms = %2)")
                         .arg(radius)
                         .arg(rms));
        if (radius / searchRadius < 0.5 || radius / searchRadius > 2.0) {
          ccLog::Warning("[ccPointPairRegistrationDlg] Sphere radius is too "
      ...
      ccLog::Warning("[ccPointPairRegistrationDlg] Failed to fit a sphere "
                     "around the picked point!");
    }
  } else {
    // not enough memory? No points inside the
    ccLog::Warning("[ccPointPairRegistrationDlg] Failed to crop points around "
                   "the picked point?!");
  }
...
static cc2DLabel *CreateLabel(cc2DLabel *label, ccPointCloud *cloud,
                              unsigned pointIndex, QString pointName,
...
static cc2DLabel *CreateLabel(ccPointCloud *cloud, unsigned pointIndex,
                              QString pointName,
                              ccGenericGLDisplay *display = nullptr) {
...
bool ccPointPairRegistrationDlg::addAlignedPoint(CCVector3d &Pin,
  ...
  for (unsigned i = 0; i < m_alignedPoints.size(); ++i) {
    ...
    if (CCCoreLib::LessThanSquareEpsilon((Pi - Pin).norm2d())) {
      ccLog::Error(
          "Point already picked or too close to an already selected one!");
      return false;
    }
  }
...
void ccPointPairRegistrationDlg::removeAlignedPoint(
  ...
  if (index >= static_cast<int>(m_alignedPoints.size())) {
    ccLog::Error(
        "[ccPointPairRegistrationDlg::removeAlignedPoint] Invalid index!");
    assert(false);
    return;
  }
    ...
    *const_cast<CCVector3 *>(m_alignedPoints.getPoint(i - 1)) =
    ...
    if (child) {
      if (child->isKindOf(CC_TYPES::LABEL_2D)) {
        ...
        label->clear();
        CreateLabel(label, &m_alignedPoints, static_cast<unsigned>(i - 1),
                    pointName, m_associatedWin);
  ...
  if (autoRemoveDualPoint && index < static_cast<int>(m_refPoints.size()) &&
      QMessageBox::question(
          nullptr, tr("Remove dual point"),
          tr("Remove the equivalent reference point as well?"),
          QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes) {
    removeRefPoint(index, false);
  }
}

bool ccPointPairRegistrationDlg::addReferencePoint(
    CCVector3d &Pin, ccHObject *entity /*=nullptr*/, bool shifted /*=true*/) {
    ...
    } else // virtual point
    {
      ...
      if (!shifted) {
        ...
        if (ccGlobalShiftManager::Handle(
                Pin, 0.0, ccGlobalShiftManager::NO_DIALOG_AUTO_SHIFT,
  ...
  for (unsigned i = 0; i < m_refPoints.size(); ++i) {
    ...
    CCVector3d Pi =
        m_refPoints.toGlobal3d<PointCoordinateType>(*m_refPoints.getPoint(i));
    if (CCCoreLib::LessThanSquareEpsilon((Pi - Pin).norm2d())) {
      ccLog::Error(
          "Point already picked or too close to an already selected one!");
      return false;
  ...
  if (sphereRadius <= 0) {
    ...
    cc2DLabel *label =
        CreateLabel(&m_refPoints, newPointIndex, pointName, m_associatedWin);
    if (originLabel) {
      // remember the associated label
      label->setMetaData("AssociatedLabelID", originLabel->getUniqueID());
    }
...
void ccPointPairRegistrationDlg::removeRefPoint(
  ...
  // auto-remove the other point?
  if (autoRemoveDualPoint && index < static_cast<int>(m_alignedPoints.size()) &&
      QMessageBox::question(
          nullptr, "Remove dual point",
          "Remove the equivalent to-be-aligned point as well?",
          QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes) {
...
static bool
UmeyamaRegistration(CCCoreLib::GenericCloud *toBeAlignedPoints,
                    CCCoreLib::GenericCloud *referencePoints,
                    CCCoreLib::PointProjectionTools::Transformation &trans,
                    bool fixedScale) {
#ifdef CC_USES_EIGEN
  // output transformation (R is invalid on initialization, T is (0,0,0) and
  // s==1)
  trans.R.invalidate();
  trans.T = CCVector3d(0, 0, 0);
  trans.s = 1.0;

  if (toBeAlignedPoints == nullptr || referencePoints == nullptr ||
  ...
  Eigen::MatrixXd resultUmeyama =
      Eigen::umeyama(srcMatrix, destMatrix, !fixedScale);

  // scale
  if (!fixedScale) {
    // calculate the scale
    trans.s = CCVector3d(resultUmeyama(0, 0), resultUmeyama(1, 0),
                         resultUmeyama(2, 0))
                  .normd();
    if (trans.s == 0) {
      assert(false);
...
bool ccPointPairRegistrationDlg::callRegistration(
    CCCoreLib::PointProjectionTools::Transformation &trans, double &rms,
    bool autoUpdateTab, bool &withUmeyama, QStringList *report /*=nullptr*/) {
  withUmeyama = false;
  ...
  if (m_alignedPoints.size() != m_refPoints.size() ||
      m_refPoints.size() < MIN_PAIRS_COUNT) {
    assert(false);
    ccLog::Error(QString("Need at least %1 points for each entity (and the "
  ...
  //! Calls Horn or Umeyama registration procedure (depending on which one is
  //! available)
  bool success = false;
#ifdef CC_USES_EIGEN
  ...
  success =
      UmeyamaRegistration(&m_alignedPoints, &m_refPoints, trans, !adjustScale);
#else
  success = CCCoreLib::HornRegistrationTools::FindAbsoluteOrientation(
  ...
  // compute RMS
  rms = CCCoreLib::HornRegistrationTools::ComputeRMS(&m_alignedPoints,
                                                     &m_refPoints, trans);
  ...
  if (autoUpdateTab || report) {
    ...
    if (rms >= 0) {
      ...
      if (report) {
        ...
        report->append(
            "; Ref.name; Ref.x; Ref.y; Ref.z; Aligned.name; Aligned.x; "
            "Aligned.y; Aligned.z; Delta X; Delta Y; Delta Z; Distance");
      }
      ...
      for (unsigned i = 0; i < m_alignedPoints.size(); ++i) {
        ...
        if (report) {
          ...
          QString alignedName;
          auto *alignedHeader =
              alignedPointsTableWidget->verticalHeaderItem(static_cast<int>(i));
          if (alignedHeader) {
          ...
          QString refName;
          auto *refHeader =
              refPointsTableWidget->verticalHeaderItem(static_cast<int>(i));
          if (refHeader) {
          ...
          QString reportLine = QString(";%1; %2; %3; %4")
                                   .arg(refName)
                                   .arg(Ri->x)
                                   .arg(Ri->y)
                                   .arg(Ri->z);

          reportLine += QString("; %1; %2; %3; %4")
                            .arg(alignedName)
                            .arg(Lit.x)
                            .arg(Lit.y)
                            .arg(Lit.z);
          ...
          for (unsigned coordIndex = 0; coordIndex < 3; ++coordIndex) {
            reportLine +=
                QString("; %1").arg(Lit[coordIndex] - Rid[coordIndex]);
          }
        ...
        if (report) {
          QString registrationMethod =
              tr("Registration method: ") + (withUmeyama ? "Umeyama" : "Horn");
        ...
        if (autoUpdateTab) {
          ...
          for (unsigned j = 0; j < 3; ++j) {
            ...
            alignedPointsTableWidget->setItem(i, RMS_COL_INDEX - 3 + j,
                                              itemAAlongAxis);

            QTableWidgetItem *itemRAlongAxis = new QTableWidgetItem();
            itemRAlongAxis->setData(Qt::EditRole, diffAlongAxis);
            refPointsTableWidget->setItem(i, RMS_COL_INDEX - 3 + j,
                                          itemRAlongAxis);
          }
        }
      }
...
void ccPointPairRegistrationDlg::resetTitle() {
  if (m_associatedWin) {
    m_associatedWin->displayNewMessage(
        QString(), ccGLWindowInterface::UPPER_CENTER_MESSAGE, false);
    m_associatedWin->displayNewMessage(
        "[Point-pair registration]", ccGLWindowInterface::UPPER_CENTER_MESSAGE,
        true, 3600);
  }
}
...
void ccPointPairRegistrationDlg::updateAlignInfo() {
  ...
  if (m_alignedPoints.size() == m_refPoints.size() &&
    ...
    if (m_associatedWin) {
      m_associatedWin->displayNewMessage(
          rmsString, ccGLWindowInterface::UPPER_CENTER_MESSAGE, true, 60 * 60);
    }
...
void ccPointPairRegistrationDlg::align() {
  ...
  if (callRegistration(trans, rms, true, withUmeyama)) {
    if (rms >= 0) {
      ...
      ccLog::Print(QString("[PointPairRegistration][%1] ")
                       .arg(withUmeyama ? "Umeyama" : "Horn") +
                   rmsString);
      m_associatedWin->displayNewMessage(
          rmsString, ccGLWindowInterface::UPPER_CENTER_MESSAGE, true, 60 * 60);
    } else {
    ...
    for (unsigned i = 0; i < m_alignedPoints.getChildrenNumber(); ++i) {
      ...
      if (child->isA(CC_TYPES::LABEL_2D)) {
        static_cast<cc2DLabel *>(child)->setRelativeMarkerScale(
            static_cast<float>(1.0 / trans.s));
      }
    }
...
void ccPointPairRegistrationDlg::apply() {
  ...
  if (callRegistration(trans, rms, false, withUmeyama, &report)) {
    ...
    if (!m_referenceEntities.isEmpty()) {
      ...
      transMatD.setTranslation(transMatD.getTranslationAsVec3D() -
                               referenceShift);
      ccLog::Print(
          "[PointPairRegistration] Transformation to global coordinates:");
      ...
      if (ccGlobalShiftManager::Handle(
              Pin, 0.0, ccGlobalShiftManager::ALWAYS_DISPLAY_DIALOG, true,
              Pshift, nullptr, &scale)) {
    ...
    for (auto it = m_alignedEntities.begin(); it != m_alignedEntities.end();
      ...
      if (alignedCloud) {
        if (referenceIsShifted) {
          ...
          ccLog::Warning(
              tr("[PointPairRegistration] Cloud %1: global shift has been "
                 "updated to match the reference: (%1,%2,%3) [x%4]")
                  ...
                  .arg(referenceScale));
        } else if (alignedCloud->isShifted()) // the aligned cloud is shifted,
                                              // but not the reference cloud
        {
          // we'll ask the user confirmation before dropping the shift
          // information on the aligned cloud
          if (!alwaysDropShiftQuestionAsked) {
            alwaysDropShift =
                (QMessageBox::question(
                     this, tr("Drop shift information?"),
                     tr("To-be-aligned cloud is shifted but reference cloud is "
                        "not: drop global shift information?"),
                     QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes);
          ...
          if (alwaysDropShift) {
            ...
            ccLog::Warning(tr("[PointPairRegistration] Cloud %1: global shift "
                              "has been reset to match the reference!")
                               .arg(alignedCloud->getName()));
          }
        }
      }
    ...
    ccLog::Warning(
        QString("[PointPairRegistration] Failed to register entities?!"));
  }

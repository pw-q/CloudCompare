...
ccTracePolylineTool::SegmentGLParams::SegmentGLParams(
    ccGenericGLDisplay *display, int x, int y) {
  if (display) {
...
ccTracePolylineTool::ccTracePolylineTool(ccPickingHub *pickingHub,
                                         QWidget *parent)
    : ccOverlayDialog(parent), m_polyTip(nullptr), m_polyTipVertices(nullptr),
      m_poly3D(nullptr), m_poly3DVertices(nullptr), m_done(false),
      m_pickingHub(pickingHub), m_ui(new Ui::TracePolyLineDlg) {
  assert(pickingHub);

  m_ui->setupUi(this);
  setWindowFlags(Qt::FramelessWindowHint | Qt::Tool);

  connect(m_ui->saveToolButton, &QToolButton::clicked, this,
          &ccTracePolylineTool::exportLine);
  connect(m_ui->resetToolButton, &QToolButton::clicked, this,
          &ccTracePolylineTool::resetLine);
  connect(m_ui->continueToolButton, &QToolButton::clicked, this,
          &ccTracePolylineTool::continueEdition);
  connect(m_ui->validButton, &QToolButton::clicked, this,
          &ccTracePolylineTool::apply);
  connect(m_ui->cancelButton, &QToolButton::clicked, this,
          &ccTracePolylineTool::cancel);
  connect(m_ui->widthSpinBox, qOverload<int>(&QSpinBox::valueChanged), this,
          &ccTracePolylineTool::onWidthSizeChanged);
  connect(m_ui->comboType, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &ccTracePolylineTool::slotTypeChanged);
  ...
  m_polyTipVertices = new ccPointCloud(
      "Tip vertices",
      static_cast<unsigned>(
          ReservedIDs::TRACE_POLYLINE_TOOL_POLYLINE_TIP_VERTICES));
  ...
  m_polyTip = new ccPolyline(
      m_polyTipVertices,
      static_cast<unsigned>(ReservedIDs::TRACE_POLYLINE_TOOL_POLYLINE_TIP));
  ...
  m_polyTip->setWidth(
      m_ui->widthSpinBox->value() < 2
          ? 0
          : m_ui->widthSpinBox
                ->value()); //'1' is equivalent to the default line size
...
ccPolyline *ccTracePolylineTool::polylineOverSampling(unsigned steps) const {
  ...
  if (m_associatedWin) {
    m_associatedWin->getSceneDB()->filterChildren(
        clouds, true, CC_TYPES::POINT_CLOUD, false, m_associatedWin);
    m_associatedWin->getSceneDB()->filterChildren(meshes, true, CC_TYPES::MESH,
                                                  false, m_associatedWin);
  }
  ...
  if (!newVertices->reserve(endSize) || !newPoly->reserve(endSize)) {
    ccLog::Warning(
        "[ccTracePolylineTool::PolylineOverSampling] Not enough memory");
    delete newPoly;
    return nullptr;
  }
  ...
  ccProgressDialog pDlg(true, m_associatedWin ? m_associatedWin->asWidget()
                                              : nullptr);
  ...
  for (unsigned i = 0; i < segmentCount; ++i) {
    ...
    {
      // we actually retro-project the 3D point in the second vertex camera
      // frame so as to get a proper behavior
      CCVector3d P2D;
    ...
    for (unsigned j = 1; j < steps; j++) {
      ...
      for (size_t c = 0; c < clouds.size(); ++c) {
        ...
        if (cloud->pointPicking(vj, m_segmentParams[i2].params,
                                ...
                                m_ui->snapSizeSpinBox->value(), true)) {
...
bool ccTracePolylineTool::linkWith(ccGLWindowInterface *win) {
  ...
  if (m_associatedWin) {
    ...
    connect(m_associatedWin->signalEmitter(),
            ...
            &ccTracePolylineTool::updatePolyLineTip);
  }
...
void ccTracePolylineTool::stop(bool accepted) {
  ...
  if (m_associatedWin) {
    m_associatedWin->displayNewMessage(
        "Polyline tracing [OFF]", ccGLWindowInterface::UPPER_CENTER_MESSAGE,
        false, 2, ccGLWindowInterface::MANUAL_SEGMENTATION_MESSAGE);
...
void ccTracePolylineTool::updatePolyLineTip(int x, int y,
  ...
  if (m_ui->comboType->currentIndex() == 0) {
    ...
    // we also update the first vertex position!
    {
      ...
      *firstP = CCVector3(
          static_cast<PointCoordinateType>(
              A2D.x - camera.viewport[2] /
                          2), // we convert A2D to centered coordinates (no need
                              // to apply high DPI scale or anything!)
          static_cast<PointCoordinateType>(A2D.y - camera.viewport[3] / 2), 0);
    }
  } else if (m_ui->comboType->currentIndex() == 1) {
    ...
    // we also update the first vertex position!
    {
      const CCVector3 *P3D_first =
          m_poly3DVertices->getPoint(m_poly3DVertices->size() - 1);
      const CCVector3 *P3D_last = m_poly3DVertices->getPoint(0);

      ccGLCameraParameters camera;
      m_associatedWin->getGLCameraParameters(camera);

      CCVector3d A2D_first, A2D_last;
      camera.project(*P3D_first, A2D_first);
      camera.project(*P3D_last, A2D_last);
      ...
      *firstP =
          CCVector3(static_cast<PointCoordinateType>(
                        A2D_first.x -
                        camera.viewport[2] /
                            2), // we convert A2D to centered coordinates (no
                                // need to apply high DPI scale or anything!)
                    static_cast<PointCoordinateType>(A2D_first.y -
                                                     camera.viewport[3] / 2),
                    0);
      CCVector3 *lastP =
          const_cast<CCVector3 *>(m_polyTipVertices->getPointPersistentPtr(2));
      *lastP = CCVector3(
          static_cast<PointCoordinateType>(
              A2D_last.x - camera.viewport[2] /
                               2), // we convert A2D to centered coordinates (no
                                   // need to apply high DPI scale or anything!)
          static_cast<PointCoordinateType>(A2D_last.y - camera.viewport[3] / 2),
          0);
    }
  }
...
void ccTracePolylineTool::initTip() {
  if (m_ui->comboType->currentIndex() == 0) {
    m_polyTipVertices->clear();
    m_polyTipVertices->reserve(2);
    m_polyTipVertices->addPoint(CCVector3(0, 0, 0));
    m_polyTipVertices->addPoint(CCVector3(1, 1, 1));
...
void ccTracePolylineTool::onItemPicked(const PickedItem &pi) {
  ...
  if (!m_poly3D || !m_poly3DVertices) {
    m_poly3DVertices = new ccPointCloud(
        "Vertices", static_cast<unsigned>(
                        ReservedIDs::TRACE_POLYLINE_TOOL_POLYLINE_VERTICES));
    m_poly3DVertices->setEnabled(false);
    m_poly3DVertices->setDisplay(m_associatedWin);

    m_poly3D = new ccPolyline(
        m_poly3DVertices,
        static_cast<unsigned>(ReservedIDs::TRACE_POLYLINE_TOOL_POLYLINE));
  ...
  m_segmentParams.emplace_back(m_associatedWin, pi.clickPoint.x(),
                               pi.clickPoint.y());

  // we replace the first point of the tip by this new point
  {
    QPointF pos2D = m_associatedWin->toCenteredGLCoordinates(pi.clickPoint.x(),
                                                             pi.clickPoint.y());
    CCVector3 P2D(static_cast<PointCoordinateType>(pos2D.x()),
                  static_cast<PointCoordinateType>(pos2D.y()), 0);
...
void ccTracePolylineTool::closePolyLine(int, int) {
  if (!m_poly3D || (QApplication::keyboardModifiers() &
                    Qt::ControlModifier)) // CTRL + right click = panning
  {
    return;
  }
  ...
  if (vertCount < 2) {
    ...
    m_poly3D->setClosed(m_ui->comboType->currentIndex() == 1);
    // update the GUI
    ...
    if (m_associatedWin) {
      m_associatedWin->setPickingMode(
          ccGLWindowInterface::NO_PICKING); // no more picking
      m_done = true;
      m_associatedWin->redraw(true, false);
    }
    m_associatedWin->redraw(false, false);
...
void ccTracePolylineTool::restart(bool reset) {
  ...
  if (m_pickingHub &&
      !m_pickingHub->addListener(
          this, true /*, true, ccGLWindowInterface::POINT_PICKING*/)) {
    ccLog::Error("The picking mechanism is already in use. Close the tool "
                 "using it first.");
  }
...
void ccTracePolylineTool::exportLine() {
  ...
  unsigned overSampling =
      static_cast<unsigned>(m_ui->oversampleSpinBox->value());
  if (overSampling > 1) {

...
public:
  //! Default constructor
  ccVolumeCalcTool(ccGenericPointCloud *cloud1, ccGenericPointCloud *cloud2,
                   ccPolyline *edge = nullptr, QWidget *parent = nullptr);
  ...
  struct ReportInfo {
    ReportInfo()
        : volume(0), addedVolume(0), removedVolume(0), surface(0),
          matchingPrecent(0), ceilNonMatchingPercent(0),
          groundNonMatchingPercent(0), averageNeighborsPerCell(0) {}
  ...
  static bool ComputeVolume(
      ccRasterGrid &grid, ccGenericPointCloud *ground,
      ccGenericPointCloud *ceil, ccPolyline *edge, const ccBBox &gridBox,

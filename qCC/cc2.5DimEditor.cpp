...
cc2Point5DimEditor::cc2Point5DimEditor()
    : m_bbEditorDlg(nullptr), m_glWindow(nullptr), m_rasterCloud(nullptr),
...
QString cc2Point5DimEditor::getGridSizeAsString() const {
  ...
  return QString("%1 x %2 (%3 cells)")
      .arg(gridWidth)
      .arg(gridHeight)
      .arg(QLocale::system().toString(gridWidth * gridHeight));
}
...
ccPointCloud *cc2Point5DimEditor::convertGridToCloud(
    ...
    bool resampleInputCloudZ, ccGenericPointCloud *inputCloud,
  ...
  return m_grid.convertToCloud(
      ...
      projectColors, resampleInputCloudXY, resampleInputCloudZ, inputCloud, Z,
      box, percentileValue, exportToOriginalCS, appendGridSizeToSFNames,
      progressDialog);
}

ccRasterGrid::EmptyCellFillOption
cc2Point5DimEditor::getFillEmptyCellsStrategy(QComboBox *comboBox) const {
  if (!comboBox) {
    assert(false);

...
ccSubsamplingDlg::ccSubsamplingDlg(unsigned maxPointCount,
    ...
    : QDialog(parent, Qt::Tool), m_maxPointCount(maxPointCount),
      m_maxRadius(maxCloudRadius), m_sfModEnabled(false), m_sfMin(0),
      m_sfMax(0), m_ui(new Ui::SubsamplingDialog) {
  m_ui->setupUi(this);
  ...
  connect(m_ui->slider, &QSlider::sliderMoved, this,
          &ccSubsamplingDlg::sliderMoved);
  connect(m_ui->valueDoubleSpinBox,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccSubsamplingDlg::valueChanged);
  connect(m_ui->samplingMethodComboBox,
          qOverload<int>(&QComboBox::currentIndexChanged), this,
          &ccSubsamplingDlg::changeSamplingMethod);

  m_ui->samplingMethodComboBox->setCurrentIndex(SPATIAL);
  sliderMoved(m_ui->slider->sliderPosition());

  // Init the 'last used values' (used when switching from one method to
  // another)
  ...
  m_lastUsedValues[OCTREE] =
      static_cast<double>(CCCoreLib::DgmOctree::MAX_OCTREE_LEVEL);
}

ccSubsamplingDlg::~ccSubsamplingDlg() { delete m_ui; }

CCCoreLib::ReferenceCloud *ccSubsamplingDlg::getSampledCloud(
  ...
  case RANDOM: {
    ...
    return CCCoreLib::CloudSamplingTools::subsampleCloudRandomly(cloud, count,
                                                                 progressCb);
  } break;
  ...
  case RANDOM_PERCENT: {
    ...
    count = static_cast<unsigned>(count *
                                  (m_ui->valueDoubleSpinBox->value() / 100.0));
    return CCCoreLib::CloudSamplingTools::subsampleCloudRandomly(cloud, count,
                                                                 progressCb);
  } break;
  ...
  case SPATIAL: {
    ...
    if (octree) {
      PointCoordinateType minDist =
          static_cast<PointCoordinateType>(m_ui->valueDoubleSpinBox->value());
      CCCoreLib::CloudSamplingTools::SFModulationParams modParams;
      {
        modParams.enabled =
            m_ui->sfGroupBox->isEnabled() && m_ui->sfGroupBox->isChecked();
      }
      ...
      return CCCoreLib::CloudSamplingTools::resampleCloudSpatially(
          cloud, minDist, modParams, octree.data(), progressCb);
    } else {
      ccLog::Warning(QString("[ccSubsamplingDlg::getSampledCloud] Failed to "
  ...
  case OCTREE: {
    ...
    if (octree) {
      assert(m_ui->valueDoubleSpinBox->value() >= 0);
      unsigned char level =
          static_cast<unsigned char>(m_ui->valueDoubleSpinBox->value());
      assert(level <= CCCoreLib::DgmOctree::MAX_OCTREE_LEVEL);
      return CCCoreLib::CloudSamplingTools::subsampleCloudWithOctreeAtLevel(
          cloud, level,
...
void ccSubsamplingDlg::sliderMoved(int sliderPos) {
  double sliderRange =
      static_cast<double>(m_ui->slider->maximum() - m_ui->slider->minimum());
  ...
  double valueRange = static_cast<double>(m_ui->valueDoubleSpinBox->maximum() -
                                          m_ui->valueDoubleSpinBox->minimum());
...
void ccSubsamplingDlg::valueChanged(double value) {
  ...
  double sliderRange =
      static_cast<double>(m_ui->slider->maximum() - m_ui->slider->minimum());
  int newSliderPos =
      m_ui->slider->minimum() + static_cast<int>(rate * sliderRange);
...
void ccSubsamplingDlg::changeSamplingMethod(int index) {
  ...
  case SPATIAL: {
    ...
    m_ui->valueDoubleSpinBox->setDisabled(m_ui->sfGroupBox->isEnabled() &&
                                          m_ui->sfGroupBox->isChecked());
    m_ui->valueDoubleSpinBox->setSuffix(QString());
  } break;
  case OCTREE: {
    m_ui->valueDoubleSpinBox->setDecimals(0);
    m_ui->valueDoubleSpinBox->setMinimum(1.0);
    m_ui->valueDoubleSpinBox->setMaximum(
        static_cast<double>(CCCoreLib::DgmOctree::MAX_OCTREE_LEVEL));
    m_ui->valueDoubleSpinBox->setSingleStep(1.0);
    m_ui->valueDoubleSpinBox->setEnabled(true);
    m_ui->valueDoubleSpinBox->setSuffix(QString());
...
void ccSubsamplingDlg::enableSFModulation(ScalarType sfMin, ScalarType sfMax) {
  m_sfModEnabled = CCCoreLib::ScalarField::ValidValue(sfMin) &&
                   CCCoreLib::ScalarField::ValidValue(sfMax);
  if (!m_sfModEnabled) {
    ccLog::Warning(
        "[ccSubsamplingDlg::enableSFModulation] Invalid input SF values");
    return;
  }

  if (sfMin == sfMax) {
    m_sfModEnabled = false;
    ccLog::Warning("[ccSubsamplingDlg::enableSFModulation] Can't modulate "
                   "sampling if all scalar values are the same");
    return;
  }
  ...
  m_ui->sfGroupBox->setEnabled(m_ui->samplingMethodComboBox->currentIndex() ==
                               SPATIAL);
...
void ccSubsamplingDlg::loadFromPersistentSettings() {
  ...
  {
    int methodIndex =
        settings.value("method", m_ui->samplingMethodComboBox->currentIndex())
            .toInt();
    double value =
        settings.value("value", m_ui->valueDoubleSpinBox->value()).toDouble();
    bool useActiveSF =
        settings.value("useActiveSF", m_ui->sfGroupBox->isChecked()).toBool();
    double minSFRatio =
        settings.value("minSFRatio", m_ui->minSFSpacingDoubleSpinBox->value())
            .toDouble();
    double maxSFRatio =
        settings.value("maxSFRatio", m_ui->maxSFSpacingDoubleSpinBox->value())
            .toDouble();

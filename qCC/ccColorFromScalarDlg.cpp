...
ccColorFromScalarDlg::ccColorFromScalarDlg(QWidget *parent,
    ...
    : QDialog(parent, Qt::Tool), m_cloud(pointCloud), m_systemInvalid(false),
  ...
  for (unsigned i = 0; i < c_channelCount; i++) {
    m_colors[i] =
        ccColorScale::Shared(new ccColorScale(QString::asprintf("%d", i)));
  }
             ...
             // replicate
    {
      ...
      for (unsigned i = 0; i < c_channelCount; i++) {
        ...
        for (unsigned int s = 0; s < m_cloud->getNumberOfScalarFields(); s++) {
          m_combos[i]->addItem(
              QString::fromStdString(m_cloud->getScalarFieldName(s)));
        }
      ...
      sf->setColorScale(
          m_colors[c_channelCount - 1]); // set grey colour ramp to start with
  ...
  connect(m_ui->channelComboR, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &ccColorFromScalarDlg::onChannelChangedR);
  connect(m_ui->channelComboG, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &ccColorFromScalarDlg::onChannelChangedG);
  connect(m_ui->channelComboB, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &ccColorFromScalarDlg::onChannelChangedB);
  connect(m_ui->channelComboA, qOverload<int>(&QComboBox::currentIndexChanged),
  ...
  connect(m_ui->minInputSpinBoxR,
          ...
          &ccColorFromScalarDlg::minSpinChangedR);
  connect(m_ui->maxInputSpinBoxR,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccColorFromScalarDlg::maxSpinChangedR);
  connect(m_ui->minInputSpinBoxG,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccColorFromScalarDlg::minSpinChangedG);
  connect(m_ui->maxInputSpinBoxG,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccColorFromScalarDlg::maxSpinChangedG);
  connect(m_ui->minInputSpinBoxB,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccColorFromScalarDlg::minSpinChangedB);
  connect(m_ui->maxInputSpinBoxB,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccColorFromScalarDlg::maxSpinChangedB);
  connect(m_ui->minInputSpinBoxA,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccColorFromScalarDlg::minSpinChangedA);
  connect(m_ui->maxInputSpinBoxA,
          qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &ccColorFromScalarDlg::maxSpinChangedA);
...
void ccColorFromScalarDlg::updateColormaps() {
  if (!m_systemInvalid) {
    ...
    if (m_ui->toggleRGB->isChecked()) {
      ...
      Qt::GlobalColor start_colors[c_channelCount] = {Qt::black, Qt::black,
                                                      Qt::black, Qt::black};
    ...
    } else // create colourmaps for HSV
    {
      ...
      // populate colour ramps
      Qt::GlobalColor start_colors[c_channelCount] = {Qt::black, Qt::gray,
                                                      Qt::black, Qt::black};
      Qt::GlobalColor end_colors[c_channelCount] = {Qt::red, Qt::green,
                                                    Qt::white, Qt::white};
      ...
      for (unsigned i = 0; i < 360; i += 2) {
        ...
        // add stop
        m_colors[0]->insert(ccColorScaleElement(
            static_cast<double>(i) / 360.0, QColor(col.r, col.g, col.b, 255)));
      }
      m_colors[0]->update();
    }
  }
...
void ccColorFromScalarDlg::updateHistogram(int n) {
  if (!m_systemInvalid) {
    if (n >= c_channelCount || n < 0) {
      ccLog::Error("[ccColorFromScalarDlg] updateHistogram called with an "
    ...
    if (fixed[n]) // this channel is/has been fixed
    {
      ...
      m_histograms[n]->setSFInteractionMode(
          ccHistogramWindow::SFInteractionMode::None); // disable interactivity
      m_histograms[n]->setAxisLabels("", "");
      m_histograms[n]->setAxisDisplayOption(
          ccHistogramWindow::AxisDisplayOption::None); // only display XAxis
      ...
      m_histograms[n]->setAxisLabels("", "");
      m_histograms[n]->setAxisDisplayOption(
          ccHistogramWindow::AxisDisplayOption::XAxis); // only display XAxis
      m_histograms[n]->refresh();
    }
...
void ccColorFromScalarDlg::updateSpinBoxLimits(int n) {
  if (!m_systemInvalid) {
    if (n >= c_channelCount || n < 0) {
      ccLog::Error("[ccColorFromScalarDlg] updateSpinBoxLimits called with an "
                   "invalid channel index");
      return;
    }
    bool fixed[c_channelCount] = {
...
void ccColorFromScalarDlg::updateChannel(int n) {
  if (!m_systemInvalid) {
    if (n >= c_channelCount || n < 0) {
      ccLog::Error("[ccColorFromScalarDlg] updateChannel called with an "
                   "invalid channel index");
      return;
    }
...
void ccColorFromScalarDlg::setDefaultSatValuePerChannel(int n) {
  if (!m_systemInvalid) {
    if (n >= c_channelCount || n < 0) {
      ccLog::Error("[ccColorFromScalarDlg] setDefaultSatValuePerChannel called "
                   "with an invalid channel index");
      return;
    }
...
void ccColorFromScalarDlg::minChanged(int n, double val, bool slider) {
  if (n >= c_channelCount || n < 0) {
    ccLog::Error("[ccColorFromScalarDlg] minChanged called with an invalid "
                 "channel index");
    return;
  }
  if (val <= m_maxSat[n]) // valid value?
  {
    ...
    // ccLog::Warning(QString("minChanged Channel %1 failed New min > max, val =
    // %2, maxval = %3").arg(n).arg(val).arg(m_maxSat[n]));
  }
}

void ccColorFromScalarDlg::maxChanged(int n, double val, bool slider) {
  if (n >= c_channelCount || n < 0) {
    ccLog::Error("[ccColorFromScalarDlg] maxChanged called with an invalid "
                 "channel index");
    return;
  }
  if (val >= m_minSat[n]) // valid value?
  {
    ...
    // ccLog::Warning(QString("maxChanged Channel %1 failed New max < min, val =
    // %2, minval = %3").arg(n).arg(val).arg(m_minSat[n]));
  }
}
...
void ccColorFromScalarDlg::onApply() {
  if (!m_systemInvalid) {
    ...
    // and which are fixed?
    bool fixed[c_channelCount]{m_ui->fixR->isChecked(), m_ui->fixG->isChecked(),
                               m_ui->fixB->isChecked(),
                               m_ui->fixA->isChecked()};
    ...
    if (m_ui->toggleRGB->isChecked()) {
      ...
      for (unsigned p = 0; p < m_cloud->size(); p++) {
        ...
        for (unsigned i = 0; i < c_channelCount; i++) {
          ...
          } else // map from scalar
          {
            col[i] = static_cast<int>(
                255.0 * (m_scalars[i]->getValue(p) - m_minSat[i]) /
        ...
        m_cloud->setPointColor(
            p, ccColor::FromQColora(QColor(col[0], col[1], col[2], col[3])));
      }
      ...
      for (unsigned p = 0; p < m_cloud->size(); p++) {
        ...
        for (unsigned i = 0; i < c_channelCount; i++) {
          if (fixed[i]) // fixed value
          {
            col[i] =
                m_boxes_min[i]->value() /
                m_boxes_min[i]->maximum(); // n.b. most 'fixed' boxes between 0
                                           // - 255, but hue between 0 and 360.
          } else                           // map from scalar
          {
        ...
        ccColor::Rgb rgb =
            ccColor::Convert::hsv2rgb(col[0] * 360.0f, col[1], col[2]);
        m_cloud->setPointColor(
            p, ccColor::Rgba(rgb, static_cast<int>(col[3] * ccColor::MAX)));
      }
    }

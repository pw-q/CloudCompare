...
MainWindow::MainWindow()
    : m_UI(new Ui::MainWindow), m_ccRoot(nullptr), m_uiFrozen(false),
      m_recentFiles(new ccRecentFiles(this)), m_3DMouseManager(nullptr),
      m_gamepadManager(nullptr), m_viewModePopupButton(nullptr),
      m_pivotVisibilityPopupButton(nullptr), m_firstShow(true),
      m_pickingHub(nullptr), m_cpeDlg(nullptr), m_gsTool(nullptr),
      m_tplTool(nullptr), m_seTool(nullptr), m_transTool(nullptr),
      m_clipTool(nullptr), m_compDlg(nullptr), m_ppDlg(nullptr),
  ...
  ccTranslationManager::Get().populateMenu(m_UI->menuLanguage,
                                           ccApp->translationPath());
  ...
  m_UI->toolBarView->insertWidget(m_UI->actionZoomAndCenter,
...
// pivot center pop-up menu
{
  ...
  m_pivotVisibilityPopupButton->setStatusTip(
      m_pivotVisibilityPopupButton->toolTip());
  m_UI->toolBarView->insertWidget(m_UI->actionZoomAndCenter,
                                  m_pivotVisibilityPopupButton);
...
// db-tree
{
  ...
  connect(m_ccRoot, &ccDBRoot::selectionChanged, this,
          &MainWindow::updateUIWithSelection, Qt::QueuedConnection);
  connect(
      ...
      Qt::QueuedConnection); // we don't call updateUI because there's no need
                             // to update the properties dialog
  connect(
...
// (do that before connecting the actions)
{
  ...
  bool doNotAutoRestoreGeometry =
      settings
          .value(ccPS::DoNotRestoreWindowGeometry(),
...
ccConsole::Print(
    QStringLiteral("[TBB] Using Intel's Threading Building Blocks %1")
...
MainWindow::~MainWindow() {
  ...
  ccConsole::ReleaseInstance(
      false); // if we flush the console, it will try to display the console
              // window while we are destroying everything!
}
...
void MainWindow::initPlugins() {
  ...
  // Set up dynamic menus
  m_UI->menubar->insertMenu(m_UI->menu3DViews->menuAction(),
                            m_pluginUIManager->pluginMenu());
  m_UI->menuDisplay->insertMenu(m_UI->menuActiveScalarField->menuAction(),
                                m_pluginUIManager->shaderAndFilterMenu());
...
void MainWindow::connectActions() {
  ...
  connect(m_UI->actionCloseAll, &QAction::triggered, this,
          &MainWindow::closeAll);
  ...
  connect(m_UI->actionSetSFsAsNormal, &QAction::triggered, this,
          &MainWindow::doActionSetSFsAsNormal);
  ...
  connect(m_UI->actionShiftPointsAlongNormals, &QAction::triggered, this,
          &MainWindow::doActionShiftPointsAlongNormals);
  ...
  //"Edit > Grid" menu
  connect(m_UI->actionDeleteScanGrid, &QAction::triggered, this,
          &MainWindow::doActionDeleteScanGrids);
  ...
  connect(m_UI->actionAddClassificationSF, &QAction::triggered, this,
          &MainWindow::doActionAddClassificationSF);
  ...
  connect(m_UI->actionSplitCloudUsingSF, &QAction::triggered, this,
          &MainWindow::doActionSplitCloudUsingSF);
  ...
  connect(m_UI->actionRasterize, &QAction::triggered, this,
          &MainWindow::doActionRasterize);
  connect(m_UI->actionConvertPolylinesToMesh, &QAction::triggered, this,
          &MainWindow::doConvertPolylinesToMesh);
  // connect(m_UI->actionCreateSurfaceBetweenTwoPolylines, &QAction::triggered,
  // this, &MainWindow::doMeshTwoPolylines); //DGM: already connected to
  ...
  //"Tools > Volume" menu
  connect(m_UI->actionCompute2HalfDimVolume, &QAction::triggered, this,
          &MainWindow::doCompute2HalfDimVolume);
  //"Tools > Statistics" menu
  connect(
      m_UI->actionComputeStatParams2, &QAction::triggered, this,
      &MainWindow::doActionComputeStatParams); // duplicated action --> we
                                               // can't use the same
                                               // otherwise we get an ugly
                                               // console warning on Linux :(
  connect(m_UI->actionStatisticalTest, &QAction::triggered, this,
          &MainWindow::doActionStatisticalTest);
  ...
  connect(m_UI->actionCrossSection, &QAction::triggered, this,
          &MainWindow::activateClippingBoxMode);
  connect(m_UI->actionExtractSections, &QAction::triggered, this,
          &MainWindow::activateSectionExtractionMode);
  //"Tools > Fit" menu
  ...
  connect(m_UI->actionRefresh, &QAction::triggered, this,
          &MainWindow::refreshAll);
  connect(m_UI->actionTestFrameRate, &QAction::triggered, this,
          &MainWindow::testFrameRate);
  connect(m_UI->actionToggleCenteredPerspective, &QAction::triggered, this,
          &MainWindow::toggleActiveWindowCenteredPerspective);
  connect(m_UI->actionToggleViewerBasedPerspective, &QAction::triggered, this,
          &MainWindow::toggleActiveWindowViewerBasedPerspective);
  connect(m_UI->actionShowCursor3DCoordinates, &QAction::toggled, this,
          &MainWindow::toggleActiveWindowShowCursorCoords);
  connect(m_UI->actionLockRotationAxis, &QAction::triggered, this,
          &MainWindow::toggleLockRotationAxis);
  connect(m_UI->actionEnterBubbleViewMode, &QAction::triggered, this,
          &MainWindow::doActionEnableBubbleViewMode);
  connect(m_UI->actionEditCamera, &QAction::triggered, this,
          &MainWindow::doActionEditCamera);
  connect(m_UI->actionAdjustZoom, &QAction::triggered, this,
          &MainWindow::doActionAdjustZoom);
  ...
  connect(m_UI->actionDisplaySettings, &QAction::triggered, this,
          &MainWindow::showDisplaySettings);
  connect(m_UI->actionToggleSunLight, &QAction::triggered, this,
          &MainWindow::toggleActiveWindowSunLight);
  connect(m_UI->actionToggleCustomLight, &QAction::triggered, this,
          &MainWindow::toggleActiveWindowCustomLight);
  connect(m_UI->actionRenderToFile, &QAction::triggered, this,
          &MainWindow::doActionRenderToFile);
  //"Display > Shaders & filters" menu
  connect(m_UI->actionLoadShader, &QAction::triggered, this,
          &MainWindow::doActionLoadShader);
  connect(m_UI->actionDeleteShader, &QAction::triggered, this,
          &MainWindow::doActionDeleteShader);
  ...
  connect(m_UI->actionRestoreWindowOnStartup, &QAction::toggled, this,
          &MainWindow::doActionToggleRestoreWindowOnStartup);
  connect(m_UI->actionResetAllVBOs, &QAction::triggered, this,
          &MainWindow::doActionResetAllVBOs);

  //"3D Views" menu
  connect(m_UI->menu3DViews, &QMenu::aboutToShow, this,
          &MainWindow::update3DViewsMenu);
  ...
  connect(m_UI->actionClose3DView, &QAction::triggered, m_mdiArea,
          &QMdiArea::closeActiveSubWindow);
  connect(m_UI->actionCloseAll3DViews, &QAction::triggered, m_mdiArea,
          &QMdiArea::closeAllSubWindows);
  connect(m_UI->actionTile3DViews, &QAction::triggered, m_mdiArea,
          &QMdiArea::tileSubWindows);
  connect(m_UI->actionCascade3DViews, &QAction::triggered, m_mdiArea,
          &QMdiArea::cascadeSubWindows);
  connect(m_UI->actionNext3DView, &QAction::triggered, m_mdiArea,
          &QMdiArea::activateNextSubWindow);
  connect(m_UI->actionPrevious3DView, &QAction::triggered, m_mdiArea,
          &QMdiArea::activatePreviousSubWindow);

  //"About" menu entry
  connect(m_UI->actionHelp, &QAction::triggered, this,
          &MainWindow::doActionShowHelpDialog);
  ...
  connect(m_UI->actionPickRotationCenter, &QAction::triggered, this,
          &MainWindow::doPickRotationCenter);
  connect(m_UI->actionZoomAndCenter, &QAction::triggered, this,
          &MainWindow::zoomOnSelectedEntities);
  connect(m_UI->actionSetPivotAlwaysOn, &QAction::triggered, this,
          &MainWindow::setPivotAlwaysOn);
  connect(m_UI->actionSetPivotRotationOnly, &QAction::triggered, this,
          &MainWindow::setPivotRotationOnly);
  connect(m_UI->actionSetPivotOff, &QAction::triggered, this,
          &MainWindow::setPivotOff);

  connect(m_UI->actionSetOrthoView, &QAction::triggered, this,
          [this]() { setOrthoView(getActiveGLWindow()); });
  connect(m_UI->actionSetCenteredPerspectiveView, &QAction::triggered, this,
          [this]() { setCenteredPerspectiveView(getActiveGLWindow()); });
  connect(m_UI->actionSetViewerPerspectiveView, &QAction::triggered, this,
          [this]() { setViewerPerspectiveView(getActiveGLWindow()); });

  connect(m_UI->actionEnableStereo, &QAction::toggled, this,
          &MainWindow::toggleActiveWindowStereoVision);
  connect(m_UI->actionAutoPickRotationCenter, &QAction::toggled, this,
...
void MainWindow::doActionComputeKdTree() {
  ...
  if (kdtree->build(s_kdTreeMaxErrorPerCell,
                    CCCoreLib::DistanceComputationTools::MAX_DIST_95_PERCENT, 4,
                    1000, &pDlg)) {
    qint64 elapsedTime_ms = eTimer.elapsed();

    ccConsole::Print("[doActionComputeKdTree] Timing: %2.3f s",
...
void MainWindow::doActionResampleWithOctree() {
  ...
  for (ccHObject *entity : getSelectedEntities()) {
    ...
    if (cloud) {
      ...
      if (!octree) {
        ...
        if (!octree) {
          ccConsole::Error(tr("Could not compute octree for cloud '%1'")
                               .arg(cloud->getName()));
          continue;
        }
      }
      ...
      QElapsedTimer eTimer;
      eTimer.start();
      CCCoreLib::GenericIndexedCloud *result =
          CCCoreLib::CloudSamplingTools::resampleCloudWithOctree(
              cloud, aimedPoints,
              CCCoreLib::CloudSamplingTools::CELL_GRAVITY_CENTER, &pDlg,
              octree.data());

      if (result) {
        ccConsole::Print("[ResampleWithOctree] Timing: %3.2f s.",
  ...
  if (errors)
    ccLog::Error(tr("[ResampleWithOctree] Errors occurred during the process, "
...
void MainWindow::applyTransformation(const ccGLMatrixd &mat,
  ...
  // we don't want any entity that would be the children of other selected
  // entities
  ccHObject::Container selectedEntities = getTopLevelSelectedEntities();

  for (ccHObject *entity : selectedEntities) // warning, getSelectedEntites may
                                             // change during this loop!
  {
    ...
    if (applyToGlobal && nullptr != dynamic_cast<ccShiftedObject *>(entity)) {
      ...
      // we compute the impact to the local coordinate system without changing
      // the current Global Shift & Scale parameters (for now) Here is the
      // formula, assuming:
      // - the Global Shift is Ts
      // - the Global scale is Sc
      // - the transformation is (R, T)
      // - the Global point coordinates Pg are derived from the local ones Pl
      // with: Pg = Pl/Sc - Ts Therefore, Pg' = R * Pg + T i.e.       Pg' =
      // R.(Pl/Sc - Ts) + T i.e.       Pg' = (R.Pl)/Sc - R.Ts + T i.e.       Pg'
      // = (R.Pl)/Sc - R.Ts + T + Ts - Ts i.e.       Pg' = (R.Pl + Sc.[Ts - R.Ts
      // + T])/Sc - Ts i.e.       Pl' = Sc.[Ts -R.Ts + T] i.e. the translation
      // of the 'local' coordinate system is: Sc.[T + Ts - R.Ts]
      CCVector3d rotatedGlobalShift = globalShift;
      mat.applyRotation(rotatedGlobalShift);
      CCVector3d localTranslation =
          globalScale *
          (globalShift - rotatedGlobalShift + mat.getTranslationAsVec3D());

      // we switch to a local transformation matrix
      transMat.setTranslation(localTranslation);
    }

    // we don't test primitives (it's always ok while the 'vertices lock' test
    // would fail)
    if (!entity->isKindOf(CC_TYPES::PRIMITIVE)) {
      // specific test for locked vertices
      bool lockedVertices;
      ccGenericPointCloud *cloud =
          ccHObjectCaster::ToGenericPointCloud(entity, &lockedVertices);
      if (cloud) {
        if (lockedVertices) {
          ccUtils::DisplayLockedVerticesWarning(entity->getName(),
                                                haveOneSelection());
          continue;
        }

        // test if the translated cloud coordinates were already "too large"
        //(in which case we won't bother the user about the fact that the
        // transformed cloud coordinates will be too large...)
        ...
        if (!ccGlobalShiftManager::NeedShift(Pl) &&
          ...
          bool needShift = ccGlobalShiftManager::NeedShift(transformedPl) ||
                           ccGlobalShiftManager::NeedRescale(transformedDl);
          if (needShift) {
            // existing shift information
            CCVector3d globalShift = cloud->getGlobalShift();
            double globalScale = cloud->getGlobalScale();

            // we compute the global coordinates and scale of the reference
            // point (= the min corner of the bounding-box)
            ...
            if (needShift) {
              ...
              ccShiftAndScaleCloudDlg sasDlg(transformedPl, transformedDl, Pg,
                                             Dg, this);
              ...
              sasDlg.showKeepGlobalPosCheckbox(
                  false); // we don't want the user to mess with this!
              sasDlg.showPreserveShiftOnSave(true);

              // add "original" entry
              int index = sasDlg.addShiftInfo(ccGlobalShiftManager::ShiftInfo(
                  tr("Original"), globalShift, globalScale));
              // add "previous" entry (if any)
              if (autoApplyPreviousGlobalShiftAndScale) {
                index = sasDlg.addShiftInfo(ccGlobalShiftManager::ShiftInfo(
                    tr("Previous"), previousShift, previousScale));
              }
              // add "last" entries (if any)
              int matchingIndex = -1;
              const auto &previousEntries = ccGlobalShiftManager::GetLast();
              for (const ccGlobalShiftManager::ShiftInfo &shiftInfo :
                   previousEntries) {
                index = sasDlg.addShiftInfo(shiftInfo);

                if (matchingIndex < 0) {
                  if (!ccGlobalShiftManager::NeedShift(Pg + shiftInfo.shift) &&
                      !ccGlobalShiftManager::NeedRescale(Dg *
                                                         shiftInfo.scale)) {
                    matchingIndex = index;
                  }
                }
              ...
              if (matchingIndex < 0) {
                ...
                matchingIndex =
                    sasDlg.addShiftInfo(ccGlobalShiftManager::ShiftInfo(
                        tr("Suggested"), suggestedShift, suggestedScale));
              }
              ...
              if (sasDlg.exec()) {
                ...
                ccLog::Warning(
                    tr("[ApplyTransformation] Process cancelled by user"));
                return;
              } else {
            ...
            if (updateShiftAndscale) {
              ...
              // get the relative modification to existing global shift/scale
              // info
              ...
              if (scaleChange != 1.0 || shiftChange.norm2() != 0) {
                ...
                ccLog::Warning(tr("[ApplyTransformation] Cloud '%1' global "
    ...
    if (applyToGlobal) {
      ccLog::Print(tr("[ApplyTransformation] Transformation matrix applied to "
                      "the local coordinates of %1:")
                       .arg(entity->getName()));
  ...
  ccLog::Print(tr("Hint: you can copy a transformation matrix (CTRL+C) and "
                  "apply it - or its inverse - to another entity with the "
...
void MainWindow::doActionApplyScale() {
  ...
  std::vector<std::pair<ccHObject *, ccGenericPointCloud *>> candidates;
  {
                                               ...
                                               // may change during this loop!
    {
      ...
      if (!cloud || !cloud->isKindOf(CC_TYPES::POINT_CLOUD)) {
        ccLog::Warning(tr("[Apply scale] Entity '%1' can't be scaled this way")
                           .arg(entity->getName()));
        continue;
      }
      if (lockedVertices) {
      ...
      if (testBigCoordinates) {
        ...
        if (!oldCoordsWereTooBig) {
          maxx = std::max(std::abs((bbMin.x - C.x) * scales.x + C.x),
                          std::abs((bbMax.x - C.x) * scales.x + C.x));
          maxy = std::max(std::abs((bbMin.y - C.y) * scales.y + C.y),
                          std::abs((bbMax.y - C.y) * scales.y + C.y));
          maxz = std::max(std::abs((bbMin.z - C.z) * scales.z + C.z),
          ...
          if (newCoordsAreTooBig) {
            if (QMessageBox::question(
                    ...
                    tr("Resutling coordinates will be too big (original "
                       "precision may be lost!). Proceed anyway?"),
  ...
  if (candidates.empty()) {
    ccConsole::Warning(tr("[Apply scale] No eligible entities (point clouds or "
  ...
  // now do the real scaling work
  {
    for (auto &candidate : candidates) {
      ...
      if (rescaleGlobalShift) {
        ...
        cloud->setGlobalShift(CCVector3d(shift.x * scales.x, shift.y * scales.y,
                                         shift.z * scales.z));
      }
...
void MainWindow::doActionEditGlobalShiftAndScale() {
    ...
    for (ccHObject *entity : getSelectedEntities()) {
      ...
      if (lockedVertices) {
        ...
        if (entity->isKindOf(CC_TYPES::MESH)) {
          ...
          vertices = dynamic_cast<ccGenericPointCloud *>(
              static_cast<ccPolyline *>(entity)->getAssociatedCloud());
        }
      ...
      if (shiftedEntities.empty()) {
        ...
        if (uniqueShift) {
          uniqueShift = CCCoreLib::LessThanEpsilon(
              (shifted->getGlobalShift() - shift).norm());
        }
        if (uniqueScale) {
          uniqueScale = CCCoreLib::LessThanEpsilon(
  ...
  int index = sasDlg.addShiftInfo(
      ccGlobalShiftManager::ShiftInfo(tr("Original"), shift, scale));
  ...
  ccLog::Print(tr("[Global Shift/Scale] New shift: (%1, %2, %3)")
                   .arg(shift.x)
                   .arg(shift.y)
                   .arg(shift.z));
    ...
    for (auto &entity : shiftedEntities) {
      ...
      if (preserveGlobalPos) {
        ...
        if (CCCoreLib::GreaterThanEpsilon(T.norm()) ||
          ...
          ccLog::Warning(tr("[Global Shift/Scale] To preserve its original "
                            "position, the entity '%1' has been translated of "
                            "(%2 ; %3 ; %4) and rescaled of a factor %5")
                             .arg(ent->getName())
                             .arg(T.x)
                             .arg(T.y)
                             .arg(T.z)
...
void MainWindow::doComputeBestFitBB() {
  if (QMessageBox::warning(this, tr("This method is for test purpose only"),
                           tr("Cloud(s) are going to be rotated while still "
                              "displayed in their previous position! Proceed?"),
    ...
    if (cloud && cloud->isA(CC_TYPES::POINT_CLOUD)) // TODO
    {
      ...
      if (covMat.isValid()) {
        ...
        if (CCCoreLib::Jacobi<double>::ComputeEigenValuesAndVectors(
          ...
          CCCoreLib::Jacobi<double>::SortEigenValuesAndVectors(eigVectors,
                                                               eigValues);
  ...
  for (ccHObject *entity : getSelectedEntities()) {
    if (entity->isKindOf(CC_TYPES::MESH)) {
      ...
      ccPointCloud *vertices = ccHObjectCaster::ToPointCloud(
          mesh ? mesh->getAssociatedCloud() : nullptr);
      if (mesh && vertices) {
        ...
        if (sfIdx < 0) {
          ...
          if (sfIdx < 0) {
            ccConsole::Warning(
                tr("Not enough memory to flag the vertices of mesh '%1'!")
                    .arg(mesh->getName()));
            errors = true;
            continue;
          }
        ...
        if (CCCoreLib::MeshSamplingTools::flagMeshVerticesByType(mesh, flags,
                                                                 &stats)) {
          ...
          ccConsole::Print(
              tr("[Mesh Quality] Mesh '%1' edges: %2 total (normal: %3 / on "
                 "hole borders: %4 / non-manifold: %5)")
                  .arg(entity->getName())
  ...
  if (success) {
    ...
    ccConsole::Print(
        tr("[Mesh Quality] SF flags: %1 (NORMAL) / %2 (BORDER) / (%3) "
           "NON-MANIFOLD")
            .arg(CCCoreLib::MeshSamplingTools::VERTEX_NORMAL)
            .arg(CCCoreLib::MeshSamplingTools::VERTEX_BORDER)
            .arg(CCCoreLib::MeshSamplingTools::VERTEX_NON_MANIFOLD));
  }
  ...
  for (ccHObject *entity : getSelectedEntities()) {
    if (entity->isKindOf(CC_TYPES::MESH)) {
      ...
      if (mesh) {
        ...
        forceConsoleDisplay();
        ccConsole::Print(tr("[Mesh Volume] Mesh '%1': V=%2 (cube units)")
                             .arg(entity->getName())
                             .arg(V));

        // check that the mesh is closed
        CCCoreLib::MeshSamplingTools::EdgeConnectivityStats stats;
        if (CCCoreLib::MeshSamplingTools::computeMeshEdgesConnectivity(mesh,
                                                                       stats)) {
          if (stats.edgesNotShared != 0) {
            ccConsole::Warning(tr("[Mesh Volume] The above volume might be "
          ...
          ccConsole::Warning(
              tr("[Mesh Volume] The above volume might be invalid (not enough "
                 "memory to check if the mesh is closed)"));
        }
      } else {
        assert(false);
      }
  ...
  for (ccHObject *entity : getSelectedEntities()) {
    if (entity->isKindOf(CC_TYPES::MESH)) {
      ...
      if (mesh) {
        ...
        // we force the console to display itself
        forceConsoleDisplay();
        ccConsole::Print(tr("[Mesh Surface] Mesh '%1': S=%2 (square units)")
                             .arg(entity->getName())
                             .arg(S));
        if (mesh->size()) {
          ccConsole::Print(
              tr("[Mesh Surface] Average triangle surface: %1 (square units)")
...
void MainWindow::doActionComputeDistancesFromSensor() {
  ...
  for (ccHObject *entity : getSelectedEntities()) {
    ...
    ccHObject *defaultCloud =
        sensor->getParent() && sensor->getParent()->isA(CC_TYPES::POINT_CLOUD)
            ? sensor->getParent()
            : nullptr;
    ccPointCloud *cloud = askUserToSelectACloud(
        defaultCloud,
        tr("Select a cloud on which to project the uncertainty:"));
    if (!cloud) {
      return;
    }
    ...
    const char *defaultRangesSFname =
        squared ? CC_DEFAULT_SQUARED_RANGES_SF_NAME : CC_DEFAULT_RANGES_SF_NAME;
    ...
    for (unsigned i = 0; i < cloud->size(); ++i) {
      ...
      ScalarType s = static_cast<ScalarType>(
          squared ? (*P - sensorCenter).norm2() : (*P - sensorCenter).norm());
...
void MainWindow::doActionComputeScatteringAngles() {
  ...
  if (!haveOneSelection() ||
      !m_selectedEntities.front()->isKindOf(CC_TYPES::GBL_SENSOR)) {
  ...
  ccHObject *defaultCloud =
      sensor->getParent() && sensor->getParent()->isA(CC_TYPES::POINT_CLOUD)
          ...
          : nullptr;
  ccPointCloud *cloud = askUserToSelectACloud(
      defaultCloud, tr("Select a cloud on which to project the uncertainty:"));
  if (!cloud) {
    return;
  }
  if (!cloud->hasNormals()) {
...
void MainWindow::doActionSetViewFromSensor() {
  // there should be only one sensor in current selection!
  if (!haveOneSelection() ||
      !m_selectedEntities.front()->isKindOf(CC_TYPES::SENSOR)) {
    ...
    if (entity->isKindOf(CC_TYPES::POINT_CLOUD)) {
      ...
      if (sensor->computeAutoParameters(cloud)) {
        ...
        ccGLWindowInterface *win =
            static_cast<ccGLWindowInterface *>(cloud->getDisplay());
        if (win) {
...
void MainWindow::doActionModifySensor() {
  ...
  if (sensor->isA(CC_TYPES::GBL_SENSOR)) {
    ...
    if (gbl->getParent() && gbl->getParent()->isKindOf(CC_TYPES::POINT_CLOUD)) {
      ...
      // ccConsole::Warning(tr("Internal error: sensor ('%1') parent is not a
      // point cloud!").arg(sensor->getName()));
    }
  }
  // Camera sensors
...
void MainWindow::doActionProjectUncertainty() {
  ...
  if (!haveOneSelection() ||
      !m_selectedEntities.front()->isKindOf(CC_TYPES::CAMERA_SENSOR)) {
    ccConsole::Error(tr("Select one and only one camera (projective) sensor!"));
    return;
  }

  ccCameraSensor *sensor =
      ccHObjectCaster::ToCameraSensor(m_selectedEntities.front());
  if (!sensor) {
    assert(false);
    return;
  }
  ...
  if (!distParams ||
    ...
    ccLog::Error(
        tr("Sensor has no associated uncertainty model! (Brown, etc.)"));
    return;
  }
  ...
  for (unsigned d = 0; d < 3; ++d) {
    // add scalar field
    QString sfName =
        tr("[%1] Uncertainty (%2)").arg(sensor->getName()).arg(dimChar[d]);
...
void MainWindow::doActionCheckPointsInsideFrustum() {
  ...
  ccPointCloud *pointCloud =
      askUserToSelectACloud(defaultCloud, tr("Select a cloud to filter:"));
  if (!pointCloud) {
    return;
  }
...
void MainWindow::doActionShowDepthBuffer() {
  ...
  for (ccHObject *entity : getSelectedEntities()) {
    if (entity->isKindOf(CC_TYPES::GBL_SENSOR)) {
      ccGBLSensor *sensor =
          static_cast<ccGBLSensor *>(m_selectedEntities.front());
      if (sensor->getDepthBuffer().zBuff.empty()) {
        // look for depending cloud
        ccGenericPointCloud *cloud =
            ccHObjectCaster::ToGenericPointCloud(entity->getParent());
        if (cloud) {
          ...
          ccConsole::Error(
              tr("Internal error: sensor ('%1') parent is not a point cloud!")
                  .arg(sensor->getName()));
          return;
        }
      }
...
void MainWindow::doActionExportDepthBuffer() {
  ...
  QString currentPath =
      settings.value(ccPS::CurrentPath(), ccFileUtils::defaultDocPath())
          .toString();

  QString filename = QFileDialog::getSaveFileName(
      this, tr("Select output file"), currentPath,
      DepthMapFileFilter::GetFileFilter(), nullptr, CCFileDialogOptions());
  if (filename.isEmpty()) {
...
void MainWindow::doActionComputePointsVisibility() {
  ...
  ccGBLSensor *sensor =
      ccHObjectCaster::ToGBLSensor(m_selectedEntities.front());
  if (!sensor)
    return;
  ...
  if (sensor->getDepthBuffer().zBuff.empty()) {
    if (defaultCloud) {
      ...
      int errorCode;
      if (sensor->computeDepthBuffer(static_cast<ccPointCloud *>(defaultCloud),
                                     errorCode)) {
        ccRenderingTools::ShowDepthBuffer(sensor, this);
      } else {
        ccConsole::Error(ccGBLSensor::GetErrorString(errorCode));
      ...
      ccConsole::Error(
          tr("Sensor has no depth buffer (and no associated cloud?)"));
      return;
    }
  }
  ...
  if (sf) {
    ...
    if (sf) {
      ...
      ccConsole::Print(
          tr("Visibility computed for cloud '%1'").arg(pointCloud->getName()));
      ccConsole::Print(tr("\tVisible = %1").arg(CCCoreLib::POINT_VISIBLE));
      ccConsole::Print(tr("\tHidden = %1").arg(CCCoreLib::POINT_HIDDEN));
      ccConsole::Print(
          tr("\tOut of range = %1").arg(CCCoreLib::POINT_OUT_OF_RANGE));
      ccConsole::Print(
          tr("\tOut of fov = %1").arg(CCCoreLib::POINT_OUT_OF_FOV));
    }
    pointCloud->redrawDisplay();
  }
...
void MainWindow::doActionSamplePointsOnMesh() {
  ...
  for (ccHObject *entity : getSelectedEntities()) {
    ...
    ccPointCloud *cloud = mesh->samplePoints(
        s_useDensity, s_useDensity ? s_ptsSamplingDensity : s_ptsSamplingCount,
  ...
  if (errors)
    ccLog::Error(tr("[doActionSamplePointsOnMesh] Errors occurred during the "
...
void MainWindow::doActionSamplePointsOnPolyline() {
  ...
  if (errors) {
    ccLog::Error(tr("[DoActionSamplePointsOnPolyline] Errors occurred during "
...
void MainWindow::doActionSmoohPolyline() {
  ...
  for (ccHObject *entity : selectedEntities) {
    ...
    ccPolyline *smoothPoly =
        poly->smoothChaikin(s_ratio, static_cast<unsigned>(s_iterationCount));
    if (smoothPoly) {
      if (poly->getParent()) {
  ...
  if (errors) {
    ccLog::Error(tr("[DoActionSmoohPolyline] Errors occurred during the "
                    "process! Result may be incomplete!"));
  }
...
void MainWindow::doRemoveDuplicatePoints() {
  ...
  double minDistanceBetweenPoints =
      settings.value(ccPS::DuplicatePointsMinDist(), 1.0e-12).toDouble();

  bool ok;
  minDistanceBetweenPoints = QInputDialog::getDouble(
      this, tr("Remove duplicate points"), tr("Min distance between points:"),
      minDistanceBetweenPoints, 0, 1.0e8, 12, &ok);
  if (!ok)
    return;
  ...
  ccHObject::Container selectedEntities =
      getSelectedEntities(); // we have to use a local copy:
                             // 'unselectAllEntities' and 'selectEntity' will
                             // change the set of currently selected entities!

  for (ccHObject *entity : selectedEntities) {
    ccPointCloud *cloud = ccHObjectCaster::ToPointCloud(entity);
    if (cloud) {
      ccPointCloud *filteredCloud =
          cloud->removeDuplicatePoints(minDistanceBetweenPoints, &pDlg);
      if (!filteredCloud) {
  ...
  for (ccHObject *entity : getSelectedEntities()) {
    ...
    if (cloud && cloud->isA(CC_TYPES::POINT_CLOUD)) {
      ...
      if (sf) {
        ...
        ccConsole::Warning(tr("Entity [%1] has no active scalar field!")
                               .arg(entity->getName()));
      }
    }
  }
  ...
  ccHObject::Container results;
  {
    for (auto &item : toFilter) {
      ...
      if (ent->isKindOf(CC_TYPES::MESH)) {
        ...
        if (ent->isA(
                CC_TYPES::
                    MESH) /*|| ent->isKindOf(CC_TYPES::PRIMITIVE)*/) // TODO
          resultInside =
              ccHObjectCaster::ToMesh(ent)->createNewMeshFromSelection(
                  false, nullptr, true);
        else if (ent->isA(CC_TYPES::SUB_MESH))
          resultInside =
              ccHObjectCaster::ToSubMesh(ent)->createNewSubMeshFromSelection(
                  false);

        if (resultInside == ent) {
          // specific case: all triangles were selected, nothing to do
          ccLog::Warning(QString("Mesh %1 is fully inside the specified range")
                             .arg(ent->getName()));
          ...
          if (resultOutside == ent) {
            // specific case: all triangles were selected, nothing to do
            ccLog::Warning(
                QString("Mesh %1 is fully outside the specified range")
                    .arg(ent->getName()));
        ...
        if (resultInside == ent) {
          ...
          ccLog::Warning(QString("Cloud %1 is fully inside the specified range")
                             .arg(ent->getName()));
          resultInside = nullptr;
        } else if (mode == ccFilterByValueDlg::SPLIT) {
          ...
          if (resultOutside == ent) {
            // specific case: all points were selected, nothing to do
            ccLog::Warning(
                QString("Cloud %1 is fully outside the specified range")
                    .arg(ent->getName()));
            ent->setEnabled(false);
            ent->prepareDisplayForRefresh();
...
void MainWindow::doApplyActiveSFAction(int action) {
  ...
  case 0: // Toggle SF color scale
    if (sfIdx >= 0) {
      ...
      ccConsole::Warning(
          tr("No active scalar field on entity '%1'").arg(ent->getName()));
    break;
...
void MainWindow::doActionOpenColorScalesManager() {
  ccColorScaleEditorDialog cseDlg(ccColorScalesManager::GetUniqueInstance(),
                                  this, ccColorScale::Shared(nullptr), this);
...
void MainWindow::doActionSmoothMeshSF() {
  if (!ccEntityAction::processMeshSF(m_selectedEntities, ccMesh::SMOOTH_MESH_SF,
                                     this))
    return;
  ...
  for (ccHObject *entity : getSelectedEntities()) {
    if (entity->isKindOf(CC_TYPES::MESH)) {
      ...
      if (entity->isA(CC_TYPES::MESH)) {
        ...
        try {
          subdividedMesh = mesh->subdivide(
              static_cast<PointCoordinateType>(s_subdivideMaxArea));
        } catch (...) {
          ccLog::Error(tr("[Subdivide] An error occurred while trying to "
                          "subdivide mesh '%1' (not enough memory?)")
                           .arg(mesh->getName()));
        }

        if (subdividedMesh) {
          subdividedMesh->setName(QString("%1.subdivided(S<%2)")
                                      .arg(mesh->getName())
          ...
          ccConsole::Warning(tr("[Subdivide] Failed to subdivide mesh '%1' "
                                "(not enough memory?)")
                                 .arg(mesh->getName()));
        }
...
void MainWindow::doActionSmoothMeshLaplacian() {
  ...
  s_laplacianSmooth_factor =
      QInputDialog::getDouble(this, tr("Smooth mesh"), tr("Smoothing factor:"),
  ...
  for (ccHObject *entity : getSelectedEntities()) {
    if (entity->isA(CC_TYPES::MESH) ||
        entity->isA(CC_TYPES::PRIMITIVE)) // FIXME: can we really do this with
                                          // primitives?
    {
      ccMesh *mesh = ccHObjectCaster::ToMesh(entity);

      if (mesh->laplacianSmooth(
              s_laplacianSmooth_nbIter,
...
void MainWindow::doActionMerge() {
  ...
  try {
    for (ccHObject *entity : getSelectedEntities()) {
      ...
      if (entity->isA(CC_TYPES::POINT_CLOUD)) {
        ...
        // check whether this cloud is an ancestor of the first cloud in the
        // selection
        if (clouds.size() > 1) {
          if (clouds.back()->isAncestorOf(clouds.front())) {
            // this way we are sure that the first cloud is not below any other
            // cloud
        ...
        if (mesh && mesh->getAssociatedCloud() &&
          ...
          ccConsole::Warning(tr("Only meshes with standard vertices are "
                                "handled for now! Can't merge entity '%1'...")
                                 .arg(entity->getName()));
        }
      } else {
        ccConsole::Warning(
            tr("Entity '%1' is neither a cloud nor a mesh, can't merge it!")
                .arg(entity->getName()));
      }
    }
  ...
  if (!clouds.empty()) {
    ...
    for (size_t i = 0; i < clouds.size(); ++i) {
      ...
      if (!firstCloud) {
        // we don't delete the first cloud (we'll merge the other one 'inside'
        // it
        firstCloud = pc;
        // we still have to temporarily detach the first cloud, as it may
        // undergo 'severe' modifications (octree deletion, etc.) --> see
        // ccPointCloud::operator +=
        ...
        if (QMessageBox::question(this, tr("Original cloud index"),
                                  tr("Do you want to generate a scalar field "
                                     "with the original cloud index?")) ==
            QMessageBox::Yes) {
          int sfIdx =
              pc->getScalarFieldIndexByName(CC_ORIGINAL_CLOUD_INDEX_SF_NAME);
          if (sfIdx < 0) {
            sfIdx = pc->addScalarField(CC_ORIGINAL_CLOUD_INDEX_SF_NAME);
          }
          if (sfIdx < 0) {
            ccConsole::Error(
                tr("Couldn't allocate a new scalar field for storing the "
                   "original cloud index! Try to free some memory ..."));
            return;
          } else {
        ...
        firstCloud->append(pc, countBefore, false,
                           false); // append without recalculating SF min/max
        ...
        if (firstCloud->size() == countBefore + countAdded) {
          ...
          // if the entity to remove is inside a group with a unique child, we
          // can remove the group as well
          ccHObject *parent = pc->getParent();
          if (parent && parent->isA(CC_TYPES::HIERARCHY_OBJECT) &&
    ...
    for (ccHObject *toRemove : toBeRemoved) {
      if (firstCloud->isAncestorOf(toRemove)) {
        // we cannot call 'removeElement' on a child of the first cloud, as it's
        // temporarily detached from the DB tree!
  ...
  // merge meshes?
    ...
    // createSubMeshes = (QMessageBox::question(this, tr("Create sub-meshes"),
    // tr("Do you want to create sub-mesh entities corresponding to each source
...
void MainWindow::zoomOn(ccHObject *object) {
  ccGLWindowInterface *win =
      static_cast<ccGLWindowInterface *>(object->getDisplay());
  if (win) {
...
void MainWindow::doActionRegister() {
  if (m_selectedEntities.size() != 2 ||
      (!m_selectedEntities.front()->isKindOf(CC_TYPES::POINT_CLOUD) &&
       !m_selectedEntities.front()->isKindOf(CC_TYPES::MESH)) ||
      (!m_selectedEntities.back()->isKindOf(CC_TYPES::POINT_CLOUD) &&
       !m_selectedEntities.back()->isKindOf(CC_TYPES::MESH))) {
  ...
  if (data->isKindOf(CC_TYPES::MESH) &&
      model->isKindOf(CC_TYPES::POINT_CLOUD)) {
  ...
  if (minRMSDecrease < ccRegistrationDlg::GetAbsoluteMinRMSDecrease()) {
    ...
    ccLog::Error(tr("Minimum RMS decrease value is too small.\n%1 will be used "
                    "instead (numerical accuracy limit).")
  ...
  CCCoreLib::ICPRegistrationTools::Parameters parameters;
  {
    ...
    parameters.useC2MSignedDistances =
        rDlg.useC2MSignedDistances(parameters.robustC2MSignedDistances);
  ...
  if (ccRegistrationTools::ICP(data, model, transMat, finalScale, finalError,
                               finalPointCount, parameters, useDataSFAsWeights,
                               useModelSFAsWeights, this)) {
    QString rmsString = tr("Final RMS*: %1 (computed on %2 points)")
                            .arg(finalError)
                            .arg(finalPointCount);
    QString rmsDisclaimerString = tr(
    ...
    if (parameters.adjustScale) {
      QString scaleString =
          tr("Scale: %1 (already integrated in above matrix!)").arg(finalScale);
    ...
    QString overlapString =
        tr("Theoretical overlap: %1%")
            .arg(static_cast<int>(parameters.finalOverlapRatio * 100));
    ...
    if (data->isKindOf(CC_TYPES::POINT_CLOUD)) {
      ...
      if (pc->isLocked()) {
        ...
        QMessageBox::StandardButton result = QMessageBox::question(
            ...
            tr("Data mesh vertices are locked (they may be shared with other "
               "meshes): Do you wish to clone this mesh to apply "
               "transformation?"),
        ...
        if (result == QMessageBox::Ok) {
          ...
          if (newMesh) {
            ...
            ccLog::Error(
                tr("Failed to clone 'data' mesh! (not enough memory?)"));
          }
        }
      }
    }
    ...
    if (pc) {
      ...
      // check if the 'model' entity is a child of the 'data' one (would be
      // strange, but can still happen!)
      bool modelIsChildOfData = data->isAncestorOf(model);
      if (modelIsChildOfData) {
        if (pc->getChildrenNumber() != 0) {
          ccLog::Warning(
              tr("[ICP] The reference entity is a child of the aligned one! CC "
                 "will move only the aligned entity, and not its children"));
        }
      ...
      if (refPC) {
        if (refPC->isShifted()) {
          ...
          ccLog::Warning(tr("[ICP] Aligned entity global shift has been "
                            "updated to match the reference: (%1,%2,%3) [x%4]")
                             .arg(Pshift.x)
                             .arg(Pshift.y)
                             .arg(Pshift.z)
                             .arg(scale));
        ...
        } else if (pc->isShifted()) // we'll ask the user first before dropping
                                    // the shift information on the aligned
                                    // cloud
        {
          if (QMessageBox::question(
                  this, tr("Drop shift information?"),
                  tr("Aligned entity is shifted but reference cloud is not: "
                     "drop global shift information?"),
                  QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes) {
...
// Aurelien BEY le 13/11/2008 : ajout de la fonction permettant de traiter la
// fonctionnalite de recalage grossier
void MainWindow::doAction4pcsRegister() {
  if (QMessageBox::warning(
          this, tr("Work in progress"),
          tr("This method is still under development: are you sure you want to "
             "use it? (a crash may likely happen)"),
  ...
  ccGenericPointCloud *data =
      ccHObjectCaster::ToGenericPointCloud(m_selectedEntities.back());
  ccGenericPointCloud *model =
      ccHObjectCaster::ToGenericPointCloud(m_selectedEntities.front());
  ...
  unsigned nbMaxCandidates =
      aDlg.isNumberOfCandidatesLimited() ? aDlg.getMaxNumberOfCandidates() : 0;

  ccProgressDialog pDlg(true, this);

  CCCoreLib::PointProjectionTools::Transformation transform;
  if (CCCoreLib::FPCSRegistrationTools::RegisterClouds(
...
void MainWindow::doActionSubsample() {
  ...
  ScalarType sfMax = CCCoreLib::NAN_VALUE;
  {
    for (ccHObject *entity : getSelectedEntities()) {
      if (entity->isA(CC_TYPES::POINT_CLOUD)) {
        ...
        maxCloudRadius =
            std::max<double>(maxCloudRadius, cloud->getOwnBB().getDiagNorm());
  ...
  bool hasValidSF =
      ccScalarField::ValidValue(sfMin) && ccScalarField::ValidValue(sfMax);
  if (hasValidSF) {
  ...
  ccHObject::Container resultingClouds;
  {
    ...
    for (size_t i = 0; i < clouds.size(); ++i) {
      ...
      CCCoreLib::ReferenceCloud *sampledCloud =
          sDlg.getSampledCloud(cloud, &pDlg);
      if (!sampledCloud) {
        ccConsole::Warning(tr("[Subsampling] Failed to subsample cloud '%1'!")
                               .arg(cloud->getName()));
        errors = true;
        continue;
      }
      ...
      if (newPointCloud) {
        ...
        if (warnings) {
          ccLog::Warning(tr("[Subsampling] Not enough memory: colors, normals "
                            "or scalar fields may be missing!"));
          errors = true;
        }
      } else {
        ccLog::Error(tr("Not enough memory!"));
...
void MainWindow::createComponentsClouds(
    ccGenericPointCloud *cloud, CCCoreLib::ReferenceCloudContainer &components,
    unsigned minPointsPerComponent, bool randomColors, bool selectComponents,
  ...
  if (sortBysize) {
    try {
      ...
      ccLog::Warning(tr("[CreateComponentsClouds] Not enough memory to sort "
                        "components by size!"));
      sortBysize = false;
    }
  ...
  {
    ccPointCloud *pc = cloud->isA(CC_TYPES::POINT_CLOUD)
    ...
    for (size_t i = 0; i < components.size(); ++i) {
      CCCoreLib::ReferenceCloud *compIndexes =
      ...
      if (compIndexes->size() >= minPointsPerComponent) {
        ...
        ccPointCloud *compCloud = (pc ? pc->partialClone(compIndexes)
        ...
        if (compCloud) {
          ...
          ccConsole::Warning(tr("[CreateComponentsClouds] Failed to create "
    ...
    if (ccGroup->getChildrenNumber() == 0) {
      ccConsole::Error(
          tr("No component was created! Check the minimum size..."));
      delete ccGroup;
      ccGroup = nullptr;
    } else {
      ...
      ccConsole::Print(tr("[CreateComponentsClouds] %1 component(s) were "
                          "created from cloud '%2'")
    ...
    if (ccGroup) {
      cloud->setEnabled(false);
      ccConsole::Warning(tr("[CreateComponentsClouds] Original cloud has been "
  ...
  for (ccGenericPointCloud *cloud : clouds) {
    if (cloud && cloud->isA(CC_TYPES::POINT_CLOUD)) {
      ...
      if (!theOctree) {
        ...
        if (!theOctree) {
          ccConsole::Error(tr("Couldn't compute octree for cloud '%1'!")
                               .arg(cloud->getName()));
          break;
        }
      }
      ...
      if (sfIdx < 0) {
        ccConsole::Error(
            tr("Couldn't allocate a new scalar field for computing CC labels! "
      ...
      int componentCount =
          CCCoreLib::AutoSegmentationTools::labelConnectedComponents(
              cloud, static_cast<unsigned char>(s_octreeLevel), false, &pDlg,
              theOctree.data());
      ...
      if (componentCount >= 0) {
        ...
        if (realComponentCount > 500) {
          ...
          if (QMessageBox::warning(
                  ...
                  tr("Do you really expect up to %1 components?\n(this may "
                     "take a lot of time to process and display)")
        ...
        if (!CCCoreLib::AutoSegmentationTools::extractConnectedComponents(
                cloud, components)) {
          ccConsole::Warning(
              tr("[DoActionLabelConnectedComponents] Something went wrong "
                 "while extracting CCs from cloud %1...")
                  .arg(cloud->getName()));
        }
      } else {
        ccConsole::Warning(
            tr("[DoActionLabelConnectedComponents] Something went wrong while "
      ...
      if (!components.empty()) {
        createComponentsClouds(cloud, components, s_minComponentSize,
                               s_randomColors, true);
      }
    }
  }
...
void MainWindow::doMeshTwoPolylines() {
  ...
  if (p1->getDisplay()) {
    useViewingDir = (QMessageBox::question(this, tr("Projection method"),
    ...
    if (useViewingDir) {
      viewingDir =
          -p1->getDisplay()->getViewportParameters().getViewDir().toPC();
    }
  }

  ccMesh *mesh = ccMesh::TriangulateTwoPolylines(
      p1, p2, useViewingDir ? &viewingDir : nullptr);
  if (mesh) {
    addToDB(mesh);
...
void MainWindow::doConvertPolylinesToMesh() {
  ...
  try {
    if (haveOneSelection() &&
        m_selectedEntities.back()->isA(CC_TYPES::HIERARCHY_OBJECT)) {
  ...
  if (!delaunayMesh->buildMesh(points2D, segments2D, errorStr)) {
    ccLog::Error(tr("Third party library error: %1")
  ...
  // Test delaunay output
  {
    ...
    for (unsigned i = 0; i < delaunayMesh->size(); ++i) {
      const CCCoreLib::VerticesIndexes *tsi =
...
void MainWindow::doActionComputeMesh(CCCoreLib::TRIANGULATION_TYPES type) {
  ...
  // if the cloud(s) already had normals, ask the use if wants to update them or
  // keep them as is (can look strange!)
  bool updateNormals = false;
  if (hadNormals) {
    updateNormals =
        (QMessageBox::question(
             this, tr("Keep old normals?"),
             tr("Cloud(s) already have normals. Do you want to update them "
                "(yes) or keep the old ones (no)?"),
             QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes);
  }
  ...
  for (size_t i = 0; i < clouds.size(); ++i) {
    ...
    if (mesh) {
      cloud->setVisible(false); // can't disable the cloud as the resulting mesh
                                // will be its child!
      cloud->addChild(mesh);
      cloud->prepareDisplayForRefresh_recursive();
      addToDB(mesh);
  ...
  for (ccHObject *entity : getSelectedEntities()) {
    ...
    if (entity->isKindOf(CC_TYPES::POINT_CLOUD)) {
      ...
      if (quadric) {
        ...
        ccConsole::Print(
            tr("[DoActionFitQuadric] Quadric local coordinate system:"));
        ccConsole::Print(
            quadric->getTransformation().toString(12, ' ')); // full precision
        ccConsole::Print(tr("[DoActionFitQuadric] Quadric equation (in local "
                            "coordinate system): ") +
        ...
        ccConsole::Warning(tr("Failed to compute quadric on cloud '%1'")
                               .arg(cloud->getName()));
        errors = true;
      }
    }
...
void MainWindow::doActionComputeDistanceMap() {
  ...
  for (ccHObject *entity : getSelectedEntities()) {
    if (!entity->isKindOf(CC_TYPES::MESH) &&
        !entity->isKindOf(CC_TYPES::POINT_CLOUD)) {
    ...
    PointCoordinateType cellDim =
        largestDim / steps +
        std::numeric_limits<PointCoordinateType>::
            epsilon(); // to avoid rounding issues when projecting triangles or
    ...
    ccPointCloud *gridCloud = new ccPointCloud(
        entity->getName() + QString(".distance_grid(%1)").arg(steps));
    {
      ...
      for (unsigned i = 0; i < steps; ++i) {
        for (unsigned j = 0; j < steps; ++j) {
          for (unsigned k = 0; k < steps; ++k) {
            ScalarType d =
                std::sqrt(static_cast<ScalarType>(cdt.getValue(i, j, k))) *
                cellDim;

            if (!filterRange || (d >= range[0] && d <= range[1])) {
              gridCloud->addPoint(
                  minCorner + CCVector3(i + 0.5, j + 0.5, k + 0.5) * cellDim);
              sf->addElement(d);
            }
          }
        }
      ...
      if (gridCloud->size() == 0) {
        ccLog::Warning(tr("[DistanceMap] Cloud '%1': no point falls inside the "
                          "specified range")
                           .arg(entity->getName()));
        delete gridCloud;
        gridCloud = nullptr;
...
void MainWindow::doActionComputeDistToBestFitQuadric3D() {
  bool ok = true;
  int steps =
      QInputDialog::getInt(this, tr("Distance to best fit quadric (3D)"),
  ...
  for (ccHObject *entity : getSelectedEntities()) {
    if (entity->isKindOf(CC_TYPES::POINT_CLOUD)) {
      ...
      if (Yk.compute3DQuadric(Q)) {
        ...
        if (!G) {
          ccConsole::Warning(
              tr("Failed to get the center of gravity of cloud '%1'!")
                  .arg(cloud->getName()));
          continue;
        }
        ...
        // Sample points on a cube and compute for each of them the distance to
        // the Quadric
        ...
        ccScalarField *sf =
            static_cast<ccScalarField *>(newCloud->getScalarField(sfIdx));
        assert(sf);

        for (int x = 0; x < steps; ++x) {
          CCVector3 P;
          P.x =
              C.x + maxDim * (static_cast<PointCoordinateType>(x) /
                                  static_cast<PointCoordinateType>(steps - 1) -
                              CCCoreLib::PC_ONE / 2);
          for (int y = 0; y < steps; ++y) {
            P.y = C.y +
                  maxDim * (static_cast<PointCoordinateType>(y) /
                                static_cast<PointCoordinateType>(steps - 1) -
                            CCCoreLib::PC_ONE / 2);
            for (int z = 0; z < steps; ++z) {
              P.z = C.z +
                    maxDim * (static_cast<PointCoordinateType>(z) /
                                  static_cast<PointCoordinateType>(steps - 1) -
                              CCCoreLib::PC_ONE / 2);
              newCloud->addPoint(P);
              ...
              ScalarType dist = static_cast<ScalarType>(
                  a * Pc.x * Pc.x + b * Pc.y * Pc.y + c * Pc.z * Pc.z +
        ...
        ccConsole::Warning(tr("Failed to compute 3D quadric on cloud '%1'")
                               .arg(cloud->getName()));
      }
    }
  }
...
void MainWindow::doActionComputeCPS() {
  ...
  ccOrderChoiceDlg dlg(m_selectedEntities.front(), tr("Compared"),
                       m_selectedEntities.back(), tr("Reference"), this);
  if (!dlg.exec())
    return;
  ...
  if (sfIdx < 0) {
    ccConsole::Error(tr("Couldn't allocate a new scalar field for computing "
                        "distances! Try to free some memory ..."));
    return;
  }
  ...
  // cmpPC->forEach(CCCoreLib::ScalarFieldTools::SetScalarValueToNaN); //now
  ...
  if (result >= 0) {
    ...
    newCloud = srcCloud->isA(CC_TYPES::POINT_CLOUD)
                   ? static_cast<ccPointCloud *>(srcCloud)->partialClone(&CPSet)
                   : ccPointCloud::From(&CPSet, srcCloud);

    newCloud->setName(QString("[%1]->CPSet(%2)")
...
void MainWindow::doActionShiftPointsAlongNormals() {
  ...
  if (candidates.empty()) {
    ccConsole::Error(
        QObject::tr("Select at least one point cloud with normals"));
    return;
  }
  ...
  bool ok = false;
  double shift = QInputDialog::getDouble(this, tr("Shift along normals"),
                                         tr("Shift quantity"), s_shift, -1.0e6,
                                         1.0e6, 6, &ok);
  if (!ok) {
    return;
  }
...
void MainWindow::doActionFindBiggestInnerRectangle() {
  ...
  ccGenericPointCloud *cloud = static_cast<ccGenericPointCloud *>(entity);
  ccBox *box =
      ccInnerRect2DFinder().process(cloud, static_cast<unsigned char>(dim));
...
void MainWindow::doActionFastRegistration(FastRegistrationMode mode) {
  ...
  for (ccHObject *entity : selectedEntities) {
    ...
    forceConsoleDisplay();
    ccConsole::Print(
        tr("[Synchronize] Transformation matrix (%1):").arg(entity->getName()));
...
void MainWindow::doActionMatchBBCenters() {
                                             ...
                                             // change during this loop!
  {
    ...
    forceConsoleDisplay();
    ccConsole::Print(
        tr("[Synchronize] Transformation matrix (%1 --> %2):")
            .arg(entity->getName(), selectedEntities[0]->getName()));
    ccConsole::Print(glTrans.toString(12, ' ')); // full precision
    ccConsole::Print(
        tr("Hint: copy it (CTRL+C) and apply it - or its inverse - on any "
           "entity with the 'Edit > Apply transformation' tool"));
...
void MainWindow::doActionMatchScales() {
  ...
  ccLibAlgorithms::ApplyScaleMatchingAlgorithm(s_msAlgorithm, selectedEntities,
                                               s_msRmsDiff, s_msFinalOverlap,
...
void MainWindow::doActionSORFilter() {
  ...
  ccHObject::Container selectedEntities =
      getSelectedEntities(); // we have to use a local copy: 'selectEntity' will
                             // change the set of currently selected entities!
  ...
  for (ccHObject *entity : selectedEntities) {
    ...
    CCCoreLib::ReferenceCloud *selection =
        CCCoreLib::CloudSamplingTools::sorFilter(
            cloud, s_sorFilterKnn, s_sorFilterNSigma, cloud->getOctree().data(),
            &pDlg);

    if (selection) {
      if (selection->size() == cloud->size()) {
        ccLog::Warning(
            tr("[DoActionSORFilter] No points were removed from cloud '%1'")
                .arg(cloud->getName()));
      } else {
        ...
        if (cleanCloud) {
          ...
          ccConsole::Warning(tr("[DoActionSORFilter] Not enough memory to "
      ...
      ccConsole::Warning(tr("[DoActionSORFilter] Failed to apply the noise "
...
void MainWindow::doActionFilterNoise() {
  ...
  ccHObject::Container selectedEntities =
      getSelectedEntities(); // we have to use a local copy: and 'selectEntity'
  ...
  for (ccHObject *entity : selectedEntities) {
    ...
    CCCoreLib::ReferenceCloud *selection =
        CCCoreLib::CloudSamplingTools::noiseFilter(
            cloud, static_cast<PointCoordinateType>(kernelRadius),
            s_noiseFilterNSigma, s_noiseFilterRemoveIsolatedPoints,
            s_noiseFilterUseKnn, s_noiseFilterKnn, s_noiseFilterUseAbsError,
            s_noiseFilterAbsError, cloud->getOctree().data(), &pDlg);

    if (selection) {
      if (selection->size() == cloud->size()) {
        ccLog::Warning(
            tr("[DoActionFilterNoise] No points were removed from cloud '%1'")
                .arg(cloud->getName()));
      } else {
        ccPointCloud *cleanCloud = cloud->partialClone(selection);
        if (cleanCloud) {
          ...
          ccConsole::Warning(tr("[DoActionFilterNoise] Not enough memory to "
                                "create a clean version of cloud '%1'!")
                                 .arg(cloud->getName()));
        }
      }
      ...
      // no points fall inside selection!
      ccConsole::Warning(tr("[DoActionFilterNoise] Failed to apply the noise "
                            "filter to cloud '%1'! (not enough memory?)")
                             .arg(cloud->getName()));
    }
  }
...
void MainWindow::doActionUnroll() {
  ...
  ccGenericPointCloud *cloud = ccHObjectCaster::ToGenericPointCloud(
      m_selectedEntities.front(), &lockedVertices);
  if (lockedVertices) {
    ccUtils::DisplayLockedVerticesWarning(m_selectedEntities.front()->getName(),
                                          true);
    return;
  }
  ...
  case ccPointCloud::CYLINDER: {
    ...
    output = pc->unroll(mode, &params, exportDeviationSF, startAngle_deg,
                        stopAngle_deg, arbitraryOutputCS, &pDlg);
  } break;
  ...
  case ccPointCloud::CONE_CYLINDRICAL_ADAPTIVE_RADIUS: {
    ...
    params.radius =
        (mode == ccPointCloud::CONE_CYLINDRICAL_FIXED_RADIUS ? radius : 0);
    params.apex = center;
  ...
  if (output) {
    if (m_selectedEntities.front()->isA(CC_TYPES::MESH)) {
      ...
      mesh->setEnabled(false);
      ccConsole::Warning(
          tr("[Unroll] Original mesh has been automatically hidden"));
      ...
      pc->setEnabled(false);
      ccConsole::Warning(
          tr("[Unroll] Original cloud has been automatically hidden"));
      if (pc->getParent()) {
...
ccGLWindowInterface *MainWindow::getGLWindow(int index) const {
  ...
  if (index >= 0 && index < subWindowList.size()) {
    ccGLWindowInterface *win =
        ccGLWindowInterface::FromWidget(subWindowList[index]->widget());
    assert(win);
    return win;
  } else {
    assert(false);
...
ccGLWindowInterface *MainWindow::new3DViewInternal(bool allowEntitySelection) {
  ...
  // restore options
  {
    ...
    bool autoPickRotationCenter =
        settings.value(ccPS::AutoPickRotationCenter(), true).toBool();
  ...
  if (allowEntitySelection) {
    connect(view3D->signalEmitter(),
            &ccGLWindowSignalEmitter::entitySelectionChanged, this,
            [=](ccHObject *entity) { m_ccRoot->selectEntity(entity); });

    connect(view3D->signalEmitter(),
            &ccGLWindowSignalEmitter::entitiesSelectionChanged, this,
  ...
  //'echo' mode
  connect(view3D->signalEmitter(), &ccGLWindowSignalEmitter::mouseWheelRotated,
          this, &MainWindow::echoMouseWheelRotate);
  connect(view3D->signalEmitter(), &ccGLWindowSignalEmitter::viewMatRotated,
          this, &MainWindow::echoBaseViewMatRotation);
  connect(view3D->signalEmitter(), &ccGLWindowSignalEmitter::cameraPosChanged,
          this, &MainWindow::echoCameraPosChanged);
  connect(view3D->signalEmitter(), &ccGLWindowSignalEmitter::pivotPointChanged,
          this, &MainWindow::echoPivotPointChanged);

  connect(view3D->signalEmitter(), &ccGLWindowSignalEmitter::aboutToClose, this,
          &MainWindow::prepareWindowDeletion);
  connect(view3D->signalEmitter(), &ccGLWindowSignalEmitter::filesDropped, this,
          &MainWindow::addToDBAuto,
          Qt::QueuedConnection); // DGM: we don't want to block the 'dropEvent'
                                 // method of ccGLWindow instances!
  connect(view3D->signalEmitter(), &ccGLWindowSignalEmitter::newLabel, this,
          &MainWindow::handleNewLabel);
  connect(view3D->signalEmitter(),
          &ccGLWindowSignalEmitter::exclusiveFullScreenToggled, this,
          &MainWindow::onExclusiveFullScreenToggled);

  if (m_pickingHub) {
    // we must notify the picking hub as well if the window is destroyed
    connect(view3D->signalEmitter(), &ccGLWindowSignalEmitter::aboutToClose,
            m_pickingHub, &ccPickingHub::onActiveWindowDeleted);
  }
...
void MainWindow::doActionResetAllVBOs() {
  ...
  if (releasedSize != 0) {
    ccLog::Print(
        tr("All VBOs have been released (%1 Mb)")
            .arg(releasedSize / static_cast<double>(1 << 20), 0, 'f', 2));
    if (ccGui::Parameters().useVBOs) {
      ccLog::Warning(tr("You might want to disable the 'use VBOs' option in "
                        "the Display Settings to keep the GPU memory empty"));
    }
  } else {
...
void MainWindow::closeEvent(QCloseEvent *event) {
  ...
  if (!opts.confirmQuit ||
      (m_ccRoot && (m_ccRoot->getRootEntity()->getChildrenNumber() == 0))) {
    event->accept();
  } else // ...otherwise confirm
  {
    QMessageBox message_box(QMessageBox::Question, tr("Quit"),
                            tr("Are you sure you want to quit?"),
                            QMessageBox::NoButton, this);
    message_box.addButton(QMessageBox::Button::Yes);
    message_box.addButton(tr("Yes, don't ask again"),
...
void MainWindow::repositionOverlayDialog(ccMDIDialogs &mdiDlg) {
  ...
  case Qt::BottomLeftCorner:
    dx = margin;
    dy = std::max(margin,
                  m_mdiArea->height() - mdiDlg.dialog->height() - margin);
    break;
...
void MainWindow::activateRegisterPointPairTool() {
  ...
  try {
    ...
    if (entities.empty()) {
      ...
      if (!ccEntitySelectionDialog::SelectEntities(
              entities, indexes, this, tr("Select to-be-aligned entities"))) {
  ...
  if (!win) {
    ccLog::Error(
        tr("[PointPairRegistration] Failed to create dedicated 3D view!"));
    return;
  }
...
void MainWindow::activateSectionExtractionMode() {
  ...
  if (!m_seTool) {
    ...
    connect(m_seTool, &ccOverlayDialog::processFinished, this,
            &MainWindow::deactivateSectionExtractionMode);
      ...
      if (entity->isKindOf(CC_TYPES::POINT_CLOUD)) {
        if (m_seTool->addCloud(static_cast<ccGenericPointCloud *>(entity))) {
          if (!firstDisplay && entity->getDisplay()) {
            firstDisplay =
                static_cast<ccGLWindowInterface *>(entity->getDisplay());
          }
...
void MainWindow::activatePointListPickingMode() {
  ...
  m_plpDlg->markerSizeSpinBox->setValue(
      win->getDisplayParameters().labelMarkerSize);
...
void MainWindow::activateClippingBoxMode() {
  ...
  if (!m_clipTool) {
    ...
    connect(m_clipTool, &ccOverlayDialog::processFinished, this,
            &MainWindow::deactivateClippingBoxMode);
  }
  m_clipTool->linkWith(win);

  ccHObject::Container selectedEntities =
      getSelectedEntities(); // we have to use a local copy: 'unselectEntity'
                             // will change the set of currently selected
                             // entities!
  for (ccHObject *entity : selectedEntities) {
...
void MainWindow::activateTranslateRotateMode() {
  ...
  if (m_transTool->getNumberOfValidEntities() == 0) {
    ccConsole::Error(
        tr("No entity eligible for manual transformation! (see console)"));
    return;
...
void MainWindow::doActionRenderToFile() {
  ...
  if (rtfDlg.exec()) {
    QApplication::processEvents();
    win->renderToFile(rtfDlg.getFilename(), rtfDlg.getZoom(),
                      rtfDlg.dontScalePoints(), rtfDlg.renderOverlayItems());
  }
}
...
void MainWindow::doActionEditCamera() {
  ...
  if (!m_cpeDlg) {
    ...
    connect(m_mdiArea, &QMdiArea::subWindowActivated, m_cpeDlg,
            qOverload<QMdiSubWindow *>(&ccCameraParamEditDlg::linkWith));
...
void MainWindow::doActionSaveViewportAsCamera() {
  ...
  cc2DViewportObject *viewportObject =
      new cc2DViewportObject(QString("Viewport #%1").arg(++s_viewportIndex));
  ...
  for (size_t i = 0; i < selNum; ++i) {
    ...
    if (win) {
      if (entity->getDisplay() == win) {
        ...
        ccLog::Error(
            tr("All selected entities must be displayed in the same 3D view!"));
        return;
      }
    }
  }
...
void MainWindow::setPivotAlwaysOn() {
  ...
  if (win) {
    ...
    if (m_pivotVisibilityPopupButton)
      m_pivotVisibilityPopupButton->setIcon(
          m_UI->actionSetPivotAlwaysOn->icon());
  }
}
...
void MainWindow::setPivotRotationOnly() {
  ...
  if (win) {
    ...
    if (m_pivotVisibilityPopupButton)
      m_pivotVisibilityPopupButton->setIcon(
          m_UI->actionSetPivotRotationOnly->icon());
  }
}
...
void MainWindow::setCenteredPerspectiveView(ccGLWindowInterface *win,
  ...
  if (win) {
    ...
    if (m_viewModePopupButton)
      m_viewModePopupButton->setIcon(
          m_UI->actionSetCenteredPerspectiveView->icon());
    if (m_pivotVisibilityPopupButton)
      m_pivotVisibilityPopupButton->setEnabled(true);
  }
}
...
void MainWindow::setViewerPerspectiveView(ccGLWindowInterface *win) {
  if (win) {
    ...
    if (m_viewModePopupButton)
      m_viewModePopupButton->setIcon(
          m_UI->actionSetViewerPerspectiveView->icon());
    if (m_pivotVisibilityPopupButton)
      m_pivotVisibilityPopupButton->setEnabled(false);
  }
}

void MainWindow::enablePickingOperation(ccGLWindowInterface *win,
                                        QString message) {
  if (!win) {
    assert(false);
    return;
  ...
  if (!m_pickingHub->addListener(this)) {
    ccLog::Error(tr("Can't start the picking mechanism (another tool is "
                    "already using it)"));
    return;
  }

  // specific case: we prevent the 'point-pair based alignment' tool to process
  // the picked point! if (m_pprDlg) 	m_pprDlg->pause(true);

  s_pickingWindow = win;
  win->displayNewMessage(message, ccGLWindowInterface::LOWER_LEFT_MESSAGE, true,
                         24 * 3600);
...
void MainWindow::cancelPreviousPickingOperation(bool aborted) {
  ...
  if (aborted) {
    s_pickingWindow->displayNewMessage(
        QString(),
        ccGLWindowInterface::LOWER_LEFT_MESSAGE); // clear previous messages
    s_pickingWindow->displayNewMessage(tr("Picking operation aborted"),
...
void MainWindow::onItemPicked(const PickedItem &pi) {
  ...
  case PICKING_LEVEL_POINTS: {
    ...
    //{
    //	ccLog::Warning(tr("[Level] Only points picked on '%1' are
    // considered!").arg(s_levelEntity->getName())); 	return;
    //}
    ...
    if (markerCount == 3) {
      ...
      // we choose 'Z' so that it points 'upward' relatively to the camera
      // (assuming the user will be looking from the top)
      CCVector3d viewDir =
          s_pickingWindow->getViewportParameters().getViewDir();
      ...
      // clear message
      s_pickingWindow->displayNewMessage(
          QString(), ccGLWindowInterface::LOWER_LEFT_MESSAGE,
    ...
    // we use the next 'case' entry (PICKING_ROTATION_CENTER) to redefine the
    // rotation center as well!
  ...
  case PICKING_ROTATION_CENTER: {
    ...
    if (m_transTool && m_transTool->started()) {
      ...
      const unsigned &precision =
          s_pickingWindow->getDisplayParameters().displayedNumPrecision;
      s_pickingWindow->displayNewMessage(
          QString(), ccGLWindowInterface::LOWER_LEFT_MESSAGE,
          false); // clear previous message
      s_pickingWindow->displayNewMessage(
          QString("Point (%1 ; %2 ; %3) set as rotation center for interactive "
                  "transformation")
      ...
      const ccViewportParameters &params =
          s_pickingWindow->getViewportParameters();
...
void MainWindow::doLevel() {
  ...
  enablePickingOperation(win,
                         tr("Pick three points on the floor plane (click the "
                            "Level button or press Escape to cancel)"));
}
...
void MainWindow::doPickRotationCenter() {
  ...
  if (perspectiveEnabled && !objectCentered) {
    ccLog::Error(tr("Perspective mode is viewer-centered: can't use a point as "
                    "rotation center!"));
    return;
  }

  s_currentPickingOperation = PICKING_ROTATION_CENTER;
  enablePickingOperation(win, tr("Pick a point to be used as rotation center "
                                 "(click on icon again to cancel)"));
}

ccPointCloud *
MainWindow::askUserToSelectACloud(ccHObject *defaultCloudEntity /*=nullptr*/,
...
void MainWindow::spawnHistogramDialog(const std::vector<unsigned> &histoValues,
                                      ...
                                      QString title, QString xAxisLabel) {
...
void MainWindow::showSelectedEntitiesHistogram() {
  for (ccHObject *entity : getSelectedEntities()) {
    ...
    if (cloud) {
      ...
      ccScalarField *sf =
          static_cast<ccScalarField *>(cloud->getCurrentDisplayedScalarField());
      if (sf) {
        ccHistogramWindowDlg *hDlg = new ccHistogramWindowDlg(this);
        hDlg->setAttribute(Qt::WA_DeleteOnClose, true);
        ...
        ccHistogramWindow *histogram = hDlg->window();
        {
          ...
          unsigned numberOfClasses =
              static_cast<unsigned>(sqrt(static_cast<double>(numberOfPoints)));
          ...
          histogram->setAxisLabels(QString::fromStdString(sf->getName()),
                                   tr("Count"));
          histogram->refresh();
        }
        hDlg->show();
...
void MainWindow::doActionClone() {
  ...
  for (ccHObject *entity : getSelectedEntities()) {
    ...
    if (entity->isKindOf(CC_TYPES::POINT_CLOUD)) {
      ...
      if (!clone) {
        ccConsole::Error(tr("An error occurred while cloning camera sensor %1")
                             .arg(entity->getName()));
      }
      ...
      if (!clone) {
        ccConsole::Error(tr("An error occurred while cloning GBL sensor %1")
                             .arg(entity->getName()));
      }
      ...
      if (!clone) {
        ccConsole::Error(tr("An error occurred while cloning image %1")
                             .arg(entity->getName()));
      }
      ...
      if (!clone) {
        ccConsole::Error(tr("An error occurred while cloning label %1")
                             .arg(entity->getName()));
      }
      ...
      if (!clone) {
        ccConsole::Error(tr("An error occurred while cloning viewport %1")
                             .arg(entity->getName()));
      }
      ...
      ccLog::Warning(tr("Entity '%1' can't be cloned (type not supported yet!)")
                         .arg(entity->getName()));
    }
...
void MainWindow::doActionAddConstantSF() {
  ...
  unsigned trys = 1;
  while (cloud->getScalarFieldIndexByName(defaultName.toStdString()) >= 0 ||
         trys > 99) {
...
void MainWindow::doActionFitSphere() {
  ...
  for (ccHObject *entity : getSelectedEntities()) {
    ...
    if (CCCoreLib::GeometricalAnalysisTools::DetectSphereRobust(
            cloud, outliersRatio, center, radius, rms, &pDlg, confidence) !=
        CCCoreLib::GeometricalAnalysisTools::NoError) {
      ccLog::Warning(tr("[Fit sphere] Failed to fit a sphere on cloud '%1'")
                         .arg(cloud->getName()));
      continue;
    }

    ccLog::Print(tr("[Fit sphere] Cloud '%1': center (%2,%3,%4) - radius = %5 "
                    "[RMS = %6]")
                     .arg(cloud->getName())
                     .arg(center.x)
                     .arg(center.y)
    ...
    ccSphere *sphere =
        new ccSphere(radius, &trans, tr("Sphere r=%1").arg(radius));
...
void MainWindow::doActionFitCircle() {
  ...
  for (ccHObject *entity : getSelectedEntities()) {
    ...
    PointCoordinateType radius = 0;
    double rms = std::numeric_limits<double>::quiet_NaN();
    if (CCCoreLib::GeometricalAnalysisTools::DetectCircle(cloud, center, normal,
                                                          radius, rms, &pDlg) !=
        CCCoreLib::GeometricalAnalysisTools::NoError) {
      ccLog::Warning(tr("[Fit circle] Failed to fit a circle on cloud '%1'")
                         .arg(cloud->getName()));
      continue;
    }
...
void MainWindow::doComputePlaneOrientation(bool fitFacet) {
  ...
  ccHObject::Container selectedEntities =
      getSelectedEntities(); // warning, getSelectedEntites may change during
                             // this loop!
    ...
    if (cloud) {
      ...
      if (plane) {
        ...
        if (N.z < 0.0)
          N *= -1.0;
        ccConsole::Print(
            tr("\t- normal: (%1, %2, %3)").arg(N.x).arg(N.y).arg(N.z));
        ...
        QString dipAndDipDirStr =
            ccNormalVectors::ConvertDipAndDipDirToString(dip, dipDir);
        ...
        ccGLMatrix makeZPosMatrix =
            ccGLMatrix::FromToRotation(N, CCVector3(0, 0, CCCoreLib::PC_ONE));
        CCVector3 Gt = C;
        makeZPosMatrix.applyRotation(Gt);
        makeZPosMatrix.setTranslation(C - Gt);
        ccConsole::Print(tr("[Orientation] A matrix that would make this plane "
        ...
        ccConsole::Warning(tr("Failed to fit a plane/facet on entity '%1'")
                               .arg(entity->getName()));
      }
    }
  }
...
void MainWindow::doActionSFGradient() {
  if (!ccLibAlgorithms::ApplyCCLibAlgorithm(
          ccLibAlgorithms::CCLIB_ALGO_SF_GRADIENT, m_selectedEntities, this))
    return;
  refreshAll();
  updateUI();
}
...
void MainWindow::doSphericalNeighbourhoodExtractionTest() {
  ...
  double val = QInputDialog::getDouble(this, tr("SNE test"), tr("Radius:"),
                                       sphereRadius, DBL_MIN, 1.0e9, 8, &ok);
  if (!ok)
    return;
  sphereRadius = val;
  ...
  for (size_t i = 0; i < selNum; ++i) {
    ...
    if (sfIdx < 0) {
      ccConsole::Error(
          tr("Failed to create scalar field on cloud '%1' (not enough memory?)")
              .arg(cloud->getName()));
      return;
    }
    ...
    for (unsigned j = 0; j < samples; ++j) {
      ...
      octree->getPointsInSphericalNeighbourhood(
          *cloud->getPoint(randIndex),
          static_cast<PointCoordinateType>(sphereRadius), neighbours, level);
      ...
      for (size_t k = 0; k < neihgboursCount; ++k) {
        cloud->setPointScalarValue(
            ...
            static_cast<ScalarType>(sqrt(neighbours[k].squareDistd)));
      }
    }
    ccConsole::Print(
        tr("[SNE_TEST] Mean extraction time = %1 ms (radius = %2, mean "
           "(neighbours) = %3)")
            .arg(eTimer.elapsed())
            .arg(sphereRadius)
            .arg(extractedPoints / static_cast<double>(samples), 0, 'f', 1));
...
void MainWindow::doCylindricalNeighbourhoodExtractionTest() {
  ...
  if (octree) {
    ...
    unsigned char level =
        octree->findBestLevelForAGivenNeighbourhoodSizeExtraction(
            static_cast<PointCoordinateType>(2.5 * radius)); // 2.5 = empirical
    const unsigned samples = 1000;
    std::random_device rd;  // non-deterministic generator
    std::mt19937 gen(rd()); // to seed mersenne twister.
    ...
    for (unsigned j = 0; j < samples; ++j) {
      ...
      {
        ccGLMatrix rot;
        rot.initFromParameters(distAngle(gen), distAngle(gen), distAngle(gen),
                               CCVector3(0, 0, 0));
      ...
      for (size_t k = 0; k < neihgboursCount; ++k) {
        cloud->setPointScalarValue(
            ...
            static_cast<ScalarType>(sqrt(cn.neighbours[k].squareDistd)));
      }
    }
    ccConsole::Print(
        tr("[CNE_TEST] Mean extraction time = %1 ms (radius = %2, height = %3, "
           "mean (neighbours) = %4))")
            .arg(subTimer.elapsed())
            .arg(radius)
            .arg(height)
            .arg(static_cast<double>(extractedPoints) / samples, 0, 'f', 1));
  } else {
...
void MainWindow::doActionCreateCloudFromEntCenters() {
  ...
  // look for clouds
  {
    for (ccHObject *entity : getSelectedEntities()) {
      ...
      // we display the cloud in the same window as the first (selected) cloud
      // we encounter
...
void MainWindow::doActionComputeBestICPRmsMatrix() {
  ...
  try {
    ...
    assert(CCCoreLib::LessThanEpsilon(
    ...
    for (unsigned j = 0; j <= thetaSteps; ++j) {
      ...
      for (unsigned i = 0; i < phiSteps; ++i) {
        ...
        trans.initFromParameters(
            static_cast<float>(CCCoreLib::DegreesToRadians(phi_deg)),
            static_cast<float>(CCCoreLib::DegreesToRadians(theta_deg)), 0,
            CCVector3(0, 0, 0));
  ...
  // let's start!
  {
    ...
    pDlg.setInfo(
        tr("%1 clouds and %2 positions").arg(cloudCount).arg(matrices.size()));
    CCCoreLib::NormalizedProgress nProgress(
        &pDlg, static_cast<unsigned>(((cloudCount * (cloudCount - 1)) / 2) *
                                     matrices.size()));
    pDlg.start();
    QApplication::processEvents();
      ...
      for (size_t j = i + 1; j < cloudCount; ++j) {
        ...
        for (size_t k = 0; k < matrices.size(); ++k) {
          ...
          result = CCCoreLib::ICPRegistrationTools::Register(
              A, nullptr, B, params, registerTrans, finalRMS, finalPointCount);
        ...
        if (bestMatrixIndex >= 0) {
          assert(bestB);
          ccHObject *group = new ccHObject(tr("Best case #%1 / #%2 - RMS = %3")
                                               .arg(i + 1)
                                               .arg(j + 1)
                                               .arg(minRMS));
          ...
          addToDB(group);
          ccLog::Print(tr("[DoActionComputeBestICPRmsMatrix] Comparison #%1 / "
                          "#%2: min RMS = %3 (phi = %4 / theta = %5 deg.)")
                           .arg(i + 1)
                           .arg(j + 1)
                           .arg(minRMS)
          ...
          assert(!bestB);
          ccLog::Warning(tr("[DoActionComputeBestICPRmsMatrix] Comparison #%1 "
                            "/ #%2: INVALID")
                             .arg(i + 1)
                             .arg(j + 1));
        }
...
void MainWindow::doActionExportPlaneInfo() {
  ...
  if (selectedEntities.size() == 1 &&
      selectedEntities.front()->isA(CC_TYPES::HIERARCHY_OBJECT)) {
    // a group
    selectedEntities.front()->filterChildren(planes, true, CC_TYPES::PLANE,
                                             false);
  } else {
  ...
  QString outputFilename =
      QFileDialog::getSaveFileName(this, tr("Select output file"), currentPath,
                                   "*.csv", nullptr, CCFileDialogOptions());
  ...
  if (!csvFile.open(QFile::WriteOnly | QFile::Text)) {
    ccConsole::Error(
        tr("Failed to open file for writing! (check file permissions)"));
    return;
  }

  // save last saving location
  settings.setValue(ccPS::CurrentPath(),
  ...
  ccConsole::Print(tr("[I/O] File '%1' successfully saved (%2 plane(s))")
                       .arg(outputFilename)
                       .arg(planes.size()));
  csvFile.close();
}
...
void MainWindow::doActionExportCloudInfo() {
  ...
  // write one line per cloud
  {
    for (ccHObject *entity : clouds) {
      ...
      for (unsigned j = 0; j < cloud->getNumberOfScalarFields(); ++j) {
        ...
        csvStream << QString::fromStdString(sf->getName())
                  << ';' /*"SF name;"*/;
  ...
  ccConsole::Print(tr("[I/O] File '%1' successfully saved (%2 cloud(s))")
                       .arg(outputFilename)
                       .arg(clouds.size()));
  csvFile.close();
}
...
void MainWindow::doActionCloudMeshDist() {
  ...
  if (meshNum == 1) {
    ...
    refMesh =
        ccHObjectCaster::ToGenericMesh(m_selectedEntities[isMesh[0] ? 0 : 1]);
    ...
    if (refMesh->isKindOf(CC_TYPES::PRIMITIVE)) {
      ...
      if (!DontShowPrimitiveDistWarning) {
        QMessageBox::StandardButton answer = QMessageBox::warning(
            this, tr("Distance to primitive"),
            tr("Computing distances to a primitive is faster and more accurate "
               "with the 'Tools > Distances > Cloud / Primitive Dist.' "
...
void MainWindow::doActionCloudPrimitiveDist() {
  ...
  for (ccHObject *entity : getSelectedEntities()) {
    if (entity->isKindOf(CC_TYPES::PRIMITIVE) ||
      ...
      if (entity->isA(CC_TYPES::PLANE) || entity->isA(CC_TYPES::SPHERE) ||
          entity->isA(CC_TYPES::CYLINDER) || entity->isA(CC_TYPES::CONE) ||
          entity->isA(CC_TYPES::BOX) || entity->isA(CC_TYPES::POLY_LINE)) {
        if (!refEntity) {
          // first primitive encountered
          refEntity = entity;
        } else {
          ccConsole::Error(tr("Select only one primitive "
                              "(Plane/Box/Sphere/Cylinder/Cone) or polyline"));
          return;
        }
      }
  ...
  for (auto &cloud : clouds) {
    ...
    int sfIdx =
        compEnt->getScalarFieldIndexByName(CC_TEMP_DISTANCES_DEFAULT_SF_NAME);
    if (sfIdx < 0) {
      // we need to create a new scalar field
      sfIdx = compEnt->addScalarField(CC_TEMP_DISTANCES_DEFAULT_SF_NAME);
      if (sfIdx < 0) {
        ccLog::Warning(tr("[Compute Primitive Distances] [Cloud: %1] Couldn't "
                          "allocate a new scalar field for computing "
                          "distances! Try to free some memory ...")
                           .arg(compEnt->getName()));
        ++errorCount;
        continue;
    ...
    if (!compEnt->enableScalarField()) {
      ccLog::Warning(
          tr("[Compute Primitive Distances] [Cloud: %1] Not enough memory")
              .arg(compEnt->getName()));
      ++errorCount;
      continue;
    }
    ...
    int returnCode = 0;
    QString errString = tr("[Compute Primitive Distances] Cloud to %1 distance "
                           "computation failed (error code = %2)");

    switch (refEntity->getClassID()) {
    case CC_TYPES::SPHERE: {
      if (!(returnCode = CCCoreLib::DistanceComputationTools::
                computeCloud2SphereEquation(
                    compEnt, refEntity->getOwnBB().getCenter(),
    ...
    case CC_TYPES::PLANE: {
      ...
      if (s_treatPlanesAsBounded) {
        CCCoreLib::SquareMatrix rotationTransform(
            plane->getTransformation().data(), true);
        if (!(returnCode = CCCoreLib::DistanceComputationTools::
                  computeCloud2RectangleEquation(
                      compEnt, plane->getXWidth(), plane->getYWidth(),
                      rotationTransform, plane->getCenter(), s_signedDist))) {
          ccConsole::Warning(
              errString.arg(tr("Bounded Plane")).arg(returnCode));
          ++errorCount;
        }
      } else {
        if (!(returnCode = CCCoreLib::DistanceComputationTools::
                  computeCloud2PlaneEquation(
                      compEnt, static_cast<ccPlane *>(refEntity)->getEquation(),
                      s_signedDist))) {
          ccConsole::Warning(
              errString.arg(tr("Infinite Plane")).arg(returnCode));
          ++errorCount;
        }
      }
      break;
    ...
    case CC_TYPES::CONE: {
      if (!(returnCode =
                CCCoreLib::DistanceComputationTools::computeCloud2ConeEquation(
                    compEnt, static_cast<ccCone *>(refEntity)->getLargeCenter(),
    ...
    case CC_TYPES::BOX: {
      ...
      if (!(returnCode =
                CCCoreLib::DistanceComputationTools::computeCloud2BoxEquation(
                    compEnt, static_cast<ccBox *>(refEntity)->getDimensions(),
                    rotationTransform, boxCenter, s_signedDist))) {
    ...
    case CC_TYPES::POLY_LINE: {
      ...
      returnCode =
          CCCoreLib::DistanceComputationTools::computeCloud2PolylineEquation(
              compEnt, line);
      if (!returnCode) {
    ...
    default: {
      ccConsole::Error(
          tr("Unsupported primitive type")); // Shouldn't ever reach here...
      return;
    }
    }
    ...
    if (sf) {
      ...
      ccLog::Print(tr("[Compute Primitive Distances] [Primitive: %1] [Cloud: "
                      "%2] [%3] Mean distance = %4 / std deviation = %5")
  ...
  if (errorCount != 0) {
    ccLog::Error(
        tr("%1 error(s) occurred: refer to the Console (F8)").arg(errorCount));
  }
...
void MainWindow::toggleActiveWindowStereoVision(bool state) {
  ...
  if (win) {
    ...
    if (isActive) {
      ...
      if (win->getStereoParams().glassType ==
              ccGLWindowInterface::StereoParams::NVIDIA_VISION ||
          win->getStereoParams().glassType ==
              ccGLWindowInterface::StereoParams::GENERIC_STEREO_DISPLAY) {
      ...
      ccLog::WarningDebug("Stereo strength: " +
                          QString::number(params.stereoStrength));

      if (!ccGLWindowInterface::StereoSupported() && !params.isAnaglyph()) {
        ccLog::Error(tr("It seems your graphic card doesn't support Quad "
                        "Buffered Stereo rendering"));
      ...
      if (smDlg.updateFOV()) {
        // set the right FOV
        double fov_deg =
            2 * CCCoreLib::RadiansToDegrees(std::atan(
                    params.screenWidth_mm / (2.0 * params.screenDistance_mm)));
...
bool MainWindow::checkStereoMode(ccGLWindowInterface *win) {
  ...
  if (win && win->getViewportParameters().perspectiveView &&
    ...
    if (QMessageBox::question(this, tr("Stereo mode"),
                              tr("Stereo-mode only works in perspective mode. "
...
void MainWindow::toggleActiveWindowCenteredPerspective() {
  ...
  if (win) {
    const ccViewportParameters &params = win->getViewportParameters();
    if (params.perspectiveView && params.objectCenteredView &&
        !checkStereoMode(win)) // we need to check this only if we are already
                               // in object-centered perspective mode
    {
      return;
    }
...
void MainWindow::toggleActiveWindowViewerBasedPerspective() {
  ...
  if (win) {
    ...
    if (params.perspectiveView && !params.objectCenteredView &&
        !checkStereoMode(win)) // we need to check this only if we are already
                               // in viewer-based perspective mode
    {
      return;
    }
...
void MainWindow::createSinglePointCloud() {
  ...
  ccAskThreeDoubleValuesDlg axisDlg("x", "y", "z", -1.0e12, 1.0e12,
                                    s_lastPoint.x, s_lastPoint.y, s_lastPoint.z,
                                    4, tr("Point coordinates"), this);
...
void MainWindow::createPointCloudFromClipboard() {
  ...
  CC_FILE_ERROR result =
      AsciiFilter().loadAsciiData(data, tr("Clipboard"), container, parameters);
  if (result != CC_FERR_NO_ERROR) {
  ...
  bool normalsDisplayedByDefault =
      ccOptions::Instance().normalsDisplayedByDefault;
  for (ccHObject *cloud : clouds) {
    if (cloud) {
  ...
  QMainWindow::statusBar()->showMessage(
      tr("%1 cloud(s) loaded from the clipboard").arg(clouds.size()), 2000);
}
...
void MainWindow::toggleLockRotationAxis() {
  ...
  if (win) {
    ...
    if (isLocked) {
      win->displayNewMessage(
          tr("[ROTATION LOCKED]"), ccGLWindowInterface::UPPER_CENTER_MESSAGE,
          false, 24 * 3600, ccGLWindowInterface::ROTAION_LOCK_MESSAGE);
    } else {
      win->displayNewMessage(QString(),
                             ccGLWindowInterface::UPPER_CENTER_MESSAGE, false,
                             0, ccGLWindowInterface::ROTAION_LOCK_MESSAGE);
    }
    win->redraw(true, false);
  }
}
...
void MainWindow::doActionEnableBubbleViewMode() {
  // special case: the selected entity is a TLS sensor or a cloud with a TLS
  // sensor
  if (m_ccRoot) {
    ccHObject::Container selectedEntities;
...
void MainWindow::addToDB(ccHObject *obj, bool updateZoom /*=true*/,
  ...
  if (checkDimensions) {
    ...
    // GLdoubles :( ).
    if (ccGlobalShiftManager::Handle(
            P, diag, ccGlobalShiftManager::DIALOG_IF_NECESSARY, false, Pshift,
            &preserveCoordinateShift, &scale)) {
      ...
      if (needRescale || needShift) {
        ...
        ccConsole::Warning(
            tr("Entity '%1' has been translated: (%2,%3,%4) and rescaled of a "
               "factor %5 [original position will be restored when saving]")
                .arg(obj->getName())
                .arg(Pshift.x, 0, 'f', 2)
                .arg(Pshift.y, 0, 'f', 2)
                .arg(Pshift.z, 0, 'f', 2)
  ...
  if (updateZoom) {
    static_cast<ccGLWindowInterface *>(obj->getDisplay())
        ->zoomGlobal(); // automatically calls ccGLWindowInterface::redraw
  } else if (autoRedraw) {
    obj->redrawDisplay();
  }
}
...
void MainWindow::onExclusiveFullScreenToggled(bool state) {
  ...
  if (!state && win->stereoModeIsEnabled() &&
      (win->getStereoParams().glassType ==
           ccGLWindowInterface::StereoParams::NVIDIA_VISION ||
       win->getStereoParams().glassType ==
           ccGLWindowInterface::StereoParams::GENERIC_STEREO_DISPLAY)) {
...
void MainWindow::addToDB(const QStringList &filenames,
  ...
  QMainWindow::statusBar()->showMessage(
      tr("%1 file(s) loaded").arg(filenames.size()), 2000);
}
...
void MainWindow::closeAll() {
  ...
  QMessageBox message_box(
      QMessageBox::Question, tr("Close all"),
...
void MainWindow::doActionLoadFile() {
  ...
  QString currentOpenDlgFilter =
      settings.value(ccPS::SelectedInputFilter(), BinFilter::GetFileFilter())
          .toString();
  ...
  QStringList selectedFiles = QFileDialog::getOpenFileNames(
      ...
      filterStrings.join(s_fileFilterSeparator), &currentOpenDlgFilter,
      CCFileDialogOptions());
  ...
  if (currentOpenDlgFilter == allFilter) {
    currentOpenDlgFilter.clear(); // this way FileIOFilter will try to guess the
...
#ifdef CC_WINDOWS
  QString sPattern("^(?!^(PRN|AUX|CLOCK\\$|NUL|CON|COM\\d|LPT\\d|\\..*)(\\..+)?"
                   "$)[^\\x00-\\x1f\\\\?*:\\"
                   ";|/]+$");
#else
...
void MainWindow::doActionSaveFile() {
  ...
  while (!entitiesToDispatch.empty()) {
    ...
    if (child->isA(CC_TYPES::HIERARCHY_OBJECT)) {
      ...
      // we don't want double insertions if the user has highlighted both the
  ...
  QStringList fileFilters;
  {
    for (const FileIOFilter::Shared &filter : FileIOFilter::GetFilters()) {
      ...
      if (hasCloud) {
        ...
        bool multiple = false;
        canExportClouds =
            (filter->canSave(CC_TYPES::POINT_CLOUD, multiple, isExclusive) &&
      ...
      if (hasMesh) {
        ...
        bool multiple = false;
        canExportMeshes =
            (filter->canSave(CC_TYPES::MESH, multiple, isExclusive) &&
      ...
      if (hasPolylines) {
        ...
        bool multiple = false;
        canExportPolylines =
            (filter->canSave(CC_TYPES::POLY_LINE, multiple, isExclusive) &&
      ...
      if (hasImages) {
        ...
        bool multiple = false;
        canExportImages =
            (filter->canSave(CC_TYPES::IMAGE, multiple, isExclusive) &&
      ...
      if (hasSerializable) {
        // check if all entities have the same type
        {
          ...
          for (unsigned j = 1; j < otherSerializable.getChildrenNumber(); ++j) {
            if (otherSerializable.getChild(j)->getUniqueID() != firstClassID) {
              // we add a virtual second 'stdSaveType' so as to properly handle
              // exlusivity
              ++stdSaveTypes;
              break;
            }
        ...
        for (unsigned j = 0; j < otherSerializable.getChildrenNumber(); ++j) {
          ...
          canExportSerializables &=
              (filter->canSave(child->getClassID(), multiple, isExclusive) &&
  ...
  if (hasCloud)
    selectedFilter =
        settings.value(ccPS::SelectedOutputFilterCloud(), selectedFilter)
            .toString();
  else if (hasMesh)
    selectedFilter =
        settings.value(ccPS::SelectedOutputFilterMesh(), selectedFilter)
            .toString();
  else if (hasImages)
    selectedFilter =
        settings.value(ccPS::SelectedOutputFilterImage(), selectedFilter)
            .toString();
  else if (hasPolylines)
    selectedFilter =
        settings.value(ccPS::SelectedOutputFilterPoly(), selectedFilter)
            .toString();
  ...
  if (haveOneSelection()) {
    ...
    if (!IsValidFileName(defaultFileName)) {
      ccLog::Warning(tr("[I/O] First entity's name would make an invalid "
  ...
  QString selectedFilename =
      QFileDialog::getSaveFileName(this, tr("Save file"), fullPathName,
                                   fileFilters.join(s_fileFilterSeparator),
                                   &selectedFilter, CCFileDialogOptions());
  ...
  if (selectedFilter == BinFilter::GetFileFilter()) {
    if (haveOneSelection()) {
      result =
          FileIOFilter::SaveToFile(m_selectedEntities.front(), selectedFilename,
      ...
      if (tempContainer.getChildrenNumber()) {
        result = FileIOFilter::SaveToFile(&tempContainer, selectedFilename,
                                          parameters, selectedFilter);
      } else {
        ccLog::Warning(
            tr("[I/O] None of the selected entities can be saved this way..."));
    ...
    //{
    //	if (!hasOther)
    //		ccConsole::Warning(tr("[I/O] The following selected entites
    // won't be saved:")); //display this warning only if not already done
    // for
    // (unsigned i = 0; i < otherSerializable.getChildrenNumber(); ++i)
    //		ccConsole::Warning(tr("\t-
    //%1").arg(otherSerializable.getChild(i)->getName()));
    //}

    result = FileIOFilter::SaveToFile(
        entitiesToSave.getChildrenNumber() > 1 ? &entitiesToSave
  ...
  if (result == CC_FERR_NO_ERROR &&
      selectedFilter == BinFilter::GetFileFilter()) {
...
void MainWindow::doActionSaveProject() {
  ...
  if (rootEntity->getChildrenNumber() == 1) {
    // If there's only on top entity, we can try to use its name as the project
    // name.
    ...
    if (!IsValidFileName(defaultFileName)) {
      ccLog::Warning(tr("[I/O] Top entity's name would make an invalid "
                        "filename! Can't use it..."));
      defaultFileName = "project";
    }
  }
...
void MainWindow::on3DViewActivated(QMdiSubWindow *mdiWin) {
  ccGLWindowInterface *win =
      mdiWin ? ccGLWindowInterface::FromWidget(mdiWin->widget()) : nullptr;
  if (win) {
    updateViewModePopUpMenu(win);
    updatePivotVisibilityPopUpMenu(win);
    ...
    m_UI->actionShowCursor3DCoordinates->setChecked(
        win->cursorCoordinatesShown());
    m_UI->actionShowCursor3DCoordinates->blockSignals(false);

    m_UI->actionAutoPickRotationCenter->blockSignals(true);
    m_UI->actionAutoPickRotationCenter->setChecked(
        win->autoPickPivotAtCenter());
...
void MainWindow::updateMenus() {
  ...
  bool shadersEnabled =
      (active3DView ? active3DView->areShadersEnabled() : false);
...
void MainWindow::update3DViewsMenu() {
  ...
  if (!windows.isEmpty()) {
    ...
    for (QMdiSubWindow *window : windows) {
      ccGLWindowInterface *child =
          ccGLWindowInterface::FromWidget(window->widget());
...
void MainWindow::updateUIWithSelection() {
  ...
  if (m_ccRoot) {
    m_ccRoot->getSelectedEntities(m_selectedEntities, CC_TYPES::OBJECT,
                                  &selInfo);
  }
...
void MainWindow::enableUIItems(dbTreeSelectionInfo &selInfo) {
  ...
  m_UI->actionSetUniqueColor->setEnabled(
      atLeastOneEntity /*atLeastOneCloud || atLeastOneMesh*/); // DGM: we can
                                                               // set color to a
                                                               // group now!
  ...
  m_UI->actionShiftPointsAlongNormals->setEnabled(atLeastOneCloud &&
                                                  atLeastOneNormal);
  ...
  m_UI->actionCreateSurfaceBetweenTwoPolylines->setEnabled(
      m_UI->actionMeshTwoPolylines
          ->isEnabled()); // clone of actionMeshTwoPolylines
  ...
  m_UI->actionAddClassificationSF->setEnabled(exactlyOneCloud ||
                                              exactlyOneMesh);
  ...
  m_UI->menuActiveScalarField->setEnabled((exactlyOneCloud || exactlyOneMesh) &&
                                          selInfo.sfCount > 0);
  ...
  m_UI->actionCloudPrimitiveDist->setEnabled(
      atLeastOneCloud && (atLeastOnePrimitive || atLeastOnePolyline));
...
void MainWindow::echoMouseWheelRotate(float wheelDelta_deg) {
  ...
  ccGLWindowInterface *sendingWindow =
      ccGLWindowInterface::FromEmitter(sender());
  if (!sendingWindow)
    return;
...
ccDBRoot *MainWindow::db() { return m_ccRoot; }
...
void MainWindow::createGLWindow(ccGLWindowInterface *&window,
...
ccMainAppInterface::ccHObjectContext
MainWindow::removeObjectTemporarilyFromDBTree(ccHObject *obj) {
  ...
  // remove the object's dependency to its father (in case it undergoes 'severe'
  // modifications)
  if (context.parent) {
...
void MainWindow::doActionGlobalShiftSeetings() {
  ...
  ui.maxAbsCoordSpinBox->setValue(
      static_cast<int>(log10(ccGlobalShiftManager::MaxCoordinateAbsValue())));
  ui.maxAbsDiagSpinBox->setValue(
      static_cast<int>(log10(ccGlobalShiftManager::MaxBoundgBoxDiagonal())));
  ...
  ccLog::Print(
      tr("[Global Shift] Max abs. coord = %1 / max abs. diag = %2")
...
void MainWindow::doActionComparePlanes() {
  ...
  info
      << tr("Angle P1/P2: %1 deg.").arg(CCCoreLib::RadiansToDegrees(angle_rad));
  ccLog::Print(tr("[Compare] ") + info.last());
  ...
  ScalarType distCenter1ToPlane2 =
      CCCoreLib::DistanceComputationTools::computePoint2PlaneDistance(&C1,
                                                                      planeEq2);
  ...
  ScalarType distCenter2ToPlane1 =
      CCCoreLib::DistanceComputationTools::computePoint2PlaneDistance(&C2,
                                                                      planeEq1);
...
void MainWindow::initWidget() {
  // File
  m_UI->actionGlobalShiftSettings->setVisible(false);
  m_UI->actionSaveProject->setVisible(false);
  m_UI->actionPrimitiveFactory->setVisible(false);

  // Edit-color
  m_UI->actionColorFromScalarField->setVisible(false);
  m_UI->actionColorize->setVisible(false);
  m_UI->actionChangeColorLevels->setVisible(false);
  m_UI->actionSetColorGradient->setVisible(false);
  m_UI->actionRGBToGreyScale->setVisible(false);
  m_UI->actionScalarFieldFromColor->setVisible(false);
  m_UI->actionInterpolateColors->setVisible(false);
  m_UI->actionEnhanceRGBWithIntensities->setVisible(false);
  m_UI->actionRGBBilateralFilter->setVisible(false);
  m_UI->actionRGBGaussianFilter->setVisible(false);
  m_UI->actionRGBMeanFilter->setVisible(false);
  m_UI->actionRGBMedianFilter->setVisible(false);

  // Edit-Normals
  m_UI->actionComputeNormals->setVisible(true);
  m_UI->actionInvertNormals->setVisible(false);
  m_UI->actionOrientNormalsMST->setVisible(false);
  m_UI->actionOrientNormalsFM->setVisible(false);
  m_UI->actionConvertNormalToHSV->setVisible(false);
  m_UI->actionConvertNormalToDipDir->setVisible(false);
  m_UI->actionExportNormalToSF->setVisible(false);
  m_UI->actionSetSFsAsNormal->setVisible(false);
  m_UI->actionShiftPointsAlongNormals->setVisible(false);
  m_UI->menuOrientNormals->setVisible(false);
  m_UI->menuConvert_to->setVisible(false);
  m_UI->actionClearNormals->setVisible(true);

  // Edit-Octree
  m_UI->actionComputeOctree->setVisible(true);
  m_UI->actionResampleWithOctree->setVisible(false);

  // Edit-Grid
  m_UI->actionDeleteScanGrid->setVisible(false);
  m_UI->actionMeshScanGrids->setVisible(false);
  m_UI->menuGrid->menuAction()->setVisible(false);

  // Edit-Cloud
  m_UI->actionCreateSinglePointCloud->setVisible(false);
  m_UI->actionPasteCloudFromClipboard->setVisible(false);
  m_UI->menuCloud->menuAction()->setVisible(false);

  // Edit-Mesh
  m_UI->actionComputeMeshAA->setVisible(true);
  m_UI->actionComputeMeshLS->setVisible(true);
  m_UI->actionMeshTwoPolylines->setVisible(false);
  m_UI->actionMeshScanGrids->setVisible(false);
  m_UI->actionConvertTextureToColor->setVisible(false);
  m_UI->actionSmoothMeshLaplacian->setVisible(false);
  m_UI->actionSubdivideMesh->setVisible(false);
  m_UI->actionFlipMeshTriangles->setVisible(false);
  m_UI->actionMeasureMeshSurface->setVisible(false);
  m_UI->actionMeasureMeshVolume->setVisible(false);
  m_UI->actionFlagMeshVertices->setVisible(false);
  m_UI->actionSmoothMeshSF->setVisible(false);
  m_UI->actionEnhanceMeshSF->setVisible(false);
  m_UI->menuMeshScalarField->menuAction()->setVisible(false);
  m_UI->actionSamplePointsOnMesh->setVisible(true);

  // Edit-Polyline
  m_UI->actionSamplePointsOnPolyline->setVisible(false);
  m_UI->actionSmoothPolyline->setVisible(false);
  m_UI->menuPolyline->menuAction()->setVisible(false);

  // Edit-Plane
  m_UI->actionCreatePlane->setVisible(false);
  m_UI->actionFitPlaneProxy->setVisible(false);
  m_UI->actionEditPlane->setVisible(false);
  m_UI->actionFlipPlane->setVisible(false);
  m_UI->actionComparePlanes->setVisible(false);
  m_UI->menuPlane->menuAction()->setVisible(false);

  // Edit-Sensors
  m_UI->actionModifySensor->setVisible(false);
  m_UI->actionCreateGBLSensor->setVisible(false);
  m_UI->actionShowDepthBuffer->setVisible(false);
  m_UI->actionExportDepthBuffer->setVisible(false);
  m_UI->actionComputePointsVisibility->setVisible(false);
  m_UI->menuGroundBasedLidar->menuAction()->setVisible(false);

  m_UI->actionCreateCameraSensor->setVisible(false);
  m_UI->actionProjectUncertainty->setVisible(false);
  m_UI->actionCheckPointsInsideFrustum->setVisible(false);
  m_UI->menuCameraSensor->menuAction()->setVisible(false);

  m_UI->actionViewFromSensor->setVisible(false);
  m_UI->actionComputeDistancesFromSensor->setVisible(false);
  m_UI->actionComputeScatteringAngles->setVisible(false);
  m_UI->menuSensors->menuAction()->setVisible(false);

  // Edit-ScalarFields
  m_UI->actionShowHistogram->setVisible(false);
  m_UI->actionComputeStatParams->setVisible(false);
  m_UI->actionSFGradient->setVisible(false);
  m_UI->actionGaussianFilter->setVisible(false);
  m_UI->actionBilateralFilter->setVisible(false);
  m_UI->actionFilterByValue->setVisible(false);
  m_UI->actionConvertToRGB->setVisible(false);
  m_UI->actionConvertToRandomRGB->setVisible(false);
  m_UI->actionRenameSF->setVisible(false);
  m_UI->actionAddConstantSF->setVisible(false);
  m_UI->actionAddClassificationSF->setVisible(false);
  m_UI->actionAddIdField->setVisible(false);
  m_UI->actionExportCoordToSF->setVisible(false);
  m_UI->actionSetSFAsCoord->setVisible(false);
  m_UI->actionExportNormalToSF->setVisible(false);
  m_UI->actionSetSFsAsNormal->setVisible(false);
  m_UI->actionInterpolateSFs->setVisible(false);
  m_UI->actionSplitCloudUsingSF->setVisible(false);
  m_UI->actionScalarFieldArithmetic->setVisible(false);
  m_UI->actionOpenColorScalesManager->setVisible(false);
  m_UI->actionDeleteScalarField->setVisible(false);
  m_UI->actionDeleteAllSF->setVisible(false);
  m_UI->menuScalarFields->menuAction()->setVisible(false);

  // Edit- waveform
  m_UI->actionShowWaveDialog->setVisible(false);
  m_UI->actionCompressFWFData->setVisible(false);
  m_UI->menuWaveform->menuAction()->setVisible(false);

  // Edit-other
  m_UI->actionClone->setVisible(true);
  m_UI->actionMerge->setVisible(true);
  m_UI->actionSubsample->setVisible(true);
  m_UI->actionApplyTransformation->setVisible(true);
  m_UI->actionApplyScale->setVisible(true);
  m_UI->actionTranslateRotate->setVisible(true);
  m_UI->actionSegment->setVisible(true);
  m_UI->actionCrop->setVisible(true);
  m_UI->actionEditGlobalShiftAndScale->setVisible(true);
  m_UI->menuToggle->menuAction()->setVisible(false);
  m_UI->actionDelete->setVisible(true);

  // Tool-Clean
  m_UI->actionSORFilter->setVisible(true);
  m_UI->actionNoiseFilter->setVisible(false);
  m_UI->menuClean->menuAction()->setVisible(true);

  // Tool-Projection
  m_UI->actionUnroll->setVisible(false);
  m_UI->actionRasterize->setVisible(true);
  m_UI->actionConvertPolylinesToMesh->setVisible(false);
  m_UI->actionExportCoordToSF->setVisible(false);
  m_UI->actionCreateSurfaceBetweenTwoPolylines->setVisible(false);

  // Tool-Register
  m_UI->actionMatchBBCenters->setVisible(false);
  m_UI->actionMatchScales->setVisible(false);
  m_UI->actionPointPairsAlign->setVisible(true);
  m_UI->actionRegister->setVisible(true);
  m_UI->actionBBCenterToOrigin->setVisible(false);
  m_UI->actionBBMinCornerToOrigin->setVisible(false);
  m_UI->actionBBMaxCornerToOrigin->setVisible(false);

  // Tool-Distance
  m_UI->actionCloudCloudDist->setVisible(false);
  m_UI->actionCloudMeshDist->setVisible(false);
  m_UI->actionCloudPrimitiveDist->setVisible(false);
  m_UI->actionCPS->setVisible(false);
  m_UI->menuDistances->menuAction()->setVisible(false);

  // Tool-Volume
  m_UI->actionCompute2HalfDimVolume->setVisible(true);

  // Tool-Statistics
  m_UI->actionStatisticalTest->setVisible(false);
  m_UI->actionComputeStatParams2->setVisible(false);
  m_UI->menuStatistics->menuAction()->setVisible(false);

  // Tool-Segmentation
  m_UI->actionLabelConnectedComponents->setVisible(false);
  m_UI->actionCrossSection->setVisible(true);
  m_UI->actionExtractSections->setVisible(false);
  m_UI->actionKMeans->setVisible(false);
  m_UI->actionFrontPropagation->setVisible(false);

  // Tool-Fit
  m_UI->actionFitPlane->setVisible(false);
  m_UI->actionFitSphere->setVisible(true);
  m_UI->actionFitCircle->setVisible(false);
  m_UI->actionFitFacet->setVisible(false);
  m_UI->actionFitQuadric->setVisible(false);
  m_UI->menuFit->menuAction()->setVisible(false);

  // Tool-BatchExport
  m_UI->actionExportCloudInfo->setVisible(false);
  m_UI->actionExportPlaneInfo->setVisible(false);
  m_UI->menuBatchExport->menuAction()->setVisible(false);

  // Tool-Other
  m_UI->actionComputeGeometricFeature->setVisible(false);
  m_UI->actionRemoveDuplicatePoints->setVisible(false);
  m_UI->menuOther->menuAction()->setVisible(false);
  ...
  m_UI->actionComputeBestFitBB->setVisible(false);
  m_UI->actionDistanceToBestFitQuadric3D->setVisible(true);
  m_UI->actionDistanceMap->setVisible(false);
  m_UI->actionAlign->setVisible(false);
  m_UI->actionSNETest->setVisible(false);
  m_UI->actionCNETest->setVisible(false);
  m_UI->actionFindBiggestInnerRectangle->setVisible(true);
  m_UI->actionCreateCloudFromEntCenters->setVisible(false);
  m_UI->actionComputeBestICPRmsMatrix->setVisible(false);
  m_UI->actionEnableVisualDebugTraces->setVisible(false);
  m_UI->menuSandBox->menuAction()->setVisible(false);

  // Display
  m_UI->actionFullScreen->setVisible(false);
  m_UI->actionExclusiveFullScreen->setVisible(false);
  m_UI->actionRefresh->setVisible(true);
  m_UI->actionToggleCenteredPerspective->setVisible(false);
  m_UI->actionToggleViewerBasedPerspective->setVisible(false);
  m_UI->actionShowCursor3DCoordinates->setVisible(false);
  m_UI->actionLockRotationAxis->setVisible(false);
  ...
  m_UI->actionAdjustZoom->setVisible(false);
  m_UI->actionTestFrameRate->setVisible(false);

  m_UI->actionToggleSunLight->setVisible(false);
  m_UI->actionToggleCustomLight->setVisible(true);
  m_UI->menuLights->menuAction()->setVisible(false);

  m_UI->actionToggleActiveSFColorScale->setVisible(false);
  m_UI->actionShowActiveSFPrevious->setVisible(false);
  m_UI->actionShowActiveSFNext->setVisible(false);
  m_UI->menuActiveScalarField->menuAction()->setVisible(false);

  m_UI->actionConsole->setVisible(true);

  m_UI->actionDisplayMainTools->setVisible(false);
  m_UI->actionDisplayScalarFieldsTools->setVisible(false);
  m_UI->actionDisplayViewTools->setVisible(false);
  m_UI->menuToolbars->menuAction()->setVisible(false);

  m_UI->menuLanguage->menuAction()->setVisible(false);
  m_UI->actionResetGUIElementsPos->setVisible(false);
  m_UI->actionRestoreWindowOnStartup->setVisible(false);
  m_UI->actionResetAllVBOs->setVisible(false);

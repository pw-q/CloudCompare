...
bool ccRegistrationTools::ICP(
    ccHObject *data, ccHObject *model, ccGLMatrix &transMat, double &finalScale,
    double &finalRMS, unsigned &finalPointCount,
    const CCCoreLib::ICPRegistrationTools::Parameters &inputParameters,
    bool useDataSFAsWeights /*=false*/, bool useModelSFAsWeights /*=false*/,
  ...
  if (data->isKindOf(CC_TYPES::MESH)) {
    dataCloud = CCCoreLib::MeshSamplingTools::samplePointsOnMesh(
        ccHObjectCaster::ToGenericMesh(data), s_defaultSampledPointsOnDataMesh,
        progressDlg.data());
    if (!dataCloud) {
  ...
  if (data->isA(CC_TYPES::POINT_CLOUD)) {
    ...
    if (dataSfIdx >= 0)
      ...
      ccLog::Error(
          "[ICP] Couldn't create temporary scalar field! Not enough memory?");
      return false;
    }
  } else {
  ...
  if (params.finalOverlapRatio < 1.0 - s_overlapMarginRatio) {
    // DGM we can now use 'approximate' distances as SAITO algorithm is exact
    // (but with a coarse resolution) level = 7 if < 1.000.000 level = 8 if
    // < 10.000.000 level = 9 if > 10.000.000
    int gridLevel = static_cast<int>(log10(static_cast<double>(
                        std::max(dataCloud->size(), modelCloud->size())))) +
                    2; // static_cast is equivalent to floor if value >= 0
    ...
    if (result < 0) {
      ccLog::Error("Failed to determine the max (overlap) distance (not enough "
                   "memory?)");
      return false;
    }
    ...
    ScalarType maxSearchDist = 0;
    {
      ...
      maxSearchDist = distances[static_cast<size_t>(std::max(
    ...
    //(should roughly correspond to 'finalOverlapRatio + margin' percent)
    {
      CCCoreLib::ReferenceCloud *refCloud =
          new CCCoreLib::ReferenceCloud(dataCloud);
      cloudGarbage.add(refCloud);
      unsigned countBefore = dataCloud->size();
      unsigned baseIncrement = static_cast<unsigned>(
          std::max(100.0, countBefore * params.finalOverlapRatio * 0.05));
      ...
      ccLog::Print(QString("[ICP][Partial overlap] Selecting %1 points out of "
                           "%2 (%3%) for registration")
                       .arg(countAfter)
                       .arg(countBefore)
  ...
  params.dataWeights = nullptr;
  {
    if (!modelMesh && useModelSFAsWeights) {
      if (modelCloud ==
              dynamic_cast<CCCoreLib::GenericIndexedCloudPersist *>(model) &&
          model->isA(CC_TYPES::POINT_CLOUD)) {
        ...
        if (!params.modelWeights)
          ccLog::Warning("[ICP] 'useDataSFAsWeights' is true but model has no "
                         "displayed scalar field!");
      } else {
        ccLog::Warning("[ICP] 'useDataSFAsWeights' is true but only point "
    ...
    if (useDataSFAsWeights) {
      if (!dataDisplayedSF) {
        if (dataCloud == ccHObjectCaster::ToPointCloud(data))
          ccLog::Warning("[ICP] 'useDataSFAsWeights' is true but data has no "
                         "displayed scalar field!");
        else
          ccLog::Warning("[ICP] 'useDataSFAsWeights' is true but only point "
                         "cloud scalar fields can be used as weights!");

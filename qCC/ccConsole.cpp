...
ccConsole::ccConsole()
    : m_textDisplay(nullptr), m_parentWidget(nullptr), m_parentWindow(nullptr),
      m_logStream(nullptr) {}
...
void ccConsole::Init(QListWidget *textDisplay /*=nullptr*/,
  ...
  if (s_redirectToStdOut) {
    // make the system console/terminal more responsive by removing any
    // buffering
    setbuf(stdout, NULL);
  }
  ...
  if (textDisplay) {
    ...
    s_showQtMessagesInConsole =
        settings.value("QtMessagesEnabled", false).toBool();
    settings.endGroup();
...
bool ccConsole::autoRefresh() const { return m_timer.isActive(); }
    ...
    if (m_textDisplay || m_logStream) {
      for (auto messagePair : m_queue) {
        ...
        if (m_textDisplay) {
          ...
          } else if ((messagePair.second & LOG_WARNING) ==
                     LOG_WARNING) // Warning
          {
...
bool ccConsole::setLogFile(const QString &filename) {
  ...
  if (!filename.isEmpty()) {
    ...
    if (!m_logFile.open(QFile::Text | QFile::WriteOnly)) {
      return Error(QString("[Console] Failed to open/create log file '%1'")
                       .arg(filename));
    }

...
public:
  ...
  **/
  ...
  void addToDB(ccHObject *obj, bool updateZoom = false,
               bool autoExpandDBTree = true, bool checkDimensions = false,
  ...
  void createGLWindow(ccGLWindowInterface *&window,
  ...
  void spawnHistogramDialog(const std::vector<unsigned> &histoValues,
                            double minVal, double maxVal, QString title,
  ...
  //! Similar to getSelectedEntities, but makes sure no children of another
  //! selected entity is in the output selection
...
private:
  ...
  //! Creates a new 3D GL sub-window (choose whether entity selection is allowed
  //! or not)
  ...
  void doCylindricalNeighbourhoodExtractionTest(); // DGM TODO: remove after
                                                   // test
  ...
  //! Generates a matrix with the best (registration) RMS for all possible
  //! couple among the selected entities
...
private:
  ...
  select an entity:') (optional) \return the selected cloud (or null if the user
  ...
  **/
  ...
  void createComponentsClouds(ccGenericPointCloud *cloud,
                              ...
                              unsigned minPointPerComponent, bool randomColors,
  ...
  void setCenteredPerspectiveView(ccGLWindowInterface *win,
  ...
  **/
  ...
  //! Updates the view mode pop-menu based for a given window (or an absence
  //! of!)
  void updateViewModePopUpMenu(ccGLWindowInterface *win);

  //! Updates the pivot visibility pop-menu based for a given window (or an
  //! absence of!)

...
ccColorLevelsDlg::ccColorLevelsDlg(QWidget *parent,
    ...
    : QDialog(parent, Qt::Tool), Ui::ColorLevelsDialog(), m_histogram(nullptr),
      m_cloud(pointCloud) {
  setupUi(this);

  // connect GUI elements
  connect(channelComboBox, qOverload<int>(&QComboBox::currentIndexChanged),
...
void ccColorLevelsDlg::updateHistogram() {
  if (m_histogram) {
    ...
    std::vector<unsigned> *histoValuesR =
        (histoValues[0].empty() ? nullptr : histoValues);
    std::vector<unsigned> *histoValuesG =
        (histoValues[1].empty() ? nullptr : histoValues + 1);
    std::vector<unsigned> *histoValuesB =
        (histoValues[2].empty() ? nullptr : histoValues + 2);
...
void ccColorLevelsDlg::onApply() {
  ...
  if (m_cloud &&
    ...
    ScaleColorFields(m_cloud, s_inputLevels[0], s_inputLevels[1],
                     s_outputLevels[0], s_outputLevels[1], applyRGB);
...
bool ccColorLevelsDlg::ScaleColorFields(ccGenericPointCloud *cloud,
                                        int inputLevelMin, int inputLevelMax,
                                        int outputLevelMin, int outputLevelMax,
                                        const bool applyRGB[3]) {
  if (!cloud) {
    assert(false);
    return false;
  }
  ...
  if (qIn == 0) {
    ccLog::Warning("(ccColorLevelsDlg::ScaleColorFields] Flat input range "
                   "(input range can't be 0)");
    return false;
  }
  ...
  for (unsigned i = 0; i < pointCount; ++i) {
    ...
    for (unsigned c = 0; c < 3; ++c) {
      if (applyRGB[c]) {
        double newC =
            outputLevelMin +
            (static_cast<int>(col.rgba[c]) - inputLevelMin) * convRatio;
        newRgb.rgba[c] = static_cast<ColorCompType>(
            std::max(std::min(newC, static_cast<double>(ccColor::MAX)), 0.0));
      }
    }

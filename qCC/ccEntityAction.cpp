...
//////////
// Colours
bool setColor(ccHObject::Container selectedEntities, bool colorize,
              QWidget *parent /*=nullptr*/) {
  static QColor s_lastColor = Qt::white;
  QColor colour = QColorDialog::getColor(s_lastColor, parent, QString(),
    ...
    if (ent->isA(CC_TYPES::HIERARCHY_OBJECT)) {
      ...
      if (ent->isA(CC_TYPES::POINT_CLOUD)) {
        ...
        if (!vertices || !vertices->isA(CC_TYPES::POINT_CLOUD) ||
          ...
          ccLog::Warning(QObject::tr("[SetColor] Can't set color for mesh '%1' "
      ...
      if (ent != cloud) {
        ent->showColors(true);
                 ...
                 cloud->getParent()->isKindOf(CC_TYPES::MESH)) {
      ...
      ccLog::Warning(QObject::tr("[SetColor] Can't change color of entity '%1'")
                         .arg(ent->getName()));
    }
  }
...
bool rgbToGreyScale(const ccHObject::Container &selectedEntities) {
  for (ccHObject *ent : selectedEntities) {
    ...
    if (lockedVertices) {
      ccUtils::DisplayLockedVerticesWarning(ent->getName(),
...
bool changeColorLevels(const ccHObject::Container &selectedEntities,
  ...
  if (selectedEntities.size() != 1) {
    ccConsole::Error(
        QObject::tr("Select one and only one colored cloud or mesh!"));
    return false;
  }
...
bool interpolateColors(const ccHObject::Container &selectedEntities,
  ...
  if (!cloud1->hasColors() && !cloud2->hasColors()) {
    ccConsole::Error(QObject::tr(
        "None of the selected entities has per-point or per-vertex colors!"));
    return false;
  } else if (cloud1->hasColors() && cloud2->hasColors()) {
    ccConsole::Error(
        QObject::tr("Both entities have colors! Remove the colors on the "
                    "entity you wish to import the colors to!"));
    return false;
  }
...
bool interpolateSFs(const ccHObject::Container &selectedEntities,
  ...
  if (!cloud1->hasScalarFields() && !cloud2->hasScalarFields()) {
    ...
    ccOrderChoiceDlg ocDlg(cloud1, QObject::tr("Source"), cloud2,
                           QObject::tr("Destination"), app);
    if (!ocDlg.exec()) {
  ...
  try {
    ...
    if (sfCount == 1) {
      ...
      ccItemSelectionDlg isDlg(true, app->getMainWindow(),
                               QObject::tr("entity"));
  ...
  if (ccPointCloudInterpolator::InterpolateScalarFieldsFrom(
    ...
    dest->setCurrentDisplayedScalarField(
        static_cast<int>(
            std::min(sfCountBefore + 1, dest->getNumberOfScalarFields())) -
        1);
    dest->showSF(true);
  } else {
    ccConsole::Error(QObject::tr("An error occurred! (see console)"));
...
//! Interpolate scalar fields from one entity and transfer them to another one
//! without the dialog
bool interpolateSFs(ccPointCloud *source, ccPointCloud *dest, int sfIndex,
                    ccPointCloudInterpolator::Parameters &params,
                    QWidget *parent /*=nullptr*/) {
  if (!source || !dest) {
  ...
  if (!source->hasScalarFields()) {
    ccConsole::Error(QObject::tr("[ccEntityAction::interpolateSFs] The source "
                                 "cloud has no scalar field!"));
    return false;
  }
  ...
  if (!ccPointCloudInterpolator::InterpolateScalarFieldsFrom(
          dest, source, sfIndexes, params, parent ? &pDlg : nullptr)) {
    ccConsole::Error(QObject::tr(
...
bool convertTextureToColor(const ccHObject::Container &selectedEntities,
  ...
  for (ccHObject *ent : selectedEntities) {
    if (ent->isA(
            CC_TYPES::MESH) /*|| ent->isKindOf(CC_TYPES::PRIMITIVE)*/) // TODO
    {
      ...
      if (!mesh->hasMaterials()) {
        ...
        if (mesh->hasColors() &&
            QMessageBox::warning(
                parent, QObject::tr("Mesh already has colors"),
                QObject::tr("Mesh '%1' already has colors! Overwrite them?")
                    .arg(mesh->getName()),
        ...
        if (mesh->convertMaterialsToVertexColors()) {
          ...
          ccLog::Warning(QObject::tr("[ConvertTextureToColor] Failed to "
...
bool enhanceRGBWithIntensities(const ccHObject::Container &selectedEntities,
  ...
  if (QMessageBox::question(
          parent, QObject::tr("Intensity range"),
          QObject::tr("Do you want to define the theoretical intensity range "
                      "(yes)\nor use the actual one (no)?"),
          QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes) {
    ccAskTwoDoubleValuesDlg atdvDlg(
        QObject::tr("Min"), QObject::tr("Max"), -1000000.0, 1000000.0, s_minI,
        s_maxI, 3, QObject::tr("Theroetical intensity"), parent);
    if (!atdvDlg.exec()) {
      // process cancelled by the user
      return false;
    }
  ...
  for (ccHObject *ent : selectedEntities) {
    ...
    if (pc->getNumberOfScalarFields() > 1) {
      ...
      if (sfIdx < 0) {
        ...
        ccPickOneElementDlg poeDlg(QObject::tr("Intensity scalar field"),
                                   QObject::tr("Choose scalar field"), parent);
        ...
        defaultSFName =
            QString::fromStdString(pc->getScalarField(sfIdx)->getName());
      }
    } else {
      sfIdx = 0;
    }
    ...
    if (pc->enhanceRGBWithIntensitySF(sfIdx, useCustomIntensityRange, s_minI,
                                      s_maxI)) {
      ent->prepareDisplayForRefresh();
      ent->showColors(true);
      ent->showSF(false);
    } else {
      ccLog::Warning(QObject::tr("[EnhanceRGBWithIntensities] Failed to apply "
                                 "the process on entity '%1'!")
                         .arg(ent->getName()));
    }
  }
...
bool rgbGaussianFilter(const ccHObject::Container &selectedEntities,
  ...
  if (selectedCloudsWithColors.empty()) {
    if (filterParams.filterType == ccPointCloud::RGB_FILTER_TYPES::BILATERAL)
      ccConsole::Error(QObject::tr("Select at least one cloud or mesh with RGB "
                                   "colors and an active scalar field"));
    else
      ccConsole::Error(
          QObject::tr("Select at least one cloud or mesh with RGB colors"));
    return false;
  }

  double sigmaSF = -1.0;
  if (filterParams.filterType == ccPointCloud::RGB_FILTER_TYPES::BILATERAL) {
    CCCoreLib::ScalarField *sf = selectedCloudsWithColors.front()
  ...
  if (!filterParams.commandLine) {
    ...
    if (filterParams.filterType == ccPointCloud::RGB_FILTER_TYPES::BILATERAL) {
      ccAskThreeDoubleValuesDlg dlg(
          QObject::tr("Spatial sigma"), QObject::tr("Scalar sigma"),
          QObject::tr("Color threshold"), DBL_MIN, 1.0e9, spatialSigma, sigmaSF,
          filterParams.burntOutColorThreshold, 8, nullptr, parent);

      dlg.setWindowTitle(QObject::tr("RGB bilateral filter"));

      dlg.doubleSpinBox1->setStatusTip(
          QObject::tr("3*sigma = 99.7% attenuation"));
      dlg.doubleSpinBox2->setStatusTip(QObject::tr(
          "Scalar sigma controls how much the filter behaves as a Gaussian "
          "Filter\nSigma at +inf uses the whole range of scalars"));
      dlg.doubleSpinBox3->setStatusTip(QObject::tr(
          "For averaging, it will only use colors for which all components are "
          "in the range[threshold:255 - threshold]"));
      if (!dlg.exec()) {
        return false;
      }
      ...
      ccAskTwoDoubleValuesDlg dlg(
          QObject::tr("Spatial sigma"), QObject::tr("Color threshold"), DBL_MIN,
          1.0e9, spatialSigma, filterParams.burntOutColorThreshold, 8, nullptr,
          parent);
      ...
      dlg.doubleSpinBox2->setStatusTip(
          QObject::tr("For averaging, it will only use colors for which all "
  ...
  for (auto entAndPC : selectedCloudsWithColors) {
      ...
      if (filterParams.applyToSFduringRGB) {
        ...
        if (filterParams.filterType ==
          ...
          sfName = QString("%1.bilsmooth(%2,%3)")
                       .arg(QString::fromStdString(outSF->getName()))
                       .arg(spatialSigma)
                       .arg(sigmaSF);
        ...
        if (sfIdx < 0)
          sfIdx = pc->addScalarField(
              sfName.toStdString()); // output SF has same type as input SF
        if (sfIdx >= 0)
          pc->setCurrentInScalarField(sfIdx);
        else {
          ccConsole::Error(QObject::tr("Failed to create scalar field for "
    ...
    if (!octree) {
      ...
      if (!octree) {
        ccConsole::Error(QObject::tr("Couldn't compute octree for cloud '%1'!")
                             .arg(pc->getName()));
        continue;
      }
    }
...
bool sfGaussianFilter(const ccHObject::Container &selectedEntities,
  ...
  double spatialSigma =
      filterParams.spatialSigma == -1
          ? ccLibAlgorithms::GetDefaultCloudKernelSize(selectedEntities)
  ...
  if (!filterParams.commandLine) {
    if (filterParams.filterType == ccPointCloud::RGB_FILTER_TYPES::BILATERAL) {
      ccAskTwoDoubleValuesDlg dlg(
          QObject::tr("Spatial sigma"), QObject::tr("Scalar sigma"), DBL_MIN,
          1.0e9, spatialSigma, scalarFieldSigma, 8, nullptr, parent);
      ...
      spatialSigma = QInputDialog::getDouble(
          parent, QObject::tr("SF gaussian/mean/median filter"),
          "sigma:", spatialSigma, DBL_MIN, 1.0e9, 8, &ok);
      if (!ok) {
        return false;
      }
    }
  ...
  for (ccHObject *ent : selectedEntities) {
    ...
    if (sf) {
      ...
      if (sfIdx >= 0)
        ...
        ccConsole::Error(QObject::tr("Failed to create scalar field for cloud "
                                     "'%1' (not enough memory?)")
                             .arg(pc->getName()));
        return false;
      }
      ...
      if (!octree) {
        ...
        if (!octree) {
          ccConsole::Error(
              QObject::tr("Couldn't compute octree for cloud '%1'!")
                  .arg(pc->getName()));
          return false;
        }
      }
      ...
      if (!CCCoreLib::ScalarFieldTools::applyScalarFieldGaussianFilter(
              static_cast<PointCoordinateType>(spatialSigma), pc,
              static_cast<PointCoordinateType>(scalarFieldSigma),
              parent ? pDlg.data() : nullptr, octree.data())) {
        ccConsole::Warning(QObject::tr(
            "[Bilateral/Gaussian/Mean/Median filter]  Failed to apply filter"));
        return false;
      }

      ccConsole::Print(
          "SF [Bilateral/Gaussian/Mean/Median filter] Timing: %3.2f s.",
      ...
      ccConsole::Warning(QObject::tr("Entity [%1] has no active scalar field!")
                             .arg(pc->getName()));
    }
  }
...
bool sfConvertToRGB(const ccHObject::Container &selectedEntities,
  ...
  QMessageBox::StandardButton answer = QMessageBox::warning(
      parent, QObject::tr("Scalar Field to RGB"),
...
bool sfConvertToRandomRGB(const ccHObject::Container &selectedEntities,
  ...
  bool ok = false;
  s_randomColorsNumber = QInputDialog::getInt(
      parent, QObject::tr("Random colors"),
      QObject::tr("Number of random colors (will be regularly sampled over the "
                  "SF interval):"),
...
bool sfRename(const ccHObject::Container &selectedEntities,
              QWidget *parent /*=nullptr*/) {
  for (ccHObject *ent : selectedEntities) {
    ...
    if (cloud != nullptr) // TODO
    {
      ...
      if (sf == nullptr) {
        ccConsole::Warning(
            QObject::tr("Cloud %1 has no displayed scalar field!")
                .arg(pc->getName()));
      } else {
        const std::string &sfName = sf->getName();
        bool ok = false;
        QString newName = QInputDialog::getText(
            parent, QObject::tr("SF name"), QObject::tr("name:"),
            QLineEdit::Normal,
            QString(!sfName.empty() ? QString::fromStdString(sfName)
                                    : QObject::tr("unknown")),
            &ok);
        if (ok)
...
bool sfAddConstant(ccPointCloud *cloud, QString sfName, bool integerValue,
                   QWidget *parent /*=nullptr*/) {
  if (!cloud) {
    assert(false);
    return false;
  ...
  if (integerValue) {
    ...
    int iValue = QInputDialog::getInt(
        parent, QT_TR_NOOP("Add classification SF"), QT_TR_NOOP("value"),
        s_constantIntSFValue, -1000000, 1000000, 1, &ok);
    if (ok) {
    ...
    double dValue = static_cast<ScalarType>(QInputDialog::getDouble(
        parent, QT_TR_NOOP("Add constant value"), QT_TR_NOOP("value"),
        s_constantDoubleSFValue, -1.0e9, 1.0e9, 8, &ok));
    if (ok) {
  ...
  ccLog::Print(QObject::tr("New scalar field '%1' added to %2 (value = %3)")
                   .arg(sfName)
                   .arg(cloud->getName())
                   .arg(sfValue));
...
bool sfSplitCloud(const ccHObject::Container &selectedEntities,
                  ccMainAppInterface *app) {
  ...
  for (ccHObject *ent : selectedEntities) {
    ...
    if (sf == nullptr) {
      ccLog::Warning(
          QString("Cloud %1 has no active scalar field").arg(cloud->getName()));
      return false;
    }
    ...
    ccLog::Print("[sfSplitCloud] " + QString::number(classes.size()) +
                 " classe(s) found in the current scalar field");

    if (classes.size() == 1) {
      ccLog::Warning(QString("[sfSplitCloud] Cloud %1: SF has only one value")
                         .arg(cloud->getName()));
      continue;
    }

    if (classes.size() > 30 && app && app->getMainWindow() &&
        !tooManyCloudsQuestionAsked) // only in GUI mode
    {
      // ask the user if creating this many clouds is expected
      if (QMessageBox::No ==
          QMessageBox::question(
              app->getMainWindow(), "Too many values",
              QString("Do you confirm that you want to create %1 clouds")
                  .arg(classes.size()),
              QMessageBox::Yes, QMessageBox::No)) {
    ...
    for (int pointClass : classes) {
      ccLog::Print("[sfSplitCloud] build cloud corresponding to class #" +
                   QString::number(pointClass));
      ...
      try {
        ...
        for (unsigned index = 0; index < static_cast<unsigned>(cloud->size());
             index++) {
...
static void SetValueFromSF(PointCoordinateType &value, int fieldIndex,
                           CCCoreLib::ScalarField *sf, unsigned pointIndex,
...
static PointCoordinateType GetDefaultValueForNaN(PointCoordinateType minSFValue,
  ...
  double out = QInputDialog::getDouble(
      ...
      QObject::tr("Enter the coordinate equivalent to NaN values:"), minSFValue,
      -1.0e9, 1.0e9, 6, &ok);
  ...
  if (ok) {
    ...
    ccLog::Warning(
        QObject::tr("[SetSFAsCoord] By default the coordinate equivalent to "
                    "NaN values will be the minimum SF value"));
    return minSFValue;
  }
}
...
bool sfSetAsCoord(ccHObject *entity, QWidget *parent /*=nullptr*/) {
  ...
  CCCoreLib::ScalarField *sfX =
      (xIndex >= 0 ? pc->getScalarField(xIndex) : nullptr);
  CCCoreLib::ScalarField *sfY =
      (yIndex >= 0 ? pc->getScalarField(yIndex) : nullptr);
  CCCoreLib::ScalarField *sfZ =
      (zIndex >= 0 ? pc->getScalarField(zIndex) : nullptr);

  std::array<CCCoreLib::ScalarField *, 3> scalarFields{sfX, sfY, sfZ};

  PointCoordinateType defaultCoordForNaN =
      std::numeric_limits<PointCoordinateType>::quiet_NaN();
  for (CCCoreLib::ScalarField *sf : scalarFields) {
    if (sf) {
      if (sf->countValidValues() < sf->size()) {
        // we have some invalid values, let's ask the user what they should be
        // replaced with
...
bool exportCoordToSF(const ccHObject::Container &selectedEntities,
  ...
  bool exportDims[3]{ectsDlg.exportX(), ectsDlg.exportY(), ectsDlg.exportZ()};
...
bool setSFsAsNormal(ccHObject *entity, QWidget *parent /*=nullptr*/) {
  ...
  CCCoreLib::ScalarField *sfX =
      (nxIndex >= 0 ? cloud->getScalarField(nxIndex) : nullptr);
  CCCoreLib::ScalarField *sfY =
      (nyIndex >= 0 ? cloud->getScalarField(nyIndex) : nullptr);
  CCCoreLib::ScalarField *sfZ =
      (nzIndex >= 0 ? cloud->getScalarField(nzIndex) : nullptr);
  ...
  if (entity != cloud) {
    ...
    if (entity->isKindOf(CC_TYPES::MESH)) {
      static_cast<ccGenericMesh *>(entity)->showTriNorms(
          false); // hide the per-triangle normals (if any)
    }
  }
...
bool exportNormalToSF(const ccHObject::Container &selectedEntities,
  ...
  for (ccHObject *entity : selectedEntities) {
    ...
    if (!pc->hasNormals()) {
      ccLog::Warning(
          QObject::tr("Cloud '%1' has no normals").arg(pc->getName()));
      continue;
    }
...
bool sfArithmetic(const ccHObject::Container &selectedEntities,
  ...
  if (!sfaDlg.apply(cloud)) {
    ccConsole::Error(
        QObject::tr("An error occurred (see Console for more details)"));
  }
...
bool sfFromColor(const ccHObject::Container &selectedEntities,
  ...
  return sfFromColor(selectedEntities, exportR, exportG, exportB, exportAlpha,
                     exportComposite);
}

bool sfFromColor(const ccHObject::Container &selectedEntities, bool exportR,
                 bool exportG, bool exportB, bool exportAlpha,
                 bool exportComposite) {
  // candidates
  ...
  for (const auto cloud : clouds) {
    ...
    fields[0] =
        (exportR ? new ccScalarField(
                       GetFirstAvailableSFName(cloud, "R").toStdString())
                 : nullptr);
    fields[1] =
        (exportG ? new ccScalarField(
                       GetFirstAvailableSFName(cloud, "G").toStdString())
                 : nullptr);
    fields[2] =
        (exportB ? new ccScalarField(
                       GetFirstAvailableSFName(cloud, "B").toStdString())
                 : nullptr);
    fields[3] =
        (exportAlpha
             ? new ccScalarField(
                   GetFirstAvailableSFName(cloud, "Alpha").toStdString())
             : nullptr);
    fields[4] =
        (exportComposite
             ? new ccScalarField(
                   GetFirstAvailableSFName(cloud, "Composite").toStdString())
             : nullptr);
    ...
    for (ccScalarField *&sf : fields) {
      if (sf && !sf->reserveSafe(count)) {
        ccLog::Warning(
            QObject::tr("[SfFromColor] Not enough memory to instantiate SF "
                        "'%1' on cloud '%2'")
                .arg(QString::fromStdString(sf->getName()), cloud->getName()));
        sf->release();
        sf = nullptr;
      }
    }
    ...
    for (unsigned j = 0; j < cloud->size(); ++j) {
      ...
      if (fields[4])
        fields[4]->addElement(static_cast<ScalarType>(col.r + col.g + col.b) /
                              3);
    }
    ...
    if (!fieldsStr.isEmpty())
      ccLog::Print(
          QObject::tr("[SfFromColor] New scalar fields (%1) added to '%2'")
...
bool processMeshSF(const ccHObject::Container &selectedEntities,
  ...
  for (ccHObject *ent : selectedEntities) {
    if (ent->isKindOf(CC_TYPES::MESH) ||
        ent->isKindOf(CC_TYPES::PRIMITIVE)) // TODO
    {
      ccMesh *mesh = ccHObjectCaster::ToMesh(ent);
      if (mesh == nullptr)
        continue;
      ...
      if (cloud->isA(CC_TYPES::POINT_CLOUD)) // TODO
      {
        ...
        if (sfIdx >= 0) {
          ...
          ccConsole::Warning(
              QObject::tr("Mesh [%1] vertices have no activated scalar field!")
                  .arg(mesh->getName()));
        }
      }
    }
...
bool computeNormals(const ccHObject::Container &selectedEntities,
    ...
    for (const auto entity : selectedEntities) {
      if (entity->isA(CC_TYPES::POINT_CLOUD)) {
        ...
        if (cloud->hasMetaData(s_NormalScaleKey)) {
          bool ok = false;
          double formerRadius =
              cloud->getMetaData(s_NormalScaleKey).toDouble(&ok);
          if (ok) {
            // remember the largest radius
            defaultRadius = std::max(
                defaultRadius, static_cast<PointCoordinateType>(formerRadius));
          } else {
            assert(false);
          }
        }
        ...
        if (entity->isA(CC_TYPES::MESH)) {
          ...
          ccConsole::Error(QObject::tr("Can't compute normals on sub-meshes! "
  ...
  if (!clouds.empty()) {
    ...
    // normals computation
    CCCoreLib::LOCAL_MODEL_TYPES model = s_lastModelType =
        ncDlg.getLocalModel();
    ...
    for (auto cloud : clouds) {
      ...
      if (useGridStructure && cloud->gridCount()) {
        ...
        result = cloud->computeNormalsWithOctree(
            model,
            s_orientNormals ? preferredOrientation : ccNormalVectors::UNDEFINED,
            defaultRadius, &pDlg);
        if (result) {
      ...
      // do we need to orient the normals? (this may have been already done if
      // 'orientNormalsForThisCloud' is true)
      if (result && s_orientNormals && !normalsAlreadyOriented) {
        if (cloud->gridCount() && orientNormalsWithGrids) {
          ...
          // some points can belong to one sensor and some others can belongs to
          // others sensors. so it's why here grid orientation has precedence
          // over sensor orientation because in this case association is more
          // explicit. Here we take the first valid viewpoint for now even if
          // it's not a good one...
          for (unsigned i = 0; i < cloud->getChildrenNumber(); ++i) {
            ...
            if (child && child->isKindOf(CC_TYPES::SENSOR)) {
              ...
              if (sensor->getActiveAbsoluteCenter(sensorPosition)) {
                result =
                    cloud->orientNormalsTowardViewPoint(sensorPosition, &pDlg);
                break;
              }
            }
          }
    ...
    if (errors != 0) {
      if (errors < clouds.size())
        ccConsole::Error(QObject::tr("Failed to compute or orient the normals "
                                     "on some clouds! (see console)"));
      else
        ccConsole::Error(QObject::tr(
  ...
  if (!meshes.empty()) {
    QMessageBox question(
        QMessageBox::Question, QObject::tr("Mesh normals"),
        QObject::tr(
            "Compute per-vertex normals (smooth) or per-triangle (faceted)?"),
        QMessageBox::NoButton, parent);

    QPushButton *perVertexButton =
        question.addButton(QObject::tr("Per-vertex"), QMessageBox::YesRole);
    QPushButton *perTriangleButton =
        question.addButton(QObject::tr("Per-triangle"), QMessageBox::NoRole);
    ...
    for (auto mesh : meshes) {
      ...
      // we remove temporarily the mesh as its normals may be removed (and they
      // can be a child object)
      ...
      if (!result) {
        ccConsole::Error(QObject::tr("Failed to compute normals on mesh '%1'")
                             .arg(mesh->getName()));
        continue;
      }
      mesh->prepareDisplayForRefresh_recursive();
...
bool orientNormalsFM(const ccHObject::Container &selectedEntities,
  ...
  int value = QInputDialog::getInt(
      parent, QObject::tr("Orient normals (FM)"), QObject::tr("Octree level"),
      s_defaultLevel, 1, CCCoreLib::DgmOctree::MAX_OCTREE_LEVEL, 1, &ok);
  if (!ok)
    return false;
  ...
  for (ccHObject *entity : selectedEntities) {
    ...
    if (!cloud->hasNormals()) {
      ccConsole::Warning(
          QObject::tr("Cloud '%1' has no normals!").arg(cloud->getName()));
      continue;
    }
  ...
  if (errors) {
    ...
    ccLog::Warning(QObject::tr(
        "Normals have been oriented: you may still have to globally invert the "
        "cloud normals however (Edit > Normals > Invert)."));
  }
...
bool orientNormalsMST(const ccHObject::Container &selectedEntities,
  ...
  unsigned kNN = static_cast<unsigned>(QInputDialog::getInt(
      parent, QObject::tr("Neighborhood size"), QObject::tr("Neighbors"),
  ...
  for (ccHObject *entity : selectedEntities) {
    ...
    if (cloud->orientNormalsWithMST(kNN, &pDlg)) {
      ...
      ccConsole::Warning(
          QObject::tr("Process failed on cloud '%1'").arg(cloud->getName()));
      ++errors;
    }
  }
    ...
    if (cloud && cloud->isA(CC_TYPES::POINT_CLOUD)) // TODO
    {
      ...
      if (ccCloud->hasNormals()) {
        ...
        case NORMAL_CONVERSION_DEST::DIP_DIR_SFS: {
          ...
          int dipSFIndex =
              ccCloud->getScalarFieldIndexByName(CC_DEFAULT_DIP_SF_NAME);
          if (dipSFIndex < 0)
          ...
          int dipDirSFIndex =
              ccCloud->getScalarFieldIndexByName(CC_DEFAULT_DIP_DIR_SF_NAME);
          if (dipDirSFIndex < 0)
          ...
          ccScalarField *dipSF =
              static_cast<ccScalarField *>(ccCloud->getScalarField(dipSFIndex));
        ...
        default:
          Q_ASSERT(false);
          ccLog::Warning(QObject::tr("[ccEntityAction::convertNormalsTo] "
                                     "Internal error: unhandled destination!"));
          success = false;
  ...
  for (const auto cloud : clouds) {
    ...
    case ccComputeOctreeDlg::CUSTOM_BBOX: {
      ...
      if (coDlg.getMode() == ccComputeOctreeDlg::MIN_CELL_SIZE) {
        ...
        PointCoordinateType halfBoxWidth = static_cast<PointCoordinateType>(
            cellSize * (1 << ccOctree::MAX_OCTREE_LEVEL) / 2.0);
      ...
      if (octree->build(bbox.minCorner(), bbox.maxCorner(), nullptr, nullptr,
                        &pDlg) > 0) {
    ...
    if (octree) {
      ccConsole::Print("[doActionComputeOctree] Timing: %2.3f s",
      ...
      ccConsole::Warning(QObject::tr("Octree computation on cloud '%1' failed!")
                             .arg(cloud->getName()));
    }
  }
...
bool clearProperty(ccHObject::Container selectedEntities,
                   CLEAR_PROPERTY property, QWidget *parent /*=nullptr*/) {
  for (ccHObject *ent : selectedEntities) {
            ...
            CC_TYPES::MESH) /*|| ent->isKindOf(CC_TYPES::PRIMITIVE)*/)) // TODO
    {
      ...
      if (mesh->hasTriNormals()) {
        ...
        ccMainAppInterface *instance =
            dynamic_cast<ccMainAppInterface *>(parent);
        ccMainAppInterface::ccHObjectContext objContext;
        if (instance)
          objContext = instance->removeObjectTemporarilyFromDBTree(mesh);
        mesh->clearTriNormals();
      ...
      } else if (mesh->hasNormals()) // per-vertex normals?
      {
        if (mesh->getParent()
					...
					{
          ccLog::Warning(QObject::tr("[DoActionClearNormals] Can't remove "
                                     "normals per-vertex on a sub mesh!"));
      ...
      case CLEAR_PROPERTY::CURRENT_SCALAR_FIELD:
        if (cloud->hasDisplayedScalarField()) {
          pointCloud->deleteScalarField(
              pointCloud->getCurrentDisplayedScalarFieldIndex());
          ent->prepareDisplayForRefresh();
        }
        break;
...
bool statisticalTest(const ccHObject::Container &selectedEntities,
                     QWidget *parent /*=nullptr*/) {
  ccPickOneElementDlg poeDlg(QObject::tr("Distribution"),
                             QObject::tr("Choose distribution"), parent);
  ...
  switch (distribIndex) {
  case 0: // Gauss
    sDlg = new ccStatisticalTestDlg(
        "mu", "sigma", QString(), QObject::tr("Local Statistical Test (Gauss)"),
        parent);
    break;
  case 1: // Weibull
    sDlg = new ccStatisticalTestDlg(
        "a", "b", "shift", QObject::tr("Local Statistical Test (Weibull)"),
        parent);
    break;
  default:
  ...
  for (ccHObject *ent : selectedEntities) {
    ...
    // necessary)
    int chi2SfIdx =
        pc->getScalarFieldIndexByName(CC_CHI2_DISTANCES_DEFAULT_SF_NAME);
    ...
    if (chi2SfIdx < 0) {
      ccConsole::Error(
          QObject::tr("Couldn't allocate a new scalar field for computing chi2 "
    ...
    double chi2dist =
        CCCoreLib::StatisticalTestingTools::testCloudWithStatisticalModel(
            distrib, pc, nn, pChi2, &pDlg, theOctree.data());

    ccConsole::Print("[Chi2 Test] Timing: %3.2f ms.",
                     eTimer.elapsed() / 1000.0);
    ccConsole::Print(QObject::tr("[Chi2 Test] %1 test result = %2")
                         .arg(distrib->getName())
                         .arg(chi2dist));

    // we set the theoretical Chi2 distance limit as the minimum displayed SF
    // value so that all points below are grayed
    {
...
bool computeStatParams(const ccHObject::Container &selectedEntities,
                       QWidget *parent /*=nullptr*/) {
  ccPickOneElementDlg pDlg(QObject::tr("Distribution"),
                           QObject::tr("Distribution Fitting"), parent);
  ...
  for (ccHObject *ent : selectedEntities) {
    ...
    if (sfValidCount == 0) {
      ccLog::Warning(
          QObject::tr("Scalar field '%1' of cloud %2 has no valid values")
    ...
    if (sf && distrib->computeParameters(
                  CCCoreLib::GenericDistribution::SFAsScalarContainer(*sf))) {
      QString description;
      const unsigned precision = ccGui::Parameters().displayedNumPrecision;
      switch (pDlg.getSelectedIndex()) {
      case 0: // GAUSS
      {
        CCCoreLib::NormalDistribution *normal =
            static_cast<CCCoreLib::NormalDistribution *>(distrib);
        description = QObject::tr("mean = %1 / std.dev. = %2")
                          .arg(normal->getMu(), 0, 'f', precision)
                          .arg(sqrt(normal->getSigma2()), 0, 'f', precision);
      } break;

      case 1: // WEIBULL
      {
        CCCoreLib::WeibullDistribution *weibull =
            static_cast<CCCoreLib::WeibullDistribution *>(distrib);
        ScalarType a;
        ScalarType b;
        ...
        description = QString("a = %1 / b = %2 / shift = %3")
                          .arg(a, 0, 'f', precision)
                          .arg(b, 0, 'f', precision)
                          .arg(weibull->getValueShift(), 0, 'f', precision);
        ccLog::Print(
            QObject::tr("[Distribution fitting] Additional Weibull distrib. "
                        "parameters: mode = %1 / skewness = %2")
      ...
      ccConsole::Print(
          QObject::tr("[Distribution fitting] %1").arg(description));

      const unsigned numberOfClasses =
          static_cast<unsigned>(ceil(sqrt(static_cast<double>(sfValidCount))));
      ...
      try {
        ...
        ccConsole::Warning(
            QObject::tr("[Distribution fitting] Not enough memory!"));
        continue;
      }
      ...
      // compute the Chi2 distance
      {
        ...
        const double chi2dist =
            CCCoreLib::StatisticalTestingTools::computeAdaptativeChi2Dist(
                distrib, pc, numberOfClasses, finalNumberOfClasses, false,
                nullptr, nullptr, histo.data(), npis.data());

        if (chi2dist >= 0.0) {
          ccConsole::Print(
              QObject::tr("[Distribution fitting] %1: Chi2 Distance = %2")
                  .arg(distrib->getName())
                  .arg(chi2dist));
        } else {
          ccConsole::Warning(QObject::tr(
      ...
      // compute RMS
      {
        ...
        ccConsole::Print(
            QObject::tr("Number of valid values = %1 / %2 (%3%)")
                .arg(sfValidCount)
                .arg(pc->size())
                .arg((100.0 * sfValidCount) / pc->size(), 0, 'f', 2));
        ccConsole::Print(QObject::tr("Sum of all valid values = %1")
                             .arg(QString::number(sum, 'f', 6)));
        ccConsole::Print(QObject::tr("Sum of all valid squared values = %1")
                             .arg(QString::number(squareSum, 'f', 6)));
        ccConsole::Print(
            QObject::tr("Average value = %1").arg(sum / sfValidCount));
      ...
      ccConsole::Warning(
          QObject::tr(
              ...
              .arg(pc->getName(), QString::fromStdString(sf->getName())));
    }
  }

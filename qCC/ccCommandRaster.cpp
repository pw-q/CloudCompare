...
static bool ReadProjectionType(ccCommandLineInterface &cmd,
                               ccRasterGrid::ProjectionType &projType,
  ...
  if (option == COMMAND_RASTER_PROJ_MIN) {
    ...
    if (cmd.arguments().size() != 0) {
      ...
      cmd.error(
          QString("Expecting the std. dev. SF index after %1").arg(option));
      return false;
    }
  } else {
    assert(false);
...
static ccRasterGrid::EmptyCellFillOption
GetEmptyCellFillingStrategy(QString option, ccCommandLineInterface &cmd) {
...
bool CommandRasterize::process(ccCommandLineInterface &cmd) {
  ...
  while (!cmd.arguments().empty()) {
    ...
    if (ccCommandLineInterface::IsCommand(argument,
      ...
      if (outputMesh) {
        cmd.warning(
            "Can't output the grid as a mesh AND a cloud at the same time");
      } else {
        outputCloud = true;
      }
      ...
      outputRasterSFs = false;
                   ...
                   argument, COMMAND_GRID_OUTPUT_RASTER_Z_AND_SF)) {
      // local option confirmed, we can move on
      cmd.arguments().pop_front();
      ...
      if (!ok) {
        return cmd.error(QString("Invalid custom height value! (after %1)")
      ...
      if (!ReadProjectionType(cmd, sfProjectionType, sfProjStdDevSFDesc)) {
        return false;
                   ...
                   argument, COMMAND_RASTER_INTERP_MAX_EDGE_LENGTH)) {
      // local option confirmed, we can move on
      cmd.arguments().pop_front();
      ...
      if (!ok || dInterpParams.maxEdgeLength < 0.0) {
        return cmd.error(QString("Invalid max edge length value! (after %1)")
                             .arg(COMMAND_RASTER_INTERP_MAX_EDGE_LENGTH));
      }
    } else if (ccCommandLineInterface::IsCommand(
                   argument, COMMAND_RASTER_FILL_KRIGING_KNN)) {
      // local option confirmed, we can move on
      cmd.arguments().pop_front();
      ...
      if (!ok || krigingParams.kNN <= 0) {
        return cmd.error(QString("Invalid Kriging knn value! (after %1)")
                             .arg(COMMAND_RASTER_FILL_KRIGING_KNN));
      }
    } else if (ccCommandLineInterface::IsCommand(argument,
  ...
  if (!projStdDevSFDesc.isEmpty()) {
    if (!sfProjStdDevSFDesc.isEmpty() &&
        projStdDevSFDesc != sfProjStdDevSFDesc) {
      cmd.warning(
          "[Rasterize] Can't set 2 different std. dev. SF for inverse variance "
          "projection modes. The point projection SF will be used by default.");
    }
  ...
  if (gridStep == 0) {
    return cmd.error(
        QString("Grid step value not defined (use %1)").arg(COMMAND_GRID_STEP));
  }

  if (std::isnan(customHeight)) {
    if (emptyCellFillStrategy == ccRasterGrid::FILL_CUSTOM_HEIGHT) {
      cmd.warning("[Rasterize] The filling stragety is set to 'fill with "
                  "custom height' but no custom height was defined...");
      emptyCellFillStrategy = ccRasterGrid::LEAVE_EMPTY;
    } else if (emptyCellFillStrategy == ccRasterGrid::INTERPOLATE_DELAUNAY) {
      cmd.warning("[Rasterize] The filling stragety is set to 'Delaunay' but "
                  "no custom height was defined. Some holes may remain "
  ...
  if (resample && !outputCloud && !outputMesh) {
    cmd.warning("[Rasterize] The 'resample' option is set while the raster "
  ...
  for (CLCloudDesc &cloudDesc : cmd.clouds()) {
    ...
    if (projectionType == ccRasterGrid::PROJ_INVERSE_VAR_VALUE) {
      ...
      invVarProjSFIndex =
          cloudDesc.pc->getScalarFieldIndexByName(stdDevSFDesc.toStdString());
      if (invVarProjSFIndex < 0) {
        ...
        if (!validValue) {
          return cmd.error(
              QString(
                  "[Rasterize] Failed to recognize the std. dev. SF '%1' "
                  "(neither an existing scalar field name nor a valid index)")
                  .arg(stdDevSFDesc));
        } else if (invVarProjSFIndex < 0 ||
                   static_cast<unsigned>(invVarProjSFIndex) >=
                       cloudDesc.pc->getNumberOfScalarFields()) {
          return cmd.error(
              "[Rasterize] Invalid std. dev. SF index (negative or greater "
              "than the number of scalar fields in the cloud");
        }
      }
    }
    ...
    if (!ccRasterGrid::ComputeGridSize(vertDir, gridBBox, gridStep, gridWidth,
                                       gridHeight)) {
      return cmd.error("Failed to compute the grid dimensions (check input "
      ...
      if (cmd.silentMode()) {
        ...
        if (s_firstTime &&
            QMessageBox::warning(
                ...
                "Grid size is huge. Are you sure you want to proceed?\n(you "
                "can avoid this message by running in SILENT mode)",
    ...
    ccRasterGrid grid;
    {
      ...
      if (grid.fillWith(cloudDesc.pc, vertDir, projectionType,
                        ...
                        sfProjectionType, pDlg.data(), invVarProjSFIndex)) {
        grid.fillEmptyCells(emptyCellFillStrategy, customHeight);
        cmd.print(
            QString(
                "[Rasterize] Raster grid: size: %1 x %2 / heights: [%3 ; %4]")
                .arg(grid.width)
                .arg(grid.height)
                .arg(grid.minHeight)
                .arg(grid.maxHeight));
    ...
    if (outputCloud || outputMesh) {
      ...
      try {
        ...
        rasterCloud = grid.convertToCloud(
            true, false, exportedStatistics, true, true, resample, resample,
            cloudDesc.pc, vertDir, gridBBox, 0.0, true, true, nullptr);
      ...
      if (outputCloud) {
        ...
        if (baseMesh) {
          ...
          cmd.print(QString("[Rasterize] Mesh '%1' successfully generated")
          ...
          cmd.warning(
              QStringLiteral("[Rasterize] Failed to create output mesh ('%1')")
    ...
    if (outputRasterZ) {
      ...
      QString exportFilename = cmd.getExportFilename(
          cloudDesc, "tif", outputRasterSFs ? "RASTER_Z_AND_SF" : "RASTER_Z",
          nullptr, !cmd.addTimestamp());
      if (exportFilename.isEmpty()) {
        exportFilename = "rasterZ.tif";
      }

      ccRasterizeTool::ExportGeoTiff(exportFilename, bands,
                                     emptyCellFillStrategy, grid, gridBBox,
                                     vertDir, customHeight, cloudDesc.pc);
    }
    ...
    if (outputRasterRGB) {
      ...
      QString exportFilename = cmd.getExportFilename(
          cloudDesc, "tif", "RASTER_RGB", nullptr, !cmd.addTimestamp());
      if (exportFilename.isEmpty()) {
...
bool CommandVolume25D::process(ccCommandLineInterface &cmd) {
  ...
  while (!cmd.arguments().empty()) {
    ...
    if (ccCommandLineInterface::IsCommand(argument,
      ...
      if (!ok) {
        return cmd.error(QString("Invalid const. height value! (after %1)")
  ...
  CLCloudDesc *ceil = nullptr;
  {
    ...
    if (index != expectedCount) {
      return cmd.error(
          QString("Not enough loaded entities (%1 found, %2 expected)")
              .arg(index)
              .arg(expectedCount));
    }
  ...
  if (gridWidth * gridHeight > (1 << 26)) // 64 million of cells
  {
    if (cmd.silentMode()) {
      ...
      if (s_firstTime &&
          QMessageBox::warning(
              cmd.widgetParent(), "Volume grid",
              "Grid size is huge. Are you sure you want to proceed?\n(you can "
              "avoid this message by running in SILENT mode)",
              QMessageBox::Yes, QMessageBox::No) == QMessageBox::No) {
        return ccLog::Warning("Process cancelled");
      }
      s_firstTime = false;
  ...
  if (ccVolumeCalcTool::ComputeVolume(
          ...
          nullptr, gridBBox, vertDir, gridStep, gridWidth, gridHeight,
          ccRasterGrid::PROJ_AVERAGE_VALUE, ccRasterGrid::LEAVE_EMPTY, 0.0,
          ccRasterGrid::LEAVE_EMPTY, 0.0, reportInfo, constHeight, constHeight,
    ...
    // save repot in a separate text file
    {
      ...
      if (cmd.addTimestamp())
        txtFilename += QString("_%1").arg(
            QDateTime::currentDateTime().toString("yyyy-MM-dd_hh'h'mm"));
    ...
    // generate the result entity (cloud by default)
    {
      ccPointCloud *rasterCloud =
          ccVolumeCalcTool::ConvertGridToCloud(grid, gridBBox, vertDir, true);
      if (!rasterCloud) {
      ...
      if (outputMesh) {
        ...
        if (rasterMesh) {
          ...
          cmd.print(QString("[Volume] Mesh '%1' successfully generated")
                        .arg(rasterMesh->getName()));
        } else {
          delete rasterCloud;
          return cmd.error(
              QStringLiteral("[Voume] Failed to create output mesh ('%1')")
                  .arg(QString::fromStdString(errorStr)));
        }
      }
      ...
      if (outputDesc && cmd.autoSaveMode()) {
        ...
        QString errorStr =
            cmd.exportEntity(*outputDesc, "HEIGHT_DIFFERENCE", &outputFilename);
        if (!errorStr.isEmpty())

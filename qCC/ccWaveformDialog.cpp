...
ccWaveWidget::ccWaveWidget(QWidget *parent /*=nullptr*/)
    : QCustomPlot(parent), m_titlePlot(nullptr), m_curve(nullptr), m_dt(0.0),
      m_minA(0.0), m_maxA(0.0), m_echoPos(-1.0), m_vertBar(nullptr),
      m_drawVerticalIndicator(false), m_verticalIndicatorPositionPercent(0.0),
      m_peakBar(nullptr), m_lastMouseClick(0, 0) {
...
void ccWaveWidget::refresh() {
  ...
  if (m_drawVerticalIndicator) // vertical hint
  {
    ...
    // horizontal position
    int curvePos =
        static_cast<int>(curveSize * m_verticalIndicatorPositionPercent);
    ...
    valueStr =
        QString("= %0").arg(curvePos < curveSize ? m_curveValues[curvePos] : 0);
...
ccWaveDialog::ccWaveDialog(ccPointCloud *cloud, ccPickingHub *pickingHub,
                           QWidget *parent /*=nullptr*/)
    : QDialog(parent, Qt::WindowMaximizeButtonHint | Qt::WindowCloseButtonHint),
      m_cloud(cloud), m_widget(new ccWaveWidget(this)),
      m_pickingHub(pickingHub), m_gui(new Ui_WaveDialog), m_waveMax(0),
  ...
  if (cloud && cloud->size()) {
    ...
    if (cloud->computeFWFAmplitude(waveMin, m_waveMax, &pDlg)) {
      ccLog::Print(QString("[ccWaveDialog] Cloud '%1': max FWF amplitude = %2")
                       .arg(cloud->getName())
                       .arg(m_waveMax));
    } else {
  ...
  connect(m_gui->pointIndexSpinBox, qOverload<int>(&QSpinBox::valueChanged),
          this, &ccWaveDialog::onPointIndexChanged);
...
void ccWaveDialog::onPointIndexChanged(int index) {
  ...
  m_widget->init(m_cloud, static_cast<unsigned>(index),
...
void ccWaveDialog::add2DLabel(ccPointCloud *cloud, unsigned pointIndex) {
  ...
  m_label->setPosition(
      static_cast<float>(Y.x + 20) / m_display->getScreenSize().width(),
...
void ccWaveDialog::onPointPickingButtonToggled(bool state) {
  ...
  if (state) {
    if (!m_pickingHub->addListener(this)) {
      ccLog::Error("Another tool is currently using the point picking "
